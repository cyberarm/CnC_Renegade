...
enum
{
    ...
    STATICAABTREE_CHUNK_AABTREE_CLASS_DATA
    = 0x00000104, // wraps the AABTreeCullSystemClass's save data
};
...
void StaticAABTreeCullClass::Collect_Visible_Objects(const FrustumClass& frustum,
                                                     VisTableClass* pvs,
    ...
    if (node->Object) {
        ...
        while (obj) {
            ...
            if ((context.PVS.Get_Bit(obj->Get_Vis_Object_ID()) != 0)
                && (CollisionMath::Overlap_Test(context.Frustum, obj->Get_Cull_Box(),
...
void StaticAABTreeCullClass::Evaluate_Occluder_Visibility(VisRenderContextClass& context,
...
void StaticAABTreeCullClass::Evaluate_Non_Occluder_Visibility(VisRenderContextClass& context,
                                                              VisSampleClass& sample)
{
    /*
        ...
        for (it.First(); !it.Is_Done(); it.Next()) {
            ...
            */
            if (!context.Camera.Cull_Box(obj->Get_Bounding_Box())
                ...
                if (context.VisRasterizer->Get_Pixel_Counter()
                    > 0) { // if any pixels passed, the obj is visible!
                    context.VisTable.Set_Bit(obj->Get_Vis_Object_ID(), true);
...
void StaticAABTreeCullClass::Render_Occluders(AABTreeNodeClass* node,
...
void StaticAABTreeCullClass::Collect_Non_Occluders(AABTreeNodeClass* node,
                                                   VisRenderContextClass& context,
...
void StaticAABTreeCullClass::Propogate_Hierarchical_Visibility_Recursive(AABTreeNodeClass* node,
                                                                         VisTableClass* pvs)
{
    /*
