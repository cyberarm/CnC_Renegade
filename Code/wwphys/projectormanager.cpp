...
TextureClass* create_projector_texture_from_filename(const char* filename)
{
    ...
    TextureClass* texture
        = WW3DAssetManager::Get_Instance()->Get_Texture(tex_name, TextureClass::MIP_LEVELS_1);
    if (texture == nullptr) {
...
void ProjectorManagerClass::Init(const ProjectorManagerDefClass& def, RenderObjClass* model)
{
    ...
    if (def.IsEnabled) {
        ...
        */
        ...
        if (Projector != nullptr) {
            ...
            */
            if (def.IsPerspective) {
                Projector->Set_Perspective_Projection(def.HorizontalFOV, def.VerticalFOV, def.NearZ,
                                                      def.FarZ);
            }
            else {
        ...
        /*
        ** If the projector is animated, resolve the bone name into a bone index and install it into
        *the scene
...
{
    if ((Projector != nullptr) && (ProjectorBoneIndex != -1)) {
        ...
        case PROJECTORMANAGERDEF_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, PROJECTORMANAGERDEF_VARIABLE_ISPERSPECTIVE,
                                     IsPerspective);
                    ...
                    READ_MICRO_CHUNK(cload, PROJECTORMANAGERDEF_VARIABLE_HORIZONTALFOV,
                                     HorizontalFOV);
                    ...
                    READ_MICRO_CHUNK_WWSTRING(cload, PROJECTORMANAGERDEF_VARIABLE_TEXTURENAME,
                                              TextureName);
                    READ_MICRO_CHUNK_WWSTRING(cload, PROJECTORMANAGERDEF_VARIABLE_BONENAME,
                                              BoneName);
