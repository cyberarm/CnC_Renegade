...
const float CONTACT_DAMPING_FACTOR
    = 0.33f; // ratio of damping / (critical damping) (less than zero is underdamped)
...
bool OctBoxClass::Is_Intersecting(NonRefPhysListClass* result_list, bool check_static_objs,
                                  bool check_dyn_objs)
{
...
void OctBoxClass::Compute_Octant_Contact(int oi, bool lock_to_centroids)
{
    ...
    */
    ...
    PhysOBBoxCollisionTestClass boxtest(octbox, corner_move, &octant_result,
    ...
    */
    if ((boxtest.CollidedPhysObj != nullptr)
        && (boxtest.CollidedPhysObj->As_RigidBodyClass() != nullptr)) {
    ...
    */
    ...
    if ((corner_result.Fraction < 1.0f)
        && (corner_result.Fraction - CORNER_BIAS < octant_result.Fraction)) {

        if (corner_result.Normal.Length2()
            > 0.0f) { // the normal will be 0,0,0 when we hit a backside
        ...
        if ((octant_result.Fraction < 1.0f) && (!octant_result.StartBad)) {
            if (octant_result.Normal.Length2()
                > 0.0f) { // the normal will be 0,0,0 when we hit a backside
