...
//////////////////////////////////////////////////////////////////////
SimplePersistFactoryClass<RenegadeTerrainPatchClass, WW3D_PERSIST_CHUNKID_RENEGADE_TERRAIN>
    _TerrainPatchFactory;
    ...
    */
    ...
    for (int index = 0; index < MaterialPassList.Count(); index++) {
        ...
        DynamicVectorClass<int>& vert_list
            = MaterialPassList[index]
                  ->VertexRenderList[RenegadeTerrainMaterialPassClass::PASS_BASE];
        DynamicVectorClass<int>& quad_list
            = MaterialPassList[index]->QuadList[RenegadeTerrainMaterialPassClass::PASS_BASE];
...
void RenegadeTerrainPatchClass::Render_By_Texture(int texture_index, int pass_type)
{
    ...
    //
    DynamicVectorClass<int>& vert_list
        = MaterialPassList[texture_index]->VertexRenderList[pass_type];
...
void RenegadeTerrainPatchClass::Build_Rendering_Buffers(int texture_index, int pass_type)
{
    ...
    material_pass->VertexBuffers[pass_type]
        = new DX8VertexBufferClass(DX8_FVF_XYZNDUV1, vert_count);
...

    {
        ...
        for (int index = 0; index < vert_count; index++) {
            ...
            if (pass_type == RenegadeTerrainMaterialPassClass::PASS_BASE) {
                ...
                vertices[index].diffuse
                    = DX8Wrapper::Convert_Color(VertexColors[vert_index], alpha);
            }
        }
    }
...
bool RenegadeTerrainPatchClass::Cast_OBBox(OBBoxCollisionTestClass& boxtest)
{
    ...
    //
    OBBoxCollisionTestClass obj_space_test(obj_space_box, obj_space_move, boxtest.Result,
                                           boxtest.CollisionType);
    ...
    for (int y_pos = min_y; y_pos <= max_y; y_pos++) {
        for (int x_pos = min_x; x_pos <= max_x; x_pos++) {
            ...
            retval |= CollisionMath::Collide(obj_space_test.Box, obj_space_test.Move, tri1,
                                             Vector3(0, 0, 0), obj_space_test.Result);
            retval |= CollisionMath::Collide(obj_space_test.Box, obj_space_test.Move, tri2,
                                             Vector3(0, 0, 0), obj_space_test.Result);
    ...
    if (retval) {
        ...
        if (MaterialPassList.Count() > 0) {
            ...
            if (MaterialPassList[best_pass]->Material != NULL) {
                boxtest.Result->SurfaceType
                    = MaterialPassList[best_pass]->Material->Get_Surface_Type();
            }
        }
...
bool RenegadeTerrainPatchClass::Cast_Vertical_Ray(RayCollisionTestClass& raytest)
{
    ...
    //
    if (p0.X >= BoundingBoxMin.X && p0.Y >= BoundingBoxMin.Y && p0.X <= BoundingBoxMax.X
        && p0.Y <= BoundingBoxMax.Y) {
...
//
//////////////////////////////////////////////////////////////////////
const PersistFactoryClass& RenegadeTerrainPatchClass::Get_Factory(void) const
{
...
void RenegadeTerrainPatchClass::Update_UVs(void)
{
    for (int y_pos = 0; y_pos < GridPointsY; y_pos++) {
        ...
        for (int x_pos = 0; x_pos < GridPointsX; x_pos++) {
            ...
            for (int index = 0; index < MaterialPassList.Count(); index++) {
                if (MaterialPassList[index]->Material != NULL) {
                    float meters_per_texture
                        = MaterialPassList[index]->Material->Get_Meters_Per_Tile();
...
void RenegadeTerrainPatchClass::Update_Vertex_Render_Lists(void)
{
    ...
    for (int index = 0; index < MaterialPassList.Count(); index++) {
        ...
        for (int pass = 0; pass < RenegadeTerrainMaterialPassClass::PASS_COUNT; pass++) {
            ...
            //
            for (int quad_index = 0; quad_index < material_pass->QuadList[pass].Count();
                 quad_index++) {
