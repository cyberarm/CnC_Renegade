...
const char* WHEELT_BONE_NAME
    = "WheelT"; // Translation constraint bone (e.g. front wheel of a motorcycle)
...
void VehiclePhysClass::Capture_Dazzles(void)
{
    ...
    int curdazzle = 0;
    for (imodel = 0; (imodel < model->Get_Num_Sub_Objects()) && (curdazzle < Dazzles.Length());
         imodel++) {
...
*/
SimplePersistFactoryClass<VehiclePhysDefClass, PHYSICS_CHUNKID_VEHICLEPHYSDEF>
    _VehiclePhysDefFactory;
        ...
        case VEHICLEPHYSDEF_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, VEHICLEPHYSDEF_VARIABLE_DAMPINGCONSTANT,
                                     DampingConstant);
                    READ_MICRO_CHUNK(cload, VEHICLEPHYSDEF_VARIABLE_SPRINGLENGTH, SpringLength);
                    READ_MICRO_CHUNK(cload, VEHICLEPHYSDEF_VARIABLE_TRACTIONMULTIPLIER,
                                     TractionMultiplier);
                    READ_MICRO_CHUNK(cload, VEHICLEPHYSDEF_VARIABLE_LATERALMOMENTARM,
                                     LateralMomentArm);
                    READ_MICRO_CHUNK(cload, VEHICLEPHYSDEF_VARIABLE_TRACTIVEMOMENTARM,
                                     TractiveMomentArm);
                    READ_MICRO_CHUNK(cload, VEHICLEPHYSDEF_VARIABLE_ENGINEFLAMELENGTH,
                                     EngineFlameLength);
