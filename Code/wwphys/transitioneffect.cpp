...
static float STEALTH_ZNEAR_AMOUNT
    = 0.25f; // amount the intensity can change as it approaches the camera
static float STEALTH_FRIENDLY_FRACTION
    = 0.75f; // fraction at which friendly stealthed objects appear
static float STEALTH_BROKEN_FRACTION = 0.25f; // fraction at which broken stealthed objects appear
static float STEALTH_DAMAGED_FRACTION
    = 0.6f; // fraction that is jumped to when an object gets damaged
      ...
      MaterialPass(NULL)
{
    ...
    VertexMaterialClass* vmtl
        = NEW_REF(VertexMaterialClass, ()); // PhysResourceMgrClass::Get_Stealth_Material();
...
bool TransitionEffectClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, TRANSITIONEFFECT_VARIABLE_RENDERTRANSITIONMATERIAL,
                      RenderBaseMaterial);
        ...
        case TRANSITIONEFFECT_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    READ_MICRO_CHUNK(cload, TRANSITIONEFFECT_VARIABLE_CURRENTPARAMETER,
                                     CurrentParameter);
                    READ_MICRO_CHUNK(cload, TRANSITIONEFFECT_VARIABLE_TARGETPARAMETER,
                                     TargetParameter);
                    READ_MICRO_CHUNK(cload, TRANSITIONEFFECT_VARIABLE_PARAMETERVELOCITY,
                                     ParameterVelocity);
                    ...
                    READ_MICRO_CHUNK(cload, TRANSITIONEFFECT_VARIABLE_RENDERBASEMATERIAL,
                                     RenderBaseMaterial);
                    READ_MICRO_CHUNK(cload, TRANSITIONEFFECT_VARIABLE_RENDERTRANSITIONMATERIAL,
                                     RenderBaseMaterial);
                    READ_MICRO_CHUNK(cload, TRANSITIONEFFECT_VARIABLE_INTENSITYSCALE,
                                     IntensityScale);
                    READ_MICRO_CHUNK(cload, TRANSITIONEFFECT_VARIABLE_UVOFFSET, UVOffset);
                    READ_MICRO_CHUNK_WWSTRING(cload, TRANSITIONEFFECT_VARIABLE_TEXTURENAME,
                                              texname);
                }
