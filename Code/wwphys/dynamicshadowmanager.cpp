...
void DynamicShadowManagerClass::Update_Shadow(void)
{
    ...
    */
    ...
    bool use_blob = (shadow_mode == PhysicsSceneClass::SHADOW_MODE_BLOBS)
        || ((shadow_mode == PhysicsSceneClass::SHADOW_MODE_BLOBS_PLUS)
            && (!Parent.Is_Force_Projection_Shadow_Enabled()));
    ...
    */
    if ((Parent.Do_Any_Effects_Suppress_Shadows())
        ...
        || (shadow_mode == PhysicsSceneClass::SHADOW_MODE_NONE) || (shadow_dist2 > shadow_shutoff2)
        || (model == nullptr) || (model->Is_Hidden()) || (objbox.Extent.Length2() < 0.1f)) {
        if (Shadow != nullptr) {
...
#pragma message("(gth) Disabling local shadows")
    ...
    **     Else create a new shadow object, immediately set its intensity to zero (since it is
    *turning on)
