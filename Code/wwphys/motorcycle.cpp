...
void MotorcycleClass::Compute_Force_And_Torque(Vector3* force, Vector3* torque)
{
...

    {
        ...
        float lvel_sign
            = WWMath::Sign(Vector3::Dot_Product(lateral_vel, Get_Transform().Get_Y_Vector()));
        lateral_vel.Z = 0;
        ...
        *torque += (LeanK0 * (target_roll - roll) + LeanK1 * (0.0f - droll))
            * Get_Transform().Get_X_Vector();
...
*/
DECLARE_DEFINITION_FACTORY(MotorcycleDefClass, CLASSID_MOTORCYCLEDEF, "Motorcycle")
_MotorcycleDefDefFactory;
