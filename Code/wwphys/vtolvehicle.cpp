...
{
    {
        ...
        */
        ...
        if (IsEngineOn) {
            ...
            Vector3 yaw_torque = def->YawControllerGain * (desired_yaw_velocity - yaw_velocity)
                * IBody[2][2] * zvec;
            ...
            *torque += yaw_torque;
            ...
            Vector3 pitch_torque = (def->PitchControllerGain * (target_pitch - pitch)
                                    + def->PitchControllerDamping * (0.0f - dpitch))
                * IBody[1][1] * yvec;
            Vector3 roll_torque = (def->RollControllerGain * (target_roll - roll)
                                   + def->RollControllerDamping * (0.0f - droll))
                * IBody[0][0] * xvec;
            ...
            *force += left_force * yvec2d;
            ...
            NormalizedEngineThrust = 0.25f + (WWMath::Fabs(pitch) / def->MaxFuselagePitch)
                + (WWMath::Fabs(roll) / def->MaxFuselageRoll) + move_up;
...
void VTOLVehicleClass::Update_Cached_Model_Parameters(void)
{
    ...
    for (ibone = 0;
         (ibone < Model->Get_Num_Bones()) && (engine_bone_count < MAX_CAPTURED_BONE_COUNT);
         ibone++) {
    ...
    for (ibone = 0;
         (ibone < Model->Get_Num_Bones()) && (rotor_bone_count < MAX_CAPTURED_BONE_COUNT);
         ibone++) {
        const char* bone_name = Model->Get_Bone_Name(ibone);
...
SimplePersistFactoryClass<VTOLVehicleDefClass, PHYSICS_CHUNKID_VTOLVEHICLEDEF>
    _VTOLVehicleDefFactory;
DECLARE_DEFINITION_FACTORY(VTOLVehicleDefClass, CLASSID_VTOLVEHICLEDEF, "VTOLVehicle")
_VTOLVehicleDefDefFactory;
      ...
      RotorDeceleration(DEG_TO_RADF(180.0f))
{
    ...
    ANGLE_EDITABLE_PARAM(VTOLVehicleDefClass, MaxFuselagePitch, DEG_TO_RADF(0.01f),
                         DEG_TO_RADF(50.0f));
    ANGLE_EDITABLE_PARAM(VTOLVehicleDefClass, MaxFuselageRoll, DEG_TO_RADF(0.01f),
                         DEG_TO_RADF(50.0f));
    ...
    ANGLE_EDITABLE_PARAM(VTOLVehicleDefClass, MaxYawVelocity, DEG_TO_RADF(0.01f),
                         DEG_TO_RADF(360.0f));
    FLOAT_EDITABLE_PARAM(VTOLVehicleDefClass, YawControllerGain, 0.0f, 10000.0f);
    ANGLE_EDITABLE_PARAM(VTOLVehicleDefClass, MaxEngineRotation, DEG_TO_RADF(0.01f),
                         DEG_TO_RADF(50.0f));
    ANGLE_EDITABLE_PARAM(VTOLVehicleDefClass, RotorSpeed, DEG_TO_RADF(0.01f), DEG_TO_RADF(640.0f));
    ANGLE_EDITABLE_PARAM(VTOLVehicleDefClass, RotorAcceleration, DEG_TO_RADF(0.01f),
                         DEG_TO_RADF(640.0f));
    ANGLE_EDITABLE_PARAM(VTOLVehicleDefClass, RotorDeceleration, DEG_TO_RADF(0.01f),
                         DEG_TO_RADF(640.0f));
}
...
bool VTOLVehicleDefClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, VTOLVEHICLEDEF_VARIABLE_MAXVERTICALACCELERATION,
                      MaxVerticalAcceleration);
    WRITE_MICRO_CHUNK(csave, VTOLVEHICLEDEF_VARIABLE_MAXHORIZONTALACCELERATION,
    ...
    WRITE_MICRO_CHUNK(csave, VTOLVEHICLEDEF_VARIABLE_PITCHCONTROLLERDAMPING,
                      PitchControllerDamping);
        ...
        case VTOLVEHICLEDEF_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_MAXVERTICALACCELERATION,
                                     MaxVerticalAcceleration);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_MAXHORIZONTALACCELERATION,
                                     MaxHorizontalAcceleration);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_MAXFUSELAGEPITCH,
                                     MaxFuselagePitch);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_MAXFUSELAGEROLL,
                                     MaxFuselageRoll);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_PITCHCONTROLLERGAIN,
                                     PitchControllerGain);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_PITCHCONTROLLERDAMPING,
                                     PitchControllerDamping);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_ROLLCONTROLLERGAIN,
                                     RollControllerGain);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_ROLLCONTROLLERDAMPING,
                                     RollControllerDamping);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_MAXYAWVELOCITY, MaxYawVelocity);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_YAWCONTROLLERGAIN,
                                     YawControllerGain);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_MAXENGINEROTATION,
                                     MaxEngineRotation);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_ROTORSPEED, RotorSpeed);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_ROTORACCELERATION,
                                     RotorAcceleration);
                    READ_MICRO_CHUNK(cload, VTOLVEHICLEDEF_VARIABLE_ROTORDECELERATION,
                                     RotorDeceleration);
                }
