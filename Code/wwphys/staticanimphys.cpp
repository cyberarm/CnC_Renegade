...
*/
SimplePersistFactoryClass<StaticAnimPhysClass, PHYSICS_CHUNKID_STATICANIMPHYS>
    _StaticAnimPhysFactory;
...
bool StaticAnimPhysClass::Load(ChunkLoadClass& cload)
{
    ...
    */
        ...
        case STATICANIMPHYS_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    READ_MICRO_CHUNK(cload, STATICANIMPHYS_VARIABLE_COLLISIONMODE,
                                     legacy_collision_mode);
                }
    ...
    if (legacy_collision_mode != -1) {
        AnimManager.Set_Collision_Mode(
            (AnimCollisionManagerClass::CollisionModeType)legacy_collision_mode);
    }
...
*/
SimplePersistFactoryClass<StaticAnimPhysDefClass, PHYSICS_CHUNKID_STATICANIMPHYSDEF>
    _StaticAnimPhysDefFactory;
...
bool StaticAnimPhysDefClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, STATICANIMPHYSDEF_VARIABLE_SHADOWIGNORESZROTATION,
                      ShadowIgnoresZRotation);
...
bool StaticAnimPhysDefClass::Load(ChunkLoadClass& cload)
{
    while (cload.Open_Chunk()) {
        ...
        case STATICANIMPHYSDEF_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    // NOTE: these two variables/microchunks have been moved into the animation
                    // manager.
                    READ_MICRO_CHUNK(cload, STATICANIMPHYSDEF_VARIABLE_COLLISIONMODE,
                                     AnimManagerDef.CollisionMode);
                    READ_MICRO_CHUNK_WWSTRING(cload, STATICANIMPHYSDEF_VARIABLE_ANIMATIONNAME,
                                              AnimManagerDef.AnimationName);

                    READ_MICRO_CHUNK(cload, STATICANIMPHYSDEF_VARIABLE_SHADOWDYNAMICOBJS,
                                     ShadowDynamicObjs);
                    READ_MICRO_CHUNK(cload, STATICANIMPHYSDEF_VARIABLE_SHADOWISADDITIVE,
                                     ShadowIsAdditive);
                    READ_MICRO_CHUNK(cload, STATICANIMPHYSDEF_VARIABLE_SHADOWIGNORESZROTATION,
                                     ShadowIgnoresZRotation);
                    READ_MICRO_CHUNK(cload, STATICANIMPHYSDEF_VARIABLE_SHADOWFARZ, ShadowFarZ);
                    READ_MICRO_CHUNK(cload, STATICANIMPHYSDEF_VARIABLE_SHADOWINTENSITY,
                                     ShadowIntensity);
                    READ_MICRO_CHUNK(cload, STATICANIMPHYSDEF_VARIABLE_SHADOWNEARZ, ShadowNearZ);
                    READ_MICRO_CHUNK(cload, STATICANIMPHYSDEF_VARIABLE_COLLIDEINPATHFIND,
