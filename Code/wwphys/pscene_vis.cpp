...
void PhysicsSceneClass::Compute_Vis_Sample_Point(const CameraClass& camera, Vector3* set_point)
{
    ...
    if (!VisSamplePointLocked) {
        ...
        Matrix3D::Transform_Vector(camera.Get_Transform(), Vector3(0, 0, -nearz - 0.05f),
                                   set_point);
    }
    else {
 ...
 *   7/5/2000   gth : Created.                                                                 *
 *=============================================================================================*/
VisSampleClass PhysicsSceneClass::Update_Vis(const Matrix3D& camera_tm,
...
VisSampleClass PhysicsSceneClass::Update_Vis(const Vector3& sample_point, const Matrix3D& camera_tm,
                                             VisDirBitsType direction_bits,
                                             CameraClass* alternate_camera, int user_vis_id)
{
    /*
...
void PhysicsSceneClass::Generate_Vis_For_Light(int light_index)
{
    ...
    */
    ...
    if ((light != nullptr) && (light->Get_Vis_Sector_ID() != 0xFFFFFFFF)
        ...
        */
        ...
        Update_Vis(light->Get_Transform().Get_Translation(), light->Get_Transform(),
                   (VisDirBitsType)(VIS_ALL | VIS_FORCE_ACCEPT), camera,
 ...
 *   7/5/2000   gth : Created.                                                                 *
 *=============================================================================================*/
void PhysicsSceneClass::Generate_Vis_Statistics_Report(
    DynamicVectorClass<VisSectorStatsClass>& report)
{
    /*
...
void PhysicsSceneClass::Optimize_Visibility_Data(VisOptProgressClass& stats)
{
    ...
    */
    stats.Reset(DynamicObjVisSystem->Partition_Node_Count() + Get_Vis_Table_Size()
                + Get_Vis_Table_Count());
    stats.Set_Initial_Bit_Count(VisTableManager.Get_Vis_Table_Size()
                                * VisTableManager.Get_Vis_Table_Count());
    ...
    */
    stats.Set_Final_Bit_Count(VisTableManager.Get_Vis_Table_Size()
                              * VisTableManager.Get_Vis_Table_Count());
