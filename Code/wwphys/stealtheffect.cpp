...
static float STEALTH_ZNEAR_AMOUNT
    = 0.40f; // amount the intensity can change as it approaches the camera
...
void StealthEffectClass::Timestep(float dt)
{
    ...
    ** from the intensity
    */
    Vector2 uv_rate = MIN_STEALTH_UV_RATE
        + IntensityScale * IntensityScale * (MAX_STEALTH_UV_RATE - MIN_STEALTH_UV_RATE);
    UVOffset += uv_rate * dt;
        ...
        case STEALTHEFFECT_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    READ_MICRO_CHUNK(cload, STEALTHEFFECT_VARIABLE_ISSTEALTHENABLED,
                                     IsStealthEnabled);
                    ...
                    READ_MICRO_CHUNK(cload, STEALTHEFFECT_VARIABLE_CURRENTFRACTION,
                                     CurrentFraction);
