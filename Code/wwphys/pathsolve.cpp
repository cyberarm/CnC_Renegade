...
//
///////////////////////////////////////////////////////////////////////////
PathSolveClass::STATE_DESC PathSolveClass::Reset(const Vector3& start, const Vector3& dest,
                                                 float sector_fudge)
{
...
void PathSolveClass::Resolve_Path(unsigned int milliseconds)
{
    ...
    // (unsigned int)((Get_Time () - start_time) / (_TicksPerMilliSec/100)), iterations,
    // (int)(m_State == TRAVERSING_PATH));
    return;
}
...
//
///////////////////////////////////////////////////////////////////////////
void Find_Adjacent_Portals(PathfindSectorClass* sector, PathfindPortalClass* portal,
                           PathfindSectorClass* dest_sector, PathfindPortalClass** portal1,
...
//
///////////////////////////////////////////////////////////////////////////
bool PathSolveClass::Can_Object_Go_Through_Portal(const Matrix3D& current_tm,
        ...
        if (sector->Can_Access_Portal(last_portal, portal)) {
            ...
            if (dest_sector != NULL) {
                ...
                if (Does_Object_Have_Access_To_Portal(portal)
                    ...
                    //
                    Submit_Node(node->Get_Traversal_Cost(), node, portal, dest_sector, current_tm,
                                ending_tm);
                }
            }
        }
...
float Calculate_Heuristic_Cost(const PathObjectClass& object, const PathfindPortalClass& portal,
                               const PathfindSectorClass& sector, const Vector3& curr_pos,
                               const Vector3& dest_pos)
{
    ...
    if (sector.As_PathfindWaypathSectorClass() != NULL) {
        ...
        // float	object_area			= (object_box.Extent.X *
        // object_box.Extent.Y);
...
//
///////////////////////////////////////////////////////////////////////////
void PathSolveClass::Submit_Node(float traversal_cost, PathNodeClass* current_node,
                                 PathfindPortalClass* portal, PathfindSectorClass* dest_sector,
                                 const Matrix3D& current_tm, const Matrix3D& ending_tm)
{
    ...
    float heuristic_cost
        = ::Calculate_Heuristic_Cost(m_PathObject, *portal, *dest_sector, dest_pos, m_DestPos);
...
//
/////////////////////////////////////////////////////////////////////////////////
bool PathSolveClass::Does_Line_Go_Through_Portal(const Vector3& start, const Vector3& end,
...
//
/////////////////////////////////////////////////////////////////////////////////
bool Check_Line(const Vector3& p0, const Vector3& p1, const AABoxClass& sector,
                const AABoxClass& portal, float width, Vector3* result)
{
    bool retval = false;
    ...
    if (WWMath::Fabs(portal.Center.X - sector.Center.X + sector.Extent.X) < WWMATH_EPSILON
        ...
        if (do_it) {
            ...
            if (pos.X + width > (sector.Center.X + sector.Extent.X)) {
             ...
             || WWMath::Fabs(portal.Center.Y - sector.Center.Y - sector.Extent.Y)
                 < WWMATH_EPSILON) {
    ...
    for (int index = 0; index < m_Path.Count() - 1; index++) {
        ...
        if ((m_Path[index].m_SectorExtent.X > 0) || (m_Path[index].m_SectorExtent.Y > 0)) {
            ...
            if (prev_portal != NULL && prev_portal->As_PathfindActionPortalClass() == NULL) {
                ...
                if (::Check_Line(start_point, next_point, sector_box, portal_box, width,
                                 &new_point)) {
