...
uint32 DynamicAABTreeCullClass::Get_Dynamic_Object_Vis_ID(const AABoxClass& obj_bounds,
                                                          int* node_id)
{
...
void DynamicAABTreeCullClass::Re_Partition(DynamicVectorClass<AABoxClass>* seed_boxes,
                                           const Vector3& grid_min, const Vector3& grid_max,
...
void DynamicAABTreeCullClass::Collect_Visible_Objects(const FrustumClass& frustum,
                                                      VisTableClass* pvs,
                                                      RefPhysListClass& visobjlist)
{
    ...
    if (pvs != nullptr) {
        ...
        */
        PhysClass* obj;
        for (obj = Get_First_Collected_Object(); obj != nullptr;
...
void DynamicAABTreeCullClass::Render_Visible_Cells(RenderInfoClass& rinfo, VisTableClass* pvs,
...
void DynamicAABTreeCullClass::evaluate_non_occluder_visibility_recursive(
    AABTreeNodeClass* node, VisRenderContextClass& context)
{
    /*
...
bool DynamicAABTreeCullClass::subtree_is_visible(AABTreeNodeClass* node,
...
void DynamicAABTreeCullClass::set_tree_visibility(AABTreeNodeClass* node,
                                                  VisRenderContextClass& context, bool onoff)
{
    if (node == nullptr) {
        return;
    }
...
int DynamicAABTreeCullClass::find_insertion_node(CullableClass* obj)
{
    ...
    if ((cullbox.Extent.X > MaxObjRadius) || (cullbox.Extent.Y > MaxObjRadius)
...
void DynamicAABTreeCullClass::find_optimal_node(AABTreeNodeClass* node, const AABoxClass& box,
                                                int& node_index)
{
    /*
...
void DynamicAABTreeCullClass::find_optimal_deflated_node(AABTreeNodeClass* node,
                                                         const Vector3& center, int& node_index)
{
    /*
    ...
    */
    if (node->Object) {
        ...
        if (context.PVS.Get_Bit(node->UserData) != 0) {
            ...
            while (obj) {
                if (CollisionMath::Overlap_Test(context.Frustum, obj->Get_Cull_Box(),
                                                context.PlanesPassed)
...
void DynamicAABTreeCullClass::collect_visible_objects_no_hvis_recursive(
    AABTreeNodeClass* node, VisObjCollectContextClass& context)
{
    /*
    ** Cull the bounding volume of this node against the frustum.
    ** If it is culled, stop descending the tree.
...
void DynamicAABTreeCullClass::render_visible_cells_recursive(AABTreeNodeClass* node,
                                                             ...
                                                             DisplayModeType mode)
{
    ...
    */
    ...
    if ((CollisionMath::Overlap_Test(rinfo.Camera.Get_Frustum(), node->Box)
         == CollisionMath::OUTSIDE)
            ...
            if ((mode != DISPLAY_OCCUPIED) || (node->Object != nullptr)) {
                ...
                if (mode == DISPLAY_CENTERS) {
                    ...
                    rbox->Set_Local_Center_Extent(
                        node->Box.Center,
                        node->Box.Extent
                            - Vector3(MaxObjRadius, MaxObjRadius,
                                      MaxObjRadius)); // set the box dimensions
                }
...
void DynamicAABTreeCullClass::prune_redundant_leaf_nodes_recursive(
    AABTreeNodeClass* node, VisOptimizationContextClass& context)
{
    /*
    ...
    */
    if ((child->Front == nullptr) && (child->Back == nullptr)) {
        ...
        float match_fraction
            = context.Compute_Object_Table_Match_Fraction(parent_object_id, child_object_id);
        WWDEBUG_SAY(("parent: %d, child: %d, match fraction: %f\n", parent_object_id,
                     child_object_id, match_fraction));
        if (match_fraction >= context.Get_Dyn_Cell_Prune_Match_Fraction()) {

            /*
            ** unlink the node from its parent so that subsequent calls back into the tree will not
            *see it.
            */
