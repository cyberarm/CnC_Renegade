...
const float MIN_STATIC_SHADOW_COS_HALF_THETA
...
public:
    ...
    //	void					Reset(); // Jani: Disabling reset. Re-allocating
    //render targets if the device is out of
...
static TextureClass* Create_Projector_Render_Target(unsigned w, unsigned h)
{
    ...
    else if (DX8Wrapper::Get_Current_Caps()->Support_Render_To_Texture_Format(
    ...
    // As a last resort, try creating with unknown format (which means using the current display
    // resolution)
...
StaticShadowTexMgrClass::ShadowTexClass::ShadowTexClass(uint32 obj_type_id,
...
TextureClass* StaticShadowTexMgrClass::Peek_Shadow_Texture(uint32 obj_type_id,
                                                           const Quaternion& orientation)
{
...
void StaticShadowTexMgrClass::Add_Shadow_Texture(uint32 obj_type_id, const Quaternion& orientation,
                                                 TextureClass* tex)
{
...
SpecialRenderInfoClass* PhysicsSceneClass::Get_Shadow_Render_Context(int width, int height)
{
    if (ShadowRenderContext == nullptr) {
        ...
        */
        ShadowRenderContext
            = new SpecialRenderInfoClass(*ShadowCamera, SpecialRenderInfoClass::RENDER_SHADOW);
    }
    ...
    if (((int)shadow_mode >= 0) && ((int)shadow_mode < SHADOW_MODE_COUNT)) {
        if (ShadowMode != shadow_mode) {
            ...
            case SHADOW_MODE_BLOBS: // projected blob shadows
                ...
                break;
            case SHADOW_MODE_BLOBS_PLUS: // projected blobs with main character having a rendered
                                         // shadow
...
{
    return (DynamicProjectorList.Is_In_List(projector)
            || StaticProjectorList.Is_In_List(projector));
}

float PhysicsSceneClass::Compute_Projector_Attenuation(TexProjectClass* dynamic_projector,
...
void PhysicsSceneClass::Apply_Projectors(const CameraClass& camera)
{
    ...
    if (StaticProjectorsEnabled) {
        ...
        */
        ...
        TexProjectClass* static_projector
            = StaticProjectorCullingSystem->Get_First_Collected_Object();
        while (static_projector != nullptr) {
            ...
            */
            if (!static_projector->Is_Intensity_Zero()) {
                ...
                */
                ...
                if (static_projector->Is_Attenuation_Enabled()) {
                    attenuation
                        = Compute_Projector_Attenuation(static_projector, view_pos, view_dir);
                ...
                */
                if (attenuation > 0.0f) {
                    ...
                    if (ProjectorDebugDisplayEnabled) {
                        DEBUG_RENDER_OBBOX(static_projector->Get_Bounding_Volume(),
                                           Vector3(0, 1, 0), 0.25f);
                    }
            ...
            static_projector
                = StaticProjectorCullingSystem->Get_Next_Collected_Object(static_projector);
        }
    }
    ...
    */
    ...
    if (DynamicProjectorsEnabled) {
        ...
        */
        ...
        TexProjectClass* dynamic_projector
            = DynamicProjectorCullingSystem->Get_First_Collected_Object();
        while (dynamic_projector != nullptr) {
            ...
            */
            if (!dynamic_projector->Is_Intensity_Zero()) {
                ...
                */
                ...
                if (dynamic_projector->Is_Attenuation_Enabled()) {
                    attenuation
                        = Compute_Projector_Attenuation(dynamic_projector, view_pos, view_dir);
                ...
                */
                if (attenuation > 0.0f) {
                    ...
                    if (ProjectorDebugDisplayEnabled) {
                        DEBUG_RENDER_OBBOX(dynamic_projector->Get_Bounding_Volume(),
                                           Vector3(1, 0, 0), 0.25f);
                    }
            ...
            dynamic_projector
                = DynamicProjectorCullingSystem->Get_Next_Collected_Object(dynamic_projector);
        }
    }
...
void PhysicsSceneClass::Apply_Projector_To_Objects(TexProjectClass* tex_proj,
    ...
    */
    if (tex_proj->Is_Affect_Static_Objects_Enabled()) {
        ...
        while (static_obj) {
            ...
            */
            if (VisibleStaticObjectList.Contains(static_obj)
                || VisibleWSMeshList.Is_In_List(static_obj)) {
            ...
            static_obj
                = (StaticPhysClass*)StaticCullingSystem->Get_Next_Collected_Object(static_obj);
        }
    }
        ...
        while (dyn_obj) {
            ...
            */
            if ((VisibleDynamicObjectList.Contains(dyn_obj))
                && (dyn_obj != tex_proj->Get_Projection_Object_ID())) {
...
static bool Test_Render_Target_Surface(TextureClass* render_target)
{
    ...
    DX8Wrapper::_Copy_DX8_Rects(surf->Peek_D3D_Surface(), nullptr, 0, new_surf->Peek_D3D_Surface(),
                                nullptr);
    ...
    BitmapHandlerClass::Read_B8G8R8A8(color1, tmpbits, desc.Format, 1 * desc.Width / 4,
                                      1 * desc.Height / 4, desc.Width, desc.Height, nullptr, 0);

    BitmapHandlerClass::Read_B8G8R8A8(color2, tmpbits, desc.Format, 3 * desc.Width / 4,
                                      1 * desc.Height / 4, desc.Width, desc.Height, nullptr, 0);

    BitmapHandlerClass::Read_B8G8R8A8(color3, tmpbits, desc.Format, 3 * desc.Width / 4,
                                      3 * desc.Height / 4, desc.Width, desc.Height, nullptr, 0);

    BitmapHandlerClass::Read_B8G8R8A8(color4, tmpbits, desc.Format, 1 * desc.Width / 4,
                                      3 * desc.Height / 4, desc.Width, desc.Height, nullptr, 0);
    ...
    WWDEBUG_SAY(("Render target test: 0x%8.8x, 0x%8.8x, 0x%8.8x, 0x%8.8x\n", color1 & 0x00ffffff,
                 color2 & 0x00ffffff, color3 & 0x00ffffff, color4 & 0x00ffffff));
...
void PhysicsSceneClass::Invalidate_Static_Shadow_Projectors()
{
    ...
    */
    ...
    for (static_anim_iterator.First(); !static_anim_iterator.Is_Done();
         static_anim_iterator.Next()) {
...
void PhysicsSceneClass::Generate_Static_Shadow_Projectors(void)
{
    ...
    */
    ...
    // Test render target functionality. Some NVidia driver versions have issues with rendering to a
    // texture and copying surface to another texture. If this fails, we'll not use static shadow
    // projectors. Dynamic shadow projectors may still work however, as they work differently.
    if (render_target) {
        Create_Render_Target_Test(render_target);
        if (!Test_Render_Target_Surface(render_target)) {
            WWDEBUG_SAY(
                ("Render target locking doesn't work correctly. Disabling static projectors\n"));
    ...
    if (render_target != nullptr) {
        ...
        */
        ...
        for (shadow_gen_iterator.First(); !shadow_gen_iterator.Is_Done();
             shadow_gen_iterator.Next()) {
...
void PhysicsSceneClass::Setup_Static_Directional_Shadow(StaticAnimPhysClass& obj,
                                                        ...
                                                        TextureClass* render_target)
{
    ...
    */
    ...
    shadow_projector->Compute_Ortho_Projection(&obj, light_dir, def->Shadow_NearZ(),
                                               def->Shadow_FarZ());
    ...
    */
    TextureClass* existing_texture
        = _StaticShadowTexMgr.Peek_Shadow_Texture(type_id, obj_orientation);
    ...
    */
    if (existing_texture == nullptr) {
        ...
        _StaticShadowTexMgr.Add_Shadow_Texture(type_id, obj_orientation,
