...
**
************************************************************************************************/
CameraShakeSystemClass::CameraShakerClass::CameraShakerClass(const Vector3& position, float radius,
...
void CameraShakeSystemClass::CameraShakerClass::Compute_Rotations(const Vector3& camera_position,
                                                                  Vector3* set_angles)
{
    ...
    */
    float intensity
        = Intensity * (1.0f - WWMath::Sqrt(len2) / Radius) * (1.0f - ElapsedTime / Duration);
    for (int i = 0; i < 3; i++) {
        float omega = Omega[i] + (END_OMEGA - Omega[i]) * ElapsedTime;
        (*set_angles)[i]
            += AXIS_ROTATION[i] * intensity * WWMath::Sin(omega * ElapsedTime + Phi[i]);
    }
}
...
void CameraShakeSystemClass::Add_Camera_Shake(const Vector3& position, float radius, float duration,
                                              float power)
{
