...
const float SUN_CHECK_DISTANCE
    = 50.0f; // If a ray this long doesn't intersect, you can see the sun...
...
LightEnvironmentClass* PhysClass::Get_Static_Lighting_Environment(void)
{
    if (Is_Pre_Lit()) {
        ...
        */
        PhysicsSceneClass::Get_Instance()->Compute_Static_Lighting(
            StaticLightingCache, Model->Get_Bounding_Sphere().Center, Get_Flag(IS_IN_THE_SUN),
            Get_Vis_Object_ID());
...
#pragma message("(gth) Need a collision group for sun-rays")
    ...
    // and check again.
    if ((sunresult.Fraction < 1.0f) && (sunraytest.CollidedPhysObj != nullptr)) {
