...
September 15, 1998
  ...
  level editor, I implemented the new PhysicsSceneClass which replaces the old classes:
PhysicsSystem PrivatePhysicsSystem and CollisionSystem and also takes over the job of the main 3D
game scene for rendering.  This caused some major re-organization and as a result most of WWPhys was
#if 0'd...
  ...
  - TileMapClass is now obsolete but much of its functionality will have to go into game code.
    The PhysicsSceneClass contains an axis-aligned bounding box tree which serves the purpose
         that TileMap used to.  Other "TerrainPhysClass's" are still going to be single objects
though.
...
December 21, 1998
  ...
  - Its a lot simpler if this system *only* contains completely static meshes! (not walls, etc)
         ...
         - want as much as we can get into this since it should be faster and has no size
restrictions
...
public:
    ...
    ** rotation is the angle between the +X axis and the line of the sun projected onto the X-Y
    *plane.
    ...
    */
    ...
    void Compute_Static_Lighting(LightEnvironmentClass* light_env, const Vector3& obj_center,
                                 bool use_sun, int vis_object_id);
    void Invalidate_Lighting_Caches(const AABoxClass& bounds);

    /*
    ** Collision Detection Methods
    ** Set_Collision_Region - collects objects in the given region into a list for subsequent
    *collision checks
    **                        this is useful if you're going to do a lot of checks in the same
    *general area (e.g. RigidBody)
    ** Release_Collision_Region - releases the collision region list
    ** Cast_Ray - casts a ray into the world, returning information about what was collided and at
    *what point along the ray
    ** Cast_AABox - casts an axis aligned box, returning information about what was collided and at
    *what point
    ** Cast_OBBox - casts an oriented box, returning information about what was collided and at what
    *point
    ** Intersection_Test - tests the given primitive for intersection with anything else in the
    *system
    */
    ...
    */
    void Collect_Objects(const Vector3& point, bool static_objs, bool dynamic_objs,
                         NonRefPhysListClass* list);
    void Collect_Objects(const AABoxClass& box, bool static_objs, bool dynamic_objs,
                         NonRefPhysListClass* list);
    void Collect_Objects(const OBBoxClass& box, bool static_objs, bool dynamic_objs,
                         NonRefPhysListClass* list);
    void Collect_Objects(const FrustumClass& frustum, bool static_objs, bool dynamic_objs,
                         NonRefPhysListClass* list);

    void Collect_Collideable_Objects(const AABoxClass& box, int colgroup, bool static_objs,
                                     bool dynamic_objs, NonRefPhysListClass* list);
    void Collect_Collideable_Objects(const OBBoxClass& box, int colgroup, bool static_objs,
                                     bool dynamic_objs, NonRefPhysListClass* list);

    void Collect_Lights(const Vector3& point, bool static_lights, bool dynamic_lights,
                        NonRefPhysListClass* list);
    void Collect_Lights(const AABoxClass& bounds, bool static_lights, bool dynamic_lights,
                        NonRefPhysListClass* list);
    ...
    ** Set_Vis_Quick_And_Dirty - when generating, make non-occluders always visible (only used in
    *preprocessing)
    ...
    ** Disable_Vis_Sector_Fallback - don't fall back to the previous sector when there is no sector
    ...
    ** pointer to your previous node_id, the lookup will be faster and this variable will be
    *modified
    ...
    ** Generate_Vis_Statistics_Report - generates a report giving the amount of polygons and
    *textures
    **                                  visible from each sector in the system.
    ** Optimize_Visibility_Data - compresses vis by removing redundant vis-id's and pruning the
    *dynamic culling
    **                            system.  This should only be called *after* all vis samples have
    *been completed.
    */
    ...
    */
    ...
    /*
    ** Generate (or re-generate) all static shadows. Invalidate must be called before calling
    *Generate.
    */
    ...
    */
    void Add_Camera_Shake(const Vector3& position, float radius = 50.0f, float duration = 1.5f,
                          float power = 1.0f);
    ...
    ** PhysStaticObjectsSaveSystem - saves static objects like terrain pieces which have no dynamic
    *state.
...
protected:
    ...
    */
    ...
    void Render_Objects(RenderInfoClass& rinfo, RefPhysListClass* static_ws_list,
    ...
    void Optimize_LODs(CameraClass& camera, RefPhysListClass* dyn_obj_list,
                       RefPhysListClass* static_obj_list, RefPhysListClass* static_ws_mesh_list);
    /*
...
protected:
    ...
    */
    ...
    void Add_Collected_Objects_To_List(bool static_objs, bool dynamic_objs,
                                       NonRefPhysListClass* list);
    void Add_Collected_Collideable_Objects_To_List(int colgroup, bool static_objs,
                                                   bool dynamic_objs, NonRefPhysListClass* list);
    void Add_Collected_Lights_To_List(bool static_lights, bool dynamic_lights,
                                      NonRefPhysListClass* list);
    ...
    */
    bool StaticProjectorsEnabled; // toggle static shadows (shadows cast by static objs onto dynamic
                                  // objs)
    bool DynamicProjectorsEnabled; // toggle dynamic shadows (shadows cast by dynamic objs onto
                                   // everything)
    ...
    */
    ...
    //--------------------------------------------------------------------------------
    //
    // This is the dynamic data/objects which all need to be saved in either the LDD file or SAV
    // file.
    //
    //-----------------------------------------------------------------------------------------------------
    ...
    */
    RefPhysListClass
        DirtyCullList; // objects that have 'dirty culling' must be re-inserted each frame...
    RefPhysListClass TimestepList; // objects which need to be time-stepped go in here
    RefPhysListClass StaticAnimList; // list of the StaticAnim objects, these can cast shadows,
                                     // change states, etc
    NonRefPhysListClass
        CollisionRegionList; // cached list of objects in the current collision region
