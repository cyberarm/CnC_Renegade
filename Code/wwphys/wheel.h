...
public:
    enum FlagsType
    {
        ...
        TILT_STEERING
        = 0x0008, // this wheel turns with tilt abount the X axis of the vehicle (bikes).
...
protected:
    ...
    */
    ...
    int AxisBone; // Bone index for the translation axis bone (if not -1, we translate on its z-axis
                  // instead)
    ...
    */
    ...
    /*
    ** The latest known location of the spring end points - this is used to determine if the spring
    *has moved
    */
    Vector3 SpringEndP1;
    Vector3 SpringEndP0;
...
public:
    ...
    virtual void Init(VehiclePhysClass* obj, int postion_bone, int rotation_bone = -1,
                      int fork_bone = -1, int axis_bone = -1);
    virtual void Compute_Force_And_Torque(Vector3* force, Vector3* torque);

    /*
    ** Accessors
    */
    bool Is_Drive_Wheel(void)
    {
        return (Get_Flag(ENGINE) || Get_Flag(LEFT_TRACK) || Get_Flag(RIGHT_TRACK));
    }
...
protected:
    ...
    */
    void Compute_Suspension_Force(const Vector3& pdot, const Vector3& local_pdot,
    ...
    */
    virtual void Compute_Traction_Forces(const Vector3& local_pdot, float normal_force,
