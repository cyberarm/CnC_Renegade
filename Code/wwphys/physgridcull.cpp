...
void PhysGridCullClass::Re_Partition(const Vector3& min, const Vector3& max, float objdim)
{
    ...
    */
    ...
    MinCellSize.Set(MIN_PHYSGRID_CELL_DIMENSION, MIN_PHYSGRID_CELL_DIMENSION,
                    MIN_PHYSGRID_CELL_DIMENSION);
...
inline void PhysGridCullClass::collect_visible_objects_in_leaf(const FrustumClass& frustum,
        ...
        */
        ...
        for (; !it.Is_Done(); it.Next()) {
            ...
            if ((pvs.Get_Bit(obj->Get_Vis_Object_ID()) != 0)
                && (CollisionMath::Overlap_Test(frustum, obj->Get_Cull_Box())
                                           ...
                                           const GridCullSystemClass::VolumeStruct& vol)
{
    ...
    if (vol.Is_Leaf()) {
        ...
        PhysClass* head
            = (PhysClass*)Cells[map_indices_to_address(vol.Min[0], vol.Min[1], vol.Min[2])];
