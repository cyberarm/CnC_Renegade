...
**
** LOAD-SAVE NOTES:
** The data in the physics scene will be broken into two different files, the LSD (Level Static
*Data)
** file and the LDD (Level Dynamic Data) file.  The primary reason for this is simply so that
** save-games can simply be a new export of the LDD file and not contain the data which is
*completely
...
enum
{
    ...
    // Chunk ID's used by the physics scene when saving the 'Static Objects'
    // (PhysStaticObjectsSaveSystem)
    ...
    // Chunk ID's used by the physics scene when saving the dynamic data and objects
    // (PhysDynamicDataSaveSystem)
...
void PhysicsSceneClass::Save_Level_Static_Objects(ChunkSaveClass& csave)
{
    /*
    ** Things to save here:
    ** - Objects which have absolutely no dynamic state and cannot be created or destroyed during a
    *level.
...
void PhysicsSceneClass::Save_Level_Dynamic_Data(ChunkSaveClass& csave)
{
    ...
    ** - DynamicPolyBudget
    ** - StaticPolyBudget
    ** - List of dynamic objects (embed each in a chunk, embed cull linkage, build the list again at
    *load time)
