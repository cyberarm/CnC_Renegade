 ...
 *   8/9/2000   gth : Created.                                                                 *
 *=============================================================================================*/
void AnimCollisionManagerClass::CollideableObjClass::Intersect_Scene(
    int colgroup, NonRefPhysListClass* intersect_list)
{
    ...
    case RenderObjClass::CLASSID_OBBOX: {
        ...
        PhysOBBoxIntersectionTestClass test(boxrobj->Get_Box(), colgroup, COLLISION_TYPE_PHYSICAL,
                                            intersect_list);
        test.CheckStaticObjs = false;
        scene->Intersection_Test(test);
    ...
    case RenderObjClass::CLASSID_AABOX: {
        ...
        PhysAABoxIntersectionTestClass test(boxrobj->Get_Box(), colgroup, COLLISION_TYPE_PHYSICAL,
                                            intersect_list);
        test.CheckStaticObjs = false;
        scene->Intersection_Test(test);
 ...
 *   8/9/2000   gth : Created.                                                                 *
 *=============================================================================================*/
AnimCollisionManagerClass::AnimModeType AnimCollisionManagerClass::Get_Animation_Mode(void)
{
...
void AnimCollisionManagerClass::Internal_Set_Animation(const char* anim_name)
{
    ...
    if (anim == NULL && anim_name != NULL) {
        WWDEBUG_SAY(
            ("FAILED TO FIND ANIM IN AnimCollisionManagerClass::Internal_Set_Animation(\"%s\")\n",
             anim_name));
    }
...
bool AnimCollisionManagerClass::Check_Collision(CollideableObjClass& collisionobj)
{
    ...
    ** - Ask physics scene to do an intersection test between our mesh and all nearby dynamic
    *objects
    ...
    */
    ...
    for (it.First(); !it.Is_Done(); it.Next()) {
        ...
        if (RiderManager.Contains(obj)
            && (obj->Peek_Carrier_Sub_Object() == collisionobj.Peek_Collision_Object())) {
 ...
 *   8/9/2000   gth : Created.                                                                 *
 *=============================================================================================*/
bool AnimCollisionManagerClass::Push_Collided_Object(PhysClass* obj,
                                                     const Matrix3D& delta_transform)
{
    ...
    if (move_obj != NULL) {
        ...
        VERBOSE_LOG(("Pushing object: %s, vector: (%f, %f, %f)\r\n",
                     move_obj->Peek_Model()->Get_Name(), delta_transform.Get_Translation().X,
                     delta_transform.Get_Translation().Y, delta_transform.Get_Translation().Z));
...
bool AnimCollisionManagerClass::Save(ChunkSaveClass& csave)
{
    ...
    if (CurAnimation != NULL) {
        WRITE_MICRO_CHUNK_STRING(csave, ANIMCOLLISIONMANAGER_VARIABLE_ANIMATIONNAME,
                                 CurAnimation->Get_Name());
    }

    if (PrevAnimation != NULL) {
        WRITE_MICRO_CHUNK_STRING(csave, ANIMCOLLISIONMANAGER_VARIABLE_PREVANIMATIONNAME,
                                 PrevAnimation->Get_Name());
    }
...
bool AnimCollisionManagerClass::Load(ChunkLoadClass& cload)
{
    ...
    */
        ...
        case ANIMCOLLISIONMANAGER_CHUNK_VARIABLES:
            ...
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    READ_MICRO_CHUNK(cload, ANIMCOLLISIONMANAGER_VARIABLE_COLLISIONMODE,
                                     CollisionMode);
                    READ_MICRO_CHUNK(cload, ANIMCOLLISIONMANAGER_VARIABLE_ANIMATIONMODE,
                                     AnimationMode);
                    ...
                    READ_MICRO_CHUNK_WWSTRING(cload, ANIMCOLLISIONMANAGER_VARIABLE_ANIMATIONNAME,
                                              anim_name);
                    READ_MICRO_CHUNK_WWSTRING(
                        cload, ANIMCOLLISIONMANAGER_VARIABLE_PREVANIMATIONNAME, prev_anim_name);
    ...
    if (!prev_anim_name.Is_Empty()) {
        ...
        if (anim == NULL) {
            WWDEBUG_SAY(("FAILED TO FIND PREV ANIM IN "
                         "AnimCollisionManagerClass::Internal_Set_Animation(\"%s\")\n",
                         prev_anim_name));
        }
...
bool AnimCollisionManagerDefClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK_WWSTRING(csave, ANIMCOLLISIONMANAGERDEF_VARIABLE_ANIMATIONNAME,
                               AnimationName);
    csave.End_Chunk();
...
bool AnimCollisionManagerDefClass::Load(ChunkLoadClass& cload)
{
    while (cload.Open_Chunk()) {
        ...
        case ANIMCOLLISIONMANAGERDEF_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    READ_MICRO_CHUNK(cload, ANIMCOLLISIONMANAGERDEF_VARIABLE_COLLISIONMODE,
                                     CollisionMode);
                    READ_MICRO_CHUNK(cload, ANIMCOLLISIONMANAGERDEF_VARIABLE_ANIMATIONMODE,
                                     AnimationMode);
                    READ_MICRO_CHUNK_WWSTRING(cload, ANIMCOLLISIONMANAGERDEF_VARIABLE_ANIMATIONNAME,
                                              AnimationName);
                }
                cload.Close_Micro_Chunk();
            }
