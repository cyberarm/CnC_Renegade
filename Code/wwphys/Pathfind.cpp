...
bool Find_Intersection_Point(const AABoxClass& box, const Vector3& p0, const Vector3& p1,
                             float* percent, Vector3* intersection_point);
void Add_New_Portal_To_List(DynamicVectorClass<PathfindWaypathPortalClass*>& portal_list,
                            PathfindSectorClass* dest_sector1, PathfindSectorClass* dest_sector2,
                            int waypath_id, int waypoint_index, const Vector3& portal_pos,
                            float percent);
...
///////////////////////////////////////////////////////////////////////////
PathfindClass* PathfindClass::_Pathfinder = nullptr;
...
//
///////////////////////////////////////////////////////////////////////////
int PathfindClass::Add_Temporary_Portal(PathfindSectorClass* sector_from,
                                        PathfindSectorClass* sector_to, const Vector3& start_pos,
                                        const Vector3& dest_pos)
{
    int retval = 0;

    if (sector_from != nullptr && sector_to != nullptr) {
        ...
        if (found == false) {
            ...
            if (m_TemporaryPortalList.Count() > MAX_TEMP_PORTALS) {
                ...
                //
                PathfindActionPortalClass* old_portal
                    = (PathfindActionPortalClass*)m_TemporaryPortalList[0];
                if (old_portal != nullptr) {
...
bool PathfindClass::Load_Portal(ChunkLoadClass& cload, PathfindPortalClass* portal)
{
    ...
    if (retval) {
        ...
        //
        if (portal->As_PathfindWaypathPortalClass() == nullptr) {
            m_PortalList.Add(portal);
        }
        else {
...
//
///////////////////////////////////////////////////////////////////////////
void PathfindClass::Collect_Sectors(DynamicVectorClass<PathfindSectorClass*>& list,
                                    const AABoxClass& box, PathfindSectorClass* exclude_sector)
{
    //
    ...
    //
    PathfindSectorClass* sector = nullptr;
    for (sector = m_SectorTree.Get_First_Collected_Object(); sector != nullptr;
                                                ...
                                                PathfindSectorClass* exclude_sector)
{
    ...
    //
    PathfindSectorClass* closest_sector = nullptr;
    PathfindSectorClass* sector = nullptr;
    for (sector = m_SectorTree.Get_First_Collected_Object(); sector != nullptr;
         sector = m_SectorTree.Get_Next_Collected_Object(sector)) {
        if (sector != exclude_sector) {
...
int PathfindClass::Count_Waypaths_Starting_In_Box(const AABoxClass& box)
{
    ...
    for (int index = 0; index < m_WaypathList.Count(); index++) {
        WaypathClass* curr_waypath = m_WaypathList[index];
        if (CollisionMath::Overlap_Test(box, curr_waypath->Get_Point(0)->Get_Position())
...
//
//////////////////////////////////////////////////////////////////////////////////
float PathfindClass::Get_Height_Value(const Vector3& pos)
{
...
bool PathfindClass::Find_Random_Spot(const Vector3& center, float max_dist, Vector3* dest)
{
    ...
    //
    PathfindSectorClass* start_sector = Find_Sector(center, 0.1F, nullptr);
    if (start_sector != nullptr) {
...
void PathfindClass::Free_Waypath_Sectors_And_Portals(void)
{
    ...
    for (int index = 0; index < m_WaypathPortalList.Count(); index++) {
        ...
        if (dest_sector1 != nullptr) {
            dest_sector1->Remove_Portal(portal->Get_ID());
        }

        if (dest_sector2 != nullptr) {
...
void PathfindClass::Generate_Waypath_Sector_And_Portals(WaypathClass* waypath)
{
    ...
    for (int index = 0; index < waypoint_count; index++) {
        ...
        if (sector_at_pos != nullptr) {

            //
            //	Add a new portal to the list at this position
            //
            ::Add_New_Portal_To_List(portal_list, new_sector, sector_at_pos, waypath_id, index,
                                     waypoint_pos, 0);
        }
    }
...
//
//////////////////////////////////////////////////////////////////////////////////
void PathfindClass::Add_Intersection_Portals_To_List(
    DynamicVectorClass<PathfindWaypathPortalClass*>& portal_list, WaypathClass* waypath,
    PathfindWaypathSectorClass* dest_sector)
{
    ...
    for (int waypoint_index = 0; waypoint_index < waypoint_count - 1; waypoint_index++) {
        ...
        //
        PathfindSectorClass* sector = nullptr;
        for (sector = m_SectorTree.Peek_First_Collected_Object(); sector != nullptr;
...
//
//////////////////////////////////////////////////////////////////////////////////
void PathfindClass::Find_Portals(const Vector3& p0, const Vector3& p1,
                                 ...
                                 bool action_portals_only)
{
    ...
    for (int index = 0; index < m_PortalList.Count(); index++) {
        ...
        if (action_portals_only == false
            || (action_portal != nullptr && portal->Get_Action_Type() != PathClass::ACTION_NONE)) {
            ...
            if (::Find_Intersection_Point(box, p0, p1, &percent1, &intersection_pt)) {
                ...
                if (action_portals_only) {
                    is_valid = false;
                    PathfindSectorClass* sector = action_portal->Peek_Dest_Sector(nullptr);
                    if (sector != nullptr) {
                        ...
                        if (exit_portal != nullptr) {
                            ...
                            if (::Find_Intersection_Point(exit_box, p0, p1, &percent1,
                                                          &intersection_pt)) {
