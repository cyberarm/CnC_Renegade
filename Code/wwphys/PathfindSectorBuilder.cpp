...
public:
    ...
    SimDirInfoClass(float _heading, float _speed, const Vector3& _move, const Vector3& min,
                    const Vector3& max)
        : heading(_heading),
          speed(_speed),
          move(_move),
...
{
        SIM_DIR_INFO ( 0.0F,						 	 Vector3 ( 0.9F,
-0.1F, -1.5F), Vector3 ( 2.0F,  0.1F, 1.5F) ), SIM_DIR_INFO ( (float)DEG_TO_RAD (90),	 Vector3
(-0.1F,  0.9F, -1.5F), Vector3 ( 0.1F,  2.0F, 1.5F) ), SIM_DIR_INFO ( (float)DEG_TO_RAD (180),
Vector3 (-2.0F, -0.1F, -1.5F), Vector3 (-0.9F,  0.1F, 1.5F) ), SIM_DIR_INFO ( (float)DEG_TO_RAD
(270), Vector3 (-0.1F, -2.0F, -1.5F), Vector3 ( 0.1F, -0.9F, 1.5F) )
};*/
...
static const SimDirInfoClass DIR_INFO[DIR_MAX]
    = { SimDirInfoClass(0.0F, 0, Vector3(1.0F, 0, 0), Vector3(0.95F, -0.05F, -2.0F),
                        Vector3(1.2F, 0.05F, 2.0F)),
        SimDirInfoClass((float)DEG_TO_RAD(90), 0, Vector3(0, 1.0F, 0),
                        Vector3(-0.05F, 0.95F, -2.0F), Vector3(0.05F, 1.2F, 2.0F)),
        SimDirInfoClass((float)DEG_TO_RAD(180), 0, Vector3(-1.0F, 0, 0),
                        Vector3(-1.2F, -0.05F, -2.0F), Vector3(-0.95F, 0.05F, 2.0F)),
        SimDirInfoClass((float)DEG_TO_RAD(270), 0, Vector3(0, -1.0F, 0),
                        Vector3(-0.05F, -1.2F, -2.0F), Vector3(0.05F, -0.95F, 2.0F)) };
...
void PathfindSectorBuilderClass::Do_Physics_Sim(const Vector3& start_pos, PATHFIND_DIR direction)
{
    ...
    PhysAABoxCollisionTestClass test(box, Vector3(0, 0, -movement), &result,
                                     m_PhysicsSim->Get_Collision_Group(), COLLISION_TYPE_PHYSICAL);
            ...
            for (int dir = 0; dir < DIR_MAX; dir++) {
                ...
                if (cull_obj->Is_New_Portal((PATHFIND_DIR)dir)) {
                    PathfindPortalClass* portal
                        = cull_obj->Make_Portal((PATHFIND_DIR)dir, m_SimBoundingBox);
