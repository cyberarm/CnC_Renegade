...
*/
SimplePersistFactoryClass<TrackedVehicleClass, PHYSICS_CHUNKID_TRACKEDVEHICLE>
    _TrackedVehicleFactory;
...
{
    {
        ...
        */
        ...
        if (Controller) {
            ...
            float left = 0.0f;
            if (WWMath::Fabs(Controller->Get_Turn_Left())
                > WWMath::Fabs(Controller->Get_Move_Left())) {
...
SimplePersistFactoryClass<TrackedVehicleDefClass, PHYSICS_CHUNKID_TRACKEDVEHICLEDEF>
    _TrackedVehicleDefFactory;
...
bool TrackedVehicleDefClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, TRACKEDVEHICLEDEF_VARIABLE_TURNTORQUESCALEFACTOR,
                      TurnTorqueScaleFactor);
    csave.End_Chunk();
    return true;
}
        ...
        case TRACKEDVEHICLEDEF_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    READ_MICRO_CHUNK(cload, TRACKEDVEHICLEDEF_VARIABLE_MAXENGINETORQUE,
                                     MaxEngineTorque);
                    READ_MICRO_CHUNK(cload, TRACKEDVEHICLEDEF_VARIABLE_TRACKUSCALEFACTOR,
                                     TrackUScaleFactor);
                    READ_MICRO_CHUNK(cload, TRACKEDVEHICLEDEF_VARIABLE_TRACKVSCALEFACTOR,
                                     TrackVScaleFactor);
                    READ_MICRO_CHUNK(cload, TRACKEDVEHICLEDEF_VARIABLE_TURNTORQUESCALEFACTOR,
                                     TurnTorqueScaleFactor);
                }
