...
#define RBODY_DEBUG_FILTER                                                                         \
    (stricmp(Model->Get_Name(), "V_GDI_ORCA_M") == 0)                                              \
        && (PhysicsSceneClass::Get_Instance()->Is_Debug_Display_Enabled())
...
public:
    ...
    const RigidBodyStateStruct& Get_Historical_State(int i)
    {
        return SnapshotArray[Wrap_Index(HeadIndex + i)];
    }
    const Vector3& Get_Historical_Position(int i)
    {
        return SnapshotArray[Wrap_Index(HeadIndex + i)].Position;
    }
...
private:
    ...
    {
    public:
        StateSnapshotStruct(void)
            : Age(0)
        {
        }
        StateSnapshotStruct(const StateSnapshotStruct& that)
            : RigidBodyStateStruct(that)
        {
            Age = that.Age;
        }
...
void RBodyHistoryClass::Compute_Old_State(float t, RigidBodyStateStruct* set_state)
{
    ...
    RigidBodyStateStruct::Lerp(SnapshotArray[prev_index], SnapshotArray[next_index], fraction,
                               set_state);
}
...
void RBodyHistoryClass::Apply_Correction(const RigidBodyStateStruct& error, float fraction)
{
    ...
    for (int counter = 0; counter < RBODY_SNAPSHOT_COUNT; counter++) {
        ...
        lerp_fraction = fraction * (RBODY_HISTORY_MIN_TIME - SnapshotArray[index].Age)
            / RBODY_HISTORY_MIN_TIME;
#endif
        RigidBodyStateStruct::Lerp(SnapshotArray[index], ideal_state, lerp_fraction,
                                   &(SnapshotArray[index]));
    }
#endif
}

void RBodyHistoryClass::Find_Nearest_State(const RigidBodyStateStruct& input,
                                           RigidBodyStateStruct* output)
{
    /*
...
void RBodyHistoryClass::StateSnapshotStruct::Lerp(const StateSnapshotStruct& a,
                                                  const StateSnapshotStruct& b, float fraction)
{
...
**   - if the resultant relative velocity is low enough, add a contact constraint/spring to the
*system
**
** Sept 23, 1999
...
** - During each timestep (and sub-timestep) each pad will push out from its face until it hits
*something
**   or reaches the edge of the outer box.  If it hits something, a contact will be set up which
*records
**   the position, normal, velocity, and surface parameters.
** - Normally all contacts could behave like damped springs.
** - If the inner box is penetrated, we could search for a time when the outer is penetrated, the
*inner
...
bool RigidBodyClass::Cast_AABox(PhysAABoxCollisionTestClass& boxtest)
{
    ...
    if (CollisionMath::Collide(boxtest.Box, boxtest.Move, ContactBox->Get_World_Inner_Box(),
...
bool RigidBodyClass::Can_Teleport(const Matrix3D& new_tm, bool check_dyn_only,
...
void RigidBodyClass::Network_Interpolate_State_Update(const Vector3& new_pos,
                                                      const Quaternion& new_orientation,
                                                      const Vector3& new_vel,
                                                      const Vector3& new_avel, float fraction)
{
...
void RigidBodyClass::Network_Latency_State_Update(const Vector3& new_pos,
                                                  const Quaternion& new_orientation,
...
void RigidBodyClass::Network_Latency_Error_Correction(float dt)
{
    if (LatencyError.Position.Length2() > 0.01f) {
        ...
        if (!LatencyError.Is_Valid()) {
            ...
            WWDEBUG_SAY((" position error: %f, %f, %f\r\n", LatencyError.Position.X,
                         LatencyError.Position.Y, LatencyError.Position.Z));
            WWDEBUG_SAY((" orientation error: %f, %f, %f, %f\r\n", LatencyError.Orientation.X,
                         LatencyError.Orientation.Y, LatencyError.Orientation.Z,
                         LatencyError.Orientation.W));
            WWDEBUG_SAY((" L momentum error: %f, %f, %f\r\n", LatencyError.LMomentum.X,
                         LatencyError.LMomentum.Y, LatencyError.LMomentum.Z));
            WWDEBUG_SAY((" A momentum error: %f, %f, %f\r\n", LatencyError.AMomentum.X,
                         LatencyError.AMomentum.Y, LatencyError.AMomentum.Z));
        }
#endif
        if (!LatencyError.Is_Valid()) {
            LatencyError.Reset();
...
int RigidBodyClass::Compute_Derivatives(float t, StateVectorClass* test_state,
                                        StateVectorClass* dydt, int index)
{
    if (test_state) {
...
#pragma message("(gth) HACK! zeroing bad velocities.")
        ...
        if ((Velocity.Is_Valid() == false) || (force->Is_Valid() == false)
        ...
        *force
            -= def->AerodynamicDragCoefficient * Vector3::Dot_Product(Velocity, Velocity) * vel_dir;
...
#pragma message("(gth) HACK! zeroing bad angular velocities.")
    ...
    if (a_dir.Length2() > WWMATH_EPSILON) {
        ...
        *torque -= PhysicsConstants::AngularDamping
            * Vector3::Dot_Product(AngularVelocity, AngularVelocity) * a_dir;
        WWASSERT(torque->Is_Valid());
    }
        ...
        for (int i = 0; i < ContactBox->Get_Contact_Count(); i++) {
            ...
            */
            ...
            float force_magnitude
                = ContactBox->Get_Stiffness() * dx - ContactBox->Get_Damping() * dv;
            ...
            if ((contact_force.Is_Valid() == false) || (contact_torque.Is_Valid() == false)) {
                WWDEBUG_SAY(("bad contact: normal=(%8.3f,%8.3f,%8.3f) r=(%8.3f,%8.3f,%8.3f) "
                             "dx=%8.3f dv=%8.3f\n",
                             contact.Normal.X, contact.Normal.Y, contact.Normal.Z, r.X, r.Y, r.Z,
                             dx, dv));
            ...
            */
            ...
            if (Get_Flag(FRICTION_DISABLED) == false) {
                ...
                Vector3 tan_vel
                    = pdot - Vector3::Dot_Product(pdot, contact.Normal) * contact.Normal;
                ...
                if (tan_vel_magnitude > WWMATH_EPSILON * WWMATH_EPSILON) {
                    ...
                    */
                    ...
                    /*
                    ** The magnitude is the friction coefficient times the portion of the weight
                    *supported
                    ** by this contact.  As the velocity approaches zero, this force approaches zero
                    */
                    float friction_magnitude = friction_coefficient
                        * WWMath::Min(0.1f * tan_vel_magnitude * contact_weight, contact_weight);
                    ...
                    */
                    if (contact.Normal.Z > 0.0f) {
                        friction_force -= gravity
                            - Vector3::Dot_Product(gravity, contact.Normal) * contact.Normal;
                    }
...
inline void RigidBodyClass::Dump_State(void) const
{
    ...
    WWDEBUG_SAY(("LMomentum:	%10.10f , %10.10f , %10.10f \r\n", State.LMomentum.X,
                 State.LMomentum.Y, State.LMomentum.Z));
    WWDEBUG_SAY(("AMomentum:	%10.10f , %10.10f , %10.10f \r\n", State.AMomentum.X,
                 State.AMomentum.Y, State.AMomentum.Z));
                        ...
                        if (dot < 0.0f) {
                            ...
                            */
                            ...
                            if ((other_obj != nullptr) && (other_obj->As_RigidBodyClass() != nullptr)) {
                                ...
                                remaining_time = 0.0f; // I don't want to resolve a crowd chain
                                                       // reaction all in one frame
                            }
                            else {
                ...
                */
                ...
                if (RBODY_DEBUG_FILTER) {
                    WWDEBUG_SAY(("Rigid Body Object: %s is intersecting (%f,%f,%f).\n",
                                 Model->Get_Name(), State.Position.X, State.Position.Y,
                                 State.Position.Z));
...
bool RigidBodyClass::Push_Phys3_Object_Away(Phys3Class* p3obj, const CastResultStruct& contact)
{
    ...
    */
    ...
    if (RBODY_DEBUG_FILTER) {
        WWDEBUG_SAY(
            ("  Pushing Phys3 Object %x away (%f, %f, %f)\r\n", p3obj, move.X, move.Y, move.Z));
    }
#endif
    ...
    ** for that first.
    */
    if (PhysicsSceneClass::Get_Instance()->Do_Groups_Collide(p3obj->Get_Collision_Group(),
                                                             Get_Collision_Group())) {
        AABoxClass p3box;
...
void RigidBodyClass::Assert_Not_Intersecting(void)
{
    ...
    PhysOBBoxCollisionTestClass test(obbox, Vector3(0, 0, 0), &result, Get_Collision_Group(),
                                     COLLISION_TYPE_PHYSICAL);
    PhysicsSceneClass::Get_Instance()->Cast_OBBox(test);
    //	WWASSERT(result.StartBad != true);
    if (result.StartBad) {
        WWDEBUG_SAY(
            ("   !!!!!!!!!!!!!!!!!!!!!!!!!   Rigid Body %s intersecting!\r\n", Model->Get_Name()));
    }
    else {
...
bool RigidBodyClass::Push(const Vector3& move)
{
    ...
    */
    ...
    CastResultStruct result;
    PhysOBBoxCollisionTestClass test(box, move, &result, Get_Collision_Group(),
                                     COLLISION_TYPE_PHYSICAL);
...
*/
DECLARE_DEFINITION_FACTORY(RigidBodyDefClass, CLASSID_RIGIDBODYDEF, "RigidBody")
_RigidBodyDefDefFactory;
...
bool RigidBodyDefClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, RIGIDBODYDEF_VARIABLE_AERODYNAMICDRAGCOEFFICIENT,
...
bool RigidBodyDefClass::Load(ChunkLoadClass& cload)
{
    while (cload.Open_Chunk()) {
        ...
        case RIGIDBODYDEF_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    READ_MICRO_CHUNK(cload, RIGIDBODYDEF_VARIABLE_AERODYNAMICDRAGCOEFFICIENT,
                                     AerodynamicDragCoefficient);
                    READ_MICRO_CHUNK(cload, RIGIDBODYDEF_VARIABLE_COLLISIONDISABLED,
                                     CollisionDisabled);
                }
                cload.Close_Micro_Chunk();
            }
