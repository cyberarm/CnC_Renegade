...
public:
    ...
    void Light_Terrain(LightSolveContextClass& context, RenegadeTerrainPatchClass* patch,
...
void VertexSolveClass::Light_Mesh(LightSolveContextClass& context, MeshClass* mesh,
                                  NonRefPhysListClass& lights)
{
    int vi;
    ...
    **			- if this vertex is supposed to use a color array in any pass copy the color
    *from the array *			- else copy the material setting from pass0
    **
    ...
    */
    ...
    for (vi = 0; vi < model->Get_Vertex_Count(); vi++) {
        ...
        for (int pi = 0; pi < model->Get_Pass_Count(); pi++) {
            ...
            if ((pass_mtl != nullptr) && (dcg != nullptr)
    ...
    */
    for (vi = 0; vi < vcount; vi++) {
        Solve[vi].X
            = AmbientSolve[vi].X * MeshAmbient[vi].X + DiffuseSolve[vi].X * MeshDiffuse[vi].X;
        Solve[vi].Y
            = AmbientSolve[vi].Y * MeshAmbient[vi].Y + DiffuseSolve[vi].Y * MeshDiffuse[vi].Y;
        Solve[vi].Z
            = AmbientSolve[vi].Z * MeshAmbient[vi].Z + DiffuseSolve[vi].Z * MeshDiffuse[vi].Z;
...
void VertexSolveClass::Light_Terrain(LightSolveContextClass& context,
                                     RenegadeTerrainPatchClass* patch, NonRefPhysListClass& lights)
{
    int vi;
...
void VertexSolveClass::Add_Light_To_Vertex(LightSolveContextClass& context, int vi,
                                           LightClass* light_obj)
{
...
void LightSolveClass::Compute_Solve(LightSolveContextClass& context, StaticPhysClass* obj)
{
    ...
    */
    if (obj->Is_Model_Pre_Lit() == false) {
        ...
        PhysicsSceneClass::Get_Instance()->Collect_Lights(obj->Get_Cull_Box(), true, false,
                                                          &light_list);
