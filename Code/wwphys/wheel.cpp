 ...
 *   12/18/2000 gth : Created.                                                                 *
 *=============================================================================================*/
void WheelClass::Init(VehiclePhysClass* obj, int position_bone, int rotation_bone, int fork_bone,
                      int axis_bone)
{
 ...
 *   12/18/2000 gth : Created.                                                                 *
 *=============================================================================================*/
void WheelClass::Compute_Suspension_Force(const Vector3& pdot, const Vector3& local_pdot,
                                          Vector3* suspension_force)
{
    ...
    float max_sforce = MAX_SUSPENSION_ACCEL * Parent->Get_Mass()
        / WWMath::Max(Parent->Get_Real_Wheel_Count(), 1.0f);
...
void WheelClass::Apply_Forces(Vector3* force, Vector3* torque)
{
    ...
    *torque += Vector3::Cross_Product(r_tractive, TractiveFrictionForce);
    ...
    Parent->Add_Debug_Vector(position + r_lateral, SuspensionForce / Parent->Get_Mass(),
                             SUSPENSION_FORCE_COLOR);
    Parent->Add_Debug_Vector(position + r_lateral, LateralFrictionForce / Parent->Get_Mass(),
                             LATERAL_FORCE_COLOR);
    Parent->Add_Debug_Vector(position + r_tractive, TractiveFrictionForce / Parent->Get_Mass(),
                             TRACTIVE_FORCE_COLOR);
#endif
}
                                           ...
                                           float* set_lateral_force, float* set_tractive_force)
{
    ...
    if (wheel_normal_force > 0.0f) {
        ...
        float drag_coefficient = PhysicsConstants::Get_Contact_Drag_Coefficient(
            PhysicsConstants::DYNAMIC_OBJ_TYPE_TIRE, ContactSurface);
        ...
        float tractive_force
            = WWMath::Min((penalty_k * wheel_normal_force * WWMath::Fabs(local_pdot.X)),
                          (wheel_normal_force * tractive_friction_coefficient));

        float lateral_force
            = WWMath::Min((penalty_k * wheel_normal_force * WWMath::Fabs(local_pdot.Y)),
...
void VTOLWheelClass::Compute_Traction_Forces(const Vector3& local_pdot, float normal_force,
                                             float* set_lateral_force, float* set_tractive_force)
{
