...
void HumanPhysClass::Timestep(float dt)
{
...

    {
        ...
        // if we didn't jump, we're not on the ground, we were on the ground, and our Z velocity is
        // >0, set it to 0
...
bool HumanPhysClass::Normal_Move(const GroundStateStruct& gs, float dt)
{
    ...
    */
    ...
    if (!JustJumped) {
        State.Velocity.Z
            = WWMath::Min(State.Velocity.Z, 0.0f); // don't ever let humans launch off slopes
    }
 ...
 *   9/16/2000  gth : Created.                                                                 *
 *=============================================================================================*/
void HumanPhysClass::Compute_Desired_Move_Vector(const GroundStateStruct& gs, float dt,
                                                 Vector3* set_move)
{
...
void HumanPhysClass::Jump_To_Point(const Vector3& dest_pos)
{
    ...
    if (displacement.Z >= 0.0f) {
        ...
        //	viy = y / t - 0.5 * g * t (3)
        //
        //	where vfy is final vertical velocity, viy is initial vertical velocity, g is
        //acceleration
...
*/
DECLARE_DEFINITION_FACTORY(HumanPhysDefClass, CLASSID_HUMANPHYSDEF, "Human")
_HumanPhysDefDefFactory;
