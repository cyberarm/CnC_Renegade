        ...
        if (portal != nullptr) {
            ...
            if (action_portal != nullptr) {
                ...
                if (index + 1 < raw_path.Count()) {
                    PathfindWaypathPortalClass* next_portal
                        = raw_path[index + 1].m_Portal->As_PathfindWaypathPortalClass();
                    if (next_portal != nullptr) {
                        ...
                        //
                        Add_Waypath_Data(node_list, path_segment, 0,
                                         path_segment->Get_Point_Count() - 1);
                ...
                /*if (index < (raw_path.Count () - 1) && raw_path[index+1].m_Portal != nullptr) {
                        ...
                        if (curr_box.Center.Z > (next_box.Center.Z + 1.5F)) {
                                ...
                                if (portal_dist < 665.0F) {
                                        ...
                                        //
                                        //	Calculate an intermediate point that is one meter
                towards
                                        // the center of the next sector
                                        //
                                        Vector3 temp_delta = (raw_path[index+1].m_SectorCenter -
                next_box.Center); temp_delta.Normalize (); Vector3 intermediate_point =
                next_box.Center + temp_delta;
                                        ...
                                        temp_node.tighten_spline	= true;
                                        temp_node.pos					=
                intermediate_point; temp_node.action_id			= ACTION_JUMP;
...
//
///////////////////////////////////////////////////////////////////////////
void PathClass::Add_Waypath_Data(DynamicVectorClass<PATH_NODE>& node_list, WaypathClass* waypath,
                                 int start_index, int end_index)
{
    ...
    for (int index = start_index; count >= 0; index += inc, count--) {
        ...
        //
        if ((waypoint == nullptr || next_point == nullptr)
            || waypoint->Get_Position() != next_point->Get_Position()) {
...
bool PathClass::Evaluate_Next_Point(const Vector3& curr_pos, Vector3& new_pos)
{
    ...
    if (m_DisplayMoveVectors) {
        ...
        PhysicsSceneClass::Get_Instance()->Add_Debug_AABox(
            AABoxClass(m_ExpectedPos, Vector3(0.25F, 0.25F, 0.25F)), Vector3(0, 1, 0));
        PhysicsSceneClass::Get_Instance()->Add_Debug_Vector(curr_pos, m_ExpectedPos - curr_pos,
                                                            Vector3(0, 0.6F, 1));
    }
...
//
///////////////////////////////////////////////////////////////////////////
float PathClass::Get_Last_Eval_Time(void) const
{
    return m_SplineTime;
}
...
//
/////////////////////////////////////////////////////////////////////////////////
bool Find_Intersection_X(const Vector3& start, const Vector3& end, const Vector3& rgn_center,
                         const Vector3& rgn_extent, Vector3* intersect_point)
{
...
//
/////////////////////////////////////////////////////////////////////////////////
bool Find_Intersection_Y(const Vector3& start, const Vector3& end, const Vector3& rgn_center,
                         const Vector3& rgn_extent, Vector3* intersect_point)
{
...
//
/////////////////////////////////////////////////////////////////////////////////
bool Find_Side_Intersection(int index, const AABoxClass& box, const Vector3& start,
                            const Vector3& end, Vector3* intersection_point)
{
    bool retval = false;
...
//
/////////////////////////////////////////////////////////////////////////////////
void PathClass::Clip_Control_Point(const Vector3& start_point, Vector3* point,
                                   BOX_LIST& sector_list, BOX_LIST& portal_list)
{
    //
...
//
/////////////////////////////////////////////////////////////////////////////////
void PathClass::Clip_Spline_To_Pathfind_Data(DynamicVectorClass<PATH_NODE>& node_list,
                                             PathSolveClass& path_solve)
{
    ...
    for (int index = 0; index < node_list.Count() - 1; index++) {
        ...
        ((HermiteSpline3DClass*)m_Spline)
            ->Get_Tangents(index + 1, &next_tangent_in, &next_tangent_out);
        ...
        ((HermiteSpline3DClass*)m_Spline)
            ->Set_Tangents(index + 1, next_tangent_in, next_tangent_out);
    }
