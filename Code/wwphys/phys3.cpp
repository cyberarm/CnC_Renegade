...
public:
    ...
    Vector3 Get_Historical_Position(int i)
    {
...
private:
    ...
    {
    public:
        StateSnapshotClass(void)
            : Position(0, 0, 0),
              Velocity(0, 0, 0),
              Age(0)
        {
        }
        StateSnapshotClass(const Vector3& pos, const Vector3& vel, float age)
            : Position(pos),
              Velocity(vel),
              Age(age)
        {
...
void Phys3HistoryClass::Apply_Correction(const Vector3& pos_correction)
{
    ...
    for (int counter = 0; counter < PHYS3_SNAPSHOT_COUNT; counter++) {
        ...
        float fraction
            = (PHYS3_HISTORY_MIN_TIME - SnapshotArray[index].Age) / PHYS3_HISTORY_MIN_TIME;
        if (fraction > 0.0f) {
...
void Phys3HistoryClass::Find_Nearest_Point(const Vector3& pos, const Vector3& vel,
                                           Vector3* set_point)
{
    /*
    ** Find the nearest line segment
    */
    ...
    for (int counter = 0; counter < PHYS3_SNAPSHOT_COUNT - 1; counter++) {
        ...
        */
        ...
        Vector3::Lerp(SnapshotArray[index0].Velocity, SnapshotArray[index1].Velocity, fraction,
                      &history_vel);
...
void Phys3HistoryClass::StateSnapshotClass::Lerp(const StateSnapshotClass& a,
                                                 const StateSnapshotClass& b, float fraction)
{
...
static const float DEFAULT_STEP_HEIGHT
    = 0.25f; // the distance an object will "step up" over an obstacle
static const float DEFAULT_SLIDE_ANGLE
    = DEG_TO_RADF(45.0f); // steepest angle the character can walk up
...
static const float WALL_EPSILON
    = 0.5f; //(GROUND_DISTANCE - 0.001f);	// Stop at this distance from walls/slides

static const float MIN_STEP_MOVE = 0.25f; // only try to step if we could move this distance
static const float MAX_STEP_MOVE_ANGLE_TAN
    = 1.0f; // only step if moving at an angle close to the x-y plane
...
bool Phys3Class::Cast_OBBox(PhysOBBoxCollisionTestClass& boxtest)
{
    ...
    if (CollisionMath::Collide(boxtest.Box, boxtest.Move, wrldbox, Vector3(0, 0, 0),
                               boxtest.Result)) {
        boxtest.CollidedPhysObj = this;
        return true;
    }
...
Phys3Class::GroundStateStruct& Phys3Class::Get_Ground_State(void)
{
    ...
    VERBOSE_LOG((" GS: OnGround = %d Normal = %f %f %f\r\n", GroundState.OnGround,
                 GroundState.Normal.X, GroundState.Normal.Y, GroundState.Normal.Z));
...
void Phys3Class::Check_Ground(const AABoxClass& box, GroundStateStruct* gs, float check_dist)
{
    ...
    PhysAABoxCollisionTestClass test(box, Vector3(0, 0, -check_dist), &result,
                                     Get_Collision_Group(), COLLISION_TYPE_PHYSICAL);
 ...
 *   9/16/2000  gth : Created.                                                                 *
 *=============================================================================================*/
bool Phys3Class::Apply_Move(const Vector3& input_move, float dt, bool allow_sliding,
                            bool allow_stepping, bool stop_on_walkable)
{
    ...
    **       - ELSE
    **			  - If (I can push the obstacle out of my way) push obstacle, go back to top
    *of loop *			  - If (I can shatter this object) shatter it, go back to top of
    *loop *			  - If (This contact is a slope, and I'm trying to walk up it)
    *modify contact to act like a wall
    ...
    ** - Can't always treat slides like walls since we can fall onto them or walk onto them from the
    *top...
    ** - Trying to get stepping for free usually by just always bumping the character up before
    *taking his move
    **
    */
    ...
    while (!done) {
        ...
        */
        ...
        CastResultStruct result;
        PhysAABoxCollisionTestClass test(box, move, &result, Get_Collision_Group(),
            ...
            if (result.Fraction == 1.0f) {
                ...
                */
                ...
                VERBOSE_LOG(("Collided with %s. normal=(%f,%f,%f)\r\n",
                             test.CollidedRenderObj->Get_Name(), result.Normal.X, result.Normal.Y,
                             result.Normal.Z));
                collided = true;
                ...
                */
                if ((dt < WWMATH_EPSILON) || (move.Length2() < WWMATH_EPSILON)
                    || (allow_sliding == false)) {
                    ...
                    */
                    ...
                    if (reaction & COLLISION_REACTION_NO_BOUNCE) {
                        ...
                        */
                        if (clip_move) {
                            ...
                            if (p3obj != nullptr) {
                                ...
                                */
                                ...
                                dc.Normalize();
                                float fraction = 1.0f;
                                if ((Mass > WWMATH_EPSILON)
                                    && (p3obj->Get_Mass() > WWMATH_EPSILON)) {
                        ...
                        /*
                        ** SHATTER: If the mesh is shatterable, shatter it.  We are doing this so
                        *that
                        ...
                        */
                        ...
                        /*
                        ** CLIP: If the obstacle didn't move out of our way and we didn't step over
                        *it,
                        ...
                        */
                        if (clip_move) {
                            ...
                            if (contact_count < MAX_CONTACTS) {
                                ...
                                if ((normal.Z < SlideNormalZ) && (normal.Z > 0.0f)
                                    && (GroundState.OnGround)
                                ...
                                VERBOSE_LOG(("exceeded max contacts in Phys3::Apply_Move! "
                                             "position: %f, %f, %f\r\n",
                                             State.Position.X, State.Position.Y, State.Position.Z));
                                done = true;
                            }
...
inline bool Phys3Class::Debug_Verify_Position(void)
{
    ...
    if (intersecting) {
        VERBOSE_LOG(("		%s is intersecting %s!\r\n", Model->Get_Name(),
                     list.Peek_Head()->Peek_Model()->Get_Name()));
    }
    else {
...
void Phys3Class::Snap_To_Ground(const Vector3& actual_move, bool was_stepping)
{
    ...
    */
    if (delta_z + delta_xy > 0.0f) {
        ...
        Compute_WS_Collision_Box(State, &box);
        CastResultStruct result;
        PhysAABoxCollisionTestClass test(box, Vector3(0, 0, -snap_dist), &result,
                                         Get_Collision_Group());
        PhysicsSceneClass::Get_Instance()->Cast_AABox(test);
    ...
    for (int i = 0; i < contact_count; i++) {
        ...
        if (fabs(contacts[i].Length() - 1.0f) > WWMATH_EPSILON) {
            WWDEBUG_SAY(("Bad Contact Normal: %f %f %f  Length = %f\r\n", contacts[i].X,
                         contacts[i].Y, contacts[i].Z, contacts[i].Length()));
        }
#endif
        if (!(fabs(contacts[i].Length() - 1.0f) < WWMATH_EPSILON)) {
            *move = Vector3(0, 0, 0);
 ...
 *   9/16/2000  gth : Created.                                                                 *
 *=============================================================================================*/
bool Phys3Class::Can_Teleport(const Matrix3D& test_tm, bool check_dyn_only,
                              NonRefPhysListClass* result_list)
{
    ...
    //
    PhysAABoxIntersectionTestClass test(collision_box, Get_Collision_Group(),
                                        COLLISION_TYPE_PHYSICAL, result_list);
...
bool Phys3Class::Can_Teleport_And_Stand(const Matrix3D& test_tm, Matrix3D* new_tm)
{
    ...
    PhysAABoxCollisionTestClass test(collision_box, move_vector, &result, 0,
                                     COLLISION_TYPE_PHYSICAL);
    ...
    if ((result.StartBad == false)
        && ((result.Fraction == 1.0F) || (result.Normal.Z >= SlideNormalZ))) {
...
{
    DEBUG_RENDER_AABOX(AABoxClass(CollisionBox.Center + net_pos, CollisionBox.Extent),
                       Vector3(0, 1, 0), 0.25f);
                                                               ...
                                                               float height)
{
    ...
    if (test.Result->Fraction < 1.0f) {
        ...
        if (test.Result->StartBad) {
            ...
            IsDirty = true;
            Normal.Set(0, 0, 1);
            //			VERBOSE_LOG(("ERROR - intersecting object:
            ...
            //normal=(%f,%f,%f)sliding=%d\r\n",gs->Normal.X,gs->Normal.Y,gs->Normal.Z,(gs->Normal.Z>SlideNormalZ
            //? 0 : 1)));
        }
    }
    else {
