...
*/
SimplePersistFactoryClass<WheeledVehicleClass, PHYSICS_CHUNKID_WHEELEDVEHICLE>
    _WheeledVehicleFactory;
        ...
        for (int iwheel = 0; iwheel < Wheels.Length(); iwheel++) {
            ...
            */
            if (Wheels[iwheel]->Get_Flag(SuspensionElementClass::ENGINE)) {
                ((WVWheelClass*)Wheels[iwheel])
                    ->Set_Axle_Torque(Get_Axle_Torque() / Get_Drive_Wheel_Count());
            }
        }
...
bool WheeledVehicleClass::Drive_Wheels_In_Contact(void)
{
    for (int i = 0; i < Wheels.Length(); i++) {
        if (Wheels[i]->Get_Flag(SuspensionElementClass::ENGINE)
            && Wheels[i]->Get_Flag(SuspensionElementClass::INCONTACT)) {
            return true;
        }
    }
    return false;
...
SimplePersistFactoryClass<WheeledVehicleDefClass, PHYSICS_CHUNKID_WHEELEDVEHICLEDEF>
    _WheeledVehicleDefFactory;
    ...
    : MaxSteeringAngle(DEFAULT_MAX_STEER_ANGLE)
{
    ...
    ANGLE_EDITABLE_PARAM(WheeledVehicleDefClass, MaxSteeringAngle, DEG_TO_RADF(0.0f),
                         DEG_TO_RADF(90.0f));
}
        ...
        case WHEELEDVEHICLEDEF_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, WHEELEDVEHICLEDEF_VARIABLE_MAXSTEERINGANGLE,
                                     MaxSteeringAngle);

                    // NOTE: these variables are now in VehiclePhysDefClass...
                    READ_MICRO_CHUNK(cload, OBSOLETE_WHEELEDVEHICLEDEF_VARIABLE_SPRINGCONSTANT,
                                     SpringConstant);
                    READ_MICRO_CHUNK(cload, OBSOLETE_WHEELEDVEHICLEDEF_VARIABLE_DAMPINGCONSTANT,
                                     DampingConstant);
                    READ_MICRO_CHUNK(cload, OBSOLETE_WHEELEDVEHICLEDEF_VARIABLE_SPRINGLENGTH,
                                     SpringLength);
                    READ_MICRO_CHUNK(cload, OBSOLETE_WHEELEDVEHICLEDEF_VARIABLE_TRACTIONMULTIPLIER,
                                     TractionMultiplier);
                    READ_MICRO_CHUNK(cload, OBSOLETE_WHEELEDVEHICLEDEF_VARIABLE_LATERALMOMENTARM,
                                     LateralMomentArm);
                    READ_MICRO_CHUNK(cload, OBSOLETE_WHEELEDVEHICLEDEF_VARIABLE_TRACTIVEMOMENTARM,
                                     TractiveMomentArm);

                    OBSOLETE_MICRO_CHUNK(
                        OBSOLETE_WHEELEDVEHICLEDEF_VARIABLE_TRACTIVEFRICTIONCURVEFILENAME);
                    OBSOLETE_MICRO_CHUNK(
                        OBSOLETE_WHEELEDVEHICLEDEF_VARIABLE_LATERALFRICTIONCURVEFILENAME);
                }
