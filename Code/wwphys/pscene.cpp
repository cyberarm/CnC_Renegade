        ...
        while (remaining > 0) {
            ...
            */
            ...
            for (it.First(); !it.Is_Done(); it.Next()) {
                ...
                // Little optimization hack - only update vehicles that are visible (for now update
                // all other physics objects regardless of the visibility to avoid problems,
                // vehicles are the most expensive anyway). This same thing is done to Post Timestep
                // couple lines lower.
...
void PhysicsSceneClass::Unregister(RenderObjClass* obj, RegType for_what)
{
    ...
    case RELEASE:
        WWASSERT_PRINT(
            0,
            ("Error! Object %s tried to un-register from the release list\r\n", obj->Get_Name()));
        break;
    }
}
    ...
    if (!use_umbra) {
        // Get the lists of visible objects
        {
            ...
            StaticCullingSystem->Collect_Visible_Objects(
                camera.Get_Frustum(), pvs, VisibleStaticObjectList, VisibleWSMeshList);
        }

        // Collect list of the visible dynamic objects
        {
            WWPROFILE("Collect Dynamic Objs");
            DynamicCullingSystem->Collect_Visible_Objects(camera.Get_Frustum(), pvs,
                                                          VisibleDynamicObjectList);
        }

        // LOD processing
        Optimize_LODs(camera, &VisibleDynamicObjectList, &VisibleStaticObjectList,
                      &VisibleWSMeshList);
 ...
 *   7/7/2000   gth : Created.                                                                 *
 *=============================================================================================*/
void PhysicsSceneClass::Optimize_LODs(CameraClass& camera, RefPhysListClass* dyn_objs,
    ...
    */
    if (VisGridDisplayMode == VIS_GRID_DISPLAY_ACTUAL_BOXES) {
        ...
        if ((pvs != nullptr) && (DynamicObjVisSystem != nullptr)) {
            DynamicObjVisSystem->Render_Visible_Cells(
                rinfo, pvs, DynamicAABTreeCullClass::DISPLAY_ACTUAL_BOXES);
        }
        REF_PTR_RELEASE(pvs);
    }
...
void PhysicsSceneClass::Render_Object(RenderInfoClass& context, PhysClass* obj)
{
    ...
    */
    bool do_lighting
        = ((obj->Is_Pre_Lit() == false) && (obj->Peek_Model()->Is_Not_Hidden_At_All()));
        ...
        if (LightingDebugDisplayEnabled) {
            if ((pos - context.Camera.Get_Position()).Length2() < 30.0f * 30.0f) {
                for (int i = 0; i < light_env.Get_Light_Count(); i++) {
                    DEBUG_RENDER_VECTOR(pos, light_env.Get_Light_Direction(i),
                                        light_env.Get_Light_Diffuse(i));
                }
            }
        }
#endif
 ...
 *   4/24/2001  gth : Created.                                                                 *
 *=============================================================================================*/
void PhysicsSceneClass::Render_Backface_Occluders(RenderInfoClass& context,
...
void PhysicsSceneClass::Re_Partition_Dynamic_Culling_System(void)
{
    ...
    DynamicObjVisSystem->Re_Partition(nullptr, wmin, wmax, MIN_GRID_CELL_SIZE, MAX_GRID_CELL_COUNT,
 ...
 *   7/7/2000   gth : Created.                                                                 *
 *=============================================================================================*/
void PhysicsSceneClass::Re_Partition_Dynamic_Culling_System(
    DynamicVectorClass<AABoxClass>& virtual_occludees)
{
    Vector3 wmin, wmax;
    Get_Level_Extents(wmin, wmax);
    ...
    DynamicObjVisSystem->Re_Partition(&virtual_occludees, bounds.Center - bounds.Extent,
                                      bounds.Center + bounds.Extent, MIN_GRID_CELL_SIZE,
                                      MAX_GRID_CELL_COUNT, MAX_DYNAMIC_OBJ_RADIUS);
...
void PhysicsSceneClass::Per_Frame_Statistics_Update(void)
{
    ...
    if (CurrentStats.FrameCount >= STATISTICS_FRAMES) {
        ...
        */
        ...
        const AABTreeCullSystemClass::StatsStruct& static_stats
            = StaticCullingSystem->Get_Statistics();
        const AABTreeCullSystemClass::StatsStruct& light_stats
            = StaticLightingSystem->Get_Statistics();

        CurrentStats.CullNodeCount
            = dyn_stats.NodeCount + static_stats.NodeCount + light_stats.NodeCount;
        CurrentStats.CullNodesAccepted
            = dyn_stats.NodesAccepted + static_stats.NodesAccepted + light_stats.NodesAccepted;
        CurrentStats.CullNodesTriviallyAccepted = dyn_stats.NodesTriviallyAccepted
            + static_stats.NodesTriviallyAccepted + light_stats.NodesTriviallyAccepted;
        CurrentStats.CullNodesRejected
            = dyn_stats.NodesRejected + static_stats.NodesRejected + light_stats.NodesRejected;
 ...
 *   7/7/2000   gth : Created.                                                                 *
 *=============================================================================================*/
void PhysicsSceneClass::Shatter_Mesh(MeshClass* mesh, const Vector3& impact_point,
                                     const Vector3& impact_normal, const Vector3& impact_velocity)
{
    ...
    */
    for (int i = 0; i < ShatterSystem::Get_Fragment_Count(); i++) {
        ...
        */
        ...
        /*
        ** Debugging, Stop the velocity, set life to infinite, move along initial velocity to offset
        *pieces
        */
#if (SHATTER_DEBUG)
...
void PhysicsSceneClass::Add_Camera_Shake(const Vector3& position, float radius, float duration,
                                         float power)
{
