...
#define UNREGISTER_DIALOG(dialog) DialogMgrClass::UnRegister_Dialog(dialog);
    ...
    try {
        ...
        if (Check_Source()) {
            ...
            while (CardSelectionDialog->Get_Card_Count() > 1) {
                ...
                WW3D_Shutdown();
                if (WW3D_Initialize(cardselection)) {
                    break;
                }
            ...
            if (!cancelapplication) {
                ...
                if (!Beta_Test()) {
                    ...
                    // If the user cancelled during the intro movie, then don't bother with the
                    // transition.
                    if (!CancelIntroduction) {
                        ...
                        while (true) {
                            ...
                            if (GameInFocus) {
                                DialogMgrClass::Render();
                                if (TransitionMusic != nullptr) {
                                    if (TransitionMusic->Get_State()
                                        == AudibleSoundClass::STATE_PAUSED) {
                                        TransitionMusic->Resume();
                                    }
                                }
                                if (!timer.Is_Active()) {
                                    timer.Start();
                                ...
                                if (TransitionMusic != nullptr) {
                                    if (TransitionMusic->Get_State()
                                        == AudibleSoundClass::STATE_PLAYING) {
                ...
                while (!CancelApplication) {
                    ...
                    if (GameInFocus) {
                        DialogMgrClass::Render();
                        if (InstallMusic != nullptr) {
                            if (InstallMusic->Get_State() == AudibleSoundClass::STATE_PAUSED) {
                                InstallMusic->Resume();
                            }
                        }
                        if (!timer.Is_Active()) {
                        ...
                        if (InstallMusic != nullptr) {
                            if (InstallMusic->Get_State() == AudibleSoundClass::STATE_PLAYING) {
                                InstallMusic->Pause();
                            }
                        }
                        if (timer.Is_Active()) {
...
bool InstallerClass::Check_Source()
{
    ...
    if (!SetupIni->Is_Loaded()) {
        if (!SetupIni->Load(RxStringClass(IDS_SETUP_INI_FILENAME))) {
            FATAL_APP_ERROR(IDS_INI_FILE_NOT_FOUND_OR_LOADED);
        }
    }

    // Look in the current directory for <product name>.ini file.
    if (!GameIni->Is_Loaded()) {
        if (!GameIni->Load(RxStringClass(IDS_GAME_INI_FILENAME))) {
            FATAL_APP_ERROR(IDS_INI_FILE_NOT_FOUND_OR_LOADED);
        }
    }
    ...
    if (!WOLIni->Is_Loaded()) {
        ...
        if (!WOLIni->Load(multibytewolinipathname)) {
            FATAL_APP_ERROR(IDS_INI_FILE_NOT_FOUND_OR_LOADED);
        }
    }
    ...
    // WARNING: Only values < 2Gb will be reported correctly due to 32-bit limit on
    // INIClass::Get_Int().
    //				However, result is returned in 64-bits so that all callers are ready
    //to handle >= 2Gb when the need arises.
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
void InstallerClass::Count_Source_Files(const WideStringClass& sourcepath, unsigned& filecount,
                                        __int64& filesize)
{
        ...
        while (!done) {
            ...
            if (finddata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                ...
                if (_wcsicmp(cabextension, extension) != 0) {
                    filecount++;
                    filesize += (finddata.nFileSizeHigh * ((__int64)MAXDWORD) + 1)
...
void InstallerClass::Check_Existing_Install()
{
    ...
    const char* sectionname[RegistryManagerClass::COMPONENT_COUNT]
        = { INI_WOLAPI_SECTION, INI_WOLREGISTER_SECTION, INI_WOLBROWSER_SECTION };

    fresh = false;
    older = false;
    for (int c = RegistryManagerClass::COMPONENT_FIRST; c < RegistryManagerClass::COMPONENT_COUNT;
         c++) {
        ...
        if (!_RegistryManager.Get_Target_WOL_Pathname((RegistryManagerClass::WOLComponentEnum)c,
                                                      pathname)) {
            fresh = true;
            older = true;
            break;
        ...
        if (!_RegistryManager.Get_Target_WOL_Version((RegistryManagerClass::WOLComponentEnum)c,
                                                     targetversion)) {
            older = true;
            break;
        }
...
void InstallerClass::Initialize()
{
    ...
    if (!WW3D_Initialize()) {
        FATAL_APP_ERROR(IDS_CANNOT_INITIALIZE_ANY_D3D_DEVICE);
    }
...
bool InstallerClass::WW3D_Initialize(int cardselection)
{
    ...
    for (int d = 0; d < WW3D::Get_Render_Device_Count(); d++) {
        ...
        // Does this card index match the supplied card index or should we try all cards until
        // success?
        if ((cardselection == -1) || (cardselection == d)) {

            // Set the card index and whether windowed mode in the registry.
            WW3D::Registry_Save_Render_Device(APPLICATION_SUB_KEY_NAME_RENDER, d, -1, -1, -1, 0,
                                              -1);
            ...
            if (WW3D::Registry_Load_Render_Device(APPLICATION_SUB_KEY_NAME_RENDER, true)
                == WW3D_ERROR_OK) {
...
__int64 InstallerClass::Get_Game_Size(bool ondisk)
{
    ...
    if (Install_Game()) {
        ...
        if (ondisk) {
            ...
            // Account for any .key (Encryption) and .wsu (Uninstall log) files that will be created
            // by the Installer.
...
bool InstallerClass::Get_WOL_Space_Available(const WideStringClass& path, __int64& diskspace)
{
    ...
    // If the game is to be installed on the same drive, subtract game space needed from disk space
    // available.
    if (Get_Disk_Space_Available(path, diskspace)) {
        if (Install_Game()
            && (Get_Target_Game_Path(targetgamepath)[0] == Get_Target_WOL_Path(targetwolpath)[0])) {
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
void InstallerClass::Dialog_Callback(DialogBaseClass* dialog, int ctrl_id, int message_id,
                                     DWORD param)
{
    ...
    if (installmenudialog == nullptr) {
        ...
        if (installmenudialog->As_WOLDirectoryDialogClass()) {
            ...
            case IDC_BUTTON_BACK:
                ...
                // Does the user want to install the game? If not skip to the What To Install
                // dialog.
                if (Install_Game()) {
                    REGISTER_DIALOG(GameFolderDialogClass, GameFolderDialog, ())
                }
...
void InstallerClass::Update_Registry()
{
    if (Install_Game()) {
        ...
        shortcutpathname += L"\\";
        shortcutpathname
            += TxWideStringClass(IDS_GAME_PRODUCT_NAME, IDS_RESOURCE_GAME_PRODUCT_NAME);
        ...
        _RegistryManager.Register_Game(
            TxWideStringClass(IDS_GAME_PRODUCT_NAME, IDS_RESOURCE_GAME_PRODUCT_NAME),
            installpathname, folderpath, shortcutpathname, languageid, encryptedserialnumber, sku,
            version);
    }
    ...
    if (Install_WOL()) {
        ...
        _RegistryManager.Register_WOLAPI(
            folderpath, wolapipathname,
            TxWideStringClass(IDS_WOL_PRODUCT_NAME, IDS_RESOURCE_WOL_PRODUCT_NAME), sku, version,
            Use_IGR_Settings());
        ...
        _RegistryManager.Register_WOLRegister(
            folderpath, wolregisterpathname,
            TxWideStringClass(IDS_WOLREGISTER_NAME, IDS_RESOURCE_WOLREGISTER_NAME), sku, version);
        ...
        _RegistryManager.Register_WOLBrowser(
            wolbrowserpathname,
            TxWideStringClass(IDS_WOLBROWSER_NAME, IDS_RESOURCE_WOLBROWSER_NAME), version);
...
void InstallerClass::Create_Links()
{
    ...
    if (Install_Game()) {
        ...
        Create_File_Link(folderpath,
                         TxWideStringClass(IDS_GAME_PRODUCT_NAME, IDS_RESOURCE_GAME_PRODUCT_NAME),
                         gamepathname, gamepathname);

        // Create Auto Update menu item.
        Create_File_Link(folderpath,
                         TxWideStringClass(IDS_AUTO_UPDATE_TITLE, IDS_RESOURCE_AUTO_UPDATE_TITLE),
                         gamepathname, gamepathname, &grabpatches);
        ...
        Create_File_Link(folderpath, TxWideStringClass(IDS_README_TITLE, IDS_RESOURCE_README_TITLE),
                         readmepathname, readmeiconpathname);
        ...
        Create_File_Link(
            folderpath,
            TxWideStringClass(IDS_GAME_UNINSTALL_TITLE, IDS_RESOURCE_GAME_UNINSTALL_TITLE),
            uninstallpathname, uninstallpathname);
        ...
        Create_File_Link(folderpath,
                         TxWideStringClass(IDS_INTERNET_REGISTRATION_TITLE,
                                           IDS_RESOURCE_INTERNET_REGISTRATION_TITLE),
                         internetpathname, internetpathname);
        ...
        if (SetupIni->Get_Hex(INI_SETUP_SECTION, INI_LANGUAGE_ENTRY, 0x0) != 0x3) {
            Create_URL_Link(
                folderpath,
                TxWideStringClass(IDS_PLAY_ONLINE_WITH_GAMESPY,
                                  IDS_RESOURCE_PLAY_ONLINE_WITH_GAMESPY),
                L"http://www.gamespyarcade.com/features/launch.asp?svcname=ccrenegade&distID=391");
        }
        ...
        Create_File_Link(folderpath,
                         TxWideStringClass(IDS_WWCONFIG_TITLE, IDS_RESOURCE_WWCONFIG_TITLE),
                         wwconfigpathname, wwconfigpathname);
    ...
    if (Install_WOL()) {
        ...
        WideStringClass folderpath, wolfolder, registerpathname, registericonpathname,
            uninstallpathname;
        ...
        Create_File_Link(folderpath,
                         TxWideStringClass(IDS_INTERNET_REGISTRATION_TITLE,
                                           IDS_RESOURCE_INTERNET_REGISTRATION_TITLE),
                         registerpathname, registerpathname);
        ...
        Create_File_Link(
            folderpath,
            TxWideStringClass(IDS_WOL_UNINSTALL_TITLE, IDS_RESOURCE_WOL_UNINSTALL_TITLE),
            uninstallpathname, uninstallpathname);
    }
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
bool InstallerClass::Create_File_Link(const WideStringClass& linkpath, const WideStringClass& title,
                                      const WideStringClass& targetpathname,
                                      const WideStringClass& iconpathname,
                                      const WideStringClass* arguments)
{
    WideStringClass linkpathname, trimmedtitle, targetpath;
    StringClass multibytelinkpathname, multibytetargetpath, multibytetitle, multibytetargetpathname,
        multibyteiconpathname;
    IShellLink* psl;
    IPersistFile* ppf;
    bool success;
    ...
    success = CoCreateInstance(CLSID_ShellLink, nullptr, CLSCTX_INPROC_SERVER, IID_IShellLink,
                               (void**)&psl)
        == S_OK;
    if (success) {
        ...
        try {
            ...
            if (psl->SetPath(multibytetargetpathname) != NOERROR) {
                throw(IDS_COM_ERROR);
            }
            if (psl->SetDescription(multibytetitle) != NOERROR) {
                throw(IDS_COM_ERROR);
            }
            if (psl->SetIconLocation(multibyteiconpathname, 0) != NOERROR) {
                throw(IDS_COM_ERROR);
            }
            if (arguments != nullptr) {
                ...
                if (psl->SetArguments(multibytearguments) != NOERROR) {
                    throw(IDS_COM_ERROR);
                }
            }

            if (psl->SetWorkingDirectory(multibytetargetpath) != NOERROR) {
                throw(IDS_COM_ERROR);
            }
            if (psl->SetShowCmd(SW_NORMAL) != NOERROR) {
                throw(IDS_COM_ERROR);
            }
            if (psl->SetHotkey(0) != NOERROR) {
                throw(IDS_COM_ERROR);
            }
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
bool InstallerClass::Create_URL_Link(const WideStringClass& linkpath, const WideStringClass& title,
                                     const WideStringClass& url)
{
    ...
    success = CoCreateInstance(CLSID_InternetShortcut, nullptr, CLSCTX_INPROC_SERVER,
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
bool InstallerClass::Get_WOL_Account(unsigned index, WideStringClass& name,
    ...
    if (Install_Game()) {
        ...
        if (wcsstr(pathname, Get_Target_Game_Path(path)) != nullptr) {
            ...
            if (size == -1) {
                ...
                WideStringClass subdirectoryname(((WideStringClass)pathname).Peek_Buffer()
                                                 + path.Get_Length() + 1);
                ...
                filelog.RelativePathname
                    = ((WideStringClass)pathname).Peek_Buffer() + path.Get_Length() + 1;
                filelog.Size = size;
...
void InstallerClass::Create_Game_Uninstall_Log()
{
    ...
    handle = CreateFile(multibytelogfilepathname, GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS,
                        FILE_ATTRIBUTE_NORMAL, nullptr);
    if (handle == INVALID_HANDLE_VALUE) {
        FATAL_SYSTEM_ERROR;
    }
    multibytelog = log;
    if (!WriteFile(handle, multibytelog.Peek_Buffer(), multibytelog.Get_Length() * sizeof(TCHAR),
...
void InstallerClass::Create_Encryption_File(const WideStringClass& pathname)
{
    ...
    handle = CreateFile(multibytepathname, GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS, 0,
                        nullptr);
    if (handle == INVALID_HANDLE_VALUE) {
        FATAL_SYSTEM_ERROR;
    }
    ...
    if (!WriteFile(handle, stringbuffer, sizeof(stringbuffer), &byteswritten, nullptr)) {
        FATAL_SYSTEM_ERROR;
    }
    if (!CloseHandle(handle)) {
        FATAL_SYSTEM_ERROR;
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
void InstallerClass::Encrypt(const char* number, const WideStringClass& pathname,
                             StringClass& encryptednumber)
{
    StringClass multibytepathname(pathname);
    char* s;
    int numberlength;
    ...
    handle = CreateFile(multibytepathname, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0,
                        nullptr);
    if (handle == INVALID_HANDLE_VALUE) {
        FATAL_SYSTEM_ERROR;
    }
...
void InstallerClass::Display_Readme()
{
    ...
    const WCHAR* wordprocessornames[applicationcount]
        = { L"Wordpad.exe", L"Notepad.exe", L"Write.exe" };
    ...
    // No success? Try to launch any of the common text processor applications with the Readme.txt
    // file.
    i = 0;
