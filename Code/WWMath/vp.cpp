...
#define BROADCAST(XMM, INDEX)                                                                      \
    __asm shufps XMM, XMM,                                                                         \
        (((INDEX) & 3) << 6 | ((INDEX) & 3) << 4 | ((INDEX) & 3) << 2 | ((INDEX) & 3))

#define TRANSPOSE(BX, BY, BZ, BW, TV)                                                              \
    __asm movaps TV, BZ __asm unpcklps BZ, BW __asm unpckhps TV, BW __asm movaps BW,               \
        BX __asm unpcklps BX, BY __asm unpckhps BW, BY __asm movaps BY, BX __asm shufps BX, BZ,    \
        SHUFFLE(1, 0, 1, 0) __asm shufps BY, BZ, SHUFFLE(3, 2, 3, 2) __asm movaps BZ,              \
        BW __asm shufps BZ, TV, SHUFFLE(1, 0, 1, 0) __asm shufps BW, TV, SHUFFLE(3, 2, 3, 2)
...
void VectorProcessorClass::Transform(Vector3* dst, const Vector3* src, const Matrix3D& mtx,
                                     const int count)
{
    if (count <= 0) {
        return;
...
void VectorProcessorClass::Transform(Vector4* dst, const Vector3* src, const Matrix4& matrix,
                                     const int count)
{
    if (count <= 0) {
        return;
...
void VectorProcessorClass::Copy(Vector4* dst, const Vector3* src, const float* srca,
                                const int count)
{
    if (count <= 0) {
        return;
...
void VectorProcessorClass::CopyIndexed(Vector2* dst, const Vector2* src, const unsigned int* index,
                                       int count)
{
    if (count <= 0) {
        return;
...
void VectorProcessorClass::CopyIndexed(Vector3* dst, const Vector3* src, const unsigned int* index,
                                       int count)
{
    if (count <= 0) {
        return;
...
void VectorProcessorClass::CopyIndexed(Vector4* dst, const Vector4* src, const unsigned int* index,
                                       int count)
{
    if (count <= 0) {
        return;
...
void VectorProcessorClass::CopyIndexed(unsigned char* dst, const unsigned char* src,
                                       const unsigned int* index, int count)
{
    if (count <= 0) {
        return;
...
void VectorProcessorClass::Clamp(Vector4* dst, const Vector4* src, const float min, const float max,
                                 const int count)
{
    if (count <= 0) {
        return;
...
void VectorProcessorClass::DotProduct(float* dst, const Vector3& a, const Vector3* b,
                                      const int count)
{
    for (int i = 0; i < count; i++) {
