...
** Save-Load stuff
*/
SimplePersistFactoryClass<CatmullRomSpline3DClass, WWMATH_CHUNKID_CATMULLROMSPLINE3D>
    _CatmullRomSpline3DFactory;
SimplePersistFactoryClass<CatmullRomSpline1DClass, WWMATH_CHUNKID_CATMULLROMSPLINE1D>
...
void CatmullRomSpline3DClass::Update_Tangents(void)
{
    ...
    for (int i = 1; i < Keys.Count() - 1; i++) {
        ...
        float in_factor
            = 2.0f * (Keys[i].Time - Keys[i - 1].Time) / (Keys[i + 1].Time - Keys[i - 1].Time);
        float out_factor
            = 2.0f * (Keys[i + 1].Time - Keys[i].Time) / (Keys[i + 1].Time - Keys[i - 1].Time);
