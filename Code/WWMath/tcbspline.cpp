...
void TCBSpline3DClass::Update_Tangents(void)
{
    ...
    if (IsLooping) {
        ...
        float k0
            = 0.5f * ((1 - Params[0].Tension) * (1 - Params[0].Continuity) * (1 - Params[0].Bias));
        float k1
            = 0.5f * ((1 - Params[0].Tension) * (1 + Params[0].Continuity) * (1 + Params[0].Bias));
        float k2
            = 0.5f * ((1 - Params[0].Tension) * (1 + Params[0].Continuity) * (1 - Params[0].Bias));
        ...
        float k2
            = 0.25f * ((1 - Params[0].Tension) * (1 + Params[0].Continuity) * (1 - Params[0].Bias));
        float k3
            = 0.25f * ((1 - Params[0].Tension) * (1 - Params[0].Continuity) * (1 + Params[0].Bias));
    ...
    for (int pi = 1; pi < Keys.Count() - 1; pi++) {
        ...
        float k0 = 0.5f
            * ((1 - Params[pi].Tension) * (1 - Params[pi].Continuity) * (1 - Params[pi].Bias));
        float k1 = 0.5f
            * ((1 - Params[pi].Tension) * (1 + Params[pi].Continuity) * (1 + Params[pi].Bias));
        float k2 = 0.5f
            * ((1 - Params[pi].Tension) * (1 + Params[pi].Continuity) * (1 - Params[pi].Bias));
