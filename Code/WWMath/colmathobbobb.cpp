 ...
 *   5/4/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obb_intersect_axis(ObbIntersectionStruct& context, const Vector3& axis, float ra,
                                      float rb)
{
    float rsum = ra + rb;
...
{
    ObbCollisionStruct(const OBBoxClass& box0, const Vector3& move0, const OBBoxClass& box1,
                       const Vector3& move1)
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obb_separation_test(ObbCollisionStruct& context, float ra, float rb, float u0,
                                       float u1)
{
    float tmp;
    float rsum = ra + rb;
...
static inline void compute_contact_point(ObbCollisionStruct& context, CastResultStruct* result)
{
    ...
    case AXIS_A0B0:
        ...
        if (WWMath::Fabs(den) > 0.0f) {
            ...
            x[0] += context.AB[0][0]
                * (Vector3::Dot_Product(-context.B[0], dcnew) + context.AB[1][0] * x[1]
                   + context.AB[2][0] * x[2]);
    ...
    case AXIS_A0B1:
        ...
        if (WWMath::Fabs(den) > 0.0f) {
            x[0] = Vector3::Dot_Product(context.A[0], dcnew);
            x[0] += context.AB[0][1]
                * (Vector3::Dot_Product(-context.B[1], dcnew) + context.AB[1][1] * x[1]
                   + context.AB[2][1] * x[2]);
    ...
    case AXIS_A0B2:
        ...
        if (WWMath::Fabs(den) > 0.0f) {
            x[0] = Vector3::Dot_Product(context.A[0], dcnew);
            x[0] += context.AB[0][2]
                * (Vector3::Dot_Product(-context.B[2], dcnew) + context.AB[1][2] * x[1]
                   + context.AB[2][2] * x[2]);
    ...
    case AXIS_A1B0:
        ...
        if (WWMath::Fabs(den) > 0.0f) {
            ...
            x[1] += context.AB[1][0]
                * (Vector3::Dot_Product(-context.B[0], dcnew) + context.AB[0][0] * x[0]
                   + context.AB[2][0] * x[2]);
    ...
    case AXIS_A1B1:
        ...
        if (WWMath::Fabs(den) > 0.0f) {
            x[1] = Vector3::Dot_Product(context.A[1], dcnew);
            x[1] += context.AB[1][1]
                * (Vector3::Dot_Product(-context.B[1], dcnew) + context.AB[0][1] * x[0]
                   + context.AB[2][1] * x[2]);
    ...
    case AXIS_A1B2:
        ...
        if (WWMath::Fabs(den) > 0.0f) {
            x[1] = Vector3::Dot_Product(context.A[1], dcnew);
            x[1] += context.AB[1][2]
                * (Vector3::Dot_Product(-context.B[2], dcnew) + context.AB[0][2] * x[0]
                   + context.AB[2][2] * x[2]);
    ...
    result->ContactPoint.X = context.Box0.Center.X + x[0] * context.A[0].X + x[1] * context.A[1].X
        + x[2] * context.A[2].X;

    result->ContactPoint.Y = context.Box0.Center.Y + x[0] * context.A[0].Y + x[1] * context.A[1].Y
        + x[2] * context.A[2].Y;

    result->ContactPoint.Z = context.Box0.Center.Z + x[0] * context.A[0].Z + x[1] * context.A[1].Z
        + x[2] * context.A[2].Z;
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
bool CollisionMath::Collide(const OBBoxClass& box0, const Vector3& move0, const OBBoxClass& box1,
                            const Vector3& move1, CastResultStruct* result)
{
 ...
 *   5/25/99    GTH : Created.                                                                 *
 *=============================================================================================*/
bool CollisionMath::Collide(const OBBoxClass& box0, const Vector3& move0, const AABoxClass& box1,
                            const Vector3& move1, CastResultStruct* result)
{
    OBBoxClass obbox1(box1.Center, box1.Extent);
 ...
 *   5/25/99    GTH : Created.                                                                 *
 *=============================================================================================*/
bool CollisionMath::Collide(const AABoxClass& box0, const Vector3& move0, const OBBoxClass& box1,
                            const Vector3& move1, CastResultStruct* result)
{
    OBBoxClass obbox0(box0.Center, box0.Extent);
