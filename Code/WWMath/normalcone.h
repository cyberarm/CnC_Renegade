...
public:
    ...
    // Note: these two Get_Coplanar functions return floats in an attempt to reduce float/int CPU
    // state changes...
    inline float Get_Coplanar_Normals(const Vector3& Input, Vector3& Output1,
                                      Vector3& Output2) const;

    // find the two vectors on the edge of the cone residing on the same plane as the input vector
    // and their dot products with the input.
    inline float Get_Coplanar_Normals_And_Dots(const Vector3& Input, Vector3& Output1,
                                               Vector3& Output2, float& dot1, float& dot2) const;
    ...
    // this function returns the dot product between the input vector and the nearest coplanar
    // normal contained by the cone. If the input vector is also contained by the cone, the result
    // is always 1.0f. Note that in the case of a complete sphere, the nearest coplanar normal will
    // be pointing in the opposite direction of the input vector.
    ...
    // A value of 1.0f indicates that it is a degenerate cone which is basically a cone with radius
    // zero. A value of zero indicates the cone is actually hemisphere. A value of -1.0f indicates
    // that it is a complete sphere.
    float Angle;
};

// find the two vectors on the edge of the cone residing on the same plane as the input vector.
inline float NormalCone::Get_Coplanar_Normals(const Vector3& Input, Vector3& Output1,
                                              Vector3& Output2) const
{
...
inline float NormalCone::Get_Coplanar_Normals_And_Dots(const Vector3& Input, Vector3& Output1,
                                                       Vector3& Output2, float& Dot1,
                                                       float& Dot2) const
{
...
inline void NormalCone::Merge(const Vector3& Input)
{
    ...
    // If true, this means that the object now represents a complete sphere with normals facing
    // every direction.
    ...
    // the smaller of the dot values we have is going to indicate which of the two coplanar normals
    // to use for averaging into the new center normal.
    if (dot1 < dot2) {
