...
public:
    ...
    */
    ...
    /*
    ** Update_Bounding_Boxes.  This function causes all bounding boxes in the tree to update
    *themselves.
    ...
    ** that the renegade level editor tries to do everything possible to not discard the
    *precalculated
    ** visibilty data for a level.  In some cases, we want to load geometry that has been edited
    *back
...
protected:
    ...
    */
    ...
    void Partition_Tree_Depth_Recursive(AABTreeNodeClass* node, int cur_depth,
                                        int& max_depth) const;
...
public:
    ...
    struct SplitChoiceStruct
    {
        SplitChoiceStruct(void)
            : Cost(FLT_MAX),
              FrontCount(0),
              BackCount(0),
              Plane(AAPlaneClass::XNORMAL, 0.0f)
        {
            FrontBox.Init_Empty();
            BackBox.Init_Empty();
        }
    ...
    */
    void Partition(void);
    void Split_Objects(const SplitChoiceStruct& sc, AABTreeNodeClass* front,
    ...
    */
    ...
    void Select_Splitting_Plane_Brute_Force(SplitChoiceStruct* sc,
