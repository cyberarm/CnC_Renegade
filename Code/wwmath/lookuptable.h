...
inline float LookupTableClass::Get_Value(float input)
{
    ...
    float normalized_input
        = (float)(OutputSamples.Length() - 1) * (input - MinInputValue) * OOMaxMinusMin;
...
inline float LookupTableClass::Get_Value_Quick(float input)
{
    ...
    int index = (OutputSamples.Length() - 1)
        * WWMath::Float_To_Long((input - MinInputValue) * OOMaxMinusMin);
...
public:
    ...
    static void Save_Table_Desc(ChunkSaveClass& csave, Curve1DClass* curve, const Vector2& min,
                                const Vector2& max);

    static void Load_Table_Desc(ChunkLoadClass& cload, Curve1DClass** curve_ptr,
                                Vector2* set_min = nullptr, Vector2* set_max = nullptr);
