...
#define AXISLEN_EPSILON2                                                                           \
    WWMATH_EPSILON* WWMATH_EPSILON // squared length of a separating axis must be larger than this
...
{
    BTCollisionStruct(const OBBoxClass& box, const Vector3& move, const TriClass& tri,
    ...
    void Reset(void)
    {
        ...
        MaxFrac = -1.0f; // maximum move allowed so far (accept slightly negative but clamp to zero
                         // at end)
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obbtri_collision_separation_test(BTCollisionStruct& context, float lp,
                                                    float leb0, float leb1)
{
    /*
    ...
    */
    ...
    if (lp - leb0 <= 0.0f) {
        eps = COLLISION_EPSILON
            * context.TestAxis.Length(); // trying to only compute epsilon if I have to
    }
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obbtri_check_collision_cross_axis(BTCollisionStruct& context, float dp, int dpi,
                                                     float leb0)
{
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obbtri_check_collision_basis_axis(BTCollisionStruct& context, float leb0,
                                                     float dp1, float dp2)
{
    float dist; // separation along the axis
    float axismove; // size of the move along the axis.
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline void obbtri_compute_contact_normal(const BTCollisionStruct& context,
                                                 Vector3* set_normal)
{
                                                ...
                                                CastResultStruct* result)
{
    ...
    */
    result->ContactPoint.X = context.Box.Center.X + x[0] * context.A[0].X + x[1] * context.A[1].X
        + x[2] * context.A[2].X;

    result->ContactPoint.Y = context.Box.Center.Y + x[0] * context.A[0].Y + x[1] * context.A[1].Y
        + x[2] * context.A[2].Y;

    result->ContactPoint.Z = context.Box.Center.Z + x[0] * context.A[0].Z + x[1] * context.A[1].Z
        + x[2] * context.A[2].Z;
 ...
 *   4/8/99     GTH : Created.                                                                 *
 *=============================================================================================*/
bool CollisionMath::Collide(const OBBoxClass& box, const Vector3& move, const TriClass& tri,
                            const Vector3& trimove, CastResultStruct* result)
{
 ...
 *=============================================================================================*/
static inline bool obbtri_intersection_separation_test(BTIntersectStruct& context, float lp,
                                                       float leb0)
{
    /*
 ...
 *   5/4/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obbtri_check_intersection_cross_axis(BTIntersectStruct& context, float dp,
                                                        float leb0)
{
    float p0; // distance from box center to vertex 0
    float lp; // leading edge of the polygon.
 ...
 *   5/4/99     GTH : Created.                                                                 *
 *=============================================================================================*/
static inline bool obbtri_check_intersection_basis_axis(BTIntersectStruct& context, float leb0,
                                                        float dp1, float dp2)
{
    float dist; // separation along the axis
    float lp; // leading edge of the polygon.
...
bool CollisionMath::Intersection_Test(const OBBoxClass& box, const TriClass& tri)
{
    ...
    */
    ...
    context.AE[0][0] = Vector3::Dot_Product(context.A[0], context.E[0]);
    context.AE[0][1] = Vector3::Dot_Product(context.A[0], context.E[1]);
    if (obbtri_check_intersection_basis_axis(context, box.Extent.X, context.AE[0][0],
                                             context.AE[0][1])) {
        return false;
    }
    ...
    */
    ...
    context.AE[1][0] = Vector3::Dot_Product(context.A[1], context.E[0]);
    context.AE[1][1] = Vector3::Dot_Product(context.A[1], context.E[1]);
    if (obbtri_check_intersection_basis_axis(context, box.Extent.Y, context.AE[1][0],
                                             context.AE[1][1])) {
        return false;
    }
    ...
    */
    ...
    context.AE[2][0] = Vector3::Dot_Product(context.A[2], context.E[0]);
    context.AE[2][1] = Vector3::Dot_Product(context.A[2], context.E[1]);
    if (obbtri_check_intersection_basis_axis(context, box.Extent.Z, context.AE[2][0],
                                             context.AE[2][1])) {
        return false;
    }
