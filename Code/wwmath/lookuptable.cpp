...
void LookupTableClass::Init(const char* name, Curve1DClass* curve)
{
    ...
    for (int i = 0; i < OutputSamples.Length(); i++) {
        float x = MinInputValue
            + (MaxInputValue - MinInputValue) * (float)i / (float)(OutputSamples.Length() - 1);
        float y;
        curve->Evaluate(x, &y);
        OutputSamples[i] = y;
    }
...
void LookupTableMgrClass::Save_Table_Desc(ChunkSaveClass& csave, Curve1DClass* curve,
...
void LookupTableMgrClass::Load_Table_Desc(ChunkLoadClass& cload, Curve1DClass** curve_ptr,
                                          Vector2* set_min_corner, Vector2* set_max_corner)
{
    *curve_ptr = nullptr;
