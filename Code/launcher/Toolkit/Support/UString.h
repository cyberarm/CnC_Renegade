...
class UString : public RefCounted
{
public:
    // Constructors
    UString();
    ...
    const WChar* Get(void) const { return (mData != nullptr) ? mData : L""; }
    ...
    bool operator==(const WChar* ws) { return (Compare(ws) == 0); }

    bool operator==(const UString& s) { return (Compare(s) == 0); }
    ...
    bool operator<(const Char* s) { return (Compare(s) == -1); }

    bool operator<(const WChar* ws) { return (Compare(ws) == -1); }

    bool operator<(const UString& s) { return (Compare(s) == -1); }
