...
#define INSTALL_PATHNAME_KEY                                                                       \
    "InstallPath" // NOTE: This is a misnomer. Actually refers to product pathname.
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
bool RegistryManagerClass::Get_Target_WOL_Pathname(WOLComponentEnum wolcomponent,
                                                   WideStringClass& pathname)
{
    return (Get_String(HKEY_LOCAL_MACHINE, &pathname, SOFTWARE_KEY, WESTWOOD_KEY,
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
bool RegistryManagerClass::Get_Target_WOL_Folder(WOLComponentEnum wolcomponent,
...
void RegistryManagerClass::Register_Game(const WideStringClass& name,
                                         ...
                                         DWORD languageid, const StringClass& serialnumber,
                                         DWORD sku, DWORD version)
{
    DWORD maskedsku;
...
void RegistryManagerClass::Register_WOLAPI(const WideStringClass& folderpath,
                                           const WideStringClass& installpathname,
                                           const WideStringClass& name, DWORD sku, DWORD version,
                                           bool useigrsettings)
{
...
void RegistryManagerClass::Register_WOLRegister(const WideStringClass& folderpath,
                                                const WideStringClass& installpathname,
                                                const WideStringClass& name, DWORD sku,
                                                DWORD version)
{
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
bool RegistryManagerClass::Get_WOL_Account(unsigned index, WideStringClass& name,
    ...
    if (keyname != NULL) {
        ...
        while (true) {
            ...
            if (nextkeyname == NULL) {
                ...
                if (RegSetValueEx(key, keyname, 0, REG_SZ, (BYTE*)multibytestring.Peek_Buffer(),
                                  multibytestring.Get_Length() + 1)
                    == ERROR_SUCCESS) {
                    success = true;
                }
                break;
