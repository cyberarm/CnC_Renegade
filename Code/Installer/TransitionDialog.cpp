...
{
    const int textstringids[LABEL_COUNT]
        = { IDS_INSTALLER_TEXT094,      IDS_INSTALLER_TEXT070,
            IDS_SCANNING_LOCAL_RADAR,   IDS_BIO_MONITOR_ACTIVATING,
            IDS_UPDATING_WEAPON_STATUS, IDS_INITIALIZING_OBJECTIVES_SYSTEM,
            IDS_TARGETING_SYSTEM_ONLINE };

    const int textcontrolids[LABEL_COUNT]
        = { IDC_EVA_UPDATING1,          IDC_PLEASE_STANDBY,
            IDC_SCANNING_LOCAL_RADAR,   IDC_BIO_MONITOR_ACTIVATING,
            IDC_UPDATING_WEAPON_STATUS, IDC_INITIALIZING_OBJECTIVES_SYSTEM,
            IDC_TARGETING_SYSTEM_ONLINE };

    const char* meshnames[MESH_COUNT] = { "RADAR", "HEALTH", "WEAPONS", "POGS", "TARGETING" };
...
void TransitionDialogClass::On_Frame_Update(void)
{
    ...
    const float labelcontroltimes[LABEL_COUNT][4]
        = { { 1.67f, 1.73f, 6.60f, 6.67f },     { 6.86f, 6.93f, 17.59f, 17.67f },
            { 17.97f, 17.97f, 38.97f, 38.97f }, { 21.97f, 21.97f, 38.97f, 38.97f },
            { 25.97f, 25.97f, 38.97f, 38.97f }, { 29.97f, 29.97f, 38.97f, 38.97f },
    ...
    for (unsigned label = 0; label < LABEL_COUNT; label++) {
        ...
        if ((Time < labelcontroltimes[label][0]) || (Time > labelcontroltimes[label][3])) {
            ...
            if (Time < labelcontroltimes[label][1]) {
                f = (Time - labelcontroltimes[label][0])
                    / (labelcontroltimes[label][1] - labelcontroltimes[label][0]);
            }
            else {
                if (Time < labelcontroltimes[label][2]) {
                    ...
                    f = 1.0f
                        - (Time - labelcontroltimes[label][2])
                            / (labelcontroltimes[label][3] - labelcontroltimes[label][2]);
                }
            }
        }
    ...
    if (Time > digitstarttime) {
        ...
        if (Time < digitendtime) {
            ...
            if (Time < digitdecaytime) {
                ...
                digit->Set_Location(Vector2(floorf(_randomnumber(0, randomness) * oow),
                                            floorf(_randomnumber(0, randomness) * ooh)));
                digit->Reset_Polys();
    ...
    for (unsigned mesh = 0; mesh < MESH_COUNT; mesh++) {
        ...
        if ((Time < meshcontroltimes[mesh][0]) || (Time > meshcontroltimes[mesh][3])) {
            ...
            if (Time < meshcontroltimes[mesh][1]) {
                f = (Time - meshcontroltimes[mesh][0])
                    / (meshcontroltimes[mesh][1] - meshcontroltimes[mesh][0]);
            }
            else {
                if (Time < meshcontroltimes[mesh][2]) {
                    ...
                    f = 1.0f
                        - (Time - meshcontroltimes[mesh][2])
                            / (meshcontroltimes[mesh][3] - meshcontroltimes[mesh][2]);
                }
            }
        }
