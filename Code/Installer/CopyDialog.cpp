...
// Static data.
static const char* _ModelNames[MODEL_COUNT]
    = { "C_AG_GDI_MG",  "C_AG_GDI_MGO", "C_AG_NOD_MG", "C_AG_GDI_LOCKE", "C_AG_NOD_SAKU",
        "IGM_PIST",     "IGM_SNIP",     "IGM_C4-R",    "VDC_GDI_HUMVEE", "V_GDI_MEDTNK",
        "V_NOD_TURRET", "V_GDI_ORCA",   "ENC_GPWR" };
...
static const int _ModelHeadings[MODEL_COUNT]
    = { IDS_GDI_SOLDIER_HEADING,     IDS_GDI_OFFICER_HEADING, IDS_NOD_SOLDIER_HEADING,
        IDS_LOCKE_HEADING,           IDS_SAKURA_HEADING,      IDS_PISTOL_HEADING,
        IDS_SNIPER_RIFLE_HEADING,    IDS_REMOTE_C4_HEADING,   IDS_GDI_HUMM_VEE_HEADING,
        IDS_GDI_MEDIUM_TANK_HEADING, IDS_NOD_TURRET_HEADING,  IDS_GDI_ORCA_HEADING,
        IDS_GDI_POWER_PLANT_HEADING };

static const int _ModelDescriptions[MODEL_COUNT]
    = { IDS_GDI_SOLDIER_DESCRIPTION,     IDS_GDI_OFFICER_DESCRIPTION, IDS_NOD_SOLDIER_DESCRIPTION,
        IDS_LOCKE_DESCRIPTION,           IDS_SAKURA_DESCRIPTION,      IDS_PISTOL_DESCRIPTION,
        IDS_SNIPER_RIFLE_DESCRIPTION,    IDS_REMOTE_C4_DESCRIPTION,   IDS_GDI_HUMM_VEE_DESCRIPTION,
        IDS_GDI_MEDIUM_TANK_DESCRIPTION, IDS_NOD_TURRET_DESCRIPTION,  IDS_GDI_ORCA_DESCRIPTION,
...
void CopyDialogClass::On_Frame_Update(void)
{
    ...
    case CopyThreadClass::STATUS_OK: {
        ...
        ProgressBarModel->Set_Animation(ProgressBarAnim, fraction * (framecount - 1),
                                        RenderObjClass::ANIM_MODE_MANUAL);
        percentagestring[sizeof(percentagestring) - 1] = '\0';
        _snprintf(percentagestring, sizeof(percentagestring) - 1, "%u%%",
                  (unsigned)(fraction * 100.0f));
        Set_Dlg_Item_Text(IDC_COPY_PERCENTAGE, WideStringClass(percentagestring));
        break;
    }

    case CopyThreadClass::STATUS_ERROR:
        FileSystemErrorPopup = MessageBoxClass::Create_Dialog(
            TxWideStringClass(IDS_APPLICATION_ERROR), TxWideStringClass(IDS_FILE_SYSTEM_ERROR),
            MessageBoxClass::MESSAGE_BOX_TYPE_RETRY_QUIT, this);
        CountdownTimer->Stop();
        break;
...
void CopyDialogClass::On_Command(int ctrl_id, int message_id, DWORD param)
{
    ...
    case IDCANCEL:
        ...
        if (!CopyThread->Is_Running()) {
            ...
            // Can the user cancel?
            // NOTE: User will not be able to cancel if the copying process is uninterruptable
            // because,
