...
void LicenseDialogClass::Load_License(const char* licensefilename)
{
    ...
    file
        = CreateFile(licensefilename, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0L, nullptr);
    ...
    if (file != INVALID_HANDLE_VALUE) {
        ...
        if ((fileinformation.nFileSizeHigh == 0)
            && (fileinformation.nFileSizeLow >= sizeof(WCHAR))) {
            ...
            if (ReadFile(file, buffer, length * sizeof(WCHAR), &bytesreadcount, nullptr)) {
                ...
                // Strip 0x000D characters from the license text - they're not parsed as carriage
                // return by the multi-line edit control.
                d = 0;
                while (true) {
                    if (buffer[s] != 0x000D) {
                        buffer[d] = buffer[s];
    ...
    case IDOK: {
        ...
        if (!check->Get_Check()) {
            MessageBoxClass::Do_Dialog(TxWideStringClass(IDS_WARNING),
                                       TxWideStringClass(IDS_MUST_ACCEPT),
                                       MessageBoxClass::MESSAGE_BOX_TYPE_OK);
            return;
        }
        else {
            break;
