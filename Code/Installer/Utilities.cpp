...
// Private prototypes.
static bool Create_Directory(const WCHAR* drive, const WCHAR* directory, WCHAR* subdirectory,
                             bool remove, DynamicVectorClass<StringClass>* log);
...
bool Get_Disk_Space_Available(const WideStringClass& path, __int64& diskspace)
{
    ...
    ULARGE_INTEGER freebytecount; // Free bytes on disk available to caller (caller may not have
                                  // access to entire disk).
    ...
    if (getfreediskspaceex != NULL) {
        ...
        // QUESTION: SDK docs say that values returned by this function are erroneous if partition >
        // 2Gb.
        //				 Does that mean that the partition is guaranteed to be <=
        //2Gb if Ex is not available?
        if (!GetDiskFreeSpace(drive, &sectorspercluster, &bytespersector, &freeclustercount,
                              &totalclustercount)) {
            return (false);
        }
        ...
        if (((path[0] >= L'A') && (path[0] <= L'Z')) || ((path[0] >= L'a') && (path[0] <= L'z'))) {
            if ((path[1] == L':') && ((path[2] == L'\\') || (path[2] == L'/'))) {
                ...
                if (*c == L'\0') {
                    ...
                    if (wcscspn(((WideStringClass)path).Peek_Buffer(), L"*?<>|")
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
bool Create_Directory(const WCHAR* drive, const WCHAR* directory, WCHAR* subdirectory, bool remove,
                      DynamicVectorClass<StringClass>* log)
{
    bool valid;
    WCHAR *c, replacement;
    ...
    if (*c != L'\0') {
        ...
        *c = L'\0';
        ...
        if ((errorcode == 0) || (errorcode == ERROR_ALREADY_EXISTS)) {
            ...
            // If caller wants it removed (and it didn't already exist) then remove it (ie. just
            // verify that it can be created).
            if (remove && (errorcode == 0)) {
                if (!RemoveDirectory(multibytepath)) {
                    FATAL_SYSTEM_ERROR;
                }
            }
        }
...
{
    // NOTE: Currently, it is not known how to obtain (or if it is possible to obtain) the cluster
    // size
    //			of a hard disk. According to MS documentation, the largest possible cluster
    //size of FAT16, 			FAT32 and NTFS file systems is 64K (and is more typically 4-16K). Therefore,
    //conservatively 			estimate the cluster size as 64K.
...
bool Is_System_Directory(const WideStringClass& path)
{
    ...
    if (!GetWindowsDirectory(multibytesystempath, _MAX_PATH)) {
        FATAL_SYSTEM_ERROR;
    }
    ...
    if (!GetSystemDirectory(multibytesystempath, _MAX_PATH)) {
        FATAL_SYSTEM_ERROR;
    }
    systempath = multibytesystempath;
    if (Is_Sub_Path(path, systempath)) {
...
void Get_Current_Directory(WideStringClass& path)
{
    ...
    if (!GetCurrentDirectory(_MAX_PATH, multibytepath.Get_Buffer(MAX_PATH))) {
        FATAL_SYSTEM_ERROR;
    }
    path = multibytepath;
}
                                 ...
                                 StringClass& multibytetemporarypathname)
{
    ...
    multibytepath = path;
    if (!GetTempFileName(multibytepath, "XXX", 0,
                         multibytetemporarypathname.Get_Buffer(_MAX_PATH))) {
        return (false);
    }
    return (true);
}
