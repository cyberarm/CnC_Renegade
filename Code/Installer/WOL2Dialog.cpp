    ...
    case IDOK: {
        ...
        if (userpassword.Get_Length() != ENCRYPTED_STRING_LENGTH) {
            MessageBoxClass::Do_Dialog(TxWideStringClass(IDS_WARNING),
                                       TxWideStringClass(IDS_INVALID_PASSWORD),
                                       MessageBoxClass::MESSAGE_BOX_TYPE_OK, this);
            return;
        }

        // Verify that the user password is alphanumeric ie. do not allow any punctuation or Asian
        // characters.
        ...
        if (accountpassword.Compare(WideStringClass(encrypteduserpassword)) != 0) {
            MessageBoxClass::Do_Dialog(TxWideStringClass(IDS_WARNING),
                                       TxWideStringClass(IDS_INCORRECT_PASSWORD),
                                       MessageBoxClass::MESSAGE_BOX_TYPE_OK, this);
            return;
        }
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
StringClass& WOL2DialogClass::Encrypt_Password(const StringClass& password,
                                               StringClass& encryptedpassword)
{
    const static char _basestring[]
    ...
    for (upcnt = 0, dncnt = ENCRYPTED_STRING_LENGTH; upcnt < ENCRYPTED_STRING_LENGTH;
         upcnt++, dncnt--) {
        if (*(password.Peek_Buffer() + upcnt) & 0x01) {
            tempbuffer[upcnt]
                = (*(password.Peek_Buffer() + upcnt) << (*(password.Peek_Buffer() + upcnt) & 0x01))
                & *(password.Peek_Buffer() + dncnt);
        }
        else {
            tempbuffer[upcnt]
                = (*(password.Peek_Buffer() + upcnt) << (*(password.Peek_Buffer() + upcnt) & 0x01))
