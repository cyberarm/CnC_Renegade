    ...
    case IDOK: {
        ...
        if (!Validate_Path(path, errormessageid)) {
            MessageBoxClass::Do_Dialog(TxWideStringClass(IDS_WARNING),
                                       TxWideStringClass(errormessageid),
        ...
        if (Is_System_Directory(path)) {
            MessageBoxClass::Do_Dialog(TxWideStringClass(IDS_WARNING),
                                       TxWideStringClass(IDS_SYSTEM_DIRECTORY),
                                       MessageBoxClass::MESSAGE_BOX_TYPE_OK, this);
            return;
        }
        ...
        if (!Get_Disk_Space_Available(path, spaceavailable)) {
            MessageBoxClass::Do_Dialog(TxWideStringClass(IDS_WARNING),
                                       TxWideStringClass(IDS_INVALID_PATH),
                                       MessageBoxClass::MESSAGE_BOX_TYPE_OK, this);
            return;
        }
        if (Get_Disk_Space_Needed() > spaceavailable) {
            MessageBoxClass::Do_Dialog(TxWideStringClass(IDS_WARNING),
                                       TxWideStringClass(IDS_NOT_ENOUGH_DISK_SPACE),
                                       MessageBoxClass::MESSAGE_BOX_TYPE_OK, this);
            return;
        }
        ...
        if (Directory_Exists(path)) {
            OverwriteDialog = MessageBoxClass::Create_Dialog(
                TxWideStringClass(IDS_WARNING), TxWideStringClass(IDS_DIRECTORY_EXISTS),
...
void GameDirectoryDialogClass::On_Init_Dialog()
{
    ...
    if (_RegistryManager.Get_Target_Game_Path(path)) {
        ...
        Set_Dlg_Item_Text(IDC_DIRECTORY_EDIT,
                          TxWideStringClass(IDS_DEFAULT_GAME_PATH, IDS_RESOURCE_DEFAULT_GAME_PATH));
    }
    Update();
...
void WOLDirectoryDialogClass::On_Init_Dialog()
{
    ...
    if (_RegistryManager.Get_Target_WOL_Path(RegistryManagerClass::WOLAPI_COMPONENT, path)) {
        ...
        Set_Dlg_Item_Text(IDC_DIRECTORY_EDIT,
                          TxWideStringClass(IDS_DEFAULT_WOL_PATH, IDS_RESOURCE_DEFAULT_WOL_PATH));
    }
    Update();
...
void WOLDirectoryDialogClass::On_Command(int ctrl_id, int message_id, DWORD param)
{
    ...
    case IDOK: {
        if (_Installer.Install_Game()) {
            ...
            if (Is_Same_Path(WideStringClass(Get_Dlg_Item_Text(IDC_DIRECTORY_EDIT)),
                             _Installer.Get_Target_Game_Path(gamepath))) {
                MessageBoxClass::Do_Dialog(TxWideStringClass(IDS_WARNING),
                                           TxWideStringClass(IDS_SAME_PATHS),
                                           MessageBoxClass::MESSAGE_BOX_TYPE_OK, this);
                return;
            }
        }
