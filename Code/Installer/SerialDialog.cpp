...
// Static data.
static const int _EditControlIds[EDIT_CONTROL_COUNT]
    = { IDC_SERIAL_EDIT1, IDC_SERIAL_EDIT2, IDC_SERIAL_EDIT3, IDC_SERIAL_EDIT4 };
            ...
            case VK_BACK:
                if (textlength == 0) {
                    if (i > 0) {
                        ...
                        EditCtrlClass* preveditcontrol
                            = Get_Dlg_Item(_EditControlIds[i - 1])->As_EditCtrlClass();
                ...
                if ((unicode >= L'0') && (unicode <= L'9')) {
                    ...
                    if (textlength < _EditControlLengths[i]) {
                        ...
                        if ((textlength == _EditControlLengths[i])
                            ...
                            // If the next edit control is empty switch focus to it and process the
                            // key.
                            if (i < EDIT_CONTROL_COUNT - 1) {
...
void SerialDialogClass::On_Command(int ctrl_id, int message_id, DWORD param)
{
    ...
    case IDOK: {
        ...
        if (valid) {
            ...
            valid = !(((_strnicmp(serialnumber, lowrange, 4) < 0)
                       || (_strnicmp(serialnumber, highrange, 4) > 0)));
#endif
    ...
    if (valid) {
        ...
        MessageBoxClass::Do_Dialog(TxWideStringClass(IDS_WARNING),
                                   TxWideStringClass(IDS_INCORRECT_SERIAL),
...
bool SerialDialogClass::Get_Serial_Number(StringClass& serialnumber)
{
    ...
    for (unsigned i = 0; i < EDIT_CONTROL_COUNT; i++) {
        ...
        // NOTE: Non-numeric characters have already been filtered out, so no need to check for
        // this.
 ...
 **
 *																															  *
 *		The checksum is divided into 2 parts.  The first is computed by grabbing 7 digits at
 *a   * grabbing 7 digits at a time and summing them mod 97. The second part is computed by
 ** grabbing 5 digits at a time and The second part is computed by grabbing 5 digits at a	  *
 *		time and	summing them mod 93.
 **
 *																															  *
 *		Note that it's important that 97 & 93 are prime (or at least relatively prime...)
 *and	  * less than 100.	7 & 5 were chosen because they can stay out of phase for the length
 *of	  * the input string.
 **
 *																															  *
 ...
 *																															  *
 *		Note, if you try and checksum ammounts over 30 chars the above	guarantees will
 *fail.	  * (That's the number of chars that the skip	& offset values will stay out of phase).
 ** *
 *																															  *
 *		Note: The checksum of all zeros will be 0 so you need to special case the check to
 *not	  * allow this (since it's a pretty obvious thing for a user to type all 0's to try and
 ** cheat the system).
 **
 *																															  *
 * HISTORY:
...
unsigned int Check_Sum(char* str)
{
    ...
    const int offsa = -5; // Everywhere the skips are in phase is a place where an 2nd error can
