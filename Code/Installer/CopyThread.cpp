...
FNFDINOTIFY(notification_function)
{
    ...
    case fdintCOPY_FILE: {
        ...
        // NOTE: If the relative path does not contain subdirectories it will not contain a
        // backslash.
        ...
        if (!DosDateTimeToFileTime(pfdin->date, pfdin->time, &localfiletime)) {
            return (-1);
        }
        if (!LocalFileTimeToFileTime(&localfiletime, &_sourcefiletime)) {
            return (-1);
        }
        ...
        if (CopyThreadClass::Replace_File(_sourcefiletime, pfdin->cb, _targetpathname)) {
            ...
            statusmessage.Format(COPY_MESSAGE_FORMAT_STRING, TxWideStringClass(IDS_COPYING),
                                 filename);
            ...
            if (!Generate_Temporary_Pathname(targetpath, _multibytetemporarypathname)) {
                return (-1);
            }

            // Open the temporary file for writing.
            handle = file_open(_multibytetemporarypathname.Peek_Buffer(),
                               _O_BINARY | _O_TRUNC | _O_CREAT | _O_WRONLY | _O_SEQUENTIAL,
                               _S_IREAD | _S_IWRITE);
    ...
    case fdintCLOSE_FILE_INFO: {
        ...
        if (targetfile != INVALID_HANDLE_VALUE) {
            if (!FindClose(targetfile)) {
                return (-1);
            }
            if (targetfiledata.dwFileAttributes & FILE_ATTRIBUTE_READONLY) {
                if (!SetFileAttributes(multibytetargetpathname, FILE_ATTRIBUTE_NORMAL)) {
                    return (-1);
                }
            }
            if (!DeleteFile(multibytetargetpathname)) {
                return (-1);
            }
        }

        // Rename the temporary file the target file.
        if (!MoveFile(_multibytetemporarypathname, multibytetargetpathname)) {
            return (-1);
        }
        ...
        if (!SetFileAttributes(multibytetargetpathname,
                               pfdin->attribs & (~FILE_ATTRIBUTE_READONLY))) {
            return (-1);
        }
...
void CopyThreadClass::Thread_Function()
{
    try {
        ...
        if (_Installer.Install_Game()) {
            ...
            if (!Create_Directory(_Installer.Get_Target_Game_Path(targetpath),
                                  &Get_Subdirectory_Log())) {
                FATAL_SYSTEM_ERROR;
            }
            i = 0;
            ...
            Copy_Directory(_Installer.Get_Source_Game_Path(sourcepath),
                           _Installer.Get_Target_Game_Path(targetpath));
        }
        ...
        if (_Installer.Install_WOL()) {
            Copy_Directory(_Installer.Get_Source_WOL_Path(sourcepath),
                           _Installer.Get_Target_WOL_Path(targetpath));
        }
    ...
    abort:
        ...
        if (_Installer.Is_Fresh_Game_Install() || _Installer.Is_Fresh_WOL_Install()) {
            ...
            //			subdirectories are removed before parent directories.	To satisfy
            //this 			requirement, remove in the following order:
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
void CopyThreadClass::Copy_Directory(const WideStringClass& sourcepath,
                                     const WideStringClass& targetpath)
{
        ...
        if (filename[0] != L'.') {
            ...
            if (finddata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                ...
                if (_wcsicmp(cabextension, extension) == 0) {
                    ...
                    filesize = ((((__int64)MAXDWORD) + 1) * ((__int64)finddata.nFileSizeHigh))
                        + ((__int64)finddata.nFileSizeLow);
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
void CopyThreadClass::Copy_File(const WideStringClass& sourcepathname,
                                const WideStringClass& targetpathname)
{
    ...
    Remove_Trailing_Name(targetpath);
    if (!Generate_Temporary_Pathname(targetpath, multibytetemporarypathname)) {
        FATAL_SYSTEM_ERROR;
    }
    ...
    while ((sourcefile = FindFirstFile(multibytesourcepathname, &sourcefiledata))
           == INVALID_HANDLE_VALUE) {
        if (!Retry()) {
            FATAL_SYSTEM_ERROR;
        }
    ...
    if (!MoveFile(multibytetemporarypathname, multibytetargetpathname)) {
        FATAL_SYSTEM_ERROR;
    }
    ...
    // Stamp the target file with file attributes of source (but clear the read-only flag to ensure
    // that the file can be patched at a later date).
    if (!SetFileAttributes(multibytetargetpathname,
                           sourcefiledata.dwFileAttributes & (~FILE_ATTRIBUTE_READONLY))) {
        FATAL_SYSTEM_ERROR;
    }
}
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
bool CopyThreadClass::Replace_File(const FILETIME& sourcefiletime, DWORD sourcefilesize,
                                   const WideStringClass& targetpathname)
{
    ...
    // So far files are deemed equal. To err on the side of caution, test file sizes and elect to
    // replace the file if sizes differ.
 ...
 *   08/22/01    IML : Created.                                                                *
 *=============================================================================================*/
bool CopyThreadClass::Replace_File(const WideStringClass& sourcepathname,
                                   const WideStringClass& targetpathname)
{
    ...
    if (GetVersionInfo(multibytesourcepathname.Peek_Buffer(), &sourceversion)
        && GetVersionInfo(multibytetargetpathname.Peek_Buffer(), &targetversion)) {
        if (sourceversion.dwFileVersionMS > targetversion.dwFileVersionMS) {
            return (true);
        }
        if (sourceversion.dwFileVersionMS < targetversion.dwFileVersionMS) {
            return (false);
        }
        if (sourceversion.dwFileVersionLS > targetversion.dwFileVersionLS) {
            return (true);
        }
        if (sourceversion.dwFileVersionLS < targetversion.dwFileVersionLS) {
            return (false);
    ...
    // So far files are deemed equal. To err on the side of caution, test file sizes and elect to
    // replace the file if sizes differ.
    return ((sourcedata.nFileSizeLow != targetdata.nFileSizeLow)
            || (sourcedata.nFileSizeHigh != targetdata.nFileSizeHigh));
}
