...
void WhatToInstallDialogClass::On_Command(int ctrl_id, int message_id, DWORD param)
{
    ...
    case IDOK: {
        // If neither of the first two options have been selected then ask the user if he wishes to
        // cancel.
        if (!(Is_Dlg_Button_Checked(IDC_WHAT_TO_INSTALL_CHECK1)
              || Is_Dlg_Button_Checked(IDC_WHAT_TO_INSTALL_CHECK2))) {
    ...
    case IDC_WHAT_TO_INSTALL_CHECK1:
        ...
        Enable_Dlg_Item(IDC_WHAT_TO_INSTALL_CHECK3,
                        Is_Dlg_Button_Checked(IDC_WHAT_TO_INSTALL_CHECK1));
        break;

    case IDC_WHAT_TO_INSTALL_CHECK2:

        // Disable option 4 (use game room settings) if the user is not installing WOL.
        Enable_Dlg_Item(IDC_WHAT_TO_INSTALL_CHECK4,
                        Is_Dlg_Button_Checked(IDC_WHAT_TO_INSTALL_CHECK2));

        // If the user wishes to install WOL but the source version is not newer than the current
        // version then issue a warning.
        if (Is_Dlg_Button_Checked(IDC_WHAT_TO_INSTALL_CHECK2)
            && (!_Installer.Is_Target_WOL_Older_Than_Source())) {
            MessageBoxClass::Do_Dialog(TxWideStringClass(IDS_WARNING),
                                       TxWideStringClass(IDS_SOURCE_WOL_OLDER),
