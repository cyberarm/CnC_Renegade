...
bool Validate_String_ID(HWND wnd, const char* string_id)
{
    ...
    if (code_id.GetLength() == 0) {
        ...
        //
        ::MessageBox(wnd, "Please enter a descriptive token for the code identifier.",
        ...
        //
        ::MessageBox(wnd,
                     "Code identifier is too short. Please enter a more descriptive token for the "
                     "code identifier.",
                     "Invalid Code ID", MB_ICONERROR | MB_OK);
    }
        ...
        //
        ::MessageBox(wnd,
                     "The code identifier must begin with a character (not a number), please enter "
                     "a valid identifier.",
                     "Invalid Code ID", MB_ICONERROR | MB_OK);
    }
    else {
        ...
        if (is_valid == false) {
            ...
            ::MessageBox(
                wnd,
                "The code identifier can only contain characters (a-z, A-Z), numbers (0-9), and "
                "the underscore character (_), please enter a valid identifier.",
                "Invalid Code ID", MB_ICONERROR | MB_OK);
        }
        else {
            retval = true;
