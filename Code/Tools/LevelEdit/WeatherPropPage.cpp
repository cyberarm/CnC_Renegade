...
WeatherPropPageClass::WeatherPropPageClass()
    : CPropertyPage(WeatherPropPageClass::IDD)
{
...
BOOL WeatherPropPageClass::OnInitDialog()
{
    ...
    float lightningintensity, lightningstartdistance, lightningenddistance, lightningheading,
    ...
    BackgroundMgrClass::Get_Lightning(lightningintensity, lightningstartdistance,
                                      lightningenddistance, lightningheading,
                                      lightningdistribution);
    if (lightningintensity > 0.0f) {
        OnLightningLightning();
    }
    else {
        BackgroundMgrClass::Get_War_Blitz(lightningintensity, lightningstartdistance,
                                          lightningenddistance, lightningheading,
                                          lightningdistribution);
        if (lightningintensity > 0.0f) {
            OnLightningWarBlitz();
    ...
    LightningStartDistanceSlider.SetPos(
        WWMath::Float_To_Long(lightningstartdistance * SLIDER_RESOLUTION));
    LightningEndDistanceSlider.SetRange(0, SLIDER_RESOLUTION);
    LightningEndDistanceSlider.SetPos(
        WWMath::Float_To_Long(lightningenddistance * SLIDER_RESOLUTION));
    ...
    LightningDistributionSlider.SetPos(
        WWMath::Float_To_Long(lightningdistribution * SLIDER_RESOLUTION));
    ...
    if (precipitationdensity > 0.0f) {
        ...
        WeatherMgrClass::Get_Precipitation(WeatherMgrClass::PRECIPITATION_SNOW,
                                           precipitationdensity);
        if (precipitationdensity > 0.0f) {
            OnPrecipitationSnow();
        }
        else {
            WeatherMgrClass::Get_Precipitation(WeatherMgrClass::PRECIPITATION_ASH,
                                               precipitationdensity);
            if (precipitationdensity > 0.0f) {
                OnPrecipitationAsh();
            }
...
void WeatherPropPageClass::Update_Lightning_Values()
{
    ...
    _snprintf(buffer, buffersize - 1, "%.2f",
              LightningIntensitySlider.GetPos() * oosliderresolution);
    GetDlgItem(IDC_LIGHTNING_INTENSITY_VALUE)->SetWindowText(buffer);
    _snprintf(buffer, buffersize - 1, "%.2f",
              LightningStartDistanceSlider.GetPos() * oosliderresolution);
    GetDlgItem(IDC_LIGHTNING_START_DISTANCE_VALUE)->SetWindowText(buffer);
    _snprintf(buffer, buffersize - 1, "%.2f",
              LightningEndDistanceSlider.GetPos() * oosliderresolution);
    ...
    _snprintf(buffer, buffersize - 1, "%.2f",
              LightningDistributionSlider.GetPos() * oosliderresolution);
...
void WeatherPropPageClass::OnOK()
{
    ...
    if (IsDlgButtonChecked(IDC_LIGHTNING_NONE) == 1) {
        ...
        if (IsDlgButtonChecked(IDC_LIGHTNING_LIGHTNING) == 1) {
            BackgroundMgrClass::Set_Lightning(lightningintensity, lightningstartdistance,
                                              lightningenddistance, lightningheading,
                                              lightningdistribution);
            BackgroundMgrClass::Set_War_Blitz(0.0f);
        }
            ...
            BackgroundMgrClass::Set_Lightning_Intensity(0.0f);
            BackgroundMgrClass::Set_War_Blitz(lightningintensity, lightningstartdistance,
                                              lightningenddistance, lightningheading,
                                              lightningdistribution);
        }
    }
    ...
    if (IsDlgButtonChecked(IDC_PRECIPITATION_NONE) == 1) {
        ...
        if (IsDlgButtonChecked(IDC_PRECIPITATION_RAIN) == 1) {
            WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_RAIN,
                                               precipitationdensity);
            WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_SNOW, 0.0f);
            WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_ASH, 0.0f);
        }
            ...
            if (IsDlgButtonChecked(IDC_PRECIPITATION_SNOW) == 1) {
                WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_RAIN, 0.0f);
                WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_SNOW,
                                                   precipitationdensity);
                WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_ASH, 0.0f);
            }
            else {
                ...
                WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_RAIN, 0.0f);
                WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_SNOW, 0.0f);
                WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_ASH,
                                                   precipitationdensity);
            }
        }
    }
