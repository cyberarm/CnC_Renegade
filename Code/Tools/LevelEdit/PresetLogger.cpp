...
void PresetLoggerClass::Log_Created(PresetClass* preset)
{
    ...
    if (Open_Log_File(file)) {
        ...
        DefinitionFactoryClass* factory
            = DefinitionFactoryMgrClass::Find_Factory(preset->Get_Class_ID());
        if (factory != nullptr) {
        ...
        //
        StringClass message;
        message.Format("(%s)	Created: %s, Type: %s", computer_name,
                       (const char*)preset->Get_Name(), (const char*)type_name);
...
void PresetLoggerClass::Log_Moved(PresetClass* preset, const char* new_parent_name)
{
    ...
    if (Open_Log_File(file)) {
        ...
        //
        StringClass message;
        message.Format("(%s)	Moved: %s. New Parent: %s", computer_name,
                       (const char*)preset->Get_Name(), (const char*)real_parent_name);
        file.Seek(0, SEEK_END);
        file.Write_Line(message);
...
void PresetLoggerClass::Log_Renamed(const char* old_name, const char* new_name)
{
    ...
    if (Open_Log_File(file)) {
        ...
        //
        StringClass message;
        message.Format("(%s)	Renamed: Old Name: %s, New Name: %s", computer_name, old_name,
                       new_name);
        file.Seek(0, SEEK_END);
        file.Write_Line(message);
...
//
//////////////////////////////////////////////////////////////////////////
void PresetLoggerClass::Log_File_Reference_Changed(PresetClass* preset, const char* param_name,
                                                   const char* new_value)
{
    ...
    if (Open_Log_File(file)) {
        ...
        //
        StringClass message;
        message.Format("Reference Changed:	Preset: %s.  Param: %s.  Value:  %s",
                       (const char*)preset->Get_Name(), param_name, new_value);
        file.Seek(0, SEEK_END);
        file.Write_Line(message);
...
bool PresetLoggerClass::Open_Log_File(TextFileClass& file_obj)
{
    ...
    for (int index = 0; index < 10; index++) {
        ...
        //
        file = ::CreateFile(
            "\\\\mobius\\project7\\projects\\renegade\\asset management\\logs\\presets.log",
            GENERIC_WRITE, FILE_SHARE_READ, nullptr, OPEN_ALWAYS, 0L, nullptr);
