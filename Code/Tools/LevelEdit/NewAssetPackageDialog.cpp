...
void NewAssetPackageDialogClass::OnOK(void)
{
    ...
    if (Is_Valid_Name(text) == false) {
        MessageBox("Names must be at least one character long and cannot contain any of the "
                   "following:  \\ / \" * ? < > |",
        ...
        MessageBox("Name is already in use, please choose another name.", "Invalid Name",
                   MB_ICONERROR | MB_OK);
        SendDlgItemMessage(IDC_PACKAGE_NAME_EDIT, EM_SETSEL, 0, -1);
        GetDlgItem(IDC_PACKAGE_NAME_EDIT)->SetFocus();
    }
