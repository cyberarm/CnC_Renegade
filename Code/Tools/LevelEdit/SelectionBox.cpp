...
void SelectionBoxClass::Display_Around_Node(const RenderObjClass& render_obj)
{
    ...
    //
    m_pBoundingLines[0]->Reset(verticies[0], verticies[0] + (verticies[1] - verticies[0]) / 4.00F,
                               width);
    m_pBoundingLines[4]->Reset(verticies[0], verticies[0] + (verticies[4] - verticies[0]) / 4.00F,
                               width);
    m_pBoundingLines[12]->Reset(verticies[0], verticies[0] + (verticies[3] - verticies[0]) / 4.00F,
                                width);

    m_pBoundingLines[1]->Reset(verticies[1], verticies[1] + (verticies[2] - verticies[1]) / 4.00F,
                               width);
    m_pBoundingLines[5]->Reset(verticies[1], verticies[1] + (verticies[5] - verticies[1]) / 4.00F,
                               width);
    m_pBoundingLines[13]->Reset(verticies[1], verticies[1] + (verticies[0] - verticies[1]) / 4.00F,
                                width);

    m_pBoundingLines[2]->Reset(verticies[2], verticies[2] + (verticies[3] - verticies[2]) / 4.00F,
                               width);
    m_pBoundingLines[6]->Reset(verticies[2], verticies[2] + (verticies[6] - verticies[2]) / 4.00F,
                               width);
    m_pBoundingLines[14]->Reset(verticies[2], verticies[2] + (verticies[1] - verticies[2]) / 4.00F,
                                width);

    m_pBoundingLines[3]->Reset(verticies[3], verticies[3] + (verticies[0] - verticies[3]) / 4.00F,
                               width);
    m_pBoundingLines[7]->Reset(verticies[3], verticies[3] + (verticies[7] - verticies[3]) / 4.00F,
                               width);
    m_pBoundingLines[15]->Reset(verticies[3], verticies[3] + (verticies[2] - verticies[3]) / 4.00F,
                                width);

    m_pBoundingLines[8]->Reset(verticies[4], verticies[4] + (verticies[5] - verticies[4]) / 4.00F,
                               width);
    m_pBoundingLines[16]->Reset(verticies[4], verticies[4] + (verticies[0] - verticies[4]) / 4.00F,
                                width);
    m_pBoundingLines[17]->Reset(verticies[4], verticies[4] + (verticies[7] - verticies[4]) / 4.00F,
                                width);

    m_pBoundingLines[9]->Reset(verticies[5], verticies[5] + (verticies[6] - verticies[5]) / 4.00F,
                               width);
    m_pBoundingLines[18]->Reset(verticies[5], verticies[5] + (verticies[4] - verticies[5]) / 4.00F,
                                width);
    m_pBoundingLines[19]->Reset(verticies[5], verticies[5] + (verticies[1] - verticies[5]) / 4.00F,
                                width);

    m_pBoundingLines[10]->Reset(verticies[6], verticies[6] + (verticies[7] - verticies[6]) / 4.00F,
                                width);
    m_pBoundingLines[20]->Reset(verticies[6], verticies[6] + (verticies[5] - verticies[6]) / 4.00F,
                                width);
    m_pBoundingLines[21]->Reset(verticies[6], verticies[6] + (verticies[2] - verticies[6]) / 4.00F,
                                width);

    m_pBoundingLines[11]->Reset(verticies[7], verticies[7] + (verticies[4] - verticies[7]) / 4.00F,
                                width);
    m_pBoundingLines[22]->Reset(verticies[7], verticies[7] + (verticies[6] - verticies[7]) / 4.00F,
                                width);
    m_pBoundingLines[23]->Reset(verticies[7], verticies[7] + (verticies[3] - verticies[7]) / 4.00F,
                                width);
