...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    for (int index = 0; index < ICON_COUNT; index++) {
        ...
        HICON hicon = (HICON)::LoadImage(::AfxGetResourceHandle(),
                                         MAKEINTRESOURCE(ICON_RESOURCE_IDS[index]), IMAGE_ICON, 16,
                                         16, LR_SHARED);
...
void CMainFrame::Create_Camera_Menu(void)
{
    ...
    if (ini_file != nullptr) {
        ...
        for (int index = 0; index < count; index++) {
            ...
            ini_file->Get_String(profile_name, "Profile_List",
                                 ini_file->Get_Entry("Profile_List", index));

            //
            //	Add this entry to the menu
            //
            camera_menu->AppendMenu(MF_STRING | MF_ENABLED, IDM_FIRST_CAM_PROFILE + index,
                                    profile_name);
        ...
        camera_menu->CheckMenuRadioItem(IDM_FIRST_CAM_PROFILE,
                                        IDM_FIRST_CAM_PROFILE + m_MaxCamProfiles,
...
void CMainFrame::Create_Toolbars(void)
{
    ...
    m_TestBar.SetBarStyle(m_TestBar.GetBarStyle() | CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC
                          | CBRS_GRIPPER);
    ...
    m_AniToolbar.SetBarStyle(m_AniToolbar.GetBarStyle() | CBRS_TOOLTIPS | CBRS_FLYBY
                             | CBRS_SIZE_DYNAMIC);
    m_wndBottomToolBar.SetBarStyle(m_wndBottomToolBar.GetBarStyle() | CBRS_TOOLTIPS | CBRS_FLYBY
                                   | CBRS_SIZE_DYNAMIC);
    ...
    FloatControlBar(&m_AmbientLightToolbar,
                    CPoint(window_rect.left + ((window_rect.Width() << 1) / 3),
                           window_rect.top + (window_rect.Height() >> 1)),
                    CBRS_ALIGN_TOP);
...
void CALLBACK PatsTextCallback(AudibleSoundClass* sound_obj, const StringClass& text,
                               uint32 user_param)
{
    CString message;
...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
    ...
    if (pview) {
        ...
        if (devicedialog.DoModal() == IDOK) {
            ...
            pview->Set_Render_Device_Info(devicedialog.GetDeviceIndex(),
                                          devicedialog.GetBitsPerPixel());
        }
        else {
...
void CMainFrame::OnClose(void)
{
    if (PresetMgrClass::Are_Presets_Dirty()) {
        ...
        //
        if (::Message_Box(m_hWnd, IDS_SAVE_PRESETS_MSG, IDS_SAVE_PRESETS_TITLE,
...
void CMainFrame::OnUpdateObjectManipulate(CCmdUI* pCmdUI)
{
    ...
    ASSERT(pmousemgr != nullptr);
    if (pmousemgr != nullptr) {
        pCmdUI->SetCheck((BOOL)pmousemgr->Get_Mouse_Mode()
                         == MouseMgrClass::MODE_OBJECT_MANIPULATE);
    }
...
void CMainFrame::OnViewChangeDevice(void)
{
    ...
    if (devicedialog.DoModal() == IDOK) {
        // Pass the user's selection from the onto the view
        ::Get_Main_View()->Set_Render_Device_Info(devicedialog.GetDeviceIndex(),
                                                  devicedialog.GetBitsPerPixel());
        ::Get_Main_View()->Initialize_Render_Device();
    }
...
void CMainFrame::OnLevelExport(void)
{
    ...
    CFileDialog saveFileDialog(FALSE, ".mix", default_name,
...
{
    if ((LOWORD(wParam) == ID_FILE_SAVE_AS) || (LOWORD(wParam) == ID_FILE_SAVE)
        ...
        //
        camera_menu->CheckMenuRadioItem(IDM_FIRST_CAM_PROFILE,
                                        IDM_FIRST_CAM_PROFILE + m_MaxCamProfiles, LOWORD(wParam),
                                        MF_BYCOMMAND);
    }
...
void CMainFrame::OnAlwaysExport(void)
{
    ...
    //
    CString path;
    if (::Browse_For_Folder(path, nullptr, nullptr,
...
void CMainFrame::Update_Texture_Mem(void)
{
    ...
    if ((current_ticks - last_update) > 500) {
        ...
        CString message;
        message.Format ("Tex Upload: %dKB", int(float(stats.texDataTransfer / stats.frames) /
        1024.0F)); m_wndStatusBar.SetPaneText (3, message); last_update = current_ticks;
...
void CMainFrame::OnUpdateEditUndo(CCmdUI* pCmdUI)
{
    ...
    if (::Get_Scene_Editor()->Get_Undo_Mgr().Get_Possible_Undo_Count() > 0) {
        text.Format("Undo (%s)\tAlt+Backspace",
                    ::Get_Scene_Editor()->Get_Undo_Mgr().Get_Next_Undo_Name());
    }
    else {
...
//  OnCreateGroup
//
void CMainFrame::OnCreateGroup(void)
{
    return;
}
...
void CMainFrame::OnSpecifyAssetDatabase(void)
{
    CFileDialog dialog(TRUE,
                       nullptr, //"srcsafe.ini",
                       "srcsafe.ini",
                       OFN_HIDEREADONLY | OFN_PATHMUSTEXIST | OFN_EXPLORER | OFN_FILEMUSTEXIST
                           | OFN_SHAREAWARE,
    ...
    if (dialog.DoModal() == IDOK) {
        ...
        //
        MessageBox("You must exit and restart the level editor for your changes to take effect.",
                   "Database Change", MB_OK | MB_ICONINFORMATION);
    }
...
//
//////////////////////////////////////////////////////////////////////////
void CMainFrame::OnUseVisCamera(void)
{
    return;
}
...
//
//////////////////////////////////////////////////////////////////////////
void CMainFrame::OnUpdateUseVisCamera(CCmdUI* pCmdUI)
{
    return;
}
...
void CMainFrame::OnTestPathfind(void)
{
    ...
    if ((node1 != nullptr) && (node2 != nullptr)) {
        ...
        //
        for (NodeClass* node = NodeMgrClass::Get_First(NODE_TYPE_WAYPATH); node != nullptr;
        ...
        //
        for (node = NodeMgrClass::Get_First(NODE_TYPE_WAYPATH); node != nullptr;
             node = NodeMgrClass::Get_Next(node, NODE_TYPE_WAYPATH)) {
            node->Post_Export();
        }
    }
...
//
//////////////////////////////////////////////////////////////////////////
void CMainFrame::OnDisplayPathfindRawData(void)
{
    return;
}
...
//
//////////////////////////////////////////////////////////////////////////
void CMainFrame::OnUpdateDisplayPathfindRawData(CCmdUI* pCmdUI)
{
    return;
}
...
void CMainFrame::OnExportPathfind(void)
{
    ...
    CFileDialog dialog(FALSE, ".pth", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
...
void CMainFrame::OnImportPathfind(void)
{
    ...
    CFileDialog dialog(TRUE, ".pth", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
                       "Pathfind database (*.pth)|*.pth||", this);
...
void CMainFrame::OnExportVis(void)
{
    ...
    CFileDialog dialog(FALSE, ".vis", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
...
//
//////////////////////////////////////////////////////////////////////////
void CMainFrame::OnDoDebug1(void)
{
    return;
}
                    ...
                    for (int index = 0; index < definition_list.Count (); index ++) {
                            ...
                            for (int index = 0; index < count; index ++) {
                                    ...
                                    if (param->Get_Type () == ParameterClass::TYPE_ZONE) {
                                            ZoneParameterClass *zone_param = (ZoneParameterClass
    *)param; const OBBoxClass &old_zone = zone_param->Get_Zone (); OBBoxClass new_zone = old_zone;
...
void CMainFrame::OnImportDynObjs(void)
{
    ...
    CFileDialog dialog(TRUE, ".dyn", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
...
void CMainFrame::OnExportDynObjs(void)
{
    ...
    CFileDialog dialog(FALSE, ".dyn", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
                       "Dynamic Objects (*.dyn)|*.dyn||", this);
...
void CMainFrame::OnImportStatic(void)
{
    ...
    CFileDialog dialog(TRUE, ".sta", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
    ...
    if (dialog.DoModal() == IDOK) {
        ...
        // Is the filename OK?
        HANDLE hfile = ::CreateFile(filename, GENERIC_READ, 0, nullptr, OPEN_EXISTING, 0L, nullptr);
...
void CMainFrame::OnExportStatic(void)
{
    ...
    CFileDialog dialog(FALSE, ".sta", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
                       "Static Data (*.sta)|*.sta||", this);
    ...
    if (enable) {
        ...
        while ((sector = PathfindClass::Get_Instance()->Peek_Sector(sector_index++)) != nullptr) {
            ...
            while (portal_index--) {
                ...
                PathDebugPlotterClass::Get_Instance()->Add(center, portal_box.Center,
                                                           Vector3(1, 0, 0));
            }
        }
    }
...
{
    CFileDialog dialog(TRUE, ".wlt", nullptr,
...
{
    CFileDialog dialog(TRUE, ".wlt", nullptr, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
                       "Westwood Light Database (*.wlt)|*.wlt||", this);
...
{
    CFileDialog dialog(TRUE, ".job", "farm01.job",
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
...
void CMainFrame::OnBuildDynaCullsys(void)
{
    ...
    for (node = NodeMgrClass::Get_First(); node != nullptr; node = NodeMgrClass::Get_Next(node)) {
...
void CMainFrame::OnImportVisRemapData(void)
{
    ...
    CFileDialog dialog(TRUE, ".vrt", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
...
void CMainFrame::OnExportVisRemapData(void)
{
    ...
    CFileDialog dialog(FALSE, ".vrt", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
                       "Vis remap table (*.vrt)|*.vrt||", this);
...
void CMainFrame::OnCheckIds(void)
{
    ...
    if (node_list.Count() > 0) {
        ...
        //
        MessageBox("No ID collision detected in this level.", "ID Validation",
...
void CMainFrame::OnExportLights(void)
{
    ...
    CFileDialog dialog(FALSE, ".wlt", nullptr, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
                       "Westwood Light Database (*.wlt)|*.wlt||", this);
...
{
    bool enabled = ::Get_Scene_Editor()->Get_Vis_Grid_Display_Mode()
        == PhysicsSceneClass::VIS_GRID_DISPLAY_CENTERS;
    if (!enabled) {
        ::Get_Scene_Editor()->Set_Vis_Grid_Display_Mode(
            PhysicsSceneClass::VIS_GRID_DISPLAY_CENTERS);
    }
    else {
...
void CMainFrame::OnRemapIds(void)
{
    if (::MessageBox(nullptr,
                     "Are you sure you want to remap the IDs of the currently selected objects "
                     "into the current ID range?",
                     "ID Remap", MB_ICONQUESTION | MB_YESNO)
        == IDYES) {
        SelectionMgrClass& sel_mgr = ::Get_Scene_Editor()->Get_Selection_Mgr();
...
{
    ::Get_Scene_Editor()->Enable_Vis_Sector_Fallback(
        !::Get_Scene_Editor()->Is_Vis_Sector_Fallback_Enabled());
    return;
}
    ...
    if (dialog.DoModal() == IDOK) {
        ...
        if (file_attributes != 0xFFFFFFFF && file_attributes & FILE_ATTRIBUTE_READONLY) {
            ::MessageBox(::AfxGetMainWnd()->m_hWnd,
                         "File is read-only, export operation can not complete.", "File Error",
...
void CMainFrame::OnBatchExport(void)
{
    ...
    CFileDialog dialog(TRUE, ".lvl", nullptr,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER | OFN_ALLOWMULTISELECT,
    ...
    if (dialog.DoModal() == IDOK) {
        ...
        if (::Browse_For_Folder(destination_dir, m_hWnd, "",
                                "Select the destination directory for the export process.")) {
            ...
            if (::MessageBox(m_hWnd,
                             "Would you like to export each level to its own sub-directory?",
                             "Batch Export", MB_ICONQUESTION | MB_YESNO)
                == IDYES) {
                create_subdirs = true;
            }
...
{
    ::Get_Scene_Editor()->Lock_Vis_Sample_Point(
        !::Get_Scene_Editor()->Is_Vis_Sample_Point_Locked());
    return;
}
...
void CMainFrame::OnExportTileList(void)
{
    ...
    CFileDialog dialog(FALSE, ".txt", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
    ...
    if (dialog.DoModal() == IDOK) {
        ...
        if (text_file.Open(FileClass::WRITE)) {
            ...
            bool do_sel_only
                = (MessageBox(
                       "Would you like to export tiles that are only in the current selection?",
                       "Export Tile List", MB_ICONQUESTION | MB_YESNO)
                   == IDYES);
            if (do_sel_only) {
                ...
                for (int index = 0; index < sel_mgr.Get_Count(); index++) {
                    NodeClass* node = sel_mgr.Get_At(index);
                    if (node != nullptr && node->Is_Proxied() == false
...
{
    CFileDialog dialog(FALSE, ".xls", "strings.xls",
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
...
{
    CFileDialog dialog(FALSE, ".txt", "files.txt",
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
                       "Text Files (*.txt)|*.txt||", this);
...
void CMainFrame::OnExportLanguage(void)
{
    ...
    CFileDialog dialog(FALSE, ".dbs", "always.dbs",
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
...
void CMainFrame::OnExportInstallerLanguageVersion(void)
{
    ...
    CFileDialog dialog(FALSE, ".tdb", "istrings.tdb",
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
...
{
    CFileDialog dialog(TRUE, ".rc", nullptr,
                       OFN_HIDEREADONLY | OFN_PATHMUSTEXIST | OFN_EXPLORER | OFN_FILEMUSTEXIST
                           | OFN_SHAREAWARE,
...
{
    bool is_checked
        = (::Get_Camera_Mgr()->Get_Camera()->Get_Projection_Type() == CameraClass::ORTHO);
...
{
    bool is_checked
        = (::Get_Camera_Mgr()->Get_Camera()->Get_Projection_Type() == CameraClass::PERSPECTIVE);
    pCmdUI->SetCheck(is_checked);
    return;
}
...
void CMainFrame::OnModExport(void)
{
    ...
    CFileDialog dialog(FALSE, ".pkg", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
...
void CMainFrame::OnSaveLevel(void)
{
    ...
    CFileDialog dialog(TRUE, ".lvl", default_name,
                       OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_EXPLORER,
                       "Level Editor Files (*.lvl)|*.lvl||", this);
...
void CMainFrame::OnUpdateModeHeightEdit(CCmdUI* pCmdUI)
{
    ...
    if (mouse_mgr != nullptr) {
        pCmdUI->SetCheck((BOOL)::Get_Mouse_Mgr()->Get_Mouse_Mode()
                         == MouseMgrClass::MODE_HEIGHTFIELD_EDIT);
    }
