...
CameraMgr::UPDATE_CAMERA_FN CameraMgr::_pfnUpdateMethods[CAMERA_MODE::MODE_COUNT]
    = { CameraMgr::Update_Camera_MOVE_ZOOM,   CameraMgr::Update_Camera_MOVE_PLANE,
        CameraMgr::Update_Camera_FLY_THROUGH, CameraMgr::Update_Camera_WALK_THROUGH,
        CameraMgr::Update_Camera_ROTATE_FREE, CameraMgr::Update_Camera_ROTATE_X,
        CameraMgr::Update_Camera_ROTATE_Y,    CameraMgr::Update_Camera_ROTATE_Z,
        CameraMgr::Update_Camera_ORBIT,       CameraMgr::Update_Camera_FLY_TO };
...
void CameraMgr::Init_Camera(void)
{
    ...
    if (_hHook == nullptr) {
        ...
        _hHook = ::SetWindowsHookEx(WH_KEYBOARD, fnCameraKeyboardHook, ::AfxGetInstanceHandle(),
                                    ::GetCurrentThreadId());
    }
            ...
            if (::GetAsyncKeyState(VK_ESCAPE) < 0 || ::GetAsyncKeyState(VK_F6) < 0) {
                ...
                if ((::GetAsyncKeyState(VK_RETURN) < 0) && (current_ticks - last_vis_gen) > 1000) {
                    Vector3 sample_point
                        = m_WalkThruObj->Get_Transform().Get_Translation() + Vector3(0, 0, 1);
...
)
{
    ...
    if (::Get_Selection_Mgr().Get_Count() > 0) {
        ...
        Vector3 vector_to_object
            = matrix_inv * ::Get_Selection_Mgr().Get_Center(); // bounding_sphere.Center;
...
void CameraMgr::Goto_Node(NodeClass* node)
{
    ...
    if (render_obj != nullptr) {
        ...
        Matrix3D transform(1);
        // transform.Look_At (sphere.Center + Vector3 (distance, distance, distance), sphere.Center,
        // 0);
...
void CameraMgr::Update_Fly_To(void)
{
    ...
    if (m_FlyToStartTime + ((DWORD)fly_ticks) < current_time) {
        ...
        Matrix3D world_to_cam(Vector3(0, -1, 0), Vector3(0, 0, 1), Vector3(-1, 0, 0),
                              Vector3(0, 0, 0));
