...
void SceneEditorClass::Initialize(void)
{
    ...
    //
    int prelit_mode
        = theApp.GetProfileInt("Config", "PrelitMode", WW3D::PRELIT_MODE_LIGHTMAP_MULTI_PASS);
...
//
//////////////////////////////////////////////////////////////////////
NodeClass* SceneEditorClass::Create_Node(PresetClass* preset, Matrix3D* transform, DWORD node_id,
                                         bool add_to_scene)
{
...
void SceneEditorClass::Update_Toolbars(void)
{
    ...
    if (m_SelectionMgr->Get_Count() > 0) {
        ...
        for (int index = 0; index < sel_count; index++) {
            ...
            if (m_SelectionMgr->Get_Count() == 1 && phys_obj != nullptr) {
                StaticPhysClass* static_phys_obj = phys_obj->As_StaticPhysClass();
                if (static_phys_obj != nullptr) {
                    selection_set += (" (");
                    CString visid_string;
                    visid_string.Format(" VisObjectId = %d, ",
                                        static_phys_obj->Get_Vis_Object_ID());
                    selection_set += visid_string;
                    visid_string.Format(" VisSectorId = %d, ",
                                        static_phys_obj->Get_Vis_Sector_ID());
                    selection_set += visid_string;
                    ...
                    CString visid_string;
                    visid_string.Format(" VisSectorId = %d, ",
                                        ((LightNodeClass*)node)->Get_Vis_Sector_ID());
                    selection_set += visid_string;
                    selection_set += (")");
                }
                else {
...
NodeClass* SceneEditorClass::Find_Node_At_Point(CPoint point, Vector3* intersect_pt)
{
    ...
    if (physobj != nullptr) {
        ...
        if (hit_info != nullptr && hit_info->Type == HITTESTINFO::Node) {
            node = hit_info->node;
...
bool SceneEditorClass::Execute_Function_At_Point(CPoint point)
{
    ...
    if (physobj != nullptr) {
        ...
        HITTESTINFO* hit_info = (HITTESTINFO*)physobj->Peek_Model()->Get_User_Data();
        if ((hit_info != nullptr) && hit_info->Type == HITTESTINFO::Function) {
...
void SceneEditorClass::Move_Selected_Nodes(void)
{
    ...
    if (::GetAsyncKeyState(VK_UP) < 0) {
        ...
        if (::GetAsyncKeyState(VK_SHIFT) < 0) {
            ...
            //
            Vector3 y_vector
                = Vector3::Cross_Product(coord_system.Get_X_Vector(), Vector3(0, 0, 1));
            y_vector.Normalize();
...
void SceneEditorClass::On_Frame(void)
{
    ...
    if (m_MovingObject != nullptr) {
		...
		if (action != nullptr && action->Get_Movement () == nullptr) {
			//m_MovingObject = nullptr;
		}
#endif
    }
...
//
///////////////////////////////////////////////////////////////
void SceneEditorClass::Record_Vis_Info(const Matrix3D& view_transform,
                                       const Vector3& sample_location)
{
    // Put up an hour glass
    CWaitCursor wait_cursor;

    PresetClass* preset
        = PresetMgrClass::Get_First(CLASSID_VIS_POINT_DEF, PresetMgrClass::ID_CLASS);
    if (preset == nullptr) {
        CString message = "Missing 'Manual Vis Point' preset.  Please create a default preset "
                          "under the 'Manual Vis Point' folder in the preset library.";
        ::MessageBox(::AfxGetMainWnd()->m_hWnd, message, "Missing Preset",
        ...
        //
        VisSampleClass vis_sample
            = Update_Vis(view_transform * Vector3(0, 0, -scene_znear), view_transform,
        ...
        if (vis_sample.Get_Bits_Changed() > 0 && vis_sample.Sample_Rejected() == false) {
            ...
            //
            Matrix3D cam_to_world(Vector3(0, 0, -1), Vector3(-1, 0, 0), Vector3(0, 1, 0),
                                  Vector3(0, 0, 0));
...
void SceneEditorClass::Begin_Operation(OPERATION_TYPE type, NODE_LIST* affected_list)
{
    ...
    if (affected_list == nullptr) {
        Build_Node_List(node_list, nullptr);
...
void SceneEditorClass::Turn_Lights_On(bool onoff)
{
    ...
    //
    /*LightInstanceClass *plight = nullptr;
...
void SceneEditorClass::Get_Vis_Camera_FOV(double& hfov, double& vfov)
{
    ...
    WWASSERT(vis_camera != nullptr);
    if (vis_camera != nullptr) {
...
//
////////////////////////////////////////////////////////////////
bool SceneEditorClass::Are_Manual_Vis_Points_Visible(void)
{
...
{
    SANITY_CHECK(node1 != nullptr && node2 != nullptr && node1->Get_Type() == NODE_TYPE_OBJECT)
    {
        return;
    }
    ...
    if (game_obj != nullptr && game_obj->As_SmartGameObj() != nullptr) {
        ActionClass* action = ((SmartGameObj*)game_obj)->Get_Action();
...
void SceneEditorClass::Export_Lights(LPCTSTR filename)
{
    ...
    if (file != nullptr && file->Create() && file->Open(FileClass::WRITE)) {
        ...
        //
        for (NodeClass* node = NodeMgrClass::Get_First(NODE_TYPE_LIGHT); node != nullptr;
            ...
            if (light != nullptr) {
                ...
                W3dLightTransformStruct w3d_tm
...
//
////////////////////////////////////////////////////////////////
void SceneEditorClass::Build_Light_List(ChunkLoadClass& cload,
                                        DynamicVectorClass<LightClass*>& light_list, int group_id)
{
    while (cload.Open_Chunk()) {
        switch (cload.Cur_Chunk_ID()) {
        ...
        case W3D_CHUNK_LIGHTSCAPE_LIGHT: {
            ...
            if (cload.Cur_Chunk_ID() == W3D_CHUNK_LIGHT_TRANSFORM) {
                ...
                //
                Matrix3D tm(w3d_tm.Transform[0][0], w3d_tm.Transform[0][1], w3d_tm.Transform[0][2],
                            w3d_tm.Transform[0][3], w3d_tm.Transform[1][0], w3d_tm.Transform[1][1],
                            w3d_tm.Transform[1][2], w3d_tm.Transform[1][3], w3d_tm.Transform[2][0],
                            w3d_tm.Transform[2][1], w3d_tm.Transform[2][2], w3d_tm.Transform[2][3]);
...
void SceneEditorClass::Display_Sound_Spheres(bool onoff)
{
    if (m_DisplaySoundSpheres != onoff) {
        ...
        // on or off.
        //
        for (NodeClass* node = NodeMgrClass::Get_First(NODE_TYPE_SOUND); node != nullptr;
...
void SceneEditorClass::Import_Sunlight(LPCTSTR filename)
{
    ...
    bool apply_changes = (sunlight != nullptr);
    if (more_than_one) {
        ...
        if (::MessageBox(::AfxGetMainWnd()->m_hWnd,
                         "There are multiple sunlight definitions in this file which have "
                         "different settings.  Do you wish to continue?",
                         "Ambiguous Lights", MB_ICONQUESTION | MB_YESNO)
            == IDNO) {
            apply_changes = false;
        }
    }
...
bool SceneEditorClass::Compare_Lights(LightClass* light1, LightClass* light2)
{
    ...
    if (tm1 == tm2 && light1->Get_Type() == light2->Get_Type()) {
        ...
        if (ambient1 == ambient2 && specular1 == specular2 && diffuse1 == diffuse2
    ...
    if (count > 0) {
        ...
        if (dialog.DoModal() == IDOK) {
            ...
            for (int index = 0; index < count; index++) {
                ...
                if (old_obj_node != nullptr && new_obj_node != nullptr) {
...
//
////////////////////////////////////////////////////////////////
void SceneEditorClass::Generate_Uniform_Sampled_Vis(float granularity, float sample_height,
                                                    bool ignore_bias, bool selection_only,
                                                    bool farm_mode, int farm_cpu_index,
                                                    int farm_cpu_total)
{
...
//
////////////////////////////////////////////////////////////////
void SceneEditorClass::Generate_Edge_Sampled_Vis(float granularity, bool ignore_bias,
                                                 bool farm_mode, int farm_cpu_index,
                                                 int farm_cpu_total)
{
    //
    ...
    for (int index = first_node; index < last_node && !progress_obj.Is_Cancel_Requested();
         index++) {
    ...
    dialog->Set_Progress_Obj(nullptr);
    ...
    for (int index = 0; index < node_list.Count(); index++) {
        ...
        if (node != nullptr) {
            ...
            if (type == NODE_TYPE_ZONE || type == NODE_TYPE_WAYPATH || type == NODE_TYPE_WAYPOINT
                || type == NODE_TYPE_LIGHT || type == NODE_TYPE_TRANSITION
                || type == NODE_TYPE_TRANSITION_CHARACTER || type == NODE_TYPE_SOUND
                || type == NODE_TYPE_VIS || type == NODE_TYPE_VIS_POINT
                || type == NODE_TYPE_PATHFIND_START || type == NODE_TYPE_DUMMY_OBJECT
                || type == NODE_TYPE_COVER_SPOT || type == NODE_TYPE_COVER_ATTACK_POINT
                || type == NODE_TYPE_DAMAGE_ZONE || type == NODE_TYPE_BUILDING
...
void SceneEditorClass::Enable_Building_Power(bool onoff)
{
    ...
    // on or off.
    //
    for (NodeClass* node = NodeMgrClass::Get_First(NODE_TYPE_BUILDING); node != nullptr;
...
{
    if (node == nullptr || node->Is_Static() == false) {
        return;
    }
    ...
    if (phys != nullptr && phys->As_StaticPhysClass() != nullptr) {
...
bool SceneEditorClass::Is_Vis_Window_Visible(void)
{
    ...
    if (m_VisWindow.m_hWnd == nullptr || m_VisWindow.IsWindowVisible() == false) {
        retval = false;
    }
...
//
////////////////////////////////////////////////////////////////
void SceneEditorClass::On_Vis_Occluders_Rendered(VisRenderContextClass& context,
