...
{
public:
    SectorEdgeTableClass(int count)
        : DynamicVectorClass<SectorEdgeClass>(count)
    {
    }
    ...
    */
    ...
    for (int fi = 0; fi < MeshBuilder->Get_Face_Count(); fi++) {
        ...
        for (int vi = 0; vi < 3; vi++) {
            ...
            edgetable.Add_Edge_Unique(SectorEdgeClass(vertex0.Position, vertex1.Position,
                                                      face.VertIdx[v0], face.VertIdx[v1]));
        }
    }
