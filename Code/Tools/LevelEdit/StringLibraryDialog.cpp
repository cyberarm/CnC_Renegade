...
void StringLibraryDialogClass::OnRemove(void)
{
    ...
    if (index > 0) {
        ...
        if (category != NULL) {
            ...
            //
            CString message;
            message.Format("Are you sure you wish to remove the %s category?",
                           (LPCTSTR)category->Get_Name());
            if (::MessageBox(m_hWnd, message, "Remove Category", MB_ICONQUESTION | MB_YESNO)
                == IDYES) {
