...
void* ::operator new(size_t size)
{
    ...
    for (int iframe = 0; iframe < DATA_LEVELS + 1; iframe++) {
        ...
        if (!::StackWalk(IMAGE_FILE_MACHINE_I386, GetCurrentProcess(), GetCurrentThread(), &sf,
        ...
        if (::SymGetSymFromAddr(::GetCurrentProcess(), sf.AddrPC.Offset, &sym_displacement,
                                psymbol)) {
            // We don't care about the first stack frame (its inside the GetThreadContext call)
            if (iframe >= 1) {
                ::lstrcpyn(debug_info[iframe - 1].function_name, psymbol->Name,
                           sizeof(MY_DEBUG_INFO) - 1);
                debug_info[iframe - 1]
                    .function_name[sizeof(debug_info[iframe - 1].function_name) - 1]
                    = 0;
            }
        }
        else {
