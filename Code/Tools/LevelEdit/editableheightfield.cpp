        ...
        for (int patch_x = min_patch_x; patch_x <= max_patch_x; patch_x++) {
            ...
            for (int patch_quad_y = 0; patch_quad_y < quads_per_patch_y; patch_quad_y++) {
                for (int patch_quad_x = 0; patch_quad_x < quads_per_patch_x; patch_quad_x++) {
                    ...
                    //
                    PatchGrid[patch_index]->Set_Quad_Flags(patch_quad_index,
                                                           QuadFlags[overall_quad_index]);
                    ...
                    //
                    //	Calculate what vertices define the four corners of this quad (in patch
                    //space)
                    //
                    ...
                    for (int layer = 0; layer < MAX_TEXTURE_PASSES; layer++) {
                        ...
                        //
                        //	Is there a significant influence from one of the textures on this
                        //quad?
                        //
                        if (TextureWeights[layer][v0_index] > 0.0F
                            ...
                            material_pass = PatchGrid[patch_index]->Get_Material_Pass(
                                layer, MaterialList[layer]);
                            ...
                            if (is_set) {
                                ...
                                //
                                //	Set the vertex alpha's for the verts inside the material
                                //layer
                                //
                                material_pass->VertexAlpha[patch_v0_index]
                                    = TextureWeights[layer][v0_index] / v0_alpha_sum;
                                material_pass->VertexAlpha[patch_v1_index]
                                    = TextureWeights[layer][v1_index] / v1_alpha_sum;
                                material_pass->VertexAlpha[patch_v2_index]
                                    = TextureWeights[layer][v2_index] / v2_alpha_sum;
                                material_pass->VertexAlpha[patch_v3_index]
                                    = TextureWeights[layer][v3_index] / v3_alpha_sum;
                            }
                            else {
...
//
//////////////////////////////////////////////////////////////////////
void EditableHeightfieldClass::Update_UVs(void)
{
    return;
}
...
//
//////////////////////////////////////////////////////////////////////
void EditableHeightfieldClass::Deform_Heightfield(const Vector3& world_space_center, float amount,
...
//
//////////////////////////////////////////////////////////////////////
void EditableHeightfieldClass::Smooth_Foundation_Heightfield(const Vector3& world_space_center,
                                                             float amount, float inner_radius,
                                                             float outter_radius)
{
    //
    //	First, transform the center point into heightfield (object) space
...
//
//////////////////////////////////////////////////////////////////////
void EditableHeightfieldClass::Smooth_Heightfield(const Vector3& world_space_center, float amount,
                                                  float inner_radius, float outter_radius)
{
    //
    //	First, transform the center point into heightfield (object) space
        ...
        for (int x_pos = min_x; x_pos <= max_x; x_pos++) {
            ...
            if (dist <= outter_radius) {
                ...
                for (int index = 0; index < 7; index++) {
                    ...
                    //
                    if (new_x_pos >= 0 && new_x_pos < GridPointsX && new_y_pos >= 0
                        && new_y_pos < GridPointsY) {
...
//
//////////////////////////////////////////////////////////////////////
void EditableHeightfieldClass::Cutout_Heightfield(const Vector3& world_space_center,
                                                  float outter_radius, bool subtract)
{
    //
    //	First, transform the center point into heightfield (object) space
    //
...
//
//////////////////////////////////////////////////////////////////////
void EditableHeightfieldClass::Paint_Heightfield(const Vector3& world_space_center,
                                                 int texture_index, float amount,
                                                 float inner_radius, float outter_radius)
{
    //
    //	First, transform the center point into heightfield (object) space
...
//
////////////////////////////////////////////////////////////////
void EditableHeightfieldClass::Create(const char* heightmap_filename, float width, float height,
                                      float meters_per_point, float scale)
{
    //
    //	Load the bitmap from disk
    //
    HBITMAP bitmap = (HBITMAP)::LoadImage(nullptr, heightmap_filename, IMAGE_BITMAP, 0, 0,
...
//
////////////////////////////////////////////////////////////////
void EditableHeightfieldClass::Create(HBITMAP bitmap, float width, float height,
                                      float meters_per_point, float scale)
{
    if (bitmap == nullptr) {
        return;
    ...
    ::GetDIBits(screen_dc, bitmap, 0, bmp_info.bmHeight, (LPVOID)bmp_bits,
...
//
////////////////////////////////////////////////////////////////
void EditableHeightfieldClass::Update_Patch_Pos_And_Normals(int min_x, int min_y, int max_x,
                                                            int max_y)
{
    //
    //	Determine which patches this operation would possibly affect
