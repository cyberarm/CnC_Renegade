    ...
    if (prender_obj != NULL) {
        ...
        for (int texture = 0; texture < count; texture++) {
            ...
            if (::GetFileAttributes(texture_file) != 0xFFFFFFFF) {
                ...
                CString cache_path
                    = m_TextureCachePath + CString("\\") + ::Get_Filename_From_Path(texture_file);
...
bool FileMgrClass::Initialize_VSS(LPCTSTR ini_file_path, LPCTSTR username, LPCTSTR password)
{
    ...
    if (m_bVSSInitialized == false) {
        ...
        m_bVSSInitialized = m_DatabaseInterface->Open_Database(
            ini_file_path, username ? username : "User", password ? password : "");
...
bool FileMgrClass::Update_Files(DynamicVectorClass<LevelFileStruct*>* pupdate_list)
{
    ...
    for (int index = 0; index < plist->Count(); index++) {
        ...
        // checked out.
        //
        if (m_DatabaseInterface->Get_File_Status(pentry->m_Filename)
            != AssetDatabaseClass::CHECKED_OUT_TO_ME) {
...
bool FileMgrClass::Update_File(LPCTSTR filename)
{
    ...
    if ((filename != NULL) && m_bVSSInitialized) {
        ...
        if (::GetFileAttributes(path) == 0xFFFFFFFF
            || m_DatabaseInterface->Get_File_Status(filename)
                != AssetDatabaseClass::CHECKED_OUT_TO_ME) {
...
//
/////////////////////////////////////////////////////////////////
void FileMgrClass::Find_Files(LPCTSTR start_path, LPCTSTR wildcard,
                              DynamicVectorClass<CString>& file_list, bool recurse)
{
    CString path = start_path;
...
//
/////////////////////////////////////////////////////////////////
bool FileMgrClass::Update_Asset_Tree(void)
{
...
void FileMgrClass::Build_Include_List(LPCTSTR ini_filename, DynamicVectorClass<CString>& list)
{
    ...
    if (pini != NULL) {
        ...
        for (int index = 0; index < instance_count; index++) {
            ...
            pini->Get_String("Assets", pini->Get_Entry("Assets", index), "", filespec,
                             sizeof(filespec));
