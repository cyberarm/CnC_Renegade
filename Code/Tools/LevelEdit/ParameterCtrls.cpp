...
inline void Get_String_Size(HWND parent_wnd, LPCTSTR text, LPSIZE text_size)
{
    ...
    HFONT hold_font
        = (HFONT)::SelectObject(hdc, (HFONT)::SendMessage(parent_wnd, WM_GETFONT, 0, 0L));
...
//	Local prototypes
///////////////////////////////////////////////////////////////////////////////////////////
void Create_Float_Ctrls(CWnd* parent_wnd, CEdit& edit_ctrl, CSpinButtonCtrl& spin_ctrl,
                        const CRect& rect, int id1, int id2);
...
//
//////////////////////////////////////////////////////////////////////
void Create_Static_Text_Controls(HWND parent_wnd, const CRect& control_rect, CStatic* left_text,
                                 LPCTSTR param_name, CStatic* right_text, LPCTSTR units_name,
                                 CRect* remaining_rect)
{
    ...
    //
    left_text->Create(param_name, WS_CHILD | WS_VISIBLE | SS_LEFT, text_rect,
    ...
    if (right_text != nullptr) {
        ...
        //
        right_text->Create(units_name, WS_CHILD | WS_VISIBLE | SS_LEFT, text2_rect,
                           CWnd::FromHandle(parent_wnd));
...
void Resize_Controls(const CRect& rect, CWnd& left_wnd, CWnd& middle_wnd, CWnd& right_wnd)
{
    ...
    //
    int new_width
        = rect.Width() - (old_rect1.Width() + SPACE_WIDTH + old_rect3.Width() + SPACE_WIDTH);
    middle_wnd.SetWindowPos(nullptr, 0, 0, new_width, old_rect2.Height(), SWP_NOZORDER | SWP_NOMOVE);
    right_wnd.SetWindowPos(nullptr, old_rect2.left + new_width + SPACE_WIDTH, old_rect3.top, 0, 0,
...
int FileParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //
    m_FilePicker.Create_Picker(WS_CHILD | WS_TABSTOP | WS_VISIBLE, picker_rect,
...
void FileParameterCtrlClass::Resize(const CRect& rect)
{
    ...
    m_StaticText.SetWindowPos(nullptr, 0, 0, rect.Width(), old_rect1.Height(),
                              SWP_NOZORDER | SWP_NOMOVE);
    m_FilePicker.SetWindowPos(nullptr, 0, 0, rect.Width(), old_rect2.Height(),
                              SWP_NOZORDER | SWP_NOMOVE);
    return;
}
...
int StringsDBEntryParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //	Create the file picker control
    //
    m_StringPicker.Create_Picker(WS_CHILD | WS_TABSTOP | WS_VISIBLE, picker_rect,
                                 CWnd::FromHandle(parent_wnd), id_start);
...
void StringsDBEntryParameterCtrlClass::Resize(const CRect& rect)
{
    ...
    m_StaticText.SetWindowPos(nullptr, 0, 0, rect.Width(), old_rect1.Height(),
                              SWP_NOZORDER | SWP_NOMOVE);
    m_StringPicker.SetWindowPos(nullptr, 0, 0, rect.Width(), old_rect2.Height(),
                                SWP_NOZORDER | SWP_NOMOVE);
    return;
}
...
int StringParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //
    m_EditCtrl.Create(WS_CHILD | WS_TABSTOP | WS_VISIBLE, edit_rect, CWnd::FromHandle(parent_wnd),
                      id_start++);
...
void StringParameterCtrlClass::Resize(const CRect& rect)
{
    ...
    m_StaticText.SetWindowPos(nullptr, 0, 0, rect.Width(), old_rect1.Height(),
                              SWP_NOZORDER | SWP_NOMOVE);
    m_EditCtrl.SetWindowPos(nullptr, 0, 0, rect.Width(), old_rect2.Height(),
                            SWP_NOZORDER | SWP_NOMOVE);
    return;
}
...
int IntParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //
    CRect edit_rect;
    Create_Static_Text_Controls(parent_wnd, *pos, &m_StaticText1, param_name, &m_StaticText2,
                                units_name, &edit_rect);
    ...
    //
    m_SpinCtrl.Create(UDS_ARROWKEYS | UDS_SETBUDDYINT | UDS_AUTOBUDDY | UDS_ALIGNRIGHT
                          | UDS_NOTHOUSANDS | WS_CHILD | WS_VISIBLE,
...
int FloatParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //
    ::Create_Float_Ctrls(CWnd::FromHandle(parent_wnd), m_EditCtrl, m_SpinCtrl, edit_rect,
                         id_start++, id_start++);
...
int BoolParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //	Create the edit control
    //
    m_CheckBoxCtrl.Create(param_name, WS_CHILD | WS_TABSTOP | WS_VISIBLE | BS_AUTOCHECKBOX,
                          check_rect, CWnd::FromHandle(parent_wnd), id_start++);
...
void BoolParameterCtrlClass::Resize(const CRect& rect)
{
    ...
    m_CheckBoxCtrl.SetWindowPos(nullptr, 0, 0, rect.Width(), old_rect.Height(),
                                SWP_NOZORDER | SWP_NOMOVE);
    return;
}
...
int Vector3ParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //	Create the static text control
    //
    m_GroupBox.Create(m_Parameter->Get_Name(), WS_CHILD | WS_TABSTOP | WS_VISIBLE | BS_GROUPBOX,
                      group_rect, CWnd::FromHandle(parent_wnd), id_start++);
    ...
    //
    m_TextCtrls[0].Create("X:", WS_CHILD | WS_VISIBLE | SS_CENTER, CRect(0, 0, 10, 10),
                          CWnd::FromHandle(parent_wnd));
    m_TextCtrls[1].Create("Y:", WS_CHILD | WS_VISIBLE | SS_CENTER, CRect(0, 0, 10, 10),
                          CWnd::FromHandle(parent_wnd));
    m_TextCtrls[2].Create("Z:", WS_CHILD | WS_VISIBLE | SS_CENTER, CRect(0, 0, 10, 10),
                          CWnd::FromHandle(parent_wnd));
...
void Vector3ParameterCtrlClass::Resize(const CRect& rect)
{
    ...
    m_GroupBox.SetWindowPos(nullptr, 0, 0, group_rect.Width(), group_rect.Height(),
                            SWP_NOZORDER | SWP_NOMOVE);
    ...
    for (int index = 0; index < 3; index++) {
        ...
        m_TextCtrls[index].SetWindowPos(nullptr, edit_rect.left, y, text_size.cx, text_size.cy,
                                        SWP_NOZORDER);

        edit_rect.left += text_size.cx;
        m_EditCtrls[index].SetWindowPos(nullptr, edit_rect.left, edit_rect.top, edit_rect.Width(),
                                        edit_rect.Height(), SWP_NOZORDER);
...
int RectParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //	Create the labels, don't worry about their position, they will be moved later.
    //
    m_TextCtrls[0].Create("X1:", WS_CHILD | WS_VISIBLE | SS_CENTER, CRect(0, 0, 10, 10),
                          CWnd::FromHandle(parent_wnd));
    m_TextCtrls[1].Create("Y1:", WS_CHILD | WS_VISIBLE | SS_CENTER, CRect(0, 0, 10, 10),
                          CWnd::FromHandle(parent_wnd));
    m_TextCtrls[2].Create("X2:", WS_CHILD | WS_VISIBLE | SS_CENTER, CRect(0, 0, 10, 10),
                          CWnd::FromHandle(parent_wnd));
    m_TextCtrls[3].Create("Y2:", WS_CHILD | WS_VISIBLE | SS_CENTER, CRect(0, 0, 10, 10),
                          CWnd::FromHandle(parent_wnd));
    m_TextCtrls[0].SendMessage(WM_SETFONT, ::SendMessage(parent_wnd, WM_GETFONT, 0, 0L));
    m_TextCtrls[1].SendMessage(WM_SETFONT, ::SendMessage(parent_wnd, WM_GETFONT, 0, 0L));
    m_TextCtrls[2].SendMessage(WM_SETFONT, ::SendMessage(parent_wnd, WM_GETFONT, 0, 0L));
                        ...
                        const CRect& rect, int id1, int id2)
{
    ...
    //	Create the spin control
    //
    spin_ctrl.Create(UDS_ARROWKEYS | UDS_SETBUDDYINT | UDS_AUTOBUDDY | UDS_NOTHOUSANDS
                         | UDS_ALIGNRIGHT | WS_CHILD | WS_VISIBLE,
...
int EnumParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //	Create the static text control
    //
    m_StaticText.Create(param_name, WS_CHILD | WS_VISIBLE | SS_LEFT, text_rect,
                        CWnd::FromHandle(parent_wnd));
    m_StaticText.SendMessage(WM_SETFONT, ::SendMessage(parent_wnd, WM_GETFONT, 0, 0L));
    ...
    m_ComboBox.Create(WS_CHILD | WS_TABSTOP | WS_VISIBLE | WS_VSCROLL | CBS_DROPDOWNLIST,
                      combo_rect, CWnd::FromHandle(parent_wnd), id_start++);
...
int ColorParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //
    m_ButtonID = id_start++;
    m_Button.Create("", WS_CHILD | WS_TABSTOP | WS_VISIBLE | BS_PUSHBUTTON | BS_OWNERDRAW,
                    button_rect, CWnd::FromHandle(parent_wnd), m_ButtonID);
...
//
//////////////////////////////////////////////////////////////////////
void ColorParameterCtrlClass::Read_Data(HWND /*parent_wnd*/)
{
    return;
}
...
bool ColorParameterCtrlClass::On_DrawItem(HWND parent_wnd, WPARAM wparam, LPARAM lparam)
{
    ...
    if ((int)wparam == m_ButtonID) {
        ...
        temp_dc.FillSolidRect(&rect,
                              RGB(int(color.X * 255), int(color.Y * 255), int(color.Z * 255)));
        temp_dc.Detach();
...
int PhysDefParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //
    m_ButtonID = id_start++;
    m_Button.Create("Edit Phyiscs Object...", WS_CHILD | WS_TABSTOP | WS_VISIBLE | BS_PUSHBUTTON,
                    button_rect, CWnd::FromHandle(parent_wnd), m_ButtonID);
    m_Button.SendMessage(WM_SETFONT, ::SendMessage(parent_wnd, WM_GETFONT, 0, 0L));
...
//
//////////////////////////////////////////////////////////////////////
void PhysDefParameterCtrlClass::Read_Data(HWND /*parent_wnd*/)
{
    return;
}
...
int DefParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //
    m_PresetPicker.Create_Picker(WS_CHILD | WS_TABSTOP | WS_VISIBLE, picker_rect,
                                 CWnd::FromHandle(parent_wnd), id_start);
...
void DefParameterCtrlClass::Resize(const CRect& rect)
{
    ...
    int new_width = rect.Width() - (old_rect1.Width() + SPACE_WIDTH);
    m_PresetPicker.SetWindowPos(nullptr, 0, 0, new_width, old_rect2.Height(),
                                SWP_NOZORDER | SWP_NOMOVE);
    return;
}
...
//
//////////////////////////////////////////////////////////////////////
void DefParameterCtrlClass::On_Destroy(void)
{
    return;
}
...
int ScriptParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //
    m_CheckBoxCtrl.Create(param_name, WS_CHILD | WS_TABSTOP | WS_VISIBLE | BS_AUTOCHECKBOX,
                          text_rect, CWnd::FromHandle(parent_wnd), id_start++);
    m_CheckBoxCtrl.SendMessage(WM_SETFONT, ::SendMessage(parent_wnd, WM_GETFONT, 0, 0L));
...
bool ScriptParameterCtrlClass::On_Command(HWND parent_wnd, WPARAM wparam, LPARAM lparam)
{
    ...
    if (LOWORD(wparam) == m_ButtonID && HIWORD(wparam) == BN_CLICKED) {
        ...
        //
        EditScriptClass* template_script
            = ScriptMgrClass::Find_Script(m_Parameter->Get_Script_Name());
...
int DefIDListParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //
    m_ButtonID = id_start++;
    m_Button.Create("Pick Presets...", WS_CHILD | WS_TABSTOP | WS_VISIBLE | BS_PUSHBUTTON,
                    button_rect, CWnd::FromHandle(parent_wnd), m_ButtonID);
    m_Button.SendMessage(WM_SETFONT, ::SendMessage(parent_wnd, WM_GETFONT, 0, 0L));
...
//
//////////////////////////////////////////////////////////////////////
void DefIDListParameterCtrlClass::Read_Data(HWND /*parent_wnd*/)
{
    return;
}
...
//
//////////////////////////////////////////////////////////////////////
void ZoneParameterCtrlClass::Read_Data(HWND /*parent_wnd*/)
{
    return;
}
...
int FilenameListParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    //
    m_ButtonID = id_start++;
    m_Button.Create("Edit File List...", WS_CHILD | WS_TABSTOP | WS_VISIBLE | BS_PUSHBUTTON,
                    button_rect, CWnd::FromHandle(parent_wnd), m_ButtonID);
    m_Button.SendMessage(WM_SETFONT, ::SendMessage(parent_wnd, WM_GETFONT, 0, 0L));
...
//
//////////////////////////////////////////////////////////////////////
void FilenameListParameterCtrlClass::Read_Data(HWND /*parent_wnd*/)
{
    return;
}
...
int SeparatorParameterCtrlClass::Create(HWND parent_wnd, int id_start, LPRECT pos)
{
    ...
    m_FrameCtrl.Create(param_name, WS_CHILD | WS_VISIBLE | SS_ETCHEDHORZ, frame_rect,
                       CWnd::FromHandle(parent_wnd), id_start++);
    ...
    m_TextCtrl.Create(param_name, WS_CHILD | WS_VISIBLE | SS_LEFT, ctrl_rect,
                      CWnd::FromHandle(parent_wnd), id_start++);
...
void SeparatorParameterCtrlClass::Resize(const CRect& rect)
{
    ...
    m_FrameCtrl.SetWindowPos(nullptr, 0, 0, rect.Width(), old_rect.Height(),
                             SWP_NOZORDER | SWP_NOMOVE);

    // m_TextCtrl.GetWindowRect (&old_rect);
    // m_TextCtrl.SetWindowPos (nullptr, 0, 0, rect.Width (), old_rect.Height (), SWP_NOZORDER |
    // SWP_NOMOVE);
    return;
}
