...
void PresetsFormClass::OnSize(UINT nType, int cx, int cy)
{
    ...
    if (::IsWindow(m_TreeCtrl) && (cx > 0) && (cy > 0)) {
        ...
        //
        m_TreeCtrl.SetWindowPos(NULL, 0, 0, cx - ((list_rect.left - parentrect.left) << 1),
                                    ...
                                    bool temps_only)
{
    ...
    PresetClass* preset = NULL;
    for (preset = PresetMgrClass::Get_First(); preset != NULL;
...
bool PresetsFormClass::Enable_Button(int cmd_id)
{
    ...
    if (current_item != NULL) {
        ...
        case IDC_CONVERT:
            ...
            if (preset != NULL && preset->Get_Parent() != NULL
...
void PresetsFormClass::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    //
    if (icon != NULL) {
        ::DrawState(lpDrawItemStruct->hDC, NULL, NULL, (LPARAM)icon, 0, rect.left + 4, rect.top + 4,
...
{
    CFileDialog dialog(TRUE, ".w3d", NULL,
        ...
        if (drag_preset != NULL && (drop_preset != NULL || factory != NULL)) {
            ...
            if (drop_preset == NULL || drop_preset->Is_A_Parent(drag_preset) == false) {
                ...
                if (drag_classid == drop_classid) {
                    ...
                    bool is_drop_temp
                        = (drop_preset != NULL && drop_preset->Get_IsTemporary()) ? true : false;
                    bool is_drag_temp
                        = (drag_preset != NULL && drag_preset->Get_IsTemporary()) ? true : false;
...
void PresetsFormClass::Do_Drop(HTREEITEM drag_item, HTREEITEM drop_item)
{
    ...
    if (is_drag_temp == false) {
        ...
        if (PresetMgrClass::Check_Out_Database(class_id, &undo_on_err)) {
            ...
            //
            if (drag_preset != NULL && drag_preset->Get_Definition()->Get_ID() == drag_preset_id
