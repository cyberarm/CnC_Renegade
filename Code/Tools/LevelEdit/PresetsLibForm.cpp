...
void PresetsFormClass::OnSize(UINT nType, int cx, int cy)
{
    ...
    if (::IsWindow(m_TreeCtrl) && (cx > 0) && (cy > 0)) {
        ...
        //
        m_TreeCtrl.SetWindowPos(nullptr, 0, 0, cx - ((list_rect.left - parentrect.left) << 1),
...
void PresetsFormClass::Reload_Presets(void)
{
    ...
    for (int index = 0; index < PRESET_CATEGORY_COUNT; index++) {
        ...
        //
        HTREEITEM tree_item
            = m_TreeCtrl.InsertItem(PRESET_CATEGORIES[index].name, FOLDER_ICON, FOLDER_ICON);
        ...
        if (factory != nullptr) {
            ...
            for (factory = DefinitionFactoryMgrClass::Get_First(PRESET_CATEGORIES[index].clsid);
                ...
                if (factory->Is_Displayed()) {
                    ...
                    HTREEITEM child_item = m_TreeCtrl.InsertItem(factory->Get_Name(), FOLDER_ICON,
                                                                 FOLDER_ICON, tree_item);
...
bool PresetsFormClass::Load_Preset_Libraries(void)
{
    ...
    //
    DefinitionFactoryClass* factory = nullptr;
    for (factory = DefinitionFactoryMgrClass::Get_First(); factory != nullptr;
...
bool PresetsFormClass::Load_Presets(uint32 class_id)
{
    ...
    //	Open the file
    //
    HANDLE file = ::CreateFile(path, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0L, nullptr);
    ...
    if (file != INVALID_HANDLE_VALUE) {
        ...
        //
        int file_size = ::GetFileSize(file, nullptr);
        if (file_size > 0 && file_size < 99999999) {
            ...
            DWORD bytes_read = 0;
            ::ReadFile(file, buffer, file_size, &bytes_read, nullptr);
...
bool PresetsFormClass::Load_Temp_Presets(void)
{
    ...
    //	Open the file
    //
    HANDLE hfile = ::CreateFile(path, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0L, nullptr);
...
//
/////////////////////////////////////////////////////////////////////////////
void PresetsFormClass::Build_DDB_File_List(DynamicVectorClass<CString>& file_list,
                                           HTREEITEM parent_item)
{
    //
    //	Loop over all the immediate children
    //
    for (HTREEITEM child_item = m_TreeCtrl.GetChildItem(parent_item); child_item != nullptr;
...
//
/////////////////////////////////////////////////////////////////////////////
bool PresetsFormClass::Save_Presets(LPCTSTR path, uint32 class_id, bool class_id_matters,
                                    bool temps_only)
{
    ///
    ...
    //
    HANDLE file = ::CreateFile(path, GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, 0L, nullptr);
...
//
/////////////////////////////////////////////////////////////////////////////
void PresetsFormClass::Save_Presets(HANDLE file, uint32 class_id, bool class_id_matters,
                                    bool temps_only)
{
    //
    ...
    PresetMgrClass::Remove_Non_Matching_Presets(class_id, class_id_matters, temps_only,
                                                preset_list);
    ...
    //
    DefinitionClass* definition = nullptr;
    for (definition = DefinitionMgrClass::Get_First(); definition != nullptr;
    ...
    PresetClass* preset = nullptr;
    for (preset = PresetMgrClass::Get_First(); preset != nullptr;
...
void PresetsFormClass::OnDelete(void)
{
    ...
    if (preset != nullptr) {
        ...
        //
        if (::Message_Box(m_hWnd, IDS_DELETE_PRESET_MSG, IDS_DELETE_PRESET_TITLE,
            ...
            ;
            if (was_temp
                || PresetMgrClass::Check_Out_Database(preset->Get_Class_ID(), &undo_on_err)) {
                sel_capture.Restore();
...
{
    for (HTREEITEM tree_item = m_TreeCtrl.GetChildItem(root_item); tree_item != nullptr;
         tree_item = m_TreeCtrl.GetNextSiblingItem(tree_item)) {
        //
        //	Sort the children
        //
...
bool PresetsFormClass::Enable_Button(int cmd_id)
{
    ...
    if (current_item != nullptr) {
        ...
        case IDC_CONVERT:
            ...
            if (preset != nullptr && preset->Get_Parent() != nullptr
...
void PresetsFormClass::Propagate_Changes(HTREEITEM modified_item, DERIVED_PARAM_LIST& list)
{
    ...
    for (int index = 0; index < list.Count(); index++) {
        ...
        for (int list_index = 0; list_index < list_count; list_index++) {
            // int param_index						=
            // info.inherited_param_list[list_index];
...
//
////////////////////////////////////////////////////////////////////////////
void PresetsFormClass::Compare_Derived_Parameters(PresetClass* curr_preset,
                                                  ...
                                                  DERIVED_PARAM_LIST& list)
{
    ...
    for (int param_index = 0; param_index < param_count; param_index++) {
        ...
        if (*curr_parameter == *parameter) {
            ...
            DefinitionClass* derived_model_def
                = DefinitionMgrClass::Find_Definition(derived_id, false);
...
void PresetsFormClass::OnExtra(void)
{
    ...
    ::ModifyMenu(menu, IDM_BATCH_IMPORT_TERRAIN, MF_BYCOMMAND | MF_OWNERDRAW,
                 IDM_BATCH_IMPORT_TERRAIN, nullptr);
    ::ModifyMenu(menu, IDM_UPDATE_EMBEDDED_NODE_LIST, MF_BYCOMMAND | MF_OWNERDRAW,
                 IDM_UPDATE_EMBEDDED_NODE_LIST, nullptr);
    ::ModifyMenu(menu, IDM_CLEAR_EMBEDDED_NODE_LIST, MF_BYCOMMAND | MF_OWNERDRAW,
                 IDM_CLEAR_EMBEDDED_NODE_LIST, nullptr);
    ...
    if (Enable_Button(IDM_UPDATE_EMBEDDED_NODE_LIST) == false) {
        ::EnableMenuItem(menu, IDM_UPDATE_EMBEDDED_NODE_LIST,
                         MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
    }

    if (Enable_Button(IDM_CLEAR_EMBEDDED_NODE_LIST) == false) {
        ::EnableMenuItem(menu, IDM_CLEAR_EMBEDDED_NODE_LIST,
                         MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
    }
...
void PresetsFormClass::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    case IDC_PLAY:
        icon = (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_PLAY), IMAGE_ICON,
                                  16, 16, LR_SHARED);
        break;

    case IDC_CONVERT:
        icon = (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_CONVERT),
                                  IMAGE_ICON, 16, 16, LR_SHARED);
        break;

    case IDC_UPDATE_VSS:
        icon = (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_UPDATE), IMAGE_ICON,
                                  16, 16, LR_SHARED);
        break;

    case IDM_BATCH_IMPORT_TERRAIN:
        icon = (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_TERRAIN),
                                  IMAGE_ICON, 16, 16, LR_SHARED);
        break;

    case IDM_UPDATE_EMBEDDED_NODE_LIST:
        icon = (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_OBJECT), IMAGE_ICON,
                                  16, 16, LR_SHARED);
        break;

    case IDM_CLEAR_EMBEDDED_NODE_LIST:
        icon = (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_DELETE), IMAGE_ICON,
                                  16, 16, LR_SHARED);
        break;
    }
    ...
    ::DrawState(lpDrawItemStruct->hDC, nullptr, nullptr, (LPARAM)(LPCTSTR)text, 0, rect.left + 24,
    ...
    //
    if (icon != nullptr) {
        ::DrawState(lpDrawItemStruct->hDC, nullptr, nullptr, (LPARAM)icon, 0, rect.left + 4, rect.top + 4,
                    16, 16, DST_ICON | dss_state);
    }
...
void PresetsFormClass::OnConvert(void)
{
    ...
    if (preset != nullptr) {
        ...
        CString message;
        message.Format(
            "Are you sure you wish to convert '%s' from a temporary preset to a shared preset?",
            preset->Get_Name());
...
void PresetsFormClass::OnBuildEmbedNodeList(void)
{
    ...
    if (::MessageBox(m_hWnd,
                     "This operation will embed a copy of all the nodes in scene into this preset. "
                     " Are you sure you wish to continue?",
                     "Embed Nodes", MB_ICONQUESTION | MB_YESNO)
        == IDNO) {
        return;
    }
        ...
        if (preset_name.GetLength() > 0) {
            ...
            if (preset != nullptr && preset->Get_Class_ID() == CLASSID_TERRAIN) {
                ...
                TerrainDefinitionClass* definition
                    = (TerrainDefinitionClass*)preset->Get_Definition();
        ...
        if (drag_preset != nullptr && (drop_preset != nullptr || factory != nullptr)) {
            ...
            if (drop_preset == nullptr || drop_preset->Is_A_Parent(drag_preset) == false) {
                ...
                if (drag_classid == drop_classid) {
                    ...
                    bool is_drop_temp
                        = (drop_preset != nullptr && drop_preset->Get_IsTemporary()) ? true : false;
                    bool is_drag_temp
                        = (drag_preset != nullptr && drag_preset->Get_IsTemporary()) ? true : false;

                    //
                    //	We allow the drop if the target is a factory, or both the drag and drops are
                    //non-temp
                    //
...
void PresetsFormClass::Do_Drop(HTREEITEM drag_item, HTREEITEM drop_item)
{
    ...
    if (is_drag_temp == false) {
        ...
        if (PresetMgrClass::Check_Out_Database(class_id, &undo_on_err)) {
            ...
            //
            if (drag_preset != nullptr && drag_preset->Get_Definition()->Get_ID() == drag_preset_id
...
void PresetsFormClass::Copy_Preset_Items(HTREEITEM preset_item, HTREEITEM parent_item)
{
    ...
    //
    for (HTREEITEM child_item = m_TreeCtrl.GetChildItem(preset_item); child_item != nullptr;
         child_item = m_TreeCtrl.GetNextSiblingItem(child_item)) {
...
//
////////////////////////////////////////////////////////////////////////////
void PresetsFormClass::Build_File_Dependencies_For_Definition(
    DynamicVectorClass<StringClass>& file_list, DefinitionClass* definition)
{
    //
    ...
    for (int index = 0; index < count; index++) {
        ...
        if (parameter->Is_Type(ParameterClass::TYPE_FILENAME)) {
            FilenameParameterClass* filename_param
                = reinterpret_cast<FilenameParameterClass*>(parameter);
