 ...
 *                                                                                             *
 *                     $Archive:: /Commando/Code/Tools/LevelEdit/PathfindSectorBuilder.cpp
 *$Modtime:: 5/27/00 4:10p                                               $*
 *                                                                                             *
 *                    $Revision:: 56                                                          $*
 *                                                                                             *
 *---------------------------------------------------------------------------------------------*
...
bool PathfindSectorBuilderClass::Find_Ground(const Vector3& pos, float* ground_pos)
{
    ...
    if ((result.StartBad == false) && (result.Fraction < 1.0F) && (result.Normal.Z > 0.71F)
        ...
        //
        (*ground_pos)
            = (sweep_box.Center.Z + (sweep_vector.Z * result.Fraction)) - sweep_box.Extent.Z;
        retval = true;
    }
...
//
//////////////////////////////////////////////////////////////////////////
bool PathfindSectorBuilderClass::Try_Moving_Here(const Vector3& start_pos,
                                                 const Vector3& expected_pos, AABoxClass* real_pos)
{
    bool is_valid = false;
...
//
//////////////////////////////////////////////////////////////////////////
bool PathfindSectorBuilderClass::Try_Standing_Here(const Vector3& expected_pos,
                                                   AABoxClass* real_pos)
{
    bool is_valid = false;
    ...
    if ((result.StartBad == false) && (result.Normal.Z > 0.71F)) {
        ...
        if (on_ground && (m_AllowWaterFloodfill || (result.SurfaceType != SURFACE_TYPE_WATER))) {
            ...
            real_pos->Extent.Z
                = ((-m_SimMovement.Z * result.Fraction) * 0.5F) + (m_SimBoundingBox.Z * 0.5F);
            real_pos->Center.X = expected_pos.X;
            real_pos->Center.Y = expected_pos.Y;
            real_pos->Center.Z
                = (sweep_box.Center.Z + (m_SimBoundingBox.Z * 0.5F) - real_pos->Extent.Z) + 0.001F;
            is_valid = true;
        }
    }
...
//
//////////////////////////////////////////////////////////////////////////
void PathfindSectorBuilderClass::Do_Real_Physics_Sim(const Vector3& start_pos,
                                                     PATHFIND_DIR direction)
{
    ...
    if (found == false) {
        ...
        PhysicsSceneClass::Get_Instance()->Collect_Collideable_Objects(
            test_box, COLLISION_TYPE_PHYSICAL, true, false, &obj_list);
...
//
//////////////////////////////////////////////////////////////////////////
FloodfillBoxClass* PathfindSectorBuilderClass::Submit_Box(FloodfillBoxClass* from_obj,
...
void PathfindSectorBuilderClass::Floodfill(const Vector3& start_pos)
{
    ...
    if (m_pDialog->Was_Cancelled() == false) {
        ...
        if (m_BeforeUpdateCount > 512) {
            CString message;
            message.Format("Floodfilled %d boxes. Approx total boxes: %d", m_TotalBoxCount,
                           m_TotalBoxGuess);
...
//
///////////////////////////////////////////////////////////////////////////
void PathfindSectorBuilderClass::Determine_Height(FloodfillBoxClass* start_box, float* min_z_pos,
                                                  float* max_z_pos)
{
...
int PathfindSectorBuilderClass::Build_Height_Values(void)
{
    ...
    WWDEBUG_SAY(
        ("Time spent generating z-values: %d secs.\r\n", (after_ticks - before_ticks) / 1000));
...
//
//////////////////////////////////////////////////////////////////////////
inline FloodfillBoxClass* PathfindSectorBuilderClass::Move_Dir(FloodfillBoxClass* start_box,
                                                               PATHFIND_DIR dir, int dir_mask)
{
    //
    ...
    //
    if (start_box->Is_Two_Way_Traversible(dir) && Can_Traverse(new_cell, dir)
...
//
//////////////////////////////////////////////////////////////////////////
inline bool PathfindSectorBuilderClass::Check_Edge(FloodfillBoxClass* start_box, int count_left,
                                                   int count_right, int count_up, int count_down,
...
//
//////////////////////////////////////////////////////////////////////////
FloodfillBoxClass* PathfindSectorBuilderClass::Find_Perimeter(FloodfillBoxClass* start_box,
                                                              BOX_PERIMETER* perimeter)
{
    ...
    m_HeightWatcher.Initialize(pos.Z - (m_SimBoundingBox.Z * 500.0F),
                               pos.Z + (m_SimBoundingBox.Z * 500.0F), pos.Z);
...
void PathfindSectorBuilderClass::Compress_Sectors(DynamicVectorClass<AABoxClass>* box_list)
{
    ...
    while (keep_going && ((start_box = FloodfillBoxClass::Get_First()) != nullptr)) {
        ...
        if ((start_box == old_start_box)
            ...
            ::MessageBox(::AfxGetMainWnd()->m_hWnd, "Found the same box twice...",
                         "Compression Error", MB_OK | MB_ICONERROR);
            keep_going = false;
        }
...
//////////////////////////////////////////////////////////////////////////
/*void
PathfindSectorBuilderClass::Filter_Useless_Sectors (DynamicVectorClass<PathfindSectorClass *>
&sector_list)
{
...
//
//////////////////////////////////////////////////////////////////////////
void Scan_Edge(FloodfillBoxClass* start_obj, int cell_count, PATHFIND_DIR check_dir,
               PATHFIND_DIR move_dir, int* best_count)
{
...
//
//////////////////////////////////////////////////////////////////////////
bool PathfindSectorBuilderClass::Is_Valid_Sector(FloodfillBoxClass** upper_left_ptr,
                                                 int& cells_right, int& cells_down)
{
    bool retval = true;
...
//
//////////////////////////////////////////////////////////////////////////
PathfindSectorClass* PathfindSectorBuilderClass::Build_Sector(FloodfillBoxClass* upper_left_ptr,
                                                              int cells_right, int cells_down)
{
    ...
    for (int right = 0; (right < cells_right); right++) {
        ...
        for (int down = 0; (down < cells_down); down++) {
            ...
            //
            if ((down == 0) || (right == 0) || (down == (cells_down - 1))
                || (right == (cells_right - 1))) {
                if (is_valid) {
            ...
            if (transition != nullptr) {
                ...
                case TransitionDataClass::LADDER_ENTER_BOTTOM: {
                    ...
                    if (transition->Get_Type() == TransitionDataClass::LADDER_ENTER_BOTTOM) {
                        ...
                        exit_transition = Find_Transition(TransitionDataClass::LADDER_EXIT_BOTTOM,
                                                          pos, -100.0F, &exit_node);
                    }
...
void PathfindSectorBuilderClass::Post_Process_Floodfill_For_Level_Features(void)
{
    ...
    for (int index = 0; index < m_LevelFeatureList.Count(); index++) {
        ...
        if (level_feature != nullptr) {
            ...
            //
            m_BodyBoxCullingSystem.Collect_Boxes(box);
            DynamicVectorClass<FloodfillBoxClass*>& list
                = m_BodyBoxCullingSystem.Get_Collection_List();
            for (int index = 0; index < list.Count(); index++) {
                FloodfillBoxClass* body_box = list[index];
...
PathfindSectorBuilderClass::Find_Transition(TransitionDataClass::StyleType transition_type,
    ...
    for (node = (TransitionNodeClass*)NodeMgrClass::Get_First(NODE_TYPE_TRANSITION); node != nullptr;
        ...
        if (index >= 0) {
            ...
            if (transition != nullptr) {
                ...
                float curr_delta
                    = min(WWMath::Fabs(start_z - curr_z_min), WWMath::Fabs(start_z - curr_z_max));
...
//
///////////////////////////////////////////////////////////////////////////
PathfindSectorClass* PathfindSectorBuilderClass::Find_Sector(const Vector3& point,
...
void PathfindSectorBuilderClass::Check_For_Level_Feature(FloodfillBoxClass* body_box)
{
    if (m_LevelFeatureCullingSystem.Object_Count() > 0) {
        ...
        //
        //	Loop through all the features in our list and process any 'portals' (ladders, doors,
        //etc)
        //
        for (int index = 0; index < feature_list.Count(); index++) {
            ...
            //
            if ((pathfind_box.Center.X + pathfind_box.Extent.X
                 < feature_box.Center.X + feature_box.Extent.X)
                && (pathfind_box.Center.X - pathfind_box.Extent.X
                    > feature_box.Center.X - feature_box.Extent.X)
                && (pathfind_box.Center.Y + pathfind_box.Extent.Y
                    < feature_box.Center.Y + feature_box.Extent.Y)
                && (pathfind_box.Center.Y - pathfind_box.Extent.Y
                    > feature_box.Center.Y - feature_box.Extent.Y)) {
                //
                //	Continue our pathfind across this feature (i.e. up the ladder, down the
                //elevator, etc)
                //
...
//
////////////////////////////////////////////////////////////////////////////
UINT fnPathfindDialogThread(DWORD dwparam1, DWORD dwparam2, DWORD /*dwparam3*/,
