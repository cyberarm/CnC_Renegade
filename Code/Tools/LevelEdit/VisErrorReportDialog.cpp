...
{
    return (HBITMAP)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(res_id), IMAGE_BITMAP, 0,
                                0, LR_LOADTRANSPARENT);
}
...
void VisErrorReportDialogClass::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    if (nIDCtl == IDC_PREV_ERROR) {
        hbmp = ::IsWindowEnabled(lpDrawItemStruct->hwndItem) ? m_PrevButton.normal
                                                             : m_PrevButton.disabled;
    }
    else if (nIDCtl == IDC_NEXT_ERROR) {
        hbmp = ::IsWindowEnabled(lpDrawItemStruct->hwndItem) ? m_NextButton.normal
...
void VisErrorReportDialogClass::Paint_Axis_Area(HDC hdc)
{
    ...
    for (int index = 0; index < 6; index++) {
        if (m_AxisSel & (1 << index)) {
            ...
            ::BitBlt(hdc, x_pos, y_pos, rect.Width(), rect.Height(), mem_dc, rect.left, rect.top,
                     SRCCOPY);
        }
    }
