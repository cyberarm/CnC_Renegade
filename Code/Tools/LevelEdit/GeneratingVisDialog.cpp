...
void GeneratingVisDialogClass::Update_Time(void)
{
    ...
    elapsed_time_text.Format("Elapsed: %.1f min.  Remaining: %.1f min.", elapsed_minutes,
                             remaining_minutes);
...
void GeneratingVisDialogClass::Render_Vis_Points(VIS_POINT_LIST& point_list)
{
    ...
    for (int index = 0; (index < count) && !m_bStop; index++) {
        ...
        VisSampleClass vis_sample
            = scene_editor->Update_Vis(sample_point, sub_point_list->transform);
        ...
        for (int sub_point = 0; sub_point < sub_point_list->Count(); sub_point++) {
            ...
            //
            vis_sample = scene_editor->Update_Vis(sample_point, transform,
                                                  VisDirBitsType(VIS_FORWARD_BIT | VIS_LEFT_BIT
                                                                 | VIS_RIGHT_BIT | VIS_UP_BIT
                                                                 | VIS_DOWN_BIT));
            vis_log.Log_Sample(vis_sample);
...
//
//////////////////////////////////////////////////////////////////////////////
void GeneratingVisDialogClass::Generate_Points(NODE_LIST& node_list,
                                               VisPointGeneratorClass& generator)
{
    ...
    for (index = 0; (index < count) && !m_bStop; index++) {
        ...
        CString status_text;
        status_text.Format("Processing %d / %d nodes, %d / %d polys.", index + 1, count, poly_count,
                           total_polys);
        SetDlgItemText(IDC_STATUS_TEXT, status_text);

        CString elapsed_text;
        elapsed_text.Format("Elapsed Time: %.1f minutes.",
                            (::GetTickCount() - m_StartTime) / 60000.0f);
