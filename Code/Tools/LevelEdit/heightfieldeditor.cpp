...
HeightfieldEditorClass::EDITING_MODE HeightfieldEditorClass::Mode
    = HeightfieldEditorClass::MODE_DEFORM;
...
//
//////////////////////////////////////////////////////////////////////
void HeightfieldEditorClass::Initialize(void)
{
    return;
}
...
void HeightfieldEditorClass::On_Frame_Update(void)
{
    ...
    if (phys_obj != nullptr) {
        ...
        PhysicsSceneClass::Get_Instance()->Add_Debug_AABox(
            AABoxClass(res.ContactPoint + Vector3(0, 0, 2.0F), Vector3(1.0F, 1.0F, 1.0F)),
            Vector3(1, 0, 0));
        ...
        if (model != nullptr && model->Class_ID() == RenderObjClass::CLASSID_RENEGADE_TERRAIN
            && CurrentHeightfield != nullptr) {
            ...
            if (::Get_Mouse_Mgr()->Is_LButton_Down()) {
                ...
                //
                if (Mode == MODE_DEFORM) {
                    heightfield->Deform_Heightfield(res.ContactPoint, BrushAmount, BrushInnerRadius,
                                                    BrushOutterRadius);
                }
                else if (Mode == MODE_DEFORM_SMOOTH) {
                    heightfield->Smooth_Heightfield(res.ContactPoint, BrushAmount, BrushInnerRadius,
                                                    BrushOutterRadius);
                }
                else if (Mode == MODE_DEFORM_SMOOTH_FOUNDATION) {
                    heightfield->Smooth_Foundation_Heightfield(res.ContactPoint, BrushAmount,
                                                               BrushInnerRadius, BrushOutterRadius);
                }
                    ...
                    heightfield->Paint_Heightfield(res.ContactPoint, CurrentTextureIndex,
                                                   BrushAmount, BrushInnerRadius,
                                                   BrushOutterRadius);
                }
            }
                ...
                //
                if (Mode == MODE_DEFORM) {
                    heightfield->Deform_Heightfield(res.ContactPoint, -BrushAmount,
                                                    BrushInnerRadius, BrushOutterRadius);
                }
                else if (Mode == MODE_QUAD_CUTOUT) {
...
//
//////////////////////////////////////////////////////////////////////
void HeightfieldEditorClass::Render(void)
{
...
HeightfieldEditorClass::Create_Texture_Thumbnail(const char* filename, int width, int height)
{
    ...
    IDirect3DSurface8* d3d_surface
        = TextureLoader::Load_Surface_Immediate(filename, WW3D_FORMAT_R8G8B8, false);
    ...
    HBITMAP bitmap = ::CreateDIBSection(screen_dc, (const BITMAPINFO*)&bitmap_info, DIB_RGB_COLORS,
                                        (void**)&dest_bits, nullptr, 0L);
