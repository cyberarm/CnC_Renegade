...
BOOL EditConversationDialogClass::OnInitDialog(void)
{
    ...
    for (int index = 0; index < ORATOR_MAX; index++) {
        ...
        //
        HICON icon
            = ::LoadIcon(::AfxGetResourceHandle(), MAKEINTRESOURCE(ORATOR_CTRLS[index].icon_id));
        ...
        //
        icon = (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_EYE2), IMAGE_ICON,
    ...
    for (index = 0; index < orator_count; index++) {
        ...
        SendDlgItemMessage(ORATOR_CTRLS[index].check_id, BM_SETCHECK,
                           (WPARAM)(!orator->Is_Invisible()));
    ...
    //
    HICON lock_icon = (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_LOCK),
...
void EditConversationDialogClass::OnOK(void)
{
    ...
    for (index = 0; index < ORATOR_MAX; index++) {
        ...
        if (SendDlgItemMessage(ORATOR_CTRLS[index].button_id, BM_GETCHECK) == 1) {
            bool is_invisible
                = (SendDlgItemMessage(ORATOR_CTRLS[index].check_id, BM_GETCHECK) != 1);
...
int EditConversationDialogClass::Get_Orator_Type(int orator_index)
{
    ...
    int curr_sel = SendDlgItemMessage(ORATOR_CTRLS[orator_index].combo_id, CB_GETCURSEL);
    if (curr_sel >= 0) {
        retval = SendDlgItemMessage(ORATOR_CTRLS[orator_index].combo_id, CB_GETITEMDATA,
                                    (WPARAM)curr_sel);
    }
