...
void Paint_Gradient(HWND hwnd, BYTE base_red, BYTE base_green, BYTE base_blue)
{
    ...
    for (int shade = 0; shade < 256; shade++) {
        // Paint this sliver
        cdc.FillSolidRect((int)posx, 0, (width_per_shade >= 1.00F) ? (int)width_per_shade : 1,
                          height, RGB(shade * base_red, shade * base_green, shade * base_blue));
...
//
////////////////////////////////////////////////////////////////////////////
void Rotate_Matrix(const Matrix3D& input_mat, const Matrix3D& rotation_mat,
                   const Matrix3D& coord_system, Matrix3D* poutput_mat)
{
    /*
...
//
////////////////////////////////////////////////////////////////////////////
void Create_Worker_Thread(MY_THREADPROC fnthread_proc, DWORD dwparam1, DWORD dwparam2,
                          DWORD dwparam3, HRESULT* presult)
{
...
UINT fnUIThread(LPVOID pParam)
{
    ...
    if (pthread_params != NULL) {
        ...
        (thread_proc)(pthread_params->dwparam1, pthread_params->dwparam2, pthread_params->dwparam3,
                      pthread_params->presult, pthread_params->phmain_wnd);
...
//
////////////////////////////////////////////////////////////////////////////
void Create_UI_Thread(MY_UITHREADPROC fnthread_proc, DWORD dwparam1, DWORD dwparam2, DWORD dwparam3,
    ...
    for (int index = 0; index < group_list.Count(); index++) {
        ...
        if (pgroup != NULL) {
            ...
            int index = ::SendMessage(hcombobox, CB_ADDSTRING, (WPARAM)0,
                                      (LPARAM)(LPCTSTR)pgroup->Get_Name());
            if (index != CB_ERR) {
...
//
////////////////////////////////////////////////////////////////////////////
UINT fnUpdatingVSSThread(DWORD dwparam1, DWORD /*dwparam2*/, DWORD /*dwparam3*/,
                         HRESULT* /*presult*/, HWND* phmain_wnd)
{
...
//
////////////////////////////////////////////////////////////////////////////
bool Get_File_Time(LPCTSTR path, LPFILETIME pcreation_time, LPFILETIME paccess_time,
...
//
/////////////////////////////////////////////////////////////////////////////
FileAccessRightsClass::FileAccessRightsClass(LPCTSTR filename, ACCESS_TYPE type,
                                             bool should_restore)
    : m_Filename(filename),
...
PersistClass* Instance_Definition(DefinitionClass* definition)
{
    ...
    if (new_object == NULL) {
        ...
        CString message;
        message.Format("Unable to create an instance of %s.\nCheck to make sure the preset is "
                       "properly configured with a model and/or physics object.",
                       name);
...
void Perform_Job(LPCTSTR filename, bool delete_on_completion)
{
    ...
    ::GetPrivateProfileString("Job Description", "Granularity", "1",
                              granularity_string.GetBufferSetLength(20), 20, filename);
    float granularity = ::atof(granularity_string);

    CString level_file;
    ::GetPrivateProfileString("Job Description", "LVL", "", level_file.GetBufferSetLength(MAX_PATH),
                              MAX_PATH, filename);

    CString output_file;
    ::GetPrivateProfileString("Job Description", "Output", "",
                              output_file.GetBufferSetLength(MAX_PATH), MAX_PATH, filename);
    ...
    if (level_file.GetLength() > 0) {
        ...
        //
#if (1)
        ::Get_Scene_Editor()->Generate_Uniform_Sampled_Vis(granularity, false, false, true, index,
                                                           total);
#else
