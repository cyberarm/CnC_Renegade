...
void EmitterLineGroupPropPageClass::Update_Blur_Times(void)
{
    ...
    if (count > 1) {
        ...
        for (int index = 1; index < count; index++) {
            ...
            m_BlurTimes.Values[index - 1]
                = Denormalize_Blur_Time(m_BlurTimeBar->Get_Graph_Percent(index));
        }
    }
...
BOOL EmitterLineGroupPropPageClass::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
    ...
    case IDC_BLUR_TIME_BAR: {
        if (color_bar_hdr->hdr.code == CBRN_DBLCLK_POINT) {
            ...
            //
            float blur_time
                = Denormalize_Blur_Time(m_BlurTimeBar->Get_Graph_Percent(color_bar_hdr->key_index));
