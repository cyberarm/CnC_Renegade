...
public:
    ...
    void Get_Opacity_Keyframes(ParticlePropertyStruct<float>& opacity)
    {
        opacity = m_CurrentOpacities;
    }
    /*const Vector3 &				Get_Start_Color (void) const { return m_StartColor;
    } const Vector3 &				Get_End_Color (void) const { return m_EndColor; }
    float								Get_Start_Opacity (void)
    const { return m_StartOpacity; } float
    Get_End_Opacity (void) const { return m_EndOpacity; } float
    Get_Fade_Time (void) const { return m_FadeTime; }*/
