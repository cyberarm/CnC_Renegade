...
BOOL EmitterRotationPropPageClass::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
    ...
    case IDC_ROTATION_BAR: {
        if (color_bar_hdr->hdr.code == CBRN_DBLCLK_POINT) {
            ...
            //
            float rotation
                = Denormalize_Rotation(m_RotationBar->Get_Graph_Percent(color_bar_hdr->key_index));
            ...
            if (dialog.DoModal() == IDOK) {
                ...
                if ((new_max != m_MaxRotation) || (new_min != m_MinRotation)) {
                    ...
                    for (int index = 0; index < count; index++) {
                        ...
                        float rotation
                            = Denormalize_Rotation(m_RotationBar->Get_Graph_Percent(index));
    ...
    case IDC_INITIAL_ORIENTATION_RANDOM_EDIT: {
        ...
        if ((HIWORD(wParam) == EN_KILLFOCUS) && SendDlgItemMessage(LOWORD(wParam), EM_GETMODIFY)) {
            ...
            m_InitialOrientationRandom
                = ::GetDlgItemFloat(m_hWnd, IDC_INITIAL_ORIENTATION_RANDOM_EDIT);
            m_pEmitterList->Set_Rotation_Keyframes(m_Rotations, m_InitialOrientationRandom);
            SetModified();
        }
        else if (HIWORD(wParam) == EN_CHANGE) {
