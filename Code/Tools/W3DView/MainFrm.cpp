...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    m_objectToolbar.AddButton(IDB_LOCK_X_UP, IDB_LOCK_X_DN, IDM_CAMERA_ALLOW_ROTATE_Y,
                              CFancyToolbar::Type2State);
    m_objectToolbar.AddButton(IDB_LOCK_Y_UP, IDB_LOCK_Y_DN, IDM_CAMERA_ALLOW_ROTATE_X,
                              CFancyToolbar::Type2State);
    m_objectToolbar.AddButton(IDB_LOCK_Z_UP, IDB_LOCK_Z_DN, IDM_CAMERA_ALLOW_ROTATE_Z,
                              CFancyToolbar::Type2State);
    m_objectToolbar.AddButton(IDB_ROTATE_Z_UP, IDB_ROTATE_Z_DN, IDM_OBJECT_ROTATE_Z,
                              CFancyToolbar::Type2State);

    m_animationToolbar.Create("Animation controls", this, 102);
    m_animationToolbar.AddButton(IDB_PLAY_UP, IDB_PLAY_DN, IDM_ANI_START,
                                 CFancyToolbar::Type2State);
    m_animationToolbar.AddButton(IDB_STOP_UP, IDB_STOP_DN, IDM_ANI_STOP);
    m_animationToolbar.AddButton(IDB_PAUSE_UP, IDB_PAUSE_DN, IDM_ANI_PAUSE,
                                 CFancyToolbar::Type2State);
    ...
    FloatControlBar(&m_animationToolbar, CPoint(rect.left + 210, rect.bottom - 100),
                    CBRS_ALIGN_LEFT);
...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT /*lpcs*/, CCreateContext* pContext)
{
    ...
    if (bReturn) {
        ...
        // of the W3D file.
        bReturn &= m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CDataTreeView), CSize(340, 10),
                                            pContext);

        // Create the 'graphic' view which will contain a pictoral representation
        // of the currently selected object
        bReturn &= m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CGraphicView), CSize(120, 10),
                                            pContext);
        ...
        if (bReturn) {
            ...
            if (pCGraphicView) {
                ...
                WW3DAssetManager::Get_Instance()->Register_Prototype_Loader(
                ...
                //
                // Restore the N-Patch Subdivision Level and Gap-Filling settings from the last
                // session.
                //
                int subdivision_level
                    = ::AfxGetApp()->GetProfileInt("Config", "NPatchesSubdivision", 4);
                int gap_filling = ::AfxGetApp()->GetProfileInt("Config", "NPatchesGapFilling", 0);
                WW3D::Set_NPatches_Gap_Filling_Mode(gap_filling
                                                        ? WW3D::NPATCHES_GAP_FILLING_ENABLED
...
void CMainFrame::OnSelectionChanged(ASSET_TYPE newAssetType)
{
    if (m_currentAssetType != newAssetType) {
        ...
        case TypeAggregate: {
            ...
            if ((m_currentAssetType == TypeAnimation)
                || (m_currentAssetType == TypeCompressedAnimation)) {
        ...
        case TypeAnimation: {
            ...
            if (pMainMenu) {
                ...
                HMENU hSubMenu
                    = ::LoadMenu(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDR_ANI_MENU));
        ...
        case TypeHierarchy: {
            ...
            if (pMainMenu) {
                // Load the menu from the resources
                HMENU hSubMenu
                    = ::LoadMenu(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDR_HIERARCHY_MENU));
                hSubMenu = ::GetSubMenu(hSubMenu, 0);
        ...
        case TypeAggregate: {
            ...
            if (pMainMenu) {
                // Load the menu from the resources
                HMENU hSubMenu
                    = ::LoadMenu(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDR_AGGREGATE_MENU));
                hSubMenu = ::GetSubMenu(hSubMenu, 0);
        ...
        case TypeLOD: {
            ...
            if (pMainMenu) {
                // Load the menu from the resources
                HMENU hSubMenu
                    = ::LoadMenu(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDR_LOD_MENU));
                hSubMenu = ::GetSubMenu(hSubMenu, 0);
    ...
    if (pCGraphicView) {
        ...
        if (pCDoc && pCDoc->GetDisplayedObject()) {
            ...
            if (prender_obj->Class_ID() == RenderObjClass::CLASSID_PARTICLEEMITTER) {
                pCGraphicView->Reset_Camera_To_Display_Emitter(
                    *((ParticleEmitterClass*)prender_obj));
            }
            else {
...
void CMainFrame::OnDeviceChange(void)
{
    ...
    Select_Device(true);
#else
    ::MessageBox(m_hWnd, "Feature removed during conversion to DX8.", "Unsupported Feature",
...
void CMainFrame::OnScaleEmitter(void)
{
    ...
    if (pdoc != nullptr) {
        ...
        //
        ScaleDialogClass dlg(
            1.0, this,
            "Enter the scaling factor you want to apply to the current particle emitter");
...
void CMainFrame::OnDestroy(void)
{
    ...
    // Cache this information in the registry
    theApp.WriteProfileInt("Config", "AnimateCamera",
                           (int)((CW3DViewDoc*)GetActiveDocument())->Is_Camera_Animated());

    // Cache this information in the registry
    theApp.WriteProfileInt("Config", "ResetCamera",
...
void CMainFrame::OnListMissingTextures(void)
{
    ...
    if (texture_list.Count() > 0) {
        ...
        ::MessageBox(::AfxGetMainWnd()->m_hWnd, "No Missing Textures!", "Texture Info",
                     MB_ICONEXCLAMATION | MB_OK);
    }
    ...
    if (doc != nullptr) {
        ...
        if (render_obj != nullptr) {
            if (render_obj->Class_ID() == RenderObjClass::CLASSID_SPHERE) {
                ...
                // Display the sphere property sheet
                //
                SpherePropertySheetClass dialog((SphereRenderObjClass*)render_obj,
                                                IDS_SPHERE_PROP_TITLE, this);
                dialog.DoModal();
            }
                ...
                // Display the ring property sheet
                //
                RingPropertySheetClass dialog((RingRenderObjClass*)render_obj, IDS_RING_PROP_TITLE,
                                              this);
                dialog.DoModal();
            }
        }
