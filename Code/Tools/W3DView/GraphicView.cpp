...
BOOL CGraphicView::InitializeGraphicView(void)
{
    ...
    bReturn = (WW3D::Set_Render_Device(g_iDeviceIndex, cx, cy, g_iBitsPerPixel, m_iWindowed)
               == WW3D_ERROR_OK);
...
void CGraphicView::RepaintView(BOOL bUpdateAnimation, DWORD ticks_to_use)
{
    ...
    if (doc->Is_Initialized() && doc->GetScene() && m_UpdateCounter == 0) {
        ...
        if ((prender_obj != nullptr) && (doc->GetScene()->Are_LODs_Switching())) {
...
void CALLBACK fnTimerCallback(UINT uID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2)
{
    ...
    if (hwnd != nullptr) {
        ...
        if ((GetProp(hwnd, "WaitingToProcess") == nullptr) && (GetProp(hwnd, "Inactive") == nullptr)) {
...
void CGraphicView::OnMouseMove(UINT nFlags, CPoint point)
{
    ...
    // Is the mouse button down?
        ...
        if ((pSceneLight != nullptr) && (m_pLightMesh != nullptr)) {
            ...
            Quaternion mouse_motion
                = Inverse(::Trackball(lastPointX, lastPointY, pointX, pointY, 0.8F));
...
void CGraphicView::Reset_Camera_To_Display_Emitter(ParticleEmitterClass& emitter)
{
    ...
    if ((velocity.X == 0) && (velocity.Y == 0) && (velocity.Z == 0)) {
        // velocity.Set (emitter.Get_Velocity_Random (), emitter.Get_Velocity_Random (),
        // emitter.Get_Velocity_Random ()); use_vel_rand = true;
    }
    ...
    if ((acceleration.X != 0) || (acceleration.Y != 0) || (acceleration.Z != 0)) {
        ...
        if ((time_max.X >= 0.0F) && (time_max.X < lifetime)) {
            distance_maxima.X
                = (velocity.X * time_max.X) + ((acceleration.X * (time_max.X * time_max.X)) / 2.0F);
        ...
        if ((time_max.Y >= 0.0F) && (time_max.Y < lifetime)) {
            distance_maxima.Y
                = (velocity.Y * time_max.Y) + ((acceleration.Y * (time_max.Y * time_max.Y)) / 2.0F);
        ...
        if ((time_max.Z >= 0.0F) && (time_max.Z < lifetime)) {
            distance_maxima.Z
                = (velocity.Z * time_max.Z) + ((acceleration.Z * (time_max.Z * time_max.Z)) / 2.0F);
...
void CGraphicView::SetCameraPos(CAMERA_POS cameraPos)
{
    ...
    if (pCRenderObj) {
        ...
        switch (cameraPos) {
        case CameraFront: {
            transform.Look_At(sphere.Center + Vector3(m_CameraDistance, 0.00F, 0.00F),
                              sphere.Center, 0);
        } break;
        ...
        case CameraLeft: {
            transform.Look_At(sphere.Center + Vector3(0.00F, -m_CameraDistance, 0.00F),
                              sphere.Center, 0);
        } break;
        ...
        case CameraTop: {
            transform.Look_At(sphere.Center + Vector3(0.00F, 0.00F, m_CameraDistance),
...
void CGraphicView::Set_Camera_Distance(float dist)
{
    ...
    //
    Matrix3D new_tm(1);
    new_tm.Look_At(m_ViewedSphere.Center + Vector3(m_CameraDistance, 0.00F, 0.00F),
                   m_ViewedSphere.Center, 0);
