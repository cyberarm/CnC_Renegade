...
BOOL CFancyToolbar::Create(LPCTSTR pszWindowName, CWnd* pCParentWnd, UINT uiID)
{
    ...
    RECT rect = { 0 };
    BOOL bReturn = CWnd::Create(TOOLBAR_CLASS_NAME, pszWindowName, WS_CHILD | WS_VISIBLE, rect,
                                pCParentWnd, uiID);
...
void CFancyToolbar::AddButton(UINT iBMPUp, UINT iBMPDn, int iCommandID, BUTTON_TYPE buttonType)
{
    ...
    m_pButtonArray[iButton].hBMPUp
        = ::LoadBitmap(::AfxGetResourceHandle(), MAKEINTRESOURCE(iBMPUp));
    m_pButtonArray[iButton].hBMPDn
        = ::LoadBitmap(::AfxGetResourceHandle(), MAKEINTRESOURCE(iBMPDn));
...
int CFancyToolbar::ButtonFromPoint(const CPoint& point)
{
    ...
    for (int iButton = 0; (iButton < m_iButtons) && (iIndex == -1); iButton++) {
        ...
        if (m_pButtonArray[iButton].bVisible && (point.x >= iXPos)
            && (point.x <= iXPos + BUTTON_WIDTH) && (point.y >= BORDER_TOP)
            && (point.y <= BORDER_TOP + BUTTON_HEIGHT)) {
...
void CFancyToolbar::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (iButton >= 0) {
        ...
        if (hDC) {
            ...
            DrawButton(hDC, BORDER_LEFT + iButton * BUTTON_WIDTH,
                       ((rect.bottom - rect.top) >> 1) - BUTTON_MIDDLE, hBMP);
...
void CFancyToolbar::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    if (m_iCurrentButton >= 0) {
        ...
        if (hDC) {
            ...
            // Paint the button
            DrawButton(hDC, BORDER_LEFT + m_iCurrentButton * BUTTON_WIDTH,
...
//  OnDraw
//
void CFancyToolbar::OnDraw(CDC* pDC)
{
    return;
}
