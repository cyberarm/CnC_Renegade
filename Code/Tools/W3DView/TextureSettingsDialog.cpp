...
void TextureSettingsDialogClass::OnBrowseButton(void)
{
    ...
    CFileDialog dialog(TRUE, ".tga", filename,
...
void TextureSettingsDialogClass::Paint_Thumbnail(void)
{
    ...
    if (m_hThumbnail != NULL) {
        ...
        ::BitBlt(hdc, rect.left + (rect.Width() >> 1) - 48, rect.top + (rect.Height() >> 1) - 48,
                 96, 96, hmem_dc, 0, 0, SRCCOPY);
...
void TextureSettingsDialogClass::OnApply(void)
{
    ...
    if (SendDlgItemMessage(IDC_ANIMATION_CHECK, BM_GETCHECK) == 1) {
        ...
        case TYPE_LOOP:
            pnew_texture = new TimeAnimTextureInstanceClass(
                *(WW3DAssetManager::Get_Instance()), texture_name, frame_count, (float)frame_rate,
                TimeAnimTextureInstanceClass::LOOP, TimeAnimTextureInstanceClass::FORWARD);
            break;

        case TYPE_ONCE:
            pnew_texture = new TimeAnimTextureInstanceClass(
                *(WW3DAssetManager::Get_Instance()), texture_name, frame_count, (float)frame_rate,
                TimeAnimTextureInstanceClass::ONE_TIME, TimeAnimTextureInstanceClass::FORWARD);
            break;

        case TYPE_PING_PONG:
            pnew_texture = new TimeAnimTextureInstanceClass(
                *(WW3DAssetManager::Get_Instance()), texture_name, frame_count, (float)frame_rate,
                TimeAnimTextureInstanceClass::PINGPONG, TimeAnimTextureInstanceClass::FORWARD);
            break;

        case TYPE_MANUAL:
            pnew_texture = new ManualAnimTextureInstanceClass(*(WW3DAssetManager::Get_Instance()),
                                                              texture_name, frame_count);
            break;
        }
    }
    else {
        ...
        /*if (resizeable) {
                pnew_texture = new ResizeableTextureInstanceClass (*(WW3DAssetManager::Get_Instance
        ()), texture_name); } else {*/
    ...
    if (pnew_texture != NULL) {
        ...
        MessageBox("Unable to create the requested texture.", "Texture Error",
                   MB_ICONERROR | MB_OK);
    }
