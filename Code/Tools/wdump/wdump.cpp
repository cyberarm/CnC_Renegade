...
BOOL CWdumpApp::InitInstance()
{
    ...
    pDocTemplate = new CSingleDocTemplate(IDR_MAINFRAME, RUNTIME_CLASS(CWdumpDoc),
    ...
    if (NoWindow) {
        if (cmdInfo.m_nShellCommand == CWDumpCommandLineInfo::FileOpen) {
            ...
            window freopen("CONOUT$", "a", stdout); freopen("CONIN$", "r", stdin); } else { int
            CrtInput; int CrtOutput; if ( (CrtInput  =_open_osfhandle((long) info.hStdInput,
            _O_RDONLY)) != -1) { if ( (CrtOutput = _open_osfhandle((long) info.hStdOutput,
            _O_APPEND)) != -1) { _dup2( CrtInput, 0); _dup2( CrtOutput, 1);
                                                    }
                                            }
            ...
            */
            ...
            CWdumpDoc* doc
                = (CWdumpDoc*)pDocTemplate->OpenDocumentFile(cmdInfo.m_strFileName, FALSE);
