...
inline bool Point_In_Triangle_2D(const Vector3& tri_point0, const Vector3& tri_point1,
                                 const Vector3& tri_point2, const Vector3& test_point, int axis_1,
                                 int axis_2, unsigned char& flags)
{
    ...
    if (p0p1p2 != 0.0f) {
        ...
        // Now perform tests
        Vector2 p0pT(test_point[axis_1] - tri_point0[axis_1],
                     test_point[axis_2] - tri_point0[axis_2]);
        factors[0] = Vector2::Perp_Dot_Product(p0p1, p0pT);
        if (factors[0] * side_factor < 0.0f) {
            return false;
        }
        Vector2 p1pT(test_point[axis_1] - tri_point1[axis_1],
                     test_point[axis_2] - tri_point1[axis_2]);
        factors[1] = Vector2::Perp_Dot_Product(p1p2, p1pT);
        if (factors[1] * side_factor < 0.0f) {
            return false;
        }
        Vector2 p2pT(test_point[axis_1] - tri_point2[axis_1],
                     test_point[axis_2] - tri_point2[axis_2]);
        ...
        if (p0p1dist2 > p1p2dist2) {
            if (p0p1dist2 > p2p0dist2) {
                ...
                pSpE = p0p1;
                pSpT.Set(test_point[axis_1] - tri_point0[axis_1],
                         test_point[axis_2] - tri_point0[axis_2]);
                max_dist2 = p0p1dist2;
            }
            else {
                ...
                pSpE = p2p0;
                pSpT.Set(test_point[axis_1] - tri_point2[axis_1],
                         test_point[axis_2] - tri_point2[axis_2]);
                max_dist2 = p2p0dist2;
            }
        }
            ...
            if (p1p2dist2 > p2p0dist2) {
                ...
                pSpE = p1p2;
                pSpT.Set(test_point[axis_1] - tri_point1[axis_1],
                         test_point[axis_2] - tri_point1[axis_2]);
                max_dist2 = p1p2dist2;
            }
            else {
...
inline bool Cast_Semi_Infinite_Axis_Aligned_Ray_To_Triangle(
    const Vector3& tri_point0, const Vector3& tri_point1, const Vector3& tri_point2,
    const Vector4& tri_plane, const Vector3& ray_start, int axis_r, int axis_1, int axis_2,
    int direction, unsigned char& flags)
{
    ...
    if (Point_In_Triangle_2D(tri_point0, tri_point1, tri_point2, ray_start, axis_1, axis_2,
                             flags_2d)) {
        ...
        float result = tri_plane[axis_r] * sign[direction]
            * (tri_plane.X * ray_start.X + tri_plane.Y * ray_start.Y + tri_plane.Z * ray_start.Z
               + tri_plane.W);
        if (result < 0.0f) {
            // Intersection!
