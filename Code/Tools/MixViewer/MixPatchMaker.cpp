...
void MixPatchMakerClass::Make_Patch(char* old_input_file, char* new_input_file, char* output_file,
    ...
    if (new_mix.Is_Valid() && new_mix.Build_Internal_Filename_List()) {
        if (old_mix.Is_Valid() && old_mix.Build_Internal_Filename_List()) {
            ...
            */
            ...
            /*
            ** Loop through all the files in the new source mix. If a file doesn't exist in the old
            *one, or it does exist but is
                ...
                */
                ...
                for (int j = 0; j < old_filename_list.Count(); j++) {
                    ...
                    if (stricmp(name, name_compare) == 0) {
                        ...
                        /*
                        ** There's a file with the same name in the old mix file. See if it's byte
                        *for byte identical. If it's not
                        ** then we need to use the newer file in the patch mix.
                        */
                        bool same
                            = Compare_File(&new_mix, &old_mix, new_filename_list[i].Peek_Buffer());
                        if (!same) {

                            /*
                            ** If it's a .dds (texture) file, compare the source art for changes.
                            *The .dds can change even when the
                            ** source art doesn't.
                            */
                            if (strstr(name, ".DDS")) {
                ...
                /*
                ** If the file doesn't exist at all in the older mixfile then it needs to be in the
                *patch mix.
                */
                if (!found) {
                    copy = true;
...
void MixPatchMakerClass::Copy_File(MixFileFactoryClass* src_mix, MixFileCreator* dest_mix,
                                   char* filename)
{
    //
 ...
 *=============================================================================================*/
bool MixPatchMakerClass::Compare_File(MixFileFactoryClass* src_mix, MixFileFactoryClass* dest_mix,
                                      char* filename)
{
    /*
    **	Get the file data from the source mix
