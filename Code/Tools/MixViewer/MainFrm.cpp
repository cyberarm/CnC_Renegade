            ...
            if (result != 0 || error == ERROR_ALREADY_EXISTS) {
                ...
                for (int index = 0; index < filenames.Count(); index++) {
                    ...
                    if (::strchr(filenames[index], '\\') != nullptr) {
                        ...
                        int result = (int)::CreateDirectory(dest_folder, nullptr);
                        int error = ::GetLastError();
                        if (result == 0 && error != ERROR_ALREADY_EXISTS) {
                            StringClass message;
                    ...
                    if (source_file && source_file->Is_Available() && dest_file
                        && dest_file->Is_Available()) {
...
{
    CFileDialog dialog(true, ".mix", nullptr, OFN_ALLOWMULTISELECT | OFN_HIDEREADONLY | OFN_EXPLORER,
                       "Mix File (*.dat, *.mix)|*.dat;*.mix||", this);
    ...
    *file_name_buffer = 0;
    ...
    if (dialog.DoModal() == IDOK) {
        ...
        //
        // Process each mix file and put the stripped mix file into a 'temp' subdirectory under the
        // current one.
        //
...
void CMainFrame::OnMakeMixPatch(void)
{
    ...
    //
    CFileDialog dialog(true, ".mix", nullptr, OFN_HIDEREADONLY | OFN_EXPLORER,
                       "Mix File (*.dat, *.mix)|*.dat;*.mix||", this);
    ...
    if (dialog.DoModal() == IDOK) {
        ...
        //
        CFileDialog dialog2(true, ".mix", nullptr, OFN_HIDEREADONLY | OFN_EXPLORER,
                            "Mix File (*.dat, *.mix)|*.dat;*.mix||", this);
        ...
        if (dialog2.DoModal() == IDOK) {
            ...
            //
            CFileDialog dialog3(true, ".mix", nullptr, OFN_HIDEREADONLY | OFN_EXPLORER,
                                "Mix File (*.dat, *.mix)|*.dat;*.mix||", this);
                    ...
                    if (SHGetPathFromIDList(itemptr, old_art_dir)) {
                        ...
                        if (itemptr) {
                            ...
                            if (SHGetPathFromIDList(itemptr, new_art_dir)) {
                                ...
                                patcher.Make_Patch(
                                    old_file_name.Peek_Buffer(), new_file_name.Peek_Buffer(),
...
//
/////////////////////////////////////////////////////////////////////////////
void CMainFrame::Add_To_Mix_File(const char* mix_filename,
                                 DynamicVectorClass<StringClass>& new_file_list)
{
    //
