    ...
    if (dest_factory.Is_Valid() && dest_factory.Build_Internal_Filename_List()) {
        ...
        for (int mix_index = 0; mix_index < mix_file_list.Count(); mix_index++) {
            ...
            for (int file_index = 0; file_index < filename_list.Count(); file_index++) {
                ...
                //
                for (int test_mix_index = 0; test_mix_index < mix_file_list.Count();
                     test_mix_index++) {
                    if (test_mix_index != mix_index
                        && Is_File_In_Factory(filename, mix_file_list[test_mix_index])) {
...
void DuplicateRemoverClass::Open_Mix_Files(DynamicVectorClass<MixFileFactoryClass*>& list)
{
    ...
    for (int index = 0; index < MixFileList.Count(); index++) {
        ...
        //
        MixFileFactoryClass* mix_factory
            = new MixFileFactoryClass(MixFileList[index], _TheFileFactory);
...
//
////////////////////////////////////////////////////////////////////////////
bool DuplicateRemoverClass::Is_File_In_Factory(const StringClass& filename,
                                               MixFileFactoryClass* factory)
{
    bool retval = false;
