...
template <class K, class V> uint32 Dictionary<K, V>::keyHash(IN K& key) RO
{
...
template <class K, class V> void Dictionary<K, V>::print(FILE* out) RO
{
    DNode<K, V>* temp;
    uint32 i;
...
template <class K, class V> Dictionary<K, V>& Dictionary<K, V>::operator=(Dictionary<K, V>& other)
{
    _ASSERTE(0);
}
...
bit8 Dictionary<K, V>::iterate(INOUT int& index, INOUT int& offset, OUT V& value) RO
{
    DNode<K, V>* temp;
...
//   is returned.
template <class K, class V>
bit8 Dictionary<K, V>::iterate(INOUT int& index, INOUT int& offset, OUT K& key, OUT V& value) RO
{
    DNode<K, V>* temp;
...
template <class K, class V> uint32 Dictionary<K, V>::getSize(void) RO
{
    return (size);
}

// Return the current number of entries in the table
template <class K, class V> uint32 Dictionary<K, V>::getEntries(void) RO
{
    return (entries);
}

// Does the Dictionary contain the key?
template <class K, class V> bit8 Dictionary<K, V>::contains(IN K& key) RO
{
    int offset;
    DNode<K, V>* node;
...
template <class K, class V> bit8 Dictionary<K, V>::updateValue(IN K& key, IN V& value)
{
    sint32 retval;
...
template <class K, class V> bit8 Dictionary<K, V>::add(IN K& key, IN V& value)
{
    int offset;
...
template <class K, class V> bit8 Dictionary<K, V>::remove(IN K& key, OUT V& value)
{
    int offset;
...
template <class K, class V> bit8 Dictionary<K, V>::remove(IN K& key)
{
    V temp;
...
template <class K, class V> bit8 Dictionary<K, V>::removeAny(OUT K& key, OUT V& value)
{
    int offset;
    DNode<K, V>*node, *last, *temp;
    float percent;
...
template <class K, class V> bool Dictionary<K, V>::getValue(IN K& key, OUT V& value) RO
{
    V* valptr = NULL;
...
//  managed data!
template <class K, class V> bool Dictionary<K, V>::getPointer(IN K& key, OUT V** valptr) RO
{
    int offset;
    DNode<K, V>* node;
