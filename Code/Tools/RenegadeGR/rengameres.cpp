...
RenegadeGameRes::~RenegadeGameRes()
{
    ...
    if (_map_name != nullptr) {
        delete[] _map_name;
    }
    if (_logins != nullptr) {
        for (int i = 0; i < _myplayercount; i++) {
            if (_logins[i] != nullptr) {
                delete[] _logins[i];
            }
        }
        delete[] _logins;
    ...
    if (_selfkills != nullptr) {
        delete[] _selfkills;
    }
    if (_damagepoints != nullptr) {
...
/*----------------------------------------------------------------------------------.
| METHOD: _addToArr
| | Takes a pointer to an array and a new item, constructs a new array and returns	| | a
pointer to it.
|
`----------------------------------------------------------------------------------*/
...
{
    long unsigned int* newarr = nullptr;
