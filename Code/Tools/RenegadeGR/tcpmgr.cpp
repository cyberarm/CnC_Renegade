...
bit8 TCPMgr::getConnection(TCPCon** conn, uint32 handle, uint16 port, sint32 wait_secs,
                           DIRECTION dir)
{
    ...
    while (1) {
        ...
        if ((remaining_wait > 0)
            && (wait(remaining_wait, 0, fdArray, ConnectArray_.length(), FALSE) > 0)) {
    ...
    for (i = 0; i < ConnectArray_.length(); i++) {
        ...
        if ((connPtr->state == CLOSED) || (connPtr->state == CONNECTING)) {
            ...
            if (connPtr->state == CONNECTING) {
                if (getpeername(connPtr->fd, (sockaddr*)&addr, &addrSize)
                    == 0) { // if get endpoint, then success
                    connPtr->state = CONNECTED;
                }
            }
            else if ((retval = ::connect(connPtr->fd, (struct sockaddr*)&addr, sizeof(addr)))
                     == SOCKET_ERROR) {
                status = getStatus();
                if (status == ISCONN) {
                    retval = 0;
