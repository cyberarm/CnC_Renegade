...
static int asf_load(const TCHAR* filename, ImpInterface* iface, Interface* gi)
{
    ...
    try {
        ...
        MessageBox(GetActiveWindow(), error.message(), "Parse error", MB_OK);
        return -1;
    }
...
int ASF_Import::DoImport(const TCHAR* filename, ImpInterface* iface, Interface* gi, BOOL)
{
    int status;
