...
Node_Key::Node_Key(int nr_frames, const char* name, Node_Key* next)
...
ClassDesc* ClipboardDesc()
{
    return &clipboard_desc;
}
...
static BOOL CALLBACK ClipboardDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg) {
    case WM_INITDIALOG:
...
static BOOL CALLBACK Pose_Name_Message_Handler(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        first_frame_spin = SetupIntSpinner(hWnd, IDC_FIRST_FRAME_SPIN, IDC_FIRST_FRAME_EDIT,
                                           first_frame, last_frame, current_frame);

        first_frame_spin->SetResetValue(current_frame);

        last_frame_spin = SetupIntSpinner(hWnd, IDC_LAST_FRAME_SPIN, IDC_LAST_FRAME_EDIT,
                                          first_frame, last_frame, current_frame);
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            SendDlgItemMessage(hWnd, IDC_NAME, WM_GETTEXT, MAX_NAME, (LPARAM)pose_name);
    ...
    case CC_SPINNER_CHANGE:
        ...
        case IDC_FIRST_FRAME_SPIN:
            if (first_frame_spin->GetIVal() > last_frame_spin->GetIVal()) {
                last_frame_spin->SetValue(first_frame_spin->GetIVal(), FALSE);
            }
            break;

        case IDC_LAST_FRAME_SPIN:
            if (last_frame_spin->GetIVal() < first_frame_spin->GetIVal()) {
                first_frame_spin->SetValue(last_frame_spin->GetIVal(), FALSE);
            }
            break;
        }
...
void Clipboard_Class::BeginEditParams(Interface* ip, IUtil* iu)
{
    this->iu = iu;
    this->ip = ip;
    hPanel = ip->AddRollupPage(hInstance, MAKEINTRESOURCE(IDD_CLIPBOARD_PANEL), ClipboardDlgProc,
                               _T("Key Clipboard"), 0);
}
...
void Clipboard_Class::Create(HWND hWnd)
{
    ...
    int rv = DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_POSE_NAME), hWnd,
                            Pose_Name_Message_Handler, 0);
      ...
      ticks_per_frame_(GetTicksPerFrame())
{
    ...
    for (int i = 0; i < number_of_nodes; ++i) {
        ...
        Node_Key* new_key = new Node_Key(nr_frames, inode->GetName(), first_key);
