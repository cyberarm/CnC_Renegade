...
///////////////////////////////////////
UINT CALLBACK W3DPageCallback(HWND hWnd, UINT uMessage, LPPROPSHEETPAGE ppsp)
{
    switch (uMessage) {
    case PSPCB_CREATE:
        return TRUE;
...
BOOL CALLBACK AnimPageDlgProc(HWND hDlg, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
        ...
        while (ptr) {
            ...
            GetItemName(pItem, id_of_interest, (void*)lpcs->m_AnimInfos, sizeof_struct,
                        found_animations);
        }
        if (found_animations) {
            ...
            // version
            sprintf(buf, "%d.%d", W3D_GET_MAJOR_VERSION(pAnim->Version),
                    W3D_GET_MINOR_VERSION(pAnim->Version));
        ...
        if (found_hierarchies) {
            ...
            sprintf(buf, "%d.%d", W3D_GET_MAJOR_VERSION(pHInfo->Version),
                    W3D_GET_MINOR_VERSION(pHInfo->Version));
            SetDlgItemText(hDlg, IDC_HIERARCHYVERSION, buf);
            sprintf(buf, "(%.3f, %.3f, %.3f)", pHInfo->Center.X, pHInfo->Center.Y,
                    pHInfo->Center.Z);
...
void SetDlgMeshParams(HWND hDlg, W3dMeshHeader3Struct* pInfo)
{
    ...
    sprintf(msg, "%d.%d", W3D_GET_MAJOR_VERSION(pInfo->Version),
            W3D_GET_MINOR_VERSION(pInfo->Version));
...
BOOL CALLBACK MeshPageDlgProc(HWND hDlg, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
        ...
        if (lpcs->m_FoundMeshes) {
            ...
            while (ptr) {
                ChunkItem* pItem = pData->Chunks.GetNext(ptr);
                GetItemName(pItem, id_of_interest, (void*)pTextureInfo, sizeof_struct,
                            found_textures);
            }
            for (int t_count(0); t_count < found_textures; t_count++) {
                if (lpcs->NotAdded(&(pTextureInfo[t_count * MAX_TEXUTRE_NAME_LEN]))) {
                    ...
                    SendDlgItemMessage(hDlg, IDC_TEXTURELIST, LB_ADDSTRING, (WPARAM)0L,
                                       (LPARAM)text); //(pTextureInfo[t_count].name)) ;
            ...
            // Set Spin range
            SendDlgItemMessage(hDlg, IDC_MESHSPIN, UDM_SETRANGE, (WPARAM)0L,
                               (LPARAM)MAKELONG(lpcs->m_FoundMeshes - 1, 0));
        }
...
STDMETHODIMP
CShellExt::AddPages(LPFNADDPROPSHEETPAGE lpfnAddPage, // Pointer to the Shell's AddPage function
                    LPARAM lParam)
{ // Passed as second parameter to lpfnAddPage
...
STDMETHODIMP CShellExt::ReplacePage(
    UINT uPageID, // ID of page to be replaced
    LPFNADDPROPSHEETPAGE lpfnReplaceWith, // Pointer to the Shell's Replace function
    LPARAM lParam)
{ // Passed as second parameter to lpfnReplaceWith
