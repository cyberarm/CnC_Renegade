...
public:
    ...
    void Set(float a = 0.0f, float b = 0.0f, float c = 0.0f, float d = 1.0f)
    {
        X = a;
        Y = b;
        Z = c;
...
inline bool Quaternion::Is_Valid(void) const
{
    ...
    return (1);
#else
    return (Is_Valid_Float(X) && Is_Valid_Float(Y) && Is_Valid_Float(Z) && Is_Valid_Float(W));
#endif
}
