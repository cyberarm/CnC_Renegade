...
BOOL VideoConfigDialogClass::OnInitDialog(void)
{
    ...
    int bit_depth = 0;
    int windowed = 0;
    if (WW3D::Registry_Load_Render_Device(RENEGADE_SUB_KEY_NAME_RENDER, device_name, 256, width,
                                          height, bit_depth, windowed, texture_depth)) {
        if (width != -1) {
            CurrentWidth = width;
        }
    ...
    for (int index = 0; index < driver_count; index++) {
        ...
        //
        CString driver_name;
        driver_name.Format("%s - %s", device_desc.Get_Device_Name(),
                           device_desc.Get_Hardware_Name());
...
void VideoConfigDialogClass::Update_Resolution_Slider(void)
{
    ...
    if (ResolutionList.Count() > 0) {
        ::qsort(&ResolutionList[0], ResolutionList.Count(), sizeof(ResolutionList[0]),
...
void VideoConfigDialogClass::Update_Color_Combo(void)
{
    ...
    for (int index = 0; index < ResolutionList.Count(); index++) {
        ...
        //
        int item_index = SendDlgItemMessage(IDC_BITDEPTH_COMBO, CB_FINDSTRINGEXACT, 0,
                                            (LPARAM)(LPCTSTR)color_string);
        if (item_index == CB_ERR) {

            //
            //	Add this entry to the combobox
            //
            int item_index = SendDlgItemMessage(IDC_BITDEPTH_COMBO, CB_ADDSTRING, 0,
                                                (LPARAM)(LPCTSTR)color_string);
            if (item_index != CB_ERR) {
                SendDlgItemMessage(IDC_BITDEPTH_COMBO, CB_SETITEMDATA, (WPARAM)item_index,
                                   (LPARAM)res_desc.BitDepth);
...
void VideoConfigDialogClass::Apply_Changes(void)
{
    ...
    WW3D::Registry_Save_Render_Device(RENEGADE_SUB_KEY_NAME_RENDER, CurrentDriverIndex,
                                      CurrentWidth, CurrentHeight, CurrentBitDepth,
                                      CurrentIsWindowed, WW3D::Get_Texture_Bitdepth());
    return;
}
