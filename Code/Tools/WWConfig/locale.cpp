...
typedef struct
{
    ...
    /*  unsigned int   LanguageOffset[LanguageCount]; \\ offsets in bytes from start of file to
     * language chunk */
} LOCALEFILE_HEADERCHUNK;

/* offset LOCALEFILE_HEADERCHUNK_LANGUAGE_OFFSET bytes from the start of the chunk to the language
 * offset table */
...
typedef struct
{
    ...
    unsigned int ChunkSize; /* size of chunk in bytes */
    unsigned int
        StringCount; /* number of string ids in this chunk (same value in all language chunks) */
...
/* offset LOCALEFILE_INDEXCHUNK_STRINGID_OFFSET bytes from the start of the chunk to the string id
 * table */
...
typedef struct
{
    ...
    /*  unsigned int   StringOffset[StringCount];   \\ offsets in bytes from start of chunk to
     * string */
    /*  const char*    Data[StringCount];           \\ StringCount nullptr terminated strings */
} LOCALEFILE_LANGUAGECHUNK;

/* offset LOCALEFILE_LANGUAGECHUNK_STRING_OFFSETbytes from the start of the chunk to the string
 * offset table */
...
{
    if (lx == nullptr) {
        //        TRACE("LOCALE API is not initialized - call LOCALE_init before calling LOCALE
        //        functions\n");
    }
    return (lx != nullptr);
}
...
int LOCALE_init(void)
{
    ...
    ASSERT(lx == nullptr); /* can only call LOCALE_init after a restore or once, cannot double init
                           locale API */
...
int LOCALE_getbanklanguageid(void)
{
    ...
    if (lx != nullptr && lx->pBank[lx->BankIndex] != nullptr) {
...
int LOCALE_getbankstringcount(void)
{
    ...
    if (lx != nullptr && lx->pBank[lx->BankIndex]) {
...
static int readheader(GSTREAM* g)
{
    ...
    if (header.Flags == 1) {
        ...
        if (lx->pIndex[lx->BankIndex]) {
            ...
            //			VERIFY(gread(g, lx->pIndex[lx->BankIndex], IndexChunkSize) ==
            //IndexChunkSize);
...
int LOCALE_loadtable(const char* PathName, int LanguageID)
{
    ...
    if (lx->pBank[lx->BankIndex] != nullptr) {
        return ok;
    }
...
static int getstringbyindex(unsigned short key, const INDEX* pIndex)
{
    ...
    ASSERT(LOCALE_isinitialized()); /* must call LOCALE_init before calling this function */
    ASSERT(pIndex
           != nullptr); /* index not loaded - .loc file must have index created (use -i option) */
...
{
    const char* p = nullptr; /* pointer to string, nullptr if string cannot be found */
