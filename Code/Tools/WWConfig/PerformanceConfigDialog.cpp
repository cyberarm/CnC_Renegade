...
const char *KEY_NAME_SETTINGS				=
"Software\\Westwood\\RenegadeMPDemo\\System Settings"; const char *KEY_NAME_OPTIONS
= "Software\\Westwood\\RenegadeMPDemo\\Options"; #else const char *KEY_NAME_SETTINGS
= "Software\\Westwood\\Renegade\\System Settings"; const char *KEY_NAME_OPTIONS
= "Software\\Westwood\\Renegade\\Options"; #endif // MULTIPLAYERDEMO
*/
...
void PerformanceConfigDialogClass::Load_Values(void)
{
    ...
    if (registry.Is_Valid()) {
        ...
        int prelit_mode
            = registry.Get_Int(VALUE_NAME_PRELIT_MODE, WW3D::PRELIT_MODE_LIGHTMAP_MULTI_TEXTURE);
        int texture_filter
            = registry.Get_Int(VALUE_NAME_TEXTURE_FILTER, TextureClass::TEXTURE_FILTER_BILINEAR);
        int shadow_mode
            = registry.Get_Int(VALUE_NAME_SHADOW_MODE, PhysicsSceneClass::SHADOW_MODE_BLOBS_PLUS);
...
void PerformanceConfigDialogClass::Apply_Changes(void)
{
    ...
    if (registry.Is_Valid()) {
        ...
        // If card can't do multi-pass, value 1 means multi-texture (multi-pass selection is missing
        // from the combo box)
        if (!CanDoMultiPass) {
            if (prelit_mode == 1) {
        ...
        registry.Set_Int(VALUE_NAME_DYN_SHADOWS,
...
/*
                ...
                registry.Set_Int (VALUE_NAME_DYN_SHADOWS, (shadow_mode !=
   PhysicsSceneClass::SHADOW_MODE_NONE)); registry.Set_Int (VALUE_NAME_STATIC_SHADOWS,
   static_shadows);
...
void AutoConfigSettings()
{
    ...
    if (video) {
        ...
        if (FAILED(d3d->GetAdapterIdentifier(current_adapter_index, D3DENUM_NO_WHQL_LEVEL,
                                             &adapter_id))) {
            d3d->Release();
            return;
        }
    ...
    // save memory.
    // If card can't do multi pass (which is the case if we've seen z-fighting problems when
    // multi-passing) select vertex solve.
    if (!caps.Can_Do_Multi_Pass()
        || CPUDetectClass::Get_Total_Physical_Memory() < 100 * 1024 * 1024) {
...
void PerformanceConfigDialogClass::OnShowWindow(BOOL bShow, UINT nStatus)
{
    ...
    if (bShow) {
        ...
        if (video) {
            ...
            DX8Caps caps(d3d, video->Get_Current_Caps(), D3DFormat_To_WW3DFormat(display_format),
                         adapter_id);
            CanDoMultiPass = caps.Can_Do_Multi_Pass();
            ...
            if (sel != CB_ERR) {
                SendDlgItemMessage(IDC_LIGHTING_MODE_COMBO, CB_GETLBTEXT, sel,
                                   (LPARAM)cur_sel_string);
            }
            else {
                cur_sel_string[0] = 0;
                sel = registry.Get_Int(VALUE_NAME_PRELIT_MODE,
            ...
            SendDlgItemMessage(IDC_LIGHTING_MODE_COMBO, CB_ADDSTRING, 0,
                               (LPARAM)Locale_GetString(IDS_VERTEX, string));
            if (caps.Can_Do_Multi_Pass()) {
                SendDlgItemMessage(IDC_LIGHTING_MODE_COMBO, CB_ADDSTRING, 0,
                                   (LPARAM)Locale_GetString(IDS_MULTI_PASS, string));
            }
            ...
            unsigned res = SendDlgItemMessage(IDC_LIGHTING_MODE_COMBO, CB_FINDSTRINGEXACT, -1,
                                              (LPARAM)cur_sel_string);
            if (res == CB_ERR) {
                if (sel == 0) {
                    res = 0;
            ...
            if (sel != CB_ERR) {
                SendDlgItemMessage(IDC_TEXTURE_FILTER_COMBO, CB_GETLBTEXT, sel,
                                   (LPARAM)cur_sel_string);
            }
            else {
                cur_sel_string[0] = 0;
                sel = registry.Get_Int(VALUE_NAME_TEXTURE_FILTER,
            ...
            SendDlgItemMessage(IDC_TEXTURE_FILTER_COMBO, CB_ADDSTRING, 0,
                               (LPARAM)Locale_GetString(IDS_BILINEAR, string));
            SendDlgItemMessage(IDC_TEXTURE_FILTER_COMBO, CB_ADDSTRING, 0,
                               (LPARAM)Locale_GetString(IDS_TRILINEAR, string));
            ...
            // Try to set the previous selection
            res = SendDlgItemMessage(IDC_TEXTURE_FILTER_COMBO, CB_FINDSTRINGEXACT, -1,
                                     (LPARAM)cur_sel_string);
            if (res == CB_ERR) {
                if (sel == 0) {
                    res = 0;
