...
static BOOL CALLBACK _bone_influence_dialog_thunk(HWND hWnd, UINT message, WPARAM wParam,
                                                  LPARAM lParam);
...
RefResult SkinWSMObjectClass::NotifyRefChanged(Interval changeInt, RefTargetHandle hTarget,
...
RefResult SkinModifierClass::NotifyRefChanged(Interval changeInt, RefTargetHandle hTarget,
                                              PartID& partID, RefMessage message)
{
    switch (message) {
...
int SkinModifierClass::HitTest(TimeValue t, INode* inode, int type, int crossing, int flags,
...
void SkinModifierClass::Install_Bone_Influence_Dialog(void)
{
    ...
    */
    BoneInfluenceHWND
        = InterfacePtr->AddRollupPage(AppInstance, MAKEINTRESOURCE(IDD_BONE_INFLUENCE_PARAMS),
...
static BOOL CALLBACK _skeleton_dialog_thunk(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (!skinobj && message != WM_INITDIALOG) {
        return FALSE;
    }
                                                  ...
                                                  LPARAM lParam)
{
    ...
    if (!skinmod && message != WM_INITDIALOG) {
        return FALSE;
    }
...
BOOL SkinModifierClass::Bone_Influence_Dialog_Proc(HWND hWnd, UINT message, WPARAM wParam,
                                                   LPARAM lParam)
{
    switch (message) {
