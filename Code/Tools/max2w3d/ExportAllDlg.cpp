...
int ExportAllDlg::DoModal(void)
{
    ...
    BOOL result
        = DialogBoxParam(AppInstance, MAKEINTRESOURCE(IDD_EXPORT_ALL), m_MaxInterface->GetMAXHWnd(),
...
void ExportAllDlg::OnBrowse()
{
    ...
    if (il) {
        ...
        if (SHGetPathFromIDList(il, folder_name)) {
            ...
            MessageBox(m_hWnd, "Error getting pathname with SHGetPathFromIDList()", "Error",
                       MB_OK | MB_ICONSTOP);
        }
    }
}
