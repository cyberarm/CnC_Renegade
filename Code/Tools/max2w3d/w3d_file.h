...
the render object name is the name of the file.
...
Version 1.0:
        ...
        MESHES -  contained the following chunks:
                ...
                W3D_CHUNK_SURRENDER_NORMALS,	// array of surrender normals (one per vertex as
req. by surrender) W3D_CHUNK_TEXCOORDS,				// array of texture coordinates
                ...
                W3D_CHUNK_MESH_USER_TEXT,		// Name of owning hierarchy, text from the
MAX comment field
...
Version 2.0:
        ...
        in addition to the normal meshes.
                ...
                W3D_CHUNK_COLLISION_CONNECTION,		// collision meshes connected to the
hierarchy W3D_CHUNK_SKIN_CONNECTION,				// skins connected to the hierarchy
                W3D_CHUNK_CONNECTION_AUX_DATA			// extension of the connection
header
...
*		W3D_CHUNK_MATERIAL_INFO					// how many passes, vertex
mtls, shaders, and textures... *		W3D_CHUNK_SHADERS
// array of W3dShaderStruct's *		W3D_CHUNK_VERTEX_MATERIALS *
W3D_CHUNK_VERTEX_MATERIAL *				W3D_CHUNK_VERTEX_MATERIAL_NAME *
...
enum
{
    ...
    W3D_CHUNK_MESH_HEADER3
    = 0x0000001F, //	mesh header contains general info about the mesh. (W3dMeshHeader3Struct)
    W3D_CHUNK_TRIANGLES = 0x00000020, // New improved triangles chunk (array of W3dTriangleStruct's)
    W3D_CHUNK_VERTEX_SHADE_INDICES
    = 0x00000022, // shade indexes for each vertex (array of uint32's)
    ...
    = 0x00000025, // optional lightmapped multi-pass material chunk wrapper
    W3D_CHUNK_PRELIT_LIGHTMAP_MULTI_TEXTURE
    = 0x00000026, // optional lightmapped multi-texture material chunk wrapper

    W3D_CHUNK_MATERIAL_INFO
    = 0x00000028, // materials information, pass count, etc (contains W3dMaterialInfoStruct)
    ...
    W3D_CHUNK_SHADER_IDS
    = 0x0000003A, // single or per-tri array of uint32 shader indices (check chunk size)
    ...
    W3D_CHUNK_TEXTURE_IDS
    = 0x00000049, // single or per-tri array of uint32 texture indices (check chunk size)
    W3D_CHUNK_STAGE_TEXCOORDS
    = 0x0000004A, // per-vertex texture coordinates (array of W3dTexCoordStruct's)
    W3D_CHUNK_PER_FACE_TEXCOORD_IDS
    = 0x0000004B, // indices to W3D_CHUNK_STAGE_TEXCOORDS, (array of Vector3i)
    ...
    W3D_CHUNK_COMPRESSED_ANIMATION_HEADER, // describes playback rate, number of frames, and type of
                                           // compression
    W3D_CHUNK_COMPRESSED_ANIMATION_CHANNEL, // compressed channel, format dependent on type of
                                            // compression
    W3D_CHUNK_COMPRESSED_BIT_CHANNEL, // compressed bit stream channel, format dependent on type of
                                      // compression

    W3D_CHUNK_MORPH_ANIMATION
    = 0x000002C0, // hierarchy morphing animation data (morphs between poses, for facial animation)
    W3D_CHUNK_MORPHANIM_HEADER, // W3dMorphAnimHeaderStruct describes playback rate, number of
                                // frames, and type of compression
    W3D_CHUNK_MORPHANIM_CHANNEL, // wrapper for a channel
    W3D_CHUNK_MORPHANIM_POSENAME, // name of the other anim which contains the poses for this morph
                                  // channel
    W3D_CHUNK_MORPHANIM_KEYDATA, // morph key data for this channel
    W3D_CHUNK_MORPHANIM_PIVOTCHANNELDATA, // uin32 per pivot in the htree, indicating which channel
                                          // controls the pivot
    ...
    W3D_CHUNK_PLACEHOLDER, // contains information about a 'dummy' object that will be instanced
                           // later
    W3D_CHUNK_TRANSFORM_NODE, // contains the filename of another w3d file that should be
                              // transformed by this node

    W3D_CHUNK_POINTS = 0x00000440, // array of W3dVectorStruct's.  May appear in meshes, hmodels,
                                   // lodmodels, or collections.
    ...
    W3D_CHUNK_AGGREGATE_CLASS_INFO, // information about the original class that created this
                                    // aggregate
    ...
    W3D_CHUNK_HLOD_AGGREGATE_ARRAY, // array of aggregates, contains
                                    // W3D_CHUNK_SUB_OBJECT_ARRAY_HEADER and
                                    // W3D_CHUNK_SUB_OBJECT_ARRAY
    W3D_CHUNK_HLOD_PROXY_ARRAY, // array of proxies, used for application-defined purposes, provides
                                // a name and a bone.
    ...
    W3D_CHUNK_LIGHT_TRANSFORM, // position and orientation (defined as right-handed 4x3 matrix
                               // transform W3dLightTransformStruct).
...
struct W3dVertexMaterialStruct
{
    ...
    {
        return (Attributes == vm.Attributes && Ambient == vm.Ambient && Diffuse == vm.Diffuse
                && Specular == vm.Specular && Emissive == vm.Emissive && Shininess == vm.Shininess
                && Opacity == vm.Opacity && Translucency == vm.Translucency);
    }
...
enum
{
    ...
    W3DSHADER_DESTBLENDFUNC_ONE_MINUS_SRC_COLOR, // destination pixel multiplied by one minus (i.e.
                                                 // inverse) fragment RGB components
    W3DSHADER_DESTBLENDFUNC_SRC_ALPHA, // destination pixel multiplied by fragment alpha component
    W3DSHADER_DESTBLENDFUNC_ONE_MINUS_SRC_ALPHA, // destination pixel multiplied by fragment inverse
                                                 // alpha
    W3DSHADER_DESTBLENDFUNC_SRC_COLOR_PREFOG, // destination pixel multiplied by fragment RGB
                                              // components prior to fogging
    W3DSHADER_DESTBLENDFUNC_MAX, // end of enumeration

    W3DSHADER_PRIGRADIENT_DISABLE
    = 0, // disable primary gradient (same as OpenGL 'decal' texture blend)
    ...
    W3DSHADER_SRCBLENDFUNC_ONE_MINUS_SRC_ALPHA, // fragment RGB components multiplied by fragment
                                                // inverse (one minus) A
    W3DSHADER_SRCBLENDFUNC_MAX, // end of enumeration

    W3DSHADER_TEXTURING_DISABLE
    = 0, // no texturing (treat fragment initial color as 1,1,1,1) (default)
...
inline void W3d_Shader_Set_PS2_Param_A(W3dShaderStruct* s, int val)
{
    s->ColorMask = val;
}
inline void W3d_Shader_Set_PS2_Param_B(W3dShaderStruct* s, int val)
{
    s->FogFunc = val;
}
inline void W3d_Shader_Set_PS2_Param_C(W3dShaderStruct* s, int val)
{
    s->ShaderPreset = val;
}
inline void W3d_Shader_Set_PS2_Param_D(W3dShaderStruct* s, int val)
{
    s->pad[0] = val;
}

inline int W3d_Shader_Get_PS2_Param_A(const W3dShaderStruct* s)
{
    return (s->ColorMask);
}
inline int W3d_Shader_Get_PS2_Param_B(const W3dShaderStruct* s)
{
    return (s->FogFunc);
}
inline int W3d_Shader_Get_PS2_Param_C(const W3dShaderStruct* s)
{
    return (s->ShaderPreset);
}
inline int W3d_Shader_Get_PS2_Param_D(const W3dShaderStruct* s)
{
    return (s->pad[0]);
}

inline int W3d_Shader_Get_PS2_Param_A(const W3dPS2ShaderStruct* s)
{
    return (s->AParam);
}
inline int W3d_Shader_Get_PS2_Param_B(const W3dPS2ShaderStruct* s)
{
    return (s->BParam);
}
inline int W3d_Shader_Get_PS2_Param_C(const W3dPS2ShaderStruct* s)
{
    return (s->CParam);
}
inline int W3d_Shader_Get_PS2_Param_D(const W3dPS2ShaderStruct* s)
{
    return (s->DParam);
}

inline int W3d_Shader_Get_Depth_Compare(const W3dPS2ShaderStruct* s)
{
    return s->DepthCompare;
}
inline int W3d_Shader_Get_Depth_Mask(const W3dPS2ShaderStruct* s)
{
    return s->DepthMask;
}
inline int W3d_Shader_Get_Pri_Gradient(const W3dPS2ShaderStruct* s)
{
    return s->PriGradient;
}
inline int W3d_Shader_Get_Texturing(const W3dPS2ShaderStruct* s)
{
    return s->Texturing;
}
inline int W3d_Shader_Get_Alpha_Test(const W3dPS2ShaderStruct* s)
{
    return s->AlphaTest;
}
...
#define W3DTEXTURE_PUBLISH                                                                         \
    0x0001 // this texture should be "published" (indirected so its changeable in code)
...
#define W3DTEXTURE_ALPHA_BITMAP                                                                    \
    0x0020 // this texture's alpha channel should be collapsed to one bit
...
#define W3D_MESH_FLAG_COLLISION_BOX                                                                \
    0x00000001 // (obsolete as of 4.1) mesh is a collision box (should be 8 verts, should be hidden,
               // etc)
...
#define W3D_MESH_FLAG_GEOMETRY_TYPE_CAMERA_ORIENTED                                                \
    0x00060000 // (4.1+) camera oriented mesh (points _towards_ camera)
...
#define W3D_MESH_FLAG_PRELIT_VERTEX                                                                \
    0x02000000 // mesh contains a precalculated vertex-lit material chunk wrapper
...
#define W3D_DEFORM_SET_MANUAL_DEFORM                                                               \
    0x00000001 // set is isn't applied during sphere or point tests.
...
struct W3dMeshAABTreeNode
{
    ...
    uint32 FrontOrPoly0; // index of the front child or poly0 (if MSB is set, then leaf and poly0 is
                         // valid)
...
// wrapper for the entire anim W3D_CHUNK_MORPHANIM_HEADER,
...
// name of the other anim which contains the poses for this morph channel
                        W3D_CHUNK_MORPHANIM_KEYDATA,
// array of W3dMorphAnimKeyStruct's (use chunk length to determine how many)
...
#define W3D_ELINE_TEXTURE_MAP_MODE_OFFSET                                                          \
    24 // By how many bits do I need to shift the texture mapping mode?
#define W3D_ELINE_UNIFORM_WIDTH_TEXTURE_MAP                                                        \
    0x00000000 // Entire line uses one row of texture (constant V)
#define W3D_ELINE_UNIFORM_LENGTH_TEXTURE_MAP                                                       \
    0x00000001 // Entire line uses one row of texture stretched length-wise
#define W3D_ELINE_TILED_TEXTURE_MAP 0x00000002 // Tiled continuously over line

#define W3D_ELINE_DEFAULT_BITS                                                                     \
    (W3D_ELINE_MERGE_INTERSECTIONS                                                                 \
     | (W3D_ELINE_UNIFORM_WIDTH_TEXTURE_MAP << W3D_ELINE_TEXTURE_MAP_MODE_OFFSET))
...
/********************************************************************************
        ...
        ----------------
        W3D_CHUNK_HLOD
=0x00000700,		// description of an HLod object (see HLodClass) W3D_CHUNK_HLOD_HEADER,
...
#define W3D_BOX_ATTRIBTUE_COLLISION_TYPE_PROJECTILE                                                \
    0x00000020 // projectiles (rays) collide with this
