...
static BOOL CALLBACK _w3d_utility_cstools_dlg_proc(HWND hWnd, UINT msg, WPARAM wParam,
                                                   LPARAM lParam);
...
public:
    ...
    RefResult NotifyRefChanged(Interval changeInt, RefTargetHandle hTarget, PartID& partID,
                               RefMessage message);
...
RefResult MaterialReferenceMaker::NotifyRefChanged(Interval changeInt, RefTargetHandle hTarget,
...
void W3DUtilityClass::BeginEditParams(Interface* ip, IUtil* iu)
{
    ...
    SettingsPanelHWND = InterfacePtr->AddRollupPage(
        AppInstance, MAKEINTRESOURCE(IDD_W3DUTILITY_SETTINGS_DIALOG), _settings_form_dlg_proc,
        Get_String(IDS_W3DUTILITY_SETTINGS), 0);

    ToolsPanelHWND = InterfacePtr->AddRollupPage(
        AppInstance, MAKEINTRESOURCE(IDD_W3DUTILITY_TOOLS_DIALOG), _w3d_utility_tools_dlg_proc,
        Get_String(IDS_W3DUTILITY_TOOLS), 0);
#if defined W3D_BIPED
    // Moumine 3/18/2002    2:25:02 PM

    ToolsPanelHWND = InterfacePtr->AddRollupPage(
        AppInstance, MAKEINTRESOURCE(IDD_W3DUTILITY_CSTOOLS), _w3d_utility_cstools_dlg_proc,
        Get_String(IDS_W3DUTILITY_CSTOOLS), 0);
#endif
    // End Moumine
    ...
    */
    for (unsigned int ni = 0; ni < list->Num_Nodes(); ni++) {
        ...
        case W3DAppData2Struct::GEO_TYPE_OBBOX:
            ...
            break;
        case W3DAppData2Struct::GEO_TYPE_nullptr:
            ns->Geometrynullptr = true;
            break;
...
void W3DUtilityClass::enable_physical_collision_in_all_selected(INodeListClass* node_list,
                                                                bool onoff)
{
    for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
        W3DAppData2Struct* wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_projectile_collision_in_all_selected(INodeListClass* node_list,
                                                                  bool onoff)
{
    for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
        W3DAppData2Struct* wdata = get_app_data_2((*node_list)[ni]);
...
void W3DUtilityClass::enable_vehicle_collision_in_all_selected(INodeListClass* node_list,
                                                               bool onoff)
{
    for (unsigned int ni = 0; ni < node_list->Num_Nodes(); ni++) {
        W3DAppData2Struct* wdata = get_app_data_2((*node_list)[ni]);
    ...
    if (rootnode != nullptr) {
        ...
        if (meshlist != nullptr) {
            ...
            for (unsigned nodeindex = 0; nodeindex < meshlist->Num_Nodes(); nodeindex++) {
                ...
                // Is this a non-nullptr material?
                if (nodemtl != nullptr) {
                    ...
                    if (!nodemtl->IsMultiMtl()) {
                        ...
                        case GAME_TO_STANDARD:
                            ...
                            if (nodemtl->ClassID() == GameMaterialClassID) {
                                ...
                                // Make a reference to the game material to ensure that it is not
                        ...
                        case STANDARD_TO_GAME:
                            ...
                            // Change materials to game materials if they were previously game
                            // materials before being converted to standard materials.
                            if (gamenodematerials[gamenodematerialindex].MaterialPtr != nullptr) {
                                ((*meshlist)[nodeindex])
                                    ->SetMtl(gamenodematerials[gamenodematerialindex].MaterialPtr);
                                ((GameMtl*)gamenodematerials[gamenodematerialindex].MaterialPtr)
                                    ->Set_Substitute_Material(nullptr);
                            }
                            break;
                        }
                        ...
                        for (int materialindex = 0; materialindex < nodemtl->NumSubMtls();
                            ...
                            // Is this a non-nullptr submaterial?
...
#if defined W3D_BIPED
        if (clsid == BIPSLAVE_CONTROL_CLASS_ID || clsid == FOOTPRINT_CLASS_ID
            || clsid == BIPBODY_CONTROL_CLASS_ID) {
...
W3DAppData1Struct* W3DUtilityClass::get_app_data_1(INode* node)
{
    ...
    if (appdata) {
        ...
        node->AddAppDataChunk(W3DUtilityClassID, UTILITY_CLASS_ID, 1, sizeof(W3DAppData1Struct),
                              wdata);
...
void SettingsFormClass::Update_All_Instances(void)
{
    ...
    */
    INodeListClass node_list(::GetCOREInterface()->GetRootNode(), ::GetCOREInterface()->GetTime(),
                             &_INodeFilter);
...
void SettingsFormClass::Init(void)
{
    ...
    // that I'm using here, reads in a nullptr-terminated string for each entry in the section.  Each
    // string is of the form 'key=value'.  Based on my testing, it appears that windows removes any
    // white space before or after the equal sign as well.
    ...
    ::GetPrivateProfileSection(DAZZLE_TYPES_SECTION, dazzle_types_buffer, DAZZLE_SECTION_BUFFERSIZE,
                               dllpath);
    ...
    */
    RegionSpin = SetupIntSpinner(Hwnd, IDC_DAMREG_INDEX_SPIN, IDC_DAMREG_INDEX_EDIT,
...
bool SettingsFormClass::Dialog_Proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND: {
        ...
        */
        ...
        if ((control_id == IDC_HIERARCHY_CHECK) || (control_id == IDC_GEOMETRY_CHECK)
            || (control_id == IDC_GEOMETRY_HIDE) || (control_id == IDC_GEOMETRY_TWO_SIDED)
            || (control_id == IDC_GEOMETRY_ZNORMALS) || (control_id == IDC_GEOMETRY_VERTEX_ALPHA)
            || (control_id == IDC_GEOMETRY_CAST_SHADOW) || (control_id == IDC_GEOMETRY_SHATTERABLE)
            || (control_id == IDC_GEOMETRY_NPATCH) || (control_id == IDC_COLLISION_PHYSICAL)
            || (control_id == IDC_COLLISION_PROJECTILE) || (control_id == IDC_COLLISION_VIS)
            || (control_id == IDC_COLLISION_CAMERA) || (control_id == IDC_COLLISION_VEHICLE)) {
        ...
        */
        ...
        case IDC_COLLISION_PHYSICAL:
            W3DUtilityClass::enable_physical_collision_in_all_selected(&node_list,
                                                                       check == BST_CHECKED);
            break;

        case IDC_COLLISION_PROJECTILE:
            W3DUtilityClass::enable_projectile_collision_in_all_selected(&node_list,
                                                                         check == BST_CHECKED);
            break;
        ...
        case IDC_COLLISION_CAMERA:
            W3DUtilityClass::enable_camera_collision_in_all_selected(&node_list,
                                                                     check == BST_CHECKED);
            break;

        case IDC_COLLISION_VEHICLE:
            W3DUtilityClass::enable_vehicle_collision_in_all_selected(&node_list,
                                                                      check == BST_CHECKED);
            break;
        ...
        */
        ...
        case IDC_GEOMETRY_nullptr:
            W3DUtilityClass::set_geometry_type_in_all_selected(&node_list,
                                                               W3DAppData2Struct::GEO_TYPE_nullptr);
            break;

        case IDC_GEOMETRY_AABOX:
            W3DUtilityClass::set_geometry_type_in_all_selected(&node_list,
                                                               W3DAppData2Struct::GEO_TYPE_AABOX);
            break;

        case IDC_GEOMETRY_OBBOX:
            W3DUtilityClass::set_geometry_type_in_all_selected(&node_list,
                                                               W3DAppData2Struct::GEO_TYPE_OBBOX);
            break;
        ...
        case IDC_GEOMETRY_DAZZLE:
            W3DUtilityClass::set_geometry_type_in_all_selected(&node_list,
                                                               W3DAppData2Struct::GEO_TYPE_DAZZLE);
            break;
...
void SettingsFormClass::Update_Controls(INodeListClass* node_list)
{
    ...
    */
    if (ns.ExportGeometry == 1) {
        ...
        EnableWindow(GetDlgItem(Hwnd, IDC_GEOMETRY_nullptr), TRUE);
        ...
        EnableWindow(GetDlgItem(Hwnd, IDC_GEOMETRY_nullptr), FALSE);
    ...
    */
    ...
    CheckDlgButton(Hwnd, IDC_GEOMETRY_nullptr, (ns.Geometrynullptr ? BST_CHECKED : BST_UNCHECKED));
...
void SettingsFormClass::Disable_Controls(void)
{
    ...
    CheckDlgButton(Hwnd, IDC_GEOMETRY_nullptr, BST_UNCHECKED);
