...
** local functions
***************************************************************************************/
static void clip_tri_to_slab(Point3 p0, Point3 p1, Point3 p2, float z0, float z1,
                             vertexstruct* outverts, int* setnum);

static void clip_poly(vertexstruct* inverts, int innum, vertexstruct* outverts, int* outnum,
                      const PlaneClass& clipplane);

static void output(const vertexstruct& outvert, vertexstruct* poly, int* numverts);
 ...
 *=============================================================================================*/
VoxelLayerClass::VoxelLayerClass(INodeListClass& object_list, TimeValue time, Matrix3 parenttm,
                                 Point3 offset, Point3 scale, float slicez, float sliceh,
 ...
 *   06/10/1997 GH  : Created.                                                                 *
 *=============================================================================================*/
void VoxelLayerClass::Intersect_Triangle(Point3 a, Point3 b, Point3 c, float z)
{
...
void VoxelLayerClass::Scan_Triangle(Point3 p0, Point3 p1, Point3 p2)
{
    ...
    if (p0.z < SliceZ0 && p1.z < SliceZ0 && p2.z < SliceZ1) {
        return;
    }
    if (p0.z > SliceZ1 && p1.z > SliceZ1 && p2.z > SliceZ1) {
        return;
    }
    ...
    for (i = 1; i < numverts; i++) {
        if (polyvert[i].Pos.y < miny) {
            miny = polyvert[i].Pos.y;
        }
        if (polyvert[i].Pos.y > maxy) {
            maxy = polyvert[i].Pos.y;
        }
    }
    ...
    for (i = (int)floor(miny); i <= (int)floor(maxy); i++) {
        ...
        Draw_Line(_scantab[i].P[LEFT].Pos.x, _scantab[i].P[LEFT].Pos.y, _scantab[i].P[RIGHT].Pos.x,
                              ...
                              const PlaneClass& plane)
{
    ...
    float num = -(plane.N[0] * p0.Pos.x + plane.N[1] * p0.Pos.y + plane.N[2] * p0.Pos.z + plane.D);

    float den = plane.N[0] * delta.x + plane.N[1] * delta.y + plane.N[2] * delta.z;
