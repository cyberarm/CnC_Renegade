 ...
 *=============================================================================================*/
GameMtlVertexMaterialDlg::GameMtlVertexMaterialDlg(HWND parent, IMtlParams* imp, GameMtl* mtl,
                                                   int pass)
...
BOOL GameMtlVertexMaterialDlg::Dialog_Proc(HWND dlg_wnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    switch (message) {
    case WM_INITDIALOG: {
        AmbientSwatch = GetIColorSwatch(GetDlgItem(dlg_wnd, IDC_AMBIENT_COLOR),
                                        TheMtl->Get_Ambient(PassIndex, IParams->GetTime()),
                                        Get_String(IDS_AMBIENT_COLOR));
        DiffuseSwatch = GetIColorSwatch(GetDlgItem(dlg_wnd, IDC_DIFFUSE_COLOR),
                                        TheMtl->Get_Diffuse(PassIndex, IParams->GetTime()),
                                        Get_String(IDS_DIFFUSE_COLOR));
        SpecularSwatch = GetIColorSwatch(GetDlgItem(dlg_wnd, IDC_SPECULAR_COLOR),
                                         TheMtl->Get_Specular(PassIndex, IParams->GetTime()),
                                         Get_String(IDS_SPECULAR_COLOR));
        EmissiveSwatch = GetIColorSwatch(GetDlgItem(dlg_wnd, IDC_EMISSIVE_COLOR),
                                         TheMtl->Get_Emissive(PassIndex, IParams->GetTime()),
                                         Get_String(IDS_EMISSIVE_COLOR));

        OpacitySpin = SetupFloatSpinner(dlg_wnd, IDC_OPACITY_SPIN, IDC_OPACITY_EDIT, 0.0f, 1.0f,
                                        TheMtl->Get_Opacity(PassIndex, IParams->GetTime()), 0.01f);
        TranslucencySpin
            = SetupFloatSpinner(dlg_wnd, IDC_TRANSLUCENCY_SPIN, IDC_TRANSULCENCY_EDIT, 0.0f, 1.0f,
                                TheMtl->Get_Translucency(PassIndex, IParams->GetTime()), 0.01f);
        ShininessSpin
            = SetupFloatSpinner(dlg_wnd, IDC_SHININESS_SPIN, IDC_SHININESS_EDIT, 1.0f, 1000.0f,
                                TheMtl->Get_Shininess(PassIndex, IParams->GetTime()), 1.0f);
        UVChannelSpin[0]
            = SetupIntSpinner(dlg_wnd, IDC_STAGE0UVCHAN_SPIN, IDC_STAGE0UVCHAN_EDIT, 1, 99, 1);
        UVChannelSpin[1]
            = SetupIntSpinner(dlg_wnd, IDC_STAGE1UVCHAN_SPIN, IDC_STAGE1UVCHAN_EDIT, 1, 99, 1);
        break;
    }
    ...
    case WM_COMMAND: {
        ...
        case IDC_COPY_SPECULAR_DIFFUSE:
            TheMtl->Set_Copy_Specular_To_Diffuse(
                PassIndex, GetCheckBox(dlg_wnd, IDC_COPY_SPECULAR_DIFFUSE) == TRUE);
            break;
