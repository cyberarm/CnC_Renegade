    ...
    case WM_COMMAND:
        ...
        case IDC_TEXTURE_BUTTON:
            if (HIWORD(wParam) == BN_CLICKED) {
                PostMessage(HwndEdit, WM_TEXMAP_BUTTON, TheMtl->Get_Displacement_Map_Index(),
                            (LPARAM)TheMtl);
            }
            break;
        }
...
BOOL GMaxMtlDlg::SurfaceTypeProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_USER + 101: {
        ...
        ::SendDlgItemMessage(hDlg, IDC_SURFACE_TYPE_COMBO, CB_SETCURSEL,
                             (WPARAM)TheMtl->Get_Surface_Type(), 0L);
    ...
    case WM_COMMAND: {
        ...
        case IDC_SURFACE_TYPE_COMBO: {
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                unsigned int type
                    = ::SendDlgItemMessage(hDlg, IDC_SURFACE_TYPE_COMBO, CB_GETCURSEL, 0, 0L);
...
{
    int res
        = DialogBoxParam(AppInstance, MAKEINTRESOURCE(IDD_GAMEMTL_PASS_COUNT_DIALOG), HwndPassCount,
                         PassCountDialogDlgProc, (LPARAM)TheMtl->Get_Pass_Count());
    if (res >= 0) {
        if (res <= 0) {
            res = 1;
        }
...
static BOOL CALLBACK PassCountDialogDlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
        ISpinnerControl* spin
            = SetupIntSpinner(hwndDlg, IDC_PASSCOUNT_SPIN, IDC_PASSCOUNT_EDIT, 1, 4, (int)lParam);
...
void GMaxMtlDlg::Make_PropertySheet(HWND hParent)
{
    ...
    ps_Header.dwFlags
        = PSH_PROPSHEETPAGE | PSH_MODELESS | PSH_USECALLBACK | PSH_USEICONID | PSH_NOAPPLYNOW; // ;
...
//================================================
void GMaxMtlDlg::Reinitialize(GameMtl* new_mtl, bool update_multimtl)
{ // = false
    // Delete pages
    int npages(0);
    if (TheMtl) {
    ...
    int sort_level = TheMtl->Get_Sort_Level();
    SetupIntSpinner(HwndSurfaceType, IDC_SORT_LEVEL_SPIN, IDC_SORT_LEVEL, 0, MAX_SORT_LEVEL,
                    sort_level);
    // Check the checkbox if sort_level is not SORT_LEVEL_NONE.
    ::SendDlgItemMessage(HwndSurfaceType, IDC_ENABLE_SORT_LEVEL, BM_SETCHECK,
                         sort_level == SORT_LEVEL_NONE ? BST_UNCHECKED : BST_CHECKED, 0);
    // Surfface type combo
    ::SendDlgItemMessage(HwndSurfaceType, IDC_SURFACE_TYPE_COMBO, CB_SETCURSEL,
                         (WPARAM)TheMtl->Get_Surface_Type(), 0L);
...
void GMaxMtlDlg::DeleteAllSceneMtl()
{
    ...
    if (scene_mtls) {
        if (!DontShowDeleteAll) {
            ...
            if (IDYES
                == MaxMsgBox(Ip->GetMAXHWnd(), msg, title, MB_YESNO | MB_ICONINFORMATION,
                             MAX_MB_DONTSHOWAGAIN, &ret)) {
                scene_mtls->Empty();
            }
...
void GMaxMtlDlg::ShowHideControls()
{
    if (nullptr == Game_multi_mtl) {
        ...
        SendDlgItemMessage(HwndSurfaceType, IDC_SUBMTL_SPIN, UDM_SETRANGE, (WPARAM)0,
                           MAKELONG(n_subs, 1));
        ICustEdit* pEdit = GetICustEdit(GetDlgItem(HwndSurfaceType, IDC_SUBMTL_NUM));
        pEdit->SetText(1);
        ReleaseICustEdit(pEdit);
        ...
        if (nullptr != new_mtl) {
            if (new_mtl->ClassID() == GameMaterialClassID) { // && new_mtl != TheMtl){
                ...
                if (!DontShowMtlType) {
                    ...
                    LoadString(AppInstance, IDS_W3DMSGCAPTION, title, MAX_PATH);
                    MaxMsgBox(HwndSurfaceType, msg, title, MB_OK | MB_ICONINFORMATION,
                              MAX_MB_DONTSHOWAGAIN, &ret);
                    if (ret & MAX_MB_DONTSHOWAGAIN) {
...
void GMaxMtlDlg::SetMultiMaterialBySlot(int iSlot, int iIndex)
{
    ...
    // (gth) MATERIAL BEING CHANGED, user double clicked a member of a multi-material in the
    // navagator
