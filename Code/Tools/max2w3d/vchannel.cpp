...
VectorChannelClass::VectorChannelClass(uint32 id, int maxframes, uint32 flags, int vectorlength,
                                       float32* identvect)
    : ID(id),
      Flags(flags),
      MaxFrames(maxframes),
...
//
float VectorChannelClass::test_compress(int filter_index, float scale, float value1, float* indata,
                                        float* outdata)
{
...
// output: float error;		// aggregate error for packet
//
float VectorChannelClass::compress(int filter_index, float scale, float value1, float* indata,
                                   unsigned char* pPacket, float* outdata)
{
    float error = 0.0f;
...
bool VectorChannelClass::SaveAdaptiveDelta(ChunkSaveClass& csave, BitChannelClass* binmov)
{
    ...
    chn->Flags = Flags;
    chn->Scale = 0.0f;
    memset(&chn->Data[0], channelsize - (sizeof(W3dAdaptiveDeltaAnimChannelStruct) - sizeof(char)),
           0x00);
    ...
    for (i = 0; i < numpackets; i++) {
        for (int vi = 0; vi < VectorLen; vi++) {
            ...
            for (int try_filter = 0; try_filter < FILTER_TABLE_SIZE; try_filter++) {
                ...
                float temp_error = test_compress(try_filter, scale, last_value, original_packet,
                                                 decompressed_packet);
            ...
            // log->printf("\nvi= %d frames %d to %d : error = %f ",vi, frame, frame+15,
            // least_error);
            ...
            pPacket
                += sizeof(AdaptiveDeltaPacketStruct) * vi; // skip up the appropriate vector index
...
void VectorChannelClass::SetSaveOptions(bool compress, int flavor, float Terr, float Rerr,
                                        bool reduce, int reduce_percent)
{
...
//  R2 - set invisible data to repeat last known position, the previous algorthm caused problems
//  with the current movie assets
//
    ...
    if (c->NumTimeCodes > 1) {
        if (c->NumTimeCodes > 2) {
            ...
            for (uint32 try_idx = 0; try_idx < (c->NumTimeCodes - 2); try_idx++) {
                ...
                float32 tStart
                    = ((*pTcSrc) & ~W3D_TIMECODED_BINARY_MOVEMENT_FLAG); // upgrade to floats
