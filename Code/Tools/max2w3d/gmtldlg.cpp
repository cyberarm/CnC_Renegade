...
BOOL GameMtlDlg::PanelProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
        ...
        DiffuseSwatch = GetIColorSwatch(GetDlgItem(hwndDlg, IDC_DIFFUSE_COLOR),
                                        TheMtl->GetDiffuse(), Get_String(IDS_DIFFUSE_COLOR));
        SpecularSwatch = GetIColorSwatch(GetDlgItem(hwndDlg, IDC_SPECULAR_COLOR),
                                         TheMtl->GetSpecular(), Get_String(IDS_SPECULAR_COLOR));

        AmbientCoeffSwatch
            = GetIColorSwatch(GetDlgItem(hwndDlg, IDC_AMBIENT_COEFF), TheMtl->GetAmbientCoeff(),
                              Get_String(IDS_AMBIENT_COEFF));
        DiffuseCoeffSwatch
            = GetIColorSwatch(GetDlgItem(hwndDlg, IDC_DIFFUSE_COEFF), TheMtl->GetDiffuseCoeff(),
                              Get_String(IDS_DIFFUSE_COEFF));
        SpecularCoeffSwatch
            = GetIColorSwatch(GetDlgItem(hwndDlg, IDC_SPECULAR_COEFF), TheMtl->GetSpecularCoeff(),
                              Get_String(IDS_SPECULAR_COEFF));
        EmissiveCoeffSwatch
            = GetIColorSwatch(GetDlgItem(hwndDlg, IDC_EMISSIVE_COEFF), TheMtl->GetEmissiveCoeff(),
                              Get_String(IDS_EMISSIVE_COEFF));

        DCTFramesSpin = SetupIntSpinner(hwndDlg, IDC_DCT_FRAMES_SPIN, IDC_DCT_FRAMES_EDIT, 1, 999,
                                        TheMtl->DCTFrames);
        DITFramesSpin = SetupIntSpinner(hwndDlg, IDC_DIT_FRAMES_SPIN, IDC_DIT_FRAMES_EDIT, 1, 999,
                                        TheMtl->DITFrames);
        SCTFramesSpin = SetupIntSpinner(hwndDlg, IDC_SCT_FRAMES_SPIN, IDC_SCT_FRAMES_EDIT, 1, 999,
                                        TheMtl->SCTFrames);
        SITFramesSpin = SetupIntSpinner(hwndDlg, IDC_SIT_FRAMES_SPIN, IDC_SIT_FRAMES_EDIT, 1, 999,
                                        TheMtl->SITFrames);

        DCTRateSpin = SetupFloatSpinner(hwndDlg, IDC_DCT_RATE_SPIN, IDC_DCT_RATE_EDIT, 0.0f, 60.0f,
                                        TheMtl->DCTFrameRate, 5.0f);
        DITRateSpin = SetupFloatSpinner(hwndDlg, IDC_DIT_RATE_SPIN, IDC_DIT_RATE_EDIT, 0.0f, 60.0f,
                                        TheMtl->DITFrameRate, 5.0f);
        SCTRateSpin = SetupFloatSpinner(hwndDlg, IDC_SCT_RATE_SPIN, IDC_SCT_RATE_EDIT, 0.0f, 60.0f,
                                        TheMtl->SCTFrameRate, 5.0f);
        SITRateSpin = SetupFloatSpinner(hwndDlg, IDC_SIT_RATE_SPIN, IDC_SIT_RATE_EDIT, 0.0f, 60.0f,
                                        TheMtl->SITFrameRate, 5.0f);

        OpacitySpin = SetupFloatSpinner(hwndDlg, IDC_OPACITY_SPIN, IDC_OPACITY_EDIT, 0.0f, 1.0f,
                                        TheMtl->GetOpacity(), 0.01f);
        TranslucencySpin = SetupFloatSpinner(hwndDlg, IDC_TRANSLUCENCY_SPIN, IDC_TRANSULCENCY_EDIT,
                                             0.0f, 1.0f, TheMtl->GetTranslucency(), 0.01f);
        ShininessSpin = SetupFloatSpinner(hwndDlg, IDC_SHININESS_SPIN, IDC_SHININESS_EDIT, 1.0f,
                                          1000.0f, TheMtl->GetShininess(), 1.0f);
        FogSpin = SetupFloatSpinner(hwndDlg, IDC_FOG_SPIN, IDC_FOG_EDIT, 0.0f, 1.0f,
                                    TheMtl->FogCoeff, 0.01f);
    ...
    case WM_COMMAND: {
        ...
        case IDC_MAPON_DCT:
            ...
            if (!GetCheckBox(hwndDlg, id)) {
                TheMtl->SetSubTexmap(ID_DI, NULL);
            }
        ...
        case IDC_MAPON_DIT:
            ...
            if (!GetCheckBox(hwndDlg, id)) {
                TheMtl->SetSubTexmap(ID_SI, NULL);
            }
        ...
        case IDC_MAPON_SCT:
            ...
            if (!GetCheckBox(hwndDlg, id)) {
                TheMtl->SetSubTexmap(ID_SP, NULL);
            }
        ...
        case IDC_MAPON_SIT:
            ...
            if (!GetCheckBox(hwndDlg, id)) {
                TheMtl->SetSubTexmap(ID_RL, NULL);
            }
...
BOOL GameMtlDlg::HintsProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND: {
        ...
        case IDC_FAST_SPECULAR_AFTER_ALPHA_CHECK:
            TheMtl->SetAttribute(GAMEMTL_FAST_SPECULAR_AFTER_ALPHA,
...
void GameMtlDlg::LoadDialog(BOOL draw)
{
    ...
    */
    if (TheMtl && HwndPanel) {
        ...
        ** Mapping types
        */
        SendDlgItemMessage(HwndPanel, IDC_DCT_MAPPING_COMBO, CB_SETCURSEL, TheMtl->DCTMappingType,
                           0);
        SendDlgItemMessage(HwndPanel, IDC_DIT_MAPPING_COMBO, CB_SETCURSEL, TheMtl->DITMappingType,
                           0);
        SendDlgItemMessage(HwndPanel, IDC_SCT_MAPPING_COMBO, CB_SETCURSEL, TheMtl->SCTMappingType,
                           0);
        SendDlgItemMessage(HwndPanel, IDC_SIT_MAPPING_COMBO, CB_SETCURSEL, TheMtl->SITMappingType,
                           0);
...
void GameMtlDlg::BuildDialog()
{
    ...
    HwndPanel = IParams->AddRollupPage(
        AppInstance, MAKEINTRESOURCE(IDD_GAMEMTL_PANEL), PanelDlgProc, Get_String(IDS_PARAMETERS),
        (LPARAM)this, (TheMtl->GetFlag(GAMEMTL_ROLLUP1_OPEN) ? 0 : APPENDROLL_CLOSED));

    HwndPsx
        = IParams->AddRollupPage(AppInstance, MAKEINTRESOURCE(IDD_GAMEMTL_PSX_PANEL), PsxDlgProc,
                                 Get_String(IDS_PSX_OPTIONS), (LPARAM)this,
                                 (TheMtl->GetFlag(GAMEMTL_ROLLUP2_OPEN) ? 0 : APPENDROLL_CLOSED));

    HwndHints
        = IParams->AddRollupPage(AppInstance, MAKEINTRESOURCE(IDD_GAMEMTL_HINTS_PANEL),
                                 HintsDlgProc, Get_String(IDS_MATERIAL_HINTS), (LPARAM)this,
                                 (TheMtl->GetFlag(GAMEMTL_ROLLUP3_OPEN) ? 0 : APPENDROLL_CLOSED));

    HwndNotes = IParams->AddRollupPage(
        AppInstance, MAKEINTRESOURCE(IDD_MATERIAL_NOTES_PANEL), NotesDlgProc, Get_String(IDS_NOTES),
        (LPARAM)this, (TheMtl->GetFlag(GAMEMTL_ROLLUP4_OPEN) ? 0 : APPENDROLL_CLOSED));
