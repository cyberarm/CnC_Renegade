 ...
 *=============================================================================================*/
MotionClass::MotionClass(IScene* scene, INode* rootnode, HierarchySaveClass* basepose,
                         W3dExportOptionsStruct& options, int framerate,
                         Progress_Meter_Class* meter, HWND MaxHwnd, char* name, Matrix3& offset)
    : BasePose(basepose),
      Scene(scene),
      RootNode(rootnode),
      RootList(NULL),
 ...
 *   10/26/1997 GH  : Created.                                                                 *
 *=============================================================================================*/
MotionClass::MotionClass(IScene* scene, INodeListClass* rootlist, HierarchySaveClass* basepose,
                         W3dExportOptionsStruct& options, int framerate,
                         Progress_Meter_Class* meter, HWND MaxHwnd, char* name, Matrix3& offset)
    : BasePose(basepose),
      Scene(scene),
...
void MotionClass::compute_frame_motion(int frame)
{
    ...
    */
    ...
    if (RootNode != NULL) {
        ...
        tree = new HierarchySaveClass(RootList, frametime, *Meter, "NoName", false, BasePose,
                                      Offset);
    }
...
{
    return Point3(EulerDelta[node][frame].x, EulerDelta[node][frame].y, EulerDelta[node][frame].z);
}
        ...
        */
        if (NodeValidFlags[nodeidx]) {
            ...
            xchan.SetSaveOptions(CompressAnimation, CompressAnimationFlavor,
                                 CompressAnimationTranslationError, CompressAnimationRotationError,
                                 ReduceAnimation, ReduceAnimationPercent);
