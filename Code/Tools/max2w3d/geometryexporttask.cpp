...
public:
          ...
          SingleMtl(nullptr)
    {
        ...
        */
        ...
        /*
        ** Store a pointer to the material if this mesh uses only one material (even inside a
        *Multi-Sub)
        */
        Update_Cached_Data();
    }
    ...
    virtual void Export_Geometry(GeometryExportContextClass& context)
    {
        ...
        */
        ...
        MeshSaveClass* mesh = new MeshSaveClass(Name, ContainerName, Node, &MeshData, ExportSpace,
                                                ExportOptions, context.HTree, context.CurTime,
                                                *context.ProgressMeter, &context.WorldInfo);
...
protected:
    ...
    W3DAppData2Struct ExportOptions; // Copy of the export options in case we want to change them
                                     // during optimization
    bool NameDirty;

    // Cached Data about the Node/Mesh.  Updated by calling Update_Cached_Data whenever the mesh
    // changes.
    Mtl* SingleMtl; // Pointer to the single material (if the mesh uses only one, even in a
                    // multi-mtl)
...
public:
    ...
    virtual void Export_Geometry(GeometryExportContextClass& context)
    {
        ...
        */
        CollisionBoxSaveClass* colbox = new CollisionBoxSaveClass(
            Name, ContainerName, Node, ExportSpace, context.CurTime, *context.ProgressMeter);
...
public:
    ...
    virtual void Export_Geometry(GeometryExportContextClass& context)
    {
        ...
        */
        DazzleSaveClass* dazzle = new DazzleSaveClass(Name, ContainerName, Node, ExportSpace,
...
/**
** nullptrGeometryExportTaskClass
...
*/
class nullptrGeometryExportTaskClass : public GeometryExportTaskClass
{
public:
    nullptrGeometryExportTaskClass(INode* node, GeometryExportContextClass& context)
        : GeometryExportTaskClass(node, context)
    {
...
protected:
    virtual int Get_Geometry_Type(void) { return nullptrOBJ; }
};
...
** geometry (similer to the nullptr export task) and it clears its container name
                                                              ...
                                                              GeometryExportContextClass& context)
{
	...
	*/
    ...
    if (::Is_Normal_Mesh(node) || Is_Camera_Aligned_Mesh(node) || Is_Camera_Oriented_Mesh(node)
        || Is_Skin(node)) {
    ...
    if (::Is_nullptr_Object(node)) {
        return new nullptrGeometryExportTaskClass(node, context);
    }
                                                ...
                                                GeometryExportContextClass& context)
{
    ...
    */
    ...
    while (meshes.Count() > 0) {
        ...
        ** Otherwise, have it split into new tasks, add them to the simple_meshes array, and delete
        *this task.
        */
 ...
 *=============================================================================================*/
void GeometryExportTaskClass::Generate_Name(char* root, int index,
                                            GeometryExportContextClass& context)
{
    /*
...
void MeshGeometryExportTaskClass::Update_Cached_Data(void)
{
    ...
    /*
    ** Set the SingleMtl pointer if this mesh uses only one material (again, even if its in a
    *Multi-Sub)
    */
    if (nodemtl == nullptr) {
