 ...
 *=============================================================================================*/
GameMtlTextureDlg::GameMtlTextureDlg(HWND parent, IMtlParams* imp, GameMtl* mtl, int pass)
...
BOOL GameMtlTextureDlg::Dialog_Proc(HWND dlg_wnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    switch (message) {
    case WM_INITDIALOG: {
        Stage0FramesSpin = SetupIntSpinner(dlg_wnd, IDC_STAGE0_FRAMES_SPIN, IDC_STAGE0_FRAMES_EDIT,
                                           1, 999, TheMtl->Get_Texture_Frame_Count(PassIndex, 0));

        Stage0RateSpin
            = SetupFloatSpinner(dlg_wnd, IDC_STAGE0_RATE_SPIN, IDC_STAGE0_RATE_EDIT, 0.0f, 60.0f,
                                TheMtl->Get_Texture_Frame_Rate(PassIndex, 0), 1.0f);

        Stage1FramesSpin = SetupIntSpinner(dlg_wnd, IDC_STAGE1_FRAMES_SPIN, IDC_STAGE1_FRAMES_EDIT,
                                           1, 999, TheMtl->Get_Texture_Frame_Count(PassIndex, 1));

        Stage1RateSpin
            = SetupFloatSpinner(dlg_wnd, IDC_STAGE1_RATE_SPIN, IDC_STAGE1_RATE_EDIT, 0.0f, 60.0f,
    ...
    case CC_SPINNER_CHANGE: {
        TheMtl->Set_Texture_Frame_Count(PassIndex, 0, Stage0FramesSpin->GetIVal());

        TheMtl->Set_Texture_Frame_Rate(PassIndex, 0, Stage0RateSpin->GetFVal());

        TheMtl->Set_Texture_Frame_Count(PassIndex, 1, Stage1FramesSpin->GetIVal());
    ...
    case WM_COMMAND: {
        ...
        case IDC_STAGE0_PUBLISH_BUTTON: {
            TheMtl->Set_Texture_Publish(PassIndex, 0,
                                        (Stage0PublishButton->IsChecked() ? TRUE : FALSE));
            break;
        }
        case IDC_STAGE1_PUBLISH_BUTTON: {
            TheMtl->Set_Texture_Publish(PassIndex, 1,
                                        (Stage1PublishButton->IsChecked() ? TRUE : FALSE));
            break;
        }

        case IDC_STAGE0_CLAMP_U_BUTTON: {
            TheMtl->Set_Texture_Clamp_U(PassIndex, 0,
                                        (Stage0ClampUButton->IsChecked() ? TRUE : FALSE));
            break;
        }
        case IDC_STAGE1_CLAMP_U_BUTTON: {
            TheMtl->Set_Texture_Clamp_U(PassIndex, 1,
                                        (Stage1ClampUButton->IsChecked() ? TRUE : FALSE));
            break;
        }
        case IDC_STAGE0_CLAMP_V_BUTTON: {
            TheMtl->Set_Texture_Clamp_V(PassIndex, 0,
                                        (Stage0ClampVButton->IsChecked() ? TRUE : FALSE));
            break;
        }
        case IDC_STAGE1_CLAMP_V_BUTTON: {
            TheMtl->Set_Texture_Clamp_V(PassIndex, 1,
                                        (Stage1ClampVButton->IsChecked() ? TRUE : FALSE));
            break;
        }
        case IDC_STAGE0_NOLOD_BUTTON: {
            TheMtl->Set_Texture_No_LOD(PassIndex, 0,
                                       (Stage0NoLODButton->IsChecked() ? TRUE : FALSE));
            break;
        }
        case IDC_STAGE1_NOLOD_BUTTON: {
            TheMtl->Set_Texture_No_LOD(PassIndex, 1,
                                       (Stage1NoLODButton->IsChecked() ? TRUE : FALSE));
            break;
        }
        case IDC_STAGE0_ALPHA_BITMAP_BUTTON: {
            TheMtl->Set_Texture_Alpha_Bitmap(PassIndex, 0,
                                             (Stage0AlphaBitmapButton->IsChecked() ? TRUE : FALSE));
            break;
        }
        case IDC_STAGE1_ALPHA_BITMAP_BUTTON: {
            TheMtl->Set_Texture_Alpha_Bitmap(PassIndex, 1,
                                             (Stage0AlphaBitmapButton->IsChecked() ? TRUE : FALSE));
            break;
        }
        case IDC_STAGE0_DISPLAY_BUTTON: {
            TheMtl->Set_Texture_Display(PassIndex, 0,
                                        (Stage0DisplayButton->IsChecked() ? TRUE : FALSE));
            TheMtl->Notify_Changed();
            break;
        }
        case IDC_STAGE1_DISPLAY_BUTTON: {
            TheMtl->Set_Texture_Display(PassIndex, 1,
                                        (Stage1DisplayButton->IsChecked() ? TRUE : FALSE));
            TheMtl->Notify_Changed();
            break;
        }
