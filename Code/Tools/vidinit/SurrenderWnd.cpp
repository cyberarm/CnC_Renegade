...
CSurrenderWnd::CSurrenderWnd(CWnd* cwnd, int driver, int width, int height, int bitdepth,
                             bool run_in_window)
    : m_CWnd(cwnd),
      m_Motion(0),
      m_Camera(0),
      ...
      m_FrameRate(60)
{
    ...
    if (run_in_window) {
        CreateEx(0, str, "Card Test Window", WS_SYSMENU | WS_MINIMIZEBOX | WS_CAPTION | WS_VISIBLE,
                 0, 0, 640, 480, nullptr, nullptr, nullptr);
    }
    else {
        CreateEx(0, str, "Card Test Window", WS_SYSMENU | WS_MINIMIZEBOX | WS_CAPTION | WS_VISIBLE,
                 0, 0, 640, 480, nullptr, nullptr, nullptr);
    ...
    m_Text = new Text2DObjClass(DefaultFont, "Frame rate = %d", 0, 0, 15, 0, DefaultConvert, false,
                                false, m_FrameRate);
...
CSurrenderWnd::~CSurrenderWnd()
{
    ...
    if (m_Motion) {
        m_Motion->Release_Ref();
    }
    if (m_Text) {
        m_Text->Release_Ref();
    }
    if (m_TextCamera) {
        m_TextCamera->Release_Ref();
    }
    if (m_Scene) {
        m_Scene->Release_Ref();
    }
    if (m_TextScene) {
        m_TextScene->Release_Ref();
    }
...
void CSurrenderWnd::OnPaint()
{
    ...
    if (m_Motion && m_RObj) {
        ...
        m_Text->Set_Text(DefaultFont, "Frame rate = %d", 0, 0, 15, 0, DefaultConvert, false, false,
                         m_FrameRate);
