...
#define PAGE_HEIGHT                                                                                \
    256 // NOTE: Elected to use the biggest texture size that is
...
#define PACKING_BYTES                                                                              \
    (1024 * 1024) // No. of page bytes to consider for packing	the current lightmap.
...
char LightmapPacker::_Statistics[LightmapPacker::STATISTICS_COUNT]
                                [LightmapPacker::STATISTICS_STRING_SIZE]
...
LightmapPacker::LightmapPacker()
{
    ...
    FilterError = pow(options.Get_Filter_Error() * 4.919e-3l,
                      2.5l); // Map s.t. 50th percentile -> 0.03% RGB error.
 ...
 *=============================================================================================*/
void LightmapPacker::Submit(PackingTriangle* principaltriangleptr,
                            const DynamicVectorClass<Triangle>& adjtriangles)
{
    ...
    if (principaltriangleptr->TextureNameNodePtr == nullptr) {
        ...
        // NOTE: Use a 'placeholder group' ID to indicate to the triangle packer that it should not
        // be merged with non-placeholder triangles.
        trianglepackerptr
            = new TrianglePacker(principaltriangleptr, noadjtriangles, placeholdergroupid,
                                 EDGE_BLEND_THICKNESS, SampleRate, FillColor);
        ASSERT(trianglepackerptr != nullptr);
    }
    else {

        // Create a triangle packer from the principal and adjacent triangles.
        trianglepackerptr = new TrianglePacker(principaltriangleptr, adjtriangles, normalgroupid,
                                               EDGE_BLEND_THICKNESS, SampleRate, FillColor);
        ASSERT(trianglepackerptr != nullptr);
    }
...
TrianglePacker* LightmapPacker::Merge(TrianglePacker* trianglepackerptr)
{
    ...
    for (int t = 0; t < TrianglePackerPtrs.Count(); t++) {
        ...
        if (mergedpackerptr != nullptr) {
            ...
            if ((mergedpackerptr->Width() <= PAGE_WIDTH)
                && (mergedpackerptr->Height() <= PAGE_HEIGHT)) {
                ...
                unmergedarea = (trianglepackerptr->Width() * trianglepackerptr->Height())
                    + (TrianglePackerPtrs[t]->Width() * TrianglePackerPtrs[t]->Height());
...
void LightmapPacker::Pack(ProceduralTexture* proceduraltexture)
{
    ...
    for (int t = 0; t < TrianglePackerPtrs.Count(); t++) {
        ...
        const unsigned rescalethreshold = 16; // When snapping to even dimension, 'round down' if
                                              // above this size - otherwise 'round up'.
        ...
        if ((clampedwidth < rasterizedtarga.Width())
            || (clampedheight < rasterizedtarga.Height())) {
        ...
        // NOTE: This will reduce texel bleeding as a side effect of mip-mapping or rendering at
        // reduced resolutions.
        ...
        // Add pad texels to compensate for edge texels that may have been lost due to filtered
        // scaling and scaling to clamp to maximum page size. NOTE: Pad to a multiple of 2 to
        // maintain 'even' dimensions.
        padwidth = 2
            * ceilf(EDGE_BLEND_THICKNESS
                    * (1.0f - (((float)rasterizedtarga.Width()) / ((float)unscaledwidth))));
        padheight = 2
            * ceilf(EDGE_BLEND_THICKNESS
                    * (1.0f - (((float)rasterizedtarga.Height()) / ((float)unscaledheight))));
        rasterizedtarga.Pad(padwidth, padheight, FillColor,
                            TrianglePackerPtrs[t]->Principal_Triangles());
        ...
        // NOTE: Although this is not desirable, it is possible for scaling to increase the targa's
        // area.
...
TrueColorTarga* TargaLoader::Load(const Triangle& triangle)
{
    ...
    if ((Cache[index].Ptr == nullptr) || (Cache[index].ID != triangle.TextureID)) {
        ...
        // Replace fill texel with padding. If the targa is all fill color substitute the
        // replacement targa.
...
void LightmapPacker::Pack(TrueColorTarga& targa, DynamicVectorClass<PackingTriangle*>& triangleptrs)
{
    ...
    if (!packed) {
        ...
        const unsigned packingwindowsize
            = PACKING_BYTES / ((PAGE_WIDTH * PAGE_HEIGHT * PageBitDepth) / CHAR_BIT);
...
{
    unsigned pagecount;
    float adjacentfaceblendfraction, edgeblendefficiency, scalingefficiency, packingefficiency,
        cullingefficiency, swappingefficiency;

    // Build statistics string.
    adjacentfaceblendfraction
        = (AdjacentFaceCount > 0) ? ((float)BlendedFaceCount) / ((float)AdjacentFaceCount) : 0.0f;
    edgeblendefficiency
        = (UnscaledTexelCount > 0) ? 1.0f - (EdgeBlendAreaSum / UnscaledTexelCount) : 0.0f;
    scalingefficiency = (UnscaledTexelCount > 0)
        ? 1.0f - (((float)ScaledTexelCount) / ((float)UnscaledTexelCount))
        : 0.0f;
    ...
    _snprintf(_Statistics[STATISTICS_PAGE_FORMAT], sizeof(_Statistics[STATISTICS_PAGE_FORMAT]) - 1,
              "%d x %d x %dbpp", PAGE_WIDTH, PAGE_HEIGHT, PageBitDepth);
    _snprintf(_Statistics[STATISTICS_LIGHTMAPS_PROCESSED],
              sizeof(_Statistics[STATISTICS_LIGHTMAPS_PROCESSED]) - 1, "%d", LightmapCount);
    _snprintf(_Statistics[STATISTICS_ADJACENT_FACE_BLEND_PERCENTAGE],
              sizeof(_Statistics[STATISTICS_ADJACENT_FACE_BLEND_PERCENTAGE]) - 1, "%.1f%%",
              adjacentfaceblendfraction * 100.0f);
    _snprintf(_Statistics[STATISTICS_EDGE_BLEND_EFFICIENCY],
              sizeof(_Statistics[STATISTICS_EDGE_BLEND_EFFICIENCY]) - 1, "%.1f%%",
              edgeblendefficiency * 100.0f);
    _snprintf(_Statistics[STATISTICS_SCALING_EFFICIENCY],
              sizeof(_Statistics[STATISTICS_SCALING_EFFICIENCY]) - 1, "%.1f%%",
              scalingefficiency * 100.0f);
    _snprintf(_Statistics[STATISTICS_PAGES_CREATED],
              sizeof(_Statistics[STATISTICS_PAGES_CREATED]) - 1, "%d", pagecount);
    _snprintf(_Statistics[STATISTICS_PACKING_EFFICIENCY],
              sizeof(_Statistics[STATISTICS_PACKING_EFFICIENCY]) - 1, "%.1f%%",
              packingefficiency * 100.0f);
    _snprintf(_Statistics[STATISTICS_CULLING_EFFICIENCY],
              sizeof(_Statistics[STATISTICS_CULLING_EFFICIENCY]) - 1, "%.1f%%",
              cullingefficiency * 100.0f);
    _snprintf(_Statistics[STATISTICS_TEXTURE_SWAP_EFFICIENCY],
              sizeof(_Statistics[STATISTICS_TEXTURE_SWAP_EFFICIENCY]) - 1, "%.1f%%",
              swappingefficiency * 100.0f);
    _snprintf(_Statistics[STATISTICS_OVERSIZE_LIGHTMAPS],
              sizeof(_Statistics[STATISTICS_TEXTURE_SWAP_EFFICIENCY]) - 1, "%d", OversizeCount);
}
 ...
 *   07/28/00    IML : Created.                                                                *
 *=============================================================================================*/
TrianglePacker::TrianglePacker(const PackingTriangle* principaltriangleptr,
                               const DynamicVectorClass<Triangle>& adjtriangles, unsigned groupid,
                               unsigned edgeblendthickness, float samplerate,
                               W3dRGBStruct fillcolor)
{
...
void TrianglePacker::Set_Bounds()
{
    ...
    // Calculate the smallest bounding rectangle that will contain the projected principal
    // triangles.
    ...
    // Increase the bounds by the edge thickness to account for edge blending and snap to integer
    // coordinates.
    lowerbound -= edge;
    upperbound += edge;
...
TrianglePacker* TrianglePacker::Merge(const TrianglePacker& trianglepacker)
{
    ...
    // Do both triangle packers have the same projection, group ID, edge blend thickness, sample
    // rate, and fill color? If not, then they are not compatible packers and cannot be merged.
    if ((Projection == trianglepacker.Projection) && (GroupID == trianglepacker.GroupID)
        && (EdgeBlendThickness == trianglepacker.EdgeBlendThickness)
        && (SampleRate == trianglepacker.SampleRate) && (FillColor == trianglepacker.FillColor)) {
        ...
        if (mergedpackerptr != nullptr) {
            ...
            for (int a = 0; a < ADJACENT_PROJECTION_COUNT; a++) {
                for (int t = 0; t < trianglepacker.AdjacentTriangles[a].Count(); t++) {
                    ...
                    if (!foundadjacent) {
                        mergedpackerptr->AdjacentTriangles[a].Add(
                            (trianglepacker.AdjacentTriangles[a])[t]);
                    }
                }
            }
...
bool TrianglePacker::Can_Project(const Vector3& normal)
{
    ...
    const float maxprojectionangle
        = (80.0f * WWMATH_PI) / 180.0f; // Max. allowed angle between triangle and projection plane.

    Vector3 projectionnormal;
    float angle;

    projectionnormal.Set(_projectionnormals[Projection][0], _projectionnormals[Projection][1],
                         _projectionnormals[Projection][2]);
 ...
 *   06/08/00    IML : Created.                                                                *
 *=============================================================================================*/
void TrianglePacker::Rasterize(TargaLoader& targaloader, ProceduralTexture* proceduraltexture,
                               TrueColorTarga& rasterizedtarga)
{
    ...
    // Allocate enough projection triangles to cover the principal triangles and those triangles
    // that can be projected.
    projectiontriangles
        = new ProjectionTriangle[PrincipalTriangles.Count()
                                 + AdjacentTriangles[ADJACENT_PROJECTION_COMMON].Count()
    ...
    for (t = 0; t < (int)projectiontrianglecount; t++) {
        ...
        projectiontriangles[t]
            = ProjectionTriangle(points, sourceuvs, sourcetargaptr, projectionuvs);
    }
    ...
    for (int a = ADJACENT_PROJECTION_COMMON; a <= ADJACENT_PROJECTION_VALID; a++) {
        for (t = 0; t < AdjacentTriangles[a].Count(); t++) {
            ...
            if (outcode == 0) {
                ...
                projectiontriangles[projectiontrianglecount]
                    = ProjectionTriangle(points, sourceuvs, sourcetargaptr, projectionuvs);
            ...
            for (v = 0; v < sqrtbackwardsamplespertexel; v++) {
                ...
                for (u = 0; u < sqrtbackwardsamplespertexel; u++) {
                    ...
                    if (sampledt < PrincipalTriangles.Count()) {
                        if (backwardsurfaceptr->Sample(samplepoint, projectiontriangles[sampledt],
                                                       principalpriority)) {
                            goto nextsample;
                        }
                    }
                    ...
                    for (t = 0; t < PrincipalTriangles.Count(); t++) {
                        if (t != sampledt) {
                            if (backwardsurfaceptr->Sample(samplepoint, projectiontriangles[t],
                                                           principalpriority)) {
                                sampledt = t;
                                goto nextsample;
                            }
                    ...
                    if ((sampledt >= PrincipalTriangles.Count())
                        && (sampledt < (int)projectiontrianglecount)) {
                        if (backwardsurfaceptr->Sample(samplepoint, projectiontriangles[sampledt],
                                                       adjacentpriority)) {
                            goto nextsample;
                        }
    ...
    // If less than 50% of the texels were sampled in the backward rasterization prepare a forward
    // rasterization.
    ...
    if (backwardsurfaceptr->Sampling_Ratio() < 0.5f) {
        ...
        for (t = 0; t < PrincipalTriangles.Count(); t++) {
            ...
            sqrtsamplecount = ceilf(
                WWMath::Sqrt(sqrtforwardsamplespertexel * sqrtforwardsamplespertexel * pseudoarea));
            ...
            for (b = 0; b < sqrtsamplecount; b++) {
                ...
                for (a = 0; a < sqrtsamplecount; a++) {
                    ...
                    if (alpha + beta <= 1.0f) {
                        forwardsurfaceptr->Sample(alpha, beta, projectiontriangles[t],
                                                  principalpriority);
                    }
                }
            }
    ...
    // Iterate over the backward (and possibly forward) surface and translate it into the rasterized
    // targa.
    for (y = 0; y < height; y++) {
        ...
        for (x = 0; x < width; x++) {
            ...
            // Find out if this texel is within edge blend texels of a texel that has a 'principal
            // sample'. NOTE: If so then this texel can potentially be read and therefore must
            // contain valid data.
 ...
 *   06/08/00    IML : Created.                                                                *
 *=============================================================================================*/
TrianglePacker::ProjectionTriangle::ProjectionTriangle(const Vector3* points,
                                                       const Vector2* sourceuvs,
                                                       const TrueColorTarga* sourcetargaptr,
                                                       const Vector2* projectionuvs)
{
    Points[0] = points[0];
...
{
    const char* loaderrormessage = "The image file "
                                   "%s"
                                   " cannot be loaded (load error: %d).\
 Please ensure that the image file exists and is in TGA (Truevision TARGA) format.";

    const char* formaterrormessage = "The image file "
                                     "%s"
                                     " is not in a suitable TGA format.\
...
{
    const char* saveerrormessage = "Cannot save the image file "
                                   "%s"
                                   " (save error: %d).";
...
void TrueColorTarga::Blit(TrueColorTarga& destination, unsigned x, unsigned y)
{
    ...
    for (unsigned r = 0; r < maxr; r++) {
        ...
        destptr = ((unsigned char*)destination.GetImage())
            + (((destination.Width() * (y + r)) + x) * destbytespertexel);
        ...
        for (unsigned c = 0; c < maxc; c++) {
            ...
            Pack_Texel(*Unpack_Texel(sourceptr, sourcebytespertexel, unpackedtexel), destptr,
                       destbytespertexel);
 ...
 *=============================================================================================*/
#define AVERAGE(a, b) (((a) + (b)) / 2) + (((a) + (b)) % 2)
...
int TrueColorTarga::Compare(TrueColorTarga& comparison, float epsilon)
{
    ...
    // For each texel in this targa measure the delta for the corresponding texel in the comparison
    // targa.
    ...
    for (unsigned y = 0; y < Height(); y++) {
        ...
        for (unsigned x = 0; x < Width(); x++) {
            ...
            comparisonimageptr = ((unsigned char*)comparison.GetImage())
                + (((ys * comparison.Width()) + xs) * comparisonbytespertexel);
...
int TrueColorTarga::Compare(TrueColorTarga& comparison, unsigned x, unsigned y, float epsilon)
{
    ...
    if (x + Width() >= comparison.Width()) {
        return (1);
    }
    if (y + Height() >= comparison.Height()) {
        return (1);
    }
    ...
    for (unsigned r = 0; r < Height(); r++) {
        ...
        comparisonptr = ((unsigned char*)comparison.GetImage())
            + (((comparison.Width() * (y + r)) + x) * comparisonbytespertexel);
...
bool TrueColorTarga::Fill(const W3dRGBStruct& fillcolor)
{
    ...
    // Step thru all texels and pad all existing four-connected texels, and add new four-connected
    // texel coordinates to dynamic array.
    index = 0;
    allfillcolor = true;
    for (unsigned y = 0; y < Height(); y++) {
        for (unsigned x = 0; x < Width(); x++) {
    ...
    if (!allfillcolor) {
        ...
        while (fourconnectedarrays[index].Count() > 0) {
            ...
            for (int e = 0; e < fourconnectedarrays[index].Count(); e++) {
                Fill_Four_Connected((fourconnectedarrays[index])[e].X,
                                    (fourconnectedarrays[index])[e].Y, filltexel,
                                    fourconnectedarrays[index ^ 1]);
            }
 ...
 *   9/27/99    IML : Created.                                                                 *
 *=============================================================================================*/
bool TrueColorTarga::Fill_Four_Connected(unsigned x, unsigned y,
                                         const UnpackedTexelStruct& filltexel,
                                         DynamicVectorClass<PointStruct>& fourconnectedarray)
{
 ...
 *=============================================================================================*/
void TrueColorTarga::Rasterize(TrueColorTarga& destination, const W3dRGBStruct& fillcolor,
                               unsigned vertexcount, const W3dRGBStruct* vertexcolors,
                               Vector2* vertexuvs)
{
    ...
    // (a) length of longest edge equals length of targa's diagonal (hypotenuse of destination
    // triangle) (b) ratio of lengths of remaining two edges of source triangle equals ratio of
    // dimensions of targa.
    ...
    r = edges[(longestedge + 1) % edgecount].Length()
        / edges[(longestedge + 2) % edgecount].Length();
    w = WWMath::Sqrt(longestlength2 / (1.0f + r));
    h = w / r;

    // Resize the destination targa.
    destination.Reformat(MAX(mindimension, (unsigned)(w * Width())),
                         MAX(mindimension, (unsigned)(h * Height())), Pixel_Depth());

    // Set up a coordinate system to define the source triangle in terms of the destination
    // triangle.
    ...
    for (unsigned v = 0; v < destination.Height(); v++) {
        for (unsigned u = 0; u < destination.Width(); u++) {
            ...
            for (unsigned sv = 0; sv <= sqrtsamplecountminusone; sv++) {
                ...
                for (unsigned su = 0; su <= sqrtsamplecountminusone; su++) {
                    ...
                    if (!clamped) {
                        ...
                        // Sample the source.
                        sourceptr
                            = ((unsigned char*)GetImage()) + (((Width() * y) + x) * bytespertexel);
                        ...
                        r += 0.5f
                            * ((colors[1].R * fu) + (colors[2].R * (1.0f - fu))
                               + (colors[2].R * (1.0f - fv)) + (colors[0].R * fv));
                        g += 0.5f
                            * ((colors[1].G * fu) + (colors[2].G * (1.0f - fu))
                               + (colors[2].G * (1.0f - fv)) + (colors[0].G * fv));
                        b += 0.5f
                            * ((colors[1].B * fu) + (colors[2].B * (1.0f - fu))
                               + (colors[2].B * (1.0f - fv)) + (colors[0].B * fv));
                    }
                    validsamplecount++;
                }
            }
            ...
            averagetexel.Set(r * oovalidsamplecount, g * oovalidsamplecount, b * oovalidsamplecount,
                             0);
    ...
    for (n = 0; n < vertexcount; n++) {
        vertexuvs[n].Set(_vertexuvs[((edgecount - 1 - longestedge) + n) % edgecount][0],
                         _vertexuvs[((edgecount - 1 - longestedge) + n) % edgecount][1]);
 ...
 *   05/22/00    IML : Created.                                                                *
 *=============================================================================================*/
void TrueColorTarga::Pad(TrueColorTarga& destination, unsigned padwidth, unsigned padheight,
                         const W3dRGBStruct& padcolor,
                         DynamicVectorClass<PackingTriangle*>& triangleptrs)
{
    unsigned w, h;
    float oobw, oobh;
    ...
    if ((padwidth > 0) || (padheight > 0)) {
        ...
        // Round pad width and height to even no. to ensure that there is an equal amount of padding
        // on each side.
        ...
        for (int t = 0; t < triangleptrs.Count(); t++) {
            for (unsigned v = 0; v < Triangle::VERTICES_COUNT; v++) {
                ...
                // NOTE: Create a homogeneous vector (3-components) from the UV vector so that it
 ...
 *=============================================================================================*/
TrianglePacker::SampleSurface::SampleSurface(unsigned width, unsigned height,
 ...
 *   06/12/00    IML : Created.                                                                *
 *=============================================================================================*/
bool TrianglePacker::SampleSurface::Sample(const Vector2& samplepoint,
                                           const ProjectionTriangle& projectiontriangle,
                                           unsigned priority)
{
    ...
    if ((alpha > 0.0f) && ((alpha + beta) < 1.0f)) {
        ...
        uv = (alpha * projectiontriangle.SourceUVs[1]) + (beta * projectiontriangle.SourceUVs[2])
            + projectiontriangle.SourceUVs[0];
        if (!projectiontriangle.SourceTargaPtr->Get_Color(uv, color)) {
            return (false);
        }
        ...
        if (BlendTexture != nullptr) {
            ...
            p = (alpha * projectiontriangle.Points[1]) + (beta * projectiontriangle.Points[2])
                + projectiontriangle.Points[0];
        ...
        // Update priority.
        if (priority > sampleptr->Priority) {
            sampleptr->Priority = priority;
        }
 ...
 *   06/12/00    IML : Created.                                                                *
 *=============================================================================================*/
bool TrianglePacker::SampleSurface::Sample(float alpha, float beta,
                                           const ProjectionTriangle& projectiontriangle,
                                           unsigned priority)
{
    Vector2 samplepoint;
    ...
    samplepoint = (alpha * projectiontriangle.ProjectionUVs[1])
        + (beta * projectiontriangle.ProjectionUVs[2]) + projectiontriangle.ProjectionUVs[0];
    u = floorf(samplepoint.U);
    v = floorf(samplepoint.V);
 ...
 *   05/19/00    IML : Created.                                                                *
 *=============================================================================================*/
bool Page::Pack(TrueColorTarga& targa, float epsilon,
                DynamicVectorClass<PackingTriangle*>& triangleptrs)
{
    ...
    targaptr[TRANSPOSED]
        = new TrueColorTarga(targaptr[UNTRANSPOSED]->Width(), targaptr[UNTRANSPOSED]->Height(),
                             targaptr[UNTRANSPOSED]->Pixel_Depth());
    ...
    transposed = (targaptr[UNTRANSPOSED]->Width() >= targaptr[UNTRANSPOSED]->Height())
        ? UNTRANSPOSED
        : TRANSPOSED;
    ...
    if (replica) {
        ...
        if (packed) {
            ...
            // Region must be aligned on an even boundary. This will reduce texel bleeding as a
            // result of mip-mapping.
...
bool Page::Lowest_Cost_Region(TrueColorTarga& targa, unsigned& lowestcost, Region& lowestcostregion)
{
    ...
    // Find a vacant region in the page that will incur the lowest cost if it contains the targa
    // asset.
    width = targa.Width();
    height = targa.Height();
    packed = false;
...
void Page::Insert_Region(const Region& insertionregion)
{
    ...
    while (regionptr != nullptr) {
        ...
        if (regionptr->Intersects(insertionregion, subregioncount, &subregions)) {
            ...
            // NOTE: Adding to the head of the list will ensure that they will not get re-processed
            // by the current 'list walker'.
            for (int s = ((int)subregioncount) - 1; s >= 0; s--) {

                // Only add the subregion if it is not contained by another region ie. avoid
                // redundancy.
 ...
 *   9/27/99    IML : Created.                                                                 *
 *=============================================================================================*/
bool Page::Region::Intersects(const Region& targetregion, unsigned& subregioncount,
                              Region** subregions)
{
    ...
    if (targetregion.X1 < X0) {
        return (false);
    }
    if (targetregion.X0 > X1) {
        return (false);
    }
    if (targetregion.Y1 < Y0) {
        return (false);
    }
    if (targetregion.Y0 > Y1) {
        return (false);
    }
...
bool Page::Contains(const Region& testregion)
{
    ...
    // Step down the vacant list until a region is found that contains the test region or end of
    // list.
    regionptr = (Region*)VacantRegionList.First_Valid();
    while (regionptr != nullptr) {
        if ((testregion.X0 >= regionptr->X0) && (testregion.X1 <= regionptr->X1)
            && (testregion.Y0 >= regionptr->Y0) && (testregion.Y1 <= regionptr->Y1)) {
