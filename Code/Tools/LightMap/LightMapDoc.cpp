...
// Static data.
const char* LightMapDoc::_TemporarySolveFilename[TEMPORARY_SOLVE_FILENAME_COUNT]
    = { // Name of temporary file created when a solve is inserted.
          "~S0", "~S1"
      };

const char* LightMapDoc::_TemporaryOptimizeFilename
    = "~O"; // Name of temporary file created when a document is optimized.
const char* LightMapDoc::_TemporaryReorderFilename
    = "~R"; // Name of temporary file created when a document is reordered.
      ...
      SolveCount(0)
{
    ...
    // LightMapDoc implicitly references some Surrender objects. Thus Surrender must be initialized
    // here.
    srInit();
    ...
    try {
        ...
        while (w3dchunk.Open_Chunk()) {
            ...
            if (w3dchunk.Cur_Chunk_ID() == W3D_CHUNK_MESH) {
                ...
                w3dchunk.Open_Chunk();
                if (w3dchunk.Cur_Chunk_ID() != W3D_CHUNK_MESH_HEADER3) {
                    throw(
                        "A mesh does not start with a valid mesh header (W3D_CHUNK_MESH_HEADER3).");
                }
                ...
                if (header.Version != W3D_CURRENT_MESH_VERSION) {
                    throw("An unsupported mesh version has been found.");
                }
                ...
                if (chunkmeshheader3count != 1) {
                    throw("A mesh does not contain exactly one chunk W3D_CHUNK_MESH_HEADER3.");
                }
        ...
        while (w3dchunk.Open_Chunk()) {
            ...
            if (w3dchunk.Cur_Chunk_ID() == W3D_CHUNK_MESH) {
                ...
                MeshStatusStruct meshstatus(meshinfo.Header.MeshName, meshinfo.Header.NumTris,
                                            meshanomaly.MeshAnomalies);

                if (meshanomaly.MeshAnomalies != 0) {
                    anomalycount++;
                }
                trianglecount += meshstatus.TriangleCount;
                MeshStatus.Add(meshstatus);

                // A solve can be inserted if a vertex solve and/or a lightmap solve can be
                // inserted.
                CanInsertSolve |= (meshstatus.Can_Insert_Vertex_Solve()
                                   || meshstatus.Can_Insert_Multi_Pass_Solve()
                                   || meshstatus.Can_Insert_Multi_Texture_Solve());
    ...
    try {
        ...
        while (w3dchunk.Open_Chunk()) {
            ...
            case W3D_CHUNK_MATERIAL_INFO:
                if (w3dchunk.Read(&MaterialInfo[UNLIT], sizeof(MaterialInfo[UNLIT]))
                    != sizeof(MaterialInfo[UNLIT])) {
        ...
        while (w3dchunk.Open_Chunk()) {
            ...
            while (w3dchunk.Open_Chunk()) {
                ...
                case W3D_CHUNK_VERTEX_MATERIAL_INFO:
                    if (w3dchunk.Read(&vertexmaterialinfo, sizeof(vertexmaterialinfo))
                        != sizeof(vertexmaterialinfo)) {
 ...
 *=============================================================================================*/
void LightMapDoc::MeshInfoStruct::Parse_Material_Pass(ChunkLoadClass& w3dchunk, unsigned prelitmode,
    ...
    while (w3dchunk.Open_Chunk()) {
        ...
        case W3D_CHUNK_DCG: {
            ...
            for (unsigned v = 0; v < dcgschunk->Get_Size() / sizeof(*dcgs); v++) {
                if ((dcgs[v].R != UCHAR_MAX) || (dcgs[v].G != UCHAR_MAX)
                    || (dcgs[v].B != UCHAR_MAX)) {
...
void LightMapDoc::MeshInfoStruct::Parse_Prelit_Chunk(ChunkLoadClass& w3dchunk, unsigned prelitmode)
{
    ...
    while (w3dchunk.Open_Chunk()) {
        ...
        case W3D_CHUNK_MATERIAL_INFO:
            if (w3dchunk.Read(&MaterialInfo[prelitmode], sizeof(MaterialInfo[prelitmode]))
                != sizeof(MaterialInfo[prelitmode])) {
                throw("Cannot read data in W3D_CHUNK_MATERIAL_INFO.");
            }
            break;
...
LightMapDoc::MeshAnomalyStruct::MeshAnomalyStruct(const MeshInfoStruct& meshinfo)
{
    ...
    if (meshinfo.Header.Attributes & W3D_MESH_FLAG_HIDDEN) {
        MeshAnomalies |= (1 << MESH_HIDDEN);
    }

    // Has this mesh already been prelit?
    if (meshinfo.Header.Attributes & W3D_MESH_FLAG_PRELIT_MASK) {
        MeshAnomalies |= (1 << MESH_PRELIT);
    }

    // Does mesh have a hierarchy?
    if ((meshinfo.Header.Attributes & W3D_MESH_FLAG_GEOMETRY_TYPE_MASK)
        == W3D_MESH_FLAG_GEOMETRY_TYPE_SKIN) {
        MeshAnomalies |= (1 << MESH_HAS_HIERARCHY);
    }

    // Is this mesh 2-sided?
    if (meshinfo.Header.Attributes & W3D_MESH_FLAG_TWO_SIDED) {
        MeshAnomalies |= (1 << MESH_TWO_SIDED);
    }

    // Are there any vertex materials?
    if (meshinfo.MaterialInfo[PrelitModeEnum::UNLIT].VertexMaterialCount == 0) {
        MeshAnomalies |= (1 << MESH_NO_VERTEX_MATERIALS);
    }

    // Are there any shaders?
    if (meshinfo.MaterialInfo[PrelitModeEnum::UNLIT].ShaderCount == 0) {
        MeshAnomalies |= (1 << MESH_NO_SHADERS);
    }

    // Is there a deform chunk?
    if (meshinfo.DeformExists) {
        MeshAnomalies |= (1 << MESH_W3D_CHUNK_DEFORM);
    }

    // Are there too many existing material passes?
    if (meshinfo.MaterialInfo[PrelitModeEnum::UNLIT].PassCount > MeshMatDescClass::MAX_PASSES - 1) {
        MeshAnomalies |= (1 << MESH_TOO_MANY_PASSES);
    }

    // Determine if one or more passes has too many texture stages.
    if (meshinfo.IsMultiStage) {
        MeshAnomalies |= (1 << MESH_TOO_MANY_STAGES);
    }
    ...
    if (meshinfo.VertexMaterials[PrelitModeEnum::UNLIT] != NULL) {
        for (int vm = 0; vm < meshinfo.VertexMaterials[PrelitModeEnum::UNLIT]->Count(); vm++) {
            ...
            //	Test for emissive.
            if ((*meshinfo.VertexMaterials[PrelitModeEnum::UNLIT])[vm].Emissive != black) {
                MeshAnomalies |= (1 << MESH_HAS_EMISSIVE);
            }

            // Test for specular to diffuse.
            if (((*meshinfo.VertexMaterials[PrelitModeEnum::UNLIT])[vm].Attributes
                 & W3DVERTMAT_COPY_SPECULAR_TO_DIFFUSE)
                != 0) {
                MeshAnomalies |= (1 << MESH_SPECULAR_TO_DIFFUSE);
            }
        }
    }
    ...
    if (meshinfo.ShaderChunk[PrelitModeEnum::UNLIT] != NULL) {
        ...
        for (unsigned s = 0;
             s < meshinfo.ShaderChunk[PrelitModeEnum::UNLIT]->Get_Size() / sizeof(W3dShaderStruct);
             s++) {

            if (shaderptr->PriGradient != W3DSHADER_PRIGRADIENT_MODULATE) {
                MeshAnomalies |= (1 << MESH_NOT_MODULATED_PRIMARY);
            }
            if (shaderptr->SecGradient != W3DSHADER_SECGRADIENT_DISABLE) {
                MeshAnomalies |= (1 << MESH_HAS_SPECULAR);
            }
            if (shaderptr->AlphaTest != W3DSHADER_ALPHATEST_DISABLE) {
                MeshAnomalies |= (1 << MESH_ALPHATEST);
            }
            shaderptr++;
        }
    }
    ...
    if (meshinfo.SCGsExist) {
        MeshAnomalies |= (1 << MESH_HAS_SCG);
    }

    // Determine if there is a DIG chunk.
    if (meshinfo.DIGsExist) {
        MeshAnomalies |= (1 << MESH_HAS_DIG);
    }
    ...
    if ((meshinfo.ShaderChunk[PrelitModeEnum::UNLIT] != NULL)
        ...
        for (unsigned s = 0;
            ...
            shaderptr = ((W3dShaderStruct*)meshinfo.ShaderChunk[PrelitModeEnum::UNLIT]->Get_Data())
                + shaderid;

            if (shaderptr->DestBlend != W3DSHADER_DESTBLENDFUNC_ZERO) {
                MeshAnomalies |= (1 << MESH_PASS_ZERO_DESTBLEND);
            }
        }
    }

    // Determine if there is vertex coloring.
    if (meshinfo.VertexColorsExist) {
        MeshAnomalies |= (1 << MESH_VERTEX_COLORING);
    }
}
 ...
 *=============================================================================================*/
LightMapDoc::SplitVertexInfoStruct::SplitVertexInfoStruct(const MeshInfoStruct& meshinfo,
    ...
    for (unsigned f = 0; f < facecount; f++) {
        ...
        for (v = 0; v < verticesperface; v++) {
            ...
            if (UVPtrTable[vertexindex] == NULL) {
                ...
                // Does this vertex reference the same UV coordinate as the vertex that has already
                // been indexed?
 ...
 *=============================================================================================*/
const char* LightMapDoc::Mesh_Anomalies_String(unsigned meshindex, bool verbose,
                                               StringBuilder& string)
{
    static char* _meshtext[2][LightMapDoc::MESH_FLAG_COUNT] = {
        { "Hidden", "Prelit", "No vertex materials", "No shaders", "Hierarchy",
          "Not modulated primary", "Specular", "Specular->diffuse", "Deform chunk", "2-sided",
          "Emissive", "SCG", "DIG", "Too many passes", "Alpha-test", "Not opaque",
          "Too many stages", "Vertex colors" },
        { ": mesh is flagged as hidden. This mesh will not receive a solve. Is this what you "
          "intended?",
          ": mesh already contains a vertex and/or lightmap solve. Please ensure that you have "
          "selected the correct W3D model file.",
          ": mesh does not contain any vertex materials. A valid mesh must have at least one "
          "vertex material.",
          ": mesh does not contain any shaders. A valid mesh must have at least one shader.",
          ": mesh contains a hierarchy and cannot be solved. Please export this model from MAX "
          "with the 'geometry only' option.",
...
gradient disabled.",
 ...
 unchecked.",
          ": unsupported chunk (DEFORM chunk).", ": mesh is two-sided. A solve cannot be inserted.",
...
cannot be inserted.",
          ": mesh uses alpha testing in one or more passes. A multi-pass lightmap solve cannot be "
          "inserted.",
          ": mesh has destination blending in the zeroth pass. A multi-pass lightmap solve cannot "
          "be inserted.",
          ": mesh has a material pass with more than one texture stage. A multi-texture lightmap "
          "solve cannot be inserted.",
    ...
    if (verbose) {
        ...
        for (unsigned meshflagindex = 0; meshflagindex < MESH_FLAG_COUNT; meshflagindex++) {
            if (meshanomalies & (1 << meshflagindex)) {
                string.Concatenate("%s%s\r\n\r\n", _meshtext[0][meshflagindex],
                                   _meshtext[1][meshflagindex]);
            }
        }
        if (string.String()[0] == '\0') {
            string.Copy("No mesh anomalies.\r\n\r\n");
        }
    }
    else {
        string.Copy("");
        for (unsigned meshflagindex = 0; meshflagindex < LightMapDoc::MESH_FLAG_COUNT;
             meshflagindex++) {
            if (meshanomalies & (1 << meshflagindex)) {
                if (string.String()[0] != '\0') {
                    string.Concatenate(", ");
 ...
 *   01/02/00    IML : Created.                                                                *
 *=============================================================================================*/
const char* LightMapDoc::Solve_Anomalies_String(unsigned meshindex, bool verbose,
                                                StringBuilder& string)
{
    static char* _solvetext[2][SolveStatistics::SOLVE_STATISTIC_COUNT] = {
        { "Vertices not found", "No color data", "Faces not found", "No lightmaps", "Not smooth",
          "Ambiguous", "Degenerate", "[U]", "[E]", "Fill color", "Placeholder used" },
 ...
 for the missing lightmap." }
    ...
    if (verbose) {
        ...
        if (Solve_Inserted()) {
            ...
            if (string.String()[0] == '\0') {
                string.Copy("No solve anomalies.\r\n\r\n");
            }
        }
    }
    else {
...
{
    static char* _insertedtext[2][2] = { { "Cannot insert", "" }, { "Cannot insert", "Inserted" } };
...

        {
            ...
            while (w3dchunk.Open_Chunk()) {
                ...
                if (w3dchunk.Cur_Chunk_ID() == W3D_CHUNK_MESH) {
                    ...
                    mesh.Chunk = chunk;
                    mesh.Position.Set(meshinfo.Header.SphCenter.X, meshinfo.Header.SphCenter.Y,
                                      meshinfo.Header.SphCenter.Z);
                    meshes.Add(mesh);
                }
                w3dchunk.Close_Chunk();
            }
                ...
                if (w3dchunk.Cur_Chunk_ID() == W3D_CHUNK_MESH) {
                    if (!outputmeshchunks) {
                        for (int m = 0; m < meshes.Count(); m++) {
                            ...
                            reorderchunk.Write(meshes[m].Chunk->Get_Data(),
                                               meshes[m].Chunk->Get_Size());
 ...
 *   07/06/01    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Optimize_Prelit_Vertex_Material_Pass(ChunkLoadClass& w3dchunk,
        ...
        if (IsModified() || stricmp(W3dFile->File_Name(), pathname) != 0) {
            ...
            if (filenamesize > W3D_NAME_LEN) {
                ...
                const char* controlstring
                    = "File name is too long. Please use %d characters or less.";
            ...
            if (!savefile.Open(FileClass::WRITE)) {
                throw("Cannot create file with this file name.");
            }

            {
                ...
                CRect prect(srect.TopLeft().x + trect.Width(), srect.TopLeft().y,
                            srect.BottomRight().x, srect.BottomRight().y);

                success
                    = progressbar.Create(WS_CHILD | WS_VISIBLE | PBS_SMOOTH, prect, statusptr, 0);
                ASSERT(success);
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Rename_Mesh(ChunkLoadClass& loadchunk, ChunkSaveClass& savechunk,
                              const char* filename, const char* lightmapdirectory)
{
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Rename_Mesh_Header(ChunkLoadClass& loadchunk, ChunkSaveClass& savechunk,
                                     const char* filename)
{
 ...
 *=============================================================================================*/
void LightMapDoc::Rename_Prelit_Chunks(ChunkLoadClass& loadchunk, ChunkSaveClass& savechunk,
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Rename_Lightmaps(ChunkLoadClass& loadchunk, ChunkSaveClass& savechunk,
                                   const char* lightmapdirectory)
{
    savechunk.Begin_Chunk(loadchunk.Cur_Chunk_ID());
    while (loadchunk.Open_Chunk()) {
        ...
        while (loadchunk.Open_Chunk()) {
            ...
            case W3D_CHUNK_TEXTURE_NAME: {
                ...
                if ((strlen(loaddrivename) == 0)
                    && (stricmp(loaddirectoryname, LightscapeSolve::Asset_Directory()) == 0)) {
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Rename_Collection(ChunkLoadClass& loadchunk, ChunkSaveClass& savechunk,
                                    const char* filename)
{
    unsigned filenamesize = strlen(filename) + 1;
    ...
    while (loadchunk.Open_Chunk()) {
        ...
        case W3D_CHUNK_COLLECTION_HEADER: {
            ...
            if (loadchunk.Read(&header, loadchunk.Cur_Chunk_Length())
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Rename_HLOD(ChunkLoadClass& loadchunk, ChunkSaveClass& savechunk,
                              const char* filename)
{
    unsigned filenamesize = strlen(filename) + 1;
        ...
        case W3D_CHUNK_HLOD_LOD_ARRAY:
            ...
            while (loadchunk.Open_Chunk()) {
                ...
                case W3D_CHUNK_HLOD_SUB_OBJECT: {
                    ...
                    if (loadchunk.Read(&subobject, loadchunk.Cur_Chunk_Length())
                        != loadchunk.Cur_Chunk_Length()) {
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Rename_Dazzle(ChunkLoadClass& loadchunk, ChunkSaveClass& savechunk,
                                const char* filename)
{
    unsigned filenamesize = strlen(filename) + 1;
...
void LightMapDoc::Save_Lights(const char* pathname)
{
    ...
    for (unsigned s = 0; s < MAX_SOLVE_COUNT; s++) {
        ...
        for (int l = 0; l < Lights[s].Count(); l++) {
            ...
            const W3dLightTransformStruct transform
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Insert_Solve(const char* solvedirectoryname, const char* solvefilenamelist,
                               const char* inclusionstring, bool invertselection, bool blendnoise)
{
    ...
    try {
        ...
        LightscapeSolve solve(solvedirectoryname, solvefilenamelist, statusptr, buildingtext,
                              blendnoise);
        ...
        CRect prect(srect.TopLeft().x + trect.Width(), srect.TopLeft().y, srect.BottomRight().x,
                    srect.BottomRight().y);
        ...
        while (w3dchunk.Open_Chunk()) {
            ...
            if (w3dchunk.Cur_Chunk_ID() == W3D_CHUNK_MESH) {
                ...
                if (includemesh
                    && (MeshStatus[meshindex].Can_Insert_Vertex_Solve()
                        || MeshStatus[meshindex].Can_Insert_Multi_Pass_Solve()
                        || MeshStatus[meshindex].Can_Insert_Multi_Texture_Solve())) {
                    ...
                    RAMFileClass meshfile(meshfilebuffer, w3dchunk.Cur_Chunk_Length());
                    MeshInfoStruct meshinfo(meshfile);
                    LightscapeMeshSolve meshsolve(solve, *meshinfo.TriangleChunk,
                                                  *meshinfo.VertexChunk);
                    SplitVertexInfoStruct splitvertexinfo(meshinfo, meshsolve);
                    unsigned long prelitflags;

                    // Update the status for this mesh.
                    // Analyze the mesh status to see if it is compatible with each solve type
                    // (vertex, lightmap or both).
                    ...
                    while (w3dchunk.Open_Chunk()) {
                        ...
                        case W3D_CHUNK_MESH_HEADER3:
                            Translate_Mesh_Header3(w3dchunk, prelitflags, solvechunk,
                                                   splitvertexinfo);
                            break;
                        ...
                        case W3D_CHUNK_MATERIAL_INFO:
                            if (prelitflags & W3D_MESH_FLAG_PRELIT_UNLIT) {
                                Insert_Solve(UNLIT, meshfile, UNLIT, solvechunk, meshinfo,
                                             meshsolve, splitvertexinfo);
                            }
                            if (prelitflags & W3D_MESH_FLAG_PRELIT_VERTEX) {
                                Insert_Solve(UNLIT, meshfile, VERTEX, solvechunk, meshinfo,
                                             meshsolve, splitvertexinfo);
                                insertedflags |= W3D_MESH_FLAG_PRELIT_VERTEX;
                            }
                            if (prelitflags & W3D_MESH_FLAG_PRELIT_LIGHTMAP_MULTI_PASS) {
                                Insert_Solve(UNLIT, meshfile, MULTI_PASS, solvechunk, meshinfo,
                                             meshsolve, splitvertexinfo);
                                insertedflags |= W3D_MESH_FLAG_PRELIT_LIGHTMAP_MULTI_PASS;
                            }
                            if (prelitflags & W3D_MESH_FLAG_PRELIT_LIGHTMAP_MULTI_TEXTURE) {
                                Insert_Solve(UNLIT, meshfile, MULTI_TEXTURE, solvechunk, meshinfo,
                                             meshsolve, splitvertexinfo);
                        ...
                        case W3D_CHUNK_PRELIT_VERTEX:
                            if (prelitflags & W3D_MESH_FLAG_PRELIT_VERTEX) {
                                Insert_Solve(VERTEX, w3dchunk, VERTEX, solvechunk, meshinfo,
                                             meshsolve, splitvertexinfo);
                                insertedflags |= W3D_MESH_FLAG_PRELIT_VERTEX;
                            }
                            else {
                        ...
                        case W3D_CHUNK_PRELIT_LIGHTMAP_MULTI_PASS:
                            if (prelitflags & W3D_MESH_FLAG_PRELIT_LIGHTMAP_MULTI_PASS) {
                                Insert_Solve(MULTI_PASS, w3dchunk, MULTI_PASS, solvechunk, meshinfo,
                                             meshsolve, splitvertexinfo);
                                insertedflags |= W3D_MESH_FLAG_PRELIT_LIGHTMAP_MULTI_PASS;
                            }
                            else {
                        ...
                        case W3D_CHUNK_PRELIT_LIGHTMAP_MULTI_TEXTURE:
                            if (prelitflags & W3D_MESH_FLAG_PRELIT_LIGHTMAP_MULTI_TEXTURE) {
                                Insert_Solve(MULTI_TEXTURE, w3dchunk, MULTI_TEXTURE, solvechunk,
                                             meshinfo, meshsolve, splitvertexinfo);
                                insertedflags |= W3D_MESH_FLAG_PRELIT_LIGHTMAP_MULTI_TEXTURE;
                            }
                            else {
                                Copy_Chunk(w3dchunk, solvechunk);
        ...
        for (unsigned m = 0; m < MeshCount; m++) {
            if (!(MeshStatus[m].SolveStatistics == 0)) {
                anomalycount++;
            }
        }
 ...
 anomalies by left-clicking on the mesh name in the list.";
        ...
        //			This will ensure that the lights will not be deleted when the solve
        //goes out of scope.
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Translate_Mesh_Header3(ChunkLoadClass& w3dchunk, unsigned long prelitflags,
 ...
 *=============================================================================================*/
void LightMapDoc::Translate_Vertices(ChunkLoadClass& w3dchunk, ChunkSaveClass& solvechunk,
                                     const SplitVertexInfoStruct& splitvertexinfo)
{
 ...
 *   04/06/01    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Translate_Vertex_Normals(ChunkLoadClass& w3dchunk, ChunkSaveClass& solvechunk,
                                           const SplitVertexInfoStruct& splitvertexinfo)
{
 ...
 *   04/06/01    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Translate_Vertex_Influences(ChunkLoadClass& w3dchunk, ChunkSaveClass& solvechunk,
                                              const SplitVertexInfoStruct& splitvertexinfo)
{
 ...
 *   04/06/01    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Translate_Triangles(ChunkLoadClass& w3dchunk, ChunkSaveClass& solvechunk,
                                      const SplitVertexInfoStruct& splitvertexinfo)
{
    const unsigned verticesperface = 3; // No. of vertices in a triangle.
 ...
 *   04/06/01    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Translate_Vertex_Shade_Indices(ChunkLoadClass& w3dchunk,
                                                 ChunkSaveClass& solvechunk,
                                                 const SplitVertexInfoStruct& splitvertexinfo)
{
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Insert_Solve(PrelitModeEnum inputmode, FileClass& meshfile,
                               ...
                               const SplitVertexInfoStruct& splitvertexinfo)
{
    ...
    while (w3dchunk.Open_Chunk()) {
        ...
        case W3D_CHUNK_MATERIAL_INFO:
            Translate_Material_Info(inputmode, w3dchunk, outputmode, solvechunk, meshinfo,
                                    meshsolve);
            break;
        ...
        case W3D_CHUNK_MATERIAL_PASS:
            ...
            Translate_Material_Pass(inputmode, w3dchunk, materialpass, outputmode, solvechunk,
                                    meshinfo, meshsolve, splitvertexinfo);
            materialpass++;
            break;
    ...
    if ((materialpass == meshinfo.MaterialInfo[inputmode].PassCount)
        && (outputmode == MULTI_PASS)) {
        Add_Lightmap_Material_Pass(inputmode, materialpass - 1, solvechunk, meshinfo, meshsolve,
                                   splitvertexinfo);
    }
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Insert_Solve(PrelitModeEnum inputmode, ChunkLoadClass& w3dchunk,
                               PrelitModeEnum outputmode, ChunkSaveClass& solvechunk,
                               const MeshInfoStruct& meshinfo, const LightscapeMeshSolve& meshsolve,
                               const SplitVertexInfoStruct& splitvertexinfo)
{
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Translate_Material_Info(PrelitModeEnum inputmode, ChunkLoadClass& w3dchunk,
                                          PrelitModeEnum outputmode, ChunkSaveClass& solvechunk,
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Translate_Vertex_Materials(PrelitModeEnum inputmode, ChunkLoadClass& w3dchunk,
                                             PrelitModeEnum outputmode, ChunkSaveClass& solvechunk,
                                             const MeshInfoStruct& meshinfo)
{
    solvechunk.Begin_Chunk(w3dchunk.Cur_Chunk_ID());
    ...
    if (inputmode == UNLIT) {
            ...
            while (w3dchunk.Open_Chunk()) {
                ...
                case W3D_CHUNK_VERTEX_MAPPER_ARGS0:
                    ...
                    case MULTI_TEXTURE: {
                        ...
                        // Any args in stage 0 must be moved to stage 1 because the base texture is
                        // in stage 1.
                        argbuffer = new char[w3dchunk.Cur_Chunk_Length()];
                        if (w3dchunk.Read(argbuffer, w3dchunk.Cur_Chunk_Length())
                            != w3dchunk.Cur_Chunk_Length()) {
                ...
                case W3D_CHUNK_VERTEX_MATERIAL_INFO: {
                    ...
                    case MULTI_TEXTURE: {
                        ...
                        stage0attributes
                            = vertexmaterialinfo.Attributes & W3DVERTMAT_STAGE0_MAPPING_MASK;
                        if (W3DVERTMAT_STAGE0_MAPPING_MASK > W3DVERTMAT_STAGE1_MAPPING_MASK) {
                            stage1attributes = stage0attributes
                                / (W3DVERTMAT_STAGE0_MAPPING_MASK / W3DVERTMAT_STAGE1_MAPPING_MASK);
                        }
                        else {
                            stage1attributes = stage0attributes
                                * (W3DVERTMAT_STAGE1_MAPPING_MASK / W3DVERTMAT_STAGE0_MAPPING_MASK);
                        }

                        // Clear the stage 0 and stage 1 attributes.
                        // NOTE: Stage 1 attributes should be unused but clear them anyway as a
                        // precaution.
                        vertexmaterialinfo.Attributes
                            &= ~(W3DVERTMAT_STAGE0_MAPPING_MASK | W3DVERTMAT_STAGE1_MAPPING_MASK);
        ...
        while (w3dchunk.Open_Chunk()) {
            ...
            solvechunk.Write(vertexmaterialchunkptr->Get_Data(),
                             vertexmaterialchunkptr->Get_Size());
            solvechunk.End_Chunk();
        ...
        for (int vm = 0; vm < vertexmaterialchunks.Count(); vm++) {
            ...
            solvechunk.Write(vertexmaterialchunks[vm]->Get_Data(),
                             vertexmaterialchunks[vm]->Get_Size());
            solvechunk.End_Chunk();
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Translate_Shaders(PrelitModeEnum inputmode, ChunkLoadClass& w3dchunk,
                                    PrelitModeEnum outputmode, ChunkSaveClass& solvechunk,
                                    const MeshInfoStruct& meshinfo)
{
    unsigned s;
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Translate_Textures(ChunkLoadClass& w3dchunk, PrelitModeEnum outputmode,
                                     ChunkSaveClass& solvechunk,
                                     const LightscapeMeshSolve& meshsolve)
{
    switch (outputmode) {
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Add_Lightmap_Textures(ChunkSaveClass& solvechunk,
                                        const LightscapeMeshSolve& meshsolve)
{
    ...
    for (unsigned l = 0; l < meshsolve.Lightmap_Count(); l++) {
        ...
        solvechunk.Write(meshsolve.Lightmap_Pathname(l),
                         strlen(meshsolve.Lightmap_Pathname(l)) + 1);
        solvechunk.End_Chunk();
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Translate_Material_Pass(PrelitModeEnum inputmode, ChunkLoadClass& w3dchunk,
                                          ...
                                          const SplitVertexInfoStruct& splitvertexinfo)
{
    ...
    while (w3dchunk.Open_Chunk()) {
        ...
        case W3D_CHUNK_DIG:
            Translate_DIGs(inputmode, w3dchunk, materialpass, solvechunk, meshinfo, meshsolve,
                           splitvertexinfo);
            break;

        case W3D_CHUNK_VERTEX_MATERIAL_IDS:
            Translate_Vertex_Material_IDs(inputmode, w3dchunk, solvechunk, meshinfo,
                                          splitvertexinfo);
            break;
        ...
        case W3D_CHUNK_TEXTURE_STAGE:
            if (inputmode == UNLIT) {
                ...
                if (((outputmode == MULTI_PASS)
                     && (materialpass == meshinfo.MaterialInfo[inputmode].PassCount - 1))
                    || ((outputmode == MULTI_TEXTURE) && (stage == 0))) {

                    Translate_Lightmap_Stage(inputmode, w3dchunk, solvechunk, meshinfo, meshsolve,
                                             splitvertexinfo);
                }
                else {
 ...
 *=============================================================================================*/
void LightMapDoc::Translate_DCGs(ChunkLoadClass& w3dchunk, ChunkSaveClass& solvechunk,
                                 const SplitVertexInfoStruct& splitvertexinfo)
{
    ChunkClass* dcgschunk;
    W3dRGBAStruct* dcgs;
 ...
 *   01/26/00    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Translate_DIGs(PrelitModeEnum inputmode, ChunkLoadClass& w3dchunk,
                                 unsigned materialpass, ChunkSaveClass& solvechunk,
                                 const MeshInfoStruct& meshinfo,
                                 const LightscapeMeshSolve& meshsolve,
                                 const SplitVertexInfoStruct& splitvertexinfo)
{
 ...
 *   01/26/00    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Add_DIGs(PrelitModeEnum inputmode, unsigned materialpass,
 ...
 *   01/26/00    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Translate_Vertex_Material_IDs(PrelitModeEnum inputmode, ChunkLoadClass& w3dchunk,
                                                ChunkSaveClass& solvechunk,
                                                const MeshInfoStruct& meshinfo,
                                                const SplitVertexInfoStruct& splitvertexinfo)
{
    ...
    if (inputmode != UNLIT) {
        ...
        if (vertexmaterialidcount == 1) {
            expandedid
                = (*vertexmaterialids) + meshinfo.MaterialInfo[inputmode].VertexMaterialCount;
            solvechunk.Write(&expandedid, sizeof(uint32));
        }
        else {
            for (unsigned v = 0; v < splitvertexinfo.Vertex_Count(); v++) {
                expandedid = *(vertexmaterialids + splitvertexinfo.Remap(v))
                    + meshinfo.MaterialInfo[inputmode].VertexMaterialCount;
                solvechunk.Write(&expandedid, sizeof(uint32));
            }
        }
        solvechunk.End_Chunk();
 ...
 *   01/26/00    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Translate_Texture_Stage(ChunkLoadClass& w3dchunk, ChunkSaveClass& solvechunk,
                                          const SplitVertexInfoStruct& splitvertexinfo)
{
    solvechunk.Begin_Chunk(w3dchunk.Cur_Chunk_ID());
 ...
 *   01/26/00    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Translate_Stage_Texcoords(ChunkLoadClass& w3dchunk, ChunkSaveClass& solvechunk,
                                            const SplitVertexInfoStruct& splitvertexinfo)
{
 ...
 *   01/26/00    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Add_Lightmap_Material_Pass(PrelitModeEnum inputmode, unsigned materialpass,
                                             ChunkSaveClass& solvechunk,
                                             const MeshInfoStruct& meshinfo,
                                             const LightscapeMeshSolve& meshsolve,
                                             const SplitVertexInfoStruct& splitvertexinfo)
{
    ...
    if (meshinfo.Lightmap_Shader_Count() == 1) {
        ...
        for (unsigned s = 0;
             ...
             s++) {
            shaderid = meshinfo.MaterialInfo[inputmode].ShaderCount
                + *((uint32*)meshinfo.ShaderIdChunk[inputmode][materialpass]->Get_Data() + s);
            solvechunk.Write(&shaderid, sizeof(shaderid));
        }
    }
    solvechunk.End_Chunk();
 ...
 *=============================================================================================*/
void LightMapDoc::Add_Lightmap_Stage(PrelitModeEnum inputmode, ChunkSaveClass& solvechunk,
                                     const MeshInfoStruct& meshinfo,
                                     const LightscapeMeshSolve& meshsolve,
                                     const SplitVertexInfoStruct& splitvertexinfo)
{
 ...
 *   08/25/00    IML : Created.                                                                *
 *=============================================================================================*/
void LightMapDoc::Translate_Lightmap_Stage(PrelitModeEnum inputmode, ChunkLoadClass& w3dchunk,
                                           ChunkSaveClass& solvechunk,
                                           const MeshInfoStruct& meshinfo,
                                           const LightscapeMeshSolve& meshsolve,
                                           const SplitVertexInfoStruct& splitvertexinfo)
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
void LightMapDoc::Add_Lightmap_Stage_Chunks(PrelitModeEnum inputmode, ChunkSaveClass& solvechunk,
                                            const MeshInfoStruct& meshinfo,
                                            const LightscapeMeshSolve& meshsolve,
                                            const SplitVertexInfoStruct& splitvertexinfo)
{
    ...
    // If all of the texture indexes are the same then just write a single ID (this will be
    // interpeted as an ID that applies to all faces in the mesh).
    ...
    for (f = 1; f < facecount; f++) {
        if ((meshinfo.MaterialInfo[inputmode].TextureCount + meshsolve.Lightmap_Index(f))
            != textureindex) {
    ...
    if (writesingleindex) {
        ...
        for (f = 0; f < facecount; f++) {
            textureindex
                = meshinfo.MaterialInfo[inputmode].TextureCount + meshsolve.Lightmap_Index(f);
            solvechunk.Write(&textureindex, sizeof(textureindex));
        }
    }
    solvechunk.End_Chunk();
 ...
 *   6/1/99    IML : Created.                                                                  *
 *=============================================================================================*/
ChunkClass::ChunkClass(ChunkLoadClass& loadchunk)
    : ChunkHeader(loadchunk.Cur_Chunk_ID(), loadchunk.Cur_Chunk_Length())
{
