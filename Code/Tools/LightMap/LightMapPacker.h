...
public:
    ...
    void Scale(TrueColorTarga& destination)
    {
        Scale(destination, destination.Width(), destination.Height());
    }
    void Scale(float filtererror) { Scale(*this, filtererror); }
    void Transpose() { Transpose(*this); }

    void Pad(unsigned padwidth, unsigned padheight, const W3dRGBStruct& padcolor,
    ...
    void Rasterize(TrueColorTarga& destination, const W3dRGBStruct& fillcolor, unsigned vertexcount,
                   const W3dRGBStruct* vertexcolors, Vector2* uvs);
...
protected:
    ...
    struct UnpackedTexelStruct
    {
        ...
        // Inequality operator.
        bool operator!=(const UnpackedTexelStruct& t) { return (!(*this == t)); }
    ...
    struct PointStruct
    {
        ...
        // Equality operator.
        bool operator==(const PointStruct& p) { return ((X == p.X) && (Y == p.Y)); }

        // Inequality operator.
        bool operator!=(const PointStruct& p) { return (!(*this == p)); }
...
private:
    ...
    UnpackedTexelStruct* Unpack_Texel(unsigned char* packedtexelptr, unsigned packedbytespertexel,
                                      UnpackedTexelStruct& unpackedtexel) const;
    void Pack_Texel(const UnpackedTexelStruct& unpackedtexel, unsigned char* packedtexelptr,
                    unsigned packedbytespertexel);
    ...
    bool Fill_Four_Connected(unsigned x, unsigned y, const UnpackedTexelStruct& filltexel,
                             DynamicVectorClass<PointStruct>& fourconnectedarray);
    void Pad(TrueColorTarga& destination, unsigned verticalpadthickness,
             unsigned horizontalpadthickness, const W3dRGBStruct& padcolor,
             DynamicVectorClass<PackingTriangle*>& triangleptrs);
};
...
public:
    ...
    bool Pack(TrueColorTarga& targa, float epsilon,
              DynamicVectorClass<PackingTriangle*>& triangleptrs);
...
private:
    ...
    public:
        // Public functions.
        Region() { }
        Region(const Region& region) { Set(region.X0, region.Y0, region.X1, region.Y1); }
...
private:
    struct TargaCacheStruct
    {
        ...
        // Equality operator.
        bool operator==(const TargaCacheStruct& t) { return ((ID == t.ID) && (Ptr == t.Ptr)); }

        // Inequality operator.
        bool operator!=(const TargaCacheStruct& t) { return (!(*this == t)); }
    ...
    unsigned Index(unsigned id) { return (id % ((unsigned)Cache.Length())); }

    // Private data.
    VectorClass<TargaCacheStruct> Cache; // A simple array of targacache objects.
    DynamicVectorClass<TrueColorTarga*>
        CleanupList; // List of targas that have been displaced from the cache but cannot
...
public:
    TrianglePacker(const PackingTriangle* principaltriangleptr,
                   const DynamicVectorClass<Triangle>& adjtriangles, unsigned groupid,
                   unsigned edgeblendthickness, float samplerate, W3dRGBStruct fillcolor);
    ...
    void Rasterize(TargaLoader& targaloader, ProceduralTexture* proceduraltexture,
...
private:
    ...
    {
        ADJACENT_PROJECTION_COMMON, // Adjacent triangles that use the same projection as this
                                    // object.
        ADJACENT_PROJECTION_VALID, // Adjacent triangles that map to a different projection but have
                                   // non-zero area if object's projection is used.
        ADJACENT_PROJECTION_NONE, // Adjacent triangles that cannot be projected using object's
                                  // projection.
    ...
    public:
        // Public functions.
        ProjectionTriangle() { }
        ProjectionTriangle(const Vector3* points, const Vector2* sourceuvs,
                           const TrueColorTarga* sourcetargaptr, const Vector2* projectionuvs);
    ...
    public:
        ...
        bool Sample(const Vector2& samplepoint, const ProjectionTriangle& projectiontriangle,
                    unsigned priority);
        bool Sample(float alpha, float beta, const ProjectionTriangle& projectiontriangle,
                    unsigned priority);
        ...
        bool Get_Color(unsigned x, unsigned y, W3dRGBStruct& color)
        {
            ...
            if (sampleptr->Count > 0) {
                ...
                color.Set((uint8)MIN(r, UCHAR_MAX), (uint8)MIN(g, UCHAR_MAX),
                          (uint8)MIN(b, UCHAR_MAX));
                return (true);
            }
            else {
                return (false);
    ...
    //		void				Get_Projection_Triangle (const Triangle &triangle,
    //ProjectionTriangleStruct &projectiontriangle);
    bool Sample(const Vector2& samplepoint, const ProjectionTriangle& projectiontriangle,
                W3dRGBStruct& color);
    ...
    float SampleRate; // No. of texels per unit length.
    W3dRGBStruct FillColor; // Color to replace during fill operations.
    DynamicVectorClass<PackingTriangle*>
        PrincipalTriangles; // Set of triangles that will be rasterized in their entirety (ie. not
                            // edge blended).
    DynamicVectorClass<Triangle>
        AdjacentTriangles[ADJACENT_PROJECTION_COUNT]; // Set of triangles that will be edge-blended.
    Vector2
        LowerBound; // Lower bound of all principal triangle points projected onto the texel plane.
    Vector2
        UpperBound; // Upper bound of all principal triangle points projected onto the texel plane.
};
...
public:
    ...
    static const char* Get_Statistic(unsigned index)
    {
...
private:
    ...
    float SampleRate; // Sample rate used by rasterizers (such as Edge_Blend()) in texels per unit
                      // length.
 ...
 *   9/27/99    IML : Created.                                                                 *
 *=============================================================================================*/
inline TrueColorTarga::UnpackedTexelStruct*
TrueColorTarga::Unpack_Texel(unsigned char* packedtexelptr, unsigned packedbytespertexel,
                             UnpackedTexelStruct& unpackedtexel) const
{
 ...
 *   9/27/99    IML : Created.                                                                 *
 *=============================================================================================*/
inline void TrueColorTarga::Pack_Texel(const UnpackedTexelStruct& unpackedtexel,
                                       unsigned char* packedtexelptr, unsigned packedbytespertexel)
{
    switch (packedbytespertexel) {
