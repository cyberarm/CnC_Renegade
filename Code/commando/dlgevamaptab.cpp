    ...
    if (map_ctrl != nullptr) {
        ...
        for (int cell_x = 0; cell_x < MapMgrClass::CLOUD_WIDTH; cell_x++) {
            for (int cell_y = 0; cell_y < MapMgrClass::CLOUD_HEIGHT; cell_y++) {
                map_ctrl->Set_Cloud_Cell(cell_x, cell_y,
                                         MapMgrClass::Is_Cell_Visible(cell_x, cell_y));
            }
        }
        ...
        for (int index = 0; index < count; index++) {
            ...
            //
            if (objective != nullptr && objective->Status == ObjectiveManager::STATUS_IS_PENDING) {
                //
                //	Now add a marker that represents this objective
                //
                int color = VRGB_TO_INT32(objective->Type_To_Color());
                int item_index = map_ctrl->Add_Marker(L"", objective->Position,
                                                      RectClass(32, 0, 64, 32), color);
...
//
////////////////////////////////////////////////////////////////
void EvaMapTabClass::On_MapCtrl_Marker_Hilighted(MapCtrlClass* map_ctrl, int ctrl_id,
                                                 int marker_index)
{
    ListCtrlClass* list_ctrl = (ListCtrlClass*)Get_Dlg_Item(IDC_OBJECT_DESC_LIST);
    ...
    if (marker_index == 0) {
        ...
        if (objective != nullptr) {
            ...
            TDBObjClass* translate_obj
                = TranslateDBClass::Find_Object(objective->LongDescriptionID);
...
//
////////////////////////////////////////////////////////////////
void EvaMapTabClass::On_MapCtrl_Pos_Clicked(MapCtrlClass* map_ctrl, int ctrl_id,
                                            const Vector3& position)
{
    ...
    LineSegClass ray(Vector3(position.X, position.Y, start_z),
                     Vector3(position.X, position.Y, end_z));
