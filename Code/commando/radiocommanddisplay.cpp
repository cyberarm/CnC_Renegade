...
void RadioCommandDisplayClass::Initialize(void)
{
    ...
    //
    TextWindow->Set_Backdrop(
        BACKDROP_TGA, settings->Get_Objectives_Screen_Rect(),
    ...
    TextWindow->Add_Column(TRANSLATE(IDS_MENU_RADIO_KEY_MESSAGE), 0.7F,
                           Vector3(0.75F, 1.0F, 0.75F));
...
void RadioCommandDisplayClass::Check_Keys(void)
{
    ...
    bool ignore_lctrl = (COMBAT_STAR && COMBAT_STAR->As_SoldierGameObj()
                         && COMBAT_STAR->As_SoldierGameObj()->Get_Vehicle()
    ...
    if (ignore_lctrl) {
        ...
        is_key_down[DISPLAY_CMDS_01]
            = Input::Is_Button_Down(DIK_LCONTROL) || Input::Is_Button_Down(DIK_RCONTROL);
        is_key_down[DISPLAY_CMDS_02]
            = Input::Is_Button_Down(DIK_LMENU) || Input::Is_Button_Down(DIK_RMENU);
    }
