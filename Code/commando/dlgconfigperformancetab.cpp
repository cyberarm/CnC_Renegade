...
void DlgConfigPerformanceTabClass::Setup_Controls(void)
{
    ...
    const WCHAR* PRELIT_MODE_NAMES[]
        = { TRANSLATE(IDS_MENU_VERTEX), TRANSLATE(IDS_MENU_MP_LIGHTMAPS),
    ...
    const WCHAR* TEXTURE_FILTER_NAMES[]
        = { TRANSLATE(IDS_BILINEAR), TRANSLATE(IDS_TRILINEAR), TRANSLATE(IDS_ANISOTROPIC) };
    ...
    if (DX8Wrapper::Get_Current_Caps()
        && DX8Wrapper::Get_Current_Caps()->Support_NPatches() == false) {
...
void DlgConfigPerformanceTabClass::Load_Values(void)
{
    ...
    SliderCtrlClass* surface_effect_slider
        = (SliderCtrlClass*)Get_Dlg_Item(IDC_SURFACE_DETAIL_SLIDER);
...
//
////////////////////////////////////////////////////////////////
void DlgConfigPerformanceTabClass::On_SliderCtrl_Pos_Changed(SliderCtrlClass* slider_ctrl,
                                                             int ctrl_id, int new_pos)
{
...
bool DlgConfigPerformanceTabClass::On_Apply(void)
{
    ...
    if (registry.Is_Valid()) {
        ...
        COMBAT_SCENE->Enable_Dynamic_Projectors(
            (shadow_mode != PhysicsSceneClass::SHADOW_MODE_NONE));
