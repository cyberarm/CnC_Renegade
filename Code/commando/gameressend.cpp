...
static void AddPlayerStats(GameResPacket& stats, cPlayer* player, WOL::Locale locale, int winnerID,
                           bool teamGame);
...
void SendGameResults(unsigned long gameID, cGameData* theGame, SList<cPlayer>* playerList)
{
    ...
    char buildDate[20];
    sprintf(buildDate, "%02d/%02d/%04d %02d:%02d:%02d", time.wMonth, time.wDay, time.wYear,
    ...
    char startTime[20];
    sprintf(startTime, "%02d/%02d/%04d %02d:%02d:%02d", gameTime->wMonth, gameTime->wDay,
            gameTime->wYear, gameTime->wHour, gameTime->wMinute, gameTime->wSecond);
...
#if (0)
#ifdef _DEBUG
    HANDLE file = CreateFile("GameRes.dat", GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS,
...
void AddPlayerStats(GameResPacket& stats, cPlayer* player, WOL::Locale locale, int winnerID,
                    bool isTeamed)
{
    if (player->Is_Human()) {
        // Players WOL name
        ...
        stats.Add_Field("FLGC",
                        (unsigned long)0); // no more CTF! (unsigned long)player->Get_Flag_Caps());
