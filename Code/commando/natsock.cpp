...
bool SocketHandlerClass::Open(int inport, int outport)
{
    ...
    */
    ...
    if (err == INVALID_SOCKET) {
        DebugString(
            ("SocketHandlerClass - Failed to set socket option SO_RCVBUF - error code %d.\n",
             LAST_ERROR));
    ...
    */
    ...
    if (err == INVALID_SOCKET) {
        DebugString(
            ("SocketHandlerClass - Failed to set socket option SO_SNDBUF - error code %d.\n",
             LAST_ERROR));
        fw_assert(err != INVALID_SOCKET);
    }
    else {
...
void SocketHandlerClass::Close(void)
{
    ...
    */
    ...
    if (Socket != INVALID_SOCKET) {
        DebugString(
            ("SocketHandlerClass - Closing socket %d bound to port %d\n", Socket, IncomingPort));
        if (closesocket(Socket) != 0) {
            DebugString(
                ("SocketHandlerClass - closesocket failed with error code %d\n", LAST_ERROR));
        }
        Socket = INVALID_SOCKET;
        Service_Never();
    }
 ...
 *=============================================================================================*/
int SocketHandlerClass::Read(void* buffer, int buffer_len, void* address, unsigned short* port,
                             int packetnum)
{
    /*
    ...
    */
    ...
    DebugString(
        ("SocketHandlerClass - SocketHandlerClass::Read - Returning %d bytes. %d InBuffers left\n",
 ...
 *    3/8/00 1:12PM ST : Created                                                               *
 *=============================================================================================*/
int SocketHandlerClass::Peek(void* buffer, int buffer_len, void* address, unsigned short* port,
                             int packetnum)
{
    /*
    ** Call the Service function in case there are any outstanding unqueued packets.
    ...
    */
    ...
    DebugString(("SocketHandlerClass - SocketHandlerClass::Peek - Returning packet %d of %d bytes. "
                 "%d InBuffers left\n",
            ...
            if (bytes == 0) {
                ...
                */
                if (result == SOCKET_ERROR) {
                    DebugString(
                        ("SocketHandlerClass - recvfrom returned error code %d\n", LAST_ERROR));
                    Clear_Socket_Error();
                    break;
                }
                else {
                    ...
                    */
                    if (!Passes_CRC_Check(packet)) {
                        ...
                        */
                        ...
                        DebugString(
                            ("SocketHandlerClass - InBuffers.Count() == %d\n", InBuffers.Count()));
                    }
                }
            }
        }
