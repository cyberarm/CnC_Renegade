...
WaitCondition::WaitResult FirewallConnectWait::GetResult(void)
{
    if (mEndResult == Waiting) {
        ...
        if (mPingsRemaining != pingsWaiting) {
            ...
            if (mPingsRemaining == 0) {
                FirewallHelper.Set_Client_Connect_Event(mEvent, mCancelEvent, &mSuccessFlag,
                                                        (int*)&mQueueCount);
                mTimeout = 32000;
                mStartTime = TIMEGETTIME();
            }
...
void FirewallConnectWait::EndWait(WaitResult result, const wchar_t* endText)
{
    ...
    // Give the firewall code a little time to respond then remove it's cancel event anyway. It'll
    // figure it out.
    for (int i = 0; i < 100; i++) {
