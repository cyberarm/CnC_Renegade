    ...
    ****************************************************************************************/
    ...
    if (Get_Text_Display()) {
        ...
        if (StatisticsDisplayManager::Is_Current_Display("fps")) {
            ...
            unsigned actualuse
                = FastAllocatorGeneral::Get_Allocator()->Get_Total_Actual_Memory_Usage();
            ...
            working_string.Format("\nMalloc count: %d\n"
                                  ...
                                  WW3D::Get_Last_Frame_Memory_Free_Count(), ffheap / (1000 * 1000),
                                  (ffheap / 1000) % 1000, ffheap % 1000, ffheap / (1024 * 1024),
                                  ffuse / (1000 * 1000), (ffuse / 10000) % 1000, ffuse % 1000,
                                  ffuse / (1024 * 1024), actualuse / (1000 * 1000),
                                  (actualuse / 1000) % 1000, actualuse % 1000,
            ...
            working_string.Format("verts/frame: %7d v/p ratio: %2.2f\n",
                                  ...
                                  float(WW3D::Get_Last_Frame_Vertex_Count())
                                      / float(WW3D::Get_Last_Frame_Poly_Count()));
            message += working_string;
#endif
            ...
            // Only display texture and vertex processor statistics if texture statistics recording
            // is enabled

            if (Debug_Statistics::Get_Record_Texture_Mode()
                != Debug_Statistics::RECORD_TEXTURE_NONE) {
                // Texture usage
                ...
                working_string.Format("total thumbnails: %5d\n"
                                      ...
                                      red_size >> 20, (10 * (red_size >> 10) >> 10) % 10);
                message += working_string;

                if (Debug_Statistics::Get_Record_Texture_Mode()
                    == Debug_Statistics::RECORD_TEXTURE_DETAILS) {
                    message += "\n"
                               "<F9 + F5> Scroll up\n"
                               "<F9 + F6> Scroll down\n"
                               "\n";
        ...
        ****************************************************************************************/
        ...
        if (StatisticsDisplayManager::Is_Current_Display("dx8")) {
            ...
            /*			bool empty=true;
                                    for (int
               a=0;a<DX8MeshRendererContainerClass::RENDER_CLASS_MAX;++a) { DX8MeshRendererClass*
               renderer=n->render_class_types[a].Get_Renderer_Head(); if (renderer) { empty=false;
                                                    break;
                                            }
                                    }
                                    ...
                                    if  (empty) {
                                            ...
                                            message+=working_string;
                                            for (int
               a=0;a<DX8MeshRendererContainerClass::RENDER_CLASS_MAX;++a) {
                                                    working_string.Format("Class: %s",
                                                            DX8MeshRendererContainerClass::Get_Render_Class_Name(a));
                                                    message+=working_string;

                                                    DX8MeshRendererClass*
               renderer=n->render_class_types[a].Get_Renderer_Head(); if (!renderer) { message+=" No
               ...
               || renderer->stats_last_frame_count) { working_string.Format("%24s Add: %3d Render:
               %3d Polys: %3d\n", renderer->name, renderer->stats_last_frame_add_calls,
            ...
            */
            ...
            /*		working_string.Format("Total add renders:
               %d\n",DX8MeshRendererClass::Get_Last_Frame_Render_Stats()); message+=working_string;
                            for (int
               ridx=0;ridx<DX8MeshRendererContainerClass::RENDER_CLASS_MAX;++ridx) {
                                    working_string.Format("Class: %s\nObjects: %d\nPolys: %d\n\n",
        ...
        if (StatisticsDisplayManager::Is_Current_Display("audio")) {
            ...
            for (int sample_index = 0; sample_index < count_2d; sample_index++) {
                ...
                if (sound_obj != nullptr) {
                    ...
                    PhysicsSceneClass::Get_Instance()->Add_Debug_AABox(
                        AABoxClass(curr_pos, Vector3(0.25F, 0.25F, 0.25F)), Vector3(1, 0, 0));
                }
                else {
                    message += "  --\n";
                }
            ...
            for (sample_index = 0; sample_index < count_3d; sample_index++) {
                ...
                if (sound_obj != nullptr) {
                    ...
                    Vector3 curr_pos = sound_obj->Get_Position();
                    PhysicsSceneClass::Get_Instance()->Add_Debug_AABox(
                        AABoxClass(curr_pos, Vector3(0.25F, 0.25F, 0.25F)), Vector3(0, 0, 1));
                }
                else {
                    message += "  --\n";
                }
        ...
        ****************************************************************************************/
        if (StatisticsDisplayManager::Is_Current_Display("star")) {
            ...
            working_string.Format("Star (%1.1f, %1.1f, %1.1f) %1.1f\n", PlayerPosition.X,
                                  PlayerPosition.Y, PlayerPosition.Z, RAD_TO_DEGF(PlayerFacing));
            message = working_string;
            ...
            if (the_scene != nullptr) {
                ...
                for (iterator.First(); !iterator.Is_Done(); iterator.Next()) {
                    ...
                    if (obj->As_Phys3Class()) {
                        ...
                        if ((obj->As_VehiclePhysClass())
                            && (obj->As_VehiclePhysClass()->Get_VehiclePhysDef()->Is_Fake()
                                == true)) {
                            is_active = false;
                        }
...
void ConsoleGameModeClass::Update_Suggestion(char* cmd, bool go_to_next)
{
    if (ConsoleInputType == INPUT_FUNCTION_BEGIN_CONSOLE) {
        ...
        if ((strlen(cmd) > 0) && (strchr(cmd, ' ') == nullptr)) {
            ...
            bool gotone = ConsoleFunctionManager::Get_Command_Suggestion(
                cmd, cur_suggestion, Suggestion, HelpLine, sizeof(Suggestion));
            if (!gotone) {
                Clear_Suggestion();
            }
        }
...
void ConsoleGameModeClass::Update_Profile(void)
{
    ...
    if (profile_log_active) {
        Process_Profile_Log();
        Vector2 pos = (Render2DClass::Get_Screen_Resolution().Upper_Left()
                       + Render2DClass::Get_Screen_Resolution().Lower_Left())
            * 0.5f;
            ...
            */
            if (parent_time > 0.0f) {
                ...
                working_string.Format("   %-25s %-6.2f   %-6.2f  %-6.2f\n", "MISSING TIME",
                                      percent_of_parent, percent_of_total, ms_per_frame);
                profile_string += working_string;
            }
        }
    ...
    if (ProfileIterator) {
        ...
        */
        ...
        if ((total_frames > 0) && (total_time > 0.0f)) {
            ...
            unsigned index = 0;
            for (ProfileIterator->First(); !ProfileIterator->Is_Done();
            ...
            */
            ...
            for (ProfileIterator->First(); !ProfileIterator->Is_Done(); ProfileIterator->Next()) {
                ...
                */
                percent_of_total = 100.0f * time / total_time;
                //				if (parent_time != 0.0f) {
                //					percent_of_parent = 100.0f * time /
                //parent_time; 					working_string.Format(" %-6.2f
                //%-6.2f",percent_of_parent,percent_of_total); 				} else { 					percent_of_parent = 100.0f;
                //					working_string.Format("   --
                ...
                */
                ...
                //					ms_per_call = 1000.0f * time / (float)calls;
                //					calls_per_frame = calls / total_frames;
                //					working_string.Format(" %-6.2f    %-4d
                //(%4d)\n",ms_per_call, calls_per_frame, calls); 				} else { 					working_string.Format(" --
                //-- \n");
                //				}
                //				profile_string += working_string;
            ...
            */
            /*			if (parent_time > 0.0f) {
                                            percent_of_parent = 100.0f * missing_parent_time /
               parent_time; percent_of_total = 100.0f * missing_parent_time / total_time;
                                            ms_per_frame = 1000.0f * missing_parent_time /
               total_frames;
            ...
            */
            ...
            //		Vector2 pos = (Render2DClass::Get_Screen_Resolution().Upper_Left() +
            //Render2DClass::Get_Screen_Resolution().Lower_Left()) * 0.5f;
            //		StatisticsDisplayManager::Set_Stat( "profile", profile_string, 0xffffffff,
            //pos );
        }
    }
}
