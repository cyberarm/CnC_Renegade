...
{
    if (cNetwork::I_Am_Server() || COMBAT_SCENE == nullptr
    ...
    NetworkObjectClass** object_list
        = (NetworkObjectClass**)_alloca((count * sizeof(NetworkObjectClass*)) + 128);
            ...
            if (p_object->Get_Clientside_Update_Frequency()) {
                ...
                if (pvs == nullptr || vis_id == -1 || pvs->Get_Bit(vis_id)) {
                    ...
                    if (player_ptr) {
                        ...
                        float priority = cPriority::Compute_Object_Priority(
                            cNetwork::Get_My_Id(), position, p_object, true);
    ...
    //
    // Look for an object that has a higher priority than it's neighbor but is getting updated much
    // less frequently
    //
    ...
    for (int i = 1; i < num_objects; i++) {
        ...
        //
        // Don't hint for objects with a recent updates. This should prevent us from hinting about
        // objects we just hinted about.
        //
        if (time - object_list[i]->Get_Last_Clientside_Update_Time() > 1500) {
            ...
            if (lower_priority_rate && higher_priority_rate) {
                ...
                if (lower_priority_rate < higher_priority_rate) {
                    ...
                    // The higher priority object is being updated by the server less frequently.
    ...
    if (most_broken_object_index != -1 && worst_percentage > 100 + (10.0f * hint_factor)) {
        ...
        // WWDEBUG_SAY(("cClientHintManager::Think, requesting hint for object id %d, avg = %5.2f,
        // max = %d\n", 	p_object->Get_Network_ID(), average_delay_ms, maximum_delay_ms));
        WWDEBUG_SAY(("cClientHintManager::Think, requesting hint for object id %d (%s), priority = "
                     "%5.2f, ave update rate = %dms\n",
