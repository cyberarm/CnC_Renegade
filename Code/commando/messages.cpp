...
static const unsigned short infinity_update_rate
    = 0xffff; // Lowest update rate - no updates at all.
...
void cNetwork::Tell_Client_About_Dynamic_Objects(int client_id, Vector3& dest_pos)
{
    ...
    if (cDevOptions::UseNewTCADO.Is_False()) {
        ...
        */
            ...
            */
            for (int index = 0; index < count; index++) {
                ...
                if (p_object->Get_App_Packet_Type() == APPPACKETTYPE_SERVERFPS) {
                    ...
                    /*
                    ** Get the base priority. This is just distance from where the server thinks the
                    *client is.
                    */
                    ...
                    /*
                    ** If we can't see it at all, ignore it unless the priority is really high. A
                    *really high priority might indicate
                    ...
                    */
                    ...
                    if (p_object->Get_Object_Dirty_Bit(client_id, NetworkObjectClass::BIT_CREATION)
                        ...
                        || p_object->Get_Object_Dirty_Bit(client_id,
                                                          NetworkObjectClass::BIT_OCCASIONAL)) {
                            ...
                            if (p_object->Get_Frequent_Update_Export_Size() == 0) {
                                ...
                                if (bits_now < bits_after) {
                                    ...
                                    */
                                    ...
                                    /*
                                    ** For some reason, some objects have a frequent bit set but
                                    *there is no frequent update export.
                                    */
        ...
        /*
        ** Now we have two lists. One list of non-guaranteed objects that we would like to send and
        *another list of guaranteed
        ...
        */
        ...
        unsigned long max_bytes
            = avail_bytes_per_update; //(avail_bytes_per_update >> 1);	//* 3) / 4;
        ...
        */
...

        {
            WWPROFILE("SendG");
            /*
            ** Send the guaranteed stuff first. This has to be done anyway so let's see how much of
            *it there is.
            */
            bool full = false;
                ...
                */
                if (!full && bytes_out > max_bytes) {
                    ...
                    if (cDevOptions::ExtraNetDebug.Is_True()) {
                        WWDEBUG_SAY(
                            ("*** WARNING: Tell_Client_About_Dynamic_Objects - Insufficient "
                             "bandwidth to send all guaranteed packets ***\n"));
        ...
        /*
        ** Work out the average priority. We will need this later when balancing bandwidth budgets
        *between clients.
        */
...

        {
            ...
            if (num_priorities) {
                ...
                ** Just set the rate based on priority then we will go back in a second pass and
                *adjust it according to remaining bandwidth.
                */
                ...
                for (i = 0; i < object_list.Count(); i++) {
                    ...
                    if (update_rate != infinity_update_rate) {
                        int bps
                            = (1000.0f / update_rate) * temp_obj->Get_Frequent_Update_Export_Size();
                        total_bps += bps;
                    }
                }
        ...
        */
        ...
        const unsigned char dirty_check = (NetworkObjectClass::BIT_FREQUENT ^ 0xffffffff)
            & (NetworkObjectClass::BIT_CREATION | NetworkObjectClass::BIT_RARE
               | NetworkObjectClass::BIT_OCCASIONAL);
        ...
        */
        ...
        /*
        ** Adjust the cut off threshold for vis hidden objects further out if more bandwidth is
        *available.
        */
                    ...
                    if (dirty & dirty_check) {
                        ...
                        */
                        if (!global_packet_allowance_full
                            || p_object->Get_App_Packet_Type() == APPPACKETTYPE_CLIENTBBOEVENT) {
                        ...
                        /*
                        ** See if we are using too much bandwidth and don't send any more guaranteed
                        *packets if we are.
                        */
                        ...
                        /*
                        ** Get the base priority. This is how important we think this object is to
                        *this client.
                        */
                        if ((dirty & NetworkObjectClass::BIT_FREQUENT) != 0) {
                            ...
                            if (p_object == player_ptr) {
                                ...
                                if (p_object->Get_Client_Hint_Count(client_id) > 0) {
                                    ...
                                    /*
                                    ** If we can't see it at all, ignore it unless the priority is
                                    *really high. A really high priority might indicate
                                    ** an object update request from the client.
                                    */
                                    /*
                                    ** Even if the client can't see the object, it's probably a good
                                    *idea to update it if it's really
                                    ...
                                    */
                                    if (update_priorities) {
                                        ...
                                        if (hidden) {
                                            int distance = cPriority::Get_Object_Distance_2(
                                                dest_pos, p_object);
                                            if (distance > min_vis_distance) {
                                                /*
                                                ** Allow some very infrequent updates for distant,
                                                *hidden objects on broadband connections.
                                                */
                                                ...
                                                /*
                                                ** It's hidden, but close (maybe as close at 15
                                                *meters), so it's probably somewhat important to us.
                                                */
                                                priority = 0.2f;
                                            }
                                        }
                                            ...
                                            /*
                                            ** Figure out the priority if it's time or if we
                                            *couldn't see the object last frame but we can now.
                                            */
                                            // if (update_priorities || priority == 0.0f) {
                                            priority = cPriority::Compute_Object_Priority_2(
                                                client_id, dest_pos, p_object, false, player_ptr);
                                            p_object->Set_Cached_Priority_2(client_id, priority);
                                            //}
                                        }
                                    }
        ...
        */
        ...
        /*
        ** If we used tons of bandwidth on guaranteed packets then throttle back on non-guaranteed
        *stuff.
        */
        if (global_packet_allowance_full) {
            avail_bytes_per_update >>= 1;
            ...
            if (num_priorities) {
                ...
                */
                ...
                for (i = 0; i < object_list.Count(); i++) {
                    ...
                    float obj_upd_rate = (float)temp_obj->Get_Update_Rate(client_id);
                    if (obj_upd_rate != infinity_update_rate
                        && obj_upd_rate < (min_update_rate + WWMATH_EPSILON)) {
                        float_rate = obj_upd_rate / factor;
                        rate = (unsigned short)float_rate;
                        temp_obj->Set_Update_Rate(client_id, rate);
                    }
...
int cNetwork::Send_Object_Update(NetworkObjectClass* object, int client_id)
{
    ...
    Debug_Network_Prolific(("Sending update for object %d to %s", object->Get_Network_ID(),
                            Get_Client_String(client_id)));
    ...
    if (object->Get_Object_Dirty_Bit(client_id, NetworkObjectClass::BIT_CREATION)) {
        ...
        //
        NetworkObjectFactoryClass* factory
            = NetworkObjectFactoryMgrClass::Find_Factory(net_classid);
        ...
        int bits_after = packet.Get_Bit_Write_Position();
        cAppPacketStats::Increment_Bits_Sent_Tier(type, PACKET_TIER_CREATION,
    ...
    if (object->Get_Object_Dirty_Bit(client_id, NetworkObjectClass::BIT_OCCASIONAL)) {
        ...
        int bits_after = packet.Get_Bit_Write_Position();
        cAppPacketStats::Increment_Bits_Sent_Tier(type, PACKET_TIER_OCCASIONAL,
                                                  bits_after - bits_before);
        mode = SEND_RELIABLE;
    }
    ...
    if (object->Get_Object_Dirty_Bit(client_id, NetworkObjectClass::BIT_FREQUENT)) {
        ...
        int bits_after = packet.Get_Bit_Write_Position();
        cAppPacketStats::Increment_Bits_Sent_Tier(type, PACKET_TIER_FREQUENT,
                                                  bits_after - bits_before);
    }
...
bool cNetwork::Client_Think(void)
{
    ...
    if (!HaveDoneMotdDialog) {
        if (I_Am_Only_Client()) {
            if ((DlgMsgBox::Get_Current_Count() == 0) && GameModeManager::Find("Combat") != nullptr
                && GameModeManager::Find("Combat")->Is_Active() && The_Game() != nullptr
                && ::wcslen(The_Game()->Get_Motd()) > 0) {
...
void cNetwork::Hibernation_Think(void)
{
    if (COMBAT_SCENE != nullptr) {
        ...
        //
        // The server can't let any player go into hibernation or updates will cease to be sent for
        // that player.
        //
        if (I_Am_Server() && !IS_MISSION) {
            for (SLNode<BaseGameObj>* p_objnode = GameObjManager::Get_Game_Obj_List()->Head();
                 p_objnode; p_objnode = p_objnode->Next()) {
            ...
            //
            for (SLNode<SmartGameObj>* p_smart_objnode
                 = GameObjManager::Get_Smart_Game_Obj_List()->Head();
                 p_smart_objnode; p_smart_objnode = p_smart_objnode->Next()) {
            ...
            for (SLNode<BaseGameObj>* p_objnode = GameObjManager::Get_Game_Obj_List()->Head();
                ...
                if (p_phys_go != nullptr) {
                    ...
                    if (p_phys_obj != nullptr
                        && (p_vis_table == nullptr
                            || p_vis_table->Get_Bit(p_phys_obj->Get_Vis_Object_ID()))) {
...
void cNetwork::Delete_Player_Objects(int client_id)
{
    ...
    for (objnode = GameObjManager::Get_Game_Obj_List()->Head(); objnode;
        ...
        if (p_phys_obj != nullptr) {
            ...
            if (p_smart_obj != nullptr && (p_smart_obj->Get_Control_Owner() == client_id)) {
                //
                //	Destroy the object
                //
...
void cNetwork::Test_For_Team_Defaulting(cPlayer* p_player)
{
    ...
    else if (p_player->Get_Player_Type() == PLAYERTYPE_GDI && count_gdi == 0
             && score_gdi > score_nod) {
