...
void WOLGameInfo::ImportFromGame(const cGameData& theGame)
{
    ...
    mIsPassworded = ((password != nullptr) && (wcslen(password) > 0));
...
void WOLGameInfo::ImportFromChannel(const RefPtr<ChannelData>& channel)
{
    ...
    int count = sscanf(exInfo, "%08lX%08lX%08lX%08lX%c%c%c%c", &version, &fileCRC, &clanID1,
                       &clanID2, &gameType, &gameFlags1, &gameFlags2, &modMapIndex);
    ...
    if (mIsMod) {
        ...
        if (ModPackageMgrClass::Get_Mod_Map_Name_From_CRC_Index(fileCRC, modMapIndex, &modName,
                                                                &mapName)
            == false) {
            // mIsDataValid = false;
            got_map_name = false;
        ...
        // Find mod based on its crc and get map filename based on the index.
        if (ModPackageMgrClass::Get_Mod_Map_Name_From_CRC(0, fileCRC, &modName, &mapName)
            == false) {
            // mIsDataValid = false;
            got_map_name = false;
        }
...
void WOLGameInfo::ExportToChannel(const RefPtr<ChannelData>& channel)
{
    if (channel.IsValid() && mIsDataValid) {
        ...
        char exInfo[41];
        sprintf(exInfo, "%08lX%08lX%08lX%08lX%c%c%c%c", mVersion, fileCRC, mClanID1, mClanID2,
                (0x30 + mGameType), gameFlags1, gameFlags2, modMapIndex);
...
bool WOLGameInfo::IsValidGameChannel(const RefPtr<ChannelData>& channel)
{
    ...
    return (gameInfo.IsDataValid()
            && (gameInfo.mVersion == (unsigned long)cNetwork::Get_Exe_Key()));
}
