...
bool ServerSettingsClass::Parse(bool apply)
{
    ...
    */
    if (IsActive) {
        ...
        /*
        ** Make sure the server config file is there. It should be since it's verified at command
        *line parsing time.
        */
        ...
        */
        INIClass ini(file);
        ini.Get_String(MasterServerSection, ConfigSettingsName, "", master_settings,
                       sizeof(master_settings));
        ...
        */
        ...
        if (stricmp(game_type, "WOL") == 0) {
            ...
            ConsoleBox.Print("Error - Unknown game type in server settings. Use 'LAN', 'WOL', or "
                             "'GameSpy' - aborting\n");
            ConsoleBox.Wait_For_Keypress();
            ;
            return (false);
        ...
        */
        ...
        ini.Get_String(MasterServerSection, "HeartBeatServers",
                       GameSpyQnR.Get_Default_HeartBeat_List(), heartbeat_list,
        ...
        */
        ...
        /*
        ** We only need to validate this for the FDS. The regular game can allow the login name to
        *be specified in the registry.
        */
...
#ifdef FREEDEDICATEDSERVER
        /*
        ** We only need to validate the serial number if we are the FDS. For the regular game, the
        *master serial will be stored
        ** in the registry.
        */
        ...
        */
        ...
        if (wol && master_port != 0xffffffff && (master_port < 0 || master_port > 0xffff)) {
            WWDEBUG_SAY(("Error - Invalid port number %d specified for master server - aborting\n",
                         master_port));
            ConsoleBox.Print(
                "Error - Invalid port number %d specified for master server - aborting\n",
                master_port);
            ConsoleBox.Wait_For_Keypress();
            return (false);
        }
        ...
        */
        ...
        if (gsqport < 0 || gsqport > 0xffff) {
            WWDEBUG_SAY(
                ("Error - Invalid port number %d specified for GameSpy Query Port - aborting\n",
                 gsqport));
            ConsoleBox.Print(
                "Error - Invalid port number %d specified for GameSpy Query Port - aborting\n",
                gsqport);
        ...
        */
        ...
        if (master_bw != 0 && master_bw != 0xffffffff && master_bw < 33600) {
            WWDEBUG_SAY(
                ("Error - Insufficient bandwidth specified for master server - aborting\n"));
            ConsoleBox.Print(
                "Error - Insufficient bandwidth specified for master server - aborting\n");
            ConsoleBox.Wait_For_Keypress();
            ;
        ...
        */
        ini.Get_String(MasterServerSection, "LoginServer", "", PreferredLoginServer,
                       sizeof(PreferredLoginServer));
        ...
        */
        ...
        if (allow_remote) {
            ini.Get_String(MasterServerSection, "RemoteAdminPassword", "", remote_admin_pass,
                           sizeof(remote_admin_pass));
            ...
            */
            ...
            /*
            ** We need to bind to more than just the loopback address when listening for control
            *messages.
            */
            ...
            */
            ini.Get_String(MasterServerSection, "RemoteAdminIP", "0.0.0.0", remote_admin_ip,
                           sizeof(remote_admin_ip));
            ...
            if (reg_netopt.Is_Valid()) {
                if (cGameSpyAdmin::Is_Gamespy_Game()) {
                    ...
                    if (wol) {
                        ...
                        /*
                        ** We want this to be set always on the first time through, but not
                        *neccessarily on the second, apply, pass.
                        */
        ...
        for (int i = 0; i < MAX_SLAVES; i++) {
            ...
            */
            ...
            if (enabled) {
                ...
                */
                ini.Get_String(slave_section, ConfigSettingsName, "", slave_settings,
                               sizeof(slave_settings));
                if (strlen(slave_settings) != 0) {
                    ...
                    if (!file.Is_Available()) {
                        ...
                        ConsoleBox.Print(
                            "Error - Slave server settings file '%s' not found - aborting\n",
                            filename);
                        ConsoleBox.Wait_For_Keypress();
                        ;
                        return (false);
                        ...
                        if (!Check_Game_Settings_File(slave_settings)) {
                            WWDEBUG_SAY(("Server settings file '%s' not usable\n", slave_settings));
                            ConsoleBox.Print(
                                "Error - server settings file '%s' contains errors - aborting\n",
                                slave_settings);
                            ConsoleBox.Wait_For_Keypress();
                            ;
                            return (false);
                ...
                */
                ...
                if (wol && strlen(slave_nick) == 0) {
                    WWDEBUG_SAY(
                        ("Error - No login nickname specified for slave %d - aborting\n", i + 1));
                    ConsoleBox.Print(
                        "Error - No login nickname specified for slave %d - aborting\n", i + 1);
                    ConsoleBox.Wait_For_Keypress();
                    ;
                ...
                if (wol && strlen(slave_serial) == 0) {
                    WWDEBUG_SAY(
                        ("Error - No serial number specified for slave %d - aborting\n", i + 1));
                ...
                */
                ...
                if (wol && strlen(slave_pass) == 0) {
                    WWDEBUG_SAY(
                        ("Error - No login password specified for slave %d - aborting\n", i + 1));
                    ConsoleBox.Print(
                        "Error - No login password specified for slave %d - aborting\n", i + 1);
                    ConsoleBox.Wait_For_Keypress();
                    ;
                ...
                */
                ...
                if (slave_port < 0 || slave_port > 0xffff) {
                    WWDEBUG_SAY(
                        ("Error - Invalid port number %d specified for slave %d - aborting\n",
                         slave_port, i + 1));
                    ConsoleBox.Print(
                        "Error - Invalid port number %d specified for slave %d - aborting\n",
                        slave_port, i + 1);
                ...
                */
                ...
                if (slave_bw != 0 && slave_bw != 0xffffffff && slave_bw < 33600) {
                    WWDEBUG_SAY(
                        ("Error - Insufficient bandwidth specified for slave %d - aborting\n",
                         i + 1));
                    ConsoleBox.Print(
                        "Error - Insufficient bandwidth specified for slave %d - aborting\n",
                        i + 1);
            ...
            */
            if (apply) {
                SlaveMaster.Add_Slave(enabled, slave_nick, slave_serial, (unsigned short)slave_port,
                                      slave_settings, slave_bw, slave_pass);
            }
        }
        if (apply) {
            SlaveMaster.Save();
 ...
 *=============================================================================================*/
void ServerSettingsClass::Encrypt_Serial(StringClass serial_in, StringClass& serial_out,
                                         bool encrypt)
{
    char* s;
    ...
    */
    HANDLE handle
        = CreateFile("woldata.key", GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0, nullptr);
...
void ServerSettingsClass::Write_Server_List(const WWOnline::IRCServerList& server_list)
{
    ...
    if (server_list.size()) {
        ...
        /*
        ** This is a bit hacky. Basically we just want the .ini file commented with the latest
        *server list.
        **
        ** We will do this by reading in the file, looking for a tag that tells us where to write
        *the server list, then writing
...
{
    cGameDataCnc* game_settings
        = (cGameDataCnc*)cGameData::Create_Game_Of_Type(cGameData::GAME_TYPE_CNC);
