...
{
    GameOptionsMutex = CreateMutex(nullptr, false, nullptr);
...
void WOLNATInterfaceClass::Init(void)
{
    ...
    */
    ...
    if (reg.Is_Valid()) {
        ...
        */
        FirewallHelper.Set_Firewall_Info((unsigned long)last_behavior,
                                         last_source_port_allocation_delta,
        ...
        */
        ...
        if (socket1.Open(PortBase, 0)) {
            if (socket2.Open(PortBase + 1, 0)) {
                ...
                WWDEBUG_SAY(
                    ("WOLNATInterface::Init - unable to open port %d\n", (int)PortBase + 1));
            }
            socket1.Close();
        }
        else {
...
unsigned short WOLNATInterfaceClass::Get_Port_As_Server_Client(void)
{
    ...
    */
    WWDEBUG_SAY(
        ("WOLNATInterface::Get_Port_As_Server_Client - returning port %d\n", (int)PortBase + 1));
    return (PortBase + 1);
}
    ...
    */
    ...
    if (reg.Is_Valid()) {
        ...
        */
        ...
        FirewallHelper.Get_Firewall_Info(last_behavior, last_source_port_allocation_delta,
                                         source_port_pool, send_delay, confidence);
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:    Ptr to registry entry object (nullptr for default)                                   *
...
void WOLNATInterfaceClass::HandleNotification(WWOnline::UserEvent& event)
{
    ...
    */
    ...
    /*
    ** If someone is leaving the channel, and we are a server they are waiting to negotiate with,
    ...
    */
    if (Am_I_Server() && event.GetEvent() == WWOnline::UserEvent::Leave) {
        const RefPtr<WWOnline::UserData>& joiner = event.Subject();
        WOL::User const& user = joiner->GetData();
        if (FirewallHelper.Remove_Player_From_Negotiation_Queue_If_Mutex_Available(
                (char*)user.name)) {
    ...
    */
    if (Am_I_Server()) {
        ...
        /*
        ** If someone is joining our channel and we are a game server then we need to start talking
        *to them.
        */
        if (event.GetEvent() == WWOnline::UserEvent::Join) {
            ...
            /*
            ** Make sure this is a game channel by comparing the channel type to the game ID in our
            *SKU.
            */
            /*
    ...
    if (ipevent.GetEvent() == WWOnline::UserIPEvent::GotIP) {
        ...
        if (Get_My_Name(namebuf)) {
            if (stricmp((char*)user.name, namebuf) == 0) {
                ...
                WWDEBUG_SAY(("WOLNATInterfaceClass::HandleNotification(WWOnline::UserIPEvent "
                             "&ipevent) : ip = %08x\n"));
                ChatExternalIP = ip;
...
{
    /*
    ** This can be called from the firewall thread so all it does is add the options to a queue. We
    *can't send them directly
 ...
 *=============================================================================================*/
bool WOLNATInterfaceClass::Get_Private_Game_Options(WOL::User* user, char* options_buffer,
                                                    int option_buffer_len)
{
    ThreadLockClass locker(this);
...
void WOLNATInterfaceClass::Service(void)
{
    ...
    /*
    ** Remove any names from the client queue that represent players who have already left the
    *channel.
    */
    ...
    */
    while (OutgoingOptions.Count()) {
        ...
        HRESULT res =
#endif // WWDEBUG
            SessionPtr->GetChatObject()->RequestPrivateGameOptions(&options->User,
                                                                   options->Options);
        fw_assert(res == S_OK);
...
{
    ThreadLockClass locker(this);
    fw_assert((socket == nullptr && ServiceSocketHandler != nullptr)
              || (socket != nullptr && ServiceSocketHandler == nullptr));
 ...
 *=============================================================================================*/
char* WOLNATInterfaceClass::Get_Silly_String(WideStringClass* silly_string, char* buffer,
                                             int buffer_size)
{
    StringClass string;
 ...
 *=============================================================================================*/
bool WOLNATInterfaceClass::Send_Game_Format_Packet_To(IPAddressClass* address, char* payload,
                                                      int payload_size,
    ...
    */
    if (Am_I_Server()) {
        ...
        */
        ...
        /*
        ** If we are the client then the game comms isn't initialised and we need to send directly.
        *Ugh.
        **
        ...
        ** Send it.
        */
        PacketManager.Take_Packet((unsigned char*)full_packet.Get_Data(),
                                  full_packet.Get_Compressed_Size_Bytes(),
                                  (unsigned char*)&sock_address.sin_addr.s_addr,
                                  sock_address.sin_port, socket_handler->Get_Socket());
        PacketManager.Flush(
            true); // Hmmmmm is this going to send a bunch of other packets to the wrong place?

#if (0)
        WWDEBUG_SAY(("WOLNATInterface - sendto %s\n", address->As_String()));
        int result = sendto(socket_handler->Get_Socket(), full_packet.Get_Data(),
                            full_packet.Get_Compressed_Size_Bytes(), 0, (LPSOCKADDR)&sock_address,
                            sizeof(SOCKADDR_IN));
...
void WOLNATInterfaceClass::Service_Receive_Queue(SocketHandlerClass* socket)
{
    ...
    */
    if (!Am_I_Server()) {
        ...
        unsigned short port;
        int bytes
            = PacketManager.Get_Packet(socket->Get_Socket(), (unsigned char*)full_packet.Get_Data(),
                                       full_packet.Get_Max_Size(), ip_address, port);
        if (bytes > 0) {
            sockaddr_in* addr_ptr
                = (LPSOCKADDR_IN)&full_packet.Get_From_Address_Wrapper()->FromAddress;
        ...
        */
        if (bytes != 0) {
            ...
            */
            ...
            result = recvfrom(
                socket->Get_Socket(), full_packet.Get_Data(), full_packet.Get_Max_Size(), 0,
...
unsigned short WOLNATInterfaceClass::Get_Next_Client_Port(void)
{
    ...
    */
    if (ForcePort) {
        WWDEBUG_SAY(
            ("WOLNATInterface::Get_Next_Client_Port - returning port %d\n", (int)ForcePort));
        return (ForcePort);
    }
