...
public:
    ...
    static bool I_Am_Only_Client(void)
    {
        return PClientConnection != nullptr && PServerConnection == nullptr;
    }
    static bool I_Am_Only_Server(void)
    {
        return PClientConnection == nullptr && PServerConnection != nullptr;
    }
    static bool I_Am_Client_Server(void)
    {
        return PClientConnection != nullptr && PServerConnection != nullptr;
    }
    ...
    static void Packet_Graph(Render2DTextClass* renderer, cMsgStatList* p_stat_list, int watch_type,
                             LPCSTR label);
    static void Watch_Packets(Render2DTextClass* renderer);
    static void Bandwidth_Graph(Render2DTextClass* renderer, LPCSTR label, int count,
                                int target_bps, float packetloss_pc, const int bar_color,
                                bool is_send, float threshold_priority);
    static void Watch_Bandwidth(Render2DTextClass* renderer);
    static void Latency_Graph(Render2DTextClass* renderer, LPCSTR label, const float bar_length,
                              cRemoteHost* p_rhost);
    static void Last_Contact_Graph(Render2DTextClass* renderer, LPCSTR label,
                                   const float bar_length, cRemoteHost* p_rhost);
    static void List_Size_Graph(Render2DTextClass* renderer, LPCSTR label, const float bar_length,
                                int count);
    static void List_Time_Graph(Render2DTextClass* renderer, LPCSTR label, const float bar_length,
                                int time_ms);
    static void List_Packet_Size_Graph(Render2DTextClass* renderer, LPCSTR label,
