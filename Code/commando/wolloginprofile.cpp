...
void LoginProfile::LoadRank(const char* valueName, LoginProfile::Ranking& rank)
{
    ...
    if (registry.Is_Valid()) {
        ...
        sscanf(rankData, "%d,%d,%d,%d,%d,%d", &rank.Wins, &rank.Losses, &rank.Deaths, &rank.Kills,
               &rank.Points, &rank.Rank);
    }
    else {
...
void LoginProfile::SaveRank(const char* valueName, const LoginProfile::Ranking& rank)
{
    ...
    if (registry.Is_Valid()) {
        char rankData[255];
        sprintf(rankData, "%d,%d,%d,%d,%d,%d", rank.Wins, rank.Losses, rank.Deaths, rank.Kills,
                rank.Points, rank.Rank);
...
void ShowProfileRanking(DialogBaseClass* dialog, const LoginProfile* profile)
{
    ...
    if (list) {
        ...
        if (profile) {
            ...
            ShowRanking(list, WWOnline::LadderType_Team,
                        profile->GetRanking(WWOnline::LadderType_Team));
            ShowRanking(list, WWOnline::LadderType_Clan,
                        profile->GetRanking(WWOnline::LadderType_Clan));
        }
    }
}
