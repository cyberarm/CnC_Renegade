...
const wchar_t* RENEGADE_LOBBY_PASSWORD
    = L"not_a_valid_password"; // Password removed per Security review requirements. LFeenanEA -
                               // 27th January 2025
...
void WolGameModeClass::Init(void)
{
    ...
    if (!product.IsValid()) {
        Product::Initializer(APPLICATION_SUB_KEY_NAME, RENEGADE_GAMECODE, RENEGADE_LOBBY_PASSWORD,
                             RENEGADE_BASE_SKU);
    }
...
void WolGameModeClass::Think(void)
{
    ...
    if (mGameInProgress && (time - _last_auto_kick > 2000 || time < _last_auto_kick)) {
        ...
        if (cNetwork::I_Am_Server() && mGameInProgress) {
            ...
            for (int i = IdleKickNameList.Count() - 1; i >= 0; i--) {
                if (time - IdleKickTimeList[i] > _kick_history_persist_time
                    || time < IdleKickTimeList[i]) {
                    ...
                    if (myname.Compare_No_Case(user->GetName())) {
                        ...
                        if (!cPlayerManager::Find_Player(user->GetName())) {
                            if (time - user->mKickTimer > 120 * 1000) {
                                ...
                                WWDEBUG_SAY(("Kicking '%s' for sitting in the channel while not in "
                                             "the game\n",
                                             tempstr.Peek_Buffer()));
    ...
    if (mMonitorConnection) {
        ...
        // Check for patches.
        // This doesn't work because you can't get a server list without also doing a complete reset
        // of wolapi.
#if (0)
...
void WolGameModeClass::Create_Game(cGameData* theGame)
{
    ...
    if (theGame) {
        ...
        DlgWOLWait::DoDialog(IDS_GAME_CREATECHANNEL, createWait, this, 0,
...
void WolGameModeClass::Start_Game(cGameData* theGame)
{
    ...
    if (cNetwork::I_Am_Server()) {
        ...
        DlgWOLWait::DoDialog(TRANSLATE(IDS_MENU_STARTING_WOL_GAME), wait, nullptr, 0,
                             mQuietMode ? 0xffffffff : 0);
            ...
            for (unsigned int index = 0; index < count; ++index) {
                ...
                if (userClanID != 0) {
                    ...
                    if (!theGame->Is_Clan_Competing(userClanID)) {
                        ...
                        if (slot != -1) {
                            WWDEBUG_SAY(
                                ("CLANS: Assigning slot %d to clan #%lu\n", slot, userClanID));
...
void WolGameModeClass::Update_Channel_Settings(cGameData* theGame,
                                               const RefPtr<ChannelData>& channel)
{
    ...
    if (theGame && channel.IsValid()) {
        ...
        if (theGame->Is_QuickMatch_Server()) {
            ...
            char newTopic[81];
            sprintf(newTopic, "%s|%02lx%02x%04x%04x%c", topic, fps, avgPing, avgPoints, avgPlayed,
                    (32 + numPlayers));
...
void WolGameModeClass::Reply_Last_Page(const wchar_t* msg)
{
    if (msg && (wcslen(msg) > 0)) {
        ...
        if (pager && (wcslen(pager) > 0)) {
            ...
            CombatManager::Get_Message_Window()->Add_Message(TRANSLATE(IDS_GAME_PAGE_REPLY_CANT),
                                                             COLOR_CONSOLE_TEXT);
        }
    }
}
...
void WolGameModeClass::Ban_Player(const wchar_t* name, unsigned long ip)
{
    ...
    if (name && cNetwork::I_Am_Server() && !mWOLSession->IsCurrentUser(name)) {
        ...
        if (!Is_Banned(pn, ip)) {
            ...
            sprintf(ipstr, "%u.%u.%u.%u\n", (unsigned int)ip_ptr[0], (unsigned int)ip_ptr[1],
                    (unsigned int)ip_ptr[2], (unsigned int)ip_ptr[3]);
            pn += ipstr;
...
void WolGameModeClass::HandleNotification(DlgWOLWaitEvent& wolEvent)
{
    if (wolEvent.Result() == WaitCondition::ConditionMet) {
        ...
        if (!mQuietMode) {
            DlgMsgBox::DoDialog(TRANSLATE(IDS_MENU_FAILED_TO_CREATE_GAME),
                                wolEvent.Subject()->GetResultText());
        }
    }
        ...
        if (strcmp("RGINFO", request) == 0) {
            ...
            while (playerNode) {
                ...
                if (player && player->Is_Active() && player->Is_Human()) {
                    ...
                    info.Format("PINFO:%S %d %d %d %d %d", (const WCHAR*)name, type, rung, kills,
                                deaths, score);
            ...
            if (cmd == request) {
                ...
                if (sysinfo_log_disabled == -1) { // Read from registry only once per run
                    ...
                    if (registry.Is_Valid()) {
                        sysinfo_log_disabled
                            = registry.Get_Int(VALUE_NAME_DISABLE_SERVER_SYSINFO_COLLECTING, -1);
                        // Write to registry to create the key if it didn't exist
                        if (sysinfo_log_disabled == -1) {
                            sysinfo_log_disabled = 0;
                            registry.Set_Int(VALUE_NAME_DISABLE_SERVER_SYSINFO_COLLECTING,
                                             sysinfo_log_disabled);
                        }
                    }
                }
                ...
                if (sysinfo_log_disabled == 0) {
                    ...
                    SYSTEMTIME time;
                    GetSystemTime(&time);
                    tmp.Format("%d/%d/%d %d:%d\t%d.%d\t", time.wMonth, time.wDay, time.wYear,
                               time.wHour, time.wMinute, versionmajor, versionminor);
                    datastring += tmp;
                    ...
                    DWORD written;
                    HANDLE file;
                    file = CreateFile(filename, GENERIC_WRITE, 0, nullptr, OPEN_ALWAYS,
                                      FILE_ATTRIBUTE_NORMAL, nullptr);
                    if (INVALID_HANDLE_VALUE != file) {
                        SetFilePointer(file, 0, nullptr, FILE_END);
                        WriteFile(file, datastring, strlen(datastring), &written, nullptr);
                        CloseHandle(file);
                    }
                }
            }
    ...
    if (ladderEvent.IsRanked()) {
        ...
        if (LadderType_Team == type) {
            ...
            if (user.IsValid()) {
                ...
                if (squad.IsValid()
                    && (stricmp(squad->GetAbbr(), ladderEvent.GetReceivedName()) == 0)) {
    ...
    if (mMonitorConnection && mConnected) {
        ...
        if (cNetwork::I_Am_Server()) {
            ...
            if (The_Game() && The_Game()->IsDedicated.Is_True() && AutoRestart.Get_Restart_Flag()) {
                ...
                /*
                ** If the game has a time limit and players then we will wait until the next map
                *change otherwise we will stop now.
                */
                    ...
                    */
                    ...
                    if (cNetwork::I_Am_Server()) {
                        WideStringClass widestring(
                            TRANSLATE(IDS_MENU_QUITTING_DUE_TO_CONN_LOSS_MSG), true);
...
{
/*
** This doesn't work because you can't get a server list without also doing a complete reset of
*wolapi.
*/
#if (0)
    ...
    if (code == CHAT_E_MUSTPATCH) {
        if (mMonitorConnection && mConnected) {
            ...
            /*
            ** If the game has a time limit and players then we will wait until the next map change
            *otherwise we will stop now.
            */
            WWASSERT(PTheGameData != nullptr);
        ...
        */
        if (mConnected) {
            ...
            if (mPatchAvailable) {
                ...
                DlgDownload::DoDialog(TRANSLATE(IDS_WOL_DOWNLOAD),
                                      wolSession->GetPatchDownloadList(), true);
            }
        }
        else {
            /*
...
void WolGameModeClass::HandleNotification(WOLPagedEvent& event)
{
    ...
    case PAGE_TURNED_OFF: {
        ...
        CombatManager::Get_Message_Window()->Add_Message(page->GetPageMessage(),
                                                         COLOR_CONSOLE_TEXT);
        break;
    }
    }
...
{
    /*
    ** If we lost connection without getting a report from WW Online then the only clue we have is
    *that we will fail to
    ** get the game ID by timing out. Since this is pretty much fatal anyway, let's quit and restart
    *and see if things fix
    ** themselves up.
    **
