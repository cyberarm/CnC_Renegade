...
bool cGameData::Is_Valid_Settings(WideStringClass& outMsg, bool check_as_server)
{
    ...
    if (GameModeManager::Find("WOL")->Is_Active()) {
        ...
        if (IsClanGame.Is_True() && (IsTeamChangingAllowed.Is_True() || RemixTeams.Is_True())) {
            ...
            outMsg
                = "Team changing or Team Remix not allowed on a clan game server."; // TRANSLATE(IDS_HOPTERR_NO_TEAMCHANGE);
            return false;
        }
#ifndef FREEDEDICATEDSERVER
    }
    ...
    if (Get_Max_Players() == 0
        && ServerSettingsClass::Get_Game_Mode() != ServerSettingsClass::MODE_WOL) {
#else
    if (Get_Max_Players() == 0) {
#endif
        PRINT_CONFIG_ERROR;
    ...
    if (check_as_server || ConsoleBox.Is_Exclusive()) {
        if (Get_Max_Players() > NetworkObjectClass::MAX_CLIENT_COUNT - 1) {
            ...
            outMsg.Format(TRANSLATE(IDS_HOPTERR_MAXPLAYER_TOOBIG),
                          NetworkObjectClass::MAX_CLIENT_COUNT);
            return (false);
        }
        ...
        */
        ...
        /*
        ** Check the game title for length. The buffers are pretty much all 256 bytes but 100 seems
...
void cGameData::Import_Tier_1_Data(cPacket& packet)
{
    ...
    if (ModPackageMgrClass::Get_Mod_Map_Name_From_CRC(mod_name_crc, map_name_crc, &mod_name,
                                                      &map_name)) {
...
void cGameData::Load_From_Server_Config(LPCSTR config_file)
{
    ...
    // p_ini->Get_String(	INI_SECTION_NAME, "Password", Get_Password(), password,
    // sizeof(password)); Set_Password(password);
    ...
    // i = p_ini->Get_Int(	INI_SECTION_NAME, "IntermissionTimeSeconds",
    // Get_Intermission_Time_Seconds()); Set_Intermission_Time_Seconds(i);
#ifndef FREEDEDICATEDSERVER
    ...
    //
    // b = p_ini->Get_Bool(	INI_SECTION_NAME, "IsClientTrusted",
    // IsClientTrusted.Get());
...
void cGameData::Save_To_Server_Config(LPCSTR config_file)
{
    ...
    // p_ini->Put_Int(	   	INI_SECTION_NAME, "IntermissionTimeSeconds",
    // Get_Intermission_Time_Seconds());
    ...
    // p_ini->Put_Bool(			INI_SECTION_NAME, "IsClientTrusted",
    // IsClientTrusted.Get());
...
void cGameData::Game_Over_Processing(void)
{
    ...
    if (Has_Config_File_Changed()) {
        ...
        //
        cGameDataCnc* test_game_settings
            = (cGameDataCnc*)Create_Game_Of_Type(cGameData::GAME_TYPE_CNC);
...
cGameData* cGameData::Create_Game_Of_Type(GameTypeEnum game_type)
{
    ...
    switch (game_type) {
            //case GAME_TYPE_DEATHMATCH:			p_game_data = new
    cGameDataDeathMatch;			break;
            //case GAME_TYPE_TEAM_DEATHMATCH:	p_game_data = new cGameDataTeamDeathMatch;
    break; case GAME_TYPE_CNC:					p_game_data = new cGameDataCnc;
    break; default:									DIE;
    break;
    }
...
void cGameData::Show_Game_Settings_Limits(void)
{
    ...
    if (!changed) {
        // Proceed the multihud by one line (as we are caching the time render which would usually
        // do this)
        float y = MultiHUDClass::Get_Bottom_Text_Y_Pos();
        y -= 1.2 * charHeight;
        MultiHUDClass::Set_Bottom_Text_Y_Pos(y);
...
void cGameData::Get_Time_Limit_Text(WideStringClass& text)
{
    if (IsIntermission.Is_True()) {
        ...
        text.Format(L"%s: %d", TRANSLATION(IDS_MP_NEXTGAME_COUNTDOWN),
...
void cGameData::Think(void)
{
    ...
    if (cNetwork::I_Am_Server()) {
        ...
        //
        int max = (int)WWMath::Max(cPlayerManager::Count(),
...
void cGameData::On_Game_Begin(void)
{
    ...
    //
    // if (!IS_MISSION && cNetwork::I_Am_Server() && Is_Team_Game() &&
    // IsTeamChangingAllowed.Is_False()) {
    if (!IS_MISSION && cNetwork::I_Am_Server()) {
        ...
        if (IsClanGame.Is_False() && IsTeamChangingAllowed.Is_False() && RemixTeams.Is_True()) {
            ...
            //
            // In non-remixed C&C mode, we have a 50-50 chance of the teams being swapped. I prefer
            // this to simply alternating, because if you alternate and the number of maps in the
...
// {WWASSERT(PTheGameData != nullptr); return PTheGameData;}
cGameData* The_Game(void)
{
    return PTheGameData;
}
...
// {WWASSERT(The_Game()->As_Deathmatch() != nullptr);			return
// The_Game()->As_Deathmatch();} cGameDataTeamDeathMatch *	The_Team_Deathmatch_Game(void)
// {WWASSERT(The_Game()->As_Team_Deathmatch() != nullptr);	return The_Game()->As_Team_Deathmatch();}
