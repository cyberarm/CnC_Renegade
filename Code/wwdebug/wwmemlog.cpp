...
static char* _MemoryCategoryNames[] = {
    ...
    "PhysicsData", "W3dData",  "StaticAllocations", "GameInit",    "Renderer",
    "Network",     "BINK",     "<undefined>",       "<undefined>", "<undefined>",
    "<undefined>",
};
...
#ifdef STEVES_NEW_CATCHER
        /*
        ** This was me trying to be clever and fix the memory leak in the memlog. Unfortunately, the
        *Get_Log member can be called
        ** during the process of exiting the process (IYSWIM) and you get it trying to re-allocate
        *the MemLogClass I just freed.
        ** Solution is just to disable memlog when I'm trying to find memory leaks. ST - 6/18/2001
        *9:51PM
        */
        if (!_MemLogAllocated) {
            atexit(&Release_Log);
...
const int WWMEMLOG_KEY0
    = (unsigned('G') << 24) | (unsigned('g') << 16) | (unsigned('0') << 8) | unsigned('l');
const int WWMEMLOG_KEY1
    = (unsigned('~') << 24) | (unsigned('_') << 16) | (unsigned('d') << 8) | unsigned('3');
