...
// Disable warning about exception handling not being enabled. It's used as part of STL - in a part
// of STL we don't use.
...
char* Addr_As_String(sockaddr_in* addr)
{
    ...
    sprintf(_string, "%d.%d.%d.%d ; %d", (int)(addr->sin_addr.S_un.S_un_b.s_b1),
            ...
            (int)(addr->sin_addr.S_un.S_un_b.s_b4), htonl((int)(addr->sin_port)));
    return (_string);
}
// #endif //WWDEBUG
...
cConnection::cConnection()
    : NumRHosts(0),
      ...
      // MaxRHost(0),							// should be altered if
      // IsServer
      ...
      RefusalHandler(nullptr),
      ServerBrokenConnectionHandler(nullptr),
      ClientBrokenConnectionHandler(nullptr),
      EvictionHandler(nullptr),
      ConnHandler(nullptr),
      ApplicationAcceptanceHandler(nullptr),
      ServerPacketHandler(nullptr),
      ClientPacketHandler(nullptr),
...
void cConnection::Init_As_Client(LPSOCKADDR_IN p_server_address, unsigned short my_port)
{
    ...
    //
    // Now the port is supplied by the firewall negotiation code. We have to use the same port here
    // that the firewall code was bound to.
    //
...
//------------------------------------------------------------------------------------
void cConnection::Init_As_Server(USHORT server_port, int max_players, bool is_dedicated_server,
                                 ULONG addr)
{
...
bool cConnection::Sender_Id_Tests(cPacket& packet)
{
    ...
    if (!cSinglePlayerData::Is_Single_Player()
        ...
        //
        WWDEBUG_SAY(
            ("Warning: Packet sender id (%d) conflicts with actual send address\n", sender_id));
        CombinedStats.StatSample[STAT_DiscardCount]++;
        return false;
    }
...
void cConnection::Set_Packet_Duplication(double percent_duplicated)
{
    ...
    // Globally:
    //
    SimulatedPacketDuplicationPerRANDMAX
        = (UINT)cMathUtil::Round(percent_duplicated / 100.0 * RAND_MAX);
...
bool cConnection::Receive_Packet()
{
    ...
    if (addressee != INVALID_RHOST_ID) {
        ...
        // PRHost[addressee]->Get_Stats().StatSample[STAT_HdrByteRcv] +=
        // cNetUtil::Get_Header_Bytes();
    ...
    if (sender_id != cPacket::UNDEFINED_ID) {
        ...
        if (p_sender_rhost == nullptr) {
            packet.Flush();
            WWDEBUG_SAY(("Packet from nullptr rhost (%d) discarded.\n", sender_id));
            return true;
        }
    }
    ...
    case PACKETTYPE_KEEPALIVE: {
        ...
        if (LocalId == ID_UNKNOWN) {
            WWDEBUG_SAY(
                ("PACKETTYPE_KEEPALIVE flushed due to LocalId == ID_UNKNOWN. Packet id = %d\n",
                 packet_id));
            packet.Flush();
            return true;
        }
    ...
    case PACKETTYPE_ACCEPT_SC: {
        ...
        if (LocalId != ID_UNKNOWN) {
            ...
            CombinedStats.StatSample[STAT_DiscardCount]++;
            WWDEBUG_SAY(
                ("PACKETTYPE_ACCEPT_SC flushed due to LocalId != ID_UNKNOWN. Packet id = %d\n",
                 packet_id));
            Send_Ack(p_from_address, packet_id);
            packet.Flush();
        }
...
int cConnection::Low_Level_Send_Wrapper(cPacket& packet, LPSOCKADDR_IN p_address)
{
    ...
    if (last_packet_len == (int)packet.Get_Compressed_Size_Bytes()) {
        int delta_size = PacketManagerClass::Build_Delta_Packet_Patch(
            last_packet, (unsigned char*)packet.Get_Data(), delta_packet, last_packet_len,
            last_packet_len);
        if (delta_size != -1) {
            WWDEBUG_SAY(
                ("Packet size = %d, last packet delta = %d\n", last_packet_len, delta_size));
        }

        int bytes = PacketManagerClass::Reconstruct_From_Delta(last_packet, fixed_packet,
                                                               delta_packet, last_packet_len);
    ...
    if (cSinglePlayerData::Is_Single_Player()) {
        ...
        //
        bool took = PacketManager.Take_Packet(
            (unsigned char*)packet.Get_Data(), packet.Get_Compressed_Size_Bytes(),
...
int cConnection::Low_Level_Receive_Wrapper(cPacket& packet)
{
    ...
    if (cSinglePlayerData::Is_Single_Player()) {
        ...
        //
        // Try getting a packet from the packet manager. Keep trying for a number of times if we get
        // a socket error. If we don't do this and just fail then we fall out of the receive packet
        // loop and no more packets are received this frame. 15 of these a second and we don't get
        // any packets in at all.
        //
        unsigned char ip_address[4];
        unsigned short port = 0;

        while (error_count < max_errors) {
            bytes = PacketManager.Get_Packet(Sock, (unsigned char*)packet.Get_Data(),
                                             packet.Get_Max_Size(), ip_address, port);

            //
            // A return value of less than 0 indicates a fatal socket error. Try and ditch the
            // offending client.
            //
            if (bytes < 0) {
                bytes = 0;
                ...
                if (IsServer) {
                    ...
                    //
                    // If we try to invoke the broken connection handler when loading a level it
                    // will access the player list which will cause a DataSafe access from the wrong
                    // (main) thread with potentially catastrophic effects. ST - 1/17/2002 11:18AM
                    //
                    if (CanProcess) {
        ...
        ret_code
            = recvfrom(Sock, packet.Get_Data(), packet.Get_Max_Size(), 0,
    ...
    for (int i = 0; i < packet.Get_Num_Sends(); i++) {
        ...
        if (SEND_RESOURCE_FAILURE(ret_code)) {
            ...
            if (rhost_id != INVALID_RHOST_ID) {
                ...
                PRHost[rhost_id]->Get_Stats().StatSample[STAT_AppByteSent]
                    += packet.Get_Compressed_Size_Bytes();
                // PRHost[rhost_id]->Get_Stats().StatSample[STAT_HdrByteSent] +=
                // cNetUtil::Get_Header_Bytes();
...
void cConnection::Send_Ack(LPSOCKADDR_IN p_address, int packet_id)
{
    ...
    WWASSERT(packet_id >= 0);
    WWASSERT(LocalId
             != ID_UNKNOWN); // TSS - bug - asserted here when 50% packet loss or crc failures
    ...
    if (addressee != INVALID_RHOST_ID) {
        ...
        PRHost[addressee]->Get_Stats().StatSample[STAT_UByteSent]
            += packet.Get_Compressed_Size_Bytes();
    }
    ...
    if (LocalId != ID_UNKNOWN) {
        for (int rhost_id = MinRHost; rhost_id <= MaxRHost; rhost_id++) {
            if (PRHost[rhost_id] != nullptr
                ...
                int service_rate
                    = (int)(1000 * (ServiceCount - PRHost[rhost_id]->Get_Last_Service_Count())
                            / (ThisFrameTimeMs - PRHost[rhost_id]->Get_Last_Keepalive_Time_Ms()));
...

    {
        ...
        for (rhost_id = MinRHost; rhost_id <= MaxRHost; rhost_id++) {
            if (PRHost[rhost_id] != nullptr) {
                ...
                for (SLNode<cPacket>* objnode
                     = PRHost[rhost_id]->Get_Packet_List(RELIABLE_RCV_LIST).Head();
                     objnode != nullptr;) {
                    ...
                    int comparison
                        = p_packet->Get_Id() - PRHost[rhost_id]->Get_Reliable_Packet_Rcv_Id();
                    ...
                    if (comparison < 0) {
                        ...
                        /*
                        WWDEBUG_SAY(("%s removing duplicate reliable packet (id %d))
  [ReliablePacketRcvId is %d]\n", IsServer ? "Server" : "Client", p_packet->Get_Id(),
  ...
  default:
     DIE;
                        ...
                        */
                        ...
                        WWASSERT(p_packet->Get_Type() >= PACKETTYPE_FIRST
                                 && p_packet->Get_Type() <= PACKETTYPE_LAST);
...

    {
        ...
        for (rhost_id = MinRHost; rhost_id <= MaxRHost; rhost_id++) {
            if (PRHost[rhost_id] != nullptr) {
                ...
                for (SLNode<cPacket>* objnode
                     = PRHost[rhost_id]->Get_Packet_List(UNRELIABLE_RCV_LIST).Head();
...
*/
                ...
                if (PRHost[rhost_id] != nullptr) {
                    PRHost[rhost_id]->Set_List_Processing_Time(
                        UNRELIABLE_RCV_LIST, TIMEGETTIME() - list_processing_start);
                }
    ...
    time_spent = (int) TIMEGETTIME() - start_time;
         if (time_spent > cNetUtil::Get_Max_Receive_Time_Ms()) {
                 WWDEBUG_SAY(("*** WWNET: Too much time (%d ms)) spent receiving unreliable queued
    packets.\n", time_spent));
    }
         */
...
void cConnection::Set_Bandwidth_Budget_Out(ULONG bw_budget)
{
    ...
    //
    // If we only have very limited bandwidth available then it's silly to have huge send buffers
    // since that can mask problems with outgoing packets until it's too late to recover. ST -
    // 10/17/2001 12:51PM
    //
    if (!cSinglePlayerData::Is_Single_Player()) {
...
void cConnection::Service_Send(bool is_urgent)
{
        ...
        if (p_rhost != nullptr) {
            ...
            for (SLNode<cPacket>* objnode = p_rhost->Get_Packet_List(RELIABLE_SEND_LIST).Head();
                ...
                if (Is_Time_To_Resend_Packet_To_Remote_Host(p_packet, p_rhost)) {
                    ...
                    p_rhost->Get_Stats().StatSample[STAT_RByteSent]
                        += p_packet->Get_Compressed_Size_Bytes();

                    //
                    // Send!
                    //
                    // if (p_packet->Get_Resend_Count() > 0) {
                    // if (p_packet->Get_Resend_Count() > 0 && p_packet->Get_Resend_Count() % 10 ==
                    // 0) { WWDEBUG_SAY(("Resending packet %d to %s after %dms. Resent %d times\n",
                    // p_packet->Get_Id(), Addr_As_String(&(p_rhost->Get_Address())),
                    ...
                    */
                    ...
                    // In a LAN game, if resend time is low. Say 4 ms, and we only resend 50 times,
                    // thats 200ms total before the game craps out. Any kind of network or machine
                    // hiccup could cause the game to quit. This is an experimant to see if that's
                    // what is causing our disconnect at game start.
                    //
                    // As it happens, the main thread can get lost loading textures for several
                    // seconds at the end of a game load so some sort of overall timeout is
                    // neccessary.
                    //
                    // TSS 09/24/01 - new loading lags worse than 10 seconds... bumping it up
                    //                to 20s.
                    //
                    // ST 1/24/2002 2:16PM. Can't time out players when we are loading or we get
                    // DataSafe access from the wrong thread.
                    //
                    if (Is_Packet_Too_Old(p_packet, p_rhost) && CanProcess) {

                        WWDEBUG_SAY(("*** WWNET: Connection timed out - assuming connection to "
                                     "rhost %d is broken.\n",
                                     rhost_id));
                        WWDEBUG_SAY(
                            ("*** WWNET: ThisFrameTimeMs - p_packet->Get_First_Send_Time() == %d\n",
                             ThisFrameTimeMs - p_packet->Get_First_Send_Time()));
                        //
                        // Define the connection as broken
                        //
                        WWDEBUG_SAY(("*** Breaking connection on packet %d ... internal ping = "
                                     "%d/%d/%d ms, ResendTimeoutMs = %d ms\n",
                        ...
                        Destroy_Connection(rhost_id);
                        // WWDEBUG_SAY(("*** WWNET: Exceeded maximum resends (%d)) - assuming
                        // connection to rhost %d is broken.\n", 	cNetUtil::MAX_RESENDS, rhost_id));

                        if (IsServer) {
                            // Server_Broken_Connection_Handler(rhost_id); // Inform app level of
                            // this disaster
                            WWASSERT(ServerBrokenConnectionHandler != nullptr);
                            ServerBrokenConnectionHandler(rhost_id);
                        }
                        else {
                            // Client_Broken_Connection_Handler(); // Inform app level of this
                            // disaster
    ...
    for (rhost_id = MinRHost; rhost_id <= MaxRHost; rhost_id++) {
        ...
        if (p_rhost != nullptr) {
            ...
            for (SLNode<cPacket>* objnode = p_rhost->Get_Packet_List(UNRELIABLE_SEND_LIST).Head();
                ...
                p_rhost->Get_Stats().StatSample[STAT_UByteSent]
                    += p_packet->Get_Compressed_Size_Bytes();
            ...
            // destroy all
            for (objnode = p_rhost->Get_Packet_List(UNRELIABLE_SEND_LIST).Head(); objnode != nullptr;
                 objnode = objnode->Next()) {
    ...
    if (IsServer && NumRHosts > 0 && sample_time_ms > cNetUtil::NETSTATS_SAMPLE_TIME_MS) {
        ...
        for (int statistic = 0; statistic < STAT_COUNT; statistic++) {
            for (rhost_id = MinRHost; rhost_id <= MaxRHost; rhost_id++) {
                if (PRHost[rhost_id] != nullptr) {
                    CombinedStats.StatSample[statistic]
                        += PRHost[rhost_id]->Get_Stats().StatSnapshot[statistic];
                }
            }

            AveragedStats.StatSample[statistic]
                = cMathUtil::Round(CombinedStats.StatSample[statistic] / (double)NumRHosts);
        }
    ...
    for (rhost_id = MinRHost; rhost_id <= MaxRHost; rhost_id++) {
        if (PRHost[rhost_id] != nullptr) {
            bool is_updated
                = PRHost[rhost_id]->Get_Stats().Update_If_Sample_Done(ThisFrameTimeMs, false);
...
bool cConnection::Is_Time_To_Resend_Packet_To_Remote_Host(const cPacket* packet, cRemoteHost* rhost)
{
    ...
    if (ThisFrameTimeMs - packet->Get_Send_Time() >= (unsigned long)total_timeout) {
        ...
        // packet->Get_Id(), (int)(ThisFrameTimeMs - packet->Get_Send_Time()), (int)total_timeout,
        // packet->Get_Resend_Count()));
        return (true);
    }
    return (false);
}
...
// MULTI_SENDS(cNetUtil::Get_Default_Multi_Sends()),		//if
// (!Application_Acceptance_Handler(packet)) {