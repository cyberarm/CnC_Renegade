...
void cPacket::Construct_Full_Packet(cPacket& full_packet, cPacket& src_packet)
{
    ...
    memcpy(full_packet.Get_Data() + PACKET_HEADER_SIZE, src_packet.Get_Data(),
    ...
    // Only CRC the meaningful data in the buffer - not the other 1300ish bytes as well. ST -
    // 9/19/2001 11:18PM
#ifndef WRAPPER_CRC
...
void cPacket::Construct_App_Packet(cPacket& packet, cPacket& full_packet)
{
    ...
    memcpy(packet.Get_Data(), full_packet.Get_Data() + PACKET_HEADER_SIZE,
    ...
    if (((bit_size / 8) + PACKET_HEADER_SIZE) < MAX_BUFFER_SIZE) {
        ...
        //
        int local_crc
            = CRC::Memory((BYTE*)(full_packet.Get_Data() + sizeof(CRC_PLACEHOLDER)),
