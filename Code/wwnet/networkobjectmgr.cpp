...
{
    WWASSERT(_NewDynamicID >= NETID_DYNAMIC_OBJECT_MIN
             && _NewDynamicID <= NETID_DYNAMIC_OBJECT_MAX);
...
void NetworkObjectMgrClass::Restore_Dirty_Bits(int client_id)
{
    ...
    for (int index = 0; index < _ObjectList.Count(); index++) {
        ...
        WWASSERT(p_object != NULL);
        BYTE generic_bits
            = p_object->Get_Object_Dirty_Bits(NetworkObjectClass::MAX_CLIENT_COUNT - 1); // TSS2001e
