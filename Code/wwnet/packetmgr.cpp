 ...
 *   9/24/2001 1:36PM ST : Created                                                             *
 *=============================================================================================*/
int PacketManagerClass::Build_Delta_Packet_Patch(unsigned char* base_packet,
                                                 ...
                                                 int add_packet_size)
{
    ...
    **
    ** The source and delta packets are compared 8 bytes at a time and am equality bit is set in the
    *output stream
    ** for each block of 8. Then, for each non-matching block, a per byte bit is set in the output
    *stream to indicate
    ** whether a byte can be used from the base packet or if a patch byte is needed. Finally, patch
    *bytes are added to
    ...
    *build_delta_ptr = 0;
    ...
    if (base_packet_size == add_packet_size) {
        ...
        */
        ...
        /*
        ** If none of the chunks were the same then this is a pointless endevour. Mark in the header
        *that we don't have
        ...
        */
        ...
        /*
        ** Try a byte for byte check in the chunks that don't match. For any bytes that are
        *different, save the changed value
        ...
        */
        ...
        /*
        ** OK, now we have bitfields that mark the differences. A 1 in the bitfield means that the
        *corresponding chunk or byte
        ** in the add packet is the same as in the base packet.
        ** Throw the non-matching bytes onto the end of the bitfield and we have a complete delta
        *patch.
        */
 ...
 *=============================================================================================*/
int PacketManagerClass::Reconstruct_From_Delta(unsigned char* base_packet,
                                               unsigned char* reconstructed_packet,
                                               unsigned char* delta_packet, int base_packet_size,
                                               int& delta_size)
{
    if (base_packet == nullptr) {
        WWDEBUG_SAY(("*** WARNING: MALFORMED PACKET - PacketManagerClass::Reconstruct_From_Delta "
                     "-- Bad base packet\n"));
        return (0);
    }
    ...
    /*
    ** If there is chunk info then extract that first. Use it to copy the approprate parts of the
    *base packet into the add packet.
    */
    if (header->ChunkPack) {
        for (int i = 0; i < base_packet_size - 7; i += 8) {
        ...
        /*
        ** If the packet isn't an even number of chunks long then pull out an extra bit and throw it
        *away. It should always be 0.
        */
        if ((base_packet_size & 7) != 0) {
            // #ifdef WWDEBUG
            unsigned char bitty =
            ...
            if (bitty != 0) {
                WWDEBUG_SAY(("*** WARNING: MALFORMED PACKET - "
                             "PacketManagerClass::Reconstruct_From_Delta -- Bad bitty\n"));
                return (0);
            }
            pm_assert(bitty == 0);
        }
...
{
    {
        ...
        */
            ...
            */
            ...
            for (i = 0; i < NumSendBuffers; i++) {
                if (SendBuffers[index].PacketLength != 0) {
                    ...
                    pm_assert(header->NumPackets > 0
                              && header->NumPackets <= PACKET_MANAGER_MAX_PACKETS);
            ...
            /*
            ** If we found a packet, see if there are more packets of the same size that we can
            *package up with it.
            */
            if (base_index != -1 && NumPackets > 1) {
                if ((new_length + length + sizeof(PacketDeltaHeaderStruct)) < PACKET_MANAGER_MTU) {
                    ...
                    for (int j = i; j < NumSendBuffers; j++) {
                        ...
                        /*
                        ** If we can't fit any more packets in this packet then break out of the
                        *loop and send it as is.
                        */
                        ...
                        /*
                        ** Is this packet the same length? If so, it might be suitable for
                        *coagulation.
                        */
                        if (SendBuffers[index].PacketLength == length
                            && SendBuffers[index].PacketSendSocket == socket) {
                            ...
                            */
                            if (SendBuffers[base_index].Port == SendBuffers[index].Port
                                ...
                                /*
                                ** See if using a delta of the two packets would be smaller than
                                *including the whole packet.
                                */
                                int bytes = Build_Delta_Packet_Patch(
                                    base_packet, SendBuffers[index].PacketBuffer->Buffer,
                                    DeltaPacket, length, length);
                                if (bytes < length && AllowDeltas) {
                                    ...
                                    */
                                    ...
                                    new_length += bytes;
                                    // WWDEBUG_SAY(("Added delta packet %d to metapacket - %d
                                    // bytes\n", index, bytes));
                                    header->NumPackets++;
                                    pm_assert(header->NumPackets > 0
                                              && header->NumPackets <= PACKET_MANAGER_MAX_PACKETS);
                                    ...
                                    /*
                                    ** The delta was no better than the original. Stick a 0 byte in
                                    *to say it's not a delta and then
                                    ** Copy the whole thing.
                                    */
                                    PacketDeltaHeaderStruct* delta_header
                                        = (PacketDeltaHeaderStruct*)next_packet_pos;
                                    ...
                                    memcpy(next_packet_pos, SendBuffers[index].PacketBuffer,
                                           length);
                                    next_packet_pos += length;
                                    new_length += length;
                                    // WWDEBUG_SAY(("Added secondary packet %d to metapacket - %d
                                    // (+1) bytes\n", index, length));
                                    header->NumPackets++;
                                    pm_assert(header->NumPackets > 0
                                              && header->NumPackets <= PACKET_MANAGER_MAX_PACKETS);
                                ...
                                /*
                                ** Discard the packet buffer now that we have added this packet to
                                *the current staging buffer area.
                                */
                                SendBuffers[index].PacketLength = 0;
                                NumPackets--;
                                // WWDEBUG_SAY(("NumPackets = %d (discarding secondary packet index
                                // %d)\n", NumPackets, index));
                                ...
                                */
                                if ((new_length + length + sizeof(PacketDeltaHeaderStruct))
                                    >= PACKET_MANAGER_MTU) {
                                    break;
                                }
                            }
        ...
        /*
        ** Merge ready buffers where possible. This is the step that adds runs of different sizeed
        *packets into the same packet.
        */
        if (AllowCombos) {
            for (i = 0; i < NumSendBuffers; i++) {
                if (SendBuffers[i].PacketReady) {

                    PacketPackHeaderStruct* current_header
                        = (PacketPackHeaderStruct*)SendBuffers[i].PacketBuffer;
                    int current_len = SendBuffers[i].PacketSendLength;
                    socket = SendBuffers[i].PacketSendSocket;

                    /*
                    ** See if we can find another packet for the same destination that will fit into
                    *the same buffer.
                    */
#ifdef WWDEBUG
                    ...
                    for (int j = i + 1; j < NumSendBuffers; j++) {
                        if (SendBuffers[j].PacketReady
                            && SendBuffers[j].PacketSendSocket == socket) {
                            if (SendBuffers[i].PacketSendLength + SendBuffers[j].PacketSendLength
                                < PACKET_MANAGER_MTU) {
                                if (SendBuffers[i].Port == SendBuffers[j].Port
                                    ...
                                    unsigned char* dest_ptr
                                        = &SendBuffers[i].PacketBuffer->Buffer[current_len];
                                    memcpy(dest_ptr, SendBuffers[j].PacketBuffer,
                                           SendBuffers[j].PacketSendLength);
       ...
       // ((PacketPackHeaderStruct*)&PacketBuffers[j][0])->NumPackets, i)); WWDEBUG_SAY(("Packet %d
       // has %d packets total\n", i, num_sub_packets));
                                }
                            }
                        }
        ...
        */
        for (i = 0; i < NumSendBuffers; i++) {
            if (SendBuffers[i].PacketReady) {
                ...
                socket = SendBuffers[i].PacketSendSocket;
#ifdef WWDEBUG
                int debug_num_packets
                    = (int)(((PacketPackHeaderStruct*)SendBuffers[i].PacketBuffer)->NumPackets);
                int debug_packet_size
                    = (int)(((PacketPackHeaderStruct*)SendBuffers[i].PacketBuffer)->PacketSize);
                ...
                // WWDEBUG_SAY(("Sending packet %d (%d bytes) to %s. Packet has %d packets of %d
                // bytes each\n", i, PacketSendLength[i], Addr_As_String(&addr), debug_num_packets,
                // debug_packet_size));
#endif // WWDEBUG

#ifdef WRAPPER_CRC

                unsigned long crc = CRC::Memory((unsigned char*)SendBuffers[i].PacketBuffer,
                ...
                *((unsigned long*)crc_and_buffer) = crc;
                ...
                Register_Packet_Out(&SendBuffers[i].IPAddress[0], SendBuffers[i].Port,
                                    SendBuffers[i].PacketSendLength + UDP_HEADER_SIZE + sizeof(crc),
                                    0);
                int result
                    = sendto(socket, crc_and_buffer, SendBuffers[i].PacketSendLength + sizeof(crc),
                             0, (LPSOCKADDR)&addr, sizeof(SOCKADDR_IN));

#else // WRAPPER_CRC

                Register_Packet_Out(&SendBuffers[i].IPAddress[0], SendBuffers[i].Port,
                                    SendBuffers[i].PacketSendLength + UDP_HEADER_SIZE, 0);
                int result = sendto(socket, (const char*)SendBuffers[i].PacketBuffer,
                                    SendBuffers[i].PacketSendLength, 0, (LPSOCKADDR)&addr,
                                    sizeof(SOCKADDR_IN));
                ...
                if (result == SOCKET_ERROR) {
                    if (WSAGetLastError() != WSAEWOULDBLOCK) {
                        ...
                        /*
                        ** No more room for outgoing packets. Unfortunately, this means we lose the
                        *lot.
                        */
                ...
                */
                ...
                //	garbage[i] = rand();
                // }
                // sendto(socket, (const char*)garbage, 540, 0, (LPSOCKADDR) &addr,
                // sizeof(SOCKADDR_IN));
            }
        }
        Update_Stats();
 ...
 *=============================================================================================*/
bool PacketManagerClass::Break_Packet(unsigned char* packet, int original_packet_size,
                                      unsigned char* ip_address, unsigned short port)
{
    ...
    */
    for (int i = 0; i < num_packets - 1; i++) {
        ...
        /*
        ** If this is a delta packet then we need to reconstruct it before copying it into the
        *buffer.
        */
        if (delta_header->ChunkPack || delta_header->BytePack) {
            ...
            if (bytes != packet_size) {
                WWDEBUG_SAY(("*** WARNING: MALFORMED PACKET - PacketManagerClass::Break_Packet -- "
                             "bytes !=  packet_size\n"));
                return (false);
            }
            if (delta_size <= 0) {
            ...
            */
            ...
            memcpy(&ReceiveBuffers[NumReceivePackets].ReceiveHoldingBuffer[0], packet_ptr,
                   packet_size);
            ReceiveBuffers[NumReceivePackets].ReceivePacketLength = packet_size;
            packet_ptr += packet_size;
            NumReceivePackets++;
    ...
    */
    if (more_packets && NumReceivePackets < PACKET_MANAGER_RECEIVE_BUFFERS) {
        if (!Break_Packet(packet_ptr, original_packet_size - bytes_pulled_from_packet, ip_address,
                          port)) {
            return (false);
        }
    }
 ...
 *=============================================================================================*/
int PacketManagerClass::Get_Packet(SOCKET socket, unsigned char* packet_buffer,
                                   int packet_buffer_size, unsigned char* ip_address,
                                   unsigned short& port)
{
    {
        WWPROFILE("Pmgr Get");
        ...
        if (NumReceivePackets == 0) {
            ...
            if (result == 0 && bytes != 0) {
                ...
                bytes = recvfrom(socket, (char*)packet_buffer, packet_buffer_size, 0,
                                 (LPSOCKADDR)&addr, &address_size);
                if (bytes > 0) {
#ifndef WRAPPER_CRC
                    Register_Packet_In((unsigned char*)&addr.sin_addr.s_addr, addr.sin_port,
                                       bytes + UDP_HEADER_SIZE, 0);
#endif // WRAPPER_CRC

#ifdef WRAPPER_CRC
                    unsigned long crc
                        = CRC::Memory((unsigned char*)packet_buffer + 4, bytes - sizeof(crc));
#if (1)
                    /*
                    ** Reverse byte order to prevent the demo from having the same CRC as the game.
                    */
                    ...
                    if (crc != *((unsigned long*)packet_buffer)) {
                        WWDEBUG_SAY(
                            ("PMC::Get_Packet: Socket %d, received packet %d bytes long from %s\n",
                        ...
                        // WWDEBUG_SAY(("PMC::Get_Packet: Socket %d, received packet %d bytes long
                        // from %s\n", socket, bytes, Addr_As_String(&addr)));
                        ReceiveSocket = socket;
                        // WWDEBUG_SAY(("Breaking packet %d bytes long from %s\n", bytes,
                        // Addr_As_String(&addr)));
                        bool broken
                            = Break_Packet(packet_buffer, bytes,
                                           (unsigned char*)&addr.sin_addr.s_addr, addr.sin_port);
                        if (!broken) {
                    ...
                    if (bytes == SOCKET_ERROR && WSAGetLastError() != WSAEWOULDBLOCK) {
                        ...
                        WWDEBUG_SAY(
                            ("PacketManagerClass - recvfrom failed with error WSAEWOULDBLOCK\n",
                             WSAGetLastError()));
                    }
                }
            }
        ...
        if (CurrentPacket < NumReceivePackets && socket == ReceiveSocket) {
            /*
            ** Copy the current packet into the return buffer. We have to zero out the rest of the
            *buffer or the CRC won't come out
            ** right. Lordy. FIxed in CRC code now ST - 9/24/2001 3:36PM
            */
            int size = (int)ReceiveBuffers[CurrentPacket].ReceivePacketLength;
            memcpy(packet_buffer, &ReceiveBuffers[CurrentPacket].ReceiveHoldingBuffer[0],
                   min(size, packet_buffer_size));
 ...
 *   10/9/2001 8:54AM ST : Created                                                             *
 *=============================================================================================*/
int PacketManagerClass::Get_Stats_Index(unsigned long ip_address, unsigned short port,
                                        bool can_create)
{
    /*
    ...
    */
    if (time - LastStatsUpdate > StatsFrequency || forced) {
        ...
        */
        for (int i = 0; i < BandwidthList.Count(); i++) {
            ...
            if (stats->CompressedBytesOut) {
                ...
                stats->CompressedBandwidthOut
                    = (int)((1000 * stats->CompressedBytesOut * 8) / (float)StatsFrequency);
                stats->CompressedBytesOut = 0;
            ...
            if (stats->CompressedBytesIn) {
                ...
                stats->CompressedBandwidthIn
                    = (int)((1000 * stats->CompressedBytesIn * 8) / (float)StatsFrequency);
                stats->CompressedBytesIn = 0;
                TotalCompressedBandwidthIn += stats->CompressedBandwidthIn;
            }

            if (stats->UncompressedBytesOut) {
                // stats->UncompressedBandwidthOut = (stats->UncompressedBytesOut * 8) /
                // StatsFrequency;
                stats->UncompressedBandwidthOut
                    = (int)((1000 * stats->UncompressedBytesOut * 8) / (float)StatsFrequency);
                stats->UncompressedBytesOut = 0;
                TotalUncompressedBandwidthOut += stats->UncompressedBandwidthOut;
            }

            if (stats->UncompressedBytesIn) {
                // stats->UncompressedBandwidthIn = (stats->UncompressedBytesIn * 8) /
                // StatsFrequency;
                stats->UncompressedBandwidthIn
                    = (int)((1000 * stats->UncompressedBytesIn * 8) / (float)StatsFrequency);
                stats->UncompressedBytesIn = 0;
        ...
        */
        ...
        // TotalUncompressedBandwidthOut)); WWDEBUG_SAY(("TotalCompressedBandwidthOut = %d bits per
        // second\n", TotalCompressedBandwidthOut)); WWDEBUG_SAY(("TotalUncompressedBandwidthIn = %d
        // bits per second\n", TotalUncompressedBandwidthIn));
        // WWDEBUG_SAY(("TotalCompressedBandwidthIn = %d bits per second\n",
        // TotalCompressedBandwidthIn)); unsigned long comp_out = 100 - ((100 *
        // TotalCompressedBandwidthOut) / TotalUncompressedBandwidthOut); unsigned long comp_in =
        // 100 - ((100 * TotalCompressedBandwidthIn) / TotalUncompressedBandwidthIn);
