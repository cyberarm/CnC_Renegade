...
void BandwidthBalancerClass::Adjust(cConnection* connection, bool is_dedicated)
{
    ...
    **
    ** Do a pass and assign bandwidth to clients based on priority. Boost bandwidth to players
    *currently loading. Clip bandwidth to
    ...
    */
    if (connection != NULL) {
        ...
        for (int i = connection->Get_Min_RHost() + (is_dedicated ? 0 : 1);
             i <= connection->Get_Max_RHost(); i++) {
            ...
            */
            ...
            if (NumClients) {
                ...
                */
                ...
                unsigned long total_bbo_allocated = Allocate_Bandwidth(
                    average_priority, bw_adjust, connection->Get_Bandwidth_Budget_Out());

                /*
                ** Ok, we have a rough bandwidth allocation. If we allocated too much or too little
                ...
                */
                ...
                if (bw_adjust) {
                    ...
                    /*
                    ** If we allocated more or less than we have then keep adjusting until it comes
                    *out at about 95%.
                    */
                    int tries = 5;
                    while ((diff < 0 || percent_diff > 10) && tries >= 0) {
                        // WWDEBUG_SAY(("Allocated too much or too little bandwidth. total_bbo = %d,
                        // total_bbo_allocated = %d, bw_adjust = %d\n", total_bbo,
                        // total_bbo_allocated, bw_adjust));
                        total_bbo_allocated = Allocate_Bandwidth(
                            average_priority, bw_adjust, connection->Get_Bandwidth_Budget_Out());
                        diff = total_bbo - total_bbo_allocated;
                        percent_diff = (100 * abs(diff)) / total_bbo;
                        bw_adjust = bw_adjust * (100 + ((100 * diff) / (((int)total_bbo * 9) / 10)))
                            / 100;
                        if (bw_adjust <= 0) {
                            break;
                        }
                    ...
                    */
                    if (diff < 0 && percent_diff > 3) {
                        WWDEBUG_SAY(("***** WARNING - BandwidthBalancer: Insufficient bandwidth "
                                     "for the number of clients in the game *********\n"));
                        WWDEBUG_SAY(("Allocated too much bandwidth. total_bbo = %d, "
                                     "total_bbo_allocated = %d, bw_adjust = %d\n",
                                     total_bbo, total_bbo_allocated, bw_adjust));
                    }

                    /*
                    ** Oh well, let's assume that's right. Set the bandwidth budgets back into the
                    *rhosts.
                    */
                    int index = 0;
                    for (int i = connection->Get_Min_RHost() + (is_dedicated ? 0 : 1);
    ...
    */
    for (int i = 0; i < NumClients; i++) {
        ...
        */
        ...
        /*
        ** Boost the bandwidth if the client is loading. He doesn't need it now but there will be a
        *huge demand for
...
void BandwidthBalancerClass::Adjust_Connection_Budget(cConnection* connection)
{
    ...
    /*
    ** Check every n seconds and see if we had any send errors that would be caused by sending too
    *much data.
    */
    if (PacketManager.Get_Error_State() == PacketManagerClass::STATE_WS_BUFFERS_FULL) {
        if (time - _last_time > 10000) {
            ...
            WWDEBUG_SAY(("*** WARNING BandwidthBalancerClass - Adjusting Server connection BBO "
                         "from %d to %d due to send overflow ***\n",
                         bbo, new_bbo));
        }
    }
}
