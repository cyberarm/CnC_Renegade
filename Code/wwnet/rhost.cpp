...
{
    WWASSERT(list_type == RELIABLE_SEND_LIST || list_type == RELIABLE_RCV_LIST
             || list_type == UNRELIABLE_SEND_LIST || list_type == UNRELIABLE_RCV_LIST);
    ...
    for (objnode = PacketList[list_type].Head(); objnode != nullptr;) {
        ...
        if (packet_id == p_packet->Get_Id()) {
            ...
            if (list_type == RELIABLE_SEND_LIST) { // &&
                ...
                // Packets that require a resend shouldn't count towards ping time calculations. It
                // may be being removed because the ACK to the first send just came in and if we
                // just resent it then the ping time will look really low so we get biased towards a
                // low resend timeout value on connections of variable quality. ST - 12/7/2001
                // 12:48PM
                if (p_packet->Get_Resend_Count() == 0 || NumInternalPings == 0) {
                    ...
                    if (p_packet->Get_Resend_Count() != 0) {
                        ...
                        // If we are not getting any timing info at all then we need to do
                        // something. Use the first send time. It's going to make it big but that
                        // should cut down on the resends and let us get better timing info.
                    ...
                    NumInternalPings++;
                    if (NumInternalPings > 0) {
                        AverageInternalPingtimeMs
                            = cMathUtil::Round(TotalInternalPingtimeMs / (double)NumInternalPings);
                    }
                    else {
                        AverageInternalPingtimeMs = 0;
                    }
...
void cRemoteHost::Adjust_Flow_If_Necessary(float sample_time_ms)
{
    ...
    if (ratio > MISCUTIL_EPSILON) {
        ...
        /*
        if (Id == 1) {
                WWDEBUG_SAY(("ratio = %-7d / %-7d = %5.2f, TPIncrement = %9.7f, ThresholdPriority =
    ...
    if (ExpectPacketFlood) {
        ...
        //
        // If we are sending way more than we know the remote host can receive then we need to send
        // much less.
        //
        if (actual > desired) {
            ...
            //
            // Another case we have to trap is a low bandwidth connection that has been accidentally
            // flooded by having several frames where we send more stuff than usual. It could also
            // be a remote host that has incorrectly reported his downstream bandwidth and can't
            // receive as much as we think we can send to him. Or it could simply be some temporary
            // condition at either end of the connection or anywhere in between. If this happens we
            // have to cut way back on sends so that the connection can catch up.
            //
            if (AllowExtraModemBandwidthThrottling) {
                if (IsOutgoingFlooded) {

                    //
                    // We recently detected a outbound flooding condition. If we are still flooding
                    // then we have to take further action.
                    //
                    ...
                    if (Is_Outgoing_Flooded()) {
                        ...
                        //
                        // Well, something is wrong. It's kind of hard to say exactly what the
                        // problem is so a remedy is only guesswork. In the short term we can try
                        // reducing the BandwidthMultiplier to clamp down on non-guaranteed packets.
                        // If the problem persists then we will have to reduce MaximumBps too.
                        //
                        NumOutgoingFloods++;
                        NextOutgoingFloodActionTime = 0;
                        Dam_The_Flood();
...
bool cRemoteHost::Is_Outgoing_Flooded(void)
{
    ...
    //
    // 1. Look for a spike in the ping times. If we are sending more than the remote host can
    // receive then ping times will quickly rise as each packet backs up at the receive end and thus
    // gets ack'd later than usual.
    //
    if (ExtendedAverageCount) {
        ...
        if ((LastAveragePingTime > 1500 && MaximumBps < 100000)
            ...
            //
            // Ping time is bigger than we expect. If we are still receiving stuff from this host
            // then chances are we are flooding him.
            //
            if (TIMEGETTIME() - LastContactTime < 1000) {
                if (!IsOutgoingFlooded) {
                    WWDEBUG_SAY(("Detected abnormal ping times - assuming outbound connection to "
                                 "rhost %d is flooded\n",
                                 Id));
    ...
    //
    // 2. An excessive number of packets in the out queue that are older than the average ping time.
    // Since acks aren't coming back we resend more and so exacerbate the problem.
    //
    int total_in_queue = PacketList[RELIABLE_SEND_LIST].Get_Count();

    // Let's say that if more than 90% of the packets in the queue have been resent then there is a
    // problem.
    if (total_in_queue > 20 && (TotalResentPacketsInQueue * 10) > (total_in_queue * 9)) {
        //
        // More resends than we expect. If we are still receiving stuff from this host then chances
        // are we are flooding him.
        //
        if (TIMEGETTIME() - LastContactTime < 1000) {
            WWDEBUG_SAY(("Detected abnormally high number of resends - assuming outbound "
                         "connection to rhost %d is flooded\n",
                         Id));
            WWDEBUG_SAY(("Total packets in queue = %d, queue packets resent = %d\n", total_in_queue,
                         TotalResentPacketsInQueue));
            return (true);
        }
    }
    return (false);
...
void cRemoteHost::Dam_The_Flood(void)
{
    ...
    if (action_time) {
        ...
        //
        // Try reducing BandwidthMultiplier. This will be a temporary change and will be allowed to
        // revert once the connection recovers.
        //
        if (BandwidthMultiplier > 0.5f || NextOutgoingFloodActionTime == 0) {
            ...
            //
            // Try reducing MaximumBps. This will be a permanent change. 30,000ish should be our
            // minimum since the min requirement is a 33600 modem.
            //
            if (MaximumBps > 30000) {
#ifdef WWDEBUG
...
void cRemoteHost::Adjust_Resend_Timeout(void)
{
    if (NumInternalPings > 0) {
        ...
        if (MaxInternalPingtimeMs && AverageInternalPingtimeMs) {
            int candidate_resend_timeout_ms
                = min(3 * AverageInternalPingtimeMs, (int)(1.3f * (float)MaxInternalPingtimeMs));
            ...
            //
            // Keep track of the last average we calculated plus the average ping over the life of
            // the connection.
            //
