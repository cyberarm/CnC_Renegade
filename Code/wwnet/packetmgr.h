...
/*
** Header used at the beginning of a packet to identify the number of packets packed in this packet
*IYSWIM.
*/
...
/*
** This class intercepts packets at the lowest level and applies delta based compression and packet
*combining to reduce
** low level bandwidth while being transparent to higher levels in the application.
**
** All packets are prefixed by a PacketHeaderStruct which is 2 bytes long. So sending a single
*packet incurs an extra
** overhead of two bytes. Normally, however, multiple small packets can be combined into a single
*large one which saves
...
public:
    ...
    */
    bool Take_Packet(unsigned char* packet, int packet_len, unsigned char* dest_ip,
                     unsigned short dest_port, SOCKET socket);
    int Get_Packet(SOCKET socket, unsigned char* packet_buffer, int packet_buffer_size,
...
private:
    ...
    */
    static int Build_Delta_Packet_Patch(unsigned char* base_packet, unsigned char* add_packet,
                                        ...
                                        int add_packet_size);
    static int Reconstruct_From_Delta(unsigned char* base_packet,
                                      unsigned char* reconstructed_packet,
                                      unsigned char* delta_packet, int base_packet_size,
                                      int& delta_size);
    bool Break_Packet(unsigned char* packet, int packet_len, unsigned char* ip_address,
                      unsigned short port);
    ...
    */
    ...
    void Register_Packet_In(unsigned char* ip_address, unsigned short port,
                            unsigned long compressed_size, unsigned long uncompressed_size);
    void Register_Packet_Out(unsigned char* ip_address, unsigned short port,
                             unsigned long compressed_size, unsigned long uncompressed_size);
