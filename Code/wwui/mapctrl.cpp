      ...
      PulseDirection(-1.0F)
{
    ...
    TextureClass* texture = WW3DAssetManager::Get_Instance()->Get_Texture(
        "map_edges.tga", TextureClass::MIP_LEVELS_1);
    EdgeRenderer.Set_Texture(texture);
    REF_PTR_RELEASE(texture);

    texture
        = WW3DAssetManager::Get_Instance()->Get_Texture("mapicons.tga", TextureClass::MIP_LEVELS_1);
        ...
        for (int cell_x = 0; cell_x < CloudSize.I; cell_x++) {
            ...
            if (cloud_rect.Width() > 0 && cloud_rect.Height() > 0) {
                ...
                if (Is_Cell_Shrouded(cell_x, cell_y)) {
                    ...
                    delta_rect.Right
                        = ((cloud_rect.Right - (cloud_x_pos + cloud_width)) / cloud_width);
                    delta_rect.Bottom
                        = ((cloud_rect.Bottom - (cloud_y_pos + cloud_height)) / cloud_height);
...
void MapCtrlClass::Set_Map_Texture(const char* filename)
{
    ...
    if (file != NULL) {
        ...
        if (is_valid) {
            texture = WW3DAssetManager::Get_Instance()->Get_Texture(overlay_texture_name,
...
//
////////////////////////////////////////////////////////////////
int MapCtrlClass::Add_Marker(const WCHAR* name, const Vector3& pos, const RectClass& uv_rect,
                             int color)
{
    //
...
{
    ::memset(CloudVector, 0xFF,
             sizeof(uint32) * ((CloudSize.I * CloudSize.J) / sizeof(uint32)) + 1);
    return;
}
