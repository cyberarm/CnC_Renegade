...
void StyleMgrClass::Initialize(void)
{
    ...
    for (int index = 0; index < FONT_MAX; index++) {
        ...
        Fonts[index] = WW3DAssetManager::Get_Instance()->Get_FontChars(
            DEFAULT_FONTS[index].name, point_size, DEFAULT_FONTS[index].is_bold);
    }

    //
    //	Load the backdrop texture name
    //
    BackdropTexture = WW3DAssetManager::Get_Instance()->Get_Texture(DEFAULT_BACKDROP_NAME,
...
void StyleMgrClass::Initialize_From_INI(const char* filename)
{
    ...
    if (ini_file != nullptr) {
        ...
        for (int index = 0; index < count; index++) {
            ...
            ini_file->Get_String(filename, FONT_FILE_SECTION,
                                 ini_file->Get_Entry(FONT_FILE_SECTION, index));
        ...
        for (index = 0; index < FONT_MAX; index++) {
            ...
            //
            Fonts[index]
                = WW3DAssetManager::Get_Instance()->Get_FontChars(font_name, point_size, is_bold);
        }
...
//
////////////////////////////////////////////////////////////////
void StyleMgrClass::Render_Title_Text(const WCHAR* text, Render2DSentenceClass* renderer,
...
//
////////////////////////////////////////////////////////////////
void StyleMgrClass::Render_Text(const WCHAR* text, Render2DSentenceClass* renderer,
                                const RectClass& rect, bool do_shadow, bool do_clip,
                                JUSTIFICATION justify, bool is_enabled, bool is_vcentered)
{
    //
    //	Determine what color to render the text in
    //
    ...
    Render_Text(text, renderer, text_color, shadow_color, rect, do_shadow, do_clip, justify,
                is_vcentered);
    return;
}
...
////////////////////////////////////////////////////////////////
void StyleMgrClass::Render_Text(const WCHAR* text, Render2DSentenceClass* renderer,
                                uint32 text_color, uint32 shadow_color, const RectClass& rect,
                                bool do_shadow, bool do_clip, JUSTIFICATION justify,
                                bool is_vcentered)
{
    //
    //	If necessary, assign the clipping rectangle
...
//
////////////////////////////////////////////////////////////////
void StyleMgrClass::Render_Wrapped_Text(const WCHAR* text, Render2DSentenceClass* renderer,
                                        const RectClass& rect, bool do_shadow, bool do_vcenter,
                                        bool is_enabled)
{
    //
    //	Determine what color to render the text in
...
//
////////////////////////////////////////////////////////////////
void StyleMgrClass::Render_Wrapped_Text_Ex(const WCHAR* text, Render2DSentenceClass* renderer,
                                           const RectClass& rect, bool do_shadow, bool do_vcenter,
                                           bool is_enabled, JUSTIFICATION justify)
{
    //
    //	Determine what color to render the text in
    //
    ...
    //	Render the text
    //
    Render_Wrapped_Text_Ex(text, renderer, text_color, shadow_color, rect, do_shadow, do_vcenter,
                           justify);
    return;
}
...
////////////////////////////////////////////////////////////////
void StyleMgrClass::Render_Wrapped_Text_Ex(const WCHAR* text, Render2DSentenceClass* renderer,
                                           uint32 text_color, uint32 shadow_color,
                                           const RectClass& rect, bool do_shadow, bool do_vcenter,
                                           JUSTIFICATION justify)
{
    //
...
#define COPY_LINE(dest, src_start, src_end)                                                        \
    ...
    while (line_start != nullptr) {
        ...
        //
        Render_Text(curr_text, renderer, text_color, shadow_color, curr_rect, do_shadow, false,
                    justify, false);
...
//
////////////////////////////////////////////////////////////////
void StyleMgrClass::Render_Glow(const WCHAR* text, Render2DSentenceClass* renderer,
                                const RectClass& rect, int radius_x, int radius_y, int color,
                                JUSTIFICATION justify)
{
    //
    //	Get the extents of the text we will be drawing
