...
void TextMarqueeCtrlClass::Create_Text_Renderer(void)
{
    ...
    //
    for (int index = curr_row; (index < CreditLines.Count()) && (text_rect.Top < ClientRect.Bottom);
         index++) {
...
void TextMarqueeCtrlClass::Update_Client_Rect(void)
{
    ...
    ClientRect = Rect;
    ClientRect.Inflate(
        Vector2(-5.0F * StyleMgrClass::Get_X_Scale(), -3.75F * StyleMgrClass::Get_Y_Scale()));
        ...
        for (int index = 1; text[index] != 0; index++) {
            ...
            if (text[index] == L'>') {
                ...
                if (::_wcsnicmp(text + 1, TAG_BOLD, ::wcslen(TAG_BOLD)) == 0) {
                    ...
                    //					::lstrcpynW (temp_buffer.Peek_Buffer (),
                    //text, (index + 1) - tag_len);
