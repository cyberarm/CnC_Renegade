...
//
////////////////////////////////////////////////////////////////
void DialogButtonClass::On_Create(void)
{
    return;
}
...
void DialogButtonClass::Create_Bitmap_Button(void)
{
    ...
    TextureClass* texture = WW3DAssetManager::Get_Instance()->Get_Texture(
        "button_test04.tga", TextureClass::MIP_LEVELS_1);
...
void Blit_Section(Render2DClass& renderer, const Vector2& screen_pos, const Vector2& texture_pos,
                  const Vector2& pixels, const Vector2& texture_dimensions)
{
    ...
    RectClass uv_rect(texture_pos.X, texture_pos.Y, texture_pos.X + pixels.X,
                      texture_pos.Y + pixels.Y);
...
void DialogButtonClass::Create_Component_Button(void)
{
    ...
    //	Upper right
    //
    ::Blit_Section(ButtonRenderers[0], Vector2(Rect.Right - edge_width, Rect.Top),
                   Vector2(right - edge_width, 0), Vector2(edge_width, height), texture_dimensions);

    ::Blit_Section(ButtonRenderers[1], Vector2(Rect.Right - edge_width, Rect.Top),
                   Vector2(right - edge_width, dn_start), Vector2(edge_width, height),
                   texture_dimensions);

    //
    //	Lower left
    //
    ::Blit_Section(ButtonRenderers[0], Vector2(Rect.Left, Rect.Bottom - height),
                   Vector2(0, bottom - height), Vector2(edge_width, height), texture_dimensions);

    ::Blit_Section(ButtonRenderers[1], Vector2(Rect.Left, Rect.Bottom - height),
                   Vector2(0, dn_start + bottom - height), Vector2(edge_width, height),
                   texture_dimensions);
    ...
    ::Blit_Section(ButtonRenderers[0], Vector2(Rect.Right - edge_width, Rect.Bottom - height),
                   Vector2(right - edge_width, bottom - height), Vector2(edge_width, height),
                   texture_dimensions);

    ::Blit_Section(ButtonRenderers[1], Vector2(Rect.Right - edge_width, Rect.Bottom - height),
                   Vector2(right - edge_width, dn_start + bottom - height),
                   Vector2(edge_width, height), texture_dimensions);
    ...
    while (remaining_width > 0) {
        ...
        ::Blit_Section(ButtonRenderers[0], Vector2(x_pos, Rect.Top), horz_top_pos, horz_size,
                       texture_dimensions);

        ::Blit_Section(ButtonRenderers[0], Vector2(x_pos, Rect.Bottom - height), horz_bottom_pos,
                       horz_size, texture_dimensions);

        ::Blit_Section(ButtonRenderers[1], Vector2(x_pos, Rect.Top),
                       horz_top_pos + Vector2(0, dn_start), horz_size, texture_dimensions);

        ::Blit_Section(ButtonRenderers[1], Vector2(x_pos, Rect.Bottom - height),
                       horz_bottom_pos + Vector2(0, dn_start), horz_size, texture_dimensions);
    ...
    while (remaining_height > 0) {
        ...
        ::Blit_Section(ButtonRenderers[0], Vector2(Rect.Left, y_pos), vert_left_pos, vert_size,
                       texture_dimensions);

        ::Blit_Section(ButtonRenderers[0], Vector2(Rect.Right - edge_width, y_pos), vert_right_pos,
                       vert_size, texture_dimensions);

        ::Blit_Section(ButtonRenderers[1], Vector2(Rect.Left, y_pos),
                       vert_left_pos + Vector2(0, dn_start), vert_size, texture_dimensions);

        ::Blit_Section(ButtonRenderers[1], Vector2(Rect.Right - edge_width, y_pos),
                       vert_right_pos + Vector2(0, dn_start), vert_size, texture_dimensions);
    ...
    while (remaining_height > 0) {
        ...
        while (remaining_width > 0) {
            ...
            ::Blit_Section(ButtonRenderers[0], Vector2(x_pos, y_pos),
                           Vector2((right / 2) - 4, (bottom / 2) - 4), size, texture_dimensions);

            ::Blit_Section(ButtonRenderers[1], Vector2(x_pos, y_pos),
                           Vector2((right / 2) - 4, dn_start + (bottom / 2) - 4), size,
                           texture_dimensions);
...
void DialogButtonClass::Create_Component_Button2(void)
{
    ...
    //
    RectClass rect = Rect;
    ButtonRenderers[0].Add_Line(Vector2(rect.Left, rect.Bottom), Vector2(rect.Left, rect.Top - 1),
                                1, WHITE);
    ButtonRenderers[0].Add_Line(Vector2(rect.Left, rect.Top), Vector2(rect.Right, rect.Top), 1,
                                WHITE);
    ButtonRenderers[0].Add_Line(Vector2(rect.Right, rect.Top), Vector2(rect.Right, rect.Bottom), 1,
                                BLACK);
    ButtonRenderers[0].Add_Line(Vector2(rect.Right, rect.Bottom), Vector2(rect.Left, rect.Bottom),
                                1, BLACK);

    ButtonRenderers[1].Add_Line(Vector2(rect.Left, rect.Bottom), Vector2(rect.Left, rect.Top - 1),
                                1, BLACK);
    ButtonRenderers[1].Add_Line(Vector2(rect.Left, rect.Top), Vector2(rect.Right, rect.Top), 1,
                                BLACK);
    ButtonRenderers[1].Add_Line(Vector2(rect.Right, rect.Top), Vector2(rect.Right, rect.Bottom), 1,
                                WHITE);
    ButtonRenderers[1].Add_Line(Vector2(rect.Right, rect.Bottom), Vector2(rect.Left, rect.Bottom),
                                1, WHITE);
...
void DialogButtonClass::Create_Text_Renderers(void)
{
    ...
    //	Draw the text
    //
    StyleMgrClass::Render_Text(Title, &TextRenderers[0], Rect, true, true,
                               StyleMgrClass::CENTER_JUSTIFY);
    StyleMgrClass::Render_Text(Title, &TextRenderers[1], Rect, true, true,
                               StyleMgrClass::CENTER_JUSTIFY);
    return;
}
...
//
////////////////////////////////////////////////////////////////
void DialogButtonClass::On_Key_Down(uint32 key_id, uint32 key_data)
{
    switch (key_id) {
    case VK_RETURN:
    case VK_SPACE:
