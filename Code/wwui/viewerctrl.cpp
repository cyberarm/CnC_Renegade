      ...
      InterfaceMode(Z_ROTATION)
{
    ...
    //	NOTE: Near and far clip planes are set to values which will suit a large range of object
    //sizes. 			However, very small objects may pass thru the near clip plane and large objects may
    //pass 			thru the far clip plane (and Z-fighting will result if the ratio is too high). If this
    //			proves to problematic replace the constant settings with a function of
    //object size that 			can be set on a per object basis.
    //
...
void ViewerCtrlClass::Calculate_Camera_Position(void)
{
    ...
    //
    float width = WWMath::Sqrt(BoundingBox.Extent.X * BoundingBox.Extent.X * 4
                               + BoundingBox.Extent.Y * BoundingBox.Extent.Y * 4);
...
void ViewerCtrlClass::On_Frame_Update(void)
{
    ...
    case VIRTUAL_TRACKBALL:
        ...
        if (DialogMgrClass::Is_Button_Down(VK_LBUTTON)) {
            ...
            const Vector2 mouseposition(DialogMgrClass::Get_Mouse_Pos().X,
                                        DialogMgrClass::Get_Mouse_Pos().Y);
