...
void MenuEntryCtrlClass::On_Create(void)
{
    if ((Style & 0xF) == BS_OWNERDRAW) {
        ...
        /*FontCharsClass *font = WW3DAssetManager::Get_Instance()->Get_FontChars ("Lucida Sans
        Unicode", 12); TextRenderer.Set_Font (font); GlowRenderer.Set_Font (font); font->Release_Ref
        ();*/
    }
    ...
    case HILIGHT: {
        ...
        if (curr_time < EndTime) {
            ...
            if (curr_time > (EndTime - 500)) {
                ...
                float red_percent
                    = float(float(curr_time - start_time) / float(EndTime - start_time));
