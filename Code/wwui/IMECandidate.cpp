...
namespace IME
{
    ...
    void IMECandidate::Read(void)
    {
        ...
        if (imc) {
            ...
            if (mCandidates != NULL) {
                ...
                WWDEBUG_SAY(("IMECandidate: Index %d, Style %08lX, Selection %d, PageStart %d, "
                             "PageSize %d, Count %d\n",
                             mIndex, GetStyle(), GetSelection(), GetPageStart(), GetPageSize(),
                             GetCount()));
            }
    ...
    const wchar_t* IMECandidate::GetCandidate(unsigned long index)
    {
        if (index < GetCount()) {
            ...
            MultiByteToWideChar(mCodePage, 0, candString, -1, mTempString,
                                (sizeof(mTempString) / sizeof(wchar_t)));
