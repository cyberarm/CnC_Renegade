...
namespace IME
{
    ...
    bool IMEManager::FinalizeCreate(HWND hwnd)
    {
        ...
        bool isWin98orLater = (osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS)
            && ((osvi.dwMajorVersion > 4)
                || ((osvi.dwMajorVersion == 4) && (osvi.dwMinorVersion >= 10)));
        bool isNT4orLater = (osvi.dwPlatformId == VER_PLATFORM_WIN32_NT)
            && ((osvi.dwMajorVersion > 4)
    ...
    bool IMEManager::ProcessMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam,
                                    LRESULT& outResult)
    {
        if (hwnd != mHWND) {
            return false;
        ...
        case WM_KEYUP:
            if (mInComposition) {
                ...
                else if ((0x30 <= virtualKey && 0x39 >= virtualKey)
                         || (0x41 <= virtualKey && 0x5A >= virtualKey)) {
        ...
        if (numLayouts) {
            ...
            if (iter != layoutList.end()) {
                ...
                while (iter != layoutList.end()) {
                    ...
                    if ((property & (IME_PROP_AT_CARET | IME_PROP_SPECIAL_UI))
                        == (IME_PROP_AT_CARET | IME_PROP_SPECIAL_UI)) {
                        iter++;
                    }
                    else {
                        return *iter;
    ...
    void IMEManager::InputLanguageChanged(HKL hkl)
    {
        ...
        mStartCandListFrom1
            = ((mIMEProperties & IME_PROP_CANDLIST_START_FROM_1) == IME_PROP_CANDLIST_START_FROM_1);
        ...
        WWDEBUG_SAY(
            ("IMEManager: Properties - %s%s%s%s%s\n",
    ...
    void IMEManager::DoComposition(unsigned int dbcs, long compFlags)
    {
        ...
        if (imc) {
            if (compFlags == 0) {
                ...
                if (ReadCompositionString(imc, GCS_RESULTSTR, mResultString,
                ...
                if (compFlags & GCS_COMPREADSTR) {
                    if (ReadCompositionString(imc, GCS_COMPREADSTR, mReadingString,
                                              sizeof(mReadingString))) {
                ...
                if (compFlags & GCS_COMPSTR) {
                    if (ReadCompositionString(imc, GCS_COMPSTR, mCompositionString,
                                              sizeof(mCompositionString))) {
                        WWDEBUG_SAY(
                            ("Composition string '%S'\n", (const wchar_t*)mCompositionString));
                        action = COMPOSITION_CHANGE;
                    }
                }
                ...
                if (compFlags & GCS_COMPATTR) {
                    long size
                        = ReadCompositionAttr(imc, mCompositionAttr, sizeof(mCompositionAttr));
                ...
                if (compFlags & GCS_COMPCLAUSE) {
                    ...
                    long size = ReadCompositionClause(imc, mCompositionClause,
                                                      sizeof(mCompositionClause));
    ...
    bool IMEManager::ReadCompositionString(HIMC imc, unsigned long flag, wchar_t* buffer,
                                           int length)
    {
        if (mUseUnicode) {
            ...
            // Convert to Unicode
            MultiByteToWideChar(mCodePage, 0, (const char*)string, -1, buffer,
    ...
    unsigned long IMEManager::GetGuideline(wchar_t* outString, int length)
    {
        ...
        if (imc) {
            ...
            if ((GL_LEVEL_NOGUIDELINE != level) && outString) {
                if (mUseUnicode) {
                    DWORD size
                        = ImmGetGuideLineW(imc, GGL_STRING, outString, (length * sizeof(wchar_t)));
    ...
    long IMEManager::ConvertClauseForUnicode(unsigned char* mbcs, long length,
