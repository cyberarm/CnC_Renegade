    ...
    if ((Style & LVS_NOCOLUMNHEADER) == 0) {
        ...
        for (int index = 0; index < ColList.Count(); index++) {
            ...
            if (SortType != SORT_NONE && SortColumn == index) {
                ...
                float tri_x_pos = rect.Left + text_extent.X + tri_size
                    + (TRI_SPACE * StyleMgrClass::Get_Y_Scale());
                float tri_y_pos = HeaderRect.Center().Y;

                if (SortType == SORT_ASCENDING) {

                    UnderlineRenderer.Add_Line(
                        Vector2(tri_x_pos - tri_half_size, tri_y_pos + tri_half_size),
                    ...
                    UnderlineRenderer.Add_Line(
                        Vector2(tri_x_pos, tri_y_pos - tri_half_size),
                        Vector2(tri_x_pos - tri_half_size, tri_y_pos + tri_half_size), 1.0F, color);
                }
                else {
            ...
            //	Render the text
            //
            StyleMgrClass::Render_Text(ColList[index]->Get_Name(), &HeaderRenderer, color,
...
void ListCtrlClass::Render_Entry(const RectClass& clip_rect, int col_index, int row_index)
{
    ...
    //	Render the text
    //
    StyleMgrClass::Render_Wrapped_Text(text, &TextRenderer, text_color, RGB_TO_INT32(0, 0, 0), rect,
                                       true, true);
...
//
////////////////////////////////////////////////////////////////
int CALLBACK ListCtrlClass::Default_Sort_Callback(ListCtrlClass* list_ctrl, int item_index1,
...
void ListCtrlClass::Scroll_Page(int direction)
{
    ...
    //
    for (int index = ScrollPos; index >= 0 && index < count; index += direction) {
        //
...
{
    if (old_index < 0 || (old_index >= EntryList.Count()) && new_index < 0
        || (new_index >= EntryList.Count())) {
        return;
    }
...
{
    if (index1 < 0 || (index1 >= EntryList.Count()) && index2 < 0
        || (index2 >= EntryList.Count())) {
        return;
    }
