...
//-----------------------------------------------------------------------------
void cScTextObj::Init(const WideStringClass& text, TextMessageEnum type, bool is_host_admin_message,
    ...
    if ((isAnsi && showLatin) || (!isAnsi && showAsian)) {
        ...
        if (!sender_name.Is_Empty() && !recipient_name.Is_Empty()) {
            ...
            if (Type == TEXT_MESSAGE_PRIVATE) {
                formatted_text.Format(L"%s (%s %s): ", sender_name, TRANSLATION(IDS_MP_TO),
                                      recipient_name);
            }
            else {
