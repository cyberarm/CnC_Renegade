...
bool WebBrowser::InstallPrerequisites(void)
{
    ...
    if (FAILED(hr)) {
        WWDEBUG_SAY(
            ("WOLBrowser component not installed, attempting to locate in run directory\n"));
    ...
    HKEY key;
    LONG result
        = RegOpenKeyEx(HKEY_LOCAL_MACHINE, APPLICATION_SUB_KEY_NAME_URL, 0, KEY_ALL_ACCESS, &key);
    ...
    if (ERROR_SUCCESS != result) {
        ...
        static URLEntry urls[] = {
            { "BattleClans",
              ...
              "cgiclient?ren_clan_manager&request=expand_template&Template=index.html&SKU=3072&"
              "LANGCODE=0&embedded=1" },
            { "BattleClansX",
              "http://renchat2.westwood.com/cgi-bin/"
              "cgiclient?ren_clan_manager&request=expand_template&Template=index.html&SKU=3072&"
              "LANGCODE=0" },
            ...
            { "NetStatus",
              "http://battleclans.westwood.com/cgi-bin/"
              "cgiclient?rosetta&request=do_netstatus&LANGCODE=0&SKU=3072&embedded=1" },
            { "NetStatusX",
              "http://battleclans.westwood.com/cgi-bin/"
              "cgiclient?rosetta&request=do_netstatus&LANGCODE=0&SKU=3072" },
            { "News",
              "http://battleclans.westwood.com/cgi-bin/"
              "cgiclient?rosetta&request=do_news&LANGCODE=0&SKU=3072&embedded=1" },
            { "NewsX",
              "http://battleclans.westwood.com/cgi-bin/"
              "cgiclient?rosetta&request=do_news&LANGCODE=0&SKU=3072" },
            { "Signup",
