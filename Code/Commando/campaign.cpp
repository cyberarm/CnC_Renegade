...
void CampaignManager::Init(void)
{
    ...
    if (campaignINI != NULL) {
        ...
        for (int entry = 0; entry < count; entry++) {
            ...
            campaignINI->Get_String(description, SECTION_CAMPAIGN,
                                    campaignINI->Get_Entry(SECTION_CAMPAIGN, entry));
        ...
        for (int state = 0; state < 100; state++) {
            ...
            if (count != 0) {
                ...
                for (int entry = 0; entry < count; entry++) {
                    StringClass description(0, true);
                    campaignINI->Get_String(description, section_name,
                                            campaignINI->Get_Entry(section_name, entry));
...
void CampaignManager::Continue(bool success)
{
    ...
    if (State == REPLAY_LEVEL) {
        ...
        ScoreScreenGameModeClass* ss
            = (ScoreScreenGameModeClass*)GameModeManager::Find("ScoreScreen");
        if (ss != NULL) {
            ss->Save_Stats();
        }
    ...
    if (State == NOT_IN_CAMPAIGN_STATE || State == REPLAY_SCORE
        || (State >= CampaignFlowDescriptions.Count() - 1)) {
        State = NOT_IN_CAMPAIGN_STATE;
        GameModeManager::Find("Movie")->Deactivate();
...
#define StringMatch(a, b) (!::strncmp(a, b, strlen(b)))
    ...
    if (StringMatch(state_description, "Message ")) {
        ...
        ::sscanf(state_description, "%s %s %s", foo.Peek_Buffer(), filename.Peek_Buffer(),
                 description.Peek_Buffer());
