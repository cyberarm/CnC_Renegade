...
void CnCReferenceMenuClass::On_Init_Dialog(void)
{
    ...
    Input::Get_Translated_Key_Name(
        Input::Get_Primary_Key_For_Function(INPUT_FUNCTION_TEAM_INFO_TOGGLE), name);
    Set_Dlg_Item_Text(IDC_KEY_NAME_01, name);

    Input::Get_Translated_Key_Name(
        Input::Get_Primary_Key_For_Function(INPUT_FUNCTION_BATTLE_INFO_TOGGLE), name);
    Set_Dlg_Item_Text(IDC_KEY_NAME_02, name);

    Input::Get_Translated_Key_Name(Input::Get_Primary_Key_For_Function(INPUT_FUNCTION_ACTION),
                                   name);
    Set_Dlg_Item_Text(IDC_KEY_NAME_03, name);

    Input::Get_Translated_Key_Name(
        Input::Get_Primary_Key_For_Function(INPUT_FUNCTION_EVA_MISSION_OBJECTIVES_TOGGLE), name);
    Set_Dlg_Item_Text(IDC_KEY_NAME_04, name);

    Input::Get_Translated_Key_Name(
        Input::Get_Primary_Key_For_Function(INPUT_FUNCTION_BEGIN_PUBLIC_MESSAGE), name);
    Set_Dlg_Item_Text(IDC_KEY_NAME_05, name);

    Input::Get_Translated_Key_Name(
        Input::Get_Primary_Key_For_Function(INPUT_FUNCTION_BEGIN_TEAM_MESSAGE), name);
...
bool is_team_change_enabled =
        ...
        The_Game()->IsTeamChangingAllowed.Is_True() &&
        (!(GameModeManager::Find("WOL") != nullptr && GameModeManager::Find("WOL")->Is_Active()) ||
The_Game()->IsLaddered.Is_False()) && GameModeManager::Find("Combat") != nullptr &&
        GameModeManager::Find("Combat")->Is_Active();
