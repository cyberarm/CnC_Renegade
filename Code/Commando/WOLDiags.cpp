...
void ShowServer(const RefPtr<Session>& session)
{
    ...
    if (server.IsValid()) {
        ...
        ConsoleFunctionClass::Print("Server: %s:%s  TimeZone: %d  Position:: %f/%f\n",
                                    (const char*)data.connlabel, (const char*)data.name,
                                    data.timezone, data.longitude, data.lattitude);
    }
    else {
...
{
    static Dispatch _dispatch[] = { { "pings", ShowPingServers },
                                    { "server", ShowServer },
                                    { "topic", ShowTopic },
                                    { "ver", ShowWOLVersion },
                                    { NULL, NULL } };
