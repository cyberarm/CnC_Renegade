...
{
public:
    ConsoleFunctionSettingBool(SystemSettingEntryBool* entry)
        : Entry(entry)
    {
    }
...
{
public:
    ConsoleFunctionSettingSlider(SystemSettingEntrySlider* entry)
        : Entry(entry)
    {
    }
    virtual const char* Get_Name(void) { return Entry->Get_Name(); }
...
{
public:
    ConsoleFunctionSettingEnum(SystemSettingEntryEnum* entry)
        : Entry(entry)
    {
    }
    virtual const char* Get_Name(void) { return Entry->Get_Name(); }
...
{
    Selection = (selection >= Get_Enum_Count()) ? Get_Enum_Count() - 1
...
public:
    ...
    {
        return "ENABLE_AUTO_TRANSITIONS - toggles automatic doors and transitions.";
    }
...
public:
    ...
    {
        formatstring.Format(
            "GAMMA_CORRECTION [%d..%d] - Adjusts gamma correction curve for the display.",
            GAMMA_SLIDER_MIN, GAMMA_SLIDER_MAX);
        return (formatstring);
    }
...
public:
    ...
    const char* Get_Help(void)
    {
        formatstring.Format("BRIGHTNESS [%d..%d] - Adjusts brightness for the display.",
...
public:
    ...
    const char* Get_Help(void)
    {
        formatstring.Format("CONTRAST [%d..%d] - Adjusts contrast for the display.",
                            CONTRAST_SLIDER_MIN, CONTRAST_SLIDER_MAX);
        return (formatstring);
    }
...
public:
    ...
    const char* Get_Help(void)
    {
        return "TEXTURE_RESOLUTION [0..7] - sets the texture resolution.";
    }
...
public:
    ...
    const char* Get_Help(void)
    {
        return "SHADOW_MODE <mode> - 0=none 1=blobs 2=blobs+ 3=projected textures";
    }
...
public:
    ...
    const char* Get_Help(void)
    {
        return "SURFACE_EFFECT_DETAIL <level> - 0=off 1=no emitters 2=full";
    }
    virtual int Get_Enum(void) { return SurfaceEffectsManager::Get_Mode(); }
    virtual void Set_Enum(int value)
    {
        SurfaceEffectsManager::Set_Mode((SurfaceEffectsManager::MODE)value);
    }
...
const char* SystemSettingEntryMeshDrawMode::names[]
    = { "Old", "New", "Debug Draw", "Debug Clip", "Box", "None", "DX8 only" };
...
public:
    ...
    const char* Get_Help(void)
    {
        return "NPATCHES_GAP_FILLING_MODE <mode> - 0=disabled 1=enabled 2=force";
    }
...
const char* SystemSettingEntryNPatchGapFillingMode::names[]
    = { "Disabled", "Enabled", "FORCE (Enabled even when not using n-patches)" };
...
const char* SystemSettingEntryNPatches::names[]
    = { "NPatches DISABLED", "NPatches DISABLED", "NPatches level 2",
...
public:
    ...
    const char* Get_Help(void)
    {
        return "PRELIT_MODE <mode> - 0=vertex 1=multi-pass 2=multi-texture.";
    }
...
public:
    ...
    const char* Get_Help(void)
    {
        return "TEXTURE_FILTER_MODE <mode> - 0=bilinear 1=trilinear 2=anisotropic.";
    }
...
void SystemSettings::Shutdown(void)
{
    ...
    /*
    ** Free the system setting so they don't look like memory leaks when we exit. ST - 6/11/2001
    *8:32PM
    */
