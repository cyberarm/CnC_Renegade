...
**
** I need to make this stuff static so that I can templatize the derived class and have all
*expansions use the same data.
*/
...
**
** For each type, we have to declare more than just the class, we have to declare static data too.
*This is taken care of by
** the DECLARE_DATA_SAFE macro.
**
** All data safe type MUST be declared here so that they are constructed after the ::TypeList and
*::TypeListCount statics above.
**
...
{
    /*
    ** This constructor could be called lots of times as new types are added to the data safe. Make
    *sure we don't get an
    ** unexpected reset if we have already added types.
    */
    if (TypeListCount == 0) {

#ifdef THREAD_SAFE_DATA_SAFE
        SafeMutex = CreateMutex(nullptr, false, nullptr);
#else
...
DataSafeEntryClass* GenericDataSafeClass::Get_Entry(DataSafeHandleClass handle)
{
    ...
    */
    ...
    /*
    ** Search through until we find a matching handle. Check to make sure we don't get stuck in an
    *infinite loop.
    */
    int times = 0;
    while (entry_ptr) {
...
int GenericDataSafeClass::Get_Random_List_For_Insertion(int type)
{
    ...
    */
    for (int i = 0; i < NumLists; i++) {
        ds_assert(Safe[i] != nullptr);
        if (Safe[i] != nullptr && (Safe[i]->EntryType == type || Safe[i]->EntryType == -1)
    ...
    */
    ...
    /*
    ** If the list we picked is currently empty then we might need to do some fixup before we can
    *use it.
    */
 ...
 *=============================================================================================*/
void GenericDataSafeClass::Random_Insertion(DataSafeEntryClass* entry_ptr, int list, int type,
                                            bool is_slop)
{
    ds_assert(Safe[list] != nullptr);
    ...
    /*
    ** If this is the first entry to be added to the list then there is no possibility of random
    *insertion. Just stick it
    ** at the beginning.
    */
 ...
 *=============================================================================================*/
void GenericDataSafeClass::Swap_Entries(DataSafeEntryClass* first, DataSafeEntryClass* second,
                                        int type)
{
    ...
    */
    ...
    /*
    ** Swap the objects in memory. Just swap the data and the handle so that the next and prev
...
void GenericDataSafeClass::Shuffle(bool forced)
{
    ...
    */
    ...
    if (forced || time < ShuffleDelay || (time | ShuffleDelay) == 0
        || (time - ShuffleDelay) > SHUFFLE_TIME) {
        ...
        */
        ...
        /*
        ** Now, mod_key contains the "modifier key" used to rekey items. We will use it after we
        *have swapped some stuff around.
        */
        for (i = 0; i < NumLists; i++) {
            ...
            if (Safe[i] != nullptr && Safe[i]->EntryCount > 1) {
                for (j = 0; j < max(Safe[i]->EntryCount / 3, 1); j++) {
                    ...
                    /*
                    ** This code has been replaced by the simplified version below to avoid a
                    *compiler bug in release mode.
                    */
                    // DataSafeEntryClass *first = Get_Entry_By_Index(i, FreeRandom.Get_Int(0,
                    // Safe[i]->EntryCount - 1)); DataSafeEntryClass *second = Get_Entry_By_Index(i,
                    // FreeRandom.Get_Int(0, Safe[i]->EntryCount - 1));
    ...
    for (int i = 0; i < NumLists; i++) {
        ...
        if (Safe[i] != nullptr) {
            if (Safe[i]->EntryCount) {
                ...
                bytes += (sizeof(DataSafeEntryClass) + Get_Type_Size(Safe[i]->EntryType))
                    * Safe[i]->EntryCount;
            }
        }
    }
