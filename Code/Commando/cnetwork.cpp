...
void cNetwork::Init_Client(unsigned short my_port)
{
    ...
    if (IS_SOLOPLAY
        ...
        bbo = cBandwidth::Get_Bandwidth_Bps_From_Type(
    ...
    WWASSERT(PTheGameData != nullptr);
    PClientConnection->Init_As_Client(The_Game()->Get_Ip_Address(), The_Game()->Get_Port(),
                                      my_port);
...
void cNetwork::Compute_Exe_Key(void)
{
    ...
    int succeeded = 0;
    succeeded = ::GetModuleFileName(nullptr, exe_filename, sizeof(exe_filename));
    WWASSERT(succeeded);
...
void cNetwork::Init_Server(void)
{
    ...
    */
    ...
    if (IS_SOLOPLAY
        ...
        unsigned long bw = cBandwidth::Get_Bandwidth_Bps_From_Type(
            (BANDWIDTH_TYPE_ENUM)cUserOptions::Get_Bandwidth_Type());
...
void cNetwork::Server_Send_Packet(cPacket& packet, int mode, int recipient)
{
    ...
    if (recipient == ALL) {
        ...
        for (objnode = cPlayerManager::Get_Player_Object_List()->Head(); objnode;
            ...
            if (p_player->Get_Is_Active().Is_True() && p_player->Is_Human()
                ...
                PServerStatListGroup->Increment_Num_Byte_Sent(client_id - 1, message_type,
        ...
        PServerStatListGroup->Increment_Num_Byte_Sent(recipient - 1, message_type,
        packet.Get_Compressed_Size_Bytes());
        */
    }
...
void cNetwork::Server_Send_Packet_To_All_Connected(cPacket& packet, int mode)
{
    ...
    for (int rhost_id = PServerConnection->Get_Min_RHost();
         rhost_id <= PServerConnection->Get_Max_RHost(); rhost_id++) {
        if (Get_Server_Rhost(rhost_id) != nullptr) {
            ...
            PServerStatListGroup->Increment_Num_Byte_Sent(rhost_id - 1, message_type,
            packet.Get_Compressed_Size_Bytes());
            */
        }
    }
...
void cNetwork::Shell_Command(LPCSTR command)
{
    ...
    HINSTANCE hinst = ShellExecute(nullptr, nullptr, command, nullptr, "", SW_SHOW);
    if ((int)hinst <= 32) {
...
void cNetwork::Connection_Handler(int new_rhost_id)
{
    ...
    cTeam * p_team;
    for (team_node = cTeamManager::Get_Team_Object_List()->Head(); team_node; team_node =
    team_node->Next()) { p_team = team_node->Data(); WWASSERT(p_team != nullptr);
...
{
    SLNode<cPlayer>* objnode;
    for (objnode = cPlayerManager::Get_Player_Object_List()->Head(); objnode != nullptr;
         objnode = objnode->Next()) {
