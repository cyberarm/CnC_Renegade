...
void cGameSpyAuthMgr::Initiate_Auth_Rejection(int player_id)
{
    ...
    if (p_player != NULL) {
        ...
        // p_message->Init(L"CD Authentication failed. Please quit.", TEXT_MESSAGE_PRIVATE, true,
        // HOST_TEXT_SENDER, player_id);
        p_message->Init(TRANSLATE(IDS_MP_CD_AUTH_FAILED), TEXT_MESSAGE_PRIVATE, true,
                        HOST_TEXT_SENDER, player_id);
