...
**		Represent a single item as stored in the data safe. DataSafeEntryClass has the
*header
** 		information used to retrieve the item from the safe and the derived
*DataSafeEntryDataClass *		also includes the actual type specific data payload.
**
**	DataSafeEntryListClass
**		This is a manager class for a linked list of DataSafeEntryDataClass<T>'s. Only the
*same *		type of DataSafeEntryDataClass<T> classes can currently be linked together and exist
*in *		the same list.
**
**	DataSafeEntryTypeClass
**		Every object put into the data safe is allocated a type ID so that like types (with
*the *		same ID) can be identified for linking and swapping purposes. DataSafeEntryTypeClass
...
**		Also included is an array of DataSafeEntryListClass's that act as pointers to the
*data safe *		entries.
**
**
**	SafeDataClass
**		This is a convenience class used to automate insertion and removal of items and
*provide *		a set of operators for common types. Using this class, adding existing data
*to the data safe *		can be as simple as just changing how it's declared. e.g.
**
**		int SomeData;
**
**			becomes
...
**
**			There are also some macros provided to make declaration easier so you could
*also just say
**
**		safe_int SomeData;
**
** 		Currently, only a subset of operators are defined but more can easily be added as
*needed.
** 		Some changes to code will be required where operators are not available. For example
**
** 		SomeData += 10;
**
** 			would be a problem because the += operator is not defined. However the 'int'
*operator, the *			= operator and the conversion operator () are defined so you
*could change the code to read
**
...
**
**		You can pretty much throw anything into the data safe using this class to automate
*isertion *		and retrieval. Some examples.
**
...
**
**		For types without existing macros, you will have to declare a new DataSafeClass<T>
*once for *		each type in DataSafe.cpp using the DECLARE_DATA_SAFE macro. So in the above
*example, you *		would have to add the following lines in DataSafe.cpp to allow storage of
*the new types
**
...
/*
** Number of linked lists we can create for data safe items. Only like sized items can be stored in
*the same list.
*/
#define MAX_DATASAFE_LISTS 8192

/*
** The maximum number of different data types that can be stored in the data safe. This should
*normally fit into a byte for
** handle encoding reasons.
*/
#define MAX_DATASAFE_TYPES 128

/*
** Max number of entries in a single list before we have to create a second list for items of the
*same size. The smaller this
** value, the quicker we will be able to retrieve data from the safe. The larger we make it, the
*more secure the data safe will
** be.
*/
#define MAX_ENTRIES_PER_LIST 16

/*
** How often the data safe entries get shuffled around and re-keyed. This is the delay between each
*shuffle.
*/
...
/*
** The number of copies of objects of any one type that can exist in an unencrypted state. These
*copies are only used as return
** values and changes to the copies can never be propagated back into the safe.
*/
#define MAX_OBJECT_COPIES 8

/*
** Sentinel values. Used to see if someone just zeroes the whole safe or tampered with the get
*function.
*/
...
/*
** We want to inline functions within inline functions to make the code harder to hack by having
...
** packing alignment becomes 8 (the size of a double) thus the offset of Data is not always as
*expected.
** i.e. you can no longer use sizeof(DataSafeEntryClass) to get the offset of the Data member of
** the derived class. This also prevents bloating (due to the bool member) when using large
*expansions
** for 'T'
*/
#pragma pack(push)
...
/*
**
** This class represents an internal data safe entry minus the actual data. In practice, this class
*will never be used
...
/*
** This class represents a single data safe entry list. There can be mutiple lists in a data safe
*but all entries in a single
...
/*
**
** This class describes a type of entry in the data safe. It's important to keep track of the types
*of objects we add since
** only similar types can be swapped in memory. This class is just a convenient container to help us
*match type codes with
** unique IDs.
**
**
*/
...
public:
    ...
    /*
    ** A unique number used to match and assign type IDs. This can come from anywhere as long as
    *it's different for every type.
    */
    unsigned long TypeCode;

    /*
    ** This is the user friendly ID that is stored along with entries in the data safe and returned
    *in the handle that's
    ...
    */
    ...
    /*
    ** The size of this type of object. Must be a multiple of 4 bytes for the data safe. (This is
    *redundant now that we
...
**
**		This class represents a single data safe. It can only function as the base class to
*one or more instances of
** 	DataSafeClass<T>.
**
**
** 	This class functions as a base class for all the templated 'DataSafeClass'es so that they
*can all share a single set of *		static data. Normally, static data in a templated
*class is duplicated for each expansion of the class.
**
**    Even though there may be many instances and expansions of the derived DataSafeClass<T> there
*is only ever one global *		data safe and that is represented by this class.
**
*/
...
public:
    ...
    */
    ...
    /*
    ** Thread safety. This is normally handled automatically but if you need to manually lock the
...
protected:
    ...
    */
    ...
    static void Encrypt(void* data, int size, unsigned long key = SimpleKey,
                        bool do_checksum = true);
    static void Decrypt(void* data, int size, unsigned long key = SimpleKey,
                        bool do_checksum = true);
    ...
    */
    ...
    static void Random_Insertion(DataSafeEntryClass* entry, int list, int type,
                                 bool is_slop = false);
    ...
    */
    ...
    /*
    ** If thread safety is not required, this class compiles out to nothing when used in release
    *mode.
    */
    class ThreadLockClass
    {
    ...
    /*
    ** Static data. We only need one each of these, regardless of how many DataSafeClass 'types'
    *there are.
    **
    **
    **
    ...
    */
    ...
    /*
    ** Array of pointers to data lists. We can't use a dynamic vector class here because the handles
    *we give out include
    ** an index into this array. Dynamic vector class moves entries around to remove gaps in the
    *array which would leave
...
**  DataSafeClass<T>
**
** 	This templated class is used to provide type specific functionality in the data safe. There
*must be at least one instance *		of this class for each type of data we want to store
*in the safe. This, along with 'GenericDataSafeClass' represents *		a single global data
*safe.
**
*/
...
private:
    ...
    **
    ** The number of elements in this list is the number of times you can call the 'Get' function
    *before you start to
    ** invalidate the data you got 'n' calls ago. For this reason, you should always make a copy of
    *data retrieved from
...
** SafeDataClass.
**
** This class is for convenience when using the data safe. Just expand this class for the type you
*want to add to the data safe
** and it will keep track of the handle and automate the adding in, and removal of, data in the
*safe.
**
*/
...
template <class T> DataSafeClass<T>::DataSafeClass(T*, int slopcount)
{
    ...
    */
    ...
    /*
    ** Get the type info for 'T'. Different types get stored in different places in the safe. Only
    *similar types can be in the
    ** same safe list.
    */
    ...
    /*
    ** Get_Type_ID will return the Type ID for this type code (instruction pointer). It will create
    *a new type if this one
    ...
    */
    ...
    /*
    ** Add some slop entries here. These are just bogues entries than can be swapped in memory with
    *real entries. The less
    ** commonly added data types should be give more slop entries.
    */
    for (int i = 0; i < slopcount; i++) {

        /*
        ** We need to make sure the constructor gets called for this object type before it's added
        *to the safe. But we must
        ** also make sure that the destructor *isn't* called when we go out of scope here - it will
        *be called explicitly
        ** when the datasafe is destructed. By using _alloca, and doing an in-place new, the memory
...
template <class T> DataSafeClass<T>::~DataSafeClass(void)
{
    ...
    /*
    ** Remove all slop enries of our type from all lists. Note that this will delete non-slop
    *entries too if any remain at this
    ** time.
    */
    for (int i = 0; i < NumLists; i++) {
        ds_assert(Safe[i] != nullptr);
        if (Safe[i] != nullptr && Safe[i]->EntryType == Type && Safe[i]->EntryCount > 0) {
            ...
            while (Safe[i]->EntryCount > 0) {
                ...
                */
                ...
                /*
                ** Safety check. If something goes wrong and we didn't delete the entry then we will
...
{
    /*
    ** Make sure this function gets expanded multiple times for different types by referencing the
    *type.
    */
    ...
    /*
    ** Since we aren't using RTTI I need some other way of distinguishing types in the safe. Because
    *it's templatised, this
    ** code will get expanded once for each type it's used with. I will use the location in memory
    *of the function to
...
template <class T> DataSafeHandleClass DataSafeClass<T>::Add_Entry(T& value, bool is_slop)
{
    ...
    */
    DataSafeEntryClass* entry_ptr
        = (DataSafeEntryClass*)new char[sizeof(DataSafeEntryDataClass<T>)];
    ds_assert(entry_ptr);
    ...
    if (entry_ptr) {
        ...
        */
        ...
        if (list != -1 && Safe[list] != nullptr) {
            ...
            if (Safe[list]->EntryCount == 1 && !is_slop) {
                for (int i = 0; i < MinSlop; i++) {
                    ...
                    if (entry_ptr) {
                        ...
                        /*
                        ** Encrypt the garbage data in the uninitialized memory we just allocated
                        *for the
                        ** object. It has to be encrypted to keep the CRC right.
                        */
                        Encrypt(&((DataSafeEntryDataClass<T>*)entry_ptr)->Data, sizeof(T),
                                SimpleKey, true);

                        /*
                        ** Get a handle ID. Not really needed since we won't be returning a handle
                        *but it
                        ** keeps the ID usage list neat. Fill in the rest of the handle to make it
                        *valid.
                        */
            ...
            */
            if (Safe[list]->SlopCount != 0 && Safe[list]->EntryCount > Safe[list]->SlopCount * 2
                && !is_slop) {
...
template <class T> void DataSafeClass<T>::Delete_Entry(DataSafeHandleClass handle)
{
    ...
    */
    ...
    /*
    ** The handles we give out are encrypted so we need to decrypt temporarily to extract the list
    *number.
    */
    ...
    */
    ...
    if (entry_ptr != nullptr) {
        ...
        */
        ...
        /*
        ** We found the entry for this handle. We need to fix up the list pointers and delete the
        *memory.
        ** Since we originally memcpy'ed this object into place we will have to call the destructor
        *explicitly. The explicit
        ** destructor call will have no effect if the object doesn't have a destructor defined.
        **
        ** We need to decrypt the object in place before calling the desturctor otherwise it will
        *have a hard time acting on
...
template <class T> bool DataSafeClass<T>::Get(DataSafeHandleClass handle, T*& item)
{
    ...
    */
    ...
    /*
    ** Copy the object from the data safe and decrypt it in one pass. Don't adjust the checksum as
...
{
    /*
    ** The data here isn't initialised since no default value was provided. I have to provide *some*
    *sort of
    ** init though as the bloody compiler won't let me turn off the 'local variable 'data' used
    *without having been initialized'
    ** message. Grrrr.
    **
    ** Allocating the memory on the stack then doing an in place new seems to mollify the compiler
    *so we don't get the warning
    ** and it does much the same thing as just declaring 'T data'. It also has the benefit that the
    ...
    */
    if (Handle.Is_Valid()) {
        ...
        if (data_ptr) {
            ...
            /*
            ** Looks like we have the value for this safe data class. Get the value for the other
            *one.
            */
            other_value = safedata.Get_Ptr();
            ds_assert(other_value != nullptr);
