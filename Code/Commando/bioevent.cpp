    ...
    if (is_accepted) {
        ...
        if (!IS_SOLOPLAY) {
            ...
            if (cNetwork::PServerConnection != nullptr) {
                cNetwork::PServerConnection->Set_Rhost_Expect_Packet_Flood(p_player->Get_Id(),
                                                                           true);
            }
            ...
            //
            // If we (the server) are already in intermission mode then deactivate the player until
            // the next map starts. The player will have already loaded the next map. ST - 1/17/2002
            // 7:43PM
            //
            if (The_Game()->IsIntermission.Is_True()) {

                //
                // If the new player has the old map loaded (which can happen if the intermission
                // starts after the client has started loading the old map) then send him a wineven
                // so he knows to load the next map.
                //
                if (stricmp(MapName, The_Game()->Get_Map_Name().Peek_Buffer()) != 0) {
                    cWinEvent* p_win = new cWinEvent;
                    p_win->Init(The_Game()->Get_Winner_ID(), SenderId,
                                The_Game()->Is_Map_Cycle_Over());
