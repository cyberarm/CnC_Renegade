...
void cDiagnostics::Render(void)
{
    ...
    if (cDevOptions::ShowDiagnostics.Is_True()) {
        ...
        if (cNetwork::I_Am_Client()) {
            ...
            Add_Diagnostic("CSC Last Ping:      %d ms",
                           CombatManager::Get_Last_Round_Trip_Ping_Ms());
            Add_Diagnostic("CSC Avg. Ping:      %d ms",
                           CombatManager::Get_Avg_Round_Trip_Ping_Ms());
        }
        ...
        Add_Diagnostic("Vehicles:           %d (%d with driver)", vehicle_count,
                       vehicle_driven_count);

        // Add_Diagnostic("AreClientsTrusted:    %d", cCsDamageEvent::Get_Are_Clients_Trusted());
        // Add_Diagnostic("DriverIsAlwaysGunner: %d",
...
/*
                ...
                if (PTheGameData != NULL) {
                        ...
                        if (PTheGameData->Is_Lms()) {
                                Add_Diagnostic("reap:	   %5.2f",
   The_Lms_Game()->Get_Reaper_Countdown_Seconds());
                        }
                }
        ...
        //
                ...
                if (cNetwork::I_Am_Only_Client()) {
                                Add_Diagnostic("CIM : %d",
   cUserOptions::ClientInterpolationModel.Get());
                        }

                        Add_Diagnostic("I_Am_Server:						%d",
   cNetwork::I_Am_Server()); Add_Diagnostic("I_Am_Client:
   %d", cNetwork::I_Am_Client()); Add_Diagnostic("cNetwork::Get_Update_Count:	%d",
   cNetwork::Get_Update_Count());
                ...
                if (cNetwork::I_Am_Client()) {
                        ...
                        if (p_me == NULL) {
                                ...
                                Add_Diagnostic("p_me->Get_Spectatee_Id() == %d",
   p_me->Get_Spectatee_Id());
                        }
                }

                //Add_Diagnostic("Menu elements = %d", MenuManager::Get_Count());
                //Add_Diagnostic("Get_Current_Entry_Index = %d",
   MenuManager::Get_Current_Entry_Index());
   ...
   p_rhost->Stats.StatSnapshot[STAT_BitsSent]);
         ...
         Add_Diagnostic("MENU STACK:");
                        ...
                        Add_Diagnostic("menu_direction:      %d",
   MenuManager::Get_Menu_Direction());
                        ...
                        if (cNetwork::I_Am_Server()) {
                                Add_Diagnostic("Host 0 = %d",
   cNetwork::PServerConnection->Get_Remote_Host(0)); Add_Diagnostic("Host 1 = %d",
   cNetwork::PServerConnection->Get_Remote_Host(1));
                        }

                        Add_Diagnostic("Duration:			 %d s",
   The_Game()->Get_Duration_Seconds());

                        Add_Diagnostic("ServerQuickstart: %d",
   cUserOptions::ServerQuickstart.Get()); Add_Diagnostic("ClientQuickstart: %d",
   cUserOptions::ClientQuickstart.Get()); Add_Diagnostic("# teams = %d", cTeamManager::Count());
                        Add_Diagnostic("Team: %d", cUserOptions::Get_Player_Type_Choice());
                        Add_Diagnostic("Players: %d", cPlayerManager::Count());
                        Add_Diagnostic("game channels:       %d",
   cGameChannelList::Get_Chan_List()->Get_Count()); extern int menu_direction;
                        Add_Diagnostic("menu_direction:      %d",
