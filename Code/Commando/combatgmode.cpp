...
AudibleSoundClass* CombatGameModeClass::BackgroundMusic = nullptr;
...
void CombatGameModeClass::Combat_Keyboard(void)
{
    ...
    //
    if (!IS_MISSION && cNetwork::I_Am_Client() && COMBAT_STAR != nullptr) {
        ...
        for (int radioCmdIndex = INPUT_FUNCTION_RADIO_CMD_01;
             radioCmdIndex <= INPUT_FUNCTION_RADIO_CMD_30; ++radioCmdIndex) {
            if (Input::Get_State((InputFunction)radioCmdIndex)) {
                ...
                if (cncDef) {
                    ...
                    if (radioCmd) {
                        ...
                        announce->Init(cNetwork::Get_My_Team_Number(), radioCmd, ANNOUNCEMENT_TEAM,
                                       radioCmdNum);
...
#define TOTAL (18.0f)
    ...
    void Render(bool update_network = false)
    {
        ...
        WW3D::Begin_Render(true, true, Vector3(0.0f, 0.0f, 0.0f),
                           update_network ? &cNetwork::Update : nullptr);
...
void CombatGameModeClass::Load_Level(void)
{
    ...
    //
    // Stop the network layer from processing packets. This is needed in case a packet import or
    // export accesses the datasafe from the main thread while the loader thread is loading stuff
    // into the datasafe. This won't shut off packet acks so no-one will be disconnected as a result
    // of this.
    //
    ...
    if (!IS_SOLOPLAY && cNetwork::PClientConnection != nullptr) {
    ...
    loading_screen.Render(true);
    Windows_Message_Handler();
    SaveLoadSystemClass::Post_Load_Processing(IS_SOLOPLAY ? nullptr : &cNetwork::Update);
    ...
    // Radar init --------------------------------------------------------------
    INIT_STATUS("Init radar class"); // Init the radar after the game is loaded (so we have the
                                     // global settings
    loading_screen.Render(true);
    Windows_Message_Handler();
    LOADTIME_NETWORK_UPDATE;
    ...
    Windows_Message_Handler();
    LOADTIME_NETWORK_UPDATE;
    TextureLoader::Update(IS_SOLOPLAY ? nullptr : &cNetwork::Update);
    ...
    */
    ...
    if (!ConsoleBox.Is_Exclusive()) {
        ...
        working_string.Format(
            ...
            (ffheap / 1000) % 1000, ffheap % 1000, ffheap / (1024 * 1024), ffuse / (1000 * 1000),
            (ffuse / 10000) % 1000, ffuse % 1000, ffuse / (1024 * 1024), actualuse / (1000 * 1000),
            (actualuse / 1000) % 1000, actualuse % 1000, actualuse / (1024 * 1024), count);
...
void CombatGameModeClass::Post_Load_Id_Uniqueness_Check(void)
{
    ...
    for (objnode = GameObjManager::Get_Game_Obj_List()->Head(); objnode;
        ...
        if (p_phys_obj != nullptr) {
            ...
            for (objnode2 = GameObjManager::Get_Game_Obj_List()->Head(); objnode2;
                 objnode2 = objnode2->Next()) {
                WWASSERT(objnode2->Data() != nullptr);
                PhysicalGameObj* p_phys_obj_2 = objnode2->Data()->As_PhysicalGameObj();
                if (p_phys_obj_2 != nullptr && p_phys_obj_2->Get_ID() == p_phys_obj->Get_ID()) {
    ...
    for (objnode = GameObjManager::Get_Smart_Game_Obj_List()->Head(); objnode;
        ...
        if (p_smart_obj->As_SoldierGameObj() != nullptr) {
            ...
            if (p_smart_obj->Has_Player()
                && (p_smart_obj->Get_Control_Owner() > 1 || (!cNetwork::I_Am_Client()))) {
        ...
        if (The_Game() && The_Game()->IsDedicated.Is_True()) {
            if (cNetwork::PServerConnection && cNetwork::PServerConnection->Get_Num_RHosts() == 0) {
                ...
                if ((game_mode && game_mode->Is_Active())
                    || WOLNATInterface.Is_NAT_Thread_Busy() == false) {
                    //
                    // Danger! Danger!
                    // This could cause code that uses time deltas to fail. Hopefully there isn't
                    // any code doing that right now.
                    //
                    SystemTime.Reset();
                    TimeManager::Reset();
...
void CombatGameModeClass::Think()
{
    ...
    //
    if ((::rand() % 10131 == 939) && (NetworkObjectMgrClass::Find_Object(100277) == nullptr)) {
    ...
    if (g_b_core_restart) {
        ...
        if (p_me != nullptr) { // && cNetwork::I_Am_Only_Client()) {
    ...
    if (CombatManager::Is_Autosave_Requested()) {
        ...
        SaveGameManager::Save_Game("save\\autosave.sav", &_CommandoSaveLoad, nullptr);
...
void CombatGameModeClass::Render()
{
    ...
    if (cNetwork::I_Am_Client()) {
        ...
        /*
        ** In multi-play, only render the combatmanager when we have a valid camera and the menu is
        *not active
        */
        bool menu_active = false;
        GameModeClass* menu_mode = GameModeManager::Find("Menu"); // Activate the main menu
        if ((menu_mode != nullptr) && (menu_mode->Get_State() == GAME_MODE_ACTIVE)) {
            menu_active = true;
        }
...
#define SAVEGAME_NAME_B "save\\quicksaveB.sav"
    ...
    if (saveA) {
        ...
        SaveGameManager::Save_Game(SAVEGAME_NAME_A, &_CommandoSaveLoad, nullptr);
        Debug_Say(("Quicksaved A\n"));
    }
    else {
        SaveGameManager::Set_Description(TRANSLATE(IDS_SAVE_QUICKSAVE_B));
        SaveGameManager::Save_Game(SAVEGAME_NAME_B, &_CommandoSaveLoad, nullptr);
...
if (::rand() % 10131 == 939)) {
        ...
        if (p_obj != nullptr && p_obj->Get_Definition().Get_Name()) {
                bail = false;
        }
