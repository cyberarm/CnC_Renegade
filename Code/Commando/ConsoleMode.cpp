        ...
        if (AllocConsole()) {
            ...
            */
            if (!SlaveMaster.Am_I_Slave()) {
                FillConsoleOutputAttribute(ConsoleOutputHandle, MASTER_COLORS, 4192 * 50, coord,
                                           &written);
                SetConsoleTextAttribute(ConsoleOutputHandle, MASTER_COLORS);
            }
            else {
                FillConsoleOutputAttribute(ConsoleOutputHandle, SLAVE_COLORS, 4192 * 50, coord,
                                           &written);
            ...
            */
            ...
            Print("v%d.%.3d %s-%s %s\n", (version_major >> 16), (version_major & 0xFFFF),
                  BuildInfoClass::Get_Builder_Initials(), BuildInfoClass::Get_Build_Number_String(),
...
void ConsoleModeClass::Print_Maybe(char const* string, ...)
{
    if (string && !ProfileMode) {
        ...
        if (Pos == 1 && (TIMEGETTIME() - LastKeypressTime > 5 * 1000)
            && ConsoleOutputHandle != INVALID_HANDLE_VALUE) {
...
const char* ConsoleModeClass::Get_Log_File_Name(void)
{
    ...
    if (_last_day != time.wDay && ServerSettingsClass::Get_Disk_Log_Size() != -1) {
        ...
        if (SystemTimeToFileTime(&time, &file_time)) {
            ...
            _int64 time_diff = ((_int64)10000000)
                * ((_int64)60 * 60 * 24 * ServerSettingsClass::Get_Disk_Log_Size());
        ...
        if (_kbhit()) {
            ...
            */
            case 9: {
                ...
                ConsoleFunctionManager::Get_Command_Suggestion(suggestion_stub, last_suggestion,
                                                               suggestion, help, 256);
            ...
            if (Pos == 1 && time - LastKeypressTime > 30 * 1000) {
                if (time - last_info_time > 60 * 1000) {
                    if (cNetwork::PServerConnection
                        && cNetwork::PServerConnection->Get_Num_RHosts() != num_players) {
    ...
    if (!ProfileMode && formatted_text && text_color
        ...
        */
        ...
        if (!SlaveMaster.Am_I_Slave()) {
            ...
            SetConsoleTextAttribute(ConsoleOutputHandle,
...
void ConsoleModeClass::Update_Profile(StringClass profile_string)
{
    ...
    if (profile_string.Get_Length() && ProfileMode && TIMEGETTIME() - LastProfilePrint > 1500) {
        ...
        */
        unsigned long crc = CRC::Memory((unsigned char*)profile_string.Peek_Buffer(),
                                        profile_string.Get_Length());
            ...
            */
            if (ok) {
                ...
                */
                if (LastProfileCRC == 0) {
                    if (!SlaveMaster.Am_I_Slave()) {
                        FillConsoleOutputAttribute(ConsoleOutputHandle, MASTER_COLORS, 20 * 80, pos,
                                                   &num_written);
                    }
                    else {
                        FillConsoleOutputAttribute(ConsoleOutputHandle, SLAVE_COLORS, 20 * 80, pos,
                                                   &num_written);
                    }
                }
            }
