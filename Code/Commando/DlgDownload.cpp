...
void DlgDownload::UpdateProgress(DownloadEvent& event)
{
    ...
    case DownloadEvent::DOWNLOAD_PROGRESS: {
        ...
        if (ConsoleBox.Is_Exclusive()) {
            ...
            ConsoleBox.Print("Got %d Kb of %d at %d Kb per second       \r", read / 1024,
                             size / 1024, transferRate / 1024);
        }
        else {
            // Update progress bar
            ...
            text.Format(TRANSLATE(IDS_MENU_TRANSFER_TIME_LEFT), (const WCHAR*)timetext,
                        (const WCHAR*)readtext, (const WCHAR*)sizetext);
