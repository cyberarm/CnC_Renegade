...
void WOLLogonMgr::ConfigureSession(void)
{
    ...
    if (wolSession.IsValid()) {
        if (ConnectionConnected == wolSession->GetConnectionStatus()) {
            bool allowFind
                = MPSettingsMgrClass::Get_Option_Flag(MPSettingsMgrClass::OPTION_ALLOW_FIND);
            bool allowPages
                = MPSettingsMgrClass::Get_Option_Flag(MPSettingsMgrClass::OPTION_ALLOW_PAGES);
...
void WOLLogonMgr::InitiateLogon(bool forced)
{
    ...
    if (BandwidthCheckerClass::Got_Bandwidth() == false
        ...
        bool detecting = DlgWOLWait::DoDialog(TRANSLATE(IDS_MENU_DETECTING_BANDWIDTH),
                                              bandwidth_wait, this, 0, mQuietMode ? 0xffffffff : 0);
    ...
    if (HasServerList() == false) {
        ...
        bool fetching
            = DlgWOLWait::DoDialog(IDS_WOL_LOGON, fetch, this, 0, mQuietMode ? 0xffffffff : 0);
    ...
    if (server.IsValid() == false) {
        ...
        if (HasValidPings() == false) {
            ...
            bool pinging
                = DlgWOLWait::DoDialog(IDS_WOL_LOGON, pings, this, 0, mQuietMode ? 0xffffffff : 0);
    ...
    if (connectWait.IsValid()) {
        ...
        bool connecting
            = DlgWOLWait::DoDialog(IDS_WOL_LOGON, connect, this, 0, mQuietMode ? 0xffffffff : 0);
    ...
    case DETECTING_BANDWIDTH:
        ...
        if ((result != WaitCondition::Waiting && result != WaitCondition::UserCancel)
            ...
            if (result != WaitCondition::UserCancel) {
                ...
                //	{
                if (mQuietMode) {
                    // Try again. The auto detect won't fire off a second time since we switched to
                    // 56k
                    ...
                    // DlgMsgBox::DoDialog(L"IDS_WOL_LOGONFAILED", TRANSLATE
                    // (IDS_MENU_NO_INET_CONNECTION));
                    //  Text reads....
                    ...
                    Add_Ref();
                    DlgMsgBox::DoDialog(L"", TRANSLATE(IDS_MENU_SET_CONNECTION_SPEED),
                                        DlgMsgBox::Okay, this);
    ...
    case WAITING_PINGS:
        ...
        if ((result == WaitCondition::ConditionMet) && HasValidPings()) {
            ...
            if (mQuietMode) {
                ...
                DlgMsgBox::DoDialog(TRANSLATE(IDS_WOL_LOGONFAILED),
                                    TRANSLATE(IDS_MENU_UNABLE_TO_DETERMINE_SERVER_LOC));
            }
        }
        break;
    ...
    case CONNECTING:
        if (WaitCondition::ConditionMet == result) {
            ...
            if (mQuietMode) {
                ...
                errorMsg.Format(TRANSLATE(IDS_MENU_UNABLE_TO_CONNECT_TO_SERVER),
                                wait->GetResultText());
