...
//
////////////////////////////////////////////////////////////////
void SplashIntroMenuDialogClass::On_Command(int ctrl_id, int mesage_id, DWORD param)
{
    return;
}
...
void SplashOutroMenuDialogClass::On_Command(int ctrl_id, int mesage_id, DWORD param)
{
    ...
    case IDC_BUY_RENEGADE:
        ::ShellExecute(nullptr, "open",
                       "http://www.ea.com/"
                       "storeEntry.jsp?sDestinationURL=DETAIL&sSku=14180W&sEntryPoint=EXPSITE",
                       nullptr, nullptr, SW_SHOW);
        Stop_Main_Loop(EXIT_SUCCESS);
        break;

    case IDC_VISIT_WEB_BUTTON:
        ::ShellExecute(nullptr, "open", "http://www.westwood.com/games/ccuniverse/renegade", nullptr,
                       nullptr, SW_SHOW);
        Stop_Main_Loop(EXIT_SUCCESS);
        break;
...
void GameSpyOptionsDialogClass::On_Command(int ctrl_id, int mesage_id, DWORD param)
{
    ...
    case IDC_MENU_BACK_BUTTON: {
        ...
        // Save the settings
        //
        /*			ComboBoxCtrlClass* connectCombo =
           (ComboBoxCtrlClass*)Get_Dlg_Item(IDC_CONNECTION_SPEED_COMBO); if (connectCombo != nullptr) {
                                        int selection = connectCombo->Get_Curr_Sel();

                                        if (selection >= 0) {
                                                BANDWIDTH_TYPE_ENUM bandwidth =
                                                        (BANDWIDTH_TYPE_ENUM)(selection +
           BANDWIDTH_FIRST);
...
////////////////////////////////////////////////////////////////
/*void
StartSPGameDialogClass::On_TreeCtrl_Needs_Children (TreeCtrlClass *tree_ctrl, int ctrl_id,
...
MPServerStartMenuClass::On_Init_Dialog (void)
{
        ...
        //
        //Get_Dlg_Item (IDC_TEAM_STATIC)->Enable (The_Game ()->IsTeamChangingAllowed.Is_True () ==
false);
        //Get_Dlg_Item (IDC_TEAM_COMBO)->Enable (The_Game ()->IsTeamChangingAllowed.Is_True () ==
false);
    ...
    if (ctrl_id == IDC_MENU_MP_JOING_BUTTON) {
        ...
        if (index >= 0) {
            ...
            if (channel != nullptr) {
                ...
                PTheGameData
                    = cGameData::Create_Game_Of_Type(channel->Get_Game_Data()->Get_Game_Type());
                WWASSERT(PTheGameData != nullptr);
...
void MPJoinMenuClass::Update_Game_List(void)
{
    ...
    for (objnode = cGameChannelList::Get_Chan_List()->Head(); objnode; objnode = objnode->Next()) {
        ...
        if (GameModeManager::Find("WOL")->Is_Active()) {
            ...
            p_channel->Get_Game_Data()->Set_Current_Players(
                p_channel->Get_Wol_Channel()->currentUsers);
        }
...
//
////////////////////////////////////////////////////////////////
EditWheeledVehicleDialogClass::EditWheeledVehicleDialogClass(WheeledVehicleDefClass* def,
                                                             float wheel_radius)
...
//
////////////////////////////////////////////////////////////////
EditTrackedVehicleDialogClass::EditTrackedVehicleDialogClass(TrackedVehicleDefClass* def,
                                                             float wheel_radius)
...
void EditTrackedVehicleDialogClass::On_Init_Dialog(void)
{
    if (VehicleDef != nullptr) {
        ...
        Set_Dlg_Item_Float(IDC_TURN_TORQUE_SCALE_FACTOR_EDIT,
                           VehicleDef->Get_Turn_Torque_Scale_Factor());
...
void EditTrackedVehicleDialogClass::On_Command(int ctrl_id, int message_id, DWORD param)
{
    ...
    case IDOK:
        if (VehicleDef != nullptr) {
            ...
            VehicleDef->Set_Turn_Torque_Scale_Factor(
                Get_Dlg_Item_Float(IDC_TURN_TORQUE_SCALE_FACTOR_EDIT));
