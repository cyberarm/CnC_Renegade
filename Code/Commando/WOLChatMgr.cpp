...
static const wchar_t* Get_Parameter_From_String(const wchar_t* command_string,
                                                WideStringClass& parameter);
...
void WOLChatMgr::JoinLobby(const RefPtr<ChannelData>& channel)
{
    if (channel.IsValid()) {
        ...
        RefPtr<WaitCondition> wait
            = mWOLSession->JoinChannel(channel, product->GetChannelPassword());
    ...
    if (wcsnicmp(name, mLobbyPrefix, prefixLength) == 0) {
        ...
        if (*extName >= L'0' && *extName <= L'9') {
            static const wchar_t* _lobbies[8] = {
                TRANSLATE(IDS_MENU_LOBBY_NAME_01), TRANSLATE(IDS_MENU_LOBBY_NAME_02),
                TRANSLATE(IDS_MENU_LOBBY_NAME_03), TRANSLATE(IDS_MENU_LOBBY_NAME_04),
                TRANSLATE(IDS_MENU_LOBBY_NAME_05), TRANSLATE(IDS_MENU_LOBBY_NAME_06),
...
void WOLChatMgr::SendPrivateMessage(const RefPtr<UserData>& user, const wchar_t* message,
                                    bool isAction)
{
    if (ProcessCommand(message)) {
        return;
...
void WOLChatMgr::AddMessage(const wchar_t* sender, const wchar_t* message, bool isPrivate,
                            bool isAction)
{
    ...
    if (message && message[0] == L'/') {
        ...
        if (command.Get_Length() > 0) {
            static struct
            {
                ...
                { L"page", SlashCmdPage },     { L"r", SlashCmdR },
                { L"locate", SlashCmdLocate }, { L"msg", SlashCmdMsg },
                { L"invite", SlashCmdInvite }, { L"kick", SlashCmdKick },
                { L"join", SlashCmdJoin },     { nullptr, nullptr },
            };
