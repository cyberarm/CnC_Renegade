...
void cGameDataCnc::Soldier_Added(SoldierGameObj* soldier)
{
    ...
    //
    if (soldier != NULL && soldier->Get_Player_Data() != NULL
...
bool cGameDataCnc::Is_Valid_Settings(WideStringClass& outMsg, bool check_as_server)
{
    ...
    */
    ...
    if (BaseDestructionEndsGame.Is_False() && BeaconPlacementEndsGame.Is_True()) {
        ...
        PRINT_CONFIG_ERROR;
        ConsoleBox.Print(
            "Cannot allow beacons to end game without base destruction ending game.\n\n");
...
bool cGameDataCnc::Is_Gameplay_Permitted(void)
{
    ...
    if (permitted && Get_Max_Players() > 1) {
        ...
        //
        permitted = cPlayerManager::Tally_Team_Size(PLAYERTYPE_NOD) > 0
...
void cGameDataCnc::Get_Description(WideStringClass & description)
{
   ...
   line.Format(L"Base Destruction Ends Game\t%s",		BaseDestructionEndsGame.Get() ? yes
: no); IDS_MENU_TEXT314 description += line; description += newline;

   line.Format(L"Beacon Placement Ends Game\t%s",		BeaconPlacementEndsGame.Get() ? yes
: no); IDS_MENU_TEXT315 description += line; description += newline;
}
*/
