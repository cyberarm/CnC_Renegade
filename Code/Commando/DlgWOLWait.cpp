...
bool DlgWOLWait::DoDialog(const WCHAR* title, RefPtr<WaitCondition>& wait,
                          Observer<DlgWOLWaitEvent>* observer, unsigned long timeout,
                          unsigned long dialog_timeout)
{
    if (wait.IsValid()) {
...
DlgWOLWait::DlgWOLWait(RefPtr<WaitCondition>& wait, unsigned long timeout,
                       unsigned long dialog_timeout)
...
void DlgWOLWait::CheckCondition(void)
{
    ...
    if (!mShowDialog && mStartTime && dialog_timeout != SHOW_NEVER
        && ((currTime - mStartTime) > 2000)) {
        mShowDialog = true;
        MouseMgrClass::End_Wait_Cursor();
    }
