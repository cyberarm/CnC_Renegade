...
//-----------------------------------------------------------------------------
void cAppPacketStats::Increment_Bits_Sent_Tier(BYTE app_packet_type, PACKET_TIER_ENUM tier,
                                               DWORD bits)
{
    WWASSERT(app_packet_type != APPPACKETTYPE_ALL && app_packet_type < APPPACKETTYPE_COUNT);
    WWASSERT(bits >= 0);
...
StringClass& cAppPacketStats::Get_Heading(void)
{
    ...
    WorkingString.Format("%-30s %-8s %-10s %-10s %-7s %-7s %-7s %-7s %-7s %-7s", "Type", "Tally",
                         "Packets", "Bytes", "PC", "Avg.", "PC TC", "PC TR", "PC TO", "PC TF");
...
StringClass& cAppPacketStats::Get_Description(BYTE type)
{
    ...
    // StringClass description;
    WorkingString.Format("%-30s %-8d %-10d %-10d %-7.1f %-7d %-7d %-7d %-7d %-7d", name,
                         ObjectTally[type], PacketsSent[type], cMathUtil::Round(num_bytes),
                         percentage, average_bytes, t0, t1, t2, t3);
