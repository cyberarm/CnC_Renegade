...
*/
char* BandwidthCheckerClass::ErrorList[13] = { "BANDTEST_OK",
...
unsigned long BandwidthCheckerClass::Bandwidths[NUM_BANDS * 2]
    = { 12000,  14400,  28000,  28800,   29999,   33600,   53000,   57600,
        62000,  67200,  105000, 115200,  125000,  128000,  250000,  256000,
        500000, 512000, 999999, 1024000, 1999999, 2048000, 3999999, 4096000 };
        ...
        if (num_times) {
            ...
            */
            ...
            if (closest_index != -1) {
                ...
                strncpy(server_name_copy, list[closest_index].Peek_Buffer(),
...
void BandwidthCheckerClass::Check(void)
{
    ...
    */
    ...
    if (host == nullptr) {
        ...
        */
        UpstreamBandwidth = Detect_Bandwidth(ntohl(address.sin_addr.s_addr), 0, 2, failure_code,
                                             DownstreamBandwidth, BANDTEST_API_VERSION, &settings,
                                             APPLICATION_SUB_KEY_NAME_BANDTEST);

        /*
        ** If we failed due to a missing final ping then try again with fewer packets and no
        *retries.
        */
        if (UpstreamBandwidth == 0) {
            if (failure_code == BANDTEST_NO_FINAL_PING_TIME
                ...
                UpstreamBandwidth
                    = Detect_Bandwidth(ntohl(address.sin_addr.s_addr), 0, 0, failure_code,
                                       DownstreamBandwidth, BANDTEST_API_VERSION, &settings);
            }
        }
        ...
        if (UpstreamBandwidth > 0x7fffffff) {
            ...
            for (int i = 0; i < NUM_BANDS; i++) {
                if (UpstreamBandwidth < Bandwidths[(i * 2) + 1]) {
                    // WWDEBUG_SAY(("\nReported upstream connection bandwidth is %s bits per
                    // second\n", BandwidthNames[i]));
        ...
        if (DownstreamBandwidth > 0x7fffffff) {
...
#endif // WWDEBUG
                WWDEBUG_SAY(
                    ("Downstream bandwidth to external router is %.1f kilobits per second\n",
                     floater));
            }
            else {
            ...
            for (int i = 0; i < NUM_BANDS; i++) {
                if (DownstreamBandwidth < Bandwidths[(i * 2) + 1]) {
                    // WWDEBUG_SAY(("\nReported downstream connection bandwidth is %s bits per
                    // second\n", BandwidthNames[i]));
...
unsigned short* BandwidthCheckerClass::Get_Bandwidth_As_String(PackedBandwidthType bandwidth)
{
    ...
    swprintf(_build_string, L"%s,%s", BandwidthNames[bandwidth.Bandwidth.Down],
...
BandwidthCheckerClass::PackedBandwidthType BandwidthCheckerClass::Get_Packed_Bandwidth(void)
{
    ...
    if (cUserOptions::Get_Bandwidth_Type() != BANDWIDTH_AUTO) {
        bwu = cBandwidth::Get_Bandwidth_Bps_From_Type(
            (BANDWIDTH_TYPE_ENUM)cUserOptions::Get_Bandwidth_Type());
        bwd = bwu;
        automode = false;
    }
...
{
    char temp[128];
    sprintf(temp, "%d\t%d\t%d\t", UpstreamBandwidth, DownstreamBandwidth,
            cUserOptions::Get_Bandwidth_Type());
    log_string = temp;
}
