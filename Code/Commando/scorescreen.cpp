...
#define TUTORIAL_LOAD_MENU_NUMBER 90
    ...
    _SSStats_secondary_objectives
        = ObjectiveManager::Get_Num_Objectives(ObjectiveManager::TYPE_SECONDARY);
    _SSStats_completed_secondary_objectives
        = ObjectiveManager::Get_Num_Completed_Objectives(ObjectiveManager::TYPE_SECONDARY);

    _SSStats_tertiary_objectives
        = ObjectiveManager::Get_Num_Objectives(ObjectiveManager::TYPE_TERTIARY);
...
void ScoreScreenDialogClass::On_Init_Dialog(void)
{
    ...
    int overall_stars = (time_to_finish_stars + level_of_play_stars + saves_loaded_stars
                         + secondary_missions_stars)
        / 4;
...
#define SMALL_STAR_TEXTURE "IF_LrgStar.tga"
    ...
    // Build Rank Image
    const char* rank_images[5] = { "IF_private.tga", "IF_sargent.tga", "IF_leutenant.tga",
                                   "IF_major.tga", "IF_general.tga" };
    ...
    text.Format(TRANSLATE(IDS_SS_X_OF_N), _SSStats_completed_tertiary_objectives,
                _SSStats_tertiary_objectives);
...
{
    WideStringClass text;
    text.Format(TRANSLATE(IDS_SS_X_OF_N), _SSStats_completed_secondary_objectives,
                _SSStats_secondary_objectives);
    ...
    if (_SSStats_secondary_objectives != 0) {
        ratio
            = (float)_SSStats_completed_secondary_objectives / (float)_SSStats_secondary_objectives;
    }
