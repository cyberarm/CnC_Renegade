...
const char* CGameSpyQnR::default_heartbeat_list
    = "master.gamespy.com:27900, master.udpsoft.com:27900";
...
void CGameSpyQnR::LaunchArcade(void)
{
    ...
    if (!launched) {
        ...
        ::strcpy(
            url,
            "http://www.gamespyarcade.com/features/launch.asp?svcname=ccrenegadedemo&distID=432");
#else
        ::strcpy(url,
...
void CGameSpyQnR::Init(void)
{
    ...
    if (m_GSEnabled && !m_GSInit && The_Game()
        && The_Game()->Get_Game_Type() == cGameData::GAME_TYPE_CNC) {
        ...
        test = qr_init(&query_reporting_rec, ip, cUserOptions::GameSpyQueryPort.Get(), gamename,
                       secret_key, c_basic_callback, c_info_callback, c_rules_callback,
...
void CGameSpyQnR::rules_callback(char* outbuf, int maxlen)
{
    ...
    while (1) {
        ...
        //			int ver = version.dwFileVersionMS;
        //
        //			b.Format("%s %s V%d.%3.3d(%s-%d)", "Win-X86", bname,
        //(ver&0xffff0000)>>16, ver&0xffff, 				BuildInfoClass::Get_Builder_Initials(),
        //BuildInfoClass::Get_Build_Number());
        //		}
        //		if (!Append_InfoKey_Pair(outbuf, maxlen, "Version", b)) break;

        //		if (!Append_InfoKey_Pair(outbuf, maxlen, "ServerMOTD",
        //WideStringClass(The_Game()->Get_Motd()))) break; 		if (!Append_InfoKey_Pair(outbuf, maxlen,
        //"OwnerName", The_Game()->Get_Owner())) break;
        ...
        if (!Append_InfoKey_Pair(outbuf, maxlen, "DED",
                                 The_Game()->IsDedicated.Get() ? one : zero)) {
            break;
        }
        if (!Append_InfoKey_Pair(outbuf, maxlen, "DG",
                                 The_Game()->DriverIsAlwaysGunner.Get() ? one : zero)) {
            break;
        }
        if (!Append_InfoKey_Pair(outbuf, maxlen, "password",
                                 The_Game()->IsPassworded.Get() ? one : zero)) {
            break;
        }
        if (!Append_InfoKey_Pair(outbuf, maxlen, "TC",
                                 The_Game()->IsTeamChangingAllowed.Get() ? one : zero)) {
            break;
        }
        //		if (!Append_InfoKey_Pair(outbuf, maxlen, "WeaponSpawning",
        //The_Game()->SpawnWeapons.Get() ? one : zero)) break; 		if (!Append_InfoKey_Pair(outbuf,
        //maxlen, "TeamRemix", The_Game()->RemixTeams.Get() ? one : zero)) break; 		if
        //(!Append_InfoKey_Pair(outbuf, maxlen, "BuildingRepair",
        //The_Game()->CanRepairBuildings.Get() ? one : zero)) break;
        if (!Append_InfoKey_Pair(outbuf, maxlen, "FF",
                                 The_Game()->IsFriendlyFirePermitted.Get() ? one : zero)) {
            break;
        }
        //		if (!Append_InfoKey_Pair(outbuf, maxlen, "BaseDestructionEndsGame",
        //The_Game()->As_Cnc()->BaseDestructionEndsGame.Get() ? one : zero)) break; 		if
        //(!Append_InfoKey_Pair(outbuf, maxlen, "BeaconPlacementEndsGame",
        //The_Game()->As_Cnc()->BeaconPlacementEndsGame.Get() ? one : zero)) break;
        ...
        //				if (!Append_InfoKey_Pair(outbuf, maxlen, "GDI_Score",
        //value)) break; 			} else if (p_team->Get_Id() == PLAYERTYPE_NOD) { 				value.Format("%.0f",
        //p_team->Get_Score()); 				if (!Append_InfoKey_Pair(outbuf, maxlen, "NOD_Score", value)) break;
        //			}
        //		}

        // These don't apply to LAN/GameSpy Games
        //		if (!Append_InfoKey_Pair(outbuf, maxlen, "ClanMatch",
        //The_Game()->IsClanGame.Get() ? one : zero)) break; 		if (!Append_InfoKey_Pair(outbuf,
        //maxlen, "Laddered", The_Game()->IsLaddered.Get() ? one : zero)) break; 		if
        //(!Append_InfoKey_Pair(outbuf, maxlen, "QuickMatch", The_Game()->Is_QuickMatch_Server() ?
        //one : zero)) break;
...
BOOL CGameSpyQnR::Append_InfoKey_Pair(char* outbuf, int maxlen, const char* key,
                                      const WideStringClass& value)
{
        ...
        /*		SmartGameObj *game_obj = player->Get_GameObj ();
                        if (game_obj != NULL && game_obj->As_SoldierGameObj () != NULL) {
                                ...
                                if (!Append_InfoKey_Pair(outbuf, maxlen, keyval.Peek_Buffer(),
                                        WideStringClass(TRANSLATE(game_obj->Get_Translated_Name_ID()))
           )) break;
           ...
           break;
                                ...
                                keyval.Format("class_%d", pindex);
                                if (!Append_InfoKey_Pair(outbuf, maxlen, keyval.Peek_Buffer(),
           "Unknown")) break; keyval.Format("vehicle_%d", pindex); if (!Append_InfoKey_Pair(outbuf,
           maxlen, keyval.Peek_Buffer(), "Unknown")) break;
                        } */
