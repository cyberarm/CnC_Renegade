...
{
    SoundVolumeTestSound
        = WWAudioClass::Get_Instance()->Create_Sound_Effect("laser_rifle_fire_01.wav");
    ...
    MusicVolumeTestSound
        = WWAudioClass::Get_Instance()->Create_Sound_Effect("sakura battle theme.mp3");
    if (MusicVolumeTestSound != NULL) {
        MusicVolumeTestSound->Set_Type(AudibleSoundClass::TYPE_MUSIC);
    }

    DialogVolumeTestSound
        = WWAudioClass::Get_Instance()->Create_Sound_Effect("m00s1_s1s1gbmg_snd.wav");
...
bool DlgConfigAudioTabClass::On_Apply(void)
{
    ...
    WWAudioClass::Get_Instance()->Save_To_Registry(
        APPLICATION_SUB_KEY_NAME_SOUND, device_name, is_stereo, bits, hertz, sound_on, music_on,
...
//
/////////////////////////////////////////////////////////////////////////////
void DlgConfigAudioTabClass::On_SliderCtrl_Pos_Changed(SliderCtrlClass* slider_ctrl, int ctrl_id,
                                                       int new_pos)
{
    if (ctrl_id == IDC_SOUND_EFFECTS_SLIDER) {
        ...
        //
        if (SoundVolumeTestSound != NULL
            && (SoundVolumeTestSound->Is_Playing() == false
...
void DlgConfigAudioTabClass::On_Frame_Update(void)
{
    ...
    //
    if (CinematicVolumeTestSound != NULL
        && (curr_time - CinematicVolumeTestSoundStartTime) > 2000) {
...
void DlgConfigAudioTabClass::Set_Default_Volumes(void)
{
    ...
    WWAudioClass::Get_Instance()->Load_Default_Volume(defaultmusicvolume, defaultsoundvolume,
                                                      defaultdialogvolume, defaultcinematicvolume);
