...
void CNCWinScreenMenuClass::On_Frame_Update(void)
{
    ...
    if (UpdateTimer <= 0) {
        ...
        gdi_score_string.Format(TRANSLATE(IDS_MENU_GDI_SCORE),
                                (int)cTeamManager::Find_Team(PLAYERTYPE_GDI)->Get_Score());
        nod_score_string.Format(TRANSLATE(IDS_MENU_NOD_SCORE),
                                (int)cTeamManager::Find_Team(PLAYERTYPE_NOD)->Get_Score());
        if (The_Game()->Get_Winner_ID() == PLAYERTYPE_GDI) {
...
//
////////////////////////////////////////////////////////////////
void CNCWinScreenMenuClass::Build_Player_Display_Name(const cPlayer* player,
