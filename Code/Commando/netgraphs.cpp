                            ...
                            LPCSTR label)
{
    ...
    for (int message_type = 0; message_type <= p_stat_list->Get_Num_Stats(); message_type++) {
        ...
        renderer->Draw_Block(RectClass(0, GraphingY, fraction * bar_length, GraphingY + bar_height),
                             COLOR_RED_DIM);
                               ...
                               float threshold_priority)
{
    ...
    if (packetloss_pc > 0) {
        ...
        renderer->Draw_Block(RectClass(0, GraphingY, full_x, GraphingY + bar_height),
                             COLOR_DARK_RED);
    }
    ...
    */
    ...
    if (target_bps != -1) {
        ...
        renderer->Draw_Block(
            RectClass(target_x - 1, GraphingY, target_x + 1, GraphingY + bar_height));
    ...
    if (threshold_priority > -1) {
        ...
        */
        ...
        // TSS2001d renderer->Draw_Block(RectClass(tp - 1, GraphingY, tp + 1, GraphingY +
        // bar_height), COLOR_GREEN);
  ...
  BandwidthScaler *= 2;
    ...
    */
    ...
    // TSS101101
    //
    if (full_x > Render2DClass::Get_Screen_Resolution().Width() || end_x >
  Render2DClass::Get_Screen_Resolution().Width()) {
            //BandwidthBarLength /= 2.0f;
  BandwidthScaler *= 2;
    }
    */
...
void cNetwork::Watch_Bandwidth(Render2DTextClass* renderer)
{
    ...
    do {
        ...
        renderer->Draw_Block(RectClass(x1, GraphingY, x2, GraphingY + bar_height),
                             (i % 2) ? COLOR_WHITE : COLOR_BLACK);
        x1 = x2;
        i++;
    ...
    if (I_Am_Client() && PClientConnection->Is_Established()) {
        ...
        Bandwidth_Graph(renderer, text,
                        ...
                        Get_Client_Rhost()->Get_Stats().Get_Pc_Packetloss_Sent(), COLOR_RED, true,
        ...
        Bandwidth_Graph(renderer, text,
                        ...
                        Get_Client_Rhost()->Get_Stats().Get_Pc_Packetloss_Received(), COLOR_RED,
                        false, -1);
...
*/
    ...
    if (PServerConnection != nullptr) {
        ...
        Bandwidth_Graph(renderer, "s<-c*",
                        ...
                        PServerConnection->Get_Combined_Stats().StatMacroSnapshot[STAT_BitsRcv], -1,
                        0, COLOR_BLUE, false, -1);

        for (int i = PServerConnection->Get_Min_RHost(); i <= PServerConnection->Get_Max_RHost();
             i++) {

            WWASSERT(i >= cNetwork::PServerConnection->Get_Min_RHost()
            ...
            if (p_rhost != nullptr) {
                ...
                Bandwidth_Graph(renderer, text,
                                ...
                                p_rhost->Get_Stats().Get_Pc_Packetloss_Received(), COLOR_RED, false,
                                -1);
...
void cNetwork::Watch_Latency(Render2DTextClass* renderer)
{
    ...
    for (int i = 0; i < 10; i++) {
        x2 = x1 + bar_length;
        renderer->Draw_Block(RectClass(x1, GraphingY, x2, end_y),
                             (i % 2) ? COLOR_WHITE : COLOR_BLACK);
        x1 = x2;
    }
    renderer->Set_Location(Vector2(0, GraphingY + 1));
