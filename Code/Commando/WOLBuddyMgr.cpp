...
void WOLBuddyMgr::LoadIgnoreList(void)
{
    ...
    HKEY hKey;
    LONG result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, APPLICATION_SUB_KEY_NAME_IGNORE_LIST, 0,
                               KEY_READ, &hKey);
...
void WOLBuddyMgr::DisplayInvitation(const RefPtr<UserData>& user, const WCHAR* message)
{
    if (user->GetLocation() != USERLOCATION_IN_CHANNEL) {
        ...
        inviteMsg.Format(TRANSLATE(IDS_MP_WOL_INVITATION_FORMAT), (const WCHAR*)name,
