...
RefPtr<WaitCondition> WOLQuickMatch::ConnectClient(void)
{
    ...
    if (connectWait.IsValid()) {
        ...
        RefPtr<WaitCondition> joinWait
            = mWOLSession->JoinChannel(QUICKMATCH_CHANNELNAME, password, 0);
...
bool WOLQuickMatch::SendClientInfo(void)
{
    ...
    clientMsg.Format(L"CINFO VER=%lu CPU=%lu MEM=%lu TPOINTS=%ld PLAYED=%lu PINGS=%S", ver, speed,
                     memory, tpoints, played, pseudoPings);
