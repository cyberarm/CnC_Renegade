    ...
    for (int index = 0; index < INPUT_NAME_COUNT; index++) {
        ...
        if (Input::Get_Primary_Key_For_Function(fn_id) == dik_id
            ...
            if ((fn_id != INPUT_FUNCTION_ZOOM_IN && fn_id != INPUT_FUNCTION_ZOOM_OUT)
                && (curr_function_id != INPUT_FUNCTION_ZOOM_IN
                    && curr_function_id != INPUT_FUNCTION_ZOOM_OUT)) {
                retval = fn_id;
                break;
            }
...
//
////////////////////////////////////////////////////////////////
bool ControlsListTabClass::On_InputCtrl_Get_Key_Info(InputCtrlClass* viewer_ctrl, int ctrl_id,
                                                     int vkey_id, WideStringClass& key_name,
                                                     int* game_key_id)
{
    bool retval = false;
    ...
    if (function_id >= 0) {
        ...
        //
        if (dik_id != DIK_APPS && dik_id != DIK_WIN && dik_id != DIK_LWIN && dik_id != DIK_RWIN
            && dik_id != DIK_LCONTROL && dik_id != DIK_RCONTROL && dik_id != DIK_LALT
            && dik_id != DIK_RALT && dik_id != DIK_ALT && dik_id != DIK_DELETE && dik_id != 0) {
            retval = true;
...
void ControlsListTabClass::Prompt_User(void)
{
    ...
    message.Format(TRANSLATE(IDS_CONTROL_REMAP_WARNING), (const WCHAR*)key_name,
                   (const WCHAR*)function_name);
...
void ControlsListTabClass::HandleNotification(DlgMsgBoxEvent& event)
{
    if (event.Event() == DlgMsgBoxEvent::Yes) {
        ...
        //
        bool clear_zoom = (PendingFunctionID == INPUT_FUNCTION_ZOOM_IN)
            || (PendingFunctionID == INPUT_FUNCTION_ZOOM_OUT);
...
void ControlsListTabClass::Clear_Key(int dik_id, bool clear_zoom)
{
    ...
    while (function_id != -1) {
        if (clear_zoom
            || (function_id != INPUT_FUNCTION_ZOOM_IN && function_id != INPUT_FUNCTION_ZOOM_OUT)) {
