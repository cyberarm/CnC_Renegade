...
cPlayer* cPlayerManager::Find_Inactive_Player(const WideStringClass& name)
{
    ...
    for (SLNode<cPlayer>* objnode = PlayerList.Head(); objnode != nullptr; objnode = objnode->Next()) {
        cPlayer* p_player = objnode->Data();
        WWASSERT(p_player != nullptr);
...
void cPlayerManager::Compute_Ladder_Points(int winning_team)
{
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        ...
        if (Player_Array[i] == nullptr || Player_Array[i]->Get_Total_Time() < min_qualifying_time_ms) {
            continue;
        }
...
void cPlayerManager::Increment_Player_Times(void)
{
    ...
    for (int i = 0; i < MAX_PLAYERS; i++) {
        if (Player_Array[i] != nullptr && Player_Array[i]->Is_Active()) {
...
bool cPlayerManager::Is_Kill_Treasonous(cPlayer* p_killer, cPlayer* p_victim)
{
    return (
        ...
        // The_Game()->Is_Team_Game() &&
        p_killer != nullptr && p_victim != nullptr && p_killer != p_victim
...
int cPlayerManager::Player_Compare(const void* elem1, const void* elem2)
{
    ...
    if (p_player1->Get_Ladder_Points() > p_player2->Get_Ladder_Points()) {
        ...
        if (p_player1->Get_Score() > p_player2->Get_Score()) {
            ...
            //	result = -1;
            //} else if (p_player1->Get_Kill_To_Death_Ratio() ==
            //p_player2->Get_Kill_To_Death_Ratio()) {
            //
            // Sort on Kills
            //
...
void cPlayerManager::Construct_Heading(WideStringClass& string, bool force_verbose)
{
    ...
    WWASSERT(PTheGameData != nullptr);
    // bool is_verbose = force_verbose || The_Game()->IsIntermission.Get() ||
...
void cPlayerManager::Render_Player_List(void)
{
    ...
    WWASSERT(PTheGameData != nullptr);
    if (GameModeManager::Find("Combat") == nullptr || !GameModeManager::Find("Combat")->Is_Active()
    ...
    bool is_verbose = The_Game()->IsIntermission.Get()
        || (MultiHUDClass::Get_Playerlist_Format() == PLAYERLIST_FORMAT_FULL);
    ...
    if (The_Game()->IsIntermission.Is_True()) {
        ...
        int combined_height = cTeamManager::Compute_Team_List_Height()
            ...
            while (cload.Open_Chunk()) {
                ...
                case CHUNKID_PLAYER:
                    ...
                    if (!The_Game()->Is_Valid_Player_Type(p_player->Get_Player_Type())) {
                            Debug_Say(("* Removing loaded player due to invalid playertype for this
                    game.\n")); Remove(p_player);
                    }
                    */
                    ...
                    //
                    // TSS110101.
                    // Do not perturb the save/load code, but nullptrify it's effect.
                    //
                    if (!IS_MISSION) {
                        Remove(p_player);
                    }
