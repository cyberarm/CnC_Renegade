...
static const char* WINDOWS_SUB_KEY_RUN_ONCE
...
void AutoRestartClass::Think(void)
{
    ...
    case STATE_FIRST:
        ...
        */
        if (!GameMode) {
            ...
            if (can_render) {
                AutoRestartProgressDialogClass::Get_Instance()->Add_Text(
                    L"Initializing Westwood Online");
            }
            ...
            */
            ...
            /*
            ** Hide the page dialog - we don't want that sucker popping up when there's no-one
            *around.
            */
        ...
        if (cGameData::Is_Manual_Exit()) {
            ...
            */
            ...
            if (!GameMode) {
                ...
                /*
                ** It won't have initialzed above if a shutdown was already pending. Maybe we need
                *to try again.
                */
    ...
    case STATE_LOGIN:
        ...
        */
        case WOLLOGON_PATCHREQUIRED: {
            ...
            DlgDownload::DoDialog(TRANSLATE(IDS_WOL_DOWNLOAD), wol_session->GetPatchDownloadList(),
                                  true);
    ...
    */
    case STATE_CREATE_GAME: {
        ...
        /*
        ** We have to wait for the firewall thread to discover the local IP before we can build the
        *game.
        */
        if (GameMode) {
        ...
        */
        if (ServerSettingsClass::Get_Game_Mode() == ServerSettingsClass::MODE_WOL
            ...
            int max_players
                = (cBandwidth::Get_Bandwidth_Bps_From_Type(BANDWIDTH_AUTO) / 250000) * 4;
            if (max_players < 2) {
    ...
    */
    case STATE_START_GAME: {
        ...
        /*
        ** Allow the page dialog to display again (it can't really anyway since it's disabled when
            ...
            if (enable) {
                ...
                */
                ...
                if (ServerSettingsClass::Is_Active()) {
                    sprintf(options, " /startserver=%s",
                            ServerSettingsClass::Get_Settings_File_Name());
                }
