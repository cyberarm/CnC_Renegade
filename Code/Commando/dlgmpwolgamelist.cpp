        ...
        if (channel.IsValid()) {
            ...
            if (gameInfo.IsDataValid()) {
                ...
                if (itemIndex >= 0) {
                    ...
                    list->Set_Entry_Data(itemIndex, COL_PLAYERS,
                                         MAKELONG(gameInfo.NumPlayers(), gameInfo.MaxPlayers()));
                    ...
                    if ((selIndex == -1)
                        && (selectedGameName.Compare_No_Case(channel->GetName()) == 0)) {
                        selIndex = itemIndex;
                    }

                    if (gameInfo.Version() != (unsigned long)cNetwork::Get_Exe_Key()
                        || !gameInfo.IsMapValid()) {
                        ChannelData* rawChannel
                            = (ChannelData*)list->Get_Entry_Data(itemIndex, COL_HOST_NAME);
                        ...
                        if (gameInfo.IsMapValid()) {
                            list->Set_Entry_Text(itemIndex, COL_GAME_TITLE,
...
void MPWolGameListMenuClass::On_ListCtrl_Sel_Change(ListCtrlClass* list, int id, int oldIndex,
                                                    int newIndex)
{
    if (IDC_GAME_LIST_CTRL == id) {
        Enable_Dlg_Item(IDC_JOIN_GAME_BUTTON, false);
        ...
        if (newIndex >= 0) {
            RefPtr<ChannelData> channel
                = (ChannelData*)list->Get_Entry_Data(newIndex, COL_HOST_NAME);
...
// Just show the EXE version until we can work this out.
#if (0)
                    text.Format(TRANSLATE(IDS_MENU_HOST_INFO_FORMAT),
                                (const WCHAR*)channel->GetName(), mSelectedGame.Title(),
                    ...
                    text.Format(TRANSLATE(IDS_MENU_HOST_INFO_FORMAT),
                                (const WCHAR*)channel->GetName(), mSelectedGame.Title(),
                                HIWORD(verMajor), LOWORD(verMajor));
#endif
                    ...
                    text.Format(TRANSLATE(IDS_MENU_HOST_OPTIONS_INFO_FORMAT),
                                mSelectedGame.IsDedicated() ? TRANSLATE(IDS_MENU_TEXT463)
                                                            : TRANSLATE(IDS_MENU_TEXT464),
                                mSelectedGame.IsPassworded() ? TRANSLATE(IDS_MENU_TEXT463)
                                                             : TRANSLATE(IDS_MENU_TEXT464),
                                mSelectedGame.IsLaddered() ? TRANSLATE(IDS_MENU_TEXT463)
                                                           : TRANSLATE(IDS_MENU_TEXT464),
                                mSelectedGame.IsQuickmatch() ? TRANSLATE(IDS_MENU_TEXT463)
                                                             : TRANSLATE(IDS_MENU_TEXT464));

                    detailsList->Insert_Entry(detailsList->Get_Entry_Count(), text);

                    text.Format(TRANSLATE(IDS_MENU_HOST_OPTIONS_INFO2_FORMAT),
                                mSelectedGame.IsClanGame() ? TRANSLATE(IDS_MENU_ON)
                                                           : TRANSLATE(IDS_MENU_OFF),
                                mSelectedGame.IsTeamChange() ? TRANSLATE(IDS_MENU_ON)
                                                             : TRANSLATE(IDS_MENU_OFF),
                                mSelectedGame.IsTeamRemix() ? TRANSLATE(IDS_MENU_ON)
                                                            : TRANSLATE(IDS_MENU_OFF),
                                mSelectedGame.IsFriendlyFire() ? TRANSLATE(IDS_MENU_ON)
                                                               : TRANSLATE(IDS_MENU_OFF));

                    detailsList->Insert_Entry(detailsList->Get_Entry_Count(), text);

                    text.Format(TRANSLATE(IDS_MENU_HOST_OPTIONS_INFO3_FORMAT),
                                mSelectedGame.IsDriverGunner() ? TRANSLATE(IDS_MENU_ON)
                                                               : TRANSLATE(IDS_MENU_OFF),
                                mSelectedGame.IsSpawnWeapons() ? TRANSLATE(IDS_MENU_ON)
                                                               : TRANSLATE(IDS_MENU_OFF),
                                mSelectedGame.IsRepairBuildings() ? TRANSLATE(IDS_MENU_ON)
                                                                  : TRANSLATE(IDS_MENU_OFF));
...
void MPWolGameListMenuClass::HandleNotification(ServerError& error)
{
    if (CHAT_E_BANNED == error.GetErrorCode()) {
        //		DlgMsgBox::DoDialog(TRANSLATE(IDS_WOL_SERVERMESSAGE),
        //TRANSLATE(IDS_WOL_BANNED));
        DlgMsgBox::DoDialog(TRANSLATE(IDS_MENU_SERVER_MESSAGE_TITLE),
                            TRANSLATE(IDS_MENU_WOL_BAN_USER_MESSAGE));
        End_Dialog();
    }
}
