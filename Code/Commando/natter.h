...
/*
** This class performs the inevitable task of interfacing with whatever piece of code happens to be
*talking to WOLAPI.
**
...
public:
    ...
    */
    ...
    /*
    ** Game options format for the firewall code only. All firewall options packets are prefixed
    *with "NAT:"
    */
...
private:
    /*
    ** PortBase is the start of the current port number range we are using. The range is added to
    *the port base every time
    ** we start a new game so we are always using a fresh port. The server uses 2 ports per game and
    *the client uses 1 port.
    */
    ...
    */
    ...
    /*
    ** This is the socket handler to use when sending or receiving game packets. It's only used on
    *the client side since
    ** the server side has to use the games mechanisms to send and receive (the game already has a
