...
BanEntry::BanEntry(const char* name, const char* ip, const char* hash_id, const char* ip_mask,
                   bool rtype)
{
...
bool cGameSpyBanList::Begin_Player_Kick(int id)
{
    ...
    if (player && player->Get_Is_Active().Is_True() && player->Is_Human()) {
        ...
        p_message->Init(L"You've been kicked from the server. Please quit.", TEXT_MESSAGE_PRIVATE,
                        true, HOST_TEXT_SENDER, player->Get_Id());
