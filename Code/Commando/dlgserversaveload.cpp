            ...
            if (config->IsCustom) {
                ...
                if (The_Game()->Is_Valid_Settings(outMsg, true)) {
                    ...
                    if (display_name[0] != 0) {
                        ...
                        //
                        //	Display a message to the user asking if they really want to do
                        //this...
                        //
                        if (prompt) {
                            DlgMsgBox::DoDialog(IDS_MENU_CONTROLS_OVERWRITE_PROMPT_TITLE,
                                                IDS_MENU_CONTROLS_OVERWRITE_PROMPT_MSG,
...
//
////////////////////////////////////////////////////////////////
void ServerSaveLoadMenuClass::On_ListCtrl_Delete_Entry(ListCtrlClass* list_ctrl, int ctrl_id,
                                                       int item_index)
{
    //
...
//
////////////////////////////////////////////////////////////////
void ServerSaveLoadMenuClass::On_ListCtrl_Sel_Change(ListCtrlClass* list_ctrl, int ctrl_id,
...
//
////////////////////////////////////////////////////////////////
int CALLBACK ServerSaveLoadMenuClass::ListSortCallback(ListCtrlClass* list_ctrl, int item_index1,
                                                       int item_index2, uint32 user_param)
{
 ...
 *=============================================================================================*/
ServerSettingsClass::ServerSettingsClass(char* filename, unsigned short* configname,
                                         int file_number)
{
...
void ServerSettingsManagerClass::Scan(void)
{
    ...
    */
    ServerSettingsList.Add(new ServerSettingsClass(
        "def_svrcfg_cnc.ini", (unsigned short*)TRANSLATE(IDS_SERVER_SAVELOAD_DEFAULT), 0));
    ...
    */
    ServerSettingsList.Add(new ServerSettingsClass(
        "svrcfg_cnc.ini", (unsigned short*)TRANSLATE(IDS_SERVER_SAVELOAD_CUSTOM_DEFAULT), 1));
    ...
    for (int i = 2; i < MAX_SETTINGS_FILES; i++) {
        ...
        if (file.Is_Available()) {
            ...
            if (ini) {
                description = ini->Get_Wide_String(
                    description, "Settings", "wConfigName",
                    L""); //(unsigned short *)TRANSLATE(IDS_SERVER_SAVELOAD_DEFAULT));
                if (description.Get_Length()) {
                    ServerSettingsList.Add(
                        new ServerSettingsClass(file_name, description.Peek_Buffer(), i));
                }
                else {
                    StringClass defaultstr((unsigned short*)TRANSLATE(IDS_SERVER_SAVELOAD_DEFAULT),
                                           true);
                    ini->Get_String("Settings", "bConfigName", defaultstr.Peek_Buffer(),
                                    char_description.Peek_Buffer(), 128);
                    ServerSettingsList.Add(new ServerSettingsClass(
                        file_name, WideStringClass(char_description, true).Peek_Buffer(), i));
                }
            }
        }
    }
    ...
    if (display_name != NULL) {
        if (The_Game()) {
            ...
            */
            ...
            if (file_number != -1) {
                ServerSettingsClass* new_settings
                    = new ServerSettingsClass(filename, display_name->Peek_Buffer(), file_number);
