...
void DlgWOLSettings::On_Init_Dialog(void)
{
    ...
    if (servers.size() > 0) {
        ...
        if (BandwidthCheckerClass::Got_Bandwidth() == 0
...
void DlgWOLSettings::On_Command(int ctrl, int message, DWORD param)
{
    ...
    case IDOK: {
        ComboBoxCtrlClass* connectCombo
            = (ComboBoxCtrlClass*)Get_Dlg_Item(IDC_CONNECTION_SPEED_COMBO);

        if (connectCombo && BandwidthCheckerClass::Got_Bandwidth() == 0) {
            int sel = connectCombo->Get_Curr_Sel();

            if (sel >= 0) {
                BANDWIDTH_TYPE_ENUM bandwidth = min<BANDWIDTH_TYPE_ENUM>(
                    (BANDWIDTH_TYPE_ENUM)(sel + BANDWIDTH_FIRST), BANDWIDTH_LAST);
                ...
                if (bandwidth == BANDWIDTH_AUTO) {
                    if (!DetectingBandwidth) {
                        ...
                        Add_Ref();
                        RefPtr<WaitCondition> bandwidth_wait = BandwidthCheckerClass::Detect();
                        DlgWOLWait::DoDialog(TRANSLATE(IDS_MENU_DETECTING_BANDWIDTH),
                                             bandwidth_wait, this);
...
void DlgWOLSettings::UpdateForPersona(void)
{
    ...
    if (combo) {
        ...
        if (sel >= 0) {
            ...
            bool autoLogin
                = (mWOLSession->IsAutoLoginAllowed() && (newLogin.Compare_No_Case(preferred) == 0));
            ...
            if (server->HasLanguageCode()) {
                ...
                if (gameSelName.Is_Empty()) {
                    ...
                    if (server->MatchesLanguageCode()) {
                        ...
                        // Find the ping server with the best time that matches this server's
                        // lat/long
...
void DlgWOLSettings::InitConnectionSpeedCombo(void)
{
    ...
    if (connectCombo) {
        ...
        if (BandwidthCheckerClass::Get_Reported_Upstream_Bandwidth()) {
            auto_set_str.Format(TRANSLATE(IDS_MENU_AUTO_FORMAT),
                                BandwidthCheckerClass::Get_Bandwidth_As_String());
        }
        else {
...
void DlgWOLSettings::On_ComboBoxCtrl_Sel_Change(ComboBoxCtrlClass* combo, int ctrl, int oldSel,
                                                int newSel)
{
    switch (ctrl) {
