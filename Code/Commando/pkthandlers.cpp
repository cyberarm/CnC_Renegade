...
void cNetwork::Server_Packet_Handler(cPacket& packet, int rhost_id)
{
    ...
    WWASSERT(rhost_id >= PServerConnection->Get_Min_RHost()
    ...
    */
    ...
    if (PServerConnection->Get_Remote_Host(rhost_id) == nullptr) {
        ...
        // APPPACKETTYPE_CSDAMAGEEVENT following messages "Insufficient bandwidth to send all
        // guaranteed packets".
        //
    ...
    if ((dirty_bits & NetworkObjectClass::BIT_CREATION) == NetworkObjectClass::BIT_CREATION) {
        ...
        if (object != nullptr) {
#ifdef WWDEBUG
            // sockaddr_in *actual_from_addr_ptr = (LPSOCKADDR_IN)
            // &packet.Get_From_Address_Wrapper()->FromAddress; sockaddr_in rhost_addr =
            // PServerConnection->Get_Remote_Host(rhost_id)->Get_Address();
#endif // WWDEBUG

            // #pragma message("(TSS) APPPACKETTYPE_CSDAMAGEEVENT workaround for unknown crash
            // bug.\n")

            // WWDEBUG_SAY(("\n"));
            // WWDEBUG_SAY(("cNetwork::Server_Packet_Handler: received BIT_CREATION (id %d,
            // is_delete_pending = %d, net_classid = %d, from rhost %d - %s) for existing
            // object:\n", 	network_obj_id, is_delete_pending, net_classid, rhost_id,
            //Addr_As_String(&rhost_addr))); WWDEBUG_SAY(("  id                 = %d\n",
            // object->Get_Network_ID())); WWDEBUG_SAY(("  AppPacketType      = %s\n",
            // cAppPacketStats::Interpret_Type(object->Get_App_Packet_Type()))); WWDEBUG_SAY(("
            // IsDeletePending    = %d\n", object->Is_Delete_Pending())); WWDEBUG_SAY(("
            // CreatedByPacketID  = %d\n", object->Get_Created_By_Packet_ID()));
            // WWDEBUG_SAY(("Packet actually from address %s\n",
            // Addr_As_String(actual_from_addr_ptr))); WWDEBUG_SAY(("\n"));
    ...
    if (object != nullptr) {
        ...
        //	Do we need to modify this object?
        //
        if ((dirty_bits & NetworkObjectClass::BIT_OCCASIONAL)
...
void cNetwork::Client_Packet_Handler(cPacket& packet)
{
    ...
    if ((dirty_bits & NetworkObjectClass::BIT_CREATION) == NetworkObjectClass::BIT_CREATION) {
        ...
        // WWASSERT (object == nullptr);
        if (object != nullptr) {
            WWDEBUG_SAY(("cNetwork::Client_Packet_Handler: received BIT_CREATION for existing "
                         "object of type %s\n",
                         cAppPacketStats::Interpret_Type(object->Get_App_Packet_Type())));
            DIE;
        }
