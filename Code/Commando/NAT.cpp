 ...
 *=============================================================================================*/
void FirewallHelperClass::Set_Firewall_Info(unsigned long last_behavior, int last_delta,
                                            unsigned short port_pool, bool send_delay,
                                            int confidence)
{
    if (port_pool) {
 ...
 *   8/20/2001 12:16PM ST : Created                                                            *
 *=============================================================================================*/
void FirewallHelperClass::Get_Firewall_Info(unsigned long& last_behavior, int& last_delta,
                                            unsigned short& port_pool, bool& send_delay,
...
unsigned long FirewallHelperClass::NAT_Thread_Main_Loop(void)
{
    ...
    */
    while (ThreadActive) {
        ...
        /*
        ** Always do some sleeping here since this thread is a relatively low priority compared to
        *stuff like running the
        ** game engine.
 ...
 *=============================================================================================*/
bool FirewallHelperClass::Send_To_Mangler(IPAddressClass* address,
                                          ...
                                          unsigned long packet_id, bool blitzme)
{
    ...
    */
    ...
    int packet_size
        = Build_Mangler_Packet(packet_buf, socket_handler->Get_Incoming_Port(), packet_id, blitzme);
 ...
 *=============================================================================================*/
unsigned short FirewallHelperClass::Get_Mangler_Response(unsigned long packet_id,
                                                         SocketHandlerClass* socket_handler,
                                                         int time, bool all_service)
{
    ...
    */
    ...
    while (timeout > TIMEGETTIME()) {
        ...
        /*
        ** Get a copy of the global packet we want to examine. This doesn't remove it from the
        *queue.
        */
        int result
            = socket_handler->Peek(packet_buf, packet_size, &temp_address, &temp_port, peek_packet);
                ...
                */
                ...
                if (packet_id == id) {
                    ...
                    */
                    ...
                    WWDEBUG_SAY(("FirewallHelper: Mangler is seeing packets from port %d as coming "
                                 "from port %d\n",
...
FirewallHelperClass::FirewallBehaviorType FirewallHelperClass::Detect_Firewall_Behavior(void)
{
    ...
    */
    static char _manglername[5][80] = { "mangler1.westwood.com", "mangler2.westwood.com",
                                        "mangler3.westwood.com", "mangler.westwood.com", "" };
    ...
    */
    if (WOLNATInterface.Get_Force_Port()) {
        ...
        if (SendDelay) {
            ...
            WWDEBUG_SAY(
                ("FirewallHelper: Netgear bug specified by command line or SendDelay flag\n"));
        }
        return (behavior);
    }
    ...
    */
    if (SlaveMaster.Am_I_Slave()) {
        ...
        ExternalAddress.Set_Address(WOLNATInterface.Get_Reg_External_IP(),
                                    WOLNATInterface.Get_Reg_External_Port());
        return (behavior);
    }
    ...
    */
    do {
        ...
        */
        ...
        if (!host_info) {
            WWDEBUG_SAY(
                ("FirewallHelper: gethostbyname failed! Error code %d\n", WSAGetLastError()));
            break;
        }
        ...
        */
        if (!found) {
            ...
            WWDEBUG_SAY(("FirewallHelper: Found mangler address at %d.%d.%d.%d\n",
                         mangler_addresses[m][0], mangler_addresses[m][1], mangler_addresses[m][2],
    ...
    */
    ...
    if (!socket.Open(port, 4321)) {
        WWDEBUG_SAY(
            ("FirewallHelper: Unable to open temp source port - returning default behavior\n"));
        return (behavior);
    }
    ...
    */
    ...
    if (mangled_port == 0) {
        ...
        ** We already tried the first one so skip that. Don't use the last one either since that
        *will be needed later to detect
    ...
    */
    if (second_ip_mangled_port == 0 || second_ip_mangled_port == port) {
        WWDEBUG_SAY(("FirewallHelper: No response or response is unmangled - returning simple "
                     "firewall behavior\n"));
        return (FIREWALL_TYPE_SIMPLE);
    }

    /*
    ** Got a mangled port. Is it the same as the last one (mangles the same regardless of
    *destination IP).
    */
    if (mangled_port == second_ip_mangled_port) {
        behavior = FIREWALL_TYPE_DUMB_MANGLING;
        WWDEBUG_SAY(
            ("FirewallHelper: Source ports mangled the same way regardless of destination IP\n"));
    }
    else {
    ...
    */
    ...
    for (try_again = 0; try_again < 3; try_again++) {
            ...
            if (!port_sockets[i]->Open(source_ports[i], 4321)) {
                ...
                */
                ...
                WWDEBUG_SAY(("FirewallHelper: Unable to open all source ports for allocation "
                             "pattern test - returning default behavior\n"));
        ...
        while (TIMEGETTIME() < timeout && num_responses < NUM_TEST_PORTS) {
            for (i = 0; i < NUM_TEST_PORTS; i++) {
                if (mangled_ports[i] == 0) {
                    Send_To_Mangler(&manglers[mangler_index_offset], port_sockets[i],
                                    packet_id + i);
                    mangled_ports[i]
                        = Get_Mangler_Response(packet_id + i, port_sockets[i], 0, true);
                    if (mangled_ports[i] != 0) {
                        num_responses++;
                    }
                }
        ...
        */
        if (num_responses < 4) {
            if (LastSourcePortAllocationDelta != 0
                && (int)LastBehavior > (int)FIREWALL_TYPE_SIMPLE) {
                /*
                ** If the delta we got last time we played looks good then use that.
                */
                SourcePortAllocationDelta = LastSourcePortAllocationDelta;
                WWDEBUG_SAY(("FirewallHelper: Unable to verify responses from 4 mangler servers - "
                             "returning default behavior\n"));
                return (behavior);
            }
            /*
            ** Try again.
        ...
        delta = Get_NAT_Port_Allocation_Scheme(num_responses, source_ports, mangled_ports,
                                               relative_delta, looks_good);
    ...
    */
    if ((behavior & FIREWALL_TYPE_SMART_MANGLING) != 0) {
        ...
        if ((behavior & FIREWALL_TYPE_SIMPLE_PORT_ALLOCATION) != 0) {
            ...
            WWDEBUG_SAY(("FirewallHelper: Testing to see if the NAT mangles differently per "
            ...
            */
            ...
            if (new_mangled_port != 0) {
                if (new_mangled_port != mangled_port + SourcePortAllocationDelta) {
                    WWDEBUG_SAY(("FirewallHelper: NAT uses different source ports for different "
                                 "destination ports\n"));
                    ...
                    if (new_mangled_port == mangled_port + SourcePortAllocationDelta) {
                        WWDEBUG_SAY(("FirewallHelper: NAT uses the same source port for different "
                                     "destination ports\n"));
                    }
                    else {
                        WWDEBUG_SAY((
                            "FirewallHelper: Unable to complete destination port mangling test\n"));
                        fw_assert(false);
                    }
                }
            }
    ...
    */
    if (save_firewall_type) {
        ...
        /*
        ** If we have high confidence in the last settings we detected then we might want to use
        *those in preference.
        */
 ...
 *=============================================================================================*/
int FirewallHelperClass::Get_NAT_Port_Allocation_Scheme(int num_ports,
                                                        unsigned short* original_ports,
                                                        unsigned short* mangled_ports,
                                                        bool& relative_delta, bool& looks_good)
{
    fw_assert(num_ports > 3);

    WWDEBUG_SAY(
        ("FirewallHelper: Looking for port allocation pattern in original_ports %d, %d, %d, %d\n",
    ...
    */
    if (diff1 == diff2 && diff2 == diff3) {
        WWDEBUG_SAY(
            ("FirewallHelper: Looks good for absolute allocation sequence delta of %d\n", diff1));
        relative_delta = false;
        looks_good = true;
        return (diff1);
    }
    ...
    */
    if (diff1 == diff2 || diff1 == diff3) {
        WWDEBUG_SAY(
            ("FirewallHelper: Detected probable broken relative port range delta of %d\n", diff1));
        relative_delta = true;
        looks_good = false;
        return (diff1);
    }
...
unsigned short FirewallHelperClass::Get_Next_Mangled_Source_Port(unsigned short source_port)
{
    ...
    */
    ...
    /*
    ** If our NAT uses the same mangled source port regardless of the dest IP then we can use any
    *previous connection to a different IP.
    */
    if (WOLNATInterface.Am_I_Server() && ((unsigned long)FIREWALL_TYPE_DUMB_MANGLING & fw) != 0) {
        for (int h = 0; h < ConnectionHistory.Count(); h++) {

            /*
            ** Make sure it isn't my address (should never be, even with players from behind the
            *same firewall).
            */
            if (!ConnectionHistory[h].Is_IP_Equal(ExternalAddress)) {
                WWDEBUG_SAY(("FirewallHelper - same port regardless of dest ip - using port %d "
                             "from connection to %s ; %d\n",
                             (unsigned long)MangledPortHistory[h],
                             ConnectionHistory[h].As_String()));
    ...
    */
    ...
    if (NumManglerServers > 0) {
        ...
        bool got_name
            = WOLNATInterface.Get_Mangler_Name_By_Index(CurrentManglerServer, mangler_name);

        if (got_name) {
            unsigned short servserv_port
                = WOLNATInterface.Get_Mangler_Port_By_Index(CurrentManglerServer);
            fw_assert(servserv_port != 0);
            if (servserv_port) {
                mangler_port = servserv_port;
            }
    ...
    */
    ...
    // maddress[1], maddress[2], maddress[3], (int)mangler_port));
    ...
    */
    if (((unsigned long)FIREWALL_TYPE_DUMB_MANGLING & fw) != 0) {
        WWDEBUG_SAY(
            ("FirewallHelper - Dumb firewall, returning next mangled port as %d\n", mangled_port));
        return (mangled_port);
    }
    ...
    if (((unsigned long)FIREWALL_TYPE_SIMPLE_PORT_ALLOCATION & fw) != 0) {
        ...
        */
        if (SourcePortAllocationDelta == 100) {
            ...
            **
            ** To get the current index we need to take the mangled port and subtract 1700 and the
            *source port
            */
 ...
 *=============================================================================================*/
int FirewallHelperClass::Build_Mangler_Packet(unsigned char* buffer, unsigned short port,
                                              unsigned long packet_id, bool blitzme)
{
    /*
    ** Asserts.
...
void FirewallHelperClass::Talk_To_New_Player(WOL::User* user)
{
    ...
    */
    ...
    sprintf(options.OptionData.Invitation.LocalIP, "%08x,",
    ...
    sprintf(options.OptionData.Invitation.ExternalIP, "%08x,",
            (unsigned long)ExternalAddress.Get_Address());
...
void FirewallHelperClass::Process_Game_Options(void)
{
    ...
    */
    if (WOLNATInterface.Get_Private_Game_Options(&user, options_buffer,
                                                 OPTIONS_STAGING_BUFFER_SIZE)) {
        ...
        */
        ...
        /*
        ** This option represents a game server inviting us to negotiate a connection through
        *firewalls. It will contain his
        ...
        */
        case WOLNATInterfaceClass::OPTION_INVITE_PORT_NEGOTIATION:
            ...
            if (!WOLNATInterface.Am_I_Server()) {
                ...
                */
                ...
                sscanf(options->OptionData.Invitation.FirewallType, "%08x",
                ...
                WWDEBUG_SAY(("FirewallHelper - Received port negotiation invitation. %d players in "
                             "the queue ahead of me\n",
                             QueuedPlayers));
                WWDEBUG_SAY(("FirewallHelper - Server is %s. Local addr = %s, ", PlayersName,
                ...
                */
                ...
                send_options.Option
                    = WOLNATInterfaceClass::OPTION_ACCEPT_PORT_NEGOTIATION_INVITATION;
                sprintf(send_options.OptionData.Accept.LocalIP, "%08x,",
                        (unsigned long)LocalChatConnectionAddress.Get_Address());
                sprintf(send_options.OptionData.Accept.LocalPort, "%04x,", ClientPort);
                sprintf(send_options.OptionData.Accept.ExternalIP, "%08x,",
                        (unsigned long)ExternalAddress.Get_Address());
                sprintf(send_options.OptionData.Accept.FirewallType, "%08x",
                        Get_Raw_Firewall_Behavior());
        ...
        /*
        ** This option represents a game client accepting our invitation to negotiate a connection
        *through firewalls. It
        ...
        case WOLNATInterfaceClass::OPTION_ACCEPT_PORT_NEGOTIATION_INVITATION:
            ...
            if (WOLNATInterface.Am_I_Server()) {
                ...
                */
                ...
                /*
                ** Make sure this client isn't in the client list already (Should never happen
                *but...)
                */
                if (Remove_Player_From_Negotiation_Queue(client->Name)) {
                    WWDEBUG_SAY(("FirewallHelper OPTION_ACCEPT_PORT_NEGOTIATION_INVITATION - "
                ...
                */
                ...
                WWDEBUG_SAY(("FirewallHelper - Got client accept from %s. Local addr = %s,",
                             client->Name, client->LocalAddress.As_String()));
                WWDEBUG_SAY((" external addr = %s, firewall = %08x\n",
        ...
        */
        case WOLNATInterfaceClass::OPTION_ABORT_NEGOTIATION:
            WWDEBUG_SAY(
                ("FirewallHelper - Got OPTION_ABORT_NEGOTIATION from %s\n", (char*)user.name));
            {
                ThreadLockClass locker(this);
                if (WOLNATInterface.Am_I_Server()) {
                ...
                */
                if (Remove_Player_From_Negotiation_Queue((char*)user.name)) {
                    WWDEBUG_SAY(
                        ("FirewallHelper OPTION_ABORT_NEGOTIATION - Removed %s from ClientQueue\n",
                         (char*)user.name));
                }
        ...
        */
        case WOLNATInterfaceClass::OPTION_CLIENT_IN_CHANNEL:
            WWDEBUG_SAY(
                ("FirewallHelper - Got OPTION_CLIENT_IN_CHANNEL from %s\n", (char*)user.name));
            if (WOLNATInterface.Am_I_Server()) {
...
{
    /*
    ** This is the main thread removing entries from the client queue that it wasn't able to remove
    *before due to thread
    ** contention.
    */
    fw_assert(Get_Main_Thread_ID() == GetCurrentThreadId());

    while (ClientQueueRemoveList.Count()) {
        if (Remove_Player_From_Negotiation_Queue_If_Mutex_Available(
                ClientQueueRemoveList[0]->Name)) {
...
bool FirewallHelperClass::Remove_Player_From_Negotiation_Queue_If_Mutex_Available(char* player_name)
{
    ...
    /*
    ** If we can grab the mutex then remove the item immediately, otherwise add it to a list and
    *return.
    */
    if (locker.WaitResult == WAIT_OBJECT_0) {
        ...
        /*
        ** Just add the name to a list that will be checked later when it's safe to remove stuff
...
void FirewallHelperClass::Send_My_Port(unsigned short port)
{
    ...
    ThreadLockClass locker(this);
    WWDEBUG_SAY(
        ("FirewallHelper - Sending my port number (%d) to %s\n", (unsigned int)port, PlayersName));
 ...
 *=============================================================================================*/
void FirewallHelperClass::Set_Client_Connect_Event(HANDLE thread_event, HANDLE cancel_event,
                                                   int* flag_ptr, int* queue_ptr)
{
    ThreadLockClass locker(this);
...
int FirewallHelperClass::Negotiate_Port(void)
{
    ...
    ** We will use the game comms to send and receive on the server side since we already have a
    ...
    */
    if (server) {
        ...
        */
        ...
        /*
        ** On the client side, we need to open a new socket using the client side port number. Use
        *any port number for the
        ** second parameter since it's the default send port and will always be overridden when we
        *send anyway.
        */
    ...
    WWDEBUG_SAY(("FirewallHelper - Negotiating port with player %s, local address = %s,",
                 PlayersName, PlayersLocalAddress.As_String()));
    ...
    */
    ...
    /*
    ** Maybe neither of our firewalls use port mangling. If so, then we can use the default port
    *number.
    */
    if (mangling) {
        if (Behavior == 0 || (Behavior & FIREWALL_TYPE_SIMPLE) != 0) {
            if (PlayersFirewallType == 0 || (PlayersFirewallType & FIREWALL_TYPE_SIMPLE) != 0) {
                /*
                ** Ok, we both have crap firewalls or modems or somesuch. Anyway, this should be
                *easy.
                */
                mangling = false;
            }
    ...
    do {
        ...
        */
            ...
            */
            if (server && trying < 2) {
                if ((Behavior & FirewallHelperClass::FIREWALL_TYPE_DESTINATION_PORT_DELTA) == 0) {
                    WWDEBUG_SAY(("FirewallHelper: No destination port delta - looking for earlier "
                    ...
                    for (int r = 0; r < ConnectionHistory.Count(); r++) {
                        if (ConnectionHistory[r].Is_IP_Equal(PlayersExternalAddress)) {
                            ...
                            used_old_port = true;
                            WWDEBUG_SAY(("FirewallHelper - Found old connection port number to use "
                                         "- port number %d\n",
                                         mangled_port));
                            break;
                        }
            ...
            /*
            ** If this isn't the first try then we might already know what the port will be since it
            *can't change between tries.
            */
            if (mangled_port == 0) {
                if ((Behavior & FirewallHelperClass::FIREWALL_TYPE_DESTINATION_PORT_DELTA) == 0) {
                    if (trying > 0) {
                        if (!used_old_port) {
                            ...
                            WWDEBUG_SAY((
                                "FirewallHelper - No dest port delta - using last port number %d\n",
                                mangled_port));
                        }
                        else {
                            used_old_port = false;
        ...
        /*
        ** Wait until we know what port to send to. Allow plenty of time for slow chat server
        *response.
        ...
        */
        ...
        while (TIMEGETTIME() < timeout
               && ((mangling == true && PlayersMangledPort == last_send_port)
        ...
        if (PlayersMangledPort != 0) {
            ...
            **
            ** When it gets an ICMP port unreachable, the netgear invalidates any existing port
            *mappings.
            **
            ** The delay has to be long enough for our port number to be passed through the chat
            *server
            ...
            */
            ...
            /*
            ** Send the packet to the other player. If a NAT mangles an outgoing port number then it
            *will be noted
            ** as the packet comes in.
            */
            WWDEBUG_SAY(
                ("FirewallHelper: Sending PACKETTYPE_FIREWALL_PROBE packet to %s at port %d\n",
            ...
            do {
                if (resend_timer < TIMEGETTIME()) {
                    // if (!CrapFlag)
                    WOLNATInterface.Send_Game_Format_Packet_To(&PlayersFirewallAddress, packet,
                                                               strlen(packet) + 1, socket);
                ...
                */
                ...
                result = WOLNATInterface.Get_Packet(receive_packet, sizeof(receive_packet),
                                                    receive_address);
                ...
                */
                if (gotit && PlayersConnectionResult == CONNRESULT_CONNECTED) {
                    WWDEBUG_SAY(("FirewallHelper: Got connection result - my packet must have got "
                                 "through\n"));
                    break;
                }
                ...
                */
                if (PlayersConnectionResult >= CONNRESULT_TRY1
                    && PlayersConnectionResult < CONNRESULT_CONNECTED) {
                    int their_try = PlayersConnectionResult - CONNRESULT_TRY1;
                    if (their_try > trying) {
                        WWDEBUG_SAY(
                            ("FirewallHelper: Other player is trying again - their_try = %d\n",
                             their_try));
                        trying = their_try - 1;
                        break;
                    }
...
bool FirewallHelperClass::Get_Local_Chat_Connection_Address(void)
{
    ...
    *((unsigned long*)(&server_address[0])) = temp;
    ...
    WWDEBUG_SAY(("FirewallHelper - Host address is %d.%d.%d.%d\n", server_address[3],
                 server_address[2], server_address[1], server_address[0]));
    ...
    */
    SnmpExtensionInitPtr
        = (int(__stdcall*)(unsigned long, void**, AsnObjectIdentifier*))GetProcAddress(
            mib_ii_dll, "SnmpExtensionInit");
    SnmpExtensionQueryPtr
        = (int(__stdcall*)(unsigned char, SnmpVarBindList*, long*, long*))GetProcAddress(
    ...
    RFC1157VarBindList* bind_list_ptr
        = (RFC1157VarBindList*)SnmpUtilMemAllocPtr(sizeof(RFC1157VarBindList) + 8192);
    ...
    */
    while (true) {
        ...
        if (!SnmpExtensionQueryPtr(ASN_RFC1157_GETNEXTREQUEST, bind_list_ptr, &error_status,
                                   &error_index)) {
        ...
        */
        case tcpConnRemAddress:
            fw_assert(index < connection_list.Count());
            connection_list[index]->RemoteIP
                = *((unsigned long*)bind_ptr->value.asnValue.address.stream);
            index++;
            break;
    ...
    SnmpUtilMemFreePtr(bind_list_ptr);
    SnmpUtilMemFreePtr(bind_ptr);
    // SnmpUtilMemFreePtr(mib_ii_name_ptr);		// Don't free this - the
    // SnmpExtensionQueryPtr call frees it apparently

    WWDEBUG_SAY(
        ("FirewallHelper - Got %d connections in list, parsing...\n", connection_list.Count()));
    ...
    */
            ...
            if (server_port == 0 || server_port == (unsigned int)connection->RemotePort) {
                ...
                */
                if (connection->State == ESTABLISHED) {
                    ...
                    my_address.Set_Address((unsigned char*)&connection->LocalIP,
                ...
                WWDEBUG_SAY(
                    ("FirewallHelper - Connection has different port. Port is %d, looking for %d\n",
    ...
    if (found) {
        WWDEBUG_SAY(
            ("FirewallHelper - Using address %s to talk to chat server\n", my_address.As_String()));
...
unsigned short FirewallHelperClass::Get_Raw_Firewall_Behavior(void)
{
    ...
    /*
    ** If we are forcing a particular port to be used then set the behavior type to be as if there
    *was no firewall there at all.
    */
