    ...
    if (list) {
        ...
        if (lobbyList.empty()) {
            DlgMsgBox::DoDialog(TRANSLATE(IDS_MENU_SERVER_MESSAGE_TITLE),
                                TRANSLATE(IDS_MENU_NO_CHAT_AVAIL));
            End_Dialog();
            return;
        }
...
//
////////////////////////////////////////////////////////////////
void MPWolChatMenuClass::Update_User_Status(ListCtrlClass* list, int index,
                                            const RefPtr<UserData>& user)
{
    WWASSERT(list);
...
void MPWolChatMenuClass::HandleNotification(WOLChatMgrEvent& event)
{
    ...
    case KickedFromChannel:
        DlgMsgBox::DoDialog(TRANSLATE(IDS_MENU_SERVER_MESSAGE_TITLE),
                            TRANSLATE(IDS_MENU_WOL_KICK_USER_MESSAGE));
        End_Dialog();
        break;
