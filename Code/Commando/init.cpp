...
static const float _ParticleLODScreenSizes[17]
    = { 0.0025f, 0.0050f, 0.0075f, 0.0100f, 0.0300f, 0.0500f, 0.0700f, 0.0900f,           0.1000f,
        0.3000f, 0.5000f, 1.0000f, 1.2500f, 1.5000f, 1.7500f, 2.0000f, NO_MAX_SCREEN_SIZE };
...
**
*/
void _stdcall AudioTextCallback(AudibleSoundClass* sound_obj, const StringClass& text,
                                uint32 user_param)
{
...
static bool Create_Log_File_Name(const StringClass& folder, StringClass& filename,
                                 bool use_numbering)
{
...
public:
    ...
    void Thread_Function()
    {
        ...
        reg.Get_String("LogPath", path, sizeof(path),
                       "\\\\tanya\\game\\projects\\renegade\\_error_logs");
        strcat(path, "\\");
...
bool Game_Init(void)
{
    ...
    // Set registry key to 1 for the duration of the init. This way we know if the program crashed
    // while the init.
    RegistryClass registry(APPLICATION_SUB_KEY_NAME_DEBUG);
    if (registry.Is_Valid()) {
    ...
    _RenegadeFileFactory.Add_FileFactory(
        new MixFileFactoryClass("Always2.dat", &RenegadeBaseFileFactory), "Always2.dat");
    _RenegadeFileFactory.Add_FileFactory(
        new MixFileFactoryClass("Always.dbs", &RenegadeBaseFileFactory), "Always.dbs");
    _RenegadeFileFactory.Add_FileFactory(
        new MixFileFactoryClass("Always.dat", &RenegadeBaseFileFactory), "Always.dat");
    ...
    for (file_find = ::FindFirstFile("data\\*.mix", &find_info);
        ...
        //
        _RenegadeFileFactory.Add_FileFactory(
            new MixFileFactoryClass(find_info.cFileName, &RenegadeBaseFileFactory),
            find_info.cFileName);
    }
    ...
    */
    ...
    if (AutoRestart.Get_Restart_Flag() || ServerSettingsClass::Is_Command_Line_Mode()
    ...
    if (TranslateDBClass::Get_Version_Number() != STRINGS_VER) {
        MessageBox(0,
                   "This build of Renegade is out of sync with the strings database (strings.tdb). "
                   " Strings will be incorrect and may cause the game to crash.",
