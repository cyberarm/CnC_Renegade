...
void TextDisplayGameModeClass::Load_Registry_Keys(void)
{
    ...
    if (registry->Is_Valid()) {
        ...
        ScrollLinesPersistTime
            = registry->Get_Float("ScrollLinesPersistTime", ScrollLinesPersistTime);
    }
    delete registry;
}
...
void TextDisplayGameModeClass::Render()
{
    ...
    if (!changed) {
        ...
        for (linenode = ScrollLines.Head(); linenode; linenode = linenode->Next(), j++) {
            ...
            if (RendererColors[j] != linenode->Data()->Color
                || RendererLines[j] != linenode->Data()->Text) {
                changed = true;
                break;
            }
        }
...
bool StatisticsDisplayManager::Is_Current_Display(
    const char* title) // Returns true if "title" is currently active
{
