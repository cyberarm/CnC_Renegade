...
bool CombatNetworkReceiverInstanceClass::Server_Update_Dynamic_Objects(bool is_urgent)
{
    ...
    if (!is_urgent
        && (time_now - last_update_time < 1000 / (float)cUserOptions::NetUpdateRate.Get())) {
        return (false);
    }
    last_update_time = time_now;

    //
    // cRemoteHost figures out when to update priorities based on this so keep it in sync with the
    // NetUpdateRate.
    //
