...
//-----------------------------------------------------------------------------
void cBandwidthGraph::Bandwidth_Graph(StringClass& label, int bps, int target_bps,
                                      float bandwidth_multiplier, float average_priority,
                                      bool is_loading)
{
    if (PTextRenderer == NULL) {
        return;
...
void cBandwidthGraph::Think(void)
{
    ...
    if (cNetwork::I_Am_Client() && cNetwork::PClientConnection->Is_Established()) {
        ...
        bps = PacketManager.Get_Compressed_Bandwidth_Out(
            &cNetwork::Get_Client_Rhost()->Get_Address());
        ...
        text.Format("c%d<-s", cNetwork::Get_My_Id());
        bps = PacketManager.Get_Compressed_Bandwidth_In(
            &cNetwork::Get_Client_Rhost()->Get_Address());
    ...
    if (cNetwork::PServerConnection != NULL) {
        ...
        for (int i = cNetwork::PServerConnection->Get_Min_RHost();
            ...
            if (p_rhost != NULL) {
                ...
                Bandwidth_Graph(text, bps, target_bps, p_rhost->Get_Bandwidth_Multiplier(),
                                p_rhost->Get_Average_Priority(), is_loading);
