...
//
    ...
    //	Load the level
    //
    CombatGameModeClass* game_mode
        = static_cast<CombatGameModeClass*>(GameModeManager::Find("Combat"));
...
void GameInitMgrClass::Continue_Game(void)
{
    ...
    // IML : First, allow a short period to process any outstanding sound effects that may have been
    // started by the caller. NOTE: Multi-play does not need this fix because it does not sound page
    // swap.
    if (IS_SOLOPLAY) {
        time = TIMEGETTIME();
        while (TIMEGETTIME() - time < PRE_SERVICE_TIME) {
...
#ifdef WWDEBUG
    cRemoteHost::Set_Allow_Extra_Modem_Bandwidth_Throttling(
        cDevOptions::ExtraModemBandwidthThrottling.Get());
#endif // WWDEBUG
