                ...
                if (The_Game()->Is_Valid_Settings(outMsg, true)) {
                    ...
                    if (cUserOptions::Get_Bandwidth_Type() == BANDWIDTH_AUTO) {
                        if (The_Game()->Get_Max_Players() > 2) {
                            ...
                            WWDEBUG_SAY(("MPLanHostOptionsMenuClass::On_Command - Setting "
                                         "BandwidthBps to %d\n",
                                         set_bw));
    ...
    */
    ...
    if (wolGame || cGameSpyAdmin::Is_Gamespy_Game()) {
        ...
        Set_Dlg_Item_Int(IDC_NUM_PLAYERS_EDIT,
                         min(max_players, NetworkObjectClass::MAX_CLIENT_COUNT - 1));
    }
    else {
        Set_Dlg_Item_Int(
            IDC_NUM_PLAYERS_EDIT,
...
bool MPLanHostBasicOptionsTabClass::On_Apply(void)
{
    ...
    The_Game()->Set_Max_Players(
        min(Get_Dlg_Item_Int(IDC_NUM_PLAYERS_EDIT), NetworkObjectClass::MAX_CLIENT_COUNT - 1));
    ...
    if (!GameModeManager::Find("WOL")->Is_Active()
        && !cGameSpyAdmin::Get_Is_Server_Gamespy_Listed()) {
        ComboBoxCtrlClass* nic_combobox = (ComboBoxCtrlClass*)Get_Dlg_Item(IDC_HOSTING_IP_COMBO);
        if (nic_combobox != NULL) {
...
void MPLanHostAdvancedOptionsTabClass::On_Init_Dialog(void)
{
    ...
    if (The_Game()
            ->IsDedicated.Is_False()) { // || (mIsWOLGame &&
                                        // strlen(MPSettingsMgrClass::Get_Auto_Login()) == 0)) {
        Check_Dlg_Button(IDC_SERVER_RESTART_CHECK, false);
        Enable_Dlg_Item(IDC_SERVER_RESTART_CHECK, false);
    }
    ...
    //
    MPLanHostOptionsMenuClass* parent_dlg
        = static_cast<MPLanHostOptionsMenuClass*>(Get_Parent_Dialog());
    if (parent_dlg != NULL) {
...
void MPLanHostAdvancedOptionsTabClass::On_Command(int ctrl_id, int message_id, DWORD param)
{
    ...
    case IDC_SERVER_RESTART_CHECK: {
        if (Is_Dlg_Button_Checked(IDC_SERVER_RESTART_CHECK)) {
            if (GameModeManager::Find("WOL")->Is_Active()
                && strlen(MPSettingsMgrClass::Get_Auto_Login()) == 0) {
                DlgMsgBox::DoDialog(IDS_MENU_TEXT329, IDS_MENU_NEED_AUTO_LOGIN, DlgMsgBox::YesNo,
                                    this);
                Check_Dlg_Button(IDC_SERVER_RESTART_CHECK, false);
            }
        }
    ...
    case IDC_REMIX_TEAMS_CHECK: {
        ...
        bool canClan = (!mRemixTeams && !Is_Dlg_Button_Checked(IDC_TEAM_CHANGE_CHECK)
                        && !Is_Dlg_Button_Checked(IDC_ALLOW_QUICKMATCH));
        Enable_Dlg_Item(IDC_CLAN_GAME_CHECK, canClan);
        Check_Dlg_Button(IDC_CLAN_GAME_CHECK, canClan && mClanGame);
    } break;
...
void MPLanHostAdvancedOptionsTabClass::ConfigureWOLControls(void)
{
    ...
    //---------------------------------------------------------------------------
    bool canClan = (mIsWOLGame && !isQuickmatch && !isTeamChange && !isTeamRemix
                    && (isClanMember || isDedicated));
    Enable_Dlg_Item(IDC_CLAN_GAME_CHECK, canClan);
...
bool MPLanHostMapCycleOptionsTabClass::On_Apply(void)
{
    ...
    if (list_ctrl != NULL) {
        ...
        if (combobox_ctrl != NULL) {
            ...
            if (curr_sel != -1) {
                const ModPackageClass* package
                    = (const ModPackageClass*)combobox_ctrl->Get_Item_Data(curr_sel);
                if (package != NULL) {
...
//
////////////////////////////////////////////////////////////////
void MPLanHostMapCycleOptionsTabClass::On_ListCtrl_DblClk(ListCtrlClass* list_ctrl, int ctrl_id,
                                                          int item_index)
{
    switch (ctrl_id) {
...
//
////////////////////////////////////////////////////////////////
void MPLanHostMapCycleOptionsTabClass::On_ComboBoxCtrl_Sel_Change(ComboBoxCtrlClass* combo_ctrl,
                                                                  int ctrl_id, int old_sel,
                                                                  int new_sel)
{
    Populate_Map_List_Ctrl();
    return;
...
bool MPLanHostVictoryOptionsTabClass::On_Apply(void)
{
    ...
    if (cnc_mode != NULL) {
        ...
        cnc_mode->BaseDestructionEndsGame.Set(
            Is_Dlg_Button_Checked(IDC_DESTROY_ALL_BUILDINGS_CHECK));
