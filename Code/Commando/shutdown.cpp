...
static void Get_Detail_String(StringClass& str)
{
    ...
    if (registry.Is_Valid()) {
        ...
        int texture_filter = registry.Get_Int(VALUE_NAME_TEXTURE_FILTER_MODE,
...
void Get_Compact_Detail_String(StringClass& str)
{
    ...
    if (registry.Is_Valid()) {
        ...
        StringClass tmp;
        tmp.Format("%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t", dynamic_lod, static_lod, shadow_mode,
                   dynamic_shadows, static_shadows, prelit_mode, texture_red, surface_effect,
                   particle_detail, texture_filter);
        str += tmp;
    ...
    if (registry.Is_Valid()) {
        ...
        if (!disable) {
            if (!SysInfoCopyThread.Is_Running()) {
                ...
                tmp.Format("%d_%d_", DX8Wrapper::Get_Current_Caps()->Get_Vendor(),
                           DX8Wrapper::Get_Current_Caps()->Get_Device());
                filename += tmp;
                filename += name;
                filename += ".txt";
