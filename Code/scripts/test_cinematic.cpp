...
#define MAX_COMMAND_LOAD_SIZE 200
    ...
    char* Get_First_Parameter(char* str)
    {
        return Get_Command_Parameter(str);
    }
    char* Get_Next_Parameter(void)
    {
    ...
    */
    void Command_Create_Object(char* params)
    {
        ...
        GameObject* obj
            = Commands->Create_Object("Generic_Cinematic", Commands->Get_Position(Owner()));
    ...
    */
    ...
    void Command_Play_Animation(char* params)
    {
        ...
        if (slot != -1) {
            ...
            GameObject* obj = Commands->Find_Object(id);
            if (obj) {
                Commands->Set_Animation(obj, name, looping, sub_obj_name, FrameSync, -1.0F,
                                        is_blended);
                ...
                //				Commands->Debug_Message( "Slot Object not found
                //%d\n", slot );
            }
        }
    }
    ...
    void Command_Attach_Script(char* params)
    {
        ...
        if (obj_slot != -1) {
            ...
            GameObject* obj = Commands->Find_Object(id);
            if (obj) {
                //				Commands->Debug_Message( "Attaching Script %s
    ...
    */
    ...
    void Parse_Commands(GameObject * obj)
    {
        ...
        //		Commands->Debug_Message( "Cinematic Time %1.3f Frame %1.3f Bump Time
    ...
    */
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if (type >= M00_CUSTOM_CINEMATIC_SET_SLOT) {
            ...
            if (slot < NUM_SLOTS) {
                if (ObjectSlots[slot] != 0) {
                    //					Commands->Debug_Message( "Slot number %d
                    //used by %d\n", slot, ObjectSlots[ slot ] );
                }
                else {
                    ObjectSlots[slot] = param;
                    //					Commands->Debug_Message( "Slot number %d set
                    //to %d\n", slot, ObjectSlots[ slot ] );
                }
            }
        }
