        ...
        if (timer_id == M00_TIMER_SIEGE_ZONE) {
            if (my_actor_id) {
                ...
                if (actor_obj) {
                    ...
                    Commands->Send_Custom_Event(obj, actor_obj,
                                                M00_CUSTOM_SIEGE_ZONE_CHECKING_ACTOR, 0, 0.0f);
                }
                else {
                    SCRIPT_DEBUG_MESSAGE(
                        ("M00_Siege_Zone_RAD cannot find the actor, clearing zone.\n"));
                    my_actor_id = 0;
                }
            }
            else {
                SCRIPT_DEBUG_MESSAGE(("M00_Siege_Zone_RAD has no actor, shouting for one.\n"));
                Commands->Create_Logical_Sound(obj, M00_SOUND_SIEGE_ZONE, my_loc,
                                               Get_Float_Parameter("Attraction_Radius"));
            }
            if (script_active) {
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if ((!my_actor_id) && (type == M00_CUSTOM_SIEGE_ACTOR_GROUP_CHECK)) {
            ...
            if ((param == my_group_id) && (sender)) {
                SCRIPT_DEBUG_MESSAGE(
                    ("M00_Siege_Zone_RAD verified that this actor is with the same group.\n"));
                my_actor_id = Commands->Get_ID(sender);
                Commands->Send_Custom_Event(obj, sender, M00_CUSTOM_SIEGE_ZONE_VERIFICATION, 0,
                                            0.0f);
            }
        }
        ...
        if ((my_actor_id) && (type == M00_CUSTOM_SIEGE_ACTOR_RESPONDING) && (sender)) {
            ...
            if (param == my_actor_id) {
                Vector3 my_loc;
                my_loc = Commands->Get_Position(obj);
                SCRIPT_DEBUG_MESSAGE(("M00_Siege_Zone_RAD has verified that this is its actor - "
                                      "setting position.\n"));
                Commands->Set_Innate_Soldier_Home_Location(sender, my_loc,
                                                           Get_Float_Parameter("Wander_Distance"));
            }
            else {
                SCRIPT_DEBUG_MESSAGE(
                    ("M00_Siege_Zone_RAD is registered to the wrong actor - clearing.\n"));
                my_actor_id = 0;
                Commands->Send_Custom_Event(obj, sender, M00_CUSTOM_SIEGE_ACTOR_UNREGISTER, 0,
                                            0.0f);
            }
        }
        ...
        if (type == M00_CUSTOM_SIEGE_OBJECT_GROUP_CHANGE) {
            SCRIPT_DEBUG_MESSAGE(
                ("M00_Siege_Zone_RAD is being ordered to change its group to %d.\n", param));
            my_group_id = param;
        }
    }
};

DECLARE_SCRIPT(M00_Siege_Actor_RAD,
               "Start_Now=1:int, Receive_Type:int, Receive_Param_On=1:int, "
               "Receive_Param_Off=0:int, My_Group_ID=0:int, Debug_Mode=0:int")
{
            ...
            if ((sound.Type == M00_SOUND_SIEGE_ZONE) && (!registered_zone)) {
                SCRIPT_DEBUG_MESSAGE(
                    ("M00_Siege_Actor_RAD has heard a zone shouting for an actor, sound ID = %d.\n",
                     sound.Type));
                Commands->Send_Custom_Event(obj, sound.Creator, M00_CUSTOM_SIEGE_ACTOR_GROUP_CHECK,
                                            my_group_id, 0.0f);
            }
        }
    }
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if (type == M00_CUSTOM_SIEGE_ZONE_CHECKING_ACTOR) {
            ...
            Commands->Send_Custom_Event(obj, sender, M00_CUSTOM_SIEGE_ACTOR_RESPONDING, my_id,
                                        0.0f);
        }
        ...
        if (type == M00_CUSTOM_SIEGE_OBJECT_GROUP_CHANGE) {
            SCRIPT_DEBUG_MESSAGE(
                ("M00_Siege_Actor_RAD is being told to change its group to %d.\n", param));
            my_group_id = param;
        }
        if (type == Get_Int_Parameter("Receive_Type")) {
            if (param == Get_Int_Parameter("Receive_Param_On")) {
    ...
    void Destroyed(GameObject * obj)
    {
        ...
        if (registered_zone) {
            ...
            if (zone_obj) {
                SCRIPT_DEBUG_MESSAGE(
                    ("M00_Siege_Actor_RAD found its siege zone, unregistering.\n"));
                Commands->Send_Custom_Event(obj, zone_obj, M00_CUSTOM_SIEGE_ACTOR_IS_DEAD, my_id,
                                            0.0f);
            }
        }
    }
