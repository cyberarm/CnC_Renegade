               ...
               "Terminal_ID:int, Send_Attempts=3:int, Send_Delay=1:int, Debug_Mode=0:int")
{
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        ...
        if (!timer_id) {
            ...
            if (current_send > send_attempts) {
                DebugPrint("ERROR - M00_Broadcaster_Registry_RAD - Object %d cannot find Terminal "
                           "%d to register with!\n",
                           item_id, terminal_id);
            }
            else {
                ...
                if (terminal_obj) {
                    SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Registry_RAD is sending custom type "
                                          "M00_CUSTOM_BROADCASTER_REGISTRATION, param %d.\n",
                                          item_id));
                    Commands->Send_Custom_Event(obj, terminal_obj,
                                                M00_CUSTOM_BROADCASTER_REGISTRATION, item_id);
                }
                else {
    ...
    virtual void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        if (type == M00_CUSTOM_BROADCASTER_REGISTRY_ERROR) {
            SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Registry_RAD received custom type "
                                  "M00_CUSTOM_BROADCASTER_REGISTRY_ERROR, param %d.\n",
                                  param));
            Commands->Start_Timer(obj, this, send_delay, 0);
        }
        else {
            SCRIPT_DEBUG_MESSAGE(("ERROR - M00_Broadcaster_Registry_RAD received custom type %d, "
                                  "param %d - unknown!\n",
                                  type, param));
        }
    }
    ...
    void Destroyed(GameObject * obj)
    {
        ...
        if (terminal_obj) {
            SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Registry_RAD is sending custom type "
                                  "M00_CUSTOM_BROADCASTER_REGISTRATION, param 0.\n"));
...
DECLARE_SCRIPT(M00_Broadcaster_Terminal_RAD,
               "Random_Percentage=100.0:float, Random_Param_Min=0:int, Random_Param_Max=0:int, "
               "Debug_Mode=0:int")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        for (object_count = 0; object_count < M00_BROADCASTER_TERMINAL_PROMPT_SIZE_RAD;
             object_count++) {
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if (ready_for_objects) {
            ...
            if (my_id == sender_id) {
                DebugPrint("ERROR - M00_Broadcaster_Terminal_RAD - Broadcaster sent a custom to "
                           "itself!\n");
            }
            else {
                ...
                case (M00_CUSTOM_BROADCASTER_REGISTRATION): {
                    ...
                    if (param) {
                        SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Terminal_RAD received custom type "
                                              "M00_CUSTOM_BROADCASTER_REGISTRATION, param %d.\n",
                                              param));
                        ...
                        for (object_count = 0; object_count < M00_BROADCASTER_TERMINAL_SIZE_RAD;
                             object_count++) {
                        ...
                        if (!found_object) {
                            ...
                            if (last_empty >= 0) {
                                ...
                                DebugPrint("ERROR - M00_Broadcaster_Terminal_RAD - Broadcaster "
                                           "Terminal %d is full!\n",
                                           obj_id);
                            }
                        }

                        // Now, insert the object into the random list regardless of number of
                        // entries.
                ...
                case (M00_CUSTOM_BROADCASTER_PROMPTER): {
                    SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Terminal_RAD received custom type "
                                          "M00_CUSTOM_BROADCASTER_PROMPTER, param %d.\n",
                                          param));

                    // Check if the object is already in the prompt list, or if there is a blank
                    // slot.
                ...
                default: {
                    SCRIPT_DEBUG_MESSAGE(
                        ("M00_Broadcaster_Terminal_RAD received custom type %d, param %d.\n", type,
                         param));
                    ...
                    switch (prompt_value) {
                    case (1): // Object is sending a custom that should be sent to random objects
                              // with one parameter.
                    {
                        SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Terminal_RAD is sending a custom to "
                                              "random objects with one parameter.\n"));
                        ...
                        for (object_count = 0; object_count < M00_BROADCASTER_TERMINAL_SIZE_RAD;
                            ...
                            if (random_value <= Get_Float_Parameter("Random_Percentage")) {
                                if (object_random_record[object_count]) {
                                    target_obj
                                        = Commands->Find_Object(object_random_record[object_count]);
                                    if (target_obj) {
                                        SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Terminal_RAD is "
                                                              "sending custom type %d, param %d.\n",
                                                              type, param));
                    ...
                    case (2): // Object is sending a custom that should be sent to everyone with
                              // random parameter.
                    {
                        SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Terminal_RAD is sending a custom to "
                                              "everyone with random parameter.\n"));
                        ...
                        for (object_count = 0; object_count < M00_BROADCASTER_TERMINAL_SIZE_RAD;
                            ...
                            if (object_specific_record[object_count]) {
                                target_obj
                                    = Commands->Find_Object(object_specific_record[object_count]);
                                if (target_obj) {
                                    random_value = int(Commands->Get_Random(float(parameter_low),
                                                                            float(parameter_high)));
                                    SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Terminal_RAD is sending "
                                                          "custom type %d, param %d.\n",
                                                          type, random_value));
                                    Commands->Send_Custom_Event(obj, target_obj, type,
                                                                random_value);
                                }
                                else {
                                    object_specific_record[object_count] = 0;
                    ...
                    case (3): // Object is sending a custom that should be sent to random objects
                              // with random parameter.
                    {
                        SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Terminal_RAD is sending a custom to "
                                              "random objects with random parameter.\n"));
                        parameter_low = Get_Int_Parameter("Random_Param_Min");
                        parameter_high = Get_Int_Parameter("Random_Param_Max");
                            ...
                            if (object_random_record[object_count]) {
                                ...
                                if (random_value <= Get_Float_Parameter("Random_Percentage")) {
                                    target_obj
                                        = Commands->Find_Object(object_random_record[object_count]);
                                    if (target_obj) {
                                        random_value2 = int(Commands->Get_Random(
                                            float(parameter_low), float(parameter_high)));
                                        SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Terminal_RAD is "
                                                              "sending custom type %d, param %d.\n",
                                                              type, random_value2));
                                        Commands->Send_Custom_Event(obj, target_obj, type,
                    ...
                    default: // Object is sending a custom that should be sent to everyone with one
                             // parameter.
                    {
                        SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Terminal_RAD is sending a custom to "
                                              "everyone with one parameter.\n"));
                        for (object_count = 0; object_count < M00_BROADCASTER_TERMINAL_SIZE_RAD;
                             object_count++) {
                            if (object_specific_record[object_count]) {
                                target_obj
            ...
            SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Terminal_RAD is sending custom type "
                                  "M00_CUSTOM_BROADCASTER_REGISTRY_ERROR, param 0.\n"));
...
DECLARE_SCRIPT(M00_Broadcaster_Activator_RAD,
               "Terminal_ID:int, Prompt_Value=0:int, Debug_Mode=0:int")
{
    ...
    {
        SCRIPT_DEBUG_MESSAGE(
            ("M00_Broadcaster_Activator_RAD received custom type %d, param %d.\n", type, param));

        GameObject* terminal_obj;

        terminal_obj = Commands->Find_Object(terminal_id);
        if (terminal_obj) {
            SCRIPT_DEBUG_MESSAGE(("M00_Broadcaster_Activator_RAD is sending custom type "
                                  "M00_CUSTOM_BROADCASTER_PROMPTER, param %d.\n",
                                  prompt_value));
            SCRIPT_DEBUG_MESSAGE(
                ("M00_Broadcaster_Activator_RAD is sending custom type %d, param %d.\n", type,
                 param));

            Commands->Send_Custom_Event(obj, terminal_obj, M00_CUSTOM_BROADCASTER_PROMPTER,
                                        prompt_value);
