...
DECLARE_SCRIPT(M02_Objective_Zone, "")
{
    ...
    void Entered(GameObject * obj, GameObject * enterer)
    {
        ...
        if (object) {
            ...
            case (401982): {
                ...
                GameObject* lear_jet
                    = Commands->Create_Object("Nod_Jet", Vector3(929.91f, 1132.34f, 70.5f));
                if (lear_jet) {
            ...
            case (405120): {
                ...
                GameObject* chinook_obj1
                    = Commands->Create_Object("GDI_Mammoth_Tank_Player", droploc);
                if (chinook_obj1) {
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        if (timer_id == 3) {
            ...
            GameObject* jet
                = Commands->Create_Object("Nod_Jet", Vector3(1209.879f, 528.647f, 21.313f));
            if (jet) {
            ...
            if (object) {
                ...
                GameObject* mendoza = Commands->Create_Object(
                    "Nod_FlameThrower_3Boss", Vector3(1260.007f, 535.460f, 18.428f));
                if (mendoza) {
    ...
    void Create_Apache(int area_id)
    {
        ...
        if (helipad) {
            // This will stop working when the Power Plant goes down, but it won't matter by that
            // time.
...
DECLARE_SCRIPT(M02_Respawn_Controller, "")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        GameObject* helicopter
            = Commands->Create_Object("GDI_Transport_Helicopter", Vector3(-26.87f, 37.42f, 20.75f));
        if (helicopter) {
...
DECLARE_SCRIPT(M02_Reset_Spawn, "Area_ID:int,Spawn_Type:int") { void Destroyed(GameObject * obj) {
    GameObject* object = Commands->Find_Object(M02_OBJCONTROLLER);
...
DECLARE_SCRIPT(M02_Nod_Soldier, "Area_Number:int,Area_Officer:int,Pre_Placed:int")
{
                ...
                if (officer) {
                    ...
                    if (preplaced > 0) {
                        ...
                        if (preplaced == 4) {
                            ...
                            if ((param == 0) || (param == 3) || (param == 6) || (param == 10)
                                || (param == 16) || (param == 21)) {
                                no_return_home = true;
                        ...
                        // Unit is not pre-placed, activate cleanup code and do not hibernate if not
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if ((type == 0) && (param == 0)) {
            ...
            // If the unit is a preplaced minigunner, forced movement code is activated for this
            // unit.
...
DECLARE_SCRIPT(M02_Helipad, "") { void Killed(GameObject * obj, GameObject* killer) {
    GameObject* object = Commands->Find_Object(M02_OBJCONTROLLER);
...
DECLARE_SCRIPT(M02_Destroy_Objective, "Objective_ID:int") {
    void Killed(GameObject * obj, GameObject* killer) { int objective_id;
...
if (object) {
    ...
    if (objective_id == 212) {
        ...
        GameObject* chinook_obj1
            = Commands->Create_Object("Invisible_Object", Vector3(580.37f, 889.69f, -0.57f));
...
DECLARE_SCRIPT(M02_Nod_Convoy_Truck, "") { void Killed(GameObject * obj, GameObject* killer) {
    GameObject* object = Commands->Find_Object(M02_OBJCONTROLLER);
...
DECLARE_SCRIPT(M02_Stationary_Vehicle,
               "Area_ID:int") { void Created(GameObject * obj) { int area_id;
area_id = Get_Int_Parameter("Area_ID");
...
DECLARE_SCRIPT(M02_Nod_Apache, "Area_ID:int")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        // First waypath means area for looping Apaches, it is set when timer expires, and then uses
        // real IDs.
...
DECLARE_SCRIPT(M02_Destroy_Vehicle,
               "") { void Created(GameObject * obj) { Commands->Enable_Engine(obj, true);
Commands->Disable_Physical_Collisions(obj);
