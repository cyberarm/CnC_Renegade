...
DECLARE_SCRIPT(RMV_Engineer_Wander,
    ...
    void Sound_Heard(GameObject * obj, const CombatSound& sound)
    {
        ...
        if ((sound.Type == M00_SOUND_ENGINEER_WANDER) && (!busy)) {
            ...
            Commands->Send_Custom_Event(obj, sound.Creator, c_type, c_param_1);
                 ...
                 == M00_SOUND_BUILDING_DESTROYED + Get_Int_Parameter("Building_Number")) {
            Evacuate();
        }
...
/*DECLARE_SCRIPT(RMV_Engineer_Wander_Terminal, "Animation_Name:string, Custom_Type:int,
Custom_Param_1:int, Custom_Param_2:int")
{
        ...
        void Timer_Expired(GameObject * obj, int timer_id)
        {
                ...
                {
                        Commands->Create_Logical_Sound(obj, M00_SOUND_ENGINEER_WANDER,
mypos, 60.0f); Commands->Start_Timer(obj, this, 2.0f, ENGINEER_WANDER_TIMER);
                }
        }
...
DECLARE_SCRIPT(RMV_Building_Engineer_Controller,
               "Killed_Broadcast_Radius:float, 25_Start_ID:int, 25_Number:int, 50_Start_ID:int, "
               "50_Number:int, 75_Start_ID:int, 75_Number:int, Building_Number:int")
{
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        if (type == CUSTOM_EVENT_BUILDING_DAMAGED) {
            ...
            if (!sent_25 && percentage >= 0.25) {
                Commands->Debug_Message(
                    "Building took 25 percent or more damage, notifying engineers.\n");

                sent_25 = true;
                for (int x = 0; x < Get_Int_Parameter("25_Number"); x++) {
                    GameObject* target
                        = Commands->Find_Object(Get_Int_Parameter("25_Start_ID") + x);
                    if (target) {
            ...
            if (!sent_50 && percentage >= 0.50) {
                Commands->Debug_Message(
                    "Building took 50 percent or more damage, notifying engineers.\n");
                ...
                for (int x = 0; x < Get_Int_Parameter("50_Number"); x++) {
                    GameObject* target
                        = Commands->Find_Object(Get_Int_Parameter("50_Start_ID") + x);
                    if (target) {
                        Commands->Send_Custom_Event(obj, target, 1000, 1000);
                    }
                }
            ...
            if (!sent_75 && percentage >= 0.75) {
                Commands->Debug_Message(
                    "Building took 75 percent or more damages, notifying engineers.\n");

                sent_75 = true;
                for (int x = 0; x < Get_Int_Parameter("75_Number"); x++) {
                    GameObject* target
                        = Commands->Find_Object(Get_Int_Parameter("75_Start_ID") + x);
                    if (target) {
                        Commands->Send_Custom_Event(obj, target, 1000, 1000);
                    }
                }
        ...
        if (type == CUSTOM_EVENT_BUILDING_REPAIRED) {
            ...
            if (percentage <= 0.25) {
                Commands->Debug_Message(
                    "Building repaired to 75 percent or more health, notifying engineers.\n");
            ...
            if (percentage <= 0.50) {
                Commands->Debug_Message(
                    "Building repaired to 50 percent or more health, notifying engineers.\n");
            ...
            if (percentage <= 0.75) {
                Commands->Debug_Message(
                    "Building repaired to 25 percent or more health, notifying engineers.\n");
    ...
    void Killed(GameObject * obj, GameObject * killer)
    {
        ...
        Commands->Create_Logical_Sound(
            obj, M00_SOUND_BUILDING_DESTROYED + Get_Int_Parameter("Building_Number"), mypos,
            Get_Float_Parameter("Killed_Broadcast_Radius"));
    }
};

DECLARE_SCRIPT(RMV_Toggled_Engineer_Target,
               "Emergency=1:int, Animation_Name:string, Custom_Type:int, Custom_Param_1:int, "
               "Custom_Param_2:int")
{
    enum
    {
        ENGINEER_WANDER_TIMER
    };
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        if ((timer_id == ENGINEER_WANDER_TIMER) && (!i_am_occupied) && active) {
            Commands->Create_Logical_Sound(obj,
                                           (Get_Int_Parameter("Emergency") == 0)
                                               ? M00_SOUND_ENGINEER_WANDER
                                               : M00_SOUND_ENGINEER_WANDER_EMERGENCY,
                                           mypos, 60.0f);
            Commands->Start_Timer(obj, this, 2.0f, ENGINEER_WANDER_TIMER);
        }
    }
...
DECLARE_SCRIPT(RMV_MCT_Switcher,
               "") { void Killed(GameObject * obj, GameObject* killer) { GameObject * temp;
...
DECLARE_SCRIPT(M00_Play_Sound,
               "Sound_Preset:string, Is_3D=1:int, Offset:vector3, Offset_Randomness:vector3, "
               "Frequency_Min=-1:float, Frequency_Max:float") { void Created(GameObject * obj) {
    if (Get_Int_Parameter("Frequency_Min") == -1) { Timer_Expired(obj, 0);
}
else
{
