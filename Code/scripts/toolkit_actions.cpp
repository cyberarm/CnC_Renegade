    ...
    "_Attack_Deviation=0.0:float, _Attack_Primary=1:int, _Attack_Crouched=0:int, Debug_Mode=0:int")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        params.Set_Basic(this, Get_Int_Parameter("Action_Priority"),
                         Get_Int_Parameter("Action_ID"));
        params.Set_Movement(Get_Vector3_Parameter("_Move_Destination"),
                            Get_Float_Parameter("_Move_Speed"),
                            Get_Float_Parameter("_Move_Arrive_Distance"));
        params.Set_Attack(Get_Vector3_Parameter("_Attack_Location"),
                          Get_Float_Parameter("_Attack_Range"),
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if (type == Get_Int_Parameter("Receive_Type")) {
            if (param == Get_Int_Parameter("Receive_Param_On")) {
                ...
                params.WaypointStartID = Get_Int_Parameter("_Move_Waypath_Start_ID");
                params.WaypointEndID = Get_Int_Parameter("_Move_Waypath_End_ID");
                params.WaypathSplined
                    = (Get_Int_Parameter("_Move_Waypath_Splined") == 1) ? true : false;
            ...
            if (action_id == Get_Int_Parameter("Action_ID")) {
                ...
                case (ACTION_COMPLETE_PATH_BAD_DEST): {
                    SCRIPT_DEBUG_MESSAGE(
                        ("ERROR - M00_Action returned a Path Bad Destination Error!\n"));
                    break;
                }
                case (ACTION_COMPLETE_MOVE_NO_PROGRESS_MADE): {
                    SCRIPT_DEBUG_MESSAGE(
                        ("ERROR - M00_Action returned a No Progress Made Error!\n"));
                    break;
                }
                case (ACTION_COMPLETE_ATTACK_OUT_OF_RANGE): {
                    SCRIPT_DEBUG_MESSAGE(
                        ("ERROR - M00_Action returned an Attack Out Of Range Error!\n"));
                    break;
                }
...
DECLARE_SCRIPT(M00_Action_Set_Home_Location,
