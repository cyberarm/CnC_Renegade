        ...
        if (!Disabled && killer && Commands->Is_A_Star(killer)
            ...
            if (star_health_percent < 0.25f) {
                Commands->Debug_Message(
                    "Soldier_Powerup_Grant: Star's Health < 25%%.  Dropping Health\n");
            ...
            if (powerup == nullptr && star_shield_percent < 0.25f) {
                Commands->Debug_Message(
                    "Soldier_Powerup_Grant: Star's Shield < 25%%.  Dropping Sheild\n");
            ...
            if (powerup == nullptr && star_health_percent > 0.75f) {
                ...
                if (weapon_powerup != nullptr && weapon_powerup[0] != 0) {
                    Commands->Debug_Message("Soldier_Powerup_Grant: Star's Health > 75%%.  "
                                            "Dropping soldier's weapon %s\n",
                                            weapon_powerup);
...
DECLARE_SCRIPT(M00_Powerup_Destroy, "") {
    ...
    void Created(GameObject * obj) { float lifespan
                                     = (30.0f - (Commands->Get_Difficulty_Level() * 10));
...
// DAY - Reveals a Building entry in the Encyclopedia.  Param is the Encyclopedia ID listed in the
// Buildings.ini file.
DECLARE_SCRIPT(M00_Reveal_Enc_Building_DAY, "BuildingEncyclopediaID:int") {
    void Custom(GameObject * obj, int type, int param,
        ...
        bool reveal
        = Commands->Reveal_Encyclopedia_Building(Get_Int_Parameter("BuildingEncyclopediaID"));
if (reveal == 1) {
...
// DAY - Reveals a Character entry in the Encyclopedia.  Param is the Encyclopedia ID listed in the
// Characters.ini file.
DECLARE_SCRIPT(M00_Reveal_Enc_Character_DAY, "CharacterEncyclopediaID:int") {
    void Custom(GameObject * obj, int type, int param,
                GameObject* sender) { if (type == CUSTOM_EVENT_POWERUP_GRANTED) {
        bool reveal
        = Commands->Reveal_Encyclopedia_Character(Get_Int_Parameter("CharacterEncyclopediaID"));
if (reveal == 1) {
    Commands->Display_Encyclopedia_Event_UI();
}
}
...
// DAY - Reveals a Vehicle entry in the Encyclopedia.  Param is the Encyclopedia ID listed in the
// Vehicles.ini file.
DECLARE_SCRIPT(M00_Reveal_Enc_Vehicle_DAY, "VehicleEncyclopediaID:int") {
    void Custom(GameObject * obj, int type, int param,
                GameObject* sender) { if (type == CUSTOM_EVENT_POWERUP_GRANTED) {
        bool reveal
        = Commands->Reveal_Encyclopedia_Vehicle(Get_Int_Parameter("VehicleEncyclopediaID"));
if (reveal == 1) {
    Commands->Display_Encyclopedia_Event_UI();
}
}
...
// DAY - Reveals a Weapon entry in the Encyclopedia.  Param is the Encyclopedia ID listed in the
// Weapons.ini file.
...
DECLARE_SCRIPT(M00_Reveal_Enc_Weapon_DAY, "WeaponEncyclopediaID:int") {
    void Custom(GameObject * obj, int type, int param,
                GameObject* sender) { if (type == CUSTOM_EVENT_POWERUP_GRANTED) {
        bool reveal
        = Commands->Reveal_Encyclopedia_Weapon(Get_Int_Parameter("WeaponEncyclopediaID"));
if (reveal == 1) {
    Commands->Display_Encyclopedia_Event_UI();
}
}
...
DECLARE_SCRIPT(M00_Tiberium_Refinery,
               "MoneyAmount:int,TimerLength:int") { void Created(GameObject * obj) {
...
DECLARE_SCRIPT(M00_CNC_Crate,
               "") { void Custom(GameObject * obj, int type, int param, GameObject* sender) {
    if ((type == CUSTOM_EVENT_POWERUP_GRANTED)
        && (sender)) { Commands->Give_Money(sender, 100.0f, false);
}
}
}
;
...
DECLARE_SCRIPT(M00_Death_Powerup,
               "") { void Custom(GameObject * obj, int type, int param, GameObject* sender) {
    if ((type == CUSTOM_EVENT_POWERUP_GRANTED)
        && (sender)) { Commands->Apply_Damage(sender, 10000.0f, "Death", nullptr);
}
}
}
;