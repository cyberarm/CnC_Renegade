...
DECLARE_SCRIPT(DLS_Test_Pickup, "Debug_Mode=0:int")
{
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        ...
        if (timer_id == TROOP_PICKUP1 && (!already_entered)) {
            ...
            GameObject* controller
                = Commands->Create_Object("Invisible_Object", Vector3(1.548f, -3.497f, 3.595f));
        ...
        if (timer_id == TROOP_PICKUP2 && (!already_entered)) {
            GameObject* controller
                = Commands->Create_Object("Invisible_Object", Vector3(-58.759f, 29.089f, -6.875f));
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if (type == M06_STOP_PICKUP) {
            ...
            Commands->Send_Custom_Event(obj, Commands->Find_Object(100018), M06_STOP_PICKUP, 1,
                                        0.0f);
            Commands->Send_Custom_Event(obj, Commands->Find_Object(100018), M06_STOP_PICKUP, 1,
                                        0.0f);
        }
...
DECLARE_SCRIPT(DLS_Cargo_Plane_Test,
               "") { // Register variables to be Auto-Saved
                     ...
                     void Damaged(GameObject * obj, GameObject* damager, float amount) {
                         ...
                         = Commands->Create_Object("Invisible_Object", Commands->Get_Position(obj));
...
DECLARE_SCRIPT(DLS_Test_Conversation, "Debug_Mode=0:int")
{
    ...
    void Poked(GameObject * obj, GameObject * poker)
    {
        ...
        int conv_id = Commands->Create_Conversation(conv_name);
        Commands->Join_Conversation(Commands->Get_A_Star(Commands->Get_Position(obj)), conv_id,
                                    false, true);
...
DECLARE_SCRIPT(DLS_Drop_Unit, "Box_ID:int") {
    ...
    void Damaged(GameObject * obj, GameObject* damager,
                 float amount) { int box_id = Get_Int_Parameter("Box_ID");
switch (box_id) {
case 1: {
...
DECLARE_SCRIPT(DLS_Test_Homepoint, "") { void Created(GameObject * obj) {
...
DECLARE_SCRIPT(DLS_Playertype, "") {
    ...
    void Poked(GameObject * obj,
               GameObject* poker) { Commands->Set_Player_Type(obj, SCRIPT_PLAYERTYPE_GDI);
...
DECLARE_SCRIPT(DLS_Cinematic_Test, "") {
    ...
    void Damaged(GameObject * obj, GameObject* damager, float amount) {
        /*	GameObject * chinook_obj0 = Commands->Create_Object ( "Invisible_Object",
           Vector3(0.0f, 0.0f, 0.0f)); Commands->Set_Facing(chinook_obj0, 0.0f);
           ...
           Vector3(0.0f, 0.0f, 0.0f)); Commands->Set_Facing(chinook_obj1, 0.0f);
...
DECLARE_SCRIPT(DLS_SSM_Test, "")
{
    ...
    void Created(GameObject * obj)
    {
        GameObject* ssm_missile
            = Commands->Create_Object_At_Bone(obj, "Invisible_Object", "V_LAUNCHER");
    ...
    void Damaged(GameObject * obj, GameObject * damager)
    {
        ...
        Commands->Set_Animation(Commands->Find_Object(ssm_missile_id),
                                "v_nod_ssm_Missl.v_nod_ssm_Missl", 0);
    }
};
...
DECLARE_SCRIPT(DLS_Math,
               "") { void Created(GameObject * obj) { Vector3 pos = Commands->Get_Position(obj);
float facing = Commands->Get_Facing(obj);
...
DECLARE_SCRIPT(DLS_Test_Evac, "") // Deadeye2
{
    ...
    void Action_Complete(GameObject * obj, int action_id, ActionCompleteReason reason)
    {
        if (action_id == WAYPATH) {
            ...
            Commands->Send_Custom_Event(obj, Commands->Find_Object(100001), M07_DEAD6_EVAC,
                                        dead6_id, 0.0f);
        }
    }
};
...
DECLARE_SCRIPT(MX0_Area4_Controller_DLS, "")
{
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if (type == MX0_STAR_AREA) {
            ...
            if (star_area == 1 && !area4_activated) {
                ...
                //"Invisible_Object", Commands->Get_Position(Commands->Find_Object(1500051)));
                //		Commands->Set_Facing(chinook_obj2, 0.0f);
                //		Commands->Attach_Script(chinook_obj2, "Test_Cinematic",
                //"MX0_GDI_TroopDrop_Area4.txt");
                // Humvee
                ...
                Commands->Attach_Script(medium_tank_blasted, "MX0_Vehicle_DLS",
                                        "1500022, 1500023, 0, 0, 1.0f");
                Commands->Attach_Script(medium_tank_blasted, "MX0_GDI_Killed_DLS", "2");
                Commands->Attach_Script(medium_tank_blasted, "M00_Damage_Modifier_DME",
                                        ".10f,1,1,1,1");

                // Medium Tank Escort
                GameObject* medium_tank_escort = Commands->Find_Object(medium_tank_escort_id);
                Commands->Attach_Script(medium_tank_escort, "MX0_Vehicle_DLS",
                                        "1500027, 1500028, 1500029, 0, 1.0f");

                // GDI Trooper 1
                GameObject* gdi_trooper1 = Commands->Find_Object(gdi_trooper1_id);
                Commands->Attach_Script(gdi_trooper1, "MX0_GDI_Soldier_DLS",
                                        "1500049, 1500050, 0, 0, 0.8f");
                // GDI RocketTrooper
                GameObject* gdi_trooper2 = Commands->Create_Object(
                    "GDI_RocketSoldier_0", Vector3(51.644f, 27.306f, 4.850f));
                Commands->Attach_Script(gdi_trooper2, "M00_Send_Object_ID", "1500017, 12, 0.0f");
                Commands->Attach_Script(gdi_trooper2, "MX0_GDI_Soldier_DLS",
                                        "1500053, 1500070, 0, 0, 0.8f");
                // Humvee announces discovery of Nod Base
                Commands->Send_Custom_Event(obj, Commands->Find_Object(humvee_id),
                                            MX0_SPECIFIC_ACTION, MX0_DISCOVERS_NOD_BASE, 0.0f);
            }
        }
        // Actor IDs
        ...
        if (type == MX0_FIRE_SAM) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(1500015), MX0_FIRE_SAM,
                                        obelisk_orca_id, 0.0f);
            Commands->Send_Custom_Event(obj, Commands->Find_Object(1500016), MX0_FIRE_SAM,
                                        obelisk_orca_id, 0.0f);
        }
        ...
        if (type == MX0_GDI_REINFORCEMENT_KILLED) {
            ...
            if (mx0_gdi_reinforcement_killed == 2) {
                ...
                // Additional GDI troop drop
                GameObject* chinook_obj2 = Commands->Create_Object(
                    "Invisible_Object", Commands->Get_Position(Commands->Find_Object(1500102)));
                Commands->Set_Facing(chinook_obj2,
                                     Commands->Get_Facing(Commands->Find_Object(1500102)));
                Commands->Attach_Script(chinook_obj2, "Test_Cinematic",
                                        "MX0_GDI_Reinforce_Area4.txt");
            }
        }
    }
    ...
    void Relocate_Soldiers(GameObject * obj)
    {
        ...
        Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper2_id), MX0_SOLDIER_MOVE,
                                    1, 0.0f);
        Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper3_id), MX0_SOLDIER_MOVE,
                                    1, 0.0f);
        // GDI Reinforcements
        int rand = Commands->Get_Random_Int(0, 3);
        Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_reinforcement1_id),
                                    MX0_SOLDIER_MOVE, rand, 0.0f);
        Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_reinforcement2_id),
                                    MX0_SOLDIER_MOVE, rand, 0.0f);
    }
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        ...
        if (timer_id == AREA4_ACTIVATED) {
            if (star_area > 0) // Area 2 or Area 3
            {
                ...
                // Humvee
                Commands->Send_Custom_Event(obj, Commands->Find_Object(humvee_id), MX0_VEHICLE_MOVE,
                                            1, 0.0f);
                // Medium Tank Blasted
                Commands->Send_Custom_Event(obj, Commands->Find_Object(medium_tank_blasted_id),
                                            MX0_VEHICLE_MOVE, 1, 0.0f);
                // Medium Tank Escort
                Commands->Send_Custom_Event(obj, Commands->Find_Object(medium_tank_escort_id),
                                            MX0_VEHICLE_MOVE, 1, 0.0f);
                // GDI Trooper 1
                Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper1_id),
                                            MX0_SOLDIER_MOVE, 1, 0.0f);
                // Relocate GDI Trooper2 & GDI Trooper3
                Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper2_id),
                                            MX0_SOLDIER_MOVE, 1, 0.0f);
                Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper3_id),
                                            MX0_SOLDIER_MOVE, 1, 0.0f);

                // Light Tank A
                GameObject* light_tank_a = Commands->Find_Object(light_tank_a_id);
                Commands->Attach_Script(light_tank_a, "MX0_Vehicle_DLS",
                                        "1500033, 1500034, 0, 0, 0.4f");
                ...
                Commands->Attach_Script(mobile_artillery, "MX0_Vehicle_DLS",
                                        "1500084, 1500085, 0, 0, 1.0f");
                ...
                Commands->Send_Custom_Event(obj, Commands->Find_Object(humvee_id),
                                            M00_ENABLE_DAMAGE_MOD, 0, 0.0f);

                // RocketTrooper sees Obelisk
                Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper1_id),
                                            MX0_SPECIFIC_ACTION, MX0_ROCKETTROOPER_SEES_OBELISK,
                                            4.0f);
            }
            else {
                if (!say_need_here) {
                    ...
                    Commands->Send_Custom_Event(obj, Commands->Find_Object(humvee_id),
                                                MX0_SPECIFIC_ACTION, MX0_NEED_YOU_HERE, 0.0f);
                }
        ...
        if (timer_id == HUMMVEE_DESTRUCTION) {
            if (star_area > 1) // Area 2 or Area 3
            {
                ...
                // Light Tank A
                Commands->Send_Custom_Event(obj, Commands->Find_Object(light_tank_a_id),
                                            MX0_VEHICLE_MOVE, 1, 0.0f);

                // Obelisk blasts Hummvee
                Commands->Send_Custom_Event(obj, Commands->Find_Object(1500020), MX0_OBELISK_HUMVEE,
                                            1, 0.0f);

                // *** Adjusted 12/12/01 Medium Tank no longer destroyed, run through function
                // anyways Obelisk blasts GDI Medium Tank
                Commands->Start_Timer(obj, this, 0.0f, MEDIUM_TANK_DESTRUCTION);

                // RocketTrooper - The Obelisk is hot!  Look out!
                Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper1_id),
                                            MX0_SPECIFIC_ACTION, MX0_ROCKETTROOPER_HOT_OBELISK,
                                            4.0f);
                ...
                Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper2_id),
                                            MX0_SPECIFIC_ACTION, MX0_ROCKETTROOPER_CANT_TURN_BACK,
                                            0.0f);
                // "We need you Commando"
        ...
        if (timer_id == MEDIUM_TANK_DESTRUCTION) {
            ...
            case 3: {
                ...
                Commands->Send_Custom_Event(obj, Commands->Find_Object(1500039),
                                            MX0_SPECIFIC_ACTION, 1, 0.0f);

                // Obelisk blasts GDI Medium Tank  **** Ajdusted 12/12/01 No longer destroy tank
                //		Commands->Send_Custom_Event( obj, Commands->Find_Object(1500020),
                //MX0_OBELISK_MEDIUM_TANK, 2, 0.0f);

                // Relocate Medium Tank Escort
                Commands->Send_Custom_Event(obj, Commands->Find_Object(medium_tank_escort_id),
                                            MX0_VEHICLE_MOVE, 2, 0.0f);
                // Mobile Artillery
                Commands->Send_Custom_Event(obj, Commands->Find_Object(mobile_artillery_id),
                                            MX0_VEHICLE_MOVE, 1, 0.0f);
            ...
            case 3: {
                ...
                if (Commands->Find_Object(1500015) || Commands->Find_Object(1500016)) {
                    ...
                    if (!say_take_out_sams) {
                        ...
                        Commands->Send_Custom_Event(obj, Commands->Find_Object(1500015),
                                                    M00_ENABLE_DAMAGE_MOD, 0, 0.0f);
                        Commands->Send_Custom_Event(obj, Commands->Find_Object(1500016),
                                                    M00_ENABLE_DAMAGE_MOD, 0, 0.0f);
                ...
                // If SAMs no longer exist, trigger A10 strike
                    ...
                    Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper2_id),
                                                MX0_SPECIFIC_ACTION, MX0_ROCKETTROOPER_SAMS_HISTORY,
                                                0.0f);
                    // Trooper1 - There, on the wall!
                    Commands->Send_Custom_Event(obj, Commands->Find_Object(gdi_trooper1_id),
                                                MX0_SPECIFIC_ACTION, MX0_TROOPER_UP_ON_WALL, 2.0f);
                    // Nod RocketSoldier on base wall fire at Havoc's tank
                    Commands->Send_Custom_Event(obj,
                                                Commands->Find_Object(nod_base_rocketsoldier_a_id),
                                                MX0_SPECIFIC_ACTION, player_tank_id, 0.0f);
                    Commands->Send_Custom_Event(obj,
                                                Commands->Find_Object(nod_base_rocketsoldier_b_id),
                                                MX0_SPECIFIC_ACTION, player_tank_id, 0.0f);
        ...
        if (timer_id == ION_CANNON_STRIKE) {
            ...
            GameObject* ion_cannon_strike = Commands->Create_Object(
                "Nod_RocketSoldier_1Off", Commands->Get_Position(Commands->Find_Object(1500087)));
...
DECLARE_SCRIPT(MX0_Area4_Zone_DLS, "Area=0:int")
{
    ...
    void Entered(GameObject * obj, GameObject * enterer)
    {
        if (Commands->Is_A_Star(enterer) && first_time) {
            ...
            Commands->Send_Custom_Event(obj, Commands->Find_Object(1500017), MX0_STAR_AREA, area,
                                        0.0f);
    ...
    "Attack_Loc0=0:int, Attack_Loc1=0:int, Attack_Loc2=0:int, Attack_Loc3=0:int, Speed=1.0:float")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        ActionParamsStruct params;
        params.Set_Basic(this, INNATE_PRIORITY_ENEMY_SEEN + 5, 10);
        params.Set_Movement(Commands->Get_Position(Commands->Find_Object(attack_loc[loc])), speed,
                            5.0f);
...
DECLARE_SCRIPT(MX0_GDI_Killed_DLS, "Unit_ID=0:int") {
    ...
    void Damaged(GameObject * obj, GameObject* damager, float amount) {
        ...
        if ((damager == Commands->Find_Object(1500020))
            && (Commands->Find_Object(1500020))) { Commands->Apply_Damage(obj, 50000.0f, "STEEL");
}
}
...
void Killed(GameObject* obj, GameObject* killer)
{
    ...
    switch (unit_id) {
    case 1: {
        GameObject* humvee_driver
            = Commands->Create_Object("GDI_Minigunner_1Off", Commands->Get_Position(obj));
        ...
        Commands->Send_Custom_Event(obj, humvee_driver, MX0_SPECIFIC_ACTION,
                                    MX0_HUMVEE_DRIVER_COWER, 0.0f);
        // Obelisk blasts Hummvee
        Commands->Send_Custom_Event(obj, Commands->Find_Object(1500020), MX0_OBELISK_DRIVER, 1,
                                    1.0f);
    } break;
    ...
    case 3: {
        Commands->Send_Custom_Event(obj, Commands->Find_Object(1500017),
                                    MX0_GDI_REINFORCEMENT_KILLED, 1, 1.0f);
    } break;
    }
}
}
    ...
    "Attack_Loc0=0:int, Attack_Loc1=0:int, Attack_Loc2=0:int, Attack_Loc3=0:int, Speed=1.0:float")
{
    ...
    void Action_Complete(GameObject * obj, int action_id, ActionCompleteReason reason)
    {
        ...
        if ((action_id == MX0_SOLDIER_MOVE) && (reason != ACTION_COMPLETE_NORMAL)
            && (mx0_soldier_move_attempts < 4)) {
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        ...
        if (timer_id == STRAFE) {
            if (left) {
                left = false;
                params.Set_Basic(this, INNATE_PRIORITY_ENEMY_SEEN + 5, 10);
                params.Set_Attack(Commands->Find_Object(Get_Int_Parameter("Left_Point")), 0.0f,
                                  0.0f, 1);
                params.AttackCheckBlocked = false;
                params.AttackForceFire = true;
                Commands->Action_Attack(obj, params);
                ...
                left = true;
                params.Set_Basic(this, INNATE_PRIORITY_ENEMY_SEEN + 5, 10);
                params.Set_Attack(Commands->Find_Object(Get_Int_Parameter("Right_Point")), 0.0f,
                                  0.0f, 1);
                params.AttackCheckBlocked = false;
                params.AttackForceFire = true;
                Commands->Action_Attack(obj, params);
...
DECLARE_SCRIPT(MX0_Nod_RocketSoldier_DLS, "Stationary_Point=0:int")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        params.Set_Movement(Commands->Find_Object(Get_Int_Parameter("Stationary_Point")), RUN,
                            0.0f);
        Commands->Action_Goto(obj, params);
    }
...
DECLARE_SCRIPT(MX0_Explosive_Barrels_DLS, "Logical_Sound=0:int, Radius:float") {
    ...
    void Sound_Heard(GameObject * obj, const CombatSound& sound) {
        if (sound.Type
            == Get_Int_Parameter("Logical_Sound")) { Commands->Apply_Damage(obj, 50000.0f, "STEEL");
}
}
...
void Killed(GameObject* obj, GameObject* killer)
{
    ...
    Commands->Create_Logical_Sound(obj, Get_Int_Parameter("Logical_Sound"),
                                   Commands->Get_Position(obj), radius);
}
}
;
