...
// OV_VERIFIED		= Sent by unit to control to verify that control is currently handling the
// unit. OV_ARRIVED		= Sent by unit to control when the unit arrives at a destination.
// OV_ANIMCOMPLETE	= Sent by unit to control when an animation has been completed.
// OV_OVERRIDE		= Sent by control to unit when control wants to override combat code on the
// unit. OV_REQUEST		= Sent by control to unit when control wants to verify that it is
// currently handling the unit.
