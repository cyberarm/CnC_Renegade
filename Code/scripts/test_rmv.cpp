...
DECLARE_SCRIPT(RMV_Test_Script, "") { void Sound_Heard(GameObject * obj, const CombatSound& sound) {
...
DECLARE_SCRIPT(RMV_Hostage_Rescue_Point, "") { void Entered(GameObject * obj, GameObject* enterer) {
    if (!Commands->Is_A_Star(enterer)) { Commands->Send_Custom_Event(obj, enterer, 999, 999);
}
}
}
;

DECLARE_SCRIPT(RMV_Trigger_Killed,
               "ID:int, Type:int, Param:int") { void Killed(GameObject * obj, GameObject* killer) {
...
DECLARE_SCRIPT(RMV_Test_Facing,
               "") { void Damaged(GameObject * obj, GameObject* damager,
                                  float amount) { Vector3 my_pos = Commands->Get_Position(obj);
...
DECLARE_SCRIPT(M00_Destroyed_Turret, "") { void Killed(GameObject * obj, GameObject* killer) {
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        ...
        if (timer_id == 0) {
            ...
            if (loading_complete) {
                ...
                char trans_anim[40];
                sprintf(trans_anim, "v_%s_trnspt.XG_EV%d_trnsz", (nod == 1) ? "NOD" : "GDI",
                        number);
                Commands->Set_Animation(traj, traj_anim, false);
                Commands->Set_Animation(transport, trans_anim, false);
                ...
                sprintf(traj_anim, "XG_TransprtBone.XG_EV%d_Pathloop", number);
                char trans_anim[40];
                sprintf(trans_anim, "v_%s_trnspt.XG_EV%d_trnsloop", (nod == 1) ? "NOD" : "GDI",
                        number);
                Commands->Set_Animation(traj, traj_anim, false);
                Commands->Set_Animation(transport, trans_anim, false);
                Commands->Start_Timer(obj, this, 94.0f / 30.0f, 1);
...
DECLARE_SCRIPT(Poke_And_Play_Cinematic, "Text_File:string, Location=0 0 0:vector3") {
    void Poked(GameObject * obj,
               GameObject* poker) { Vector3 pos = Get_Vector3_Parameter("Location");
...
DECLARE_SCRIPT(M00_Ion_Cannon_Sound,
               "Number=0:int") { void Created(GameObject * obj) { int num = Get_Int_Parameter(0);
Vector3 pos = Commands->Get_Position(obj);
