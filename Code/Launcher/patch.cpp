...
void Apply_Patch(char* patchfile, ConfigFile& config, int skuIndex, bool show_dialog)
{
    ...
    if (strcasecmp(patchfile + strlen(patchfile) - strlen(".exe"), ".exe") == 0) {
        ...
        regRetval = RegCreateKeyEx(
            HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\RunOnce", 0, "",
            REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, nullptr, &regKey, &regPrevious);
    ...
    //
        ...
        DWORD patchID = 0;
        HANDLE patchThread
            = CreateThread(nullptr, 0, ApplyPatchThread, (LPVOID)patchfile, 0, &patchID);

        int dotcnt = 9;
        DWORD last_time = GetTickCount();
        BOOL forward = FALSE;
        char worktext[256];
        LoadString(nullptr, IDS_WORKING_TEXT, worktext, sizeof(worktext) - 12);
        ...
        HKEY regKey;
        LONG regRetval;
        regRetval = RegOpenKeyEx(HKEY_LOCAL_MACHINE, path.get(), 0, KEY_ALL_ACCESS, &regKey);
        if (regRetval != ERROR_SUCCESS) {
            DBGMSG("Can't open reg key for writing");
        }
        regRetval
            = RegSetValueEx(regKey, "Version", 0, REG_DWORD, (uint8*)&version, sizeof(version));

        // Create blocking DLG for update info
        if (show_dialog) {
            DialogBox(Global_instance, MAKEINTRESOURCE(IDD_CHANGELOG), nullptr,
