...
template <class K, class V> uint32 Dictionary<K, V>::keyHash(IN K& key)
{
...
template <class K, class V> void Dictionary<K, V>::print(IN FILE* out) const
{
    DNode<K, V>* temp;
    uint32 i;
...
//   is returned.
template <class K, class V>
bit8 Dictionary<K, V>::iterate(INOUT int& index, INOUT int& offset, OUT V& value) const
{
    DNode<K, V>* temp;
...
template <class K, class V> uint32 Dictionary<K, V>::getSize(void) const
{
    return (size);
}

// Return the current number of entries in the table
template <class K, class V> uint32 Dictionary<K, V>::getEntries(void) const
{
    return (entries);
}

// Does the Dictionary contain the key?
template <class K, class V> bit8 Dictionary<K, V>::contains(IN K& key)
{
    int offset;
    DNode<K, V>* node;
...
template <class K, class V> bit8 Dictionary<K, V>::getValue(IN K& key, OUT V& value)
{
    int offset;
    DNode<K, V>* node;
