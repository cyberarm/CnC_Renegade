...
RefPtr<UString> Protect::GetPassKey(void) const
{
    ...
    if (passKey.IsValid()) {
        ...
        */
        ...
        LONG result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Westwood\\RenegadeFDS", 0,
                                   KEY_READ, &hKey);
#elif defined(MULTIPLAYERDEMO)
        LONG result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Westwood\\RenegadeMPDemo", 0,
                                   KEY_READ, &hKey);
#elif defined(BETACLIENT)
        LONG result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Westwood\\RenegadeBeta", 0,
                                   KEY_READ, &hKey);
#else
        LONG result
            = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Westwood\\Renegade", 0, KEY_READ, &hKey);
#endif
        ...
        result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows\\CurrentVersion", 0,
                              KEY_READ, &hKey);
...
void SendProtectMessage(HANDLE process, DWORD threadID)
{
    ...
    if (buffer && (bufferSize > 0)) {
        ...
        /*
                        ...
                        LONG result = RegOpenKeyEx(HKEY_LOCAL_MACHINE,
        "Software\\Westwood\\RenegadeFDS", 0, KEY_READ, &hKey); #else  //FREEDEDICATEDSERVER LONG
        result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Westwood\\Renegade", 0, KEY_READ,
        &hKey); #endif //FREEDEDICATEDSERVER
        */
