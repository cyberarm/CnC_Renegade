...
#define APPLICATION_SUB_KEY_NAME "Software\\Westwood\\RenegadeMPDemo\\" #else  //MULTIPLAYERDEMO
...
void RunGame(char* thePath, ConfigFile& config, Process& proc)
{
    ...
    while (true) {
        ...
        while ((skuIndex = Find_Patch(patchFile, MAX_PATH, config)) != 0) {
            //
            // If there is a patch and we are a slave server then we need to exit quick so the
            // master server can apply it.
            //
            bool slave = false;
            bool restart = Get_Restart_Flag(proc, slave);

            if (!restart || !slave) {
                // Pass in the restart flag so that the patch code knows not to bring up the changes
                // dialog.
...
int main(int argc, char* argv[])
{
    ...
    if (hasSecondEXE) {
        ...
        if (time(NULL) > cutoffTime) {
            ...
            // We don't have to wait around since it'll do the entire talk to game, look for
            // patches, etc. deal.
