...
//   Whenever an object loads a pointer, it gives a "pointer to that pointer" to the save load
//   system. Then, after all of the objects have been loaded, the system goes through that list of
//   pointers and finds them in the pointer pair table.  NOTE: use the macros for re-mapping your
...
#ifdef WWDEBUG
    static void Request_Pointer_Remap(void** pointer_to_convert, const char* file = nullptr,
                                      int line = 0);
...
#define REQUEST_REF_COUNTED_POINTER_REMAP(pp)                                                      \
    SaveLoadSystemClass::Request_Ref_Counted_Pointer_Remap(pp)
#endif
