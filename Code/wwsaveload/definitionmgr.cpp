...
DefinitionClass* DefinitionMgrClass::Find_Typed_Definition(const char* name, uint32 class_id,
                                                           bool twiddle)
{
    //
    //	Sanity check
    ...
    if (DefinitionHash == nullptr) {
        WWDEBUG_SAY(("DefinitionMgrClass::Find_Typed_Definition () failed due to a nullptr "
                     "DefinitionHash. %s\n",
                     name));
        return nullptr;
    }
    ...
    // Check the hash table first. The hash table is built as we need the definitions, so if
    // definition is not in the table, it will be added there.

    //
    // TSS nullptr deref on this sucker 08/03/01
    //
            ...
            if (curr_def != nullptr) {
                ...
                if ((curr_class_id == class_id)
                    ...
                    if (::stricmp(curr_def->Get_Name(), name) == 0) {
                        definition = curr_def;
                        // Add the definition to the hash table, so that it can be quickly accessed
...
DefinitionClass* DefinitionMgrClass::Get_Next(DefinitionClass* curr_def, uint32 id, ID_TYPE type)
{
    ...
    for (int index = curr_def->m_DefinitionMgrLink + 1;
         (definition == nullptr) && (index < _DefinitionCount); index++) {
        DefinitionClass* curr_def = _SortedDefinitionArray[index];
        if (curr_def != nullptr) {
...
void DefinitionMgrClass::Free_Definitions(void)
{
    ...
    if (DefinitionHash) {
        HashTemplateIterator<StringClass, DynamicVectorClass<DefinitionClass*>*> ite(
            *DefinitionHash);
...
void DefinitionMgrClass::Unregister_Definition(DefinitionClass* definition)
{
    ...
    // WWASSERT (definition->m_DefinitionMgrLink >= 0 && definition->m_DefinitionMgrLink <
    // _DefinitionCount);
