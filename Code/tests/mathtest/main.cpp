...
void test_eulers(void)
{
    ...
    */
    ...
    CHECK(0,
          (((mat2[0] - mat[0]).Length() < TOLERANCE) && ((mat2[1] - mat[1]).Length() < TOLERANCE)
...
void test_concatenation(void)
{
    ...
    for (i = 0; i < 10; i++) {
        ...
        ok = ((((res1[0] - res2[0]).Length()) < 0.001f) && (((res1[1] - res2[1]).Length()) < 0.001f)
    ...
    for (i = 0; i < 10; i++) {
        ...
        bool ok = ((((res1[0] - res2[0]).Length()) < 0.001f)
                   ...
                   && (((res1[2] - res2[2]).Length()) < 0.001f));
        printf("Test %3d   op_cycles: %d\tc_cycles: %d\t", i + 10, operator_cycles,
               concatenate_cycles);
        if (ok) {
            printf("passed\n");
        }
...
Matrix3D random_matrix(void)
{
    ...
    m.Set_Translation(
        Vector3(WWMath::Random_Float(), WWMath::Random_Float(), WWMath::Random_Float()));
...
void test_aabox_transform(void)
{
    ...
    CHECK(ci++,
          ((newmin - Vector3(-10, -10, -10)).Length() < TOLERANCE)
    ...
    CHECK(ci++,
          ((newmin - Vector3(-10, -1, -1)).Length() < TOLERANCE)
...
void test_sqrt_time(void)
{
    ...
    for (i = 0; i < SAMPLES; i++) {
        _Global += results[i]; // keeping the compiler from optimizing this away
    }
