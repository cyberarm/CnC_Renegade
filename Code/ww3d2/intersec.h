...
public:
    ...
    float Alpha, Beta; // stored for use by Process_Intersection_Results() - used to interpolate
                       // normal across a polygon
    bool Intersects; // could be in the intersection_type, but it is used in a number of bool
                     // operations
...
public:
    enum
    {
        ...
        = 1000, // arbitrary size of stack array used for storing intersection results within
    ...
    Vector3* IntersectionNormal; // if non-zero then Process_Intersection_Results() will place the
                                 // (perhaps vertex interpolated) surface normal here.
    ...
    // If true, all intersections will be convex tests. If false, test mode will be determined
    // (generally) by the render object or whatever called the intersection functions and passes the
    // Convex argument.
    bool ConvexTest;
    ...
    inline void Set(Vector3* location, Vector3* direction, Vector3* intersection_normal,
                    bool interpolate_normal, float max_distance, bool convex_test = false)
    {
    ...
    {
        Set(location, direction, intersection_normal, interpolate_normal, max_distance,
            convex_test);
    }

    virtual ~IntersectionClass() { }

    // this copy routine is used when the model coords are needed to be copied along with the other
    // information.
    inline void IntersectionClass::Copy_Results(IntersectionResultClass* Destination,
    ...
    // this is called only for the nearest intersection. If the request passes a Interpolated_Normal
    // pointer then it will be calculated. otherwise the results are copied into the request
    // structure. This does not copy the matrix or location members; it is intended to be used
    // during poly testing where these values are identical between results, or as a completion
    // function for Copy_Results()
    inline void IntersectionClass::Copy_Partial_Results(IntersectionResultClass* Destination,
                                                        IntersectionResultClass* Source)
    {
    ...
    {
        Set(source->RayLocation, source->RayDirection, source->IntersectionNormal,
            source->InterpolateNormal, source->MaxDistance, source->ConvexTest);
    ...
    // To find the actual intersection location and perhaps the intersection normal, use
    // Intersection_Sphere() instead. loosly based on code found in Graphics Gems I,  p388 this will
    // only set the result's range if intersection occurs; it is intended to be used as a first pass
    // intersection test before intersecting the mesh polygons itself. Note: Does NOT do
    // Max_Distance testing
    inline bool IntersectionClass::Intersect_Sphere_Quick(SphereClass& Sphere,
                                                          IntersectionResultClass* FinalResult)
    {
        ...
        FinalResult->Beta = Sphere.Radius * Sphere.Radius
            - (Vector3::Dot_Product(sphere_vector, sphere_vector)
               - FinalResult->Alpha * FinalResult->Alpha);
    ...
    inline bool IntersectionClass::Intersect_Sphere(SphereClass& Sphere,
                                                    IntersectionResultClass* FinalResult)
    {
    ...
    inline bool Intersect_Plane(IntersectionResultClass* Result, Vector3& Plane_Normal,
                                Vector3& Plane_Point);
    inline bool Intersect_Plane_Quick(IntersectionResultClass* Result, Vector3& Plane_Normal,
                                      Vector3& Plane_Point);
    inline bool Intersect_Polygon(IntersectionResultClass* Result, Vector3& PolygonNormal,
                                  Vector3& v1, Vector3& v2, Vector3& v3);
    inline bool Intersect_Polygon(IntersectionResultClass* Result, Vector3& v1, Vector3& v2,
                                  Vector3& v3);
    inline bool Intersect_Polygon_Z(IntersectionResultClass* Result, Vector3& PolygonNormal,
                                    Vector3& v1, Vector3& v2, Vector3& v3);
    ...
    */
    static inline int _Intersect_Triangles_Z(Vector3 ClipPoints[3], Vector3 TrianglePoints[3],
                                             Vector2 UV[3], Vector3 ClippedPoints[6],
                                             Vector2 ClippedUV[6]);
    ...
    */
    ...
    inline bool Intersect_Screen_Object(IntersectionResultClass* Final_Result, Vector4& Area,
    ...
    // member structure and perform normal interpolation as a final step if indicated in the member
    // data.

    bool Intersect_Object_Array(int ObjectCount, RenderObjClass** ObjectArray,
    ...
    bool Intersect_Screen_Point_RenderObject(float screen_x, float screen_y,
                                             const LayerClass& Layer, RenderObjClass* RObj,
                                             IntersectionResultClass* FinalResult);

    bool Intersect_Screen_Point_Layer_Range(float ScreenX, float ScreenY,
    ...
    bool Intersect_Hierarchy_Sphere(RenderObjClass* Hierarchy,
                                    IntersectionResultClass* FinalResult);
    bool Intersect_Hierarchy_Sphere_Quick(RenderObjClass* Hierarchy,
                                          IntersectionResultClass* FinalResult);

    /*
    ** Identifies exactly what sub object of a render object is under the screen space vector
    */
    RenderObjClass* Intersect_Sub_Object(float screenx, float screeny, LayerClass& layer,
    ...
    */
    ...
    static inline bool _Point_In_Polygon(IntersectionResultClass* FinalResult, Vector3& Normal,
                                         Vector3& loc1, Vector3& loc2, Vector3& loc3);
    static inline bool _Point_In_Polygon(IntersectionResultClass* FinalResult, Vector3& loc1,
                                         Vector3& loc2, Vector3& loc3, int axis_1, int axis_2);
    static inline bool _Point_In_Polygon(Vector3& Point, Vector3& loc1, Vector3& loc2,
                                         Vector3& loc3, int axis_1, int axis_2, float& Alpha,
                                         float& Beta);
    static inline bool _Point_In_Polygon_Z(Vector3& Point, Vector3 Corners[3]);
    static inline bool _Point_In_Polygon_Z(Vector3& Point, Vector3& Corner1, Vector3& Corner2,
                                           Vector3& Corner3);
...
protected:
    ...
    */
    // static inline void _Intersect_Lines_Z(Vector3 &A, Vector3 &B, Vector2 &UVStart, Vector2
    // &UVEnd, Vector3 &C, Vector3 &D, Vector3 ClippedPoints[6], Vector2 ClippedUV[6], int
    // &DestIndex);
    static inline bool IntersectionClass::In_Front_Of_Line(const Vector3& p, // point to test
                                                           const Vector3& e0, // point on edge
                                                           const Vector3& de // direction of edge
    );

    static inline float
    IntersectionClass::Intersect_Lines(const Vector3& p0, // start of line segment
                                       const Vector3& p1, // end of line segment
                                       const Vector3& e0, // point on clipping edge
                                       const Vector3& de // direction of clipping edge
    );

    static inline int IntersectionClass::Clip_Triangle_To_LineXY(int incount, Vector3* InPoints,
                                                                 Vector2* InUVs, Vector3* OutPoints,
                                                                 Vector2* OutUVs,
