...
static const char* VendorNames[]
    = { "Unknown", "NVidia", "ATI",    "Intel",       "S3",       "PowerVR",
...
static const char* DeviceNames3Dfx[]
    = { "Unknown 3Dfx Device", "Voodoo 5500 AGP", "Voodoo 3", "Banshee", "Voodoo 2",
...
DX8Caps::DX8Caps(IDirect3D8* direct3d, IDirect3DDevice8* D3DDevice, WW3DFormat display_format,
...
DX8Caps::DX8Caps(IDirect3D8* direct3d, const D3DCAPS8& caps, WW3DFormat display_format,
                 const D3DADAPTER_IDENTIFIER8& adapter_id)
    : Direct3D(direct3d),
      Caps(caps),
      MaxDisplayWidth(0),
...
void DX8Caps::Compute_Caps(WW3DFormat display_format, const D3DADAPTER_IDENTIFIER8& adapter_id)
{
    ...
    DXLOG(("Product=%d, Version=%d, SubVersion=%d, Build=%d\r\n", Product, Version, SubVersion,
           DriverBuildVersion));

    VendorId = Define_Vendor(adapter_id.VendorId);
    // Make a guess - if driver doesn't intruduce itself and the name starts with 3, what could it
    // possibly be?
    ...
    DXLOG(("GUID = {0x%x, 0x%x, 0x%x}, {0x%2.2x, 0x%2.2x, 0x%2.2x, 0x%2.2x, 0x%2.2x, 0x%2.2x, "
           "0x%2.2x, 0x%2.2x}\r\n",
           adapter_id.DeviceIdentifier.Data1, adapter_id.DeviceIdentifier.Data2,
           adapter_id.DeviceIdentifier.Data3, adapter_id.DeviceIdentifier.Data4[0],
           adapter_id.DeviceIdentifier.Data4[1], adapter_id.DeviceIdentifier.Data4[2],
           adapter_id.DeviceIdentifier.Data4[3], adapter_id.DeviceIdentifier.Data4[4],
           adapter_id.DeviceIdentifier.Data4[5], adapter_id.DeviceIdentifier.Data4[6],
...
void DX8Caps::Check_Texture_Format_Support(WW3DFormat display_format, const D3DCAPS8& caps)
{
    ...
    for (unsigned i = 0; i < WW3D_FORMAT_COUNT; ++i) {
        if (i == WW3D_FORMAT_UNKNOWN) {
            ...
            SupportTextureFormat[i] = SUCCEEDED(Direct3D->CheckDeviceFormat(
                caps.AdapterOrdinal, caps.DeviceType, d3d_display_format, 0, D3DRTYPE_TEXTURE,
...
void DX8Caps::Check_Render_To_Texture_Support(WW3DFormat display_format, const D3DCAPS8& caps)
{
    ...
    for (unsigned i = 0; i < WW3D_FORMAT_COUNT; ++i) {
        if (i == WW3D_FORMAT_UNKNOWN) {
            ...
            WW3DFormat format = (WW3DFormat)i;
            SupportRenderToTextureFormat[i] = SUCCEEDED(Direct3D->CheckDeviceFormat(
                caps.AdapterOrdinal, caps.DeviceType, d3d_display_format, D3DUSAGE_RENDERTARGET,
...
void DX8Caps::Check_Driver_Version_Status()
{
    ...
    case VENDOR_NVIDIA:
        ...
        if (!stricmp(DriverDLL, "nv4_disp.dll") || !stricmp(DriverDLL, "nvdd32.dll")) {
            ...
            case 1260: // Byon - BYONG
                DriverVersionStatus = DRIVER_STATUS_BAD;
                break;
            case 1241: // Steve Tall gets occasional blue screening with this driver version (blue
                       // screen happens in the driver dll)
                DriverVersionStatus = DRIVER_STATUS_BAD;
                break;
            ...
            case 1101:
                DriverVersionStatus
                    = DRIVER_STATUS_BAD; // DriverVersionStatus=DRIVER_STATUS_UNKNOWN;
                break;
    ...
    case VENDOR_ATI:
        if (!stricmp(DriverDLL, "ati2dvag.dll")) {
            ...
            case 3281:
                DriverVersionStatus
                    = DRIVER_STATUS_OK; // Not really ok, but we have to accept something...
                break;
            case 3063:
                DriverVersionStatus = DRIVER_STATUS_BAD;
                break;
...
void DX8Caps::Vendor_Specific_Hacks(const D3DADAPTER_IDENTIFIER8& adapter_id)
{
    if (VendorId == VENDOR_NVIDIA) {
        ...
        SupportDXTC = SupportTextureFormat[WW3D_FORMAT_DXT1]
            | SupportTextureFormat[WW3D_FORMAT_DXT2] | SupportTextureFormat[WW3D_FORMAT_DXT3]
            | SupportTextureFormat[WW3D_FORMAT_DXT4] | SupportTextureFormat[WW3D_FORMAT_DXT5];
    }
    if (VendorId == VENDOR_ATI) {
        ...
        if (DeviceId == DEVICE_ATI_RAGE_128_MOBILITY || DeviceId == DEVICE_ATI_RAGE_128_MOBILITY_M3
            ...
            || DeviceId == DEVICE_ATI_RAGE_128_PRO_ULTRA || DeviceId == DEVICE_ATI_RAGE_128_4X
            || DeviceId == DEVICE_ATI_RAGE_128_PRO_GL || DeviceId == DEVICE_ATI_RAGE_128_PRO_VR
            || DeviceId == DEVICE_ATI_RAGE_128_GL || DeviceId == DEVICE_ATI_RAGE_128_VR) {
        ...
        if (DeviceId == DEVICE_ATI_MOBILITY_RADEON || DeviceId == DEVICE_ATI_MOBILITY_RADEON_VE_M6
            || DeviceId == DEVICE_ATI_RADEON_VE || DeviceId == DEVICE_ATI_RADEON_DDR
            || DeviceId == DEVICE_ATI_RADEON || DeviceId == DEVICE_ATI_MOBILITY_R7500
        ...
        // CAT-lab reported that selecting anisotorpic filtering on Radeon VE causes a lock up after
        // a while
    ...
    if (VendorId == VENDOR_3DFX) {
        ...
        if (DeviceId == DEVICE_3DFX_VOODOO_3 || DeviceId == DEVICE_3DFX_BANSHEE
