 ...
 *---------------------------------------------------------------------------------------------*
 * Functions:                                                                                  *
 *   TextureFileCache::TextureFileCache -- Open cache. * ~TextureFileCache::TextureFileCache -- Shut
 *down texture cache system.         				* TextureFileCache::Save_Texture --
 *Save the texture into the cache.     							*
                                    ...
                                    srColorSurfaceIFace& origsurface)
{
    ...
    for (idx = 0, lod = mreq.largeLOD; lod <= mreq.smallLOD; idx++, lod++) {
        ...
        lzocode = LZOCompressor::Compress(
            (const lzo_byte*)surface->getDataPtr(), (lzo_uint)surface->getDataSize(),
...
bool TextureFileCache::Load_Texture(const char* texturename, srTextureIFace::MultiRequest& mreq)
{
    ...
    //			for a given texture name.  If a user susequently requests mip maps of
    //different
    // 		sizes that are not in the cache, they will be created by use of surface copy
    // functions.
    //			When this happens it is unfortunate because it indicates some sort of
    //inefficiency. 			In code below I have put in comments 'A WASTE' to indicate what part of code
    //deels with it.
    int idx;
    unsigned lod;
    ...
    if (idxsize == lodsize) {
        ...
        while ((lod <= mreq.smallLOD) && (idx < Header.NumMipMaps)) {
            ...
            if (cached) {
                ...
                // Copy data over using size of dest surface incase asserts disabled and sizes are
                // not right.
                memcpy(surface->getDataPtr(), cached->getDataPtr(), size);
            }
            else {
...
void TextureFileCache::Read_Texture(int offsetidx, srColorSurface* surface)
{
    ...
    lzocode = LZOCompressor::Decompress((const lzo_byte*)Get_Compression_Buffer(compsize),
                                        (lzo_uint)compsize, (lzo_byte*)surface->getDataPtr(),
                                        (unsigned*)&decompsize);
 ...
 *=============================================================================================*/
srColorSurfaceIFace*
TextureFileCache::Create_First_Texture_As_Surface(srColorSurfaceIFace* surfacetype)
{
...
{
    // The functions used in here are all thread safe so this function doesn't have to be mutex
    // guarded
