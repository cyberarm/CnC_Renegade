                                   ...
                                   const HTreeClass* htree)
{
    if (rinfo.BWRenderer != nullptr) {
        ...
        rinfo.BWRenderer->Render_Triangles(
            reinterpret_cast<const unsigned long*>(Poly->Get_Array()), PolyCount * 3);
        return;
    }
}
...
void MeshModelClass::compose_deformed_vertex_buffer(VertexFormatXYZNDUV2* verts, const Vector2* uv0,
...
// Destination pointers MUST point to arrays large enough to hold all vertices
void MeshModelClass::get_deformed_screenspace_vertices(Vector4* dst_vert,
                                                       const RenderInfoClass& rinfo,
                                                       const Matrix3D& mesh_transform,
                                                       const HTreeClass* htree)
{
    Matrix4 prj
        = rinfo.Camera.Get_Projection_Matrix() * rinfo.Camera.Get_View_Matrix() * mesh_transform;
...
template <>
inline unsigned int HashTemplateKeyClass<Vector3>::Get_Hash_Value(const Vector3& location)
{
...
template <>
inline unsigned int HashTemplateKeyClass<TriangleSide>::Get_Hash_Value(const TriangleSide& side)
{
      ...
      PolygonCount(0)
{
    ...
    ArraySize = mmc->Get_Polygon_Count()
        * 6; // Each side of each triangle can have 2 polygons added, in the worst case
...
void GapFillerClass::Add_Polygon(unsigned polygon_index, unsigned vidx1, unsigned vidx2,
                                 unsigned vidx3)
{
