...
// This array has vertex locations for screenspace mode - calculated to cover exactly 1x1 and 2x2
// pixels.
...
void PointGroupClass::Set_Arrays(ShareBufferClass<Vector3>* locs,
                                 ...
                                 ShareBufferClass<unsigned char>* frames, int active_point_count,
                                 float vpxmin, float vpymin, float vpxmax, float vpymax)
{
...
void PointGroupClass::Render(RenderInfoClass& rinfo)
{
    ...
    if (APT) {
        ...
        if (PointDiffuse) {
            ...
            VectorProcessorClass::CopyIndexed(&compressed_diffuse[0], PointDiffuse->Get_Array(),
                                              APT->Get_Array(), PointCount);
        ...
        if (PointSize) {
            ...
            VectorProcessorClass::CopyIndexed(&compressed_size[0], PointSize->Get_Array(),
                                              APT->Get_Array(), PointCount);
        ...
        if (PointOrientation) {
            ...
            VectorProcessorClass::CopyIndexed(&compressed_orient[0], PointOrientation->Get_Array(),
                                              APT->Get_Array(), PointCount);
        ...
        if (PointFrame) {
            ...
            VectorProcessorClass::CopyIndexed(&compressed_frame[0], PointFrame->Get_Array(),
                                              APT->Get_Array(), PointCount);
    ...
    while (current < vnum) {
        ...
        DynamicVBAccessClass PointVerts(
            sort ? BUFFER_TYPE_DYNAMIC_SORTING : BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type, delta);
...
void PointGroupClass::Update_Arrays(Vector3* point_loc, Vector4* point_diffuse, float* point_size,
                                    ...
                                    int active_points, int total_points, int& vnum, int& pnum)
{
    ...
    */
    ...
    case TRIS_NOSIZE_NOORIENT: {
        ...
        scaled_offset[0]
            = _TriVertexLocationOrientationTable[DefaultPointOrientation][0] * DefaultPointSize;
        scaled_offset[1]
            = _TriVertexLocationOrientationTable[DefaultPointOrientation][1] * DefaultPointSize;
        scaled_offset[2]
            = _TriVertexLocationOrientationTable[DefaultPointOrientation][2] * DefaultPointSize;
    ...
    case QUADS_NOSIZE_NOORIENT: {
        ...
        scaled_offset[0]
            = _QuadVertexLocationOrientationTable[DefaultPointOrientation][0] * DefaultPointSize;
        scaled_offset[1]
            = _QuadVertexLocationOrientationTable[DefaultPointOrientation][1] * DefaultPointSize;
        scaled_offset[2]
            = _QuadVertexLocationOrientationTable[DefaultPointOrientation][2] * DefaultPointSize;
        scaled_offset[3]
            = _QuadVertexLocationOrientationTable[DefaultPointOrientation][3] * DefaultPointSize;
...
void PointGroupClass::_Init(void)
{
    ...
    */
    ...
    // Unrotated locations
    Vector3 tri_locs[3]
        = { Vector3(0.0f, -2.0f, 0.0f), Vector3(-1.732f, 1.0f, 0.0f), Vector3(1.732f, 1.0f, 0.0f) };
    Vector3 quad_locs[4] = { Vector3(-0.5f, 0.5f, 0.0f), Vector3(-0.5f, -0.5f, 0.0f),
