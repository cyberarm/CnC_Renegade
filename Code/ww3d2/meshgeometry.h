...
public:
    ...
    {
        return get_shade_indices(create);
    }
    ...
    void Generate_Rigid_APT(const OBBoxClass& local_box, const Vector3& view_dir,
                            SimpleDynVecClass<uint32>& apt);

    void Generate_Skin_APT(const OBBoxClass& world_box, SimpleDynVecClass<uint32>& apt,
    ...
    // W3D File Format support.  Note that derived classes have to override these functions
    // completely so that they can handle their extra chunks.  Using these functions you could load
    // mesh data out of a W3D file while ignoring all materials, textures, etc.
