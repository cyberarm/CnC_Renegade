...
void VisPolyClass::Clip(const PlaneClass& plane, VisPolyClass& dest) const
{
    ...
    // perform clipping
    prev_point_in_front
        = !plane.In_Front(Verts[iprev]); // Note, plane normal is outward so we invert this test
    for (int j = 0; j < vcount; j++) {

        cur_point_in_front
            = !plane.In_Front(Verts[i]); // Note, plane nomral is out so we invert this test
...
bool VisRasterizerClass::Render_Triangles(const Vector3* verts, int vcount, const TriIndex* tris,
...
bool VisRasterizerClass::Render_Triangles_No_Clip(const Vector3* verts, int vcount,
                                                  const TriIndex* tris, int tcount)
{
...
struct GradientsStruct
{
    GradientsStruct(const Vector3* verts)
    {
        ...
        DOOZ_DX = oodx
            * (((OOZ[1] - OOZ[2]) * (verts[0].Y - verts[2].Y))
               - ((OOZ[0] - OOZ[2]) * (verts[1].Y - verts[2].Y)));

        DOOZ_DY = oody
            * (((OOZ[1] - OOZ[2]) * (verts[0].X - verts[2].X))
...
int IDBufferClass::Render_Occluder_Scanline(GradientsStruct& grads, EdgeStruct* left,
                                            EdgeStruct* right)
{
...
int IDBufferClass::Render_Non_Occluder_Scanline(GradientsStruct& grads, EdgeStruct* left,
                                                EdgeStruct* right)
{
    if ((left->Y < 1) || (left->Y >= ResHeight)) {
        return 0;
