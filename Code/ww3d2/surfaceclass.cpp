...
void SurfaceClass::Convert_Pixel(Vector3& rgb, const SurfaceClass::SurfaceDescription& sd,
...
void SurfaceClass::Convert_Pixel(unsigned char* pixel, const SurfaceClass::SurfaceDescription& sd,
                                 const Vector3& rgb)
{
    unsigned char r, g, b;
 ...
 *   2/13/2001  hy : Created.                                                                  *
 *=============================================================================================*/
void SurfaceClass::Copy(unsigned int dstx, unsigned int dsty, unsigned int srcx, unsigned int srcy,
                        unsigned int width, unsigned int height, const SurfaceClass* other)
{
    WWASSERT(other);
    WWASSERT(width);
    WWASSERT(height);
 ...
 *   2/13/2001  hy : Created.                                                                  *
 *=============================================================================================*/
void SurfaceClass::Stretch_Copy(unsigned int dstx, unsigned int dsty, unsigned int dstwidth,
                                unsigned int dstheight, unsigned int srcx, unsigned int srcy,
...
void SurfaceClass::FindBB(Vector2i* min, Vector2i* max)
{
    ...
    for (y = min->J; y < max->J; y++) {
        for (x = min->I; x < max->I; x++) {
            ...
            unsigned char* alpha
                = (unsigned char*)((unsigned int)lock_rect.pBits + (y - min->J) * lock_rect.Pitch
                                   + (x - min->I) * size);
 ...
 *   4/9/2001   hy : Created.                                                                  *
 *=============================================================================================*/
void SurfaceClass::DrawHLine(const unsigned int y, const unsigned int x1, const unsigned int x2,
                             unsigned int color)
{
    SurfaceDescription sd;
    Get_Description(sd);
