...
public:
    ...
    void Generate_APT(const OBBoxClass& box, const Vector3& viewdir,
                      SimpleDynVecClass<uint32>& apt);

    bool Cast_Ray(RayCollisionTestClass& raytest);
    int Cast_Semi_Infinite_Axis_Aligned_Ray(const Vector3& start_point, int axis_dir,
                                            unsigned char& flags);
...
private:
    ...
    */
    struct CullNodeStruct
    {
        ...
        inline int
        Get_Front_Child(void); // returns index of front child (only call for non-LEAFs!!!)
    ...
    */
    ...
    {
        OBBoxRayAPTContextStruct(const OBBoxClass& box, const Vector3& viewdir,
                                 SimpleDynVecClass<uint32>& apt)
            : Box(box),
              ViewVector(viewdir),
              APT(apt)
    ...
    int Cast_Semi_Infinite_Axis_Aligned_Ray_Recursive(CullNodeStruct* node,
                                                      const Vector3& start_point, int axis_r,
                                                      int axis_1, int axis_2, int direction,
                                                      unsigned char& flags);
    ...
    MeshGeometryClass*
        Mesh; // pointer to the parent mesh (non-ref-counted; we are a member of this mesh)
                                                             ...
                                                             int axis_dir, unsigned char& flags)
{
    ...
    return Cast_Semi_Infinite_Axis_Aligned_Ray_Recursive(&(Nodes[0]), start_point, axis_r[axis_dir],
                                                         axis_1[axis_dir], axis_2[axis_dir],
...
{
    WWASSERT(!Is_Leaf());
    return FrontOrPoly0; // we shouldn't be calling this on a leaf and the leaf bit should be
                         // zero...
}
