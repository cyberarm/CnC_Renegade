...
TextureClass::TextureClass(unsigned width, unsigned height, WW3DFormat format,
                           MipCountType mip_level_count, PoolType pool, bool rendertarget)
    : D3DTexture(nullptr),
      ...
      LastInactivationSyncTime(0)
{
    ...
    D3DTexture = DX8Wrapper::_Create_DX8_Texture(width, height, format, mip_level_count, d3dpool,
                                                 rendertarget);
      ...
      LastInactivationSyncTime(0)
{
    ...
    case WW3D_FORMAT_X8L8V8U8: // Bumpmap
        // If requesting bumpmap format that isn't available we'll just return the surface in
        // whatever color format the texture file is in. (This is illegal case, the format support
        // should always be queried before creating a bump texture!)
        if (!DX8Wrapper::Is_Initted()
            || !DX8Wrapper::Get_Current_Caps()->Support_Texture_Format(TextureFormat)) {
            TextureFormat = WW3D_FORMAT_UNKNOWN;
        }
        // If bump format is valid, make sure compression is not allowed so that we don't even
        // attempt to load from a compressed file (quality isn't good enough for bump map). Also
        // disable mipmapping.
        else {
            IsCompressionAllowed = false;
...
void TextureClass::Init()
{
    ...
    // If the texture has recently been inactivated, increase the inactivation time (this texture
    // obviously should not have been inactivated yet).
...
{
    if (!D3DTexture) {
        WWASSERT_PRINT(0, "Get_Mip_Level_Count: D3DTexture is nullptr!\n");
        return 0;
    }
...
void TextureClass::Apply_nullptr(unsigned int stage)
{
        ...
        */
        if (hastexinfo) {
            ...
            case W3DTEXTURE_TYPE_BUMPMAP: {
                if (DX8Wrapper::Is_Initted()
                    && DX8Wrapper::Get_Current_Caps()->Support_Bump_Envmap()) {
                    ...
                    else if (DX8Wrapper::Get_Current_Caps()->Support_Texture_Format(
