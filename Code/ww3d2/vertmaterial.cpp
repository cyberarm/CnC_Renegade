...
unsigned long VertexMaterialClass::Compute_CRC(void) const
{
    ...
    crc = CRC_Memory(reinterpret_cast<const unsigned char*>(&EmissiveColorSource),
                     sizeof(EmissiveColorSource), crc);
    crc = CRC_Memory(reinterpret_cast<const unsigned char*>(&UVSource), sizeof(UVSource), crc);
    crc = CRC_Memory(reinterpret_cast<const unsigned char*>(&UseLighting), sizeof(UseLighting),
                     crc);
    ...
    for (i = 0; i < MeshBuilderClass::MAX_STAGES; i++) {
        if (Mapper[i]) {
            ...
            DX8Wrapper::Set_DX8_Texture_Stage_State(i, D3DTSS_TEXCOORDINDEX,
                                                    D3DTSS_TCI_PASSTHRU | UVSource[i]);
...
void VertexMaterialClass::Apply_nullptr(void)
{
    int i;
