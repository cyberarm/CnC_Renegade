...
bool MotionChannelClass::Load_W3D(ChunkLoadClass& cload)
{
    ...
    // There was a bug in the exporter which saved too much data, so let's try and not load
    // everything.
...
void AdaptiveDeltaMotionChannelClass::decompress(uint32 src_idx, float* srcdata, uint32 frame_idx,
                                                 float* outdata)
{
...
float AdaptiveDeltaMotionChannelClass::getframe(uint32 frame_idx, uint32 vector_idx)
{
    ...
    if (frame_idx < CacheFrame) {
        // Requested Frame isn't cached, so cache it, and frame_idx+1, and return the decompressed
        // data
        // from frame_idx
