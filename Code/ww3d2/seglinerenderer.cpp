...
void SegLineRendererClass::Init(const W3dEmitterLinePropertiesStruct& props)
{
    ...
    int texture_mode
        = ((props.Flags & W3D_ELINE_TEXTURE_MAP_MODE_MASK) >> W3D_ELINE_TEXTURE_MAP_MODE_OFFSET);
    switch (texture_mode) {
...
void SegLineRendererClass::Render(RenderInfoClass& rinfo, const Matrix3D& transform,
    ...
    */
    ...
    for (unsigned int chidx = 0; chidx < num_points - 1; chidx += (chunk_size - 1)) {
        ...
        */
        ...
        Matrix3D view2(view[0].X, view[0].Y, view[0].Z, view[0].W, view[1].X, view[1].Y, view[1].Z,
                       view[1].W, view[2].X, view[2].Y, view[2].Z, view[2].W);
        ...
        */
        ...
        subdivision_util(point_cnt, xformed_pts, base_tex_v, &sub_point_cnt, xformed_subdiv_pts,
                         subdiv_tex_v);
        ...
        */
        ...
        top = first_point
            - first_plane[TOP_EDGE] * Vector3::Dot_Product(first_plane[TOP_EDGE], first_point);
        top.Normalize();
        ...
        top = last_point
            - last_plane[TOP_EDGE] * Vector3::Dot_Product(last_plane[TOP_EDGE], last_point);
        top.Normalize();
        intersection[last_isec][TOP_EDGE].Direction = top;
        bottom = last_point
            - last_plane[BOTTOM_EDGE] * Vector3::Dot_Product(last_plane[BOTTOM_EDGE], last_point);
        bottom.Normalize();
        ...
        for (iidx = 2; iidx < num_intersections[TOP_EDGE]; iidx++) {
            ...
            if (fabs(vdp) < parallel_factor) {
                ...
                // Not parallel - intersect planes to get line (get vector, normalize it, ensure it
                // is pointing towards the midpoint)
                ...
                // Parallel (or almost): find point on av. plane closest to midpoint, convert to
                // line
            ...
            if (fabs(vdp) < parallel_factor) {
                ...
                if (Vector3::Dot_Product(intersection[iidx][BOTTOM_EDGE].Direction, midpoint)
                    < 0.0f) {
                    intersection[iidx][BOTTOM_EDGE].Direction
                        = -intersection[iidx][BOTTOM_EDGE].Direction;
                }
            ...
            Vector3::Cross_Product(intersection[iidx][TOP_EDGE].Direction,
                                   intersection[iidx][BOTTOM_EDGE].Direction, &start_pl);
            start_pl.Normalize();
        ...
        if (Is_Merge_Intersections()) {
            ...
            while (merged) {
                ...
                for (edge = FIRST_EDGE; edge <= MAX_EDGE; edge = (SegmentEdge)((int)edge + 1)) {
                    // Merge top and bottom edge intersections: loop through the intersections from
                    // the first intersection to the penultimate intersection, for each intersection
                    // check if it needs to be merged with the next one (which is why the loop
                    // doesn't go all the way to the last intersection). We start at 1 because 0 is
                    // the dummy "pre-first-point" intersection.
                    unsigned int num_isects
                        = num_intersections[edge]; // Capture here because will change inside loop
                    for (iidx_r = 1, iidx_w = 1; iidx_r < num_isects; iidx_r++, iidx_w++) {

                        // Check for either of two possible reasons to merge this intersection with
                        // the next: either the segment on the far side of the next intersection
                        // overlaps this intersection, or the previous segment overlaps the next
                        // intersection. Note that some other conditions need to be true as well.

                        // Note: iidx_r is used for anything at or after the current position,
                        // iidx_w is used for anything before the current position (previous
                        // positions have potentially already been merged). Note: iidx_r is used for
                        // anything at or after the current position, iidx_w is used for anything
                        // before the current position (previous positions have potentially already
                        // been merged).
                        ...
                        while (
                            (!next_int->Fold
                             && (Vector3::Dot_Product(curr_int->Direction, next_seg->StartPlane)
                                 > 0.0f)
                             && (Vector3::Dot_Product(curr_int->Direction,
                                                      next_seg->EdgePlane[edge])
                                 > 0.0f))
                            || (!curr_int->Fold
                                && (Vector3::Dot_Product(next_int->Direction, -curr_seg->StartPlane)
                                    > 0.0f)
                                && (Vector3::Dot_Product(next_int->Direction,
                                                         prev_seg->EdgePlane[edge])
                                    > 0.0f))) {

                            // First calculate location of merged intersection - this is so we can
                            // abort the merge if it yields funky results.
                            ...
                            // Calculate new intersection direction by intersecting prev_seg with
                            // next_seg
                            bool new_parallel;
                            ...
                            if (fabs(vdp) < parallel_factor) {
                                ...
                                // normalize it, ensure it is pointing towards the current point)
                                Vector3::Cross_Product(prev_seg->EdgePlane[edge],
                                                       next_seg->EdgePlane[edge], &new_direction);
                                ...
                                // skipped plane. If the current intersection is parallel, find the
                                // average plane, and find the direction vector on it closest to the
                                // current intersections direction vector.
                            ...
                            // Now check to see if the merge caused any funky results - if so abort
                            // it. Currently we check to see if the distance of the direction from
                            // the two points is larger than the radius times the merge_abort
                            // factor.
                    ...
                    // If iidx_r is exactly equal to num_isects (rather than being larger by one) at
                    // this point, this means that the last intersection was not merged with the
                    // previous one. In this case, we need to do one last copy:
                    if (iidx_r == num_isects) {
                        LineSegmentIntersection* write_int = &(intersection[iidx_w][edge]);
                        LineSegmentIntersection* curr_int = &(intersection[iidx_r][edge]);
                        write_int->PointCount = curr_int->PointCount;
        ...
        */
        ...
        // GENERALIZE FOR WHEN NO TEXTURE (DO NOT SET UV IN THESE CASES? NEED TO GENERALIZE FOR
        // DIFFERENT TEXTURING MODES ANYWAY).
        ...
        for (;;) {
            ...
            if (residual_top_points == 1 && residual_bottom_points == 1) {
                ...
                // Generate two vertices for next point by picking nearest point on each direction
                // line
                ...
                // Exactly one of the pointcounts is greater than one - advance it and draw one
                // triangle
            ...
            if ((top_int_idx >= num_intersections[TOP_EDGE] && residual_top_points == 1)
                || (bottom_int_idx >= num_intersections[BOTTOM_EDGE]
                    && residual_bottom_points == 1)) {
                // Debugging check - if either intersection index is before end, both of them should
                // be and the points should be before the end.
        ...
        */
        ...
        // If color is not white or opacity not 100%, enable gradient in shader and in renderer -
        // otherwise disable.
        unsigned int rgba;
        rgba = DX8Wrapper::Convert_Color(Color, Opacity);
        bool rgba_all = (rgba == 0xFFFFFFFF);

        // Enable sorting if sorting has not been disabled and line is translucent and alpha testing
        // is not enabled.
        ...
        */
        ...
        DynamicVBAccessClass Verts(
            (sorting ? BUFFER_TYPE_DYNAMIC_SORTING : BUFFER_TYPE_DYNAMIC_DX8), dynamic_fvf_type,
            vnum);
        ...
        DynamicIBAccessClass ib_access(
            (sorting ? BUFFER_TYPE_DYNAMIC_SORTING : BUFFER_TYPE_DYNAMIC_DX8), tidx * 3);
        {
            unsigned int i;
    ...
    for (unsigned int pidx = 0; pidx < point_cnt - 1; pidx++) {
        ...
        for (; tos >= 0;) {
            if (stack[tos].Level == SubdivisionLevel) {
                ...
                stack[tos + 1].EndPos
                    = (stack[tos].StartPos + stack[tos].EndPos) * 0.5f + randvec * stack[tos].Rand;
