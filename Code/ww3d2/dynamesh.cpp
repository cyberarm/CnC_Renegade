        ...
        */
        ...
        while (!done) {
            ...
            if (done) {
                ...
                texture_changed
                    = texture_array0 && texture_array0[cur_tri_idx] != texture_array0[next_tri_idx];
                texture1_changed
                    = texture_array1 && texture_array1[cur_tri_idx] != texture_array1[next_tri_idx];
                material_changed = material_array
                    && material_array[tris[cur_tri_idx].I] != material_array[tris[next_tri_idx].I];
                shader_changed
                    = shader_array && shader_array[cur_tri_idx] != shader_array[next_tri_idx];
            }
            ...
            if (done || texture_changed || material_changed || shader_changed) {
                if (buffer_type == BUFFER_TYPE_DYNAMIC_SORTING) {
                    ...
                    DX8Wrapper::Draw_Triangles((start_tri_idx * 3),
                                               (1 + cur_tri_idx - start_tri_idx), min_vert_idx,
...
bool DynamicMeshClass::End_Vertex()
{
    ...
    for (int color_array_index = 0; color_array_index < MAX_COLOR_ARRAYS; color_array_index++) {
        if (MultiVertexColor[color_array_index]) {
            ...
            //CurVertexColor[color_array_index].X; 			color->Y = CurVertexColor[color_array_index].Y;
...
int DynamicMeshClass::Set_Vertex_Material(VertexMaterialClass* material, bool dont_search, int pass)
{
    ...
    if (!MultiVertexMaterial[pass]) {
        Model->Set_Single_Material(
            Peek_Material_Info()->Peek_Vertex_Material(VertexMaterialIdx[pass]), pass);
    }
    return (VertexMaterialIdx[pass]);
}
