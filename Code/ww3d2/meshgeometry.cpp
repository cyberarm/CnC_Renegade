                                                           ...
                                                           unsigned char& flags)
{
    ...
    if (CullTree) {
        ...
        // These tables translate between the axis_dir representation (which is an integer in which
        // 0 indicates a ray along the positive x axis, 1 along the negative x axis, 2 the positive
        // y axis, 3 negative y axis, 4 positive z axis, 5 negative z axis) and a four-integer
        // representation (axis_r is the axis number - 0, 1 or 2 - of the axis along which the ray
        // is cast; axis_1 and axis_2 are the axis numbers of the other two axes; direction is 0 for
        ...
        */
        ...
        for (int poly_counter = 0; poly_counter < poly_count; poly_counter++) {
            ...
            count += (unsigned int)Cast_Semi_Infinite_Axis_Aligned_Ray_To_Triangle(
                v0, v1, v2, tri_plane, start_point, axis_r[axis_dir], axis_1[axis_dir],
...
WW3DErrorType MeshGeometryClass::Load_W3D(ChunkLoadClass& cload)
{
    ...
    ** Note that derived classes need to completely replace this function; only re-using the
    *individual
