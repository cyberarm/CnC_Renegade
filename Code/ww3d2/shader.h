...
#define SHADE_CNST(depth_compare, depth_mask, color_mask, src_blend, dst_blend, fog, pri_grad,     \
                   sec_grad, texture, alpha_test, cullmode, post_det_color, post_det_alpha)        \
...
public:
    ...
    enum DstBlendFuncType
    {
        ...
        DSTBLEND_ONE_MINUS_SRC_COLOR, // destination pixel multiplied by one minus (i.e. inverse)
                                      // fragment RGB components
    ...
    {
        GRADIENT_DISABLE
        = 0, //	000	disable primary gradient (same as OpenGL 'decal' texture blend)
        GRADIENT_MODULATE, //	001	modulate fragment ARGB by gradient ARGB (default)
        GRADIENT_ADD, //	010	add gradient RGB to fragment RGB, copy gradient A to
                      //fragment A
    ...
    enum SrcBlendFuncType
    {
        ...
        SRCBLEND_ONE_MINUS_SRC_ALPHA, // fragment RGB components multiplied by fragment inverse (one
                                      // minus) A
    ...
    {
        return (DetailAlphaFuncType)((ShaderBits & MASK_POSTDETAILALPHAFUNC)
                                     >> SHIFT_POSTDETAILALPHAFUNC);
    }
    inline DetailColorFuncType Get_Post_Detail_Color_Func(void) const
    {
        return (DetailColorFuncType)((ShaderBits & MASK_POSTDETAILCOLORFUNC)
    ...
    inline FogFuncType Get_Fog_Func(void) const
    {
        return (FogFuncType)((ShaderBits & MASK_FOG) >> SHIFT_FOG);
    }
