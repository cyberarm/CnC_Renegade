...
public:
    ...
    static IDirect3DTexture8* Load_Thumbnail(const StringClass& filename);
    //		WW3DFormat texture_format);	// Pass WW3D_FORMAT_UNKNOWN if you don't care

    static IDirect3DSurface8*
    ...
    // Textures can only be created and locked by the main thread so this function sends a request
    // to the texture handling system to load the texture immediatelly next time it enters the main
    // thread. If this function is called from the main thread the texture is loaded immediatelly.
