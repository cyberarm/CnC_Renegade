...
RenderObjClass* ENBAssetManager::Create_Render_Obj(const char* name, float Scale,
                                                   Vector3& HSV_shift)
{
    char newname[512];
    ...
    if (hsv_shift) {
        Recolor_Asset(rendobj, HSV_shift);
    }
...
TextureClass* ENBAssetManager::Recolor_Texture(TextureClass* texture, Vector3& hsv_shift)
{
    ...
    if (!texture->Is_Initialized()) {
        TextureLoader::Request_High_Priority_Loading(
            texture, 0, (TextureClass::MipCountType)texture->Get_Mip_Level_Count());
    }
    ...
    // return NULL
    smallsurf = texture->Get_Surface_Level(
        (TextureClass::MipCountType)texture->Get_Mip_Level_Count() - 1);
...
void ENBAssetManager::Recolor_Mesh(RenderObjClass* robj, Vector3& hsv_shift)
{
    ...
    if (color) {
        Recolor_Vertices(color, model->Get_Vertex_Count(), hsv_shift);
    }
