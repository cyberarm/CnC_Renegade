...
#define VRGB_TO_INT32(rgb)                                                                         \
    (unsigned(rgb[0] * 255.0f) << 16) | (unsigned(rgb[1] * 255.0f) << 8)                           \
        | (unsigned(rgb[2] * 255.0f)) | 0xFF000000
#define VRGBA_TO_INT32(rgb)                                                                        \
    (unsigned(rgb[3] * 255.0f) << 24) | (unsigned(rgb[0] * 255.0f) << 16)                          \
        | (unsigned(rgb[1] * 255.0f) << 8) | (unsigned(rgb[2] * 255.0f))
...
#define FRGBA_TO_INT32(r, g, b, a)                                                                 \
    (unsigned(a * 255.0f) << 24) | (unsigned(r * 255.0f) << 16) | (unsigned(g * 255.0f) << 8)      \
        | (unsigned(b * 255.0f))
...
public:
    ...
    void Add_Quad_Backfaced(const Vector2& v0, const Vector2& v1, const Vector2& v2,
                            const Vector2& v3, const RectClass& uv,
    ...
    void Add_Quad_VGradient(const RectClass& screen, unsigned long top_color,
                            unsigned long bottom_color);
    void Add_Quad_HGradient(const RectClass& screen, unsigned long left_color,
                            unsigned long right_color);

    // Add Tri
    void Add_Tri(const Vector2& v0, const Vector2& v1, const Vector2& v2, const Vector2& uv0,
                 const Vector2& uv1, const Vector2& uv2, unsigned long color = 0xFFFFFFFF);

    // Primitive support
    void Add_Line(const Vector2& a, const Vector2& b, float width,
                  unsigned long color = 0xFFFFFFFF);
    void Add_Line(const Vector2& a, const Vector2& b, float width, const RectClass& uv,
                  unsigned long color = 0xFFFFFFFF);
    void Add_Outline(const RectClass& rect, float width = 1.0F, unsigned long color = 0xFFFFFFFF);
    void Add_Outline(const RectClass& rect, float width, const RectClass& uv,
                     unsigned long color = 0xFFFFFFFF);
    void Add_Rect(const RectClass& rect, float border_width = 1.0F,
                  uint32 border_color = 0xFF000000, uint32 fill_color = 0xFFFFFFFF);
...
protected:
    ...
    void Internal_Add_Quad_Vertices(const Vector2& v0, const Vector2& v1, const Vector2& v2,
                                    const Vector2& v3);
