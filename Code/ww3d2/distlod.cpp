    ...
    if (subobj->Class_ID() == CLASSID_DISTLOD) {
        ...
        for (int i = 0; i < LodCount; i++) {
            Lods[i].Model->Add_Sub_Object_To_Bone(
                sub_lod_obj->Lods[MIN(i, sub_lod_obj->LodCount - 1)].Model, bone_index);
        }
    }
    else {
 ...
 *   3/3/99     GTH : Created.                                                                 *
 *=============================================================================================*/
void DistLODClass::Set_Animation(HAnimClass* motion0, float frame0, HAnimClass* motion1,
...
{
    // Highest LOD is used since lowest may be a nullptr3DObjClass.
...
void DistLODClass::Update_Lod(const CameraClass& camera)
{
    ...
    float dist
        = (camera.Get_Position() - Lods[CurLod].Model->Get_Bounding_Sphere().Center).Quick_Length();
