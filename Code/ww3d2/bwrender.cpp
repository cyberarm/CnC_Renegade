...
void BWRenderClass::Set_Vertex_Locations(Vector2* vertices_, int count)
{
    ...
    VectorProcessorClass::MulAdd(reinterpret_cast<float*>(vertices), half_scale, half_scale,
                                 count * 2);
}
...
void BWRenderClass::Render_Preprocessed_Triangle(Vector3& xcf, Vector3i& yci)
{
    ...
    if (ycnt) {
        ...
        while (ycnt > 0) {
            pixel_buffer.Set_H_Line(WWMath::Float_To_Long(x_left), WWMath::Float_To_Long(x_right),
                                    y);
