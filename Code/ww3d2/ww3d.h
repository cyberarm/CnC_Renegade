...
public:
    ...
    static WW3DErrorType Set_Render_Device(int dev = -1, int resx = -1, int resy = -1,
                                           int bits = -1, int windowed = -1,
                                           bool resize_window = false);
    static WW3DErrorType Set_Render_Device(const char* dev_name, int resx = -1, int resy = -1,
                                           int bits = -1, int windowed = -1,
                                           bool resize_window = false);
    ...
    static void Get_Render_Target_Resolution(int& set_w, int& set_h, int& get_bits,
                                             bool& get_windowed);
    static void Get_Device_Resolution(int& set_w, int& set_h, int& get_bits, bool& get_windowed);
    static WW3DErrorType Set_Device_Resolution(int w = -1, int h = -1, int bits = -1,
    ...
    static WW3DErrorType Registry_Save_Render_Device(const char* sub_key, int device, int width,
                                                     int height, int depth, bool windowed,
                                                     int texture_depth);
    static WW3DErrorType Registry_Load_Render_Device(const char* sub_key,
                                                     bool resize_window = false);
    ...
    /*
    ** Rendering functions
    ** Each frame should be bracketed by a Begin_Render and End_Render call.  Between these two
    *calls you will
    ** normally render scenes.  The render function which accepts a single render object is
    *implemented for
    ** special cases like generating a shadow texture for an object.  Basically this function will
    *have the
    ...
    */
    ...
    static WW3DErrorType Render(SceneClass* scene, CameraClass* cam, bool clear = false,
                                bool clearz = false, const Vector3& color = Vector3(0, 0, 0));
    static WW3DErrorType Render(RenderObjClass& obj, RenderInfoClass& rinfo);
    static void Flush(RenderInfoClass& rinfo); // NOTE: "normal" usage should *NEVER* require the
                                               // user to call this function
    ...
    */
    ...
    static void
    Normalize_Coordinates(int x, int y, float& fx,
    ...
    */
    ...
    static void Override_Current_Static_Sort_Lists(RefRenderObjListClass* sort_list,
