      ...
      DateTime(0)
{
    ...
    unsigned level_size
        = Calculate_DXTC_Surface_Size(SurfaceDesc.Width, SurfaceDesc.Height, Format);
...
unsigned DDSFileClass::Calculate_DXTC_Surface_Size(unsigned width, unsigned height,
                                                   WW3DFormat format)
{
...
// ----------------------------------------------------------------------------
//
// Note that this is NOT an efficient way of extracting pixels from compressed image - we should
// implement faster block-copy method for non-scaled copying.
//
// ----------------------------------------------------------------------------
...
unsigned DDSFileClass::Get_Pixel(unsigned level, unsigned x, unsigned y) const
{
    ...
    switch (Format) {
    // Note that we don't currently really support alpha on DXT1 - all alpha textures should use
    // DXT5. The reason for this is that when converting from DXT1 to 16 bit uncompressed texture we
    // want to be able to use RGB565 format instead of ARGB4444. As the alpha is encoded in DXT1
    // per-block basis there isn't really a way to tell if the surface has an alpha or not so either
    // we use alpha or we don't.
    case WW3D_FORMAT_DXT1: {
    ...
    unsigned source_y) const // DDS y offset to copy from, must be aligned by 4!
{
    ...
    // we use alpha or we don't.
    case WW3D_FORMAT_DXT1: {
        const unsigned char* block_memory = Get_Memory_Pointer(level) + (source_x / 4) * 8
    ...
    case WW3D_FORMAT_DXT5: {
        // Init alphas
        const unsigned char* alpha_block = Get_Memory_Pointer(level) + (source_x / 4) * 16
                                ...
                                for (int y=0;y<4;++y) {
                                        ...
                                        for (int x=0;x<4;++x,bit_idx+=3) {
                                                ...
                                                for (int i=0;i<3;++i) {
                                                        WWASSERT(byte_idx<6);
                                                        unsigned
           alpha_bit=(alpha_block[2+byte_idx]>>(tmp_bit_idx))&1; alpha_index|=alpha_bit<<(i);
                                                        tmp_bit_idx++;
                                                        if (tmp_bit_idx>=8) {
                                                                tmp_bit_idx=0;
                                                                byte_idx++;
        ...
        */
        return contains_alpha != 0xff; // Alpha block... DXT5 should only be used when the image
