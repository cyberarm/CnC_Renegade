...
public:
    ...
    */
    ...
    inline float Char_U_Offset(WCHAR ch = (WCHAR)'H')
    {
        return UOffsetTable[ch & 0xFF];
    } // & 0xFF]; }
    inline float Char_V_Offset(WCHAR ch = (WCHAR)'H')
    {
        return VOffsetTable[ch & 0xFF];
    } // & 0xFF]; }
    inline float Char_U_Width(WCHAR ch = (WCHAR)'H')
    {
    ...
    {
        //		ch &= 0xFF;
        return Vector4(UOffsetTable[ch], VOffsetTable[ch], UOffsetTable[ch] + UWidthTable[ch],
...
public:
    ...
    */
    // u and v are in normalized texture space
    //	inline float	Char_U_Offset( WCHAR ch = (WCHAR)'H') { return FontData->Char_U_Offset( ch &
    //0xFF ); } 	inline float	Char_V_Offset( WCHAR ch = (WCHAR)'H') { return
    //FontData->Char_V_Offset( ch & 0xFF ); } 	inline float	Char_U_Width( WCHAR ch = (WCHAR)'H'
    //) { return FontData->Char_U_Width( ch & 0xFF );  } 	inline float	Char_V_Height( WCHAR ch =
    //(WCHAR)'H') { return FontData->Char_V_Height( ch & 0xFF ); } 	Vector4 Char_UV_Corners( WCHAR ch
    //= (WCHAR)'H' )	{ return FontData->Char_UV_Corners( ch & 0xFF ); }
