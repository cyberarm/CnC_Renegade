...
{
        // NOTE: The texture reduction factor represents the number of powers of two that the
texture
        // must be reduced by in both dimensions. The reason that such an inherently integral
quantity
        // is represented as a float is for the texture reduction algorithms to incorporate
hysteresis
        // properly.
 ...
 *					root transform rather than failing.  This is due to the fact
 ...
 *				the cost array and # LODs + 1 for the value array) to contain the
 *desired data.    *
 *                                                                                             *
 ...
 *   3/11/99    NH : Created.                                                                  *
 *=============================================================================================*/
int RenderObjClass::Calculate_Cost_Value_Arrays(float screen_area, float* values,
                                                float* costs) const
{
    values[0] = AT_MIN_LOD;
    values[1] = AT_MAX_LOD;
                               ...
                               IntersectionResultClass* Final_Result)
{
    ...
    if (Intersect_Sphere_Quick(Intersection, Final_Result)) {
        ...
        Vector3 end
            = *Intersection->RayLocation + *Intersection->RayDirection * Intersection->MaxDistance;
...
PersistClass* RenderObjPersistFactoryClass::Load(ChunkLoadClass& cload) const
{
    ...
    while (cload.Open_Chunk()) {
        ...
        case RENDOBJFACTORY_CHUNKID_VARIABLES: {
            ...
            // if the object we saved didn't have a name, replace it with nullptr
            if (strlen(name) == 0) {
                static int count = 0;
                if (++count < 10) {
                    WWDEBUG_SAY(("RenderObjPersistFactory attempted to load an un-named render "
                                 "object!\r\n"));
            ...
            if (new_obj == nullptr) {
                ...
                if (++count < 10) {
                    ...
                    WWDEBUG_SAY(("Either the asset for this object is gone or you tried to save a "
                                 "procedural object.\r\n"));
                    WWDEBUG_SAY(("Replacing it with a nullptr render object!\r\n"));
                }
                strcpy(name, "nullptr");
...
void RenderObjClass::Save_Sub_Object_User_Lighting(ChunkSaveClass& csave, RenderObjClass* sub_obj,
                                                   int bone_index)
{
