...
public:
    ...
    */
    ...
    // Note that *_DX8_Transform() functions take the matrix in DX8 format - transposed from
    // Westwood convention.
    ...
    static void Set_DX8_Texture_Stage_State(unsigned stage, D3DTEXTURESTAGESTATETYPE state,
                                            unsigned value);
    ...
    static void Apply_Render_State_Changes(); // Apply deferred render state changes (will be called
    ...
    */
    ...
    static IDirect3DTexture8* _Create_DX8_Texture(const char* filename,
                                                  TextureClass::MipCountType mip_level_count);
    static IDirect3DTexture8* _Create_DX8_Texture(IDirect3DSurface8* surface,
                                                  TextureClass::MipCountType mip_level_count);

    static IDirect3DSurface8* _Create_DX8_Surface(unsigned int width, unsigned int height,
                                                  WW3DFormat format);
    ...
    {
        return CurrentAdapterIdentifier;
    }
    ...
    */
    ...
    /*
    ** Render target interface. If render target format is WW3D_FORMAT_UNKNOWN, current display
    *format is used.
    */
    ...
    static void Get_DX8_Texture_Stage_State_Value_Name(StringClass& name,
                                                       D3DTEXTURESTAGESTATETYPE state,
                                                       unsigned value);
    static void Get_DX8_Render_State_Value_Name(StringClass& name, D3DRENDERSTATETYPE state,
                                                unsigned value);
...
protected:
    ...
    */
    ...
    static void Get_Render_Target_Resolution(int& set_w, int& set_h, int& set_bits,
                                             bool& set_windowed);
    ...
    ** Internal functions
    */
    static bool Find_Color_And_Z_Mode(int resx, int resy, int bitdepth, D3DFORMAT* set_colorbuffer,
                                      D3DFORMAT* set_zmode);
...
WWINLINE void DX8Wrapper::_Copy_DX8_Rects(IDirect3DSurface8* pSourceSurface,
                                          ...
                                          CONST POINT* pDestPointsArray)
{
    DX8CALL(CopyRects(pSourceSurface, pSourceRectsArray, cRects, pDestinationSurface,
                      pDestPointsArray));
}
...
WWINLINE unsigned int DX8Wrapper::Convert_Color(const Vector3& color, float alpha)
{
    ...
    // Multiply r, g, b and a components (0.0,...,1.0) by 255 and convert to integer. Or the integer
    // values togerher such that 32 bit ingeger has AAAAAAAARRRRRRRRGGGGGGGGBBBBBBBB.
    __asm
    {
		push	ebp
...
WWINLINE void DX8Wrapper::Set_Projection_Transform_With_Z_Bias(const Matrix4& matrix, float znear,
                                                               float zfar)
{
    ZFar = zfar;
    ZNear = znear;
