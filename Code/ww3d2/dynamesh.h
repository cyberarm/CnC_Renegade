...
public:
    ...
    {
        return MatDesc->Get_UV_Array_By_Index(uv_array_index);
    }

    void Set_Single_Material(VertexMaterialClass* vmat, int pass = 0)
    {
        MatDesc->Set_Single_Material(vmat, pass);
    }
...
public:
    ...
    {
        Model->Set_Material_Info(mat_info);
    }
    ...
    {
        // Note that the active poly count has changed since the last render call by setting the
        // dirty flag
        Reset_Flags();
        Reset_Native_Mesh();
        Reset_Mesh_Counters();
    ...
    {
        Color(v.X, v.Y, v.Z, v.W, color_array_index);
    }
    ...
    /*
    ** The following are a bunch of inlined functions for setting & clearing the mesh model's
    *various flags
    */
    // dirty flags
    ...
    {
        Model->Set_Flag(MeshGeometryClass::DIRTY_VNORMALS, true);
    }
    void Clear_Dirty_Vertex_Normals(void)
    {
        Model->Set_Flag(MeshGeometryClass::DIRTY_VNORMALS, false);
    }
    ...
    {
        Model->Set_Flag(MeshGeometryClass::DISABLE_BOUNDING_BOX, true);
    }
    void Enable_Bounding_Box(void)
    {
        Model->Set_Flag(MeshGeometryClass::DISABLE_BOUNDING_BOX, false);
    }
    bool Test_Bounding_Box(void)
    {
        return (Model->Get_Flag(MeshGeometryClass::DISABLE_BOUNDING_BOX) == 0);
    }

    void Disable_Bounding_Sphere(void)
    {
        Model->Set_Flag(MeshGeometryClass::DISABLE_BOUNDING_SPHERE, true);
    }
    void Enable_Bounding_Sphere(void)
    {
        Model->Set_Flag(MeshGeometryClass::DISABLE_BOUNDING_SPHERE, false);
    }
    bool Test_Bounding_Sphere(void)
    {
        return (Model->Get_Flag(MeshGeometryClass::DISABLE_BOUNDING_SPHERE) == 0);
    }
