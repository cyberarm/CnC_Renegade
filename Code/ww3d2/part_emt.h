...
*/
template <class T>
__inline void Copy_Emitter_Property_Struct(ParticlePropertyStruct<T>& dest,
...
public:
    ...
    {
        EmitRate = rate > 0.0f ? (unsigned int)(1000.0f / rate) : 1000U;
    }
    ...
    // float						Get_Velocity_Random (void) const	{
    // return VelRand ? (VelRand->Get_Maximum_Extent() * 1000.0f) : 0.0f; }
    ...
    {
        Buffer->Get_Color_Key_Frames(colors);
    }
    ...
    {
        Buffer->Get_Size_Key_Frames(sizes);
    }
    ...
    {
        Buffer->Get_Frame_Key_Frames(frames);
    }
    ...
    {
        return Buffer->Get_Initial_Orientation_Random();
    }

    // Line rendering accessors
    int Get_Line_Texture_Mapping_Mode(void) const
    {
        return Buffer->Get_Line_Texture_Mapping_Mode();
    }
...
private:
    ...
    void Initialize_Particle(NewParticleStruct* newpart, unsigned int age, const Quaternion& quat,
                             const Vector3& orig);
