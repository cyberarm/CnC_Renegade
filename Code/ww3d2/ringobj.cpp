...
private:
    ...
    TriIndex* tri_poly; // array of triangle polys, vertex indices (can be discard if switch to fan
                        // renderer)
};
 ...
 *   1/19/00    gth : Created.                                                                 *
 *=============================================================================================*/
void RingRenderObjClass::render_ring(RenderInfoClass& rinfo, const Vector3& center,
...
#else
    SortingRendererClass::Insert_Triangles(Get_Bounding_Sphere(), 0, ring.face_ct, 0,
                                           ring.Vertex_ct);
#endif
 ...
 *   1/19/00    gth : Created.                                                                 *
 *=============================================================================================*/
void RingRenderObjClass::vis_render_ring(SpecialRenderInfoClass& rinfo, const Vector3& center,
                                         const Vector3& extent)
{
    WWASSERT(0);
} // vis_render_ring
...
void RingRenderObjClass::Render(RenderInfoClass& rinfo)
{
    ...
    if (WW3D::Are_Static_Sort_Lists_Enabled() && sort_level != SORT_LEVEL_NONE) {
        ...
        // This code assumes that screen_size returns back the percentage of the screen, 0.0 =
        // 0%, 1.0f = 100%
        ...
        if (Flags & USE_CAMERA_ALIGN) {
            ...
            Matrix3D tm(1.0f, 0.0f, 0.0f, cpos.x, 0.0f, 1.0f, 0.0f, cpos.y, 0.0f, 0.0f, 1.0f,
                        cpos.z);
...
/***********************************************************************************************
 * SphereRenderObjClass::Update_On_Visibilty	-- Either starts or stops the animation based on
 *visibility*
 *                                                                                             *
 * INPUT:                                                                                      *
 *                                                                                             *
...
void RingRenderObjClass::animate()
{
    if (Is_Animating()) {
        ...
        if (ColorChannel.Get_Key_Count() > 0 || AlphaChannel.Get_Key_Count() > 0
            || InnerScaleChannel.Get_Key_Count() > 0 || OuterScaleChannel.Get_Key_Count() > 0) {
            //
