...
public:
    ...
    enum MipCountType
    {
        ...
        MIP_LEVELS_11,
        MIP_LEVELS_12,
        MIP_LEVELS_MAX // This isn't to be used (use MIP_LEVELS_ALL instead), it is just an enum for
                       // creating static tables etc.
    };

    // Create texture with desired height, width and format.
    TextureClass(unsigned width, unsigned height, WW3DFormat format,
                 MipCountType mip_level_count = MIP_LEVELS_ALL, PoolType pool = POOL_MANAGED,
    ...
    IDirect3DTexture8* Peek_DX8_Texture() { return D3DTexture; }
