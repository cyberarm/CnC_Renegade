...
** This is a record of a batch/range of polygons to be rendered.  These hang off of the
*DX8TextureCategoryClass's
...
public:
    DX8PolygonRendererClass(unsigned index_count, MeshClass* mesh_,
                            DX8TextureCategoryClass* tex_cat, unsigned vertex_offset,
                            unsigned index_offset, bool strip);
    ...
    void Render_Sorted(/*const Matrix3D & tm,*/ int base_vertex_offset,
...
{
    //	WWDEBUG_SAY(("Set_Vertex_Index_Range - min: %d, range:
    //%d\n",min_vertex_index_,vertex_index_range_)); 	if (vertex_index_range_>30000) { 		int a=0; 		a++;
    //	}
...
inline void DX8PolygonRendererClass::Render(/*const Matrix3D & tm,*/ int base_vertex_offset)
{
    ...
    if (strip) {
        SNAPSHOT_SAY(("Draw_Strip(%d,%d,%d,%d)\n", index_offset, index_count - 2, min_vertex_index,
                      vertex_index_range));
        ...
        DX8Wrapper::Draw_Triangles(index_offset, index_count / 3, min_vertex_index,
                                   vertex_index_range);
    }
}
                                                   ...
                                                   const SphereClass& bounding_sphere)
{
    ...
    DX8Wrapper::Set_Index_Buffer_Index_Offset(base_vertex_offset);
    SortingRendererClass::Insert_Triangles(bounding_sphere, index_offset, index_count / 3,
                                           min_vertex_index, vertex_index_range);
}
