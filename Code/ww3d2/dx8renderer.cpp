...
DX8TextureCategoryClass::DX8TextureCategoryClass(DX8FVFCategoryContainer* container_,
                                                 TextureClass** texs, ShaderClass shd,
...
void DX8FVFCategoryContainer::Remove_Texture_Category(DX8TextureCategoryClass* tex_category)
{
    ...
    for (pass = 0; pass < passes; pass++) {
        ...
        if (texture_category_list[pass].Peek_Head() != nullptr) {
            return;
        }
    }
...
void DX8FVFCategoryContainer::Render_Procedural_Material_Passes(void)
{
    ...
    visible_matpass_head = visible_matpass_tail = nullptr;
}
...
DX8TextureCategoryClass* DX8FVFCategoryContainer::Find_Matching_Texture_Category(
    TextureClass* texture, unsigned pass, unsigned stage, DX8TextureCategoryClass* ref_category)
{
    // Find texture category which matches ref_category's properties but has 'texture' on given pass
    // and stage.
            ...
            for (unsigned int s = 0; s < MeshMatDescClass::MAX_TEX_STAGES; s++) {
                if (stage != s) {
                    all_textures_same = all_textures_same
                        && (dest_tex_category->Peek_Texture(s) == ref_category->Peek_Texture(s));
                }
            }
...
void DX8FVFCategoryContainer::Change_Polygon_Renderer_Texture(
    DX8PolygonRendererList& polygon_renderer_list, TextureClass* texture, TextureClass* new_texture,
    ...
    while (!src_it.Is_Done()) {
        ...
        if (src_tex_category->Peek_Texture(stage) == texture) {
            ...
            while (!poly_it.Is_Done()) {
                // If source texture category contains polygon renderer, move to destination
                // category
                ...
                if (prc == src_tex_category) {
                    DX8TextureCategoryClass* dest_tex_category = Find_Matching_Texture_Category(
                        new_texture, pass, stage, src_tex_category);
...
unsigned DX8FVFCategoryContainer::Define_FVF(MeshModelClass* mmc, unsigned int* user_lighting,
                                             bool enable_lighting)
{
    ...
    if (mmc->Get_Color_Array(0, false) || (user_lighting != nullptr)) {
        fvf |= D3DFVF_DIFFUSE;
    }
...
public:
    ...
    {
        if ((index == 0) && (mesh->Get_User_Lighting_Array() != nullptr)) {
    ...
    ShaderClass Peek_Shader(unsigned idx, unsigned pass)
    {
        if (mmc->Has_Shader_Array(pass)) {
            ...
            if (idx >= unsigned(mmc->Get_Polygon_Count())) {
                WWASSERT(mmc->Get_Gap_Filler());
                shader
                    = mmc->Get_Gap_Filler()->Get_Shader_Array(pass)[idx - mmc->Get_Polygon_Count()];
            }
            else {
    ...
    */
    ...
    while (!it.Is_Done()) {
        ...
        for (unsigned int stage = 0; stage < MeshMatDescClass::MAX_TEX_STAGES; stage++) {
            all_textures_same
                = all_textures_same && (tex_category->Peek_Texture(stage) == texs[stage]);
        }
        if (all_textures_same && Equal_Material(tex_category->Peek_Material(), mat)
            && tex_category->Get_Shader() == shader) {
            used_indices += tex_category->Add_Mesh(split_table, vertex_offset, used_indices,
                                                   index_buffer, pass);
    ...
    if (!fit_in_existing_category) {
        ...
        DX8TextureCategoryClass* new_tex_category
            = new DX8TextureCategoryClass(this, texs, shader, mat, pass);
        used_indices += new_tex_category->Add_Mesh(split_table, vertex_offset, used_indices,
                                                   index_buffer, pass);
...
struct Textures_Material_And_Shader_Booking_Struct
{
    ...
    bool Add_Textures_Material_And_Shader(TextureClass** texs, VertexMaterialClass* mat,
                                          ShaderClass shd)
    {
...
void DX8SkinFVFCategoryContainer::Render(void)
{
    ...
    DX8Wrapper::Set_Vertex_Buffer(
        nullptr); // Free up the reference to the current vertex buffer
               // (in case it is the dynamic, which may have to be resized)

    DynamicVBAccessClass vb(sorting ? BUFFER_TYPE_DYNAMIC_SORTING : BUFFER_TYPE_DYNAMIC_DX8,
                            dynamic_fvf_type, VisibleVertexCount);
    SNAPSHOT_SAY(("DynamicVBAccess - %s - %d vertices\n", sorting ? "sorting" : "non-sorting",
                  VisibleVertexCount));
...
unsigned DX8TextureCategoryClass::Add_Mesh(Vertex_Split_Table& split_table, unsigned vertex_offset,
                                           unsigned index_offset, IndexBufferClass* index_buffer,
                                           unsigned pass)
{
    ...
    */
    if (polygons) {
        ...
        const TriIndex* src_indices
            = (const TriIndex*)split_table.Get_Polygon_Array(pass); // mmc->Get_Polygon_Array();
        ...
        if (stripify) {
            ...
            if (index_count < unsigned(strip[0])) {
                ...
                DX8PolygonRendererClass* p_renderer
                    = new DX8PolygonRendererClass(index_count, split_table.Get_Mesh_Class(), this,
                                                  vertex_offset, index_offset, true);
...
void DX8TextureCategoryClass::Render(void)
{
    ...
    if (!WW3D::Expose_Prelit()) {
        ...
        for (unsigned i = 0; i < MAX_TEXTURE_STAGES; ++i) {
            SNAPSHOT_SAY(("Set_Texture(%d,%s)\n", i,
                          Peek_Texture(i) ? Peek_Texture(i)->Get_Texture_Name() : "nullptr"));
    ...
    SNAPSHOT_SAY(("Set_Material(%s)\n", Peek_Material() ? Peek_Material()->Get_Name() : "nullptr"));
    ...
    while (prt) {
...
#ifdef WWDEBUG
        // Debug rendering: if it exists, expose prelighting on this mesh by disabling all base
        // textures.
        if (WW3D::Expose_Prelit()) {
        ...
        ** states untouched.  This way they can set a couple global lights that affect the entire
        *scene.
        */
        ...
        */
        ...
        /*
        ** Move to the next render task.  Note that the delete should be fast because prt's are
        *pooled
        */
...
DX8MeshRendererClass::~DX8MeshRendererClass()
{
    ...
    if (texture_category_container_list_skin != nullptr) {
...
static void Add_Rigid_Mesh_To_Container(FVFCategoryList* container_list, unsigned fvf,
                                        MeshClass* mesh)
{
...
void DX8MeshRendererClass::Unregister_Mesh_Type(MeshClass* mesh)
{
    ...
    _RegisteredMeshTable.Remove(
        MeshRegKeyStruct(mesh->Peek_Model(), mesh->Get_User_Lighting_Array()), mesh);
...
void DX8MeshRendererClass::Register_Mesh_Type(MeshClass* mesh)
{
    ...
    WWDEBUG_SAY(("Registering mesh: %s (%d polys, %d verts + %d gap polygons)\n", mmc->Get_Name(),
                 mmc->Get_Polygon_Count(), mmc->Get_Vertex_Count(),
                 mmc->Get_Gap_Filler_Polygon_Count()));
#endif
    ...
    if (skin) {
        ...
        */
        ...
        if (existing_mesh != nullptr) {
            ...
            // We found another instance of this mesh model so we can simply clone the poly
            // renderers
            DX8PolygonRendererListIterator it(&(existing_mesh->PolygonRendererList));
            while (!it.Is_Done()) {
                DX8PolygonRendererClass* src_renderer = it.Peek_Obj();
                DX8PolygonRendererClass* new_renderer
                    = new DX8PolygonRendererClass(*src_renderer, mesh);
            ...
            */
            ...
            /*
            ** Done processing the mesh, add its polygon renderers to the global registered mesh
            *list
            */
