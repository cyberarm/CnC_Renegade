...
WW3DFormat Get_Valid_Texture_Format(WW3DFormat format, bool is_compression_allowed)
{
    ...
    if (!DX8Wrapper::Get_Current_Caps()->Support_DXTC() || !is_compression_allowed) {
        ...
        switch (format) {
        case WW3D_FORMAT_DXT1:
            // NVidia hack - switch to DXT2 is there is no DXT1 support (which is disabled on NVidia
            // cards)
    ...
    if (bits <= 16) {
        ...
        case WW3D_FORMAT_P8:
        default:
            // Basically, anything goes here (just make sure the most common 32 bit formats are
            // converted to 16 bit
            break;
        }
    }
