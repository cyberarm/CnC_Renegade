...
//
RenderObjClass*
AggregateDefClass::Find_Subobject(RenderObjClass& model,
                                  ...
                                  const char bone_path[MESH_PATH_ENTRIES][MESH_PATH_ENTRY_LEN])
{
    ...
    for (int index = 1; (mesh_path[index][0] != 0) && (parent_model != nullptr); index++) {
        ...
        if (bone_path[index][0] == 0) {
            ...
            for (int subobj_index = 0; (subobj_index < subobj_count) && (sub_obj == nullptr);
                 subobj_index++) {
    ...
    for (int index = 0; index < m_SubobjectList.Count(); index++) {
        ...
        if (psubobj_info != nullptr) {
            ...
            if (prender_obj != nullptr) {
                ...
                if (base_model.Add_Sub_Object_To_Bone(prender_obj, psubobj_info->BoneName)
                    == false) {
                    WWDEBUG_SAY(("Unable to attach %s to %s.\r\n", psubobj_info->SubobjectName,
                                 psubobj_info->BoneName));
                }
                ...
                WWDEBUG_SAY(
                    ("Unable to load aggregate subobject %s.\r\n", psubobj_info->SubobjectName));
            }
        }
    }
...
RenderObjClass* AggregateDefClass::Create_Render_Object(const char* passet_name)
{
    ...
    // load it from file
    if ((prender_obj == nullptr) && Load_Assets(passet_name)) {
...
void AggregateDefClass::Initialize(RenderObjClass& base_model)
{
    ...
    m_MiscInfo.Flags = 0;
    m_MiscInfo.Flags
        |= base_model.Is_Sub_Objects_Match_LOD_Enabled() ? W3D_AGGREGATE_FORCE_SUB_OBJ_LOD : 0;
            ...
            for (int node_index = 0; node_index < node_count; node_index++) {
                ...
                if (psubobject != nullptr
                    ...
                    RenderObjClass* prender_obj
                        = WW3DAssetManager::Get_Instance()->Create_Render_Obj(prototype_name);
                                          ...
                                          DynamicVectorClass<RenderObjClass*>& node_list)
{
    ...
    for (int node_index = 0; (node_index < node_list.Count()) && (retval == false); node_index++) {
        ...
        if (prender_obj != nullptr && ::lstrcmpi(prender_obj->Get_Name(), passet_name) == 0) {
            retval = true;
        }
    }
...
WW3DErrorType AggregateDefClass::Read_Info(ChunkLoadClass& chunk_load)
{
    ...
    if (chunk_load.Read(&m_Info, sizeof(m_Info)) == sizeof(m_Info)) {
        ...
        for (UINT isubobject = 0;
             (isubobject < m_Info.SubobjectCount) && (ret_val == WW3D_ERROR_OK); isubobject++) {
