...
bool HTreeClass::read_pivots(ChunkLoadClass& cload, bool pre30)
{
    ...
    */
    ...
    for (int pidx = first_piv; pidx < NumPivots; pidx++) {
        ...
        Pivot[pidx].BaseTransform = Pivot[pidx].BaseTransform
            * Build_Matrix3D(Quaternion(piv.Rotation.Q[0], piv.Rotation.Q[1], piv.Rotation.Q[2],
                                        piv.Rotation.Q[3]));
 ...
 *=============================================================================================*/
bool HTreeClass::Simple_Evaluate_Pivot(HAnimClass* motion, int pivot_index, float frame,
                                       const Matrix3D& obj_tm, Matrix3D* end_tm) const
{
    bool retval = false;
 ...
 *   04/13/2000 PDS  : Created.                                                                *
 *=============================================================================================*/
bool HTreeClass::Simple_Evaluate_Pivot(int pivot_index, const Matrix3D& obj_tm,
 ...
 *   3/4/98     GTH : Created.                                                                 *
 *=============================================================================================*/
void HTreeClass::Blend_Update(const Matrix3D& root, HAnimClass* motion0, float frame0,
...
#define ASSUME_NORMALIZED_ANIM_COMBO_WEIGHTS
            ...
            if (weight_total != 0.0f) {
                ...
                // 	  One anim moves X, the other moves Y.  Assert was just in to warn
                // programmers.
                //				WWASSERT(WWMath::Fabs( weight_total - 1.0 ) <
                //WWMATH_EPSILON);
                                       ...
                                       const HTreeClass* tree_array[])
{
    ...
    for (int pi = 0; pi < new_tree->NumPivots; pi++) {
        ...
        for (int nm = 0; nm < num_morph_sources; nm++) {
            pos.X
                += tree_array[nm]->Pivot[pi].BaseTransform.Get_Translation().X * morph_weights[nm];
            pos.Y
                += tree_array[nm]->Pivot[pi].BaseTransform.Get_Translation().Y * morph_weights[nm];
            pos.Z
                += tree_array[nm]->Pivot[pi].BaseTransform.Get_Translation().Z * morph_weights[nm];
        }
...
HTreeClass* HTreeClass::Create_Interpolated(const HTreeClass* tree_a0_b0,
                                            ...
                                            const HTreeClass* tree_a1_b1, float lerp_a,
                                            float lerp_b)
{
