        ...
        if (time < m_Data[key_count - 1].Get_Time()) {
            ...
            value.intensity
                = (key1->Get_Value().intensity
                   + (key2->Get_Value().intensity - key1->Get_Value().intensity) * percent);
...
public:
    ...
    void Set_Alpha_Vector(const AlphaVectorStruct& v, bool inverse, bool is_additive,
                          bool force = false);
...
private:
    ...
    TriIndex* tri_poly; // array of triangle poly's, vertex indices  (can be discard if switched to
                        // strips + fans)
};
...
protected:
    ...
    void render_sphere();
    void vis_render_sphere(SpecialRenderInfoClass& rinfo, const Vector3& center,
...
inline void SphereRenderObjClass::Set_Local_Center_Extent(const Vector3& center,
                                                          const Vector3& extent)
{
