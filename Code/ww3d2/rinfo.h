...
public:
    ...
    enum RINFO_OVERRIDE_FLAGS
    {
        ...
        = 0x0001, // Override mesh settings to force no backface culling
        RINFO_OVERRIDE_FORCE_SORTING = 0x0002, // Override mesh settings to force sorting
        RINFO_OVERRIDE_ADDITIONAL_PASSES_ONLY
        = 0x0004, // Do not render base passes (only additional passes)
    ...
    void Push_Override_Flags(
        RINFO_OVERRIDE_FLAGS flg); // Saves current override flags on stack and installs a new one
