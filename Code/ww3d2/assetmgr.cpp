 ...
 *   WW3DAssetManager::Release_Font3DData -- Release a Font3DData * WW3DAssetManager::Add_Material
 *-- Add a material to the list                              * WW3DAssetManager::Add_Font3DData --
 *Add a Font3DData to the list                          * WW3DAssetManager::Get_Texture -- get a
 *TextureClass for the specified targa               * WW3DAssetManager::Release_All_Textures --
 *release all textures in the system              * WW3DAssetManager::Register_Prototype_Loader --
 *add a new loader to the system             * WW3DAssetManager::Find_Prototype_Loader -- find the
 *loader that handles this chunk type   * WW3DAssetManager::Add_Prototype -- adds the prototype to
 *the hash table                   * WW3DAssetManager::Find_Prototype -- searches the hash table for
 *the prototype             * WW3DAssetManager::Open_Texture_File_Cache -- Turn on the texture cache
 *system.            * WW3DAssetManager::Close_Texture_File_Cache -- Turn off the texture cache
 *system.          * CachedTextureFileClass::getMipmapData -- get data for texture - check to see if
 *in cache. * CachedTextureFileClass::getMipmapLevelPartial -- not yet implemented *
...
void WW3DAssetManager::Free_Assets(void)
{
    ...
    // NONONONOO.... Don't close it as we might want to free the assets and still be able to load
    // textures.
    //	Close_Texture_File_Cache();
}
...
void WW3DAssetManager::Release_All_Textures(void)
{
    ...
    */
    ...
    for (ite.First(); !ite.Is_Done(); ite.Next()) {
        TextureClass* tex = ite.Peek_Value();
        //		WWASSERT(tex->Num_Refs()==1);	// If asset manager is releasing the
        //texture,
    ...
    for (ite.First(); !ite.Is_Done(); ite.Next()) {
        ...
        if (tex->Num_Refs() == 1) {
            ...
            if (count == 256) {
                ...
                count = 0;
                ite.First(); // iterator doesn't support modifying the hash table while iterating,
