...
public:
    ...
    // Special_Render - all special-case rendering goes here to avoid polluting the main render pipe
    // (e.g. VIS) On_Frame_Update - render objects can register for an On_Frame_Update call; the
    // scene will call this once
    //                   per frame if they do so.
    // Restart - This interface is used to facilitate model recycling.  If a render object is
    // "Restarted" it should
    //           put itself back into a state as if it has never been rendered (e.g. particle
    //           emitters should reset their "emitted particle counts" so they can be re-used.)
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    virtual void Set_Animation(HAnimClass* motion0, float frame0, HAnimClass* motion1, float frame1,
                               float percentage)
    {
    }
    ...
    virtual void Control_Bone(int bindex, const Matrix3D& objtm,
    ...
    virtual bool Intersect_Sphere(IntersectionClass* Intersection,
                                  IntersectionResultClass* Final_Result);
    virtual bool Intersect_Sphere_Quick(IntersectionClass* Intersection,
                                        IntersectionResultClass* Final_Result);
    ...
    virtual bool Build_Dependency_List(DynamicVectorClass<StringClass>& file_list,
                                       bool recursive = true);
    virtual bool Build_Texture_List(DynamicVectorClass<StringClass>& texture_file_list,
                                    bool recursive = true);
    ...
    //	virtual float					Calculate_Texture_Reduction_Factor(float
    //norm_screensize); 	virtual void
    //Set_Texture_Reduction_Factor(float trf);
...
protected:
    ...
    void Save_Sub_Object_User_Lighting(ChunkSaveClass& csave, RenderObjClass* sub_obj,
                                       int bone_index);
    ...
    enum
    {
        ...
        // IS_VERTEX_PROCESSOR =		0x00008000,			// is or has a
        // vertex processor, OBSOLETE!
    ...
    float NativeScreenSize; // The screen size at which the object was designed to be viewed (used
                            // in texture resizing).
...
// Warning: Be sure to call this function only if the transform is known to be valid!
WWINLINE const Matrix3D&
RenderObjClass::Get_Transform_No_Validity_Check(bool& is_transform_identity) const
{
    is_transform_identity = IsTransformIdentity;
    return Transform;
}
