...
FrameGrabClass::FrameGrabClass(const char* filename, MODE mode, int width, int height, int bitcount,
                               float framerate)
{
    HRESULT hr;
    ...
    BitmapInfoHeader.biSizeImage
        = ((((UINT)BitmapInfoHeader.biBitCount * BitmapInfoHeader.biWidth + 31) & ~31) / 8)
...
void FrameGrabClass::GrabAVI(void* BitmapPointer)
{
    ...
    HRESULT hr = AVIStreamWrite(Stream, Counter++, 1, BitmapPointer, BitmapInfoHeader.biSizeImage,
                                AVIIF_KEYFRAME, nullptr, nullptr);
    if (hr != 0) {
        char buf[256];
