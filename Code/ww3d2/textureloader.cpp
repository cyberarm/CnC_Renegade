...
{
    // task should non-nullptr and not on any list
...
{
    // task should be non-nullptr and not on any list
    WWASSERT(task != nullptr && task->Next == nullptr && task->Prev == nullptr);
...
IDirect3DTexture8* Load_Compressed_Texture(const StringClass& filename, unsigned reduction_factor,
...
static bool Is_Format_Compressed(WW3DFormat texture_format, bool allow_compression)
{
    ...
    // If hardware supports DXTC compression, load a compressed texture. Proceed only if the texture
    // format hasn't been defined as non-compressed.
    compressed |= (texture_format == WW3D_FORMAT_UNKNOWN
...
IDirect3DTexture8*
TextureLoader::Load_Thumbnail(const StringClass& filename) //,WW3DFormat texture_format)
{
    ...
    if (texture_format == WW3D_FORMAT_UNKNOWN) {
        dest_format
            = Get_Valid_Texture_Format(WW3D_FORMAT_A4R4G4B4, false); // no compressed formats please
    }
    else {
        dest_format
            = Get_Valid_Texture_Format(texture_format, false); // no compressed formats please
        WWASSERT(dest_format == texture_format);
    }

    IDirect3DTexture8* sysmem_texture = DX8Wrapper::_Create_DX8_Texture(
        thumb->Get_Width(), thumb->Get_Height(), dest_format, TextureClass::MIP_LEVELS_ALL,
    ...
    for (level = 0; level < sysmem_texture->GetLevelCount(); ++level) {
        DX8_ErrorCode(sysmem_texture->LockRect(level, &locked_rects[level], nullptr, 0));
    }
    ...
    for (level = 0; level < sysmem_texture->GetLevelCount() - 1; ++level) {
        BitmapHandlerClass::Copy_Image_Generate_Mipmap(
            width, height, (unsigned char*)locked_rects[level].pBits, locked_rects[level].Pitch,
            dest_format, src_surface, src_pitch, src_format,
    ...
    IDirect3DTexture8* d3d_texture
        = DX8Wrapper::_Create_DX8_Texture(thumb->Get_Width(), thumb->Get_Height(), dest_format,
                                          TextureClass::MIP_LEVELS_ALL, D3DPOOL_DEFAULT);
                                                         ...
                                                         bool allow_compression)
{
    ...
    if (src_format == WW3D_FORMAT_A1R5G5B5 || src_format == WW3D_FORMAT_R5G6B5
        ...
        || src_format == WW3D_FORMAT_L8 || src_width != width || src_height != height) {
        ...
        BitmapHandlerClass::Copy_Image(converted_surface, width, height, width * 4,
                                       ...
                                       src_surface, src_width, src_height, src_width * src_bpp,
                                       src_format, (unsigned char*)targa.GetPalette(),
    ...
    BitmapHandlerClass::Copy_Image((unsigned char*)locked_rect.pBits, width, height,
                                   locked_rect.Pitch, dest_format, src_surface, src_width,
                                   src_height, src_pitch, src_format,
...
void TextureLoader::Request_Thumbnail(TextureClass* tc)
{
    ...
    if (Is_DX8_Thread()) {
        ...
        if (!task
            && (!load_task || load_task->Get_State() < TextureLoadTaskClass::STATE_LOAD_MIPMAP)) {

            // create a thumbnail load task and add to foreground queue.
            task = TextureLoadTaskClass::Create(tc, TextureLoadTaskClass::TASK_THUMBNAIL,
...
void TextureLoader::Suspend_Texture_Load()
{
    WWASSERT_PRINT(Is_DX8_Thread(),
                   "TextureLoader::Suspend_Texture_Load must be called from the main thread!");
...
{
    WWASSERT_PRINT(Is_DX8_Thread(),
                   "TextureLoader::Continue_Texture_Load must be called from the main thread!");
...
TextureLoadTaskClass* TextureLoadTaskClass::Create(TextureClass* tc, TaskType type,
...
static bool Get_Texture_Information(const char* filename, unsigned reduction, unsigned& w,
                                    unsigned& h, WW3DFormat& format, unsigned& mip_count,
                                    bool compressed)
{
    ThumbnailClass* thumb = nullptr;
    ThumbnailManagerClass* thumb_man = ThumbnailManagerClass::Peek_List().Head();
    ...
    if (!thumb) {
        if (compressed) {
            ...
            // Destination size will be the next power of two square from the larger width and
            // height...
...
bool TextureLoadTaskClass::Begin_Compressed_Load(void)
{
    ...
    if (!Get_Texture_Information(Texture->Get_Full_Path(), Get_Reduction(), orig_w, orig_h,
                                 orig_format, orig_mip_count, true)) {
        return false;
    }
    ...
    // Once more, verify that the mip level count is correct (in case it was changed here it might
    // not match the size...well actually it doesn't have to match but it can't be bigger than the
    // size)
...
bool TextureLoadTaskClass::Begin_Uncompressed_Load(void)
{
    ...
    WW3DFormat orig_format;
    if (!Get_Texture_Information(Texture->Get_Full_Path(), Get_Reduction(), width, height,
                                 orig_format, orig_mip_count, false)) {
        return false;
    }
    ...
    dest_format = Get_Valid_Texture_Format(
        dest_format, false); // No compressed destination format if reading from targa...
...
void TextureLoadTaskClass::Lock_Surfaces(void)
{
    ...
    for (unsigned int i = 0; i < MipLevelCount; ++i) {
        D3DLOCKED_RECT locked_rect;
        DX8_ErrorCode(D3DTexture->LockRect(i, &locked_rect, nullptr, 0));
...
bool TextureLoadTaskClass::Load_Uncompressed_Mipmap(void)
{
    ...
    dest_format = Get_Format(); // Texture can be requested in different format than the most
                                // obvious from the TGA
    ...
    // NOTE: We load the palette but we do not yet support paletted textures!
    if (TARGA_ERROR_HANDLER(targa.Load(Texture->Get_Full_Path(), TGAF_IMAGE, false),
                            Texture->Get_Full_Path())) {
        return false;
    }
    ...
    for (unsigned int level = 0; level < Get_Mip_Level_Count(); ++level) {
        ...
        BitmapHandlerClass::Copy_Image(Get_Locked_Surface_Ptr(level), width, height,
                                       Get_Locked_Surface_Pitch(level), Get_Format(), src_surface,
                                       src_width, src_height, src_pitch, src_format, nullptr, 0, true);
