...
bool IntersectionClass::Intersect_Screen_Point_RenderObject(float screen_x, float screen_y,
...
bool IntersectionClass::Intersect_Screen_Point_Layer_Range(float screen_x, float screen_y,
...
bool IntersectionClass::Intersect_Screen_Point_Layer(float screen_x, float screen_y,
                                                     const LayerClass& Layer)
{
...
void IntersectionClass::Append_Object_Array(int MaxCount, int& CurrentCount,
                                            RenderObjClass** ObjectArray, RenderObjClass* Object)
{
...
// determines if specified plane-intersection point (co-planar with polygon) is within the the
// passed polygon. If Interpolated_Normal is specified, it will interpolate the normal for the
// intersection point note: Polygon normal MUST BE CORRECT
                                      ...
                                      IntersectionResultClass* FinalResult)
{
    ...
    for (counter = 0; counter < PLANE_COUNT; counter++) {
        if ((quadrant[counter] != MIDDLE) && ((*RayDirection)[counter] != 0.0f)) {
            distance[counter]
                = (candidate_plane[counter] - (*RayLocation)[counter]) / (*RayDirection)[counter];
        }
        else {
    ...
    for (counter = 0; counter < PLANE_COUNT; counter++) {
        if (nearest_plane != counter) {
            (*intersection)[counter]
                = (*RayLocation)[counter] + distance[nearest_plane] * (*RayDirection)[counter];
...
void IntersectionClass::Append_Hierarchy_Objects(int MaxCount, int& CurrentCount,
    ...
    if (Test_Bounding_Sphere) {
        while (counter--) {
            ...
            if (obj->Intersect_Sphere_Quick(this, &result)) {
                ...
                //				OutputDebugString("o"); // this shows one o per
                //sphere intersection
            }
            else {
                //				OutputDebugString("."); // this shows one . per
                //sphere miss
            }
        }
    }
                                            ...
                                            bool Test_Bounding_Sphere, bool Convex)
{
    ...
    Append_Hierarchy_Objects(MAX_HIERARCHY_NODE_COUNT, candidate_count, candidate_objects,
                             Hierarchy, Test_Bounding_Sphere, Convex);
...
RenderObjClass* IntersectionClass::Intersect_Sub_Object(float screenx, float screeny,
                                                        LayerClass& layer, RenderObjClass* robj,
                                                        IntersectionResultClass* result)
{
                                               ...
                                               bool Test_Bounding_Sphere, bool Convex)
{
    ...
    return Intersect_Object_Array(Object_Count, ObjectArray, FinalResult, TemporaryResults,
                                               ...
                                               bool Test_Bounding_Sphere, bool Convex)
{
    ...
    // if it's a convex hierarchy (ie a control panel) then find the first hit otherwise use the
    // more expensive exact intersection routine for use with potentially concave hierarchies.
    int nearest_index = -1;
    if (ConvexTest || Convex) {
        if (Test_Bounding_Sphere) {
            while (counter--) {
                if (ObjectArray[counter]->Intersect_Sphere_Quick(this,
                                                                 &TemporaryResults[counter])) {
