...
WW3DErrorType MeshModelClass::Load_W3D(ChunkLoadClass& cload)
{
    ...
    */
    ...
    BoundSphereCenter.Set(context->Header.SphCenter.X, context->Header.SphCenter.Y,
    ...
    */
    if ((((W3dAttributes & W3D_MESH_FLAG_COLLISION_TYPE_MASK) >> W3D_MESH_FLAG_COLLISION_TYPE_SHIFT)
         != 0)
        && (CullTree == nullptr)) {
                                                 ...
                                                 MeshLoadContextClass* context)
{
    ...
    ** 2 - Always set the DCG source for this pass to the array (vertex materials will be fixed
    *later)
    **
    ...
    */
    if (CurMatDesc->Has_Color_Array(0) == nullptr) {
        ...
        for (int i = 0; i < Get_Vertex_Count(); i++) {
            ...
            Vector4 col;
            col.Set((float)color.R / 255.0f, (float)color.G / 255.0f, (float)color.B / 255.0f,
                    1.0f);
...
WW3DErrorType MeshModelClass::read_textures(ChunkLoadClass& cload, MeshLoadContextClass* context)
{
    ...
    for (TextureClass* newtex = ::Load_Texture(cload); newtex != nullptr;
...
{
    /*
    ** Determine whether this chunk should be read into the default or alternate material
    *description
    */
...
WW3DErrorType MeshModelClass::read_dcg(ChunkLoadClass& cload, MeshLoadContextClass* context)
{
    ...
    **
    ** 1 - If this is the first diffuse color array we've encountered, load the values.
    ** 2 - Otherwise, if we are in PRELIT_VERTEX mode, put the alpha from this array into the color
    *array.
...
void MeshModelClass::post_process_fog(void)
{
    ...
    if (DefMatDesc->Get_Pass_Count() == 2) {
        ...
        if (!DefMatDesc->ShaderArray[0] && !DefMatDesc->ShaderArray[1]) {
            ...
            // Analyze the mesh to determine if it is the emissive map effect and if it is, fix it
            // up appropriately.
            ...
            if (emissive_map_effect) {
                ...
                // Change the shader/texture setting into an equivalent one which will enable
                // setting fog correctly: Note that we are setting up pass 0 to have a texture now.
            ...
            // Analyze the mesh to determine if it is the shiny mask effect and if it is, fix it up
            // appropriately.
 ...
 *   12/10/98   GTH : Created.                                                                 *
 *=============================================================================================*/
void MeshLoadContextClass::Add_Legacy_Material(ShaderClass shader, VertexMaterialClass* vmat,
                                               TextureClass* tex)
{
...
#if 0 // MESH SAVING CODE HAS NOT BEEN MAINTAINED... Leaving here for future reference if we ever
      // need it :-)
...
{
	// first check if all vertex material pointers are nullptr (is this legal?)
