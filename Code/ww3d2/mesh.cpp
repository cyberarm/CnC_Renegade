 ...
 *	  MeshClass::operator == -- assignment operator for MeshClass * MeshClass::~MeshClass --
 *destructor                                                       * MeshClass::Contains --
 *Determines whether mesh contains a (worldspace) point.             * MeshClass::Free -- Releases
 *all memory/assets in use by this mesh                         * MeshClass::Clone -- Creates a
 *clone of this mesh                                          * MeshClass::Get_Name -- returns the
 *name of the mesh                                       * MeshClass::Set_Name -- sets the name of
 *this mesh                                         * MeshClass::Get_W3D_Flags -- access to the W3D
 *flags                                       * MeshClass::Get_User_Text -- access to the text
 *buffer                                     * MeshClass::Scale -- Scales the mesh *
 ...
 *	  MeshClass::read_vertices -- reads the vertex chunk * MeshClass::read_texcoords -- read in
 *the texture coordinates chunk                        * MeshClass::install_texture_coordinates --
 *installs the given u-v's in each channel that is* MeshClass::read_vertex_normals -- reads a
 *surrender normal chunk from the wtm file        * MeshClass::read_v3_materials -- Reads in version
 *3 materials.                             * MeshClass::read_map -- Reads definition of a texture
 *map from the file                    * MeshClass::read_triangles -- read the triangles chunk *
...
void MeshClass::Compose_Deformed_Vertex_Buffer(VertexFormatXYZNDUV2* verts, const Vector2* uv0,
                                               const Vector2* uv1, const unsigned* diffuse)
{
    WWASSERT(Model->Get_Flag(MeshGeometryClass::SKIN));
        ...
        if (Model->Get_Flag(MeshGeometryClass::SKIN)
            ...
            */
            DX8FVFCategoryContainer* fvf_container
                = PolygonRendererList.Peek_Head()->Get_Texture_Category()->Get_Container();
            ...
            */
            ...
            bool is_alpha
                ...
                || (Model->Get_Single_Shader().Get_Src_Blend_Func()
            ...
            if (render_base_passes) {
                ...
                */
                ...
                while (!it.Is_Done()) {
                    ...
                    polygon_renderer->Get_Texture_Category()->Add_Render_Task(polygon_renderer,
                                                                              this);
                    it.Next();
                }
...
void MeshClass::Render_Material_Pass(MaterialPassClass* pass, IndexBufferClass* ib)
{
    ...
    else if ((pass->Get_Cull_Volume() != nullptr)
...
void MeshClass::Special_Render(SpecialRenderInfoClass& rinfo)
{
    ...
    if (rinfo.RenderType == SpecialRenderInfoClass::RENDER_VIS) {
        ...
        rinfo.VisRasterizer->Enable_Two_Sided_Rendering(
            !!Model->Get_Flag(MeshGeometryClass::TWO_SIDED));
        ...
        if (Model->Get_Flag(MeshModelClass::SKIN) == 0) {
            ...
            rinfo.VisRasterizer->Render_Triangles(
                Model->Get_Vertex_Array(), Model->Get_Vertex_Count(), Model->Get_Polygon_Array(),
            ...
            rinfo.VisRasterizer->Render_Triangles(dst_vert, Model->Get_Vertex_Count(),
...
void MeshClass::Update_Cached_Bounding_Volumes(void) const
{
    ...
    if (Model->Get_Flag(MeshModelClass::ALIGNED) || Model->Get_Flag(MeshModelClass::ORIENTED)) {
        ...
        CachedBoundingBox.Extent.Set(CachedBoundingSphere.Radius, CachedBoundingSphere.Radius,
...
{
    // (gth) The terrain pre-lit stuff *must* use the diffuse and ambient arrays (to get vertex
    // alpha working).  So, for now we'll plug the color array into diffuse and ambient and the
    // light environment will be set up so that ambient_light = 1,1,1
    Vector3 emissive;
