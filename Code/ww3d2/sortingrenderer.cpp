                                            ...
                                            unsigned short vertex_count)
{
    ...
    D3DXMATRIX mtx
        = (D3DXMATRIX&)state->sorting_state.world * (D3DXMATRIX&)state->sorting_state.view;
...
void SortingRendererClass::Flush_Sorting_Pool()
{
                                       ...
                                       overlapping_vertex_count);
    {
        ...
        for (node_id = 0; node_id < overlapping_node_count; ++node_id) {
            ...
            for (unsigned i = 0; i < state->vertex_count; ++i, ++src_verts) {
                vertex_z_array[i] = (mtx[2][0] * src_verts->x + mtx[2][1] * src_verts->y
                ...
                // This illegal state is usually caused by Quake3-engine powered games such as
                // MOHAA.
            ...
            for (i = 0; i < state->polygon_count; ++i) {
                ...
                polygon_idx_array[array_index]
                    = ShortVectorIStruct(idx1 + vertex_array_offset, idx2 + vertex_array_offset,
    ...
    DX8Wrapper::Set_Index_Buffer(
        dyn_ib_access, 0); // Override with this buffer (do something to prevent need for this!)
    DX8Wrapper::Set_Vertex_Buffer(
        dyn_vb_access); // Override with this buffer (do something to prevent need for this!)
    ...
    for (unsigned i = 1; i < overlapping_polygon_count; ++i) {
        if (node_id != tis[i].idx) {
            ...
            DX8Wrapper::Draw_Triangles(start_index * 3, count_to_render, state->min_vertex_index,
                                       state->vertex_count);
