    ...
    while (cload.Open_Chunk()) {
        ...
        case W3D_CHUNK_COMPRESSED_ANIMATION_CHANNEL:
            ...
            case ANIM_FLAVOR_TIMECODED:
                ...
                if (tc_chan->Get_Pivot() < NumNodes) {
                    ...
                    delete tc_chan;
                    WWDEBUG_SAY(("ERROR! animation %s indexes a bone not present in the model. "
                                 "Please re-export!\r\n",
                                 Name));
                }
        ...
        case W3D_CHUNK_COMPRESSED_BIT_CHANNEL:
            ...
            if (newbitchan->Get_Pivot() < NumNodes) {
                ...
                // GTH 09-25-2000: print a warning and survive this error
                delete newbitchan;
                WWDEBUG_SAY(("ERROR! animation %s indexes a bone not present in the model. Please "
                             "re-export!\r\n",
                             Name));
            }
 ...
 *   1/19/98    GTH : Created.                                                                 *
 *=============================================================================================*/
bool HCompressedAnimClass::read_bit_channel(ChunkLoadClass& cload,
                                            TimeCodedBitChannelClass** newchan)
{
...
bool HCompressedAnimClass::Is_Node_Motion_Present(int pividx)
{
    ...
    if (NodeMotion[pividx].vd.X != nullptr) {
        return true;
    }
    if (NodeMotion[pividx].vd.Y != nullptr) {
        return true;
    }
    if (NodeMotion[pividx].vd.Z != nullptr) {
        return true;
    }
    if (NodeMotion[pividx].vd.Q != nullptr) {
        return true;
    }
    if (NodeMotion[pividx].Vis != nullptr) {
        return true;
