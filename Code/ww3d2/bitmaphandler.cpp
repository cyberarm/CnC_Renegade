...
void BitmapHandlerClass::Create_Mipmap_B8G8R8A8(unsigned char* dest_surface,
                                                ...
                                                unsigned src_surface_pitch, unsigned width,
                                                unsigned height)
{
...
void BitmapHandlerClass::Copy_Image_Generate_Mipmap(unsigned width, unsigned height,
                                                    ...
                                                    unsigned char* mip_surface, unsigned mip_pitch)
{
    ...
    for (unsigned y = 0; y < height / 2; ++y) {
        ...
        unsigned b8g8r8a8_10;
        unsigned b8g8r8a8_11;
        for (unsigned x = 0; x < width / 2;
             x++, dest_ptr += dest_bpp * 2, src_ptr += src_bpp * 2, mip_ptr += dest_bpp) {
    ...
    if (dest_surface_width == src_surface_width && dest_surface_height == src_surface_height) {
        ...
        if (generate_mip_level) {
            ...
            if (dest_surface_width == 1) {
                ...
                for (unsigned y = 0; y < dest_surface_height / 2; ++y) {
                    ...
                    unsigned b8g8r8a8_10;
                    unsigned b8g8r8a8_11;
                    for (unsigned x = 0; x < dest_surface_width / 2; x++, dest_ptr += dest_bpp * 2,
                                  src_ptr += src_bpp * 2, mip_ptr += src_bpp) {
                        ...
                        Read_B8G8R8A8(b8g8r8a8_10, src_ptr + src_surface_pitch, src_surface_format,
                                      src_palette, src_palette_bpp);
        ...
        for (unsigned y = 0; y < dest_surface_height; ++y) {
            ...
            for (unsigned x = 0; x < dest_surface_width; ++x, dest_ptr += dest_bpp) {
                unsigned src_x = x * src_surface_width / dest_surface_width;
                src_x *= src_bpp;
                Copy_Pixel(dest_ptr, dest_surface_format, src_ptr + src_x, src_surface_format,
                           src_palette, src_palette_bpp);
            }
        }
    }
