                                      ...
                                      MeshMatDescClass& alternate_materials)
{
    ...
    // add-ref the arrays in default_materials except when the same array is present in
    // alternate_materials
    for (int pass = 0; pass < MAX_PASSES; pass++) {
        for (int stage = 0; stage < MAX_TEX_STAGES; stage++) {

            // UV Coorindate arrays, Each UVSource[pass][stage] which is -1 in the
            // alternate_materials but not -1 in the default_materials causes us to copy over a uv
            // array from the default_materials and set its index into our UVSource array.
        ...
        // Shaders, currently I can't tell if the alternate data has a shader...  Can't override the
        // shader for now.
...
{
    /*
    ** Configure all vertex materials to source the uv coordinates and colors from the correct
    *arrays
    ...
    */
    ...
    for (int pass = 0; pass < PassCount; pass++) {
        ...
        */
        ...
        // Analyze material array types and apply hacks for supporting SR-lighting pipeline if
        // possible.
        ...
        if ((DCGSource[pass] != VertexMaterialClass::MATERIAL) && (ColorArray[0] != NULL)) {
            ...
            for (int vidx = 0; vidx < VertexCount; vidx++) {
                ...
                // If only diffuse is used apply diffuse to color channel and set diffuse source to
                // color 1
                ...
                // If only ambient is used apply ambient to color channel and set ambient source to
                // color 1
                if (!diffuse_used && ambient_used && !emissive_used) {
                    Vector4 diffuse = DX8Wrapper::Convert_Color(diffuse_array[vidx]);
                ...
                // If only emissive is used apply emissive to color channel, set diffuse source to
                // color 1, and turn off lighting
                if (!diffuse_used && !ambient_used && emissive_used) {
                    Vector4 diffuse = DX8Wrapper::Convert_Color(diffuse_array[vidx]);
                    ...
                    if (PassCount != 1) {
                        set_lighting_to_false = false; // Lighting can only be set to false if ALL
                                                       // passes and ALL materials are requesting it
                    }
                }
            }
        }
    ...
    ** HACK: Set lighting to false on all passes if all passes are of type NO DIFFUSE, NO AMBIENT,
    *YES EMISSIVE
    */
    for (pass = 0; pass < PassCount; pass++) {
        ...
        if ((Shader[pass].Get_Primary_Gradient() == ShaderClass::GRADIENT_BUMPENVMAP)
            && (!DX8Wrapper::Is_Initted()
                || DX8Wrapper::Get_Current_Caps()->Support_Bump_Envmap() == false)) {
            kill_pass = true;
        }

        if ((Shader[pass].Get_Primary_Gradient() == ShaderClass::GRADIENT_BUMPENVMAPLUMINANCE)
            && (!DX8Wrapper::Is_Initted()
                || DX8Wrapper::Get_Current_Caps()->Support_Bump_Envmap_Luminance() == false)) {
            kill_pass = true;
        }
...
{
    if (DX8Wrapper::Is_Initted() && DX8Wrapper::Get_Current_Caps()->Support_NPatches()
