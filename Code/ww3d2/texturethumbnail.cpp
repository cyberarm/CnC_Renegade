...
ThumbnailClass::ThumbnailClass(ThumbnailManagerClass* manager, const char* name,
                               unsigned char* bitmap, unsigned w, unsigned h, unsigned original_w,
                               unsigned original_h, unsigned original_mip_level_count,
      ...
      DateTime(0)
{
    ...
    // If DDS file can't be used try loading from TGA
        ...
        BitmapHandlerClass::Copy_Image(Bitmap, Width, Height, Width * 2, WW3D_FORMAT_A4R4G4B4,
...
void ThumbnailManagerClass::Load()
{
    ...
    if (thumb_file->Is_Available()) {
        ...
        if (tmp[0] == ThumbFileHeader[0] && tmp[1] == ThumbFileHeader[1]
            && tmp[2] == ThumbFileHeader[2] && tmp[3] == ThumbFileHeader[3]) {
            ...
            if (total_thumb_count) {
                ...
                for (int i = 0; i < total_thumb_count; ++i) {
                    ...
                    // If per-texture time stamp test is enabled, thumbnail is only used if its time
                    // stamp matches the texture's time stamp.
                    bool valid = true;
                    ...
                    if (valid) {
                        new ThumbnailClass(
                            this, name, ThumbnailMemory + offset - total_header_length, width,
                            height, original_width, original_height, original_mip_level_count,
                            original_format, false, date_time);
                    }
                }
...
void ThumbnailManagerClass::Update_Thumbnail_File(const char* mix_file_name,
                                                  bool display_message_box)
{
    ...
    thumb_file->Open(FileClass::READ); //|FileClass::WRITE);	Changed this to READ only since we
                                       //never use the handle for writing and it may cause
                                       //contention amongst slave servers ST - 12/14/2001 8:26PM
    ...
    // we don't currently have a thumbnail file (either we just deleted it or it never existed, we
    // don't care) so we must create one now.
...
{
    // Renegade uses always.dat... Should all our games use, or should this be moved to somewhere
    // else?
