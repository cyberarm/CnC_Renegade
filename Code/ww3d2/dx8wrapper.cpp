...
void DX8Wrapper::Shutdown(void)
{
    if (D3DDevice) {
        ...
        // If in full screen, reset device to windowed mode before releasing it. This is an attempt
        // to fix some random bugs and crashes on some devices.
#if (0)
        if (!IsWindowed) {
            IsWindowed = true;
            ResolutionWidth = DEFAULT_RESOLUTION_WIDTH;
...
bool DX8Wrapper::Create_Device(void)
{
    ...
    if (FAILED(D3DInterface->GetAdapterIdentifier(CurRenderDevice, D3DENUM_NO_WHQL_LEVEL,
    ...
    // JANI HACK! Some objects flicker on ATI Radeons. This can be fixed by locking the back buffer
    // before flipping. For this to work the back buffers need to be created as lockable!
    DX8Caps dx8_caps(D3DInterface, caps,
    ...
    HRESULT hr
        = D3DInterface->CreateDevice(CurRenderDevice, WW3D_DEVTYPE, _Hwnd, vertex_processing_type,
                                     &_PresentParameters, &D3DDevice);

    if (FAILED(hr)) {
        // The device selection may fail because the device lied that it supports 32 bit zbuffer
        // with 16 bit display. This happens at least on Voodoo2.
        ...
        if ((_PresentParameters.BackBufferFormat == D3DFMT_R5G6B5
            ...
            hr = D3DInterface->CreateDevice(CurRenderDevice, WW3D_DEVTYPE, _Hwnd,
                                            vertex_processing_type, &_PresentParameters,
                                            &D3DDevice);
            if (FAILED(hr)) {
                return false;
            }
...
void DX8Wrapper::Enumerate_Devices()
{
    ...
    for (int adapter_index = 0; adapter_index < adapter_count; adapter_index++) {
        ...
        if (res == D3D_OK) {
            ...
            ** TODO: Fill in more fields of the render device description?  (need some lookup
            *tables)
            */
            ...
            */
            ...
            for (int mode_index = 0; mode_index < mode_count; mode_index++) {
                ...
                if (res == D3D_OK) {
                    ...
                    ** TODO: should we handle more formats?  will any cards report more than 24 or
                    *16 bit?
                    */
                    if (bits != 0) {
            ...
            if (desc.Enumerate_Resolutions().Count() > 0) {
                ...
                */
                ...
                _RenderDeviceShortNameTable.Add(
                    device_name); // for now, just add the same name to the "pretty name table"
...
bool DX8Wrapper::Set_Render_Device(const char* dev_name, int width, int height, int bits,
                                   ...
                                   bool resize_window)
{
    ...
    */
    ...
    WWDEBUG_SAY(("Attempting Set_Render_Device: name: %s, width: %d, height: %d, windowed: %d\r\n",
                 _RenderDeviceNameTable[CurRenderDevice], ResolutionWidth, ResolutionHeight,
                 (IsWindowed ? 1 : 0)));
    ...
    */
    ...
    _PresentParameters.MultiSampleType = D3DMULTISAMPLE_NONE;
    _PresentParameters.SwapEffect = IsWindowed
    ...
    */
    ...
    if (IsWindowed) {
        ...
        */
        ...
        SetWindowPos(_Hwnd, HWND_TOPMOST, 0, 0, GetSystemMetrics(SM_CXSCREEN),
                     GetSystemMetrics(SM_CYSCREEN), SWP_SHOWWINDOW | SWP_NOCOPYBITS);
    ...
    if (resize_window) {
        ...
        if ((rect.right - rect.left) != ResolutionWidth
            ...
            ::SetWindowPos(_Hwnd, nullptr, 0, 0, rect.right - rect.left, rect.bottom - rect.top,
...
bool DX8Wrapper::Set_Device_Resolution(int width, int height, int bits, int windowed,
                                       bool resize_window)
{
...
void DX8Wrapper::Get_Render_Target_Resolution(int& set_w, int& set_h, int& set_bits,
                                              bool& set_windowed)
{
    WWASSERT(IsInitted);
    ...
    if (CurrentRenderTarget != nullptr) {
        ...
        set_windowed
            = IsWindowed; // this doesn't really make sense for render targets (shouldn't matter)...
    }
    else {
...
bool DX8Wrapper::Registry_Save_Render_Device(const char* sub_key)
{
    ...
    return Registry_Save_Render_Device(sub_key, CurRenderDevice, ResolutionWidth, ResolutionHeight,
                                       BitDepth, IsWindowed, TextureBitDepth);
}

bool DX8Wrapper::Registry_Save_Render_Device(const char* sub_key, int device, int width, int height,
    ...
    if (Registry_Load_Render_Device(sub_key, name, sizeof(name), width, height, depth, windowed,
        ...
        if (Set_Render_Device(name, width, height, depth, windowed, resize_window) != true) {
            ...
            while (true) {
                ...
                for (int i = 0; i < 2; ++i) {
                    if (Set_Render_Device(name, width, height, depth, windowed, resize_window)
                        == true) {
                        return true;
                    }
                    if (depth == 16) {
...
bool DX8Wrapper::Registry_Load_Render_Device(const char* sub_key, char* device, int device_len,
                                             int& width, int& height, int& depth, int& windowed,
                                             int& texture_depth)
{
...
bool DX8Wrapper::Find_Color_And_Z_Mode(int resx, int resy, int bitdepth, D3DFORMAT* set_colorbuffer,
                                       D3DFORMAT* set_zmode)
{
...
bool DX8Wrapper::Test_Z_Mode(D3DFORMAT colorbuffer, D3DFORMAT backbuffer, D3DFORMAT zmode)
{
    ...
    if (FAILED(D3DInterface->CheckDeviceFormat(D3DADAPTER_DEFAULT, WW3D_DEVTYPE, colorbuffer,
                                               D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, zmode))) {
    ...
    if (FAILED(D3DInterface->CheckDepthStencilMatch(D3DADAPTER_DEFAULT, WW3D_DEVTYPE, colorbuffer,
                                                    backbuffer, zmode))) {
        WWDEBUG_SAY(("CheckDepthStencilMatch failed.  Colorbuffer format = %d  Backbuffer format = "
                     "%d Zbufferformat = %d\r\n",
...
void DX8Wrapper::End_Scene(bool flip_frames)
{
    ...
    if (flip_frames) {
        // JANI HACK! Some objects flicker on ATI Radeons. This can be fixed by locking the back
        // buffer before flipping. Remember that for this to work the back buffers need to have been
        // create as lockable!
        if (Get_Current_Caps()->Get_Vendor() == DX8Caps::VENDOR_ATI) {
            IDirect3DSurface8* bb;
            DX8CALL(GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &bb));
            D3DLOCKED_RECT rect;
            HRESULT res = bb->LockRect(&rect, nullptr, D3DLOCK_READONLY);
            if (res == D3D_OK) {
                bb->UnlockRect();
                bb->Release();
            }
        ...
        // If the device was lost we need to check for cooperative level and possibly reset the
        // device
...
void DX8Wrapper::Clear(bool clear_color, bool clear_z_stencil, const Vector3& color, float z,
...
void DX8Wrapper::Set_Vertex_Buffer(const DynamicVBAccessClass& vba_)
{
    ...
    render_state_changed
        |= INDEX_BUFFER_CHANGED; // vba_offset changes so index buffer needs to be reset as well.
}
...
void DX8Wrapper::Draw_Sorting_IB_VB(unsigned primitive_type, unsigned short start_index,
                                    ...
                                    unsigned short vertex_count)
{
    ...
    DX8CALL(SetStreamSource(
        0, static_cast<DX8VertexBufferClass*>(dyn_vb_access.VertexBuffer)->Get_DX8_Vertex_Buffer(),
                      ...
                      unsigned short vertex_count)
{
    ...
    case BUFFER_TYPE_DYNAMIC_DX8:
        ...
        case BUFFER_TYPE_DYNAMIC_DX8: {
            DX8_RECORD_RENDER(polygon_count, vertex_count, render_state.shader);
            DX8CALL(DrawIndexedPrimitive((D3DPRIMITIVETYPE)primitive_type, min_vertex_index,
                                         vertex_count, start_index + render_state.iba_offset,
                                         polygon_count));
        } break;
        case BUFFER_TYPE_SORTING:
        case BUFFER_TYPE_DYNAMIC_SORTING:
    ...
    case BUFFER_TYPE_DYNAMIC_SORTING:
        ...
        case BUFFER_TYPE_SORTING:
        case BUFFER_TYPE_DYNAMIC_SORTING:
            Draw_Sorting_IB_VB(primitive_type, start_index, polygon_count, min_vertex_index,
                               vertex_count);
            break;
        case BUFFER_TYPE_INVALID:
            WWASSERT(0);
...
void DX8Wrapper::Draw_Triangles(unsigned buffer_type, unsigned short start_index,
                                unsigned short polygon_count, unsigned short min_vertex_index,
                                unsigned short vertex_count)
{
    if (buffer_type == BUFFER_TYPE_SORTING || buffer_type == BUFFER_TYPE_DYNAMIC_SORTING) {
        SortingRendererClass::Insert_Triangles(start_index, polygon_count, min_vertex_index,
                                               vertex_count);
    }
    else {
...
void DX8Wrapper::Draw_Strip(unsigned short start_index, unsigned short polygon_count,
                            unsigned short min_vertex_index, unsigned short vertex_count)
{
...
void DX8Wrapper::Apply_Render_State_Changes()
{
        ...
        if (render_state_changed & mask) {
            SNAPSHOT_SAY(
                ("DX8 - apply texture %d (%s)\n", i,
                 render_state.Textures[i] ? render_state.Textures[i]->Get_Full_Path() : "nullptr"));
            if (render_state.Textures[i]) {
                render_state.Textures[i]->Apply(i);
            }
            else {
                TextureClass::Apply_nullptr(i);
            }
        }
        else {
            SNAPSHOT_SAY(
                ("DX8 - texture %d not changed (%s)\n", i,
                 render_state.Textures[i] ? render_state.Textures[i]->Get_Full_Path() : "nullptr"));
        }
    }
    ...
    if (render_state_changed & MATERIAL_CHANGED) {
        ...
        if (material) {
            ...
            VertexMaterialClass::Apply_nullptr();
        }
    }
    ...
    if (render_state_changed & INDEX_BUFFER_CHANGED) {
        ...
        if (render_state.index_buffer) {
            ...
            case BUFFER_TYPE_DX8:
            case BUFFER_TYPE_DYNAMIC_DX8:
                DX8CALL(SetIndices(static_cast<DX8IndexBufferClass*>(render_state.index_buffer)
...
IDirect3DTexture8* DX8Wrapper::_Create_DX8_Texture(unsigned int width, unsigned int height,
                                                   WW3DFormat format,
                                                   ...
                                                   D3DPOOL pool, bool rendertarget)
{
    ...
    if (rendertarget) {
        unsigned ret = D3DXCreateTexture(DX8Wrapper::_Get_D3D_Device8(), width, height,
        ...
        if (ret == D3DERR_OUTOFVIDEOMEMORY) {
            WWDEBUG_SAY(("Error: Out of memory while creating render target. Trying to release "
                         "assets...\n"));
            ...
            ret = D3DXCreateTexture(DX8Wrapper::_Get_D3D_Device8(), width, height, mip_level_count,
                                    D3DUSAGE_RENDERTARGET, WW3DFormat_To_D3DFormat(format), pool,
                                    &texture);
    ...
    if (ret == D3DERR_OUTOFVIDEOMEMORY) {
        ...
        if (SUCCEEDED(ret)) {
            ...
            WWDEBUG_SAY(("...Texture creation failed. (%d x %d, format: %s, mips: %d\n", width,
                         height, format_name, mip_level_count));
        }
    }
    DX8_ErrorCode(ret);
                                                   ...
                                                   TextureClass::MipCountType mip_level_count)
{
    ...
    unsigned result
        = D3DXCreateTextureFromFileExA(_Get_D3D_Device8(), filename, D3DX_DEFAULT, D3DX_DEFAULT,
                                       mip_level_count, // create_mipmaps ? 0 : 1,
                                       0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_FILTER_BOX,
                                       D3DX_FILTER_BOX, 0, nullptr, nullptr, &texture);
                                                   ...
                                                   TextureClass::MipCountType mip_level_count)
{
    ...
    DX8_ErrorCode(D3DXLoadSurfaceFromSurface(tex_surface, nullptr, nullptr, surface, nullptr, nullptr,
                                             D3DX_FILTER_BOX, 0));
    tex_surface->Release();
...
IDirect3DSurface8* DX8Wrapper::_Create_DX8_Surface(unsigned int width, unsigned int height,
                                                   WW3DFormat format)
{
    DX8_THREAD_ASSERT();
    DX8_Assert();
...
void DX8Wrapper::Compute_Caps(WW3DFormat display_format)
{
    ...
    delete CurrentCaps;
    CurrentCaps
        = new DX8Caps(_Get_D3D8(), D3DDevice, display_format, Get_Current_Adapter_Identifier());
}
...
void DX8Wrapper::Set_Light_Environment(LightEnvironmentClass* light_env)
{
    if (light_env) {
        ...
        Set_DX8_Render_State(D3DRS_AMBIENT,
...
TextureClass* DX8Wrapper::Create_Render_Target(int width, int height, WW3DFormat format)
{
    ...
    TextureClass* tex = NEW_REF(
        TextureClass,
        (width, height, format, TextureClass::MIP_LEVELS_1, TextureClass::POOL_DEFAULT, true));
...
void DX8Wrapper::Set_Gamma(float gamma, float bright, float contrast, bool calibrate, bool uselimit)
{
    ...
    // IML: I'm not really sure what the intent of the 'limit' variable is. It does not produce
    // useful results for my purposes.
    if (uselimit) {
...
void DX8Wrapper::Get_DX8_Render_State_Value_Name(StringClass& name, D3DRENDERSTATETYPE state,
                                                 unsigned value)
{
    switch (state) {
    case D3DRS_ZENABLE:
