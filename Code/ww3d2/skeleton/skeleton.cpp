    ...
    while (running) {
        ...
        if (shatter == 1) {
            if (my_object->Class_ID() == RenderObjClass::CLASSID_MESH) {
                ...
                ShatterSystem::Shatter_Mesh((MeshClass*)my_object, Vector3(0, 0, 0),
                                            Vector3(1, 0, 0));
                ...
                for (i = 0; i < count; i++) {
                    ...
                    my_object->Set_Position(my_object->Get_Position()
        ...
        if (randmat) {
            if (my_object->Class_ID() == RenderObjClass::CLASSID_MESH) {
                ...
                newvm->Set_Emissive(rand() % 256 / 256.0, rand() % 256 / 256.0,
                                    rand() % 256 / 256.0);
        ...
        if (randtex) {
            if (my_object->Class_ID() == RenderObjClass::CLASSID_MESH) {
                ...
                switch (ran) {
                case 0:
                    sprintf(name, "catwalk.tga");
                    break;
                case 1:
                    sprintf(name, "chickonmap.tga");
                    break;
                case 2:
                    sprintf(name, "fullmoon.tga");
                    break;
                }
...
RenderObjClass* Create_Dynamesh(void)
{
    ...
    VertexMaterialClass* vm
        = VertexMaterialClass::Get_Preset(VertexMaterialClass::PRELIT_NODIFFUSE);
...
RenderObjClass* Create_bitmap2d(void)
{
    Bitmap2DObjClass* bmp
        = NEW_REF(Bitmap2DObjClass, ("chickonmap.tga", 0.0, 0.0, false, false, false));
    return bmp;
}
...
void Create_Decal(RenderObjClass* robj, Matrix3D& tm, float radius, char* texture_name)
{
    ...
    */
    ...
    gen->Set_Ortho_Projection(-radius, radius, -radius, radius, 0.01f,
                              backup_dist + 2.0f * DECAL_HALF_SLAB_THICKNESS);
...
MaterialPassClass* Create_Bump_Material_Pass(void)
{
    TextureClass* texture = WW3DAssetManager::Get_Instance()->Get_Texture(
        "earthbump.tga", TextureClass::MIP_LEVELS_1);
    TextureClass* bump_texture
        = WW3DAssetManager::Get_Instance()->Get_Bumpmap_Based_On_Texture(texture);
    REF_PTR_RELEASE(texture);
    TextureClass* env_texture
        = WW3DAssetManager::Get_Instance()->Get_Texture("catwalk.tga", TextureClass::MIP_LEVELS_1);
    ...
    /*
            ...
            TextureClass*
    normal_map=WW3DAssetManager::Get_Instance()->Get_Bumpmap_Based_On_Texture(texture);
            VertexMaterialClass * mtl = NEW_REF(VertexMaterialClass,());
...
void Init_Assets()
{
    ...
    /*	AssetManager->Load_3D_Assets("GreenSphere.w3d");		// Tests regular meshes
            AssetManager->Load_3D_Assets("TexSphere.w3d");			// Tests texture
       mapping AssetManager->Load_3D_Assets("TexScrlSphere.w3d");		// Tests Linear
       Offset mapper AssetManager->Load_3D_Assets("SphereAnim.w3d");			// Tests
       hierarchical animation (if you install its anim)
       AssetManager->Load_3D_Assets("SkinTest.w3d");			// Tests skinning and
       environment mapping AssetManager->Load_3D_Assets("sorting_test.w3d");			//
       Tests skinning and environment mapping AssetManager->Load_3D_Assets("boxtest.w3d");
       // Tests BoxRenderObjClass (turn on the box display mask)
            AssetManager->Load_3D_Assets("Test1.w3d");				// Test yellow
       particles AssetManager->Load_3D_Assets("Test2.w3d");				// Tests
       alpha particles AssetManager->Load_3D_Assets("MPassSphere.w3d");		// Tests more than 2
       passes on a mesh AssetManager->Load_3D_Assets("MPassTeapot.w3d");		// Tests
       more that 2 passes AssetManager->Load_3D_Assets("DupSphere.w3d");			//
       Tests 3 passes which all share the same uv coords
            AssetManager->Load_3D_Assets("vcolortest.w3d");			// Tests vertex
       color AssetManager->Load_3D_Assets("valphatest.w3d");			// Tests vertex
       alpha AssetManager->Load_3D_Assets("LightmapTest0.w3d");		// Tests pre-lighting, 3
       solves: vertex, multi-pass, multi-stage AssetManager->Load_3D_Assets("LightmapTest1.w3d");
       // Tests pre-lighting with alternate solves! (not currently runtime switchable)
            AssetManager->Load_3D_Assets("CubeSkin.w3d");			// Tests a simple
       skin, also can be used to test decals on skin meshes
            AssetManager->Load_3D_Assets("ProjectorTest.w3d");		// This model contains a
       mesh and an animated "ProjectorBone" for testing texture projections
            AssetManager->Load_3D_Assets("PolyRateTest.w3d");		// This model has lots of
       polys! AssetManager->Load_3D_Assets("PPVF01A.w3d");				// Game
       asset AssetManager->Load_3D_Assets("SPHERE_TEST.w3d");		// Tests multitexturing
            AssetManager->Load_3D_Assets("SCREEN_ALIGN.w3d");		// Tests camera aligned
       meshes AssetManager->Load_3D_Assets("halfsphere.w3d");			// Tests two-sided
       flag, camera aligned, and camera oriented meshes AssetManager->Load_3D_Assets("tsi01a.w3d");
       // Tests static sort level AssetManager->Load_3D_Assets("tsi08a.w3d");			//
       Tests static sort level
    */
