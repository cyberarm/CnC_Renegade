...
public:
    ...
    */
    ...
    {
        return (Texture[pass][stage] != nullptr) || (TextureArray[pass][stage] != nullptr);
    }
    bool Has_Shader_Data(int pass)
    {
        return (Shader[pass] != nullptrShader) || (ShaderArray[pass] != nullptr);
    }
    bool Has_Material_Data(int pass)
    {
        return (Material[pass] != nullptr) || (MaterialArray[pass] != nullptr);
    }

    /*
    ** "Get" functions for Materials, Textures, and Shaders when there are more than one
    *(per-polygon or per-vertex)
    */
    ...
    /*
    ** "Peek" functions for Materials and Textures when there are more than one (per-polygon or
    *per-vertex)
    */
    ...
    */
    ...
    static ShaderClass nullptrShader; // Used to mark no shader data
...
{
public:
    MatBufferClass(int count)
        : ShareBufferClass<VertexMaterialClass*>(count)
    {
        Clear();
    }
...
{
public:
    TexBufferClass(int count)
        : ShareBufferClass<TextureClass*>(count)
    {
        Clear();
    }
...
{
public:
    UVBufferClass(int count)
        : ShareBufferClass<Vector2>(count),
          CRC(0xFFFFFFFF)
    {
    }
...
** These functions manage the data associated with the material description for the mesh.  Since
*there
