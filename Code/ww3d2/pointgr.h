...
public:
    enum PointModeEnum
    {
        ...
        SCREENSPACE // each point is a tri placed to affect certain pixels (should be used with 2D
                    // camera)
    };
    ...
    void Set_Arrays(ShareBufferClass<Vector3>* locs, ShareBufferClass<Vector4>* diffuse = nullptr,
                    ...
                    ShareBufferClass<unsigned char>* frames = nullptr, int active_point_count = -1,
                    float vpxmin = 0.0f, float vpymin = 0.0f, float vpxmax = 0.0f,
                    float vpymax = 0.0f);
...
protected:
    // Update arrays.
    void Update_Arrays(Vector3* point_loc, Vector4* point_diffuse, float* point_size,
    ...
    unsigned char
        DefaultPointOrientation; // point orientation (orientation array overrides if present)
