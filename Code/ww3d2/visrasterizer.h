...
** it is a floating point z-buffer and an id buffer which is used by the visiblity precalculation
*system.
...
public:
    ...
    */
    ...
    bool Render_Triangles(const Vector3* verts, int vcount, const TriIndex* tris, int tcount,
                          const AABoxClass& bounds);
    const uint32* Get_Pixel_Row(int y, int min_x, int max_x)
    {
        return IDBuffer.Get_Pixel_Row(y, min_x, max_x);
    }
...
protected:
    ...
    bool Render_Triangles_No_Clip(const Vector3* verts, int vcount, const TriIndex* tris,
                                  int tcount);
