...
** CullTree - culling tree becomes instance specific if a vertex moves (shouldn't even use this with
*skins...)
**
** ALWAYS UNIQUE, BUT SHARE ARRAYS BETWEEN ALTERNATE MATERIAL REPRESENTATIONS (should we share some
*of these?)
** UV, DIG, DCG, SCG
...
public:
    ...
    {
        return CurMatDesc->Get_DCG_Source(pass);
    }
    VertexMaterialClass::ColorSourceType Get_DIG_Source(int pass)
    {
        return CurMatDesc->Get_DIG_Source(pass);
    }
    ...
    {
        CurMatDesc->Set_Single_Material(vmat, pass);
    }
    ...
    {
        return CurMatDesc->Peek_Single_Material(pass);
    }
    ...
    {
        CurMatDesc->Set_Material(vidx, vmat, pass);
    }
    void Set_Shader(int pidx, ShaderClass shader, int pass = 0)
    {
        CurMatDesc->Set_Shader(pidx, shader, pass);
    }
    ...
    // Queries for determining whether this model has per-polygon arrays of Materials, Shaders, or
    // Textures
    ...
    {
        return CurMatDesc->Has_Texture_Array(pass, stage);
    }

    // "Get" functions for Materials, Textures, and Shaders when there are more than one
    // (per-polygon/per-vertex)
    VertexMaterialClass* Get_Material(int vidx, int pass = 0) const
    {
        return CurMatDesc->Get_Material(vidx, pass);
    }
    ...
    // "Peek" functions for Materials and Textures when there are more than one
    // (per-polygon/per-vertex)
    VertexMaterialClass* Peek_Material(int vidx, int pass = 0) const
    {
        return CurMatDesc->Peek_Material(vidx, pass);
    }
...
protected:
    ...
    void get_deformed_screenspace_vertices(Vector4* dst_vert, const RenderInfoClass& rinfo,
                                           const Matrix3D& mesh_tm, const HTreeClass* htree);
    void compose_deformed_vertex_buffer(VertexFormatXYZNDUV2* verts, const Vector2* uv0,
    ...
    // AlternateMatDes - an optional alternate material description, allocated at load time if
    // needed CurMatDesc - pointer to the currently active material description.
