...
void StripOptimizerClass::Optimize_Strip_Order(int* strips, int strip_count)
{
    ...
    int outSize
        = Get_Strip_Index_Count(strips, strip_count) + strip_count; // output memory alloc size
...
template <>
inline unsigned int HashTemplateKeyClass<Strip::Edge>::Get_Hash_Value(const Strip::Edge& s)
{
...
namespace Strip
{
    ...
    inline Vector3i Stripify::getTriangleNodeConnectivityWeights(const TriangleQueue& queue,
                                                                 const Triangle& tri)
    {
        int weight[3];
        for (int i = 0; i < 3; i++) {
                ...
                if (!t->m_neighbors[j]) // if neighbor not defined yet
                {
                    ...
                    if (n) // if edge is already in the hash...
                    {
                        ...
                        if (k < 3) // (k==3) -> no match
                        {
                            ...
                            hash.Remove(e); // remove from hash (this speeds up the hash queries
                                            // considerably)
                        }
                    }
                    else {
    ...
    int* Stripify::stripify(const Vector3i* inTris, int N)
    {
        ...
        for (;;) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                {
                    int weight
                        = t->m_neighbors[i]->getConnectivity(); // get connectivity of neighbor
