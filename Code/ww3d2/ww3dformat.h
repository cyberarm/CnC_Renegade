...
void Get_WW3D_Format(WW3DFormat& dest_format, WW3DFormat& src_format, unsigned& src_bpp,
                     const Targa& targa);
...
// Get valid texture format (on current hardware) that is closest to the given format (for instance,
// 32 bit ARGB8888 would return 16 bit ARGB4444 if the device doesn't support 32 bit textures). Pass
// false to the second parameter if you don't wish to consider compressed textures on hardware that
// supports them. The parameter has no effect on hardware that doesn't support compression.
