...
// After the dot product between the camera direction and the camera space location of the light
// source, we do a power to Halopow for the halo size and power to DazzlePow for the dazzle size.
// Halo/DazzleArea defines the angle where the values are valid, so any angle beyond
// HaloArea/DazzleArea results the halo or dazzle being scaled to zero. The angles are re-scaled
// from normalized angle scale of (-1.0...1.0) only HaloArea/DazzleArea defined part is used.
...
// Direction area defines the maximum difference between the light direction and the eyespace
// location, so the dazzle can only be seen if the camera is inside the light cone. Value 0.0 means
// dazzle is not directional, it can be seen from any direction. Halo is not affected. Dazzle
// direction defines the light direction vector.
...
// Dazzle and halo start to afde out after FadeoutStart meters and are completely gone by FadeoutEnd
// meters.
...
// The weight of history for the intensities. The history weight is per millisecond, so if you want
// to have any real history, values higher than 0.95 are recommended (don't use value of 1.0 or
// anything higher!)
...
// If false, camera matrix's translation term isn't used. If translation isn't used the dazzle is
// treated as always visible, scene graph visibility is not used.
...
// Dazzle test color could be white in many cases but if the level has a lot of white then another
// color can be defined.
const char* DAZZLE_TEST_COLOR_STRING = "DazzleTestColor";

// Dazzles can blink on and off.  The blink period defines the time for one cycle, the blink "on
// time" defines the amount of time within that period that the dazzle is "on"
...
// The number of flare sprites in lensflare. The FlareLocation, FlareSize and FlareColor paremeters
// are procedural, the names are produced based on this parameter. If FlareCount is 3, there exists
// FlareLocation1, FlareLocation2 and FlareLocation3... And so on.
const char* FLARE_COUNT_STRING = "FlareCount";

// 1D-locations of the flares. 0.0 means the location of the center of the screen and 1.0 means the
// location of the lightsource. The values can be smaller than zero and larger than 1.0.
...
// Uv coordinates (as there can be only one texture used, but one may want multiple images). The
// values is a 4-float vector: start_u, start_v, end_u, end_v.
...
const Vector2 DazzleINIClass::Get_Vector2(char const* section, char const* entry,
...
const Vector3 DazzleINIClass::Get_Vector3(char const* section, char const* entry,
...
const Vector4 DazzleINIClass::Get_Vector4(char const* section, char const* entry,
...
void LensflareTypeClass::Generate_Vertex_Buffers(VertexFormatXYZNDUV2* vertex, int& vertex_count,
                                                 float screen_x_scale, float screen_y_scale,
                                                 float dazzle_intensity,
                                                 const Vector4& transformed_location)
{
    // Lensflares are placed on a line 2D that goes through the lightsource and the center of the
    // screen
    //	float
    ...
    float distance_multiplier = sqrt(transformed_location[0] * transformed_location[0]
...
TextureClass* DazzleTypeClass::Get_Halo_Texture()
{
    if (!secondary_texture) {
        secondary_texture
            = WW3DAssetManager::Get_Instance()->Get_Texture(ic.secondary_texture_name);
        if (secondary_texture) {
...
void DazzleTypeClass::Calculate_Intensities(float& dazzle_intensity, float& dazzle_size,
                                            float& halo_intensity, const Vector3& camera_dir,
                                            const Vector3& dazzle_dir, float distance) const
{
...
void DazzleRenderObjClass::Init_From_INI(const INIClass* ini)
{
    ...
    for (entry = 0; entry < count; entry++) {
        ...
        ini->Get_String(LENSFLARE_LIST_STRING, ini->Get_Entry(LENSFLARE_LIST_STRING, entry), "",
        ...
        for (int flare = 0; flare < lic.flare_count; ++flare) {
            ...
            TPoint3D<float> tp
                = ini->Get_Point(section_name, tmp_name, TPoint3D<float>(1.0f, 1.0f, 1.0f));
            ...
            lic.flare_uv[flare]
                = dini->Get_Vector4(section_name, tmp_name, Vector4(0.0f, 0.0f, 1.0f, 1.0f));
        }
        lic.type = entry;
    ...
    for (entry = 0; entry < count; entry++) {
        char section_name[80];
        ini->Get_String(DAZZLE_LIST_STRING, ini->Get_Entry(DAZZLE_LIST_STRING, entry), "",
                        section_name, sizeof(section_name));
...
void DazzleRenderObjClass::Render(RenderInfoClass& rinfo)
{
    ...
    if (Is_Not_Hidden_At_All() && _dazzle_rendering_enabled
        ...
        if (is_on == false) {
            ...
            params->Calculate_Intensities(dazzle_intensity, dazzle_size, current_halo_intensity,
                                          camera_dir, current_dir, current_distance);
...
DazzleTypeClass* DazzleRenderObjClass::Get_Type_Class(
    unsigned id) // Return dazzle type class pointer, or nullptr if not found
{
    if (id >= type_count) {
        return nullptr;
    }
...
LensflareTypeClass* DazzleRenderObjClass::Get_Lensflare_Class(
    unsigned id) // Return lensflare type class pointer, or nullptr if not found
{
        ...
        case DAZZLEFACTORY_CHUNKID_VARIABLES:
            ...
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK_STRING(cload, DAZZLEFACTORY_VARIABLE_TYPENAME, dazzle_type,
                                            sizeof(dazzle_type));
                }
...
float DazzleVisibilityClass::Compute_Dazzle_Visibility(RenderInfoClass& rinfo,
