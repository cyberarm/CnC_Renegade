...
//			  Therefore treat each of these characters as a word which can be preceeded
//by a line break.
...
void Render2DSentenceClass::Build_Textures(void)
{
    ...
    for (int index = 0; index < PendingSurfaces.Count(); index++) {
        ...
        //
        TextureClass* new_texture = new TextureClass(desc.Width, desc.Width, WW3D_FORMAT_A4R4G4B4,
        ...
        //
        for (int renderer_index = 0; renderer_index < surface_info.Renderers.Count();
             renderer_index++) {
...
const FontCharsClass::CharDataStruct* FontCharsClass::Store_GDI_Char(WCHAR ch)
{
    ...
    ::ExtTextOutW(MemDC, x_pos, 0, ETO_OPAQUE, &rect, &ch, 1, nullptr);
...
void FontCharsClass::Create_GDI_Font(const char* font_name)
{
    ...
    GDIFont = ::CreateFont(font_height, 0, 0, 0, bold, italic, FALSE, FALSE, charset,
    ...
    //
    GDIBitmap = ::CreateDIBSection(screen_dc, (const BITMAPINFO*)&bitmap_info, DIB_RGB_COLORS,
                                   (void**)&GDIBitmapBits, nullptr, 0L);
...
bool FontCharsClass::Is_Font(const char* font_name, int point_size, bool is_bold)
{
    ...
    //
    if ((GDIFontName.Compare_No_Case(font_name) == 0) && (point_size == PointSize)
        && (is_bold == IsBold)) {
        retval = true;
    }
...
void FontCharsClass::Free_Character_Arrays(void)
{
    if (UnicodeCharArray != nullptr) {
        ...
        for (int index = 0; index < count; index++) {
            if (UnicodeCharArray[index] != nullptr) {
    ...
    for (int index = 0; index < 256; index++) {
        if (ASCIICharArray[index] != nullptr) {
