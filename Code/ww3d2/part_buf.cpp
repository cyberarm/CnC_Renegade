...
ParticleBufferClass::ParticleBufferClass(
    ParticleEmitterClass* emitter, unsigned int buffer_size, ParticlePropertyStruct<Vector3>& color,
      ...
      DefaultTailDiffuse(src.DefaultTailDiffuse)
{
    ...
    */
    ...
    case W3D_EMITTER_RENDER_MODE_TRI_PARTICLES: {
        ...
        PointGroup->Set_Point_Mode(PointGroupClass::TRIS);
        PointGroup->Set_Frame_Row_Column_Count_Log2(
            src.PointGroup->Get_Frame_Row_Column_Count_Log2());
    } break;
    case W3D_EMITTER_RENDER_MODE_QUAD_PARTICLES: {
        // Set up worldspace point group
        WWASSERT(src.PointGroup);
...
void ParticleBufferClass::Combine_Color_And_Alpha()
{
    ...
    if (Color || Alpha) {
        ...
        if (Color && Alpha) {
            VectorProcessorClass::Copy(Diffuse->Get_Array(), Color->Get_Array(), Alpha->Get_Array(),
                                       cnt);
        }
        else if (Color) {
            ...
            VectorProcessorClass::Copy(Diffuse->Get_Array(), Vector3(1.0f, 1.0f, 1.0f),
                                       Alpha->Get_Array(), cnt);
        }
...
void ParticleBufferClass::Render_Particles(RenderInfoClass& rinfo)
{
    ...
    PointGroup->Set_Arrays(Position[pingpong], Diffuse, apt, Size, Orientation, Frame,
                           active_point_count);
    Update_Bounding_Box();
...
void ParticleBufferClass::Render_Line_Group(RenderInfoClass& rinfo)
{
    ...
    case SAME_AS_HEAD_ALPHA_ZERO:
        ...
        if (!Diffuse) {
            REF_PTR_RELEASE(TailDiffuse);
            DefaultTailDiffuse.Set(ColorKeyFrameValues[0].X, ColorKeyFrameValues[0].Y,
                                   ColorKeyFrameValues[0].Z, 0);
        }
        else {
    ...
    case SAME_AS_HEAD:
        ...
        if (!Diffuse) {
            REF_PTR_RELEASE(TailDiffuse);
            DefaultTailDiffuse.Set(ColorKeyFrameValues[0].X, ColorKeyFrameValues[0].Y,
                                   ColorKeyFrameValues[0].Z, AlphaKeyFrameValues[0]);
        }
        else {
            // otherwise allocate and copy tail diffuse
            if (!TailDiffuse) {
    ...
    LineGroup->Set_Arrays(Position[pingpong], TailPosition, Diffuse, TailDiffuse, apt, Size, UCoord,
                          active_point_count);
    Update_Bounding_Box();
...
int ParticleBufferClass::Calculate_Cost_Value_Arrays(float screen_area, float* values,
                                                     float* costs) const
{
    unsigned int lod = 0;
    ...
    for (; lod < LodCount; lod++) {
        ...
        // be less than one in some cases (we want to avoid 0 polycounts except for true nullptr LODs)
        float polycount = costs[lod];
        float benefit_factor
            = (polycount > WWMATH_EPSILON) ? (1 - (0.5f / (polycount * polycount))) : 0.0f;
...
void ParticleBufferClass::Reset_Colors(ParticlePropertyStruct<Vector3>& new_props)
{
    ...
    bool color_rand_zero = (fabs(new_props.Rand.X) < eps_byte && fabs(new_props.Rand.Y) < eps_byte
...
void ParticleBufferClass::Reset_Size(ParticlePropertyStruct<float>& new_props)
{
    ...
    if (size_rand_zero && new_props.NumKeyFrames == 0) {
        ...
        float last_size = SizeKeyFrameValues[NumSizeKeyFrames - 1]
...
void ParticleBufferClass::Reset_Rotations(ParticlePropertyStruct<float>& new_props,
                                          float orient_rnd)
{
    ...
    /*
    ** NOTE: Input rotations are in rotations per second. These will be converted to rotations per
    *millisecond.
    */
    ...
    // If both randomizers are effectively zero and rotation is constant zero, then all arrays are
    // nullptr.
    ...
    if (orientation_rand_zero && rotation_rand_zero && new_props.NumKeyFrames == 0
        ...
        for (i = 0; i < NumRotationKeyFrames - 1; i++) {
            HalfRotationKeyFrameDeltas[i] = 0.5f
        ...
        if (rotation_constant_at_end) {
            ...
            // This is OK because if rotation_constant_at_end is false, NumRotationKeyFrames is
            // equal or smaller than new_props.NumKeyFrames so new_props.Values[NumRotationKeyFrames
            // - 1] and new_props.KeyTimes[NumRotationKeyFrames - 1] exist.
            HalfRotationKeyFrameDeltas[i] = 0.5f
                * (new_props.Values[i] * 0.001f - RotationKeyFrameValues[i])
        ...
        for (i = 1; i < NumRotationKeyFrames; i++) {
            ...
            OrientationKeyFrameValues[i] = OrientationKeyFrameValues[i - 1]
                + delta_t
        ...
        if (orientation_rand_zero) {
            ...
            if (RandomOrientationEntries) {
                // Reuse RandomOrientationEntries if the right size, otherwise release and
                // reallocate.
...
void ParticleBufferClass::Reset_Blur_Times(ParticlePropertyStruct<float>& new_blur_times)
{
    ...
    if (blurtime_rand_zero && new_blur_times.NumKeyFrames == 0) {
        ...
        // Reuse BlurTimeKeyFrameValues, BlurTimeKeyFrameTimes and BlurTimeKeyFrameDeltas if the
        ...
        if (blurtime_constant_at_end) {
            ...
            // This is OK because if frame_constant_at_end is false, NumBlurTimeKeyFrames is equal
            // or smaller than new_props.NumKeyFrames so new_props.Values[NumBlurTimeKeyFrames - 1]
            // and new_props.KeyTimes[NumBlurTimeKeyFrames - 1] exist.
...
void ParticleBufferClass::Update_Visual_Particle_State(void)
{
    ...
    for (part = Start; part < sub1_end; part++) {
        ...
        if (color) {
            // We go from older to younger particles, so we go backwards from the last keyframe
            // until age >= keytime. This loop must terminate because the 0th keytime is 0.
        ...
        if (orientation) {
            ...
            float tmp_orient = OrientationKeyFrameValues[rkey]
                + (RotationKeyFrameValues[rkey] + HalfRotationKeyFrameDeltas[rkey] * f_delta_t)
                    * f_delta_t
...
void ParticleBufferClass::Get_New_Particles(void)
{
    ...
    for (; NewParticleQueueCount;) {
        ...
        if (HasAccel) {
            position[NewEnd]
                = new_particle.Position + (new_particle.Velocity + 0.5f * Accel * fp_age) * fp_age;
...
void ParticleBufferClass::Get_Opacity_Key_Frames(ParticlePropertyStruct<float>& opacities) const
{
    ...
    //	Determine if there is a keyframe at the very end of the particle's lifetime
    //
    if ((AlphaKeyFrameDeltas != nullptr) && (AlphaKeyFrameDeltas[NumAlphaKeyFrames - 1] != 0)) {
        real_keyframe_count++;
        create_last_keyframe = true;
    }
...
void ParticleBufferClass::Get_Size_Key_Frames(ParticlePropertyStruct<float>& sizes) const
{
    ...
    //	Determine if there is a keyframe at the very end of the particle's lifetime
    //
    if ((SizeKeyFrameDeltas != nullptr) && (SizeKeyFrameDeltas[NumSizeKeyFrames - 1] != 0)) {
        real_keyframe_count++;
        create_last_keyframe = true;
    }
...
void ParticleBufferClass::Get_Rotation_Key_Frames(ParticlePropertyStruct<float>& rotations) const
{
    ...
    /*
    ** NOTE: Rotations are stored internally in rotations per millisecond. These will be converted
    *to rotations per second.
    */
...
void ParticleBufferClass::Get_Frame_Key_Frames(ParticlePropertyStruct<float>& frames) const
{
    ...
    //	Determine if there is a keyframe at the very end of the particle's lifetime
    //
    if ((FrameKeyFrameDeltas != nullptr) && (FrameKeyFrameDeltas[NumFrameKeyFrames - 1] != 0)) {
        real_keyframe_count++;
        create_last_keyframe = true;
    }
