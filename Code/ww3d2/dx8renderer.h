...
** This class is used for each Material-Texture-Shader combination that is encountered during
*rendering.
** Each polygon_renderer that uses the same 'TextureCategory' will be linked to the
*'TextureCategory' object.
** Then, all polygons will be rendered in 'TextureCategory' batches to reduce the number of stage
*changes
...
public:
    DX8TextureCategoryClass(DX8FVFCategoryContainer* container, TextureClass** textures,
                            ShaderClass shd, VertexMaterialClass* mat, int pass);
    ~DX8TextureCategoryClass();
    ...
    unsigned Add_Mesh(Vertex_Split_Table& split_buffer, unsigned vertex_offset,
    ...
    void Add_Polygon_Renderer(DX8PolygonRendererClass* p_renderer,
...
protected:
    ...
    DX8TextureCategoryClass* Find_Matching_Texture_Category(TextureClass* texture, unsigned pass,
                                                            unsigned stage,
                                                            DX8TextureCategoryClass* ref_category);

    DX8TextureCategoryClass* Find_Matching_Texture_Category(VertexMaterialClass* vmat,
                                                            unsigned pass,
                                                            DX8TextureCategoryClass* ref_category);
...
public:
    ...
    static unsigned Define_FVF(MeshModelClass* mmc, unsigned int* user_lighting,
                               bool enable_lighting);
    bool Is_Sorting() const { return sorting; }

    void Change_Polygon_Renderer_Texture(DX8PolygonRendererList& polygon_renderer_list,
                                         TextureClass* texture, TextureClass* new_texture,
                                         unsigned pass, unsigned stage);

    void Change_Polygon_Renderer_Material(DX8PolygonRendererList& polygon_renderer_list,
...
public:
    ...
    /*
    ** This method adds a material pass which must be rendered after all of the other rendering is
    *complete.
    ** This is needed whenever a mesh turns off its base passes and renders a translucent pass on
    *its geometry.
    */
...
public:
    ...
    /*
    ** Since skins are already rendered after the rigid meshes, the Add_Delayed_Material_Pass
    *function simply
...
{
    MeshRegKeyStruct(void)
        : Model(nullptr),
          UserLighting(nullptr)
    {
    }
    MeshRegKeyStruct(MeshModelClass* mdl, unsigned int* lighting)
        : Model(mdl),
          UserLighting(lighting)
    {
    }
    bool operator==(const MeshRegKeyStruct& that)
    {
...
inline unsigned int
HashTemplateKeyClass<MeshRegKeyStruct>::Get_Hash_Value(const MeshRegKeyStruct& key)
{
...
public:
    ...
    // Enable or disable lighting on all objects inserted from now on. (Doesn't affect the objects
    // that are already in the lists)
    void Enable_Lighting(bool enable) { enable_lighting = enable; }

    // This should be called at the beginning of a game or menu or after a major modifications to
    // the scene...
    void Invalidate();
