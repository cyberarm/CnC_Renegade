 ...
 *   2/13/2002  gth : Created.                                                                 *
 *=============================================================================================*/
void LightClass::Compute_Lighting(const Vector3& pos, const Vector3& norm, Vector3* set_ambient,
                                  Vector3* set_diffuse)
{
    ...
    */
    ...
    if (Type == LightClass::SPOT) {
        ...
        float spot_atten
            = (Vector3::Dot_Product(spot_dir, dir) - SpotAngleCos) / (1.0f - SpotAngleCos);
        atten *= spot_atten;
        atten = WWMath::Clamp(atten, 0.0f, 1.0f);
    }
...
bool LightClass::Load(ChunkLoadClass& cload)
{
    ...
    while (cload.Open_Chunk()) {
        ...
        case LIGHT_CHUNK_W3DFILE:
            ...
            // but the load function expects the external user to have opened it.  So open it
            // here...
            cload.Open_Chunk();
            Load_W3D(cload);
            cload.Close_Chunk();
