...
#define Reverse_LONG(a)                                                                            \
    ((a >> 24) & 0x000000FFL) | ((a >> 8) & 0x0000FF00L) | ((a << 8) & 0x00FF0000L)                \
        | ((a << 24) & 0xFF000000L)
...
//{
//	return(unsigned long)( (unsigned long)( (unsigned long)( (unsigned long)X ) << (int)n ) |
//(unsigned long)( ((unsigned long) X ) >> ( (int)((int)(sizeof(long)*(long)8) - (long)n) ) ) );
// }
    ...
    */
    ...
    for (index = 0; index < PROC_BLOCK_SIZE / sizeof(long); index++) {
        long temp = _rotl(alt.Long[0], 5)
            + Do_Function(index, alt.Long[1], alt.Long[2], alt.Long[3]) + alt.Long[4] + block[index]
            + Get_Constant(index);
