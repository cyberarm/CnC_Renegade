 ...
 *otherwise) * INIClass::Entry_Count -- Fetches the number of entries in a specified section. *
 ...
 *   INIClass::Enumerate_Entries -- Count how many entries begin with a certain prefix followed by a
 *range *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
 ...
 *   9/29/99    EHC : Created.                                                                 *
 *=============================================================================================*/
unsigned INIClass::Enumerate_Entries(const char* Section, const char* Entry_Prefix,
 ...
 *=============================================================================================*/
const WideStringClass& INIClass::Get_Wide_String(WideStringClass& new_string, char const* section,
                                                 char const* entry,
                                                 unsigned short const* defvalue) const
{
...
bool INIClass::Put_Wide_String(char const* section, char const* entry, const unsigned short* string)
{
    ...
    if (len == 0) {
        ...
        BufferStraw straw(string,
                          (len * 2) + 2); // Convert from shorts to bytes, plus 2 for terminator.
        Base64Straw bstraw(Base64Straw::ENCODE);
        bstraw.Get_From(straw);
 ...
 *                                                                                             *
 *          text     -- Pointer to a nullptr terminated text string that holds the block of       *
 ...
 *                      buffer with the last character being forced to nullptr.                   *
 *                                                                                             *
 * OUTPUT:  Returns with the number of characters placed into the buffer. The trailing nullptr    *
 *          is not counted.                                                                    *
 *                                                                                             *
 * WARNINGS:   none                                                                            *
 *                                                                                             *
 ...
 *   07/02/1996 JLB : Created.                                                                 *
 *=============================================================================================*/
int INIClass::Get_String(char const* section, char const* entry, char const* defvalue, char* buffer,
                         int size) const
{
    /*
...
** GetString
*/
const StringClass& INIClass::Get_String(StringClass& new_string, char const* section,
                                        char const* entry, char const* defvalue) const
{
    if (section == nullptr || entry == nullptr) {
        new_string = "";
        return new_string;
...
int INIClass::Get_List_Index(char const* section, char const* entry, int const defvalue,
                             char* list[])
{
    if (section == nullptr || entry == nullptr) {
        return (0);
...
{
    // if we can't find the entry or the entry is nullptr just return the default value
    INIEntry* entryptr = Find_Entry(section, entry);
    if (entryptr == nullptr || entryptr->Value == nullptr) {
        return defvalue;
    }
 ...
 *                                                                                             *
 * INPUT:   buffer   -- Pointer to the nullptr terminate string to be processed.                  *
 *                                                                                             *
 * OUTPUT:  none                                                                               *
 *                                                                                             *
 * WARNINGS:   none                                                                            *
 ...
 *                                                                                             *
 *                                                                                             *
 * INPUT:  pointer to nullptr terminated string                                                   *
 *                                                                                             *
