...
class FastAllocatorGeneral; // Allocates and deletes items of any size. Can use as a fast
                            // replacement for global new/delete.
...
//
//    void Example(int nSize){
//       StackAllocator<int*, 512, 0> stackAllocator; //Create an instance. We use the 'construct'
//       hint feature here. int** pArray = stackAllocator.New(nSize);    //Allocate memory.
//       memset(pArray, 0, nSize*sizeof(int*));       //Do something with the memory.
//       stackAllocator.Delete(pArray);               //In this example, we explicity free the
//       memory.
//    }
//
//    void Example(int nSize){
...
//       pArray = stackAllocator.New(nSize*2);        //Allocate some new memory. We'll let the
//       allocator free it. memset(pArray, 0, nSize*2*sizeof(char));     //Do something with the new
//       memory. stackAllocator.Delete(pArray2);              //Delete the additional memory.
//    }
//
        ...
        if (mnAllocCount == -1) {
            ...
            if (nCount < nStackCount) { // If the request is small enough to come from the stack...
                ...
                if (bConstruct) { // Since this constant, the comparison gets optimized away.
                    ...
                    while (pTArray < pTArrayEnd) {
                        new (pTArray)
                            T; // Use the placement operator new. This simply calls the constructor
...
WWINLINE void FastAllocatorGeneral::Free(void* pAlloc)
{
    if (pAlloc) {
        unsigned int* n
            = ((unsigned int*)pAlloc) - 1; // Subtract four bytes and the count is stored there.
...
WWINLINE void* FastAllocatorGeneral::Realloc(void* pAlloc, unsigned int n)
{
    if (n) {
        ...
        if (pAlloc) {
            n = *(((unsigned int*)pAlloc) - 1); // Subtract four bytes and the count is stored
                                                // there.
...
template <class T> WWINLINE bool operator==(const FastSTLAllocator<T>&, const FastSTLAllocator<T>&)
{
    return true;
}
template <class T> WWINLINE bool operator!=(const FastSTLAllocator<T>&, const FastSTLAllocator<T>&)
