...
public:
    ...
    virtual void BlitForward(void* dest, void const* source, int length) const
    {
        for (int index = 0; index < length; index++) {
            ...
            if (color != 0) {
                *((T*)dest)
                    = (T)((((*(T*)dest) >> 1) & Mask) + ((TranslateTable[color] >> 1) & Mask));
            }
            dest = ((T*)dest) + 1;
        }
    }
...
inline void BlitTrans<unsigned char>::BlitForward(void* dest, void const* source, int len) const
{
    ...
    again : __asm { dec ecx jz fini mov al, [esi] inc edi inc esi test al, al jz again mov[edi],
                    al jmp again } fini :;
}

inline void BlitTransXlat<unsigned short>::BlitForward(void* dest, void const* source,
                                                       int len) const
{
    ...
    again : __asm { dec ecx jz over add edi,  2 mov al,    [esi] inc esi or al, al jz again mov dx,
                    [ebx + eax * 2] mov[edi], dx jmp again } over :;
}

inline void BlitTransRemapXlat<unsigned short>::BlitForward(void* dest, void const* source,
                                                            int len) const
{
    ...
    */
    again : __asm { dec ecx jz over add edi,
                    2 xor eax,
                    eax lodsb or al,
                    al jz again mov al,
                    [ebx + eax] // First remap step (8 bit to 8 bit).
                    mov ax,
                    [edx + eax * 2] // Second remap step (8 bit to 16 bit).
                    mov[edi],
                    ax jmp again } over :;
}

inline void BlitTransZRemapXlat<unsigned short>::BlitForward(void* dest, void const* source,
                                                             int len) const
{
    unsigned short const* translator = TranslateTable;
    unsigned char const* remapper = *RemapTable;
...
inline void BlitPlainXlat<unsigned short>::BlitForward(void* dest, void const* source,
                                                       int len) const
{
    ...
    again :
        ...
        */
        __asm
    {
        xor eax, eax add edi, 2 mov al, [esi] inc esi mov ax, [ebx + eax * 2] mov[edi],
            ax dec ecx jnz again
    }
}
