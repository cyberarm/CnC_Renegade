...
public:
    ...
    {
        return (TPoint2D<T>(X - rvalue.X, Y - rvalue.Y));
    }
    TPoint2D<T> const operator+(TPoint2D<T> const& rvalue) const
    {
        return (TPoint2D<T>(X + rvalue.X, Y + rvalue.Y));
    ...
    {
        return (TPoint2D<T>(Y - rvalue.Y, rvalue.X - X));
    }
...
template <class T> TPoint2D<T> const operator*(T lvalue, TPoint2D<T> const& rvalue)
{
...
public:
    ...
    {
        return (TPoint3D<T>(X - rvalue.X, Y - rvalue.Y, Z - rvalue.Z));
    }
    TPoint3D<T> const operator-(TPoint2D<T> const& rvalue) const
    {
        return (TPoint3D<T>(X - rvalue.X, Y - rvalue.Y, Z));
    }
    TPoint3D<T> const operator+(TPoint3D<T> const& rvalue) const
    {
        return (TPoint3D<T>(X + rvalue.X, Y + rvalue.Y, Z + rvalue.Z));
    ...
    {
        return (TPoint3D<T>(X * rvalue, Y * rvalue, Z * rvalue));
    }
    ...
    {
        return TPoint3D<T>(Y * rvalue.Z - Z * rvalue.Y, Z * rvalue.X - X * rvalue.Z,
...
template <class T> TPoint3D<T> const operator*(T lvalue, TPoint3D<T> const& rvalue)
{
    return (rvalue * lvalue);
}
