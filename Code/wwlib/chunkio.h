...
/*
** WRITE_MICRO_CHUNK	- use this one-line macro to easily make a micro chunk for an individual
*variable.
...
#define READ_MICRO_CHUNK_WWSTRING(cload, id, var)                                                  \
    case (id):                                                                                     \
        cload.Read(var.Get_Buffer(cload.Cur_Micro_Chunk_Length()),                                 \
                   cload.Cur_Micro_Chunk_Length());                                                \
        break;

#define READ_MICRO_CHUNK_WIDESTRING(cload, id, var)                                                \
    case (id):                                                                                     \
        cload.Read(var.Get_Buffer((cload.Cur_Micro_Chunk_Length() + 1) / 2),                       \
                   cload.Cur_Micro_Chunk_Length());                                                \
        break;
