...
**	really isn't all that fast (if an FPU is guaranteed to be present than using "float" might
*be *	more efficient). It doesn't detect overflow or underflow in mathematical or bit-shift
*operations.
**
...
**	fixed point numbers. This allows the result of these operators to generate values with
*greater *	magnitude than is normally possible if the result were coerced into a fixed point
*number. *	If the result should be fixed point, then ensure that both parameters are fixed
*point.
**
**	Note that although integers are used as the parameters in the mathematical operators, this
**	does not imply that negative parameters are supported. The use of integers is as a
*convenience *	to the programmer -- constant integers are presumed signed. If unsigned parameters
...
**
**	Note that when integer values are returns from the arithmetic operators, the value is
*rounded *	to the nearest whole integer value. This differs from normal integer math that
*always rounds down.
*/
class fixed
{
public:
    ...
    */
    //		const fixed operator * (fixed const & rvalue) const {return(fixed(*this) *=
    //rvalue);}
    ...
    /*
    **	The Shift operators are more efficient than using multiplies or divides by power-of-2
    *numbers.
    */
