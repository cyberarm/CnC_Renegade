...
public:
    ...
    // Put current thread sleep for ms milliseconds (can be called from any thread, ThreadClass or
    // other)
    static void Sleep_Ms(unsigned ms = 0);

    // Put current thread in sleep and switch to next one (Useful for balansing the thread switches
    // with game update)
...
protected:
    // User defined thread function. The thread function should check for "running" flag every now
    // and then and exit the thread if running is false.
