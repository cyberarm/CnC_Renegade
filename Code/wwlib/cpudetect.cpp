...
CPUDetectClass::ProcessorManufacturerType CPUDetectClass::ProcessorManufacturer
...
void CPUDetectClass::Init_Cache()
{
    ...
    // This code can only determine cache sizes for cpuid count 1 (see Intel manuals for more info
    // on the subject)
    ...
    case 6:
        ...
        case 5:
            // This one is either PII, PIIXeon or Celeron with no L2 cache!
            {
                ...
                else if (L2CacheSize <= 512 * 1024) { // If Xeon has 512k L2 cache we will assume a
                                                      // PII - tough luck!
        ...
        case 7:
            if (L2CacheSize
                <= 512 * 1024) { // If Xeon has 512k L2 cache we will assume a PIII - tough luck!
...
void CPUDetectClass::Init_Processor_String()
{
    ...
    if (ext_id.Eax & 0x80000000) {
        CPUDetectClass::CPUID((unsigned&)ProcessorString[0], (unsigned&)ProcessorString[4],
                              (unsigned&)ProcessorString[8], (unsigned&)ProcessorString[12],
                              0x80000002);
        CPUDetectClass::CPUID((unsigned&)ProcessorString[16], (unsigned&)ProcessorString[20],
                              (unsigned&)ProcessorString[24], (unsigned&)ProcessorString[28],
                              0x80000003);
        CPUDetectClass::CPUID((unsigned&)ProcessorString[32], (unsigned&)ProcessorString[36],
                              (unsigned&)ProcessorString[40], (unsigned&)ProcessorString[44],
                              0x80000004);
    }
...
void CPUDetectClass::Init_Compact_Log()
{
    ...
    Get_OS_Info(os_info, OSVersionPlatformId, OSVersionNumberMajor, OSVersionNumberMinor,
...
public:
    CPUDetectInitClass::CPUDetectInitClass()
    {
        ...
        // Every processor our games run should have CPUID so it would be extremely unlikely for it
        // not to be present. One can never be sure about the clones though...
        if (CPUDetectClass::Has_CPUID_Instruction()) {
