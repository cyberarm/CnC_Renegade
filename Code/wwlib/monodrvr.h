...
FILE_ATTRIBUTE_NORMAL, nullptr); if (handle != INVALID_HANDLE_VALUE)  { long retval;		//
Return code from IoControl functions. unsigned short * pointer;	// Working pointer to mono RAM.
                union {
                        int X,Y;
                ...
                DeviceIoControl(handle, (DWORD)IOCTL_MONO_LOCK, nullptr, 0, &pointer, sizeof(pointer),
&retval, 0); if (pointer != nullptr) { *pointer = 0x0721;		// '!' character appears in upper
left corner (attribute 0x07).
                }
                DeviceIoControl(handle, (DWORD)IOCTL_MONO_UNLOCK, nullptr, 0, nullptr, 0, &retval, 0);

                // Set cursor to column 5, row 10.
                cursor.X = 5;
                cursor.Y = 10;
                DeviceIoControl(handle, (DWORD)IOCTL_MONO_SET_CURSOR, &cursor, sizeof(cursor), nullptr,
0, &retval, 0);

                // Clear the screen.
                DeviceIoControl(handle, (DWORD)IOCTL_MONO_CLEAR_SCREEN, nullptr, 0, nullptr, 0, &retval,
0);
