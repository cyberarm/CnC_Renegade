...
typedef BOOL(WINAPI* SymLoadModuleType)(HANDLE hProcess, HANDLE hFile, LPSTR ImageName,
...
typedef BOOL(WINAPI* StackWalkType)(DWORD MachineType, HANDLE hProcess, HANDLE hThread,
...
static char const* ImagehelpFunctionNames[] = { "SymCleanup",
                                                "SymGetSymFromAddr",
                                                "SymInitialize",
                                                "SymLoadModule",
                                                "SymSetOptions",
...
void Dump_Exception_Info(EXCEPTION_POINTERS* e_info)
{
    ...
    */
    static char const* _code_txt[] = {
        ...
        "Error code: EXCEPTION_ILLEGAL_INSTRUCTION\r\r\nDescription:	The thread tried to "
        "execute an invalid instruction.",
        "Error code: EXCEPTION_IN_PAGE_ERROR\r\r\nDescription:	The thread tried to access a page "
        "that was not present, and the system was unable to load the page. For example, this "
        "exception might occur if a network connection is lost while running a program over the "
        "network.",
        ...
        "Error code: EXCEPTION_INT_OVERFLOW\r\r\nDescription: The result of an integer operation "
        "caused a carry out of the most significant bit of the result.",
        "Error code: EXCEPTION_INVALID_DISPOSITION\r\r\nDescription: An exception handler returned "
        "an invalid disposition to the exception dispatcher. Programmers using a high-level "
        ...
        "Error code: EXCEPTION_PRIV_INSTRUCTION\r\r\nDescription: The thread tried to execute an "
        ...
        "single-instruction mechanism signaled that one instruction has been executed.",
    ...
    */
    ...
    /*
    ** If this is the first time through then fix up the imagehelp function pointers since
    *imagehlp.dll
    ...
    */
    ...
    if (!IsBadCodePtr((FARPROC)context->Eip)) {
        if (_SymGetSymFromAddr != nullptr
            && _SymGetSymFromAddr(GetCurrentProcess(), context->Eip, &displacement, symptr)) {
            sprintf(scrap, "Exception occurred at %08X - %s + %08X\r\n", context->Eip, symptr->Name,
                    displacement);
        }
        else {
    ...
    */
    for (int thread = 0; thread < ThreadList.Count(); thread++) {
        sprintf(scrap, "  ID: %08X - %s", ThreadList[thread]->ThreadID,
                ThreadList[thread]->ThreadName);
        Add_Txt(scrap);
    ...
    */
    ...
    sprintf(scrap, "CS:%04x  SS:%04x  DS:%04x  ES:%04x  FS:%04x  GS:%04x\r\n", context->SegCs,
            context->SegSs, context->SegDs, context->SegEs, context->SegFs, context->SegGs);
    Add_Txt(scrap);
    ...
    */
    ...
    /*
    ** Dump the bytes at EIP. This will make it easier to match the crash address with later
            ...
            */
            if (IsBadCodePtr((FARPROC)*stackptr)) {
                ...
                if (symbols_available) {
                    ...
                    if (_SymGetSymFromAddr != nullptr
                        && _SymGetSymFromAddr(GetCurrentProcess(), *stackptr, &displacement,
                                              symptr)) {
                        char symbuf[256];
...
int Exception_Handler(int exception_code, EXCEPTION_POINTERS* e_info)
{
    ...
    if (!WindowedMode) {
        ...
        MessageBox(MainWindow, "This demo has timed out. Thank you for playing Red Alert 2.",
                   "Byeee!", MB_ICONEXCLAMATION | MB_OK);
        return (EXCEPTION_EXECUTE_HANDLER);
    }
#endif // DEMO_TIME_OUT

    /*
    ** If we were trying to quit and we got another exception then just shut down the whole shooting
    *match right here.
    */
    if (TryingToExit) {
        ExitProcess(0);
    ...
    */
    ...
    if (ExceptionRecursions == 0) {
        ...
        */
        HANDLE debug_file;
        DWORD actual;
        debug_file = CreateFile("_except.txt", GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS,
...
#ifndef _DEBUG
            /*
            ** Copy the exception debug file to the network. No point in doing this for the debug
            *version
    ...
    */
    ...
    /*
    ** If an exit is required then turn of memory leak reporting (there will be lots of them) and
    *use
...
bool Lookup_Symbol(void* code_ptr, char* symbol, int& displacement)
{
    ...
    */
    if (_SymGetSymFromAddr(GetCurrentProcess(), (unsigned long)code_ptr,
                           (unsigned long*)&displacement, symbol_struct_ptr)) {
...
here:
    ...
    */
    for (int i = 0; i < num_addresses + 1; i++) {
        if (_StackWalk(IMAGE_FILE_MACHINE_I386, GetCurrentProcess(), GetCurrentThread(),
                       &stack_frame, nullptr, nullptr, _SymFunctionTableAccess, _SymGetModuleBase,
                       nullptr)) {
