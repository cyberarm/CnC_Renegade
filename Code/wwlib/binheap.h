...
public:
    ...
    // This is pure virtual so that any type of key can be used as long as it uses the comparison
    // operators.
...
public:
    ...
    // Move the element up in the tree if necessary.  Use this if the key value becomes smaller when
    ...
    HeapNodeClass<Key_Type>* Remove_Min(void)
    {
        ...
        for (unsigned int i = 1; (i * 2) <= Number_Of_Elements; i = child) {
            ...
            child = i * 2;
            if ((child != Number_Of_Elements)
                && (Less_Than(Elements[child + 1], Elements[child]))) {
                child++;
            }
