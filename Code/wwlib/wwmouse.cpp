...
void WWMouseClass::Draw_Mouse(Surface* surface, bool issidebarsurface)
{
    ...
    if (!Is_Hidden() && surface != NULL && surface != SurfacePtr && savesurface != NULL) {
        ...
        *savedregion = SavedRegion;
        ...
        savesurface->Blit_From(Rect(0, 0, savedregion->Width, savedregion->Height), *surface,
                               *savedregion);
        if (issidebarsurface) {
            if (savedregion->X < 0 && -savedregion->X < Background->Get_Width()) {
                Background->Blit_From(
                    Rect(-savedregion->X, 0, savedregion->Width, savedregion->Height), *surface,
                    Rect(0, savedregion->Y, savedregion->Width, savedregion->Height));
            }
        }
        else {
            Background->Blit_From(Rect(0, 0, savedregion->Width, savedregion->Height), *surface,
                                  *savedregion);
        }
...
void WWMouseClass::Erase_Mouse(Surface* surface, bool issidebarsurface)
{
    if (!Is_Hidden() && surface != NULL && surface != SurfacePtr && Alternate != NULL
        ...
        */
        ...
        // Rect old = surface->Window.Get_Rect();
        // surface->Window.Reset();
        surface->Blit_From(savedregion, *savesurface,
                           Rect(0, 0, savedregion.Width, savedregion.Height));
        // surface->Window.Set(old);
        Unblock_Mouse();
    }
}
...
void WWMouseClass::Raw_Draw_Mouse(Surface* surface, int xoffset, int yoffset)
{
    if (Is_Data_Valid() && surface != NULL) {
        ...
        */
        ...
        if (surface == SurfacePtr) {
            Blit_Block(*surface, *NormalDrawer, mdata, Rect(0, 0, rect.Width, rect.Height),
                       Point2D(rect.X, rect.Y), ConfiningRect);
        }
        else {
            Blit_Block(*surface, *NormalDrawer, mdata, Rect(0, 0, rect.Width, rect.Height),
                       Point2D(rect.X, rect.Y), surface->Get_Rect());
        }
