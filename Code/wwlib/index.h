...
**	identifier number to the type objects that it is indexing. The regular binary sort and
*search *	function are used for speedy index retrieval. Typical use of this would be to index
*pointers to *	normal data objects, but it can be used to hold the data objects themselves. Keep in
*mind that *	the data object "T" is copied around by this routine in the internal tables so not
*only must *	it have a valid copy constructor, it must also be efficient. The internal algorithm
*will *	create an arbitrary number of default constructed objects of type "T". Make sure it has a
**	default constructor that is efficient and trivial. The constructor need not perform any
*actual *	initialization since this class has prior knowledge about the legality of these
*temporary *	objects and doesn't use them until after the copy constructor is used to initialize
*them. *	This means that the default constructed object of type "T" can be technically in an
*unusable *	state since it won't ever be used by this routine and won't ever be returned by any
*of *	this template's methods.
**
...
private:
    ...
    */
    struct NodeElement
    {
        ...
        NodeElement(INDEX const& id, T& data)
            : ID(id),
              Data(data)
        {
        }
 ...
 *   11/02/1996 JLB : Created.                                                                 *
 *=============================================================================================*/
template <class INDEX, class T> IndexClass<INDEX, T>::~IndexClass(void)
{
    Clear();
}
 ...
 *   11/02/1996 JLB : Created.                                                                 *
 *=============================================================================================*/
template <class INDEX, class T> int IndexClass<INDEX, T>::Count(void) const
{
    return (IndexCount);
}
...
template <class INDEX, class T> T const& IndexClass<INDEX, T>::operator[](INDEX const& id) const
{
    if (Is_Present(id)) {
        ...
        /*
        **	Count on the fact that the archive pointer is always valid after a call to
        *Is_Present *	that returns "true".
        */
        return (Archive->Data);
    }
    static T x;
 ...
 *   11/02/1996 JLB : Created.                                                                 *
 *=============================================================================================*/
template <class INDEX, class T> void IndexClass<INDEX, T>::Invalidate_Archive(void) const
{
    Archive = 0;
}
...
#else
IndexClass<INDEX, T>::NodeElement const*
IndexClass<INDEX, T>::Search_For_Node(INDEX const& id) const
#endif
{
    /*
