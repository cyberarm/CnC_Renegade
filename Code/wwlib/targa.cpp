...
long Targa::Save(const char* name, long flags, bool addextension)
{
    ...
    if (Open(name, TGA_WRITEMODE) == nullptr) {
        ...
        if (!error && (flags & TGAF_IMAGE) && (mImage != nullptr)) {
            ...
            if ((Header.ImageType == TGA_TRUECOLOR)
            ...
            // Bug fix (IML) : If the image was inverted, invert it again to restore it to its prior
            // state.
            if (imageinverted) {
                InvertImage();
            }
        ...
        if (addextension) {
            if (!error) {
                ...
                if (!error
                    && (File_Write(&mExtension, sizeof(TGA2Extension)) != sizeof(TGA2Extension))) {
                    error = TGAERR_WRITE;
                }
            }
        }
