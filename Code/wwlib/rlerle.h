...
#if defined(_MSC_VER)
void RLEBlitTransZRemapXlat<unsigned short>::Blit(void* dest, void const* source, int len,
                                                  int leadskip) const
{
    unsigned char const* remapper = *RemapTable;
    ...
    */
    moreskip : __asm { test edx, edx jle nomoreskip dec edx lodsb test al,
                       al jnz moreskip lodsb sub edx, eax inc edx jmp moreskip } nomoreskip :
        ...
        **	pixel skip process.
        */
        __asm { neg edx sub ecx,
                edx // Account for any left over transparent pixels
                    lea edi,
                [edi + edx * 2] mov edx, [transtable] }

        /*
        **	Output the pixel data to the destination.
        */
        moredata : __asm { xor eax,
                           eax or ecx,
                           ecx jle fini lodsb test al,
                           al jz transparent mov al,
                           [ebx + eax] mov ax,
                           [edx + eax * 2] dec ecx stosw jmp moredata }

                   /*
                   **	A transparent pixel run just causes the destination pointer
                   **	and length count to be adjusted by the length of the run.
                   */
                   transparent : __asm { lodsb lea edi, [edi + eax * 2] sub ecx, eax jmp moredata }

                                 fini :;
}

void RLEBlitTransRemapXlat<unsigned short>::Blit(void* dest, void const* source, int len,
                                                 int leadskip) const
{
    unsigned char const* remapper = RemapTable;
    unsigned short const* transtable = TranslateTable;
...
void RLEBlitTransXlat<unsigned short>::Blit(void* dest, void const* source, int len,
                                            int leadskip) const
{
    unsigned short const* transtable = TranslateTable;
    ...
    */
    moreskip : __asm { test edx, edx jle nomoreskip dec edx lodsb test al,
        ...
        **	Output the pixel data to the destination.
        */
        moredata : __asm { xor eax, eax or ecx, ecx jle fini lodsb test al,
                           al jz transparent mov ax, [ebx + eax * 2] dec ecx stosw jmp moredata }
