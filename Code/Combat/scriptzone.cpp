        ...
        case CHUNKID_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_IS_ENVIRONMENT_ZONE,
                                     IsEnvironmentZone);
...
** ScriptZoneGameObj
*/
SimplePersistFactoryClass<ScriptZoneGameObj, CHUNKID_GAME_OBJECT_SCRIPT_ZONE>
    _ScriptZoneGameObjPersistFactory;
    ...
    if (Get_Definition().CheckStarsOnly) {
        ...
        for (it.First(); !it.Is_Done(); it.Next()) {
            if (it.Peek_Obj()->Get_Observer() != nullptr) {
                PhysicalGameObj* gameobj = ((CombatPhysObserverClass*)it.Peek_Obj()->Get_Observer())
                                               ->As_PhysicalGameObj();
                if (gameobj != nullptr) {
...
**
*/
ScriptZoneGameObj* ScriptZoneGameObj::Find_Closest_Zone(const Vector3& pos,
                                                        ZoneConstants::ZoneType type)
{
