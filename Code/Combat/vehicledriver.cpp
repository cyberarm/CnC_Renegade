...
//
///////////////////////////////////////////////////////////////////////////
static float Find_Tangent_Angle(const Vector2& center, float radius, const Vector2& point,
                                bool clockwise)
{
    float angle = 0;
...
PhysicsSceneClass::Get_Instance ()->Add_Debug_AABox (AABoxClass (tm.Get_Translation (), Vector3
(0.5F,0.5F,0.5F)), Vector3 (1, 0, 1));*/
    ...
    if (is_complete == false) {
        ...
        /*if ((turn_radius * turn_radius) > current_dest.Length2 () && WWMath::Fabs (turn_angle) >
        DEG_TO_RADF (60.0F)) { m_IsBackingUp = true;
        }*/
...
//
////////////////////////////////////////////////////////////////
float VehicleDriverClass::Calculate_Brake(float dist_to_goal, float expected_velocity,
                                          float curr_speed, float max_speed)
{
    ...
    if (m_GameObj->Peek_Physical_Object()->As_VTOLVehicleClass() == nullptr) {
        ...
        if (m_BrakingDist > 0) {
            ...
            float percent
                = WWMath::Clamp(dist_to_goal / (m_BrakingDist - m_ArrivedDist), -1.0F, 1.0F);
        ...
        if (m_BrakingDist > 0) {
            float percent
                = WWMath::Clamp(dist_to_goal / (m_BrakingDist - m_ArrivedDist), -1.0F, 1.0F);
            float end_velocity = WWMath::Clamp(
                (-(curr_speed / max_speed) * 5.0F) + (0.75F / max_speed), -1.0F, 1.0F);
            brake_velocity = ((1.0F - percent) * end_velocity) + (percent * expected_velocity);
        }
    }
...
void VehicleDriverClass::Apply_Controls(float forward_accel, float left_accel)
{
    if (!WWMath::Is_Valid_Float(forward_accel)) {
        ...
        WWDEBUG_SAY(("VehicleDriverClass::Apply_Controls - BAD FLOAT forward_access = %f - Model "
                     "name = %s\n",
                     forward_accel, name));
#endif // WWDEBUG
        forward_accel = 0.0f;
