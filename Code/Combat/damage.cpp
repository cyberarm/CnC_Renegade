    ...
    if (armorINI != nullptr) {
        ...
        for (entry = 0; entry < count; entry++) {
            ...
            armorINI->Get_String(SECTION_ARMOR_TYPES,
                                 armorINI->Get_Entry(SECTION_ARMOR_TYPES, entry), "", entry_name,
                                 sizeof(entry_name));
        ...
        for (entry = 0; entry < count; entry++) {
            ...
            armorINI->Get_String(SECTION_WARHEAD_TYPES,
                                 armorINI->Get_Entry(SECTION_WARHEAD_TYPES, entry), "", entry_name,
                                 sizeof(entry_name));
        ...
        for (entry = 0; entry < count; entry++) {
            char entry_name[80];
            armorINI->Get_String(SECTION_SOFT_ARMOR, armorINI->Get_Entry(SECTION_SOFT_ARMOR, entry),
        ...
        for (warhead_num = 0; warhead_num < WarheadNames.Count(); warhead_num++) {
            ...
            armorINI->Get_String(special_damage_type, SECTION_SPECIAL_DAMAGE_TYPE,
                                 WarheadNames[warhead_num]);
            ...
            SpecialDamageProbability[warhead_num] = armorINI->Get_Float(
                SECTION_SPECIAL_DAMAGE_PROBABILITY, WarheadNames[warhead_num], 0.0f);

            VisceroidProbability[warhead_num] = armorINI->Get_Float(
                SECTION_VISCEROID_PROBABILITY, WarheadNames[warhead_num], 0.0f);

            //			Debug_Say(( "Warhead %s %d %f\n", WarheadNames[warhead_num],
            //				SpecialDamageTypes[warhead_num],
        ...
        for (i = SPECIAL_DAMAGE_TYPE_FIRE; i < NUM_SPECIAL_DAMAGE_TYPES; i++) {
            ...
            SpecialDamageWarhead[i]
                = Get_Warhead_Type(armorINI->Get_String(temp_string, section, ENTRY_WARHEAD));
        ...
        //
        SpecialDamageWarhead[SPECIAL_DAMAGE_TYPE_SUPER_FIRE]
            = SpecialDamageWarhead[SPECIAL_DAMAGE_TYPE_FIRE];
        SpecialDamageDuration[SPECIAL_DAMAGE_TYPE_SUPER_FIRE]
            = SpecialDamageDuration[SPECIAL_DAMAGE_TYPE_FIRE];
        SpecialDamageScale[SPECIAL_DAMAGE_TYPE_SUPER_FIRE]
            = SpecialDamageScale[SPECIAL_DAMAGE_TYPE_FIRE];
        SpecialDamageExplosion[SPECIAL_DAMAGE_TYPE_SUPER_FIRE]
            = SpecialDamageExplosion[SPECIAL_DAMAGE_TYPE_FIRE];
        ...
        for (int damage = SPECIAL_DAMAGE_TYPE_FIRE; damage < NUM_SPECIAL_DAMAGE_TYPES; damage++) {
            ...
            for (entry = 0; entry < count; entry++) {
                StringClass entry_name = armorINI->Get_String(temp_string, section,
                                                              armorINI->Get_Entry(section, entry));
...
ArmorWarheadManager::SpecialDamageType
ArmorWarheadManager::Get_Special_Damage_Type(WarheadType type)
{
    WWASSERT((int)type >= 0);
    WWASSERT((int)type < Get_Num_Warhead_Types());
        ...
        case CHUNKID_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_SAFE_MICRO_CHUNK(cload, MICROCHUNKID_SHIELD_STRENGTH, ShieldStrength,
                                          float);
                    READ_SAFE_MICRO_CHUNK(cload, MICROCHUNKID_SHIELD_STRENGTH_MAX,
                                          ShieldStrengthMax, float);
...
**
*/
float DefenseObjectClass::Apply_Damage(const OffenseObjectClass& offense, float scale,
                                       int alternate_skin)
{
#if 1
...
**
*/
float DefenseObjectClass::Do_Damage(const OffenseObjectClass& offense, float scale,
                                    int alternate_skin)
{
    ...
    if (alternate_skin != -1) {
        damage_scale
            = ArmorWarheadManager::Get_Damage_Multiplier(alternate_skin, offense.Get_Warhead());
        shield_damage_scale = 0;
    }
    else {
        damage_scale = ArmorWarheadManager::Get_Damage_Multiplier(Skin, offense.Get_Warhead());
        shield_damage_scale
            = ArmorWarheadManager::Get_Damage_Multiplier(ShieldType, offense.Get_Warhead());
    }
...
#define PUNISH_DELAY 60
    ...
    if ((damage * damage_scale < 0) || (damage * shield_damage_scale < 0)) { // We are repairing
        ...
        if ((float)ShieldStrength > 0.0f && alternate_skin == -1) {
            shield_damage = damage
                * ArmorWarheadManager::Get_Shield_Absorbsion(ShieldType, offense.Get_Warhead());
            ...
            if (shield_damage_scale != 0) {
                ...
                shield_damage_to_apply
                    /= shield_damage_scale; // how much scaled damage did we try to apply
            }
        ...
        // (gth) added alternate_skin feature which is used by buildings when their MCT is being
        // damaged.
        damage *= damage_scale;
    ...
    if (COMBAT_STAR != nullptr && // dedicated server test
        ...
        DIAG_LOG(("DRCV", "%s; %d; %1.2f; %1.2f; %1.2f; %1.2f; %1.2f; %1.2f", weapon_name,
                  hitter_id, points, armor, Health, pos.X, pos.Y, pos.Z));
    }
    ...
    if ((smart == COMBAT_STAR) && (smart != nullptr)) {
        ...
        float armor = Get_Shield_Strength();
        DIAG_LOG(("DEFC",
                  "%1.2f; %1.2f; %1.2f; %s; %d; %d; %1.2f; %1.2f; %1.2f; %1.2f; %1.2f; %1.2f; %s ",
    ...
    if (damage > 0 && (float)Health <= 0) {
        ...
        DIAG_LOG(("OBDE", "%d; %1.2f; %1.2f; %1.2f; %d; %1.2f; %1.2f; %1.2f; %s", victim_id,
                  victim_pos.X, victim_pos.Y, victim_pos.Z, killer_id, killer_pos.X, killer_pos.Y,
                  killer_pos.Z, weapon_name));
        if (Get_Owner() == COMBAT_STAR) {
            DIAG_LOG(("STDE", "%1.2f; %1.2f; %1.2f; %d; %1.2f; %1.2f; %1.2f; %s", victim_pos.X,
                      victim_pos.Y, victim_pos.Z, killer_id, killer_pos.X, killer_pos.Y,
                      killer_pos.Z, weapon_name));
        }
    }

    if (damage > 0 && (float)Health <= 0 && smart != nullptr && smart->As_SoldierGameObj() != nullptr
        && Get_Owner() != nullptr && Get_Owner()->As_SoldierGameObj() != nullptr) {

        CombatManager::On_Soldier_Kill(smart->As_SoldierGameObj(),
    ...
    if (smart != nullptr && smart->Get_Player_Data()) {
        ...
        if (offense.Get_Owner() != Get_Owner()) {
            ...
            smart->Get_Player_Data()->Apply_Damage_Points(
                points_dir * (damage + shield_damage) * points, Get_Owner());
        }

        if (damage > 0 && (float)Health <= 0) {

            if (offense.Get_Owner() != Get_Owner()) {
                smart->Get_Player_Data()->Apply_Death_Points(points_dir * (float)DeathPoints,
                                                             Get_Owner());
            }
        }
    }
...
bool DefenseObjectDefClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_SAFE_MICRO_CHUNK(csave, DEFENSEOBJECTDEF_VARIABLE_SHIELDSTRENGTHMAX, ShieldStrengthMax,
                           float);
    int shield_save_id = ArmorWarheadManager::Get_Armor_Save_ID(ShieldType);
        ...
        case DEFENSEOBJECTDEF_CHUNK_VARIABLES:
            ...
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_SAFE_MICRO_CHUNK(cload, DEFENSEOBJECTDEF_VARIABLE_HEALTHMAX, HealthMax,
                                          float);
                    READ_MICRO_CHUNK(cload, DEFENSEOBJECTDEF_VARIABLE_SKIN, skin_save_id);
                    READ_SAFE_MICRO_CHUNK(cload, DEFENSEOBJECTDEF_VARIABLE_SHIELDSTRENGTH,
                                          ShieldStrength, float);
                    READ_SAFE_MICRO_CHUNK(cload, DEFENSEOBJECTDEF_VARIABLE_SHIELDSTRENGTHMAX,
                                          ShieldStrengthMax, float);
                    READ_MICRO_CHUNK(cload, DEFENSEOBJECTDEF_VARIABLE_SHIELDTYPE, shield_save_id);
                    READ_SAFE_MICRO_CHUNK(cload, DEFENSEOBJECTDEF_VARIABLE_DAMAGE_POINTS,
                                          DamagePoints, float);
                    READ_SAFE_MICRO_CHUNK(cload, DEFENSEOBJECTDEF_VARIABLE_DEATH_POINTS,
                                          DeathPoints, float);
                }
                cload.Close_Micro_Chunk();
            }
