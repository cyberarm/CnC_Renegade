...
CollisionReactionType C4GameObj::Collision_Occurred(const CollisionEventClass& event)
{
    ...
    if (!Stuck) {
        ...
        if (event.OtherObj->Get_Observer() != nullptr) {
            ...
            building
                = ((CombatPhysObserverClass*)event.OtherObj->Get_Observer())->As_BuildingGameObj();
        }
        ...
        if (other != nullptr && Get_Owner() != nullptr) {
            ...
            //			if ( !other->Is_Teammate( Get_Owner() ) )
            {
                ...
                Vector3 my_pos;
                Get_Position(&my_pos);
                Matrix3D::Inverse_Transform_Vector(parent_model->Get_Bone_Transform(StuckBone),
                                                   my_pos, &StuckOffset);
            ...
            if (event.OtherObj->As_StaticAnimPhysClass() != nullptr) {
                ...
                if (StuckStaticAnimObj->Peek_Model()) {
                    StuckBone = StuckStaticAnimObj->Peek_Model()->Get_Sub_Object_Bone_Index(
                        event.CollidedRenderObj);
                    Vector3 my_pos;
                    Get_Position(&my_pos);
                    Matrix3D::Inverse_Transform_Vector(
                        StuckStaticAnimObj->Peek_Model()->Get_Bone_Transform(StuckBone), my_pos,
                        &StuckOffset);
                }
            }
            ...
            if (event.CollisionResult != nullptr
                && SurfaceEffectsManager::Is_Surface_Permeable(
        ...
        case CHUNKID_VARIABLES: {
            ...
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_STUCK_STATIC_ANIM_OBJ_ID,
                                     static_anim_obj_id);
...
void C4GameObj::Detonate(void)
{
    if (CombatManager::I_Am_Server()) {
        ...
        if (AmmoDefinition && AmmoDefinition->ExplosionDefID) {
            ...
            ExplosionManager::Server_Explode(AmmoDefinition->ExplosionDefID,
                                             Get_Transform().Get_Translation(), owner_id,
                                             force_victim);
        }
        ...
        if (Stuck) {
            if (StuckObject.Get_Ptr() != nullptr) {
                ...
                if (building) {
                    ExplosionManager::Explosion_Damage_Building(AmmoDefinition->ExplosionDefID,
                                                                building, StuckMCT, Get_Owner());
                }
            }
        }
    }
...
void C4GameObj::Export_Rare(BitStreamClass& packet)
{
    ...
    if (Stuck) {
        ...
        if (StuckToObject) {
            ...
            packet.Add(StuckOffset.X,
                       BITPACK_VEHICLE_VELOCITY); // offset, using velocity packing...
...
void C4GameObj::Import_Rare(BitStreamClass& packet)
{
    ...
    if (Stuck) {
        ...
        if (stuck_static_anim) {
            ...
            if (static_anim_obj_id != 0xFFFFFFFF) {
                StaticPhysClass* pobj = PhysicsSceneClass::Get_Instance()->Get_Static_Object_By_ID(
                    static_anim_obj_id);
                if (pobj && (pobj->As_StaticAnimPhysClass() != nullptr)) {
                    REF_PTR_SET(StuckStaticAnimObj, (StaticAnimPhysClass*)pobj);
                }
...
void C4GameObj::Defuse(void)
{
    if (CombatManager::I_Am_Server()) {
        ...
        if (disarmed_ammo && disarmed_ammo->ExplosionDefID) {
            ...
            ExplosionManager::Server_Explode(disarmed_ammo->ExplosionDefID,
                                             Get_Transform().Get_Translation(), owner_id);
        }
    }
...
{
    if (Get_Owner() == nullptr && OwnerBackup != nullptr && AmmoDefinition
    ...
    for (objnode = GameObjManager::Get_Game_Obj_List()->Head(); objnode;
        ...
        if (phys) {
            ...
            if (c4 && c4->Get_Player_Type() == player_type && c4->AmmoDefinition
                && (int)c4->AmmoDefinition->AmmoType
                    != (int)AmmoDefinitionClass::AMMO_TYPE_C4_TIMED) {
                count++;
