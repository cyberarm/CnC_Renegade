      ...
      DefaultPlayerType(PLAYERTYPE_NEUTRAL)
{
    ...
    FILENAME_PARAM(DamageableGameObjDef, InfoIconTextureFilename, "InfoIconTextureFilename",
                   ".TGA");
...
bool DamageableGameObjDef::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_INFO_ICON_TEXTURE_FILENAME,
                               InfoIconTextureFilename);
        ...
        case CHUNKID_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_INFO_ICON_TEXTURE_FILENAME,
                                              InfoIconTextureFilename);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_DEFAULT_PLAYER_TYPE,
                                     DefaultPlayerType);
...
void DamageableGameObj::Re_Init(const DamageableGameObjDef& definition)
{
    ...
    // float shield_percent	= DefenseObject.Get_Shield_Strength() /
    // DefenseObject.Get_Shield_Strength_Max();
    ...
    // DefenseObject.Set_Shield_Strength (DefenseObject.Get_Shield_Strength_Max () *
    // shield_percent);
        ...
        case CHUNKID_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_IS_HEALTH_BAR_DISPLAYED,
                                     IsHealthBarDisplayed);
                default:
                    Debug_Say(("Unhandled MicroChunk:%d File:%s Line:%d\r\n",
                               cload.Cur_Micro_Chunk_ID(), __FILE__, __LINE__));
                    break;
...
void DamageableGameObj::Apply_Damage(const OffenseObjectClass& damager, float scale,
                                     int alternate_skin)
{
