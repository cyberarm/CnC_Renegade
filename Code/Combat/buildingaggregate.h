...
protected:
    ...
    **		is played to the new state's Frame0.  If the power state changes, we pop to the same
    *relative
    **    position in the alternate power state.
    ** ANIM_LOGIC_LOOP - In this mode, each state has a pair of frames which it loops between.
    *Whenever *		the state changes, we pop to the same relative position in the new loop.  In
    *addition, certain *		states can have animation disabled; this will cause the
    *object to freeze on the appropriate frame *		right after the state change.
    */
    enum
    {
        ANIM_LOGIC_LINEAR = 0,
    ...
    /*
    ** Animation controls for each state.  In LOOP mode, we use Frame0 and Frame1 to define a loop.
    *In
