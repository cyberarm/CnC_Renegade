...
SoldierGameObj* GameObjManager::Find_Soldier_Of_Client_ID(int client_id)
{
    {
        WWPROFILE("FSOC id");
        for (SLNode<SmartGameObj>* objnode = Get_Smart_Game_Obj_List()->Head(); objnode;
             objnode = objnode->Next()) {
    ...
    for (objnode = GameObjList.Head(); objnode; objnode = objnode->Next()) {
        ...
        if (scriptable) {
            ...
            if (script_zone && script_zone->Is_Environment_Zone()) {
                if (CollisionMath::Overlap_Test(script_zone->Get_Bounding_Box(), pos)
...
physical game obj! if ( CombatManager::I_Am_Server() ) { CombatManager::Object_Destroyed(obj);
                        }
