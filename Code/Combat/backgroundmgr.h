...
public:
    ...
    void Configure(const Vector3& orientation, float length, float radius, const Vector3& color0,
...
{
public:
    CloudLayerClass(float maxdistance, const char* texturename, const Vector2& velocity,
                    float tilefactor, bool rotate);
    ~CloudLayerClass();
...
{
public:
    LightningClass(float extent, float startdistance, float enddistance, float heading,
                   float distribution);
    ~LightningClass();
...
{
public:
    WarBlitzClass(float extent, float startdistance, float enddistance, float heading,
                  float distribution);
    ~WarBlitzClass();
...
protected:
    ...
    Vector3 Interpolate_Color(const unsigned char colortable[][3], unsigned colorcount,
                              float interpolant);
    float Interpolate_Scalar(const unsigned char scalartable[], unsigned scalarcount,
                             float interpolant);
    unsigned Lightning_Delay();
    unsigned War_Blitz_Delay();

    float Extent; // Radius of sphere (and half cube width) which contains all sky objects (for
                  // clipping purposes).
    Vector3 Color;
...
public:
    ...
    static bool Set_Lightning(float intensity, float startdistance, float enddistance,
                              float heading, float distribution, float ramptime = 0.0f);
    static bool Override_Lightning(float intensity, float startdistance, float enddistance,
                                   float heading, float distribution, float ramptime = 0.0f);
    static void Get_Lightning(float& intensity, float& startdistance, float& enddistance,
    ...
    static bool Set_War_Blitz(float intensity, float startdistance, float enddistance,
                              float heading, float distribution, float ramptime = 0.0f,
                              bool override = false);
    static void Get_War_Blitz(float& intensity, float& startdistance, float& enddistance,
                              float& heading, float& distribution);
...
#define VARID_PARAMETER(varname)                                                                   \
    VARID_##varname##_CURRENT_VALUE, VARID_##varname##_NORMAL_VALUE,                               \
        VARID_##varname##_NORMAL_TARGET, VARID_##varname##_NORMAL_DURATION,                        \
        VARID_##varname##_OVERRIDE_TARGET, VARID_##varname##_OVERRIDE_DURATION
...
#undef VARID_PARAMETER
    ...
    static bool Set_Lightning(float intensity, float startdistance, float enddistance,
                              float heading, float distribution, float ramptime, bool override);
