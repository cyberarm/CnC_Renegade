...
**
*/
SimplePersistFactoryClass<SoldierObserverClass, CHUNKID_SOLDIER_OBSERVER>
    _SoldierObserverPersistFactory;
        ...
        case CHUNKID_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_TAKE_COVER_PROBABILITY,
                                     TakeCoverProbability);
...
void SoldierObserverClass::Action_Complete(GameObject* obj, int action_id,
                                           ActionCompleteReason complete_reason)
{
    if (action_id == INNATE_ACTION_ID) {
        SubStateString += "\nComplete";
        //	ActionTimer = MIN( ActionTimer, 10 );		// When he completes, only stay in
        //the action state for 5 more seconds
...
**
*/
bool SoldierObserverClass::Set_State(SoldierGameObj* soldier, int state, const Vector3& location,
                                     GameObject* enemy)
{
    ...
    if ((old_state == SOLDIER_AI_RELAXED_IDLE) || (old_state == SOLDIER_AI_ALERT_IDLE)) {
        ...
        if (State == SOLDIER_AI_FOOTSTEPS_HEARD || State == SOLDIER_AI_BULLET_HEARD
...
void SoldierObserverClass::State_Act_Idle(SoldierGameObj* soldier)
{
    ...
    if (FreeRandom.Get_Float() < 0.5f) { // 50% chance of facing a random direction
        ...
        PathfindClass* pathfind
            = PathfindClass::Get_Instance(); //	Lookup a safe random position to walk to
    ...
    if (dive) {
        ...
        if (!done) {
            if (FreeRandom.Get_Float() < TakeCoverProbability || IsStationary) {
                ...
                PathfindClass* pathfind = PathfindClass::Get_Instance();
                if (pathfind->Find_Random_Spot(AlertPosition, ALERTED_RUN_DISTANCE,
                                               &walk_position)) {
                    Action_Goto_Location(soldier, walk_position, AI_STATE_SEARCH, ALERTED_RUN_SPEED,
                                         ALERTED_RUN_RANGE, true);
                }
            }
        }
    }
    ...
    if (!IsStationary) {
        ...
        if (!done) {
            ...
            if (pathfind->Find_Random_Spot(AlertPosition, ALERTED_RUN_DISTANCE, &position)) {
                Action_Goto_Location_Facing(soldier, position, AI_STATE_SEARCH, AlertPosition,
                                            ALERTED_RUN_SPEED, ALERTED_RUN_RANGE);
            }
            else {
...
#define WALK_SPEED 0.2f
    ...
    if (CoverPosition != NULL) {
        if (CoveredAttack == true) {
            ...
            Action_Attack_Object(soldier, enemy, weapon_range, CoverPosition->Get_Crouch(),
                                 cover_position, 0.5f);
            CoveredAttack = false;
    ...
    if (!done && (FreeRandom.Get_Float() < TakeCoverProbability)
        && !IsStationary) { // 25% chance of not finding cover
...
void SoldierObserverClass::Action_Goto_Location_Facing(SoldierGameObj* soldier,
                                                       const Vector3& location,
                                                       SoldierAIState ai_state,
...
void SoldierObserverClass::Action_Attack_Object(SoldierGameObj* soldier, PhysicalGameObj* enemy,
                                                float range, bool kneel,
...
void SoldierObserverClass::Stay_Within_Home(SoldierGameObj* soldier, Vector3* location,
                                            float* distance)
{
                                      ...
                                      const Vector3& face_pos)
{
    ...
    if (FreeRandom.Get_Float() < TakeCoverProbability) {
        ...
        if (cover != NULL) { // Yes, take it
            ...
            if (force_face) {
                Action_Goto_Location_Facing(soldier, cover_position, AI_STATE_SEARCH, face_pos,
                                            ALERTED_RUN_SPEED, 0.5, cover->Get_Crouch());
            }
            else {
                Action_Goto_Location(soldier, cover_position, AI_STATE_SEARCH, ALERTED_RUN_SPEED,
                                     0.5, cover->Get_Crouch());
            }
