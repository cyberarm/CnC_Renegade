...
void RadarManager::Init()
{
    ...
    if (HUDGlobalSettingsDef::Get_Instance() != nullptr) {
        BlipColors[BLIP_COLOR_TYPE_NOD]
            = HUDGlobalSettingsDef::Get_Instance()->Get_Nod_Color().Convert_To_ARGB();
        BlipColors[BLIP_COLOR_TYPE_GDI]
            = HUDGlobalSettingsDef::Get_Instance()->Get_GDI_Color().Convert_To_ARGB();
        BlipColors[BLIP_COLOR_TYPE_NEUTRAL]
            = HUDGlobalSettingsDef::Get_Instance()->Get_Neutral_Color().Convert_To_ARGB();
        BlipColors[BLIP_COLOR_TYPE_MUTANT]
            = HUDGlobalSettingsDef::Get_Instance()->Get_Mutant_Color().Convert_To_ARGB();
        BlipColors[BLIP_COLOR_TYPE_RENEGADE]
            = HUDGlobalSettingsDef::Get_Instance()->Get_Renegade_Color().Convert_To_ARGB();
        ...
        BlipColors[BLIP_COLOR_TYPE_SECONDARY_OBJECTIVE] = HUDGlobalSettingsDef::Get_Instance()
                                                              ->Get_Secondary_Objective_Color()
                                                              .Convert_To_ARGB();
        BlipColors[BLIP_COLOR_TYPE_TERTIARY_OBJECTIVE] = HUDGlobalSettingsDef::Get_Instance()
                                                             ->Get_Tertiary_Objective_Color()
                                                             .Convert_To_ARGB();
    }
    ...
    // Don't reset the markers here, Some may have already been added when loading the level
    // (Created)
...
float RadarManager::Add_Blip(const Vector3& pos, int shape_type, int color_type, float intensity,
                             bool bracket, bool altitude_fade)
{
...
void RadarManager::Update(const Matrix3D& player_tm, const Vector2& center)
{
    ...
    Renderer->Add_Quad(draw.Lower_Left(), draw.Lower_Right(), draw.Upper_Left(), draw.Upper_Right(),
                       uv, RadarColor);
    draw += center + RADAR_RINGS_R_OFFSET - draw.Upper_Left();
    Renderer->Add_Quad_Backfaced(draw.Lower_Right(), draw.Lower_Left(), draw.Upper_Right(),
                                 draw.Upper_Left(), uv, RadarColor);
...
//	compass += Render2DClass::Get_Screen_Resolution().Lower_Left() + settings->RadarCompassOffset;
    ...
    // If the renderer object for this particular radar direction hasn't been created, create it
    // now...
    if (!CompassRenderers[CurrentCompassRendererIndex]) {
        ...
        CompassRenderers[CurrentCompassRendererIndex]->Build_Sentence(
            TRANSLATE(dir[CurrentCompassRendererIndex]));
        Vector2 text_size = CompassRenderers[CurrentCompassRendererIndex]->Get_Text_Extents(
            TRANSLATE(dir[CurrentCompassRendererIndex]));
...

    {
        ...
        for (objnode = GameObjManager::Get_Game_Obj_List()->Head(); objnode;
            ...
            if (obj) {
                ...
                if (obj->Peek_Physical_Object() == nullptr) {
                    continue;
                }

                // Don't draw dead soldiers
                if (obj->As_SoldierGameObj() && obj->Get_Defense_Object()
                ...
                //			if ( (WWMath::Fabs( z_diff ) < RADAR_Z_RANGE) ||
                //(obj->Get_Radar_Blip_Shape_Type() == BLIP_SHAPE_TYPE_OBJECTIVE) ) {
                float intensity = obj->Get_Radar_Blip_Intensity();
                bool bracket = obj == BracketObj;
                intensity
                    = Add_Blip(objpos, obj->Get_Radar_Blip_Shape_Type(),
                               obj->Get_Radar_Blip_Color_Type(), intensity, bracket, altitude_fade);
#if 0
...

    {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (objective->DrawBlip && objective->Status == ObjectiveManager::STATUS_IS_PENDING) {
                ...
                intensity = Add_Blip(objective->Position, BLIP_SHAPE_TYPE_OBJECTIVE,
                                     objective->Radar_Blip_Color_Type(), intensity, false);
#if 0
			intensity -= RadarSweepMove / DEG_TO_RAD( 360 ) * 0.666f;				// Drop Blip intensity
#endif
...

    {
        ...
        for (i = 0; i < Markers.Count(); i++) {
            ...
            intensity = Add_Blip(Markers[i].Position, Markers[i].Type, Markers[i].Color, intensity,
                                 false);
#if 0
		intensity -= RadarSweepMove / DEG_TO_RAD( 360 ) * 0.666f;				// Drop Blip intensity
#endif
