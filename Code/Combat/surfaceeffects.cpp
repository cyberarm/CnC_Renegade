...
const float _MAX_SURFACE_EFFECT_DISTANCE2
    = 50.0f * 50.0f; // no surface effects created farther than this from camera
const float _SURFACE_EFFECT_NEAR_DISTANCE2
    = 10.0f * 10.0f; // force surface effects within this distance
...
static const char* _HitterTypeName[SurfaceEffectsManager::NUM_HITTER_TYPES] = {
    "None", // HITTER_TYPE_NONE,							// used to
            // shut off persistant effects
    ...
    "Eject Casing", // HITTER_TYPE_EJECT_CASING,				// casings ejected
                    // from weapons (all are assumed to make the same sounds)
    "Shell Shotgun", // HITTER_TYPE_SHELL_SHOTGUN,

    "Tire Rolling", // HITTER_TYPE_TIRE_ROLLING,				// wheels on
                    // humvees, etc...
...
SurfaceEffectClass* SurfaceEffectsDatabase[SURFACE_TYPE_MAX]
                                          [SurfaceEffectsManager::NUM_HITTER_TYPES];
...
void SurfaceEffectsManager::Init(void)
{
    ...
    if (ini != NULL) {
        ...
        for (surface = 0; surface < SURFACE_TYPE_MAX; surface++) {
            SurfaceStopsBullets[surface]
                = ini->Get_Bool(SECTION_BULLET_STOPPING, SURFACE_TYPE_STRINGS[surface], true);
        }
        ...
        for (surface = 0; surface < SURFACE_TYPE_MAX; surface++) {
            ...
            SurfaceDamageRate[surface]
                = ini->Get_Float(SECTION_DAMAGE_RATE, SURFACE_TYPE_STRINGS[surface], 0);

            if (SurfaceDamageRate[surface] != 0) {
                //				Debug_Say(( "Surface %s Rate %f Warhead %s\n",
                //SURFACE_TYPE_STRINGS[surface], 					SurfaceDamageRate[surface],
                //ArmorWarheadManager::Get_Warhead_Name( SurfaceDamageWarhead[surface] ) ));
            }
        }
        ...
        for (surface = 0; surface < SURFACE_TYPE_MAX; surface++) {
            for (int hitter = 0; hitter < NUM_HITTER_TYPES; hitter++) {
                ...
                section_name.Format("%s_%s", SURFACE_TYPE_STRINGS[surface],
                                    _HitterTypeName[hitter]);
                ...
                surface_effect->DecalSizeRandom
                    = ini->Get_Float(section_name, "DecalSizeRandom", 0);
                ...
                if (HitterPhysicsTypes[hitter] != -1) {
                    ...
                    float friction = ini->Get_Float(section_name, "Friction",
                                                    PhysicsConstants::DefaultContactFriction);

                    float drag = ini->Get_Float(section_name, "Drag",
        ...
        Debug_Say(
            ("SurfaceEffectsManager::Init - Unable to load %s\n", SURFACE_EFFECTS_INI_FILENAME));
    }
...
void SurfaceEffectsManager::Apply_Effect(int surface_type, int hitter_type, const Matrix3D& tm,
                                         PhysClass* hit_obj, PhysicalGameObj* creator,
                                         bool allow_decals, bool allow_emitters)
{
    ...
    // Return if we don't find any surface effect settings for this combination of surface_type,
    // hitter_type
    ...
    if (SurfaceEffectsManagerDebug) {
        Debug_Say(("Applying Surface Effect for %s and %s\n--Sound:%s Emitter:%s Decal:%s\n",
                   ...
                   sound_name ? sound_name : "none", emitter_name ? emitter_name : "none",
    ...
    if (sound_name) {
        ...
        WWAudioClass::Get_Instance()->Create_Instant_Sound(sound_name, tm, owner_ref, 0,
                                                           CLASSID_PSEUDO3D);
    ...
    if ((dist2 > _SURFACE_EFFECT_NEAR_DISTANCE2)) {
        if ((dist2 > _MAX_SURFACE_EFFECT_DISTANCE2)
            || (CollisionMath::Overlap_Test(COMBAT_CAMERA->Get_Frustum(), effect_point)
...
#else
        ParticleEmitterClass* emitter
            = (ParticleEmitterClass*)WW3DAssetManager::Get_Instance()->Create_Render_Obj(
                emitter_name);
        if (emitter) {
            SET_REF_OWNER(emitter);
    ...
    if ((allow_decals) && (decal_name)) {
        ...
        size += FreeRandom.Get_Float(-surface_effect->DecalSizeRandom,
                                     surface_effect->DecalSizeRandom);
        ...
        if ((surface_type == SURFACE_TYPE_GLASS)
            || (surface_type == SURFACE_TYPE_GLASS_PERMEABLE)) {
            PhysicsSceneClass::Get_Instance()->Create_Decal(tm, new_name, size, false, true,
                                                            hit_obj);
        }
        else {
...
void SurfaceEffectsManager::Update_Persistant_Sound(PersistantSurfaceSoundClass* effect,
                                                    int surface_type, int hitter_type,
                                                    const Matrix3D& tm)
{
    ...
    if ((surface_type != effect->CurrentSurfaceType)
        || (hitter_type != effect->CurrentHitterType)) {
...
void SurfaceEffectsManager::Apply_Damage(int surface_type, PhysicalGameObj* obj)
{
    ...
    if (SurfaceDamageRate[surface_type] > 0) {
        OffenseObjectClass offense_obj(SurfaceDamageRate[surface_type],
                                       SurfaceDamageWarhead[surface_type]);
