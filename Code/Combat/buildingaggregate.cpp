...
SimplePersistFactoryClass<BuildingAggregateClass, PHYSICS_CHUNKID_BUILDINGAGGREGATE>
    _BuildingAggregatePersistFactory;
    ...
    if ((new_state != CurrentState) || (force_update == true)) {
        ...
        case BuildingAggregateDefClass::ANIM_LOGIC_LOOP: {
            ...
            */
            anim_mgr.Set_Current_Frame(def->Frame0[new_state]
                                       + normalized_frame
                                           * (def->Frame1[new_state] - def->Frame0[new_state]));
        ...
        case BuildingAggregateDefClass::ANIM_LOGIC_LINEAR: {
            ...
            */
            ...
            /*
            ** If we changed power states or animation is disabled, then we need to warp the current
            *frame
            */
            if ((BuildingStateClass::Is_Power_On(new_state)
                 != BuildingStateClass::Is_Power_On(CurrentState))
...
*/
DECLARE_DEFINITION_FACTORY(BuildingAggregateDefClass, CLASSID_BUILDINGAGGREGATEDEF,
                           "BuildingAggregate")
      ...
      IsMCT(false)
{
    ...
    for (i = 0; i < BuildingStateClass::STATE_COUNT; i++) {
        ...
        NAMED_EDITABLE_PARAM(BuildingAggregateDefClass, ParameterClass::TYPE_INT, Frame0[i],
                             "Frame0");
        NAMED_EDITABLE_PARAM(BuildingAggregateDefClass, ParameterClass::TYPE_INT, Frame1[i],
                             "Frame1");
        NAMED_EDITABLE_PARAM(BuildingAggregateDefClass, ParameterClass::TYPE_BOOL,
...
bool BuildingAggregateDefClass::Load(ChunkLoadClass& cload)
{
    while (cload.Open_Chunk()) {
        ...
        case BAGDEF_CHUNK_DESTROYED_POWEROFF_VARIABLES:
            Load_State_Animation_Data(
                cload, cload.Cur_Chunk_ID() - BAGDEF_CHUNK_HEALTH100_POWERON_VARIABLES);
            break;
...
bool BuildingAggregateDefClass::Load_State_Animation_Data(ChunkLoadClass& cload, int state_index)
{
    ...
    while (cload.Open_Micro_Chunk()) {
        switch (cload.Cur_Micro_Chunk_ID()) {
            ...
            READ_MICRO_CHUNK(cload, BAGDEF_VARIABLE_ANIMATIONENABLED,
                             AnimationEnabled[state_index]);
        default:
            WWDEBUG_SAY(("Unrecognized BuildingAggregateDef Variable chunkID\n"));
            break;
        }
