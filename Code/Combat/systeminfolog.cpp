...
// #define	COMBAT_SUB_KEY_NAME_DEBUG
// "Software\\Westwood\\Renegade\\Debug"
...
void SystemInfoLog::Get_Final_String(StringClass& string)
{
    ...
    vnum.Format("%d.%2.2d", DebugManager::Get_Version_Number() >> 16,
                DebugManager::Get_Version_Number() & 0xffff);
    ptime.Format("%2.2d:%2.2d", TotalPlayingTime / 60, TotalPlayingTime % 60);
    avgfps.Format("%d.%1.1d", AvgFPS / 10, AvgFPS % 10);
    string.Format("%5s %16s %6s %6d %6d %6s %8d ", vnum, CurrentLevel, ptime,
                  MinFPS != 1000 ? MinFPS : 0, MaxFPS, avgfps, CurrentLoadingTime);
}
...
void SystemInfoLog::Get_Log(StringClass& string)
{
    ...
    if (registry.Is_Valid()) {
        string.Format("Ten latest levels played:\r\n"
                      ...
                      "#", "Build", "Level", "Length", "MinFPS", "MaxFPS", "AvgFPS", "LoadTime",
                      "Status");
...
void PlayerInfoLog::Append_To_Log(PlayerDataClass* data)
{
    ...
    DWORD written;
    HANDLE file = CreateFile("history.txt", GENERIC_WRITE, 0, nullptr, OPEN_ALWAYS,
...
void PlayerInfoLog::Get_Compact_Log(StringClass& string)
{
    ...
    string.Format("%s\t%d.%d\t%d.%d\t", CurrentMapName, team0size / 10, team0size % 10,
                  team1size / 10, team1size % 10);
