        ...
        case CHUNKID_VARIABLES: {
            ...
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_IS_SECONDARY_TRIGGERED,
                    ...
                    READ_SAFE_MICRO_CHUNK(cload, MICROCHUNKID_INVENTORY_ROUNDS, InventoryRounds,
                                          int);
...
void WeaponClass::Add_Rounds(int num)
{
    ...
    if (CheatMgrClass::Get_Instance() != nullptr
        && CheatMgrClass::Get_Instance()->Is_Cheat_Enabled(CheatMgrClass::CHEAT_INFINITE_AMMO)
        && Owner == COMBAT_STAR) {
...
{
    return (PrimaryAmmoDefinition != nullptr) ? PrimaryAmmoDefinition->Range : 0;
}
...
bool WeaponClass::Fire_Beacon(const AmmoDefinitionClass* ammo_def)
{
    ...
    if (CombatManager::I_Am_Server()) {
        ...
        //	Create the beacon
        //
        BeaconGameObj* beacon
            = (BeaconGameObj*)ObjectLibraryManager::Create_Object(ammo_def->BeaconDefID);
        if (beacon != nullptr) {
...
void WeaponClass::Do_Fire(bool primary)
{
    ...
    // Stats
    if (Get_Owner() && Get_Owner()->As_SoldierGameObj()
...
void WeaponClass::Fire_Bullet(const AmmoDefinitionClass* ammo_def, bool primary)
{
    ...
    if (Get_Owner() == COMBAT_STAR) {
        ...
        DIAG_LOG(("WFRD", "%1.2f; %1.2f; %1.2f; %s; %d; %d", pos.X, pos.Y, pos.Z,
                  Get_Definition()->Get_Name(), !primary, Get_Total_Rounds()));
    }
    ...
    if (ammo_def->IsTracking) {
        ...
        // (gth) this modification makes bikes able to target anything even though their muzzle
        // doesn't rotate...
        ...
        if (raytest.CollidedPhysObj != nullptr && raytest.CollidedPhysObj->Get_Observer() != nullptr) {
            target_obj = ((CombatPhysObserverClass*)raytest.CollidedPhysObj->Get_Observer())
...
#define SPRAY_CHEAT 0
        ...
        // Create a bullet
        BulletManager::Create_Bullet(ammo_def, position, velocity, bullet_owner, bump_time,
                                     bullet_target, target_obj);
    }
...
bool WeaponClass::Is_Muzzle_Clear()
{
    ...
    if (Get_Owner()->As_SoldierGameObj() != nullptr) {
        ...
        // Estimate the distance we need to sweep back along the muzzle in order to be inside the
        // vehicle.
        Vector3 owner_pos;
...
void WeaponClass::Do_Firing_Effects(void)
{
    ...
    if ((Get_Owner() != COMBAT_STAR) || !CombatManager::Is_First_Person()) {
        ...
        if (Definition->MuzzleFlashPhysDefID != 0) {
            ...
            DefinitionClass* def
                = DefinitionMgrClass::Find_Definition(Definition->MuzzleFlashPhysDefID);
            if (def != nullptr) {
                ...
                if (muzzle_flash) {
                    ...
                    if (model != nullptr) {
                        //						muzzle.Rotate_X(
                        //FreeRandom.Get_Float( DEG_TO_RAD( 360 ) ) );
    ...
    if (FiringSound != nullptr) {
        ...
        if (sound_id != 0) {
            ...
            FiringSound
                = WWAudioClass::Get_Instance()->Create_Sound(sound_id, owner_ref, 0, classid_hint);
...
void WeaponClass::Do_Continuous_Effects(bool enable)
{
    ...
    if (enable) {
        ...
        for (i = 0; i < ContinuousEmitters.Length(); i++) {
            if (ContinuousEmitters[i] == nullptr && !ammo_def->ContinuousEmitterName.Is_Empty()) {
                RenderObjClass* renderobj
                    = Create_Render_Obj_From_Filename(ammo_def->ContinuousEmitterName);
                if (renderobj) {
        ...
        if (ContinuousSound == nullptr && ammo_def->ContinuousSoundDefID != 0) {
            ContinuousSound = WWAudioClass::Get_Instance()->Create_Continuous_Sound(
                ammo_def->ContinuousSoundDefID);
            if (ContinuousSound != nullptr) {
...
void WeaponClass::Update_State(float pending_time)
{
    ...
    while (pending_time > 0.0f) {
        ...
        if (Get_Style() == WEAPON_HOLD_STYLE_BEACON && Get_Owner()
        ...
        if ((State == STATE_READY) || (State == STATE_IDLE)) {
            if ((IsPrimaryTriggered || IsSecondaryTriggered) && (BurstCount != 0)
                && CombatManager::Is_Gameplay_Permitted() && trigger_ok) {
                if (SafetySet) {
                    LockTriggers = true;
                }
                else {
                    if (Is_Loaded()) {
                        ...
                        if (Definition->EmptySoundDefID != 0) {
                            ...
                            if (EmptySoundTimer <= 0) {
                                ...
                                RefCountedGameObjReference* owner_ref
                                    = new RefCountedGameObjReference;
                                owner_ref->Set_Ptr(Get_Owner());

                                AudibleSoundClass* sound
                                    = WWAudioClass::Get_Instance()->Create_Sound(
                                        Definition->EmptySoundDefID, owner_ref);
                                if (sound != nullptr) {
...
void WeaponClass::Force_Reload(void)
{
    ...
    if (Is_Reload_OK() && State <= STATE_READY) {
        ...
        if (Definition->ReloadSoundDefID != 0) {
            ...
            AudibleSoundClass* sound = WWAudioClass::Get_Instance()->Create_Sound(
                Definition->ReloadSoundDefID, owner_ref);
            if (sound != nullptr) {
                sound->Set_Transform(muzzle);
                sound->Attach_To_Object(Model);
                sound->Add_To_Scene(true);
...
void WeaponClass::Update(void)
{
    ...
    if (Get_Style() == WEAPON_HOLD_STYLE_C4 || Get_Style() == WEAPON_HOLD_STYLE_BEACON
        || Get_Can_Snipe()) {
        IsSecondaryTriggered = false;
    }
    ...
    if ((int)PrimaryAmmoDefinition->BurstMax == 0) { // if not using bursts
        ...
        if ((!IsPrimaryTriggered && !IsSecondaryTriggered) || (BurstDelayTimer <= 0.0f)) {
            BurstDelayTimer
                = PrimaryAmmoDefinition->BurstDelayTime * FreeRandom.Get_Float(0.50f, 1.0f);
    ...
    if ((IsPrimaryTriggered == false && LastFrameIsPrimaryTriggered == true)
        ...
        if (State == STATE_START_SWITCH || State == STATE_END_SWITCH || State == STATE_RELOAD
            || Get_Clip_Rounds() == 0) {
            emitters_on = false;
        }
...
PhysicalGameObj* WeaponClass::Cast_Weapon(const Vector3& target)
{
    ...
    //	Debug_Say(( "Weapon Focus %f %f %f\n", bullet_hit_position->X, bullet_hit_position->Y,
    //bullet_hit_position->Z ));

    PhysicalGameObj* hit_obj = nullptr;
    if (raytest.CollidedPhysObj) {
        if (raytest.CollidedPhysObj->Get_Observer() != nullptr) {
            hit_obj = ((CombatPhysObserverClass*)raytest.CollidedPhysObj->Get_Observer())
                          ->As_PhysicalGameObj();
        }
    }
...
void WeaponClass::Display_Targeting(void)
{
    ...
    if (raytest.CollidedPhysObj && raytest.CollidedPhysObj->Get_Observer() != nullptr) {
        DamageableGameObj* obj = ((CombatPhysObserverClass*)raytest.CollidedPhysObj->Get_Observer())
                                     ->As_DamageableGameObj();
...
const Matrix3D& WeaponClass::Get_Muzzle(int index)
{
    ...
    if (Get_Owner() == COMBAT_STAR && CombatManager::Is_First_Person()) {
        ...
        _muzzle.Obj_Look_At(COMBAT_CAMERA->Get_Transform().Get_Translation(),
                            Get_Owner()->Get_Targeting_Pos(), 0);
        return _muzzle;
    }
