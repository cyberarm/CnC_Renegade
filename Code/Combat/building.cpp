...
**
************************************************************************************************/
SimplePersistFactoryClass<BuildingGameObjDef, CHUNKID_GAME_OBJECT_DEF_BUILDING>
    _BuildingGameObjDefPersistFactory;

DECLARE_DEFINITION_FACTORY(BuildingGameObjDef, CLASSID_GAME_OBJECT_DEF_BUILDING,
                           "<Generic Building>")
      ...
      NodDestroyReportID(0)
{
    ...
    for (skin_type_counter = 0; skin_type_counter < ArmorWarheadManager::Get_Num_Armor_Types();
         skin_type_counter++) {
        mct_skin_param->Add_Value(ArmorWarheadManager::Get_Armor_Name(skin_type_counter),
                                  skin_type_counter);
    }
...
//
/////////////////////////////////////////////////////////////////////////////
uint32 BuildingGameObjDef::Get_Class_ID(void) const
{
...
//
/////////////////////////////////////////////////////////////////////////////
const PersistFactoryClass& BuildingGameObjDef::Get_Factory(void) const
{
...
**
************************************************************************************************/
SimplePersistFactoryClass<BuildingGameObj, CHUNKID_GAME_OBJECT_BUILDING>
    _BuildingGameObjPersistFactory;
...
//
/////////////////////////////////////////////////////////////////////////////
const PersistFactoryClass& BuildingGameObj::Get_Factory(void) const
{
...
bool BuildingGameObj::Load(ChunkLoadClass& cload)
{
    ...
    //
    if (CollectionSphere.Center.X == 0 && CollectionSphere.Center.Y == 0
        && CollectionSphere.Center.Z == 0) {
...
void BuildingGameObj::Apply_Damage(const OffenseObjectClass& damager, float scale,
                                   int alternate_skin)
{
    /*
...
void BuildingGameObj::Apply_Damage_Building(const OffenseObjectClass& offense,
...
void BuildingGameObj::Set_Normalized_Health(float health)
{
    ...
    WWDEBUG_SAY(("  prefix = %s mesh count = %d aggregate count = %d light count = %d\r\n",
                 Get_Definition().MeshPrefix, InteriorMeshes.Count() + ExteriorMeshes.Count(),
                 Aggregates.Count(), PowerOnLights.Count() + PowerOffLights.Count()));
}
    ...
    if ((new_state != CurrentState) || (force_update == true)) {
        ...
        */
        Enable_Alternate_Materials(
            InteriorMeshes, ((health_state == BuildingStateClass::HEALTH_0) || (!IsPowerOn)));
        ...
        */
        ...
        disable &= (!PowerOffLights.Is_Empty()); // for lights, power is not disabled if there are
                                                 // not power-off lights
        ...
        disable = !disable;
        for (light_iterator.First(&PowerOffLights); !light_iterator.Is_Done();
             light_iterator.Next()) {
            light_iterator.Peek_Obj()->Set_Disabled(disable);
            light_bounds.Add_Box(light_iterator.Peek_Obj()->Peek_Model()->Get_Bounding_Box());
        }
...
//
/////////////////////////////////////////////////////////////////////////////
void BuildingGameObj::Enable_Alternate_Materials(RefMultiListClass<StaticPhysClass>& models,
                                                 bool onoff)
{
...
void BuildingGameObj::Collect_Building_Components(void)
{
    ...
    //
    RefPhysListIterator staticobj_iterator
    ...
    // the appropriate building controller
    //
    RefPhysListIterator staticlight_iterator
        = PhysicsSceneClass::Get_Instance()->Get_Static_Light_Iterator();
    for (staticlight_iterator.First(); !staticlight_iterator.Is_Done();
    ...
    if (model->Class_ID() == RenderObjClass::CLASSID_MESH) {
        ...
        if (mesh_model != NULL) {
            ...
            for (index = 0; index < poly_count; index++) {
                ...
                //
                Vector3 poly_center
                    = (vert_array[vert1] + vert_array[vert2] + vert_array[vert3]) * 0.33F;
