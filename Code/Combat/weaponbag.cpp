...
**
*/
WeaponClass* WeaponBagClass::Add_Weapon(const WeaponDefinitionClass* def, int rounds,
                                        bool give_weapon)
{
    if (def == NULL) {
...
bool WeaponBagClass::Move_Contents(WeaponBagClass* source)
{
    ...
    for (int i = 1; i < source->WeaponList.Count(); i++) {
        ...
        if (my_weapon) {
            ...
            // else, give it to me
            Add_Weapon(weapon->Get_Definition(), weapon->Get_Total_Rounds(),
                       weapon->Does_Weapon_Exist());
...
void WeaponBagClass::Store_Inventory(InventoryClass* inventory)
{
    for (int i = 1; i < WeaponList.Count(); i++) {
        ...
        inventory->Add_Weapon(weapon->Get_ID(), weapon->Get_Total_Rounds(),
                              weapon->Does_Weapon_Exist());
    }
}
