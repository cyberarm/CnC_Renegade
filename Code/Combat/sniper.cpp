...
void SniperHUDClass::Build_Base(void)
{
    ...
    _Sniper2DBaseRenderer->Add_Line(Vector2(screen_center.X, 0),
                                    Vector2(screen_center.X, top_line_1.Y), 1, blackuv);

    // Draw the top line 2
    Vector2 top_line_2 = TOP_LINE_2;
    top_line_2.Scale(screen_scale);
    _Sniper2DBaseRenderer->Add_Line(Vector2(screen_center.X, box.Top),
                                    Vector2(screen_center.X, top_line_2.Y), 1, blackuv);

    // Draw the top line 3
    Vector2 top_line_3 = TOP_LINE_3;
    top_line_3.Scale(screen_scale);
    _Sniper2DBaseRenderer->Add_Line(Vector2(screen_center.X, box.Top),
                                    Vector2(screen_center.X, top_line_3.Y), 3, blackuv);

    // Draw the bottom line
    Vector2 bottom_line_1 = BOTTOM_LINE_1;
    bottom_line_1.Scale(screen_scale.Y, screen_scale.Y);
    _Sniper2DBaseRenderer->Add_Line(Vector2(screen_center.X, bottom_line_1.X),
                                    Vector2(screen_center.X, bottom_line_1.Y), 1, blackuv);
    ...
    _Sniper2DBaseRenderer->Add_Line(Vector2(screen_center.X - bottom_line_2.X, bottom_line_2.Y),
                                    Vector2(screen_center.X + bottom_line_2.X, bottom_line_2.Y), 1,
                                    blackuv);
    ...
    for (y = screen_center.Y; y < LEFT_LINE_UL.Y * screen_scale.Y;
         y += LEFT_LINE_LR.Y * screen_scale.Y) {
    ...
    // Draw the right line
    _Sniper2DBaseRenderer->Add_Line(Vector2(box.Right, screen_center.Y),
                                    Vector2(screen_size.X, screen_center.Y), 1, blackuv);
    ...
    _Sniper2DBaseRenderer->Add_Line(Vector2(center_line.X, screen_center.Y),
                                    Vector2(center_line.Y, screen_center.Y), 1, blackuv);
    ...
    center_line += Vector2(screen_center.X, screen_center.X);
    _Sniper2DBaseRenderer->Add_Line(Vector2(center_line.X, screen_center.Y),
                                    Vector2(center_line.Y, screen_center.Y), 3, blackuv);
    center_line -= Vector2(screen_center.X, screen_center.X);
    center_line *= -1;
    center_line += Vector2(screen_center.X, screen_center.X);
    _Sniper2DBaseRenderer->Add_Line(Vector2(center_line.X, screen_center.Y),
    ...
    // Draw the left line
    _Sniper2DBaseRenderer->Add_Line(Vector2(0, screen_center.Y),
                                    Vector2(LEFT_LINE_2.X * screen_scale.X, screen_center.Y), 1,
                                    blackuv);
}
