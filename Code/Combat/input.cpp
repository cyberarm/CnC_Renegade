...
void Input::Update_Sliders(void)
{
    ...
    //
    float joystick_x
        = (float)DirectInput::Get_Joystick_Axis_State(DirectInput::JOYSTICK_X_AXIS) / 1000.0f;
    float joystick_y
        = (float)DirectInput::Get_Joystick_Axis_State(DirectInput::JOYSTICK_Y_AXIS) / 1000.0f;
    ...
    if (WWMath::Fabs(joystick_x) < DEAD_ZONE) {
        ...
        joystick_x = WWMath::Sign(joystick_x) * (WWMath::Fabs(joystick_x) - DEAD_ZONE) * 1.5f
            / (1.0f - DEAD_ZONE);
    }
...
void Input::Update(void)
{
    ...
    if (COMBAT_CAMERA && COMBAT_CAMERA->Is_In_Cinematic()
        && !DebugManager::Allow_Cinematic_Keys()) {
        return;
    }
    ...
    if (MenuMode == false || ConsoleMode) {
        ...
        FunctionValue[INPUT_FUNCTION_VEHICLE_TURN_RIGHT] = max(
            FunctionValue[INPUT_FUNCTION_MOVE_RIGHT], FunctionValue[INPUT_FUNCTION_TURN_RIGHT]);
...
// Ignore the clamp
        ...
        if ((input & 0xFF) >= DIK_1 && (input & 0xFF) <= DIK_0) {
            ...
            //			modifier |= ((DirectInput::Get_Button_Value(DIK_SHIFT) &
            //BUTTON_BIT_HELD) ? BUTTON_SHIFT : 0);
            modifier
                |= ((DirectInput::Get_Button_Value(DIK_CONTROL) & BUTTON_BIT_HELD) ? BUTTON_CTRL
                                                                                   : 0);
            modifier
                |= ((DirectInput::Get_Button_Value(DIK_ALT) & BUTTON_BIT_HELD) ? BUTTON_ALT : 0);
        }
...
**
*/
int Input::Get_Secondary_Key_For_Function(int function_id)
{
...
void Input::Load_Misc_Settings(INIClass* input_ini)
{
    DamageIndicatorsEnabled
        = input_ini->Get_Bool(SECTION_MISC_SETTINGS, ENTRY_DAMAGE_INDICATORS, true);
...
void Input::Save_Misc_Settings(INIClass* input_ini)
{
    ...
    input_ini->Put_Bool(SECTION_MISC_SETTINGS, ENTRY_TARGET_STEERING,
...
**
*/
int Input::Find_First_Function_By_Primary_Key(int key_id)
{
...
**
*/
int Input::Find_First_Function_By_Secondary_Key(int key_id)
{
