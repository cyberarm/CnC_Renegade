...
void WeaponViewClass::Think()
{
    ...
    if (weapon) {
        Get_Render_Obj_Name_From_Filename(new_weapon_model_name,
    ...
    if (HandsPhysObj != nullptr && WeaponModel != nullptr) {
        ...
        if (is_current_complete && WeaponState == WEAPON_STATE_FIRE
            && new_weapon_state == WEAPON_STATE_IDLE) {
            // if the anims exist...
            ...
            if (is_current_complete) {
                ...
                if (WeaponState == WEAPON_STATE_FIRE && ForceFireLoop) {
                    ...
                    if (hands_model != nullptr) {
                        //						hands_model->Set_Animation(
                        //HandsAnims[ WeaponState ], 0, RenderObjClass::ANIM_MODE_ONCE );
                        HandAnimControl.Set_Animation(HandsAnims[WeaponState]);
                        HandAnimControl.Set_Mode(ANIM_MODE_ONCE, 0);
                        HandAnimControl.Update(0);
                    }
                    //					WeaponModel->Set_Animation( WeaponAnims[
                    //WeaponState ], 0, RenderObjClass::ANIM_MODE_ONCE );
        ...
        if (COMBAT_STAR != nullptr) {
            ...
            Matrix3D::Inverse_Transform_Vector(obj_convention_camera, target_pos,
                                               &camera_space_target);
            Matrix3D::Inverse_Transform_Vector(obj_convention_camera, tm.Get_Translation(),
                                               &camera_space_source);
        ...
        if (WeaponModel && COMBAT_CAMERA) {
            ...
            /*			if ( raytest.CollidedPhysObj != nullptr &&
               raytest.CollidedPhysObj->Get_Observer() != nullptr ) { DamageableGameObj * obj =
               ((CombatPhysObserverClass
               *)raytest.CollidedPhysObj->Get_Observer())->As_DamageableGameObj(); if ( obj ) {
                                                    Debug_Say(( "Hit %s\n",
...
** For each weapon state, except ENTER and EXIT, we have a weapon anim tied to the weapon model (use
*IDLE anim for ENTER/EXIT )
...
** Weapon anims should be the same duration (number of frames) and the corresponding hand anim
*(except looping +IDLES)
*/
static void Aquire_Weapon_Assets(const WeaponClass* weapon)
{
    if (HandsPhysObj == nullptr) {
    ...
    if (WeaponModel != nullptr) {
        ...
        for (int i = 0; i < NUM_WEAPON_STATES; i++) {
            ...
            anim_name.Format("%s.F_GA_%s_%s", weapon_htree_name, weapon_name,
            ...
            if (HandsAnims[i] == nullptr) {
                ...
                HandsAnims[i]
                    = WW3DAssetManager::Get_Instance()->Get_HAnim("F_SKELETON.F_HA_PIST_IDLE");
            }
        }
    }
}
...
** The hand models can be different for each player model, and are in the form F_HM_XXX ie:
*F_HM_HAVOC
