        ...
        case CHUNKID_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DESCRIPTION_SOUND,
                                              DescriptionSoundFilename);
                    ...
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_HUD_POG_TEXTURE_NAME,
                                              HUDPogTextureName);
...
bool ObjectiveManager::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_NUM_SPECIFIED_TERTIARY_OBJECTIVES,
                      NumSpecifiedTertiaryObjectives);
    csave.End_Chunk();
...
bool ObjectiveManager::Load(ChunkLoadClass& cload)
{
    ...
    while (cload.Open_Chunk()) {
        ...
        case CHUNKID_MANAGER_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_NUM_SPECIFIED_TERTIARY_OBJECTIVES,
                                     NumSpecifiedTertiaryObjectives);
                default:
                    Debug_Say(("Unhandled Chunk:%d File:%s Line:%d\r\n", cload.Cur_Chunk_ID(),
                               __FILE__, __LINE__));
    ...
    if (objective != NULL) {
        ...
        if (is_unhiding && (status != ObjectiveManager::STATUS_ACCOMPLISHED)) {
            WideStringClass description
                = TranslateDBClass::Get_String(objective->ShortDescriptionID);
            message.Format(TRANSLATE(IDS_OBJ_NEW_OBJ), objective->Type_To_Name(), description);
            ...
            if (objective->Status != ObjectiveManager::STATUS_HIDDEN) {
                message.Format(TRANSLATE(IDS_OBJ_STATUS_CHANGED), objective->Type_To_Name(),
                               objective->Status_To_Name());
            }
        }
        CombatManager::Get_Message_Window()->Add_Message(message, objective->Type_To_Base_Color());
#endif
...
void ObjectiveManager::Sort_Objectives(void)
{
    ...
    if (ObjectiveList.Count() != 0) {
        ::qsort(&ObjectiveList[0], ObjectiveList.Count(), sizeof(ObjectiveList[0]),
                ObjectiveSortCallback);
    }
}

void ObjectiveManager::Set_Objective_HUD_Info(int id, float priority, const char* texture_name,
                                              int message_id)
{
    Objective* objective = Find_Objective(id);
    if (objective != NULL) {
