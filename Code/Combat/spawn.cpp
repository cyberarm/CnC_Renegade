      ...
      IsMultiplayWeaponSpawner(false)
{
    ...
    GENERIC_DEFID_PARAM(SpawnerDefClass, SpecialEffectsObjID,
...
bool SpawnerDefClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_DEF_APPLY_SPAWN_MATERIAL_EFFECT,
                      ApplySpawnMaterialEffect);
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_DEF_IS_MULTIPLAY_WEAPON_SPAWNER,
    ...
    for (i = 0; i < ScriptNameList.Count(); i++) {
        ...
        WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_SCRIPT_PARAMETERS,
                                   ScriptParameterList[i]);
    }
        ...
        case CHUNKID_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                ...
                case MICROCHUNKID_DEF_DEFINITION_ID: {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_SPAWN_DELAY_VARIATION,
                                     SpawnDelayVariation);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_POST_EFFECT_SPAWN_DELAY,
                                     PostVisualSpawnDelay);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_SPECIAL_EFFECTS_OBJ_ID,
                                     SpecialEffectsObjID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_GOTO_SPAWNER_POS, GotoSpawnerPos);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_GOTO_SPAWNER_POS_PRIORITY,
                                     GotoSpawnerPosPriority);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_TELEPORT_FIRST_SPAWN,
                                     TeleportFirstSpawn);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_STARTS_DISABLED, StartsDisabled);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_KILL_HIBERNATING_SPAWN,
                                     KillHibernatingSpawn);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_APPLY_SPAWN_MATERIAL_EFFECT,
                                     ApplySpawnMaterialEffect);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_IS_MULTIPLAY_WEAPON_SPAWNER,
                                     IsMultiplayWeaponSpawner);
        ...
        case CHUNKID_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                ...
                case MICROCHUNKID_DEFINITION_ID:
                    ...
                    Definition = (const SpawnerDefClass*)DefinitionMgrClass::Find_Definition(
                        definition_id);
                    WWASSERT(Definition != NULL);
                    break;
    ...
    */
    ...
    if (spawn_id != -1) {
        PhysicalGameObjDef* def
            = (PhysicalGameObjDef*)DefinitionMgrClass::Find_Definition(spawn_id);
        if (def == NULL) {
...
PhysicalGameObj* SpawnerClass::Spawn(int obj_id)
{
    ...
    if (Get_Definition().SpecialEffectsObjID != 0) {
        PhysicalGameObj* effect_obj
            = ObjectLibraryManager::Create_Object(Get_Definition().SpecialEffectsObjID);
        if (effect_obj != NULL) {
    ...
    // Start Def Scripts
    WWASSERT(Get_Definition().ScriptNameList.Count()
             == Get_Definition().ScriptParameterList.Count());
...
Matrix3D SpawnManager::Get_Primary_Spawn_Location(void)
{
    ...
    if (index == -1) {
        Debug_Say(
            ("Get_Primary_Spawn_Location: failed to find suitable spawner, return origin.\n"));
        return Matrix3D(1);
    }
    else {
...
Matrix3D SpawnManager::Get_Multiplayer_Spawn_Location(int player_type, SoldierGameObj* soldier)
{
    ...
    if (suitable_spawners == 0) {
        Debug_Say(
            ("Get_Multiplayer_Spawn_Location: failed to find suitable spawner, return origin.\n"));
        tm = Matrix3D(1);
    }
    else {
