...
DECLARE_DEFINITION_FACTORY(RaveshawBossGameObjDefClass, CLASSID_GAME_OBJECT_DEF_RAVESHAW_BOSS,
                           "Raveshaw Boss")
...
//
//////////////////////////////////////////////////////////////////////////
uint32 RaveshawBossGameObjDefClass::Get_Class_ID(void) const
{
...
//
///////////////////////////////////////////////////////////////////////////
void RaveshawBossGameObjDefClass::Save_Variables(ChunkSaveClass& csave)
{
    return;
}
...
void RaveshawBossGameObjDefClass::Load_Variables(ChunkLoadClass& cload)
{
    /*while (cload.Open_Micro_Chunk ()) {
            switch (cload.Cur_Micro_Chunk_ID ()) {
                    ...
                    default:
                            Debug_Say (("Unrecognized RaveshawBossGameObjDefClass Variable chunkID
...
//
///////////////////////////////////////////////////////////////////////////
void RaveshawBossGameObjClass::Find_Closest_Catwalk_Pos(const Vector3& curr_pos,
                                                        Vector3* catwalk_pos)
{
...
//
///////////////////////////////////////////////////////////////////////////
void RaveshawBossGameObjClass::STATE_IMPL_END(RAVESHAW_STATE_GRAB_SOLDIER)(void)
{
    return;
}
...
#define PASSED_FRAME(frame) (LastMeleeAnimFrame < frame && curr_frame >= frame)
        ...
        if (PASSED_FRAME(37.0F)) {
            ...
            DefinitionClass* definition
                = DefinitionMgrClass::Find_Named_Definition("Explosion_Raveshaw_Bodyslam");
            if (definition != nullptr) {
                ...
                ExplosionManager::Create_Explosion_At(definition->Get_ID(),
                                                      pelvis_tm.Get_Translation(), nullptr);
            }
        }
...
void RaveshawBossGameObjClass::STATE_IMPL_BEGIN(STEALTH_SOLDIER_STATE_FLYING)(void)
{
    ...
    Vector3 relative_vector
        = stealth_soldier->Get_Transform().Inverse_Rotate_Vector(FlyingObjectVector);
...
void RaveshawBossGameObjClass::STATE_IMPL_THINK(LIGHTNING_ROD_STATE_ACTIVE)(void)
{
    ...
    if (LightningRodStateTimer <= 0) {
        ...
        const Matrix3D& start_tm
            = LightningRodList[index0]->Peek_Model()->Get_Bone_Transform("BBZZZT");
        const Matrix3D& end_tm
            = LightningRodList[index1]->Peek_Model()->Get_Bone_Transform("BBZZZT");
        ...
        if (FreeRandom.Get_Int(6) == 1) {
            ...
            //
            WWAudioClass::Get_Instance()->Create_Instant_Sound("Rav_Elec_Twiddler",
                                                               Get_Transform());
        }
...
//
///////////////////////////////////////////////////////////////////////////
bool RaveshawBossGameObjClass::Apply_Bone_Collision_Damage(float damage_scale,
                                                           const char* bone_name)
{
    ...
    if (retval) {
        ...
        if (delta_vector.Z > 0.0F) {
            delta_vector.Z = 0.5F;
            delta_vector.Normalize();
            COMBAT_STAR->Peek_Physical_Object()->As_Phys3Class()->Apply_Impulse(delta_vector
                                                                                * 2.0F);
        }
...
void RaveshawBossGameObjClass::Determine_New_Overall_State(void)
{
    ...
    if (health_percent <= 0.05F) {
        ...
        bool go_heal = (health_percent <= 0.75F && possibility > 0)
            && (FreeRandom.Get_Int(possibility) == 0);
        if (go_heal) {
...
void RaveshawBossGameObjClass::Collect_Lightning_Rods(void)
{
    ...
    if (rod_count > 0) {
        ::qsort(&LightningRodList[0], rod_count, sizeof(DamageableStaticPhysClass*),
...
//
///////////////////////////////////////////////////////////////////////////
void RaveshawBossGameObjClass::Add_Lightning_Arc(const Vector3& start_point,
    ...
    for (int index = 0; index < ARC_OBJ_COUNT; index++) {
        if ((ArcLifeRemaining[index] <= 0)) {
            ...
            for (int bone_index = 0; bone_index < BONE_COUNT; bone_index++) {
                ...
                float percent = WWMath::Fabs(Bones[bone_index].Get_Translation().X
                                             / EndTM.Get_Translation().X);
...
//
////////////////////////////////////////////////////////////////
int __cdecl RaveshawBossGameObjClass::fnSortLightningRodsCallback(const void* elem1,
                                                                  const void* elem2)
{
