...
*/
SimplePersistFactoryClass<ElevatorPhysDefClass, PHYSICS_CHUNKID_ELEVATORPHYSDEF>
    _ElevatorPhysDefPersistFactory;

DECLARE_DEFINITION_FACTORY(ElevatorPhysDefClass, CLASSID_ELEVATORPHYSDEF, "ElevatorPhys")
_ElevatorPhysDefDefFactory;
      ...
      ElevatorMovingSoundDefID(0)
{
    ...
    EDITABLE_PARAM(ElevatorPhysDefClass, ParameterClass::TYPE_FLOAT,
                   ElevatorStoppedBottom_FrameNum);

    PARAM_SEPARATOR(ElevatorPhysDefClass, "Sounds");
    EDITABLE_PARAM(ElevatorPhysDefClass, ParameterClass::TYPE_SOUNDDEFINITIONID,
                   DoorOpenSoundDefID);
    EDITABLE_PARAM(ElevatorPhysDefClass, ParameterClass::TYPE_SOUNDDEFINITIONID,
                   DoorCloseSoundDefID);
    EDITABLE_PARAM(ElevatorPhysDefClass, ParameterClass::TYPE_SOUNDDEFINITIONID,
...
bool ElevatorPhysDefClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_DEF_ELEVATOR_STOPPED_FRAMENUM,
                      ElevatorStoppedBottom_FrameNum);
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_DEF_DOOR_ACCESS_DENIED_SOUNDID,
                      DoorAccessDeniedSoundDefID);
        ...
        case CHUNKID_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_LOWER_CALL_ZONE,
                                     CallZones[ZONE_LOWER_CALL]);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_LOWER_INSIDE_ZONE,
                                     CallZones[ZONE_LOWER_INSIDE]);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_UPPER_CALL_ZONE,
                                     CallZones[ZONE_UPPER_CALL]);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_UPPER_INSIDE_ZONE,
                                     CallZones[ZONE_UPPER_INSIDE]);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_DOORCLOSED_FRAMENUM,
                                     DoorClosedTop_FrameNum);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_DOOROPENING_FRAMENUM,
                                     DoorOpeningBottom_FrameNum);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_ELEVATOR_START_FRAMENUM,
                                     ElevatorStartTop_FrameNum);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_ELEVATOR_STOPPED_FRAMENUM,
                                     ElevatorStoppedBottom_FrameNum);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_DOOR_OPEN_SOUNDID, DoorOpenSoundDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_DOOR_CLOSE_SOUNDID,
                                     DoorCloseSoundDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_DOOR_UNLOCK_SOUNDID,
                                     DoorUnlockSoundDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_DOOR_ACCESS_DENIED_SOUNDID,
                                     DoorAccessDeniedSoundDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_ELEVATOR_MOVING_SOUNDID,
                                     ElevatorMovingSoundDefID);
...
**
*/
SimplePersistFactoryClass<ElevatorPhysClass, PHYSICS_CHUNKID_ELEVATORPHYS>
    _ElevatorPhysPersistFactory;
...
void ElevatorPhysClass::Update_Sound_Effects(void)
{
    ...
    if (State == STATE_MOVING_DOWN) {
        ...
        //	Check to see if we should play the opening or closing sounds on the door
        //
        if (Has_Frame_Occured(prev_frame, curr_frame,
                              AnimManager.Peek_Animation()->Get_Num_Frames() - 1)) {
        ...
        if (def->ElevatorMovingSoundDefID != 0) {
            ...
            if (model != NULL) {
                ...
                if (bone_index > 0) {
                    ...
                    //
                    if (MovingSoundObj == NULL) {
                        MovingSoundObj = WWAudioClass::Get_Instance()->Create_Continuous_Sound(
                            def->ElevatorMovingSoundDefID);
                    }
...
void ElevatorPhysClass::Get_Door_Transform(bool is_top, Matrix3D& tm)
{
    ...
    if (is_top) {
        Vector3 zone_vector
            = def->CallZones[ZONE_UPPER_CALL].Center - def->CallZones[ZONE_UPPER_INSIDE].Center;
        ...
        Vector3 zone_vector
            = def->CallZones[ZONE_LOWER_CALL].Center - def->CallZones[ZONE_LOWER_INSIDE].Center;
...
void ElevatorPhysClass::Set_Precision(void)
{
    ...
    cEncoderList::Set_Precision(BITPACK_ELEVATOR_TOP_DOOR_STATE, 0,
                                (int)ElevatorPhysClass::DOOR_STATE_MAX);
    cEncoderList::Set_Precision(BITPACK_ELEVATOR_BOTTOM_DOOR_STATE, 0,
                                (int)ElevatorPhysClass::DOOR_STATE_MAX);
    return;
}
