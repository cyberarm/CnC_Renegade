...
////////////////////////////////////////////////////////////////
SimplePersistFactoryClass<RepairBayGameObjDef, CHUNKID_GAME_OBJECT_DEF_REPAIR_BAY>
    _RepairBayGameObjDefPersistFactory;
SimplePersistFactoryClass<RepairBayGameObj, CHUNKID_GAME_OBJECT_REPAIR_BAY>
    _RepairBayGameObjPersistFactory;
...
//
////////////////////////////////////////////////////////////////
uint32 RepairBayGameObjDef::Get_Class_ID(void) const
{
...
void RepairBayGameObjDef::Load_Variables(ChunkLoadClass& cload)
{
    while (cload.Open_Micro_Chunk()) {
        switch (cload.Cur_Micro_Chunk_ID()) {
            ...
            READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_REPARING_STATICANIM_DEFID,
                             RepairingStaticAnimDefID);
...
//
////////////////////////////////////////////////////////////////
const PersistFactoryClass& RepairBayGameObjDef::Get_Factory(void) const
{
...
//
////////////////////////////////////////////////////////////////
const PersistFactoryClass& RepairBayGameObj::Get_Factory(void) const
{
...
void RepairBayGameObj::CnC_Initialize(BaseControllerClass* base)
{
    ...
    for (iterator.First(); !iterator.Is_Done(); iterator.Next()) {
        ...
        if (anim_phys_obj != NULL
            && anim_phys_obj->Get_Definition()->Get_ID()
                == (uint32)Get_Definition().RepairingStaticAnimDefID) {
...
void RepairBayGameObj::Play_Repairing_Animation(bool onoff)
{
    ...
    StaticPhysClass* static_phys_obj
        = PhysicsSceneClass::Get_Instance()->Find_Static_Object(RepairAnimationID);
...
#pragma message("(TSS) available_funds			= 1000 looks suspect...")
                    ...
                    int shield_restored
                        = min(int(shield_max - curr_shield), (points_restored - health_restored));
...
void RepairBayGameObj::Emit_Welding_Arc(RenderObjClass* vehicle_model)
{
    ...
    if (mesh != NULL) {
        ...
        if (mesh_model != NULL) {
            ...
            if (vertex_count > 0) {
                endpoint
                    = mesh->Get_Transform() * mesh_model->Get_Vertex_Array()[random_vert_index];
            }
