...
SimplePersistFactoryClass<SoldierGameObjDef, CHUNKID_GAME_OBJECT_DEF_SOLDIER>
    _SoldierGameObjDefPersistFactory;

DECLARE_DEFINITION_FACTORY(SoldierGameObjDef, CLASSID_GAME_OBJECT_DEF_SOLDIER, "Soldier")
_SoldierGameObjDefDefFactory;
      ...
      DeathSoundPresetID(0)
{
    ...
    GENERIC_DEFID_PARAM(SoldierGameObjDef, HumanAnimOverrideDefID,
                        CLASSID_GLOBAL_SETTINGS_DEF_HUMAN_ANIM_OVERRIDE);
    GENERIC_DEFID_PARAM(SoldierGameObjDef, HumanLoiterCollectionDefID,
...
bool SoldierGameObjDef::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_DEF_HUMAN_LOITER_COLLECTION_DEF_ID,
                      HumanLoiterCollectionDefID);
        ...
        case CHUNKID_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_USE_INNATE_BEHAVIOR,
                                     UseInnateBehavior);
                    // READ_MICRO_CHUNK( cload, MICROCHUNKID_DEF_USE_INNATE_CONVERSATIONS,
                    // UseInnateConversations );
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_INNATE_AGGRESSIVENESS,
                                     InnateAggressiveness);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_INNATE_TAKE_COVER_PROB,
                                     InnateTakeCoverProbability);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_INNATE_IS_STATIONARY,
                                     InnateIsStationary);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_FIRST_PERSON_HANDS,
                                              FirstPersonHands);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_ORATOR_TYPE, OratorType);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_HUMAN_ANIM_OVERRIDE_DEF_ID,
                                     HumanAnimOverrideDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_HUMAN_LOITER_COLLECTION_DEF_ID,
                                     HumanLoiterCollectionDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_DEATH_SOUND_PRESET,
                                     DeathSoundPresetID);
...
** SolderGameObj
*/
SimplePersistFactoryClass<SoldierGameObj, CHUNKID_GAME_OBJECT_SOLDIER>
    _SoldierGameObjPersistFactory;
...
void SoldierGameObj::Copy_Settings(const SoldierGameObjDef& definition)
{
    ...
    HumanState.Set_Anim_Control(
        (HumanAnimControlClass*)
            Get_Anim_Control()); // Must set the anim control after the phys object
    ...
    if (InnateObserver == nullptr && Get_Definition().UseInnateBehavior
        ...
        case CHUNKID_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    //MICROCHUNKID_FORCE_FACING, ForceFacing );
                ...
                case MICROCHUNKID_INNATE_OBSERVER_PTR:
                    ...
                    //						READ_MICRO_CHUNK( cload,
                    //MICROCHUNKID_FLAME_TIMER, FlameTimer );
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_IS_USING_GHOST_COLLISION,
                                     IsUsingGhostCollision);
...
CollisionReactionType SoldierGameObj::Collision_Occurred(const CollisionEventClass& event)
{
    ...
    if (event.OtherObj != nullptr && event.OtherObj->Get_Observer() != nullptr) {
        PhysicalGameObj* obj
            = ((CombatPhysObserverClass*)event.OtherObj->Get_Observer())->As_PhysicalGameObj();
                    ...
                    if ((vel.Length() > obj->As_VehicleGameObj()->Get_Squish_Velocity())
                        ...
                        if (HumanState.Get_State() != HumanStateClass::DEATH) {
                            ...
                            if ((obj->As_VehicleGameObj()->Get_Driver() != nullptr)
                                && (obj->As_VehicleGameObj()->Get_Driver()->Get_Player_Data()
                                    != nullptr)) {
...
{
    Set_Weapon_Model(nullptr);
    // If we are changing to the same def, the above line made us lose our weapon model, and we
    // won't get it back. The next line makes us get it back.
...
void SoldierGameObj::Import_Frequent(BitStreamClass& packet)
{
    ...
    float max_error
        = cEncoderList::Get_Encoder_Type_Entry(BITPACK_WORLD_POSITION_Z).Get_Resolution() / 2.0f;
...
//-----------------------------------------------------------------------------
void SoldierGameObj::Interpret_Sc_State_Data(HumanStateClass::HumanStateType state, int sub_state,
                                             LPCSTR trans_name, const Vector3& velocity,
                                             const Vector3& sc_position)
{
    ...
    if ((Get_State() != state) || (Get_Sub_State() != sub_state)) {
        if ((Get_State() == HumanStateClass::TRANSITION)
            && // If in TRANSITION, don't switch to TRANSITION_COMPLETE
            ...
            if (Is_Controlled_By_Me()
                && (((Get_State() == HumanStateClass::UPRIGHT)
                     || (Get_State() == HumanStateClass::AIRBORNE)
...
int SoldierGameObj::Tally_Vis_Visible_Soldiers(void)
{
    ...
    if (pvs != nullptr) {
        ...
        for (SLNode<SmartGameObj>* smart_objnode
             = GameObjManager::Get_Smart_Game_Obj_List()->Head();
             smart_objnode != nullptr; smart_objnode = smart_objnode->Next()) {
...
bool SoldierGameObj::Is_In_Elevator(void)
{
    ...
    return p_human_phys != nullptr && p_human_phys->Peek_Carrier_Object() != nullptr
    ...
    if (CombatManager::I_Am_Server()
        ...
        if (!transition_triggered) {
            if (action_triggered) {
                ...
                //
                //				DamageableGameObj *damageable_target =
                //HUDInfo::Get_Weapon_Target_Object();
                ...
                if (damageable_target != nullptr) {
                    ...
                    if (physical_target != nullptr) {
                        ...
                        if ((target_pos - my_pos).Length() <= 2) {
                            ...
                            //
                            if (CombatManager::I_Am_Server()) {
                                const GameObjObserverList& observer_list
                                    = physical_target->Get_Observers();
                            ...
                            if (COMBAT_STAR == this) {
                                ...
                                if (physical_target->As_SimpleGameObj() != nullptr) {
                                    ...
                                    type = (physical_target->As_SimpleGameObj())
                                               ->Get_Definition()
                                               .Get_Player_Terminal_Type();
                                    if (type != PlayerTerminalClass::TYPE_NONE) {

                                        //
                                        //	Show the player terminal
                                        //
                                        PlayerTerminalClass::Get_Instance()->Display_Terminal(this,
                                                                                              type);
                                    }
                                }
                            }
...
SyncLegs = true;
    ...
    // Footsteps
    if (!Is_Sniping() && !InFlyMode && CombatManager::Is_Gameplay_Permitted() && do_steps
        && Is_Control_Enabled()) {
        ...
        if (leg_mode != LastLegMode) {
            ...
            SurfaceEffectsManager::Apply_Effect(ground_type, my_type, tm, nullptr, this, true,
                                                allow_emitters);
...
{
    {
        ...
        */
        ...
        if (DisplayDebugBoxForGhostCollision
            ...
            PhysicsSceneClass::Get_Instance()->Add_Debug_AABox(soldier_box,
...

    {
        ...
        if (CombatManager::I_Am_Server()) {
            ...
            if (Get_Weapon() && (Get_Weapon()->Get_Style() == WEAPON_HOLD_STYLE_C4)
                && (Get_Weapon()->Is_Firing()) && (Get_State() == HumanStateClass::UPRIGHT)) {
        ...
        */
        ...
        if (SpecialDamageMode != ArmorWarheadManager::SPECIAL_DAMAGE_TYPE_NONE
            ...
            if ((int)SpecialDamageTimer != (int)previous_timer) {
                StringClass explosion_name
                    = ArmorWarheadManager::Get_Special_Damage_Explosion(SpecialDamageMode);
                if (!explosion_name.Is_Empty()) {
                    ExplosionDefinitionClass* def
            ...
            if (Get_Defense_Object()->Get_Health() > 0
                ...
                // do damage
                WarheadType warhead
                    = ArmorWarheadManager::Get_Special_Damage_Warhead(SpecialDamageMode);
                float damage = ArmorWarheadManager::Get_Special_Damage_Scale(SpecialDamageMode)
                    * TimeManager::Get_Frame_Seconds();
                OffenseObjectClass off(damage, warhead,
                                       (ArmedGameObj*)SpecialDamageDamager.Get_Ptr());
                //			Apply_Damage( off, 1 );
        ...
        if (Get_State() == HumanStateClass::DESTROY) {
            if (HumanState.Get_State_Timer() > CORPSE_PERSIST_TIME) {
                ...
                if (Is_Human_Controlled()) {
                    //				PowerUpGameObj::Create_Backpack( this );	//
                    //No more backpacks
                }
        ...
        */
        ...
        // Apply Surface damage
        {
            ...
            if (Peek_Human_Phys()) {
                SurfaceEffectsManager::Apply_Damage(Peek_Human_Phys()->Get_Contact_Surface_Type(),
                                                    this);
            }
        }
        ...
        // - Otherwise, do a ray cast to find the water surface, if it hits
        {
            ...
            if ((Peek_Human_Phys()->Get_Contact_Surface_Type() == SURFACE_TYPE_UNDERWATER_DIRT)
                ...
                if ((res.Fraction < 1.0f) && (res.SurfaceType == SURFACE_TYPE_WATER_PERMEABLE)) {
                    ...
                    SurfaceEffectsManager::Update_Persistant_Emitter(WaterWake, res.SurfaceType,
                                                                     hit_type, Matrix3D(point));
                    in_water = true;
                }
            }

            if (!in_water) {
                SurfaceEffectsManager::Update_Persistant_Emitter(WaterWake, SURFACE_TYPE_DEFAULT,
                                                                 hit_type, Matrix3D(p0));
            }
        }
...
void SoldierGameObj::Post_Think(void)
{
...

    {
        ...
        //				Get_Weapon()->Reset_Anim_Update();		// reset
        //model anim update needed
        //			}

        if (WeaponAnimControl) {
            WeaponAnimControl->Update(
                TimeManager::Get_Frame_Seconds()); // update the animation control
        }
        ...
        if (!returning) {
            ...
            if (HeadLookAngle.Length() > 0.001f) {
                ...
                if (HeadLookAngleTimer < 0) {
                    HeadLookAngle
                        = Vector3(FreeRandom.Get_Float(-HEAD_TURN_LIMIT, HEAD_TURN_LIMIT),
                ...
                //
                //	Release the captured bone, this will have the effect of causing the
                //Get_Bone_Transform ()
                // methods to return the un-modified (due to being controlled) transform of the
                // bone.
                //
                ...
                // as the Z axis in object space, the Y axis is the same as the X axis in object
                // space, and the Z axis is the same as the Y axis in object space.
                //
                desired_head_rotation.X
                    = WWMath::Atan2(relative_head_target.Z, relative_head_target.Y);
                desired_head_rotation.Z
                    = -WWMath::Fast_Asin(relative_head_target.X / relative_head_target.Length());
                ...
                // animation is turning the head and the amount we need to turn to look at the
                // target.
                //
                ...
                //
                desired_head_rotation.X
                    = WWMath::Clamp(desired_head_rotation.X, min_twist, max_twist);
                desired_head_rotation.Z
                    = WWMath::Clamp(desired_head_rotation.Z, min_tilt, max_tilt);
            }
        }
...
#define HEAD_TILT_RATE (DEG_TO_RAD(180) / 2)
        ...
        HeadRotation.X
            += WWMath::Clamp((desired_head_rotation.X - HeadRotation.X), -max_turn, max_turn);
        HeadRotation.Z
            += WWMath::Clamp((desired_head_rotation.Z - HeadRotation.Z), -max_tilt, max_tilt);
...
//------------------------------------------------------------------------------------
void SoldierGameObj::Set_Blended_Animation(const char* animation_name, bool looping,
                                           float frame_offset, bool play_backwards)
{
...
//------------------------------------------------------------------------------------
void SoldierGameObj::Start_Transition_Animation(const char* anim_name,
                                                TransitionCompletionDataStruct* data)
{
...
#define TILT_DOWN_SPEED 4.0
    ...
    //)); 	Debug_Say(( "rel_target_pos %f %f %f\n", rel_target_pos.X, rel_target_pos.Y,
    //rel_target_pos.Z ));
...
void SoldierGameObj::Say_Dialogue(int dialog_id)
{
    ...
    if (conversation_id > 0) {
        ...
        ActiveConversationClass* conversation
            = ConversationMgrClass::Start_Conversation(this, conversation_id, true);
                                           ...
                                           AudibleSoundClass** sound_obj_to_return)
{
    ...
    if (translate_obj != nullptr) {
        ...
        //
        if (string != nullptr && string[0] == L'/' && string[1] == L'/') {
            string += 2;
            display_text = false;
        }
        ...
        if (display_text && string != nullptr && string[0] != 0) {
            ...
            CombatManager::Get_Message_Window()->Add_Message(string, Vector3(1, 1, 1), speaker,
                                                             message_duration);
        }
...
//------------------------------------------------------------------------------------
void SoldierGameObj::Apply_Damage(const OffenseObjectClass& damager, float scale,
                                  int alternate_skin)
{
    if (!Is_In_Vehicle()) {
                                           ...
                                           const Vector3& direction, const char* collision_box_name)
{
    ...
    if (this == COMBAT_STAR) {
        ...
        DIAG_LOG(("DRCE", "%s; %d; %s; %1.2f; %1.2f; %1.2f; %1.2f; %1.2f", weapon_name, hitter_id,
                  body_part, armor, health, pos.X, pos.Y, pos.Z));
    }
    ...
    if ((damager.Get_Owner() == COMBAT_STAR) && (COMBAT_STAR != nullptr)) {
        ...
        DIAG_LOG(("DEFE",
                  "%1.2f; %1.2f; %1.2f; %s; %d; %d; %s; %1.2f; %1.2f; %1.2f; %1.2f; %1.2f; %s ",
                  pos.X, pos.Y, pos.Z, weapon_name, ammo, hittee_id, body_part, armor, health,
    ...
    if (ambushed) {
        ...
        float armor = Get_Defense_Object()->Get_Shield_Strength();
        DIAG_LOG(("AMGE", "%1.2f; %1.2f; %1.2f; %d; %1.2f; %1.2f; %1.2f; %1.2f; %1.2f",
                  victim_pos.X, victim_pos.Y, victim_pos.Z, victim_id, shooter_pos.X, shooter_pos.Y,
                  shooter_pos.Z, armor, health));
    }
    ...
    ArmorWarheadManager::SpecialDamageType special_damage
        = ArmorWarheadManager::Get_Special_Damage_Type(warhead);
    if (special_damage != 0) {
    ...
    if (this == COMBAT_STAR) {
        ...
        if (_cry_delay < 0) {
            ...
            AudibleSoundClass* sound
                = WWAudioClass::Get_Instance()->Create_Sound("Take_Damage_Sound");
            if (sound) {
                sound->Play();
                sound->Release_Ref();
            }
...
#define VISCEROID_NAME "Visceroid"
        ...
        if (HumanState.Get_State() != HumanStateClass::DEATH) {
            ...
            if (Get_Definition().KilledExplosion != 0) {
                ...
                ExplosionManager::Create_Explosion_At(
                    Get_Definition().KilledExplosion, pos,
            ...
            //
            if (damage_owner != nullptr && damage_owner != this
...
void SoldierGameObj::Enter_Vehicle(VehicleGameObj* vehicle, const char* anim_name)
{
    ...
    if (this == COMBAT_STAR && vehicle != nullptr) {
        ...
        DIAG_LOG(("ENVE", "%1.2f; %1.2f; %1.2f; %1.2f; %1.2f; %d", pos.X, pos.Y, pos.Z,
                  defense->Get_Shield_Strength(), defense->Get_Health(), ammo));
    }
}
...
void SoldierGameObj::Exit_Vehicle(void)
{
    if (this == COMBAT_STAR && Vehicle != nullptr) {
        ...
        DefenseObjectClass* defense = Vehicle->Get_Defense_Object();
        DIAG_LOG(("EXVE", "%1.2f; %1.2f; %1.2f; %1.2f; %1.2f; %d", pos.X, pos.Y, pos.Z,
                  defense->Get_Shield_Strength(), defense->Get_Health(), ammo));
    }
...
void SoldierGameObj::Give_All_Weapons(void)
{
    ...
    for (WeaponDefinitionClass* weapon_def
         = (WeaponDefinitionClass*)DefinitionMgrClass::Get_First(CLASSID_DEF_WEAPON);
         weapon_def != nullptr; weapon_def
         = (WeaponDefinitionClass*)DefinitionMgrClass::Get_Next(weapon_def, CLASSID_DEF_WEAPON)) {
...
VehicleGameObj* SoldierGameObj::Get_Profile_Vehicle(void)
{
    ...
    if ((Get_State() == HumanStateClass::TRANSITION) && (TransitionCompletionData != nullptr)) {
...
bool SoldierGameObj::Can_See(SoldierGameObj* p_soldier)
{
    ...
    if (raytest.CollidedPhysObj != nullptr) {
        ...
        if (p_blocker != nullptr && p_blocker->Get_Control_Owner() == p_soldier->Get_Control_Owner()) {
            can_see = true;
        }
    }
...
void SoldierGameObj::Adjust_Skeleton(float height, float width)
{
    ...
    if ((tree_base != nullptr) && (tree_tall != nullptr) && (tree_wide != nullptr)) {
        ...
        HTreeClass* tree
            = HTreeClass::Create_Interpolated(tree_base, tree_tall, tree_wide, height, width);
        if (tree) {
            robj->Set_HTree(tree);
            delete tree;
        }
...
struct
{
    ...
    //	{ ArmorWarheadManager::SPECIAL_DAMAGE_TYPE_FIRE,	"E_FLAME01",		"C L
    //UPPERARM" }, 	{ ArmorWarheadManager::SPECIAL_DAMAGE_TYPE_FIRE,	"E_FLAME01",		"C R
    //UPPERARM" },
    { ArmorWarheadManager::SPECIAL_DAMAGE_TYPE_CHEM, "e_tib_dump", "C SPINE" },
    //	{ ArmorWarheadManager::SPECIAL_DAMAGE_TYPE_CHEM,	"e_tib_dump",		"C L
    //UPPERARM" }, 	{ ArmorWarheadManager::SPECIAL_DAMAGE_TYPE_CHEM,	"e_tib_dump",		"C R
    //UPPERARM" },
};
#define NUM_SPECIAL_DAMAGE_EMITTERS                                                                \
    (sizeof(_SpecialDamageEmitters) / sizeof(_SpecialDamageEmitters[0]))

void SoldierGameObj::Set_Special_Damage_Mode(ArmorWarheadManager::SpecialDamageType mode,
    ...
    if (mode == ArmorWarheadManager::SPECIAL_DAMAGE_TYPE_NONE) {
        ...
        for (int emitter = 0; emitter < NUM_SPECIAL_DAMAGE_EMITTERS; emitter++) {
            if (_SpecialDamageEmitters[emitter].Mode == SpecialDamageMode) {
                ...
                while (--sub_count >= 0) {
                    ...
                    if (::stricmp(sub->Get_Name(), _SpecialDamageEmitters[emitter].EmitterName)
                        == 0) {
        ...
        if (mode != SpecialDamageMode) {
            ...
            if (Allow_Special_Damage_State_Lock() && IS_MISSION
                ...
                HumanState.Set_State((HumanStateClass::HumanStateType)(
                    mode - ArmorWarheadManager::SPECIAL_DAMAGE_TYPE_FIRE
            ...
            for (int emitter = 0; emitter < NUM_SPECIAL_DAMAGE_EMITTERS; emitter++) {
                if (_SpecialDamageEmitters[emitter].Mode == mode) {
                    ...
                    if (ro != nullptr) {
                        Peek_Model()->Add_Sub_Object_To_Bone(
                            ro, _SpecialDamageEmitters[emitter].BoneName);
                        ro->Release_Ref();
                    }
                }
            }
...
void SoldierGameObj::Perturb_Position(float max_perturb)
{
    ...
    bool succeeded
        = Peek_Human_Phys()->Find_Teleport_Location(initial_position, max_perturb, &new_position);
    if (succeeded) {
...
void SoldierGameObj::Set_AI_State(SoldierAIState state)
{
    ...
    if (AIState != state) {
        ...
        //
        if (state >= AI_STATE_SEARCH && state > AIState && ActiveConversation != nullptr
                        ...
                        if (pos_error.Length() > 0.02f) {
                                ...
                                if (Get_Control_Owner() == CombatManager::Get_My_Id()) {
                                        if (pos_error.Length() > 2) {
                                                //
                                                // Client is authoritative over his own guy, unless
error
                                        ...
                                        //} else {
                                                ...
                                                //
                                                // For NPC's the server could transmit the final
destination as well as
                                                // the proper location. Pop if appropriate to get as
close to
                                                // proper dest as possible, but pathfind instead to
the final dest.
                                                //
         ...
         !CombatManager::Is_In_Camera_Frustrum(from_position)) {
                        ...
                        if (!CombatManager::Is_In_Camera_Frustrum(to_position)) {
                                ...
                                do {
                                        ...
                                        //if (CombatManager::Is_In_Camera_Frustrum(next_pos +
height_offset)) {
                                        //	is_finished = true;
                                        //
                                        //	//
                                        //	// Logically it would be better if the next line
could be omitted.
...
bool SoldierGameObj::Is_Safe_To_Disable_Ghost_Collision(const Vector3& curr_pos)
{
    ...
    for (it.First(); !it.Is_Done(); it.Next()) {
        ...
        //
        if (game_obj != nullptr && game_obj != this && game_obj->As_SoldierGameObj() != nullptr
...
bool SoldierGameObj::Is_Soldier_Blocked(const Vector3& curr_pos)
{
    ...
    //
    if ((Get_Action() != nullptr && Get_Action()->Is_Busy()) || Is_Destroyed()
        || (IS_SOLOPLAY == false)) {
        return false;
    }
                    ...
                    if (distance > curr_action->Get_Parameters().MoveArrivedDistance) {
                        ...
                        /*if (Is_Human_Controlled () || curr_id > my_id) {
                                ...
                                if (soldier->Peek_Human_Phys () != nullptr) {
                                        soldier->Peek_Human_Phys ()->Collide (delta_vector *
                        TimeManager::Get_Frame_Seconds ());
                                }
                        }*/
                    }
