...
void SoldierGameObj::Copy_Settings(const SoldierGameObjDef& definition)
{
    ...
    if (InnateObserver == NULL && Get_Definition().UseInnateBehavior
...
int SoldierGameObj::Tally_Vis_Visible_Soldiers(void)
{
    ...
    if (pvs != NULL) {
        ...
        for (SLNode<SmartGameObj>* smart_objnode
             ...
             smart_objnode != NULL; smart_objnode = smart_objnode->Next()) {
...
SyncLegs = true;
    ...
    if (!Is_Sniping() && !InFlyMode && CombatManager::Is_Gameplay_Permitted() && do_steps
        ...
        if (leg_mode != LastLegMode) {
            ...
            SurfaceEffectsManager::Apply_Effect(ground_type, my_type, tm, NULL, this, true,
                                                allow_emitters);
...
#define VISCEROID_NAME "Visceroid"
        ...
        if (HumanState.Get_State() != HumanStateClass::DEATH) {
            ...
            //
            if (damage_owner != NULL && damage_owner != this
...
bool SoldierGameObj::Is_Soldier_Blocked(const Vector3& curr_pos)
{
    ...
    //
    if ((Get_Action() != NULL && Get_Action()->Is_Busy()) || Is_Destroyed()
        || (IS_SOLOPLAY == false)) {
        return false;
    }
