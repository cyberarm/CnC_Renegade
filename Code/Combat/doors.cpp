...
DECLARE_DEFINITION_FACTORY(DoorPhysDefClass, CLASSID_DOORPHYSDEF, "DoorPhys")
_DoorPhysDefDefFactory;
      ...
      DoorOpensForVehicles(false)
{
    ...
    EDITABLE_PARAM(DoorPhysDefClass, ParameterClass::TYPE_SOUNDDEFINITIONID,
                   AccessDeniedSoundDefID);
        ...
        case CHUNKID_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    //MICROCHUNKID_DEF_TRIGGER_RADIUS, TriggerRadius );
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_ACCESS_DENIED_SOUND_DEF_ID,
                                     AccessDeniedSoundDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_DOOR_OPENS_FOR_VEHICLES,
                                     DoorOpensForVehicles);
...
bool DoorPhysClass::Set_State(int new_state)
{
    ...
    if (State != new_state) {
        ...
        case STATE_OPENING_DOOR: {
            ...
            if (Get_DoorPhysDef()->LockCode != 0) {
                WWAudioClass::Get_Instance()->Create_Instant_Sound(
                    Get_DoorPhysDef()->UnlockSoundDefID, Get_Transform());
            }

            //
            //	Play the 'door-opening' sound
            //
            WWAudioClass::Get_Instance()->Create_Instant_Sound(Get_DoorPhysDef()->OpenSoundDefID,
                                                               Get_Transform());
        ...
        case STATE_CLOSING_DOOR: {
            ...
            //
            WWAudioClass::Get_Instance()->Create_Instant_Sound(Get_DoorPhysDef()->CloseSoundDefID,
                                                               Get_Transform());
        ...
        case STATE_ACCESS_DENIED: {
            ...
            //
            WWAudioClass::Get_Instance()->Create_Instant_Sound(
                Get_DoorPhysDef()->AccessDeniedSoundDefID, Get_Transform());
        } break;
        }
...
void DoorPhysClass::Update_State(float dt)
{
    ...
    case STATE_OPENED_DOOR: {
        ...
        //
        // Client shouldn't be able to close his own door unless he hears from the server. ST -
        // 1/15/2002 12:58PM
        //
        if (CombatManager::I_Am_Server()) {
            Timer -= dt;
            if (Timer < 0 && LockState == false && OpenRequestPending == false
...
bool DoorPhysClass::Can_Unlock_Me(SoldierGameObj* soldier) const
{
    if (soldier == COMBAT_STAR) {
        ...
        soldier->Get_Position(&pos);
        const char* desc = soldier->Has_Key(Get_DoorPhysDef()->LockCode) ? "KEUS" : "KEDE";
        DIAG_LOG((desc, "%1.2f; %1.2f; %1.2f; %d; %d", pos.X, pos.Y, pos.Z, Get_ID(),
                  Get_DoorPhysDef()->LockCode));
    }
