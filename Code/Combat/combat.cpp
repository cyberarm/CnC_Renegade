...
void CombatManager::Scene_Init(void)
{
    ...
    GameScene->Set_Fog_Color(
        Vector3(0.6f, 0.6f, 0.6f)); // Vector3(80.0f/255.0f,130.0f/255.0f,180.0f/255.0f));
    ...
    COMBAT_SCENE->Enable_Collision_Detection(TERRAIN_AND_BULLET_COLLISION_GROUP,
    ...
    COMBAT_SCENE->Disable_Collision_Detection(SOLDIER_GHOST_COLLISION_GROUP,
...
{
public:
    LoadThreadClass(const char* thread_name = "Game loader thread")
...
void CombatManager::Handle_Input()
{
    if (Input::Get_State(INPUT_FUNCTION_FIRST_PERSON_TOGGLE)) {
        ...
        if (COMBAT_STAR) {
            ...
            DIAG_LOG(("SWPE", "%1.2f; %1.2f; %1.2f; %s ", pos.X, pos.Y, pos.Z,
                      Is_First_Person() ? "First" : "Third"));
        }
    }
}
...
float CombatManager::Get_Damage_Factor(ArmedGameObj* p_armed_damager,
...
bool CombatManager::Is_In_Camera_Frustrum(Vector3& position)
{
    ...
    bool is_visible
        = (COMBAT_CAMERA->Project(projected_mid_point, position) == CameraClass::INSIDE_FRUSTUM);
    return is_visible;
}
...
void CombatManager::Update_Combat_Mode(void)
{
    ...
    case COMBAT_MODE_FIRST_PERSON:
        star->Peek_Model()->Set_Hidden(
            true); // Putting it here, because vehicles are turning it back on
        COMBAT_CAMERA->Set_Profile_Height(
            star->Get_Weapon_Height()); // This is every frame for crouching
        //			COMBAT_CAMERA->Enable_2D_Targeting(
        //Input::Get_State(INPUT_FUNCTION_CURSOR_TARGETING) );
        COMBAT_CAMERA->Enable_2D_Targeting(false);
        break;
    ...
    case COMBAT_MODE_IN_VEHICLE: {
        ...
        //			bool target_2d = ( !vehicle->Use_2D_Aiming() ^
        //Input::Get_State(INPUT_FUNCTION_CURSOR_TARGETING) ^ VehicleGameObj::Is_Target_Steering()
        //);

        bool target_2d = (Input::Get_State(INPUT_FUNCTION_CURSOR_TARGETING)
        ...
        if (target_2d) {
            ...
            if (vehiclephys != NULL) {
                ...
                heading_offset
                    = WWMath::Clamp(heading_offset, DEG_TO_RADF(-45.0f), DEG_TO_RADF(45.0f));
