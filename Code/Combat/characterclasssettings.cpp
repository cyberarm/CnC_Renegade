...
DECLARE_DEFINITION_FACTORY(CharacterClassSettingsDefClass, CLASSID_GLOBAL_SETTINGS_DEF_CHAR_CLASS,
                           "Character Classes")
...
static const char* CHAR_RANK_NAMES[CharacterClassSettingsDefClass::RANK_COUNT]
    = { "Enlisted", "Officer", "Special-Forces", "Boss" };
...
CharacterClassSettingsDefClass::CharacterClassSettingsDefClass(void)
{
    ...
    for (int class_index = 0; class_index < CLASS_COUNT; class_index++) {
        for (int team_index = 0; team_index < TEAM_COUNT; team_index++) {
            ...
            //
            StringClass name;
            name.Format("Class %s (%s)", CHAR_CLASS_NAMES[class_index],
                        CHAR_TEAM_NAMES[team_index]);
            ...
            for (int rank_index = 0; rank_index < RANK_COUNT; rank_index++) {
                ...
                NAMED_EDITABLE_PARAM(CharacterClassSettingsDefClass, ParameterClass::TYPE_INT,
                                     CostTable[class_index][rank_index][team_index],
                                     (const char*)name);

                name.Format("%s Object", CHAR_RANK_NAMES[rank_index]);

#ifdef PARAM_EDITING_ON
                GenericDefParameterClass* param = new GenericDefParameterClass(
                    &(DefinitionTable[class_index][rank_index][team_index]));
...
//
///////////////////////////////////////////////////////////////////////////////////////////
uint32 CharacterClassSettingsDefClass::Get_Class_ID(void) const
{
...
bool CharacterClassSettingsDefClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_MINIGUNNER_ENLISTED,
                      CostTable[CLASS_MINIGUNNER][RANK_ENLISTED][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_MINIGUNNER_OFFICER,
    ...
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_MINIGUNNER_BOSS,
                      CostTable[CLASS_MINIGUNNER][RANK_BOSS][TEAM_GDI]);
    ...
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_ROCKET_SOLDIER_BOSS,
                      CostTable[CLASS_ROCKET_SOLDIER][RANK_BOSS][TEAM_GDI]);

    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_GRENADIER_ENLISTED,
                      CostTable[CLASS_GRENADIER][RANK_ENLISTED][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_GRENADIER_OFFICER,
                      CostTable[CLASS_GRENADIER][RANK_OFFICER][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_GRENADIER_SPECIAL_FORCES,
                      CostTable[CLASS_GRENADIER][RANK_SPECIAL_FORCES][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_GRENADIER_BOSS,
                      CostTable[CLASS_GRENADIER][RANK_BOSS][TEAM_GDI]);

    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_ENGINEER_ENLISTED,
                      CostTable[CLASS_ENGINEER][RANK_ENLISTED][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_ENGINEER_OFFICER,
                      CostTable[CLASS_ENGINEER][RANK_OFFICER][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_ENGINEER_SPECIAL_FORCES,
                      CostTable[CLASS_ENGINEER][RANK_SPECIAL_FORCES][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_ENGINEER_BOSS,
                      CostTable[CLASS_ENGINEER][RANK_BOSS][TEAM_GDI]);
    ...
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_FLAME_THROWER_BOSS,
                      CostTable[CLASS_FLAME_THROWER][RANK_BOSS][TEAM_GDI]);

    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_MUTANT_ENLISTED,
                      CostTable[CLASS_MUTANT][RANK_ENLISTED][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_MUTANT_OFFICER,
                      CostTable[CLASS_MUTANT][RANK_OFFICER][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_MUTANT_SPECIAL_FORCES,
                      CostTable[CLASS_MUTANT][RANK_SPECIAL_FORCES][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_GDI_MUTANT_BOSS,
                      CostTable[CLASS_MUTANT][RANK_BOSS][TEAM_GDI]);
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_GDI_MINIGUNNER_BOSS,
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_GDI_GRENADIER_OFFICER,
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_GDI_GRENADIER_BOSS,
                      DefinitionTable[CLASS_GRENADIER][RANK_BOSS][TEAM_GDI]);

    WRITE_MICRO_CHUNK(csave, VARID_DEFID_GDI_ENGINEER_ENLISTED,
                      DefinitionTable[CLASS_ENGINEER][RANK_ENLISTED][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_GDI_ENGINEER_OFFICER,
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_GDI_ENGINEER_BOSS,
                      DefinitionTable[CLASS_ENGINEER][RANK_BOSS][TEAM_GDI]);
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_GDI_MUTANT_ENLISTED,
                      DefinitionTable[CLASS_MUTANT][RANK_ENLISTED][TEAM_GDI]);
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_GDI_MUTANT_OFFICER,
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_GDI_MUTANT_BOSS,
                      DefinitionTable[CLASS_MUTANT][RANK_BOSS][TEAM_GDI]);

    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_MINIGUNNER_ENLISTED,
                      CostTable[CLASS_MINIGUNNER][RANK_ENLISTED][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_MINIGUNNER_OFFICER,
    ...
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_MINIGUNNER_BOSS,
                      CostTable[CLASS_MINIGUNNER][RANK_BOSS][TEAM_NOD]);
    ...
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_ROCKET_SOLDIER_BOSS,
                      CostTable[CLASS_ROCKET_SOLDIER][RANK_BOSS][TEAM_NOD]);

    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_GRENADIER_ENLISTED,
                      CostTable[CLASS_GRENADIER][RANK_ENLISTED][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_GRENADIER_OFFICER,
                      CostTable[CLASS_GRENADIER][RANK_OFFICER][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_GRENADIER_SPECIAL_FORCES,
                      CostTable[CLASS_GRENADIER][RANK_SPECIAL_FORCES][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_GRENADIER_BOSS,
                      CostTable[CLASS_GRENADIER][RANK_BOSS][TEAM_NOD]);

    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_ENGINEER_ENLISTED,
                      CostTable[CLASS_ENGINEER][RANK_ENLISTED][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_ENGINEER_OFFICER,
                      CostTable[CLASS_ENGINEER][RANK_OFFICER][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_ENGINEER_SPECIAL_FORCES,
                      CostTable[CLASS_ENGINEER][RANK_SPECIAL_FORCES][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_ENGINEER_BOSS,
                      CostTable[CLASS_ENGINEER][RANK_BOSS][TEAM_NOD]);
    ...
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_FLAME_THROWER_BOSS,
                      CostTable[CLASS_FLAME_THROWER][RANK_BOSS][TEAM_NOD]);

    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_MUTANT_ENLISTED,
                      CostTable[CLASS_MUTANT][RANK_ENLISTED][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_MUTANT_OFFICER,
                      CostTable[CLASS_MUTANT][RANK_OFFICER][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_MUTANT_SPECIAL_FORCES,
                      CostTable[CLASS_MUTANT][RANK_SPECIAL_FORCES][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_COST_NOD_MUTANT_BOSS,
                      CostTable[CLASS_MUTANT][RANK_BOSS][TEAM_NOD]);
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_NOD_MINIGUNNER_BOSS,
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_NOD_GRENADIER_OFFICER,
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_NOD_GRENADIER_BOSS,
                      DefinitionTable[CLASS_GRENADIER][RANK_BOSS][TEAM_NOD]);

    WRITE_MICRO_CHUNK(csave, VARID_DEFID_NOD_ENGINEER_ENLISTED,
                      DefinitionTable[CLASS_ENGINEER][RANK_ENLISTED][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_NOD_ENGINEER_OFFICER,
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_NOD_ENGINEER_BOSS,
                      DefinitionTable[CLASS_ENGINEER][RANK_BOSS][TEAM_NOD]);
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_NOD_MUTANT_ENLISTED,
                      DefinitionTable[CLASS_MUTANT][RANK_ENLISTED][TEAM_NOD]);
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_NOD_MUTANT_OFFICER,
    ...
    WRITE_MICRO_CHUNK(csave, VARID_DEFID_NOD_MUTANT_BOSS,
                      DefinitionTable[CLASS_MUTANT][RANK_BOSS][TEAM_NOD]);
            ...
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_MINIGUNNER_SPECIAL_FORCES,
                                     CostTable[CLASS_MINIGUNNER][RANK_SPECIAL_FORCES][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_MINIGUNNER_BOSS,
                                     CostTable[CLASS_MINIGUNNER][RANK_BOSS][TEAM_GDI]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_ROCKET_SOLDIER_OFFICER,
                                     CostTable[CLASS_ROCKET_SOLDIER][RANK_OFFICER][TEAM_GDI]);
                    READ_MICRO_CHUNK(
                        cload, VARID_COST_GDI_ROCKET_SOLDIER_SPECIAL_FORCES,
                        CostTable[CLASS_ROCKET_SOLDIER][RANK_SPECIAL_FORCES][TEAM_GDI]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_GRENADIER_ENLISTED,
                                     CostTable[CLASS_GRENADIER][RANK_ENLISTED][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_GRENADIER_OFFICER,
                                     CostTable[CLASS_GRENADIER][RANK_OFFICER][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_GRENADIER_SPECIAL_FORCES,
                                     CostTable[CLASS_GRENADIER][RANK_SPECIAL_FORCES][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_GRENADIER_BOSS,
                                     CostTable[CLASS_GRENADIER][RANK_BOSS][TEAM_GDI]);

                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_ENGINEER_ENLISTED,
                                     CostTable[CLASS_ENGINEER][RANK_ENLISTED][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_ENGINEER_OFFICER,
                                     CostTable[CLASS_ENGINEER][RANK_OFFICER][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_ENGINEER_SPECIAL_FORCES,
                                     CostTable[CLASS_ENGINEER][RANK_SPECIAL_FORCES][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_ENGINEER_BOSS,
                                     CostTable[CLASS_ENGINEER][RANK_BOSS][TEAM_GDI]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_MUTANT_ENLISTED,
                                     CostTable[CLASS_MUTANT][RANK_ENLISTED][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_MUTANT_OFFICER,
                                     CostTable[CLASS_MUTANT][RANK_OFFICER][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_MUTANT_SPECIAL_FORCES,
                                     CostTable[CLASS_MUTANT][RANK_SPECIAL_FORCES][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_COST_GDI_MUTANT_BOSS,
                                     CostTable[CLASS_MUTANT][RANK_BOSS][TEAM_GDI]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_DEFID_GDI_MINIGUNNER_OFFICER,
                                     DefinitionTable[CLASS_MINIGUNNER][RANK_OFFICER][TEAM_GDI]);
                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_GDI_MINIGUNNER_SPECIAL_FORCES,
                        DefinitionTable[CLASS_MINIGUNNER][RANK_SPECIAL_FORCES][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_DEFID_GDI_MINIGUNNER_BOSS,
                                     DefinitionTable[CLASS_MINIGUNNER][RANK_BOSS][TEAM_GDI]);

                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_GDI_ROCKET_SOLDIER_ENLISTED,
                        DefinitionTable[CLASS_ROCKET_SOLDIER][RANK_ENLISTED][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_DEFID_GDI_ROCKET_SOLDIER_OFFICER,
                                     DefinitionTable[CLASS_ROCKET_SOLDIER][RANK_OFFICER][TEAM_GDI]);
                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_GDI_ROCKET_SOLDIER_SPECIAL_FORCES,
                        DefinitionTable[CLASS_ROCKET_SOLDIER][RANK_SPECIAL_FORCES][TEAM_GDI]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_DEFID_GDI_GRENADIER_OFFICER,
                                     DefinitionTable[CLASS_GRENADIER][RANK_OFFICER][TEAM_GDI]);
                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_GDI_GRENADIER_SPECIAL_FORCES,
                        DefinitionTable[CLASS_GRENADIER][RANK_SPECIAL_FORCES][TEAM_GDI]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_DEFID_GDI_ENGINEER_OFFICER,
                                     DefinitionTable[CLASS_ENGINEER][RANK_OFFICER][TEAM_GDI]);
                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_GDI_ENGINEER_SPECIAL_FORCES,
                        DefinitionTable[CLASS_ENGINEER][RANK_SPECIAL_FORCES][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_DEFID_GDI_ENGINEER_BOSS,
                                     DefinitionTable[CLASS_ENGINEER][RANK_BOSS][TEAM_GDI]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_DEFID_GDI_FLAME_THROWER_OFFICER,
                                     DefinitionTable[CLASS_FLAME_THROWER][RANK_OFFICER][TEAM_GDI]);
                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_GDI_FLAME_THROWER_SPECIAL_FORCES,
                        DefinitionTable[CLASS_FLAME_THROWER][RANK_SPECIAL_FORCES][TEAM_GDI]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_DEFID_GDI_MUTANT_SPECIAL_FORCES,
                                     DefinitionTable[CLASS_MUTANT][RANK_SPECIAL_FORCES][TEAM_GDI]);
                    READ_MICRO_CHUNK(cload, VARID_DEFID_GDI_MUTANT_BOSS,
                                     DefinitionTable[CLASS_MUTANT][RANK_BOSS][TEAM_GDI]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_MINIGUNNER_SPECIAL_FORCES,
                                     CostTable[CLASS_MINIGUNNER][RANK_SPECIAL_FORCES][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_MINIGUNNER_BOSS,
                                     CostTable[CLASS_MINIGUNNER][RANK_BOSS][TEAM_NOD]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_ROCKET_SOLDIER_OFFICER,
                                     CostTable[CLASS_ROCKET_SOLDIER][RANK_OFFICER][TEAM_NOD]);
                    READ_MICRO_CHUNK(
                        cload, VARID_COST_NOD_ROCKET_SOLDIER_SPECIAL_FORCES,
                        CostTable[CLASS_ROCKET_SOLDIER][RANK_SPECIAL_FORCES][TEAM_NOD]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_GRENADIER_ENLISTED,
                                     CostTable[CLASS_GRENADIER][RANK_ENLISTED][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_GRENADIER_OFFICER,
                                     CostTable[CLASS_GRENADIER][RANK_OFFICER][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_GRENADIER_SPECIAL_FORCES,
                                     CostTable[CLASS_GRENADIER][RANK_SPECIAL_FORCES][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_GRENADIER_BOSS,
                                     CostTable[CLASS_GRENADIER][RANK_BOSS][TEAM_NOD]);

                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_ENGINEER_ENLISTED,
                                     CostTable[CLASS_ENGINEER][RANK_ENLISTED][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_ENGINEER_OFFICER,
                                     CostTable[CLASS_ENGINEER][RANK_OFFICER][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_ENGINEER_SPECIAL_FORCES,
                                     CostTable[CLASS_ENGINEER][RANK_SPECIAL_FORCES][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_ENGINEER_BOSS,
                                     CostTable[CLASS_ENGINEER][RANK_BOSS][TEAM_NOD]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_MUTANT_ENLISTED,
                                     CostTable[CLASS_MUTANT][RANK_ENLISTED][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_MUTANT_OFFICER,
                                     CostTable[CLASS_MUTANT][RANK_OFFICER][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_MUTANT_SPECIAL_FORCES,
                                     CostTable[CLASS_MUTANT][RANK_SPECIAL_FORCES][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_COST_NOD_MUTANT_BOSS,
                                     CostTable[CLASS_MUTANT][RANK_BOSS][TEAM_NOD]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_DEFID_NOD_MINIGUNNER_OFFICER,
                                     DefinitionTable[CLASS_MINIGUNNER][RANK_OFFICER][TEAM_NOD]);
                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_NOD_MINIGUNNER_SPECIAL_FORCES,
                        DefinitionTable[CLASS_MINIGUNNER][RANK_SPECIAL_FORCES][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_DEFID_NOD_MINIGUNNER_BOSS,
                                     DefinitionTable[CLASS_MINIGUNNER][RANK_BOSS][TEAM_NOD]);

                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_NOD_ROCKET_SOLDIER_ENLISTED,
                        DefinitionTable[CLASS_ROCKET_SOLDIER][RANK_ENLISTED][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_DEFID_NOD_ROCKET_SOLDIER_OFFICER,
                                     DefinitionTable[CLASS_ROCKET_SOLDIER][RANK_OFFICER][TEAM_NOD]);
                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_NOD_ROCKET_SOLDIER_SPECIAL_FORCES,
                        DefinitionTable[CLASS_ROCKET_SOLDIER][RANK_SPECIAL_FORCES][TEAM_NOD]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_DEFID_NOD_GRENADIER_OFFICER,
                                     DefinitionTable[CLASS_GRENADIER][RANK_OFFICER][TEAM_NOD]);
                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_NOD_GRENADIER_SPECIAL_FORCES,
                        DefinitionTable[CLASS_GRENADIER][RANK_SPECIAL_FORCES][TEAM_NOD]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_DEFID_NOD_ENGINEER_OFFICER,
                                     DefinitionTable[CLASS_ENGINEER][RANK_OFFICER][TEAM_NOD]);
                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_NOD_ENGINEER_SPECIAL_FORCES,
                        DefinitionTable[CLASS_ENGINEER][RANK_SPECIAL_FORCES][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_DEFID_NOD_ENGINEER_BOSS,
                                     DefinitionTable[CLASS_ENGINEER][RANK_BOSS][TEAM_NOD]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_DEFID_NOD_FLAME_THROWER_OFFICER,
                                     DefinitionTable[CLASS_FLAME_THROWER][RANK_OFFICER][TEAM_NOD]);
                    READ_MICRO_CHUNK(
                        cload, VARID_DEFID_NOD_FLAME_THROWER_SPECIAL_FORCES,
                        DefinitionTable[CLASS_FLAME_THROWER][RANK_SPECIAL_FORCES][TEAM_NOD]);
                    ...
                    READ_MICRO_CHUNK(cload, VARID_DEFID_NOD_MUTANT_SPECIAL_FORCES,
                                     DefinitionTable[CLASS_MUTANT][RANK_SPECIAL_FORCES][TEAM_NOD]);
                    READ_MICRO_CHUNK(cload, VARID_DEFID_NOD_MUTANT_BOSS,
                                     DefinitionTable[CLASS_MUTANT][RANK_BOSS][TEAM_NOD]);
...
int CharacterClassSettingsDefClass::Get_Cost(CLASS char_class, RANK char_rank, TEAM char_team)
{
    ...
    //
    if (char_class >= 0 && char_class < CLASS_COUNT && char_rank >= 0 && char_rank < RANK_COUNT
