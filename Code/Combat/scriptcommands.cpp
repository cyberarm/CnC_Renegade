...
#define SCRIPT_PTR_CHECK(x)                                                                        \
    if (x == nullptr) {                                                                               \
        Debug_Say(("nullptr Script Ptr at %s line %d\n", __FILE__, __LINE__));                        \
        return;                                                                                    \
    }
...
void Modify_Action(GameObject* obj, int action_id, const ActionParamsStruct& params,
                   bool modify_move, bool modify_attack)
{
...
GameObject* Create_Object_At_Bone(GameObject* host_obj, const char* new_obj_type_name,
...
{
    SCRIPT_PTR_CHECK(obj);
    SCRIPT_TRACE(("ST>Trigger_Weapon( %d, %d, (%f %f %f ) %d\n", obj->Get_ID(), trigger, target.X,
                  target.Y, target.Z, primary_fire));
...
void Send_Damaged_Event(GameObject* object, GameObject* damager)
{
    ...
    SCRIPT_TRACE(("ST>Send_Damaged_Event( %d, %d )\n", object->Get_ID(),
                  ((damager != nullptr) ? damager->Get_ID() : 0)));
...
void Set_Model(GameObject* obj, const char* model_name)
{
    ...
    if (obj->As_PhysicalGameObj() && obj->As_PhysicalGameObj()->As_SimpleGameObj()) {
        if (obj->As_PhysicalGameObj()
                ->As_SimpleGameObj()
                ->Get_Definition()
                .Get_Is_Editor_Object()) {
            Debug_Say(("Can't Set_Model on a IsEditorObject %s %s\n",
                       obj->As_PhysicalGameObj()->As_SimpleGameObj()->Get_Definition().Get_Name(),
                       model_name));
            //			WWASSERT(0);
        }
    }
                   ...
                   float start_frame, float end_frame, bool is_blended)
{
    ...
    SCRIPT_TRACE(("ST>Set_Animation( %d, %s, %d )\n", obj->Get_ID(),
                  anim_name == nullptr ? "nullptr" : anim_name, looping));
    ...
    if ((sub_obj_name != nullptr) && (*sub_obj_name != 0)) {
        ...
        if (sub != nullptr) {
            ...
            if (anim != nullptr) {
                sub->Set_Animation(anim, start_frame,
                                   looping ? RenderObjClass::ANIM_MODE_LOOP
        ...
        if (cinobj != nullptr) {
            ...
            DynamicAnimPhysClass* dynanim
                = cinobj->Peek_Physical_Object()->As_DynamicAnimPhysClass();
            if (dynanim != nullptr) {
            ...
            //
            //	If the anim_name is the same as the current anim, then the frame_offset isn't set,
            //so
            // force it.
            //
...
void Set_Animation_Frame(GameObject* obj, const char* anim_name, int frame)
{
    ...
    SCRIPT_TRACE(("ST>Set_Animation_Frame( %d, %s, %d )\n", obj->Get_ID(),
                  anim_name == nullptr ? "nullptr" : anim_name, frame));
...
**
*/
GameObject* Find_Closest_Soldier(const Vector3& pos, float min_dist, float max_dist,
                                 bool only_human)
{
...
void Clear_Map_Region_By_Pos(const Vector3& world_space_pos, int pixel_radius)
{
    SCRIPT_TRACE(("ST>Clear_Map_Region_By_Pos( %f, %f, %f, %d)\n", world_space_pos.X,
                  world_space_pos.Y, world_space_pos.Z, pixel_radius));
...
void Add_Radar_Marker(int id, const Vector3& position, int shape_type, int color_type)
{
    SCRIPT_TRACE(("ST>Add_Radar_Marker( %d, (%f,%f,%f), %d, %d)\n", id, position.X, position.Y,
                  position.Z, shape_type, color_type));
...
void Create_Explosion_At_Bone(const char* explosion_def_name, GameObject* object,
                              const char* bone_name, GameObject* creator)
{
    ...
    SCRIPT_TRACE(("ST>Create_Explosion_At_Bone( %s, %d, %s, %d )\n", explosion_def_name,
                  (object != nullptr) ? object->Get_ID() : 0, bone_name,
...
void Give_PowerUp(GameObject* obj, const char* preset_name, bool display_on_hud)
{
    ...
    if (smart) {
        ...
        PowerUpGameObjDef* def = (PowerUpGameObjDef*)DefinitionMgrClass::Find_Typed_Definition(
            preset_name, CLASSID_GAME_OBJECT_DEF_POWERUP);
        if (def != nullptr) {
...
void Set_Innate_Soldier_Home_Location(GameObject* obj, const Vector3& home_pos, float home_radius)
{
    ...
    SCRIPT_TRACE(("ST>Set_Innate_Soldier_Home_Location( %d, (%f %f %f), %f )\n", obj->Get_ID(),
                  home_pos.X, home_pos.Y, home_pos.Z, home_radius));
...
void Innate_Force_State_Enemy_Seen(GameObject* obj, GameObject* enemy)
{
    ...
    if (smart != nullptr) {
        ...
        if (soldier != nullptr) {
            SoldierObserverClass* innate = soldier->Get_Innate_Controller();
            if (innate) {
                innate->Set_State(soldier, SoldierObserverClass::SOLDIER_AI_ENEMY_SEEN,
                                  Vector3(0, 0, 0), enemy);
                set = true;
            }
        }
...
void Static_Anim_Phys_Goto_Frame(int obj_id, float frame, const char* anim_name)
{
    ...
    if (spc != nullptr) {
        ...
        if (sapc) {
            ...
            sapc->Get_Animation_Manager().Set_Animation_Mode(
                AnimCollisionManagerClass::ANIMATE_TARGET);
...
void Static_Anim_Phys_Goto_Last_Frame(int obj_id, const char* anim_name)
{
    ...
    if (spc != nullptr) {
        ...
        if (sapc) {
            ...
            sapc->Get_Animation_Manager().Set_Animation_Mode(
                AnimCollisionManagerClass::ANIMATE_TARGET);
            sapc->Get_Animation_Manager().Set_Target_Frame(
                sapc->Get_Animation_Manager().Peek_Animation()->Get_Num_Frames() - 1);
        }
    }
}
...
** Objectives
*/
void Add_Objective(int id, int type, int status, int short_description_id,
                   char* description_sound_filename, int long_description_id)
{
    SCRIPT_TRACE(("ST>Add_Objective( %d, %d, %d, %d %d )\n", id, type, status, short_description_id,
                  long_description_id));
...
void Attach_To_Object_Bone(GameObject* object, GameObject* host_object, const char* bone_name)
{
    ...
    if (host_object == nullptr) {
        ...
        SCRIPT_TRACE(("ST>Attach_To_Object_Bone( %d, %d, %s )\n", object->Get_ID(),
                      host_object->Get_ID(), bone_name ? "nullptr" : bone_name));
    }
    ...
    if (object->As_PhysicalGameObj()) {
        if (host_object == nullptr) {
            ...
            object->As_PhysicalGameObj()->Attach_To_Object_Bone(host_object->As_PhysicalGameObj(),
                                                                bone_name);
        }
    }
    else {
...
int Create_Conversation(const char* conversation_name, int priority, float max_dist,
                        bool is_interruptable)
{
    ...
    if (conversation != nullptr) {
        ...
        ActiveConversationClass* active_conversation
            = ConversationMgrClass::Create_New_Conversation(conversation);
...
void Join_Conversation_Facing(GameObject* object, int active_conversation_id, int obj_id_to_face)
{
    ...
    SCRIPT_TRACE(("ST>Join_Conversation_Facing( %d, %d, %d )\n", object->Get_ID(),
                  active_conversation_id, obj_id_to_face));
...
void Join_Conversation(GameObject* object, int active_conversation_id, bool allow_move,
                       bool allow_head_turn, bool allow_face)
{
    int obj_id = 0;
    if (object != nullptr) {
    ...
    SCRIPT_TRACE(("ST>Join_Conversation( %d, %d, %d, %d )\n", obj_id, active_conversation_id,
                  allow_move, allow_head_turn));
...
**
*/
void Apply_Damage(GameObject* object, float amount, const char* warhead_name,
                  GameObject* damager = nullptr)
{
    SCRIPT_PTR_CHECK(object);
...
void Give_Points(GameObject* object, float points, bool entire_team)
{
    ...
    if (smart) {
        if (entire_team) {
            ...
            for (objnode = GameObjManager::Get_Star_Game_Obj_List()->Head(); objnode;
                 objnode = objnode->Next()) {
...
GameObject* Find_Nearest_Building_To_Pos(const Vector3& position, const char* mesh_prefix)
{
    ...
    for (obj_node = GameObjManager::Get_Building_Game_Obj_List()->Head(); obj_node;
...
void Set_Lightning(float intensity, float startdistance, float enddistance, float heading,
                   float distribution, float ramptime)
{
    if (!BackgroundMgrClass::Set_Lightning(intensity, startdistance, enddistance, heading,
                                           distribution, ramptime)) {
        Debug_Say(("Cannot set lightning\n"));
    }
}

void Set_War_Blitz(float intensity, float startdistance, float enddistance, float heading,
                   float distribution, float ramptime)
{
    if (!BackgroundMgrClass::Set_War_Blitz(intensity, startdistance, enddistance, heading,
                                           distribution, ramptime)) {
...
{
    if (!WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_RAIN, density,
                                            ramptime)) {
...
{
    if (!WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_SNOW, density,
                                            ramptime)) {
...
{
    if (!WeatherMgrClass::Set_Precipitation(WeatherMgrClass::PRECIPITATION_ASH, density,
                                            ramptime)) {
...
void Display_GDI_Player_Terminal(void)
{
    ...
    PlayerTerminalClass::Get_Instance()->Display_Terminal(COMBAT_STAR,
                                                          PlayerTerminalClass::TYPE_GDI);
    return;
}
...
void Display_Mutant_Player_Terminal(void)
{
    ...
    PlayerTerminalClass::Get_Instance()->Display_Terminal(COMBAT_STAR,
                                                          PlayerTerminalClass::TYPE_MUTANT);
    return;
}
