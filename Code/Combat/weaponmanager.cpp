...
DECLARE_DEFINITION_FACTORY(WeaponDefinitionClass, CLASSID_DEF_WEAPON, "Weapon")
_WeaponDefDefFactory;

uint32 WeaponDefinitionClass::Get_Class_ID(void) const
{
    return CLASSID_DEF_WEAPON;
}
const PersistFactoryClass& WeaponDefinitionClass::Get_Factory(void) const
{
    return _WeaponDefPersistFactory;
}
      ...
      IconOffset(0, 0)
{
    ...
    int i;
    //	EDITABLE_PARAM( WeaponDefinitionClass, ParameterClass::TYPE_INT,
    //Style);
    ...
    EDITABLE_PARAM(WeaponDefinitionClass, ParameterClass::TYPE_AMMOOBJDEFINITIONID,
                   PrimaryAmmoDefID);
    EDITABLE_PARAM(WeaponDefinitionClass, ParameterClass::TYPE_AMMOOBJDEFINITIONID,
                   SecondaryAmmoDefID);
...
bool WeaponDefinitionClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_WEAPON_DEF_MUZZLE_FLASH_PHYS_DEF_ID,
                      MuzzleFlashPhysDefID);
    ...
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_WEAPON_DEF_HUD_ICON_TEXTURE_NAME,
                               HUDIconTextureName);
    ...
    WRITE_SAFE_MICRO_CHUNK(csave, MICROCHUNKID_WEAPON_DEF_MAX_INVENTORY_ROUNDS, MaxInventoryRounds,
                           int);
        ...
        case CHUNKID_WEAPON_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    //						READ_MICRO_CHUNK(
                    //cload, 	LEGACY_MICROCHUNKID_WEAPON_DEF_FIRE_SOUND_DEFID,
                    //LegacyFireSoundDefID );
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_SWITCH_TIME, SwitchTime);
                    READ_SAFE_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_RELOAD_TIME, ReloadTime,
                                          float);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_GENERIC_AMMO_OK,
                                     CanReceiveGenericCnCAmmo);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_EJECT_PHYS_DEF_ID,
                                     EjectPhysDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_MUZZLE_FLASH_PHYS_DEF_ID,
                                     MuzzleFlashPhysDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_RATING, Rating);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_WEAPON_DEF_FIRST_PERSON_MODEL,
                                              FirstPersonModel);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_FIRST_PERSON_OFFSET,
                                     FirstPersonOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_RECOIL_IMPULSE, RecoilImpulse);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_WEAPON_DEF_HUD_ICON_TEXTURE_NAME,
                                              HUDIconTextureName);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_RELOAD_SOUND_DEFID,
                                     ReloadSoundDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_EMPTY_SOUND_DEFID,
                                     EmptySoundDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_PRIMARY_AMMO_DEF_ID,
                                     PrimaryAmmoDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_SECONDARY_AMMO_DEF_ID,
                                     SecondaryAmmoDefID);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_A_GIVE_WEAPONS_WEAPON,
                                     AGiveWeaponsWeapon);
                    READ_SAFE_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_DEF_MAX_INVENTORY_ROUNDS,
                                          MaxInventoryRounds, int);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_WEAPON_ICON_NAME_ID, IconNameID);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_WEAPON_ICON_TEXTURE_NAME,
                                              IconTextureName);
                    ...
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_HUMAN_FIRING_ANIMATION,
...
uint32 AmmoDefinitionClass::Get_Class_ID(void) const
{
    return CLASSID_DEF_AMMO;
}
const PersistFactoryClass& AmmoDefinitionClass::Get_Factory(void) const
{
    return _AmmoDefPersistFactory;
}
      ...
      GrenadeSafetyTime(0)
{
    ...
    //	EDITABLE_PARAM( AmmoDefinitionClass, ParameterClass::TYPE_WEAPONOBJDEFINITIONID,
    //WeaponDefID);
    ...
    EDITABLE_PARAM(AmmoDefinitionClass, ParameterClass::TYPE_SOUNDDEFINITIONID,
...
bool AmmoDefinitionClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_AMMO_DEF_CONTINUOUS_EMITTER_NAME,
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_AMMO_DEF_BEAM_SUBDIVISION_ENABLED,
        ...
        case CHUNKID_AMMO_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    //						READ_MICRO_CHUNK(
                    //cload, 	MICROCHUNKID_AMMO_DEF_WEAPON_DEF_ID,			WeaponDefID
                    //);
                    ...
                    READ_SAFE_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_SPRAY_COUNT, SprayCount,
                                          int);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_AMMO_DEF_TRAIL_EMITTER,
                                              TrailEmitter);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_SOFT_PIERCE_LIMIT,
                                     SoftPierceLimit);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_TERRAIN_ACTIVATED,
                                     TerrainActivated);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_IS_TRACKING, IsTracking);
                    READ_SAFE_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_EFFECTIVE_RANGE,
                                          EffectiveRange, float);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_EXPLOSION_DEF_ID, ExplosionDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_RANDOM_TRACKING_SCALE,
                                     RandomTrackingScale);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_DISPLAY_LASER, DisplayLaser);
                    READ_SAFE_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_CHARGE_TIME, ChargeTime,
                                          float);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_CONTINUOUS_SOUND_DEF_ID,
                                     ContinuousSoundDefID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_FIRE_SOUND_DEFID, FireSoundDefID);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_AMMO_DEF_CONTINUOUS_EMITTER_NAME,
                                              ContinuousEmitterName);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_MAX_BOUNCES, MaxBounces);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_SPRAY_BULLET_COST,
                                     SprayBulletCost);
                    READ_SAFE_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_AMMO_TYPE, AmmoType, int);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_C4_TRIGGER_TIME_1,
                                     C4TriggerTime1);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_C4_TRIGGER_TIME_2,
                                     C4TriggerTime2);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_C4_TRIGGER_TIME_3,
                                     C4TriggerTime3);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_C4_TRIGGER_RANGE_1,
                                     C4TriggerRange1);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_C4_TRIGGER_RANGE_2,
                                     C4TriggerRange2);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_C4_TRIGGER_RANGE_3,
                                     C4TriggerRange3);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_C4_TIMING_SOUND_1_ID,
                                     C4TimingSound1ID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_C4_TIMING_SOUND_2_ID,
                                     C4TimingSound2ID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_C4_TIMING_SOUND_3_ID,
                                     C4TimingSound3ID);
                    ...
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_AMMO_DEF_BEAM_TEXTURE,
                                              BeamTexture);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_BEAM_SUBDIVISION_ENABLED,
                                     BeamSubdivisionEnabled);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_BEAM_SUBDIVISION_SCALE,
                                     BeamSubdivisionScale);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_DEF_BEAM_SUBDIVISION_FROZEN,
                    ...
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_AMMO_ICON_TEXTURE_NAME,
                                              IconTextureName);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_AMMO_GRENADE_SAFETY_TIME,
                                     GrenadeSafetyTime);
...
{
    return (const WeaponDefinitionClass*)DefinitionMgrClass::Find_Typed_Definition(
        name, CLASSID_DEF_WEAPON);
}
...
{
    return (const AmmoDefinitionClass*)DefinitionMgrClass::Find_Typed_Definition(name,
                                                                                 CLASSID_DEF_AMMO);
}
