...
const char* TransitionTypeNames[] = {
    "LADDER_EXIT_TOP",       "LADDER_EXIT_BOTTOM",     "LADDER_ENTER_TOP",
    "LADDER_ENTER_BOTTOM",   "LEGACY_VEHICLE_ENTER_0", "LEGACY_VEHICLE_ENTER_1",
    "LEGACY_VEHICLE_EXIT_0", "LEGACY_VEHICLE_EXIT_1",  "VEHICLE_ENTER",
    "VEHICLE_EXIT",
};
...
//			COMBAT_CAMERA->Set_Lerp_Time( anim_duration * 1.3f );
    ...
    if (!pos.Is_Valid()) {
        WWDEBUG_SAY(
            ("Transition EndingTM has invalid position: %f, %f, %f\r\n", pos.X, pos.Y, pos.Z));
    }
#endif
    ...
    if (obj->Peek_Physical_Object() != nullptr
        && obj->Peek_Physical_Object()->As_Phys3Class() != nullptr) {
...
void TransitionManager::Build_Ladder_List(DynamicVectorClass<TransitionInstanceClass*>& list)
{
    ...
    for (ti_node = Transitions.Head(); ti_node; ti_node = ti_node->Next()) {
        ...
        if (type == TransitionDataClass::LADDER_EXIT_TOP
            || type == TransitionDataClass::LADDER_EXIT_BOTTOM
            || type == TransitionDataClass::LADDER_ENTER_TOP
            || type == TransitionDataClass::LADDER_ENTER_BOTTOM) {
