...
{
    HumanLoiterCollection
        = (HumanLoiterGlobalSettingsDef*)DefinitionMgrClass::Find_Definition(def_id);
}
        ...
        case CHUNKID_VARIABLES:
            ...
            while (cload.Open_Micro_Chunk()) {
                ...
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUMAN_ANIM_OVERRIDE_DEF_ID,
                                     human_anim_override_def_id);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUMAN_LOITER_COLLECTION_DEF_ID,
                                     human_loiter_collection_def_id);
...
#pragma MESSAGE("StateLocked Hack")
    ...
    if ((State == LADDER) || (State == IN_VEHICLE) || (State == TRANSITION)
        || (State == TRANSITION_COMPLETE) || (State == DEBUG_FLY)) {
...
{
    return (State == UPRIGHT) || (State == WOUNDED) || (State == LAND) || (State == LOITER)
        || (State == ANIMATION);
}
...
void HumanStateClass::Update_Animation(void)
{
    ...
    if ((State == UPRIGHT) || (State == AIRBORNE)) {
        ...
        if (single_anim) {
            ...
            //			float frame = AnimControl->Get_Frame();			// Maintain
            //the frame number for moving
            AnimControl->Set_Animation(anim_name, blend_time);
            //		AnimControl->Set_Mode( ANIM_MODE_LOOP, frame );
...
void HumanStateClass::Begin_Jump(void)
{
    ...
    SurfaceEffectsManager::Apply_Effect(ground_type,
                                        SurfaceEffectsManager::HITTER_TYPE_FOOTSTEP_JUMP, tm);
}
...
//
    ...
    if (fall.Z > settings->Get_Falling_Damage_Min_Distance()) {
        ...
        float scale = (fall.Z - settings->Get_Falling_Damage_Min_Distance())
            / (settings->Get_Falling_Damage_Max_Distance()
               - settings->Get_Falling_Damage_Min_Distance());
        ...
        if (owner != NULL) {
            ...
            for (int index = 0; index < observer_list.Count(); index++) {
                observer_list[index]->Custom(owner, CUSTOM_EVENT_FALLING_DAMAGE, damage * scale,
                                             NULL);
            }
            //			}
    ...
    (); PathfindSectorClass *start_sector	= PathfindClass::Get_Instance ()->Find_Sector
    (JumpTM.Get_Translation (), 2.0F); PathfindSectorClass *end_sector		=
    PathfindClass::Get_Instance ()->Find_Sector (curr_pos, 2.0F);
    ...
    if (add_portal && start_sector != NULL && end_sector != NULL && start_sector != end_sector) {
            PathfindClass::Get_Instance ()->Add_Temporary_Portal (start_sector, end_sector,
    JumpTM.Get_Translation (), curr_pos);
    }*/
    ...
    int ground_type = HumanPhys->Get_Contact_Surface_Type();
    SurfaceEffectsManager::Apply_Effect(ground_type,
                                        SurfaceEffectsManager::HITTER_TYPE_FOOTSTEP_LAND, tm);
}
