...
public:
    ...
    //	static void		Set_Damage_Multiplier( ArmorType armor, WarheadType warhead, float
    //multiplier );
    static float Get_Damage_Multiplier(ArmorType armor, WarheadType warhead);

    // Shield Absorbsion Settings
    //	static void		Set_Shield_Absorbsion( ArmorType armor, WarheadType warhead, float
    //percent );
...
public:
    ...
    // Special flag to disable client damage.  So it only works for bullets.  Only needed for client
    // authoratative damage
...
public:
    ...
    DefenseObjectClass(float health = DEFAULT_HEALTH, ArmorType skin = 0)
        : Health(health),
          ...
          ShieldType(0),
          CanObjectDie(true) { }; //,
    // LastSentHealth( -1 ), LastSentSkin( -1 ), LastSentShieldStrength( -1 ), LastSentShieldType(
    // -1 )  {};
    ~DefenseObjectClass() { };

    // DefenseObjects now have a pointer to their cooresponing GameObj to report damage and scoring
    // to the PlayerData
    ...
    // Apply Damage
    float Apply_Damage(const OffenseObjectClass& offense, float scale = 1.0f,
...
#define DEFENSEOBJECTDEF_EDITABLE_PARAMS(class_name, member_name)                                  \
    ...
    NAMED_EDITABLE_PARAM(class_name, ParameterClass::TYPE_FLOAT, member_name##.HealthMax,          \
                         "HealthMax");                                                             \
                                                                                                   \
    EnumParameterClass* skin_param = new EnumParameterClass((int*)&member_name##.Skin);            \
    skin_param->Set_Name("Skin");                                                                  \
    for (param_counter = 0; param_counter < ArmorWarheadManager::Get_Num_Armor_Types();            \
         param_counter++) {                                                                        \
    ...
    GENERIC_EDITABLE_PARAM(class_name, skin_param)                                                 \
                                                                                                   \
    NAMED_EDITABLE_PARAM(class_name, ParameterClass::TYPE_FLOAT, member_name##.ShieldStrength,     \
                         "ShieldStrength");                                                        \
    NAMED_EDITABLE_PARAM(class_name, ParameterClass::TYPE_FLOAT, member_name##.ShieldStrengthMax,  \
                         "ShieldStrengthMax");                                                     \
                                                                                                   \
    ...
    for (param_counter = 0; param_counter < ArmorWarheadManager::Get_Num_Armor_Types();            \
         param_counter++) {                                                                        \
        shield_param->Add_Value(ArmorWarheadManager::Get_Armor_Name(param_counter),                \
                                param_counter);                                                    \
    }                                                                                              \
    GENERIC_EDITABLE_PARAM(class_name, shield_param)                                               \
                                                                                                   \
    NAMED_EDITABLE_PARAM(class_name, ParameterClass::TYPE_FLOAT, member_name##.DamagePoints,       \
                         "DamagePoints");                                                          \
    NAMED_EDITABLE_PARAM(class_name, ParameterClass::TYPE_FLOAT, member_name##.DeathPoints,        \
                         "DeathPoints");
