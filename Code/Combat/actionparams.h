...
public:
    ...
    void Set_Basic(long observer_id, float priority, int action_id,
                   SoldierAIState ai_state = NO_AI_STATE_CHANGE)
    {
    ...
    void Set_Movement(const Vector3& location, float speed, float arrived_distance,
                      bool crouched = false)
    {
    ...
    void Set_Movement(GameObject* object, float speed, float arrived_distance,
                      bool crouched = false)
    {
        MoveObject = object;
        MoveSpeed = speed;
    ...
    void Join_Conversation(int active_conversation_id)
    {
    ...
    bool AttackFaceTarget; // (gth) control over whether VTOL vehicles face their target when
                           // attacking
...
void inline ActionParamsStruct::Set_Face_Location(const Vector3& obj_pos, float angle,
                                                  float duration)
{
