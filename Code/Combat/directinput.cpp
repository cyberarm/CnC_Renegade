...
typedef HRESULT(WINAPI* DirectInput8CreateType)(HINSTANCE hinst, DWORD dwVersion, REFIID riidltf,
...
void DirectInput::Init(void)
{
    ...
    if (DirectInputLibrary != nullptr) {
        DirectInput8CreatePtr = (DirectInput8CreateType)GetProcAddress(
            (HINSTANCE)DirectInputLibrary, "DirectInput8Create");

        if (DirectInput8CreatePtr) {

            // Create the DirectInput Object
            hr = DirectInput8CreatePtr(ProgramInstance, DIRECTINPUT_VERSION, IID_IDirectInput8,
            ...
            if FAILED (hr) {
                ...
                hr = DirectInput8CreatePtr(ProgramInstance, MINIMUM_DIRECTINPUT_VERSION,
                                           IID_IDirectInput8, (void**)&DIObject, nullptr);
                if FAILED (hr) {
    ...
    if (DIJoystickDevice != nullptr) {
        ...
        hr = DIJoystickDevice->SetCooperativeLevel(MainWindow,
...
void DirectInput::ReadKeyboard(void)
{
    ...
    while (!done) {
        ...
        hr = DIKeyboardDevice->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), &input_buffer,
                                             &buffer_size, 0);
    ...
    // Set Dupe Keys
    DIKeyboardButtons[DIK_CONTROL]
        = DIKeyboardButtons[DIK_LCONTROL] | DIKeyboardButtons[DIK_RCONTROL];
...
void DirectInput::ReadMouse(void)
{
    ...
    while (!done) {
        ...
        hr = DIMouseDevice->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), &input_buffer, &buffer_size,
                                          0);
...
retry_joystick:
    ...
    DIJoystickButtons[0] = Button_State_Table[((DIJoystickButtons[0] & 1) << 1)
                                              + ((DIJoystickState.rgbButtons[0] & 0x80) ? 1 : 0)];
    DIJoystickButtons[1] = Button_State_Table[((DIJoystickButtons[1] & 1) << 1)
                                              + ((DIJoystickState.rgbButtons[1] & 0x80) ? 1 : 0)];
}
