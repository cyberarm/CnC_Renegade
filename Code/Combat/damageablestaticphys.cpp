...
DECLARE_DEFINITION_FACTORY(DamageableStaticPhysDefClass, CLASSID_DAMAGEABLESTATICPHYSDEF,
                           "DamageableStaticPhys")
      ...
      PlayTwitchesToCompletion(false)
{
    ...
    EDITABLE_PARAM(DamageableStaticPhysDefClass, ParameterClass::TYPE_EXPLOSIONDEFINITIONID,
                   KilledExplosion);
    ...
    EDITABLE_PARAM(DamageableStaticPhysDefClass, ParameterClass::TYPE_BOOL,
                   PlayTwitchesToCompletion);
...
bool DamageableStaticPhysDefClass::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, DAMAGEABLESTATICPHYSDEF_VARIABLE_DEATHTRANSITIONSTART,
                      DeathTransitionStart);
    WRITE_MICRO_CHUNK(csave, DAMAGEABLESTATICPHYSDEF_VARIABLE_DEATHTRANSITIONEND,
                      DeathTransitionEnd);
    ...
    WRITE_MICRO_CHUNK(csave, DAMAGEABLESTATICPHYSDEF_VARIABLE_PLAYTWITCHESTOCOMPLETION,
                      PlayTwitchesToCompletion);
    csave.End_Chunk();
        ...
        case DAMAGEABLESTATICPHYSDEF_CHUNK_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    READ_MICRO_CHUNK(cload, DAMAGEABLESTATICPHYSDEF_VARIABLE_KILLEDEXPLOSION,
                                     KilledExplosion);
                    READ_MICRO_CHUNK(cload, DAMAGEABLESTATICPHYSDEF_VARIABLE_LIVELOOPSTART,
                                     LiveLoopStart);
                    READ_MICRO_CHUNK(cload, DAMAGEABLESTATICPHYSDEF_VARIABLE_LIVELOOPEND,
                                     LiveLoopEnd);
                    READ_MICRO_CHUNK(cload, DAMAGEABLESTATICPHYSDEF_VARIABLE_LIVETWITCHSTART,
                                     LiveTwitchStart);
                    READ_MICRO_CHUNK(cload, DAMAGEABLESTATICPHYSDEF_VARIABLE_LIVETWITCHEND,
                                     LiveTwitchEnd);
                    READ_MICRO_CHUNK(cload, DAMAGEABLESTATICPHYSDEF_VARIABLE_DEATHTRANSITIONSTART,
                                     DeathTransitionStart);
                    READ_MICRO_CHUNK(cload, DAMAGEABLESTATICPHYSDEF_VARIABLE_DEATHTRANSITIONEND,
                                     DeathTransitionEnd);
                    READ_MICRO_CHUNK(cload, DAMAGEABLESTATICPHYSDEF_VARIABLE_DEADLOOPSTART,
                                     DeadLoopStart);
                    READ_MICRO_CHUNK(cload, DAMAGEABLESTATICPHYSDEF_VARIABLE_DEADLOOPEND,
                                     DeadLoopEnd);
                    READ_MICRO_CHUNK(cload, DAMAGEABLESTATICPHYSDEF_VARIABLE_DEADTWITCHSTART,
                                     DeadTwitchStart);
                    READ_MICRO_CHUNK(cload, DAMAGEABLESTATICPHYSDEF_VARIABLE_DEADTWITCHEND,
                                     DeadTwitchEnd);
    ...
    if (def != nullptr) {
        ...
        */
        if ((def->PlayTwitchesToCompletion == false)
            ...
            if (frame0 != frame1) {
                Get_Animation_Manager().Set_Animation_Mode(
