...
SimplePersistFactoryClass<VehicleGameObjDef, CHUNKID_GAME_OBJECT_DEF_VEHICLE>
    _VehicleGameObjDefPersistFactory;

DECLARE_DEFINITION_FACTORY(VehicleGameObjDef, CLASSID_GAME_OBJECT_DEF_VEHICLE, "Vehicle")
_VehicleGameObjDefDefFactory;
...
bool VehicleGameObjDef::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_ENGINE_START_SOUND,
                      EngineSound[ENGINE_SOUND_STATE_STARTING]);
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_ENGINE_RUN_SOUND,
                      EngineSound[ENGINE_SOUND_STATE_RUNNING]);
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_ENGINE_STOP_SOUND,
                      EngineSound[ENGINE_SOUND_STATE_STOPPING]);
        ...
        case CHUNKID_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_ENGINE_SOUND_MAX_PITCH_FACTOR,
                                     EngineSoundMaxPitchFactor);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_ENGINE_START_SOUND,
                                     EngineSound[ENGINE_SOUND_STATE_STARTING]);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_ENGINE_RUN_SOUND,
                                     EngineSound[ENGINE_SOUND_STATE_RUNNING]);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_ENGINE_STOP_SOUND,
                                     EngineSound[ENGINE_SOUND_STATE_STOPPING]);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_ENGINE_OFF_SOUND,
                                     EngineSound[ENGINE_SOUND_STATE_OFF]);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_GDI_DAMAGE_REPORT_ID,
                                     GDIDamageReportID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_NOD_DAMAGE_REPORT_ID,
                                     NodDamageReportID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_GDI_DESTROY_REPORT_ID,
                                     GDIDestroyReportID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_NOD_DESTROY_REPORT_ID,
                                     NodDestroyReportID);
                default:
...
** VehicleGameObj
*/
SimplePersistFactoryClass<VehicleGameObj, CHUNKID_GAME_OBJECT_VEHICLE>
    _VehicleGameObjPersistFactory;
...
void VehicleGameObj::On_Post_Load(void)
{
    ...
    // For some reason???  some vehicles come in with an anim control, but no model in the anim
    // control.
...
void VehicleGameObj::Create_And_Destroy_Transitions(void)
{
    ...
    bool should_have_enter_transitions
        = (OccupiedSeats < Get_Definition().NumSeats) && TransitionsEnabled;
...
bool VehicleGameObj::Set_Targeting(const Vector3& target_pos, bool do_tilt)
{
    ...
    if (TurretBone != 0) {
        ...
        if (BarrelOffset) {
            ...
            //			Debug_Say(( "barrel angle %f %f\n", RAD_TO_DEG( barrel_offset_angle
            //), RAD_TO_DEG( relative_turn + barrel_offset_angle ) ));
    ...
    TurretTurn
        = WWMath::Clamp(TurretTurn, Get_Definition().WeaponTurnMin, Get_Definition().WeaponTurnMax);
    ...
    BarrelTilt
        = WWMath::Clamp(BarrelTilt, Get_Definition().WeaponTiltMin, Get_Definition().WeaponTiltMax);
    ...
    if (Get_Definition().WeaponTurnRate > DEG_TO_RAD(1000)
        && (WWMath::Fabs(relative_turn) >= DEG_TO_RAD(2)
            || WWMath::Fabs(relative_tilt) > DEG_TO_RAD(2))) {
        static int calls = 0;
        if (calls < 3) {
            calls++;
...
gunner->Get_Control().Get_Boolean( ControlClass::BOOLEAN_WEAPON_FIRE_SECONDARY ) );
                Control.Set_Boolean( ControlClass::BOOLEAN_WEAPON_NEXT,
gunner->Get_Control().Get_Boolean( ControlClass::BOOLEAN_WEAPON_NEXT ) ); Control.Set_Boolean(
ControlClass::BOOLEAN_WEAPON_PREV,		gunner->Get_Control().Get_Boolean(
ControlClass::BOOLEAN_WEAPON_PREV ) ); Control.Set_Boolean( ControlClass::BOOLEAN_WEAPON_RELOAD,
gunner->Get_Control().Get_Boolean( ControlClass::BOOLEAN_WEAPON_RELOAD ) ); Control.Set_Boolean(
ControlClass::BOOLEAN_WEAPON_USE,		gunner->Get_Control().Get_Boolean(
ControlClass::BOOLEAN_WEAPON_USE ) );
        }
...
); float target_direction = WWMath::Atan2( obj_space_target.Y, obj_space_target.X ); float
turn_amount = target_direction / DEG_TO_RAD( 60 ); Control.Set_Analog(
ControlClass::ANALOG_TURN_LEFT, turn_amount );
        }
...
void VehicleGameObj::Apply_Control(void)
{
    ...
    if (gunner && (gunner != driver) && !gunner->In_Transition() && !Get_Driver_Is_Gunner()) {
        Control.Set_Boolean(
            ControlClass::BOOLEAN_WEAPON_FIRE_PRIMARY,
            gunner->Get_Control().Get_Boolean(ControlClass::BOOLEAN_WEAPON_FIRE_PRIMARY));
        Control.Set_Boolean(
            ControlClass::BOOLEAN_WEAPON_FIRE_SECONDARY,
            gunner->Get_Control().Get_Boolean(ControlClass::BOOLEAN_WEAPON_FIRE_SECONDARY));
    ...
    if (target_steering && _Use_Target_Steering) {
        ...
        if (Control.Get_Analog(ControlClass::ANALOG_MOVE_FORWARD) >= 0.0f) {
            ...
            target_direction
                += TARGET_STRAFE_ANGLE * Control.Get_Analog(ControlClass::ANALOG_TURN_LEFT);
            ...
            if (Control.Get_Analog(ControlClass::ANALOG_MOVE_FORWARD) > 0.0f) {
                Control.Set_Analog(ControlClass::ANALOG_TURN_LEFT,
                                   0.5f * Control.Get_Analog(ControlClass::ANALOG_TURN_LEFT));
            }
        }
    }
...

    {
        ...
        if (Get_Weapon() && !Get_Definition().Fire0Anim.Is_Empty()) {
            ...
            if (state == WEAPON_ANIM_NOT_FIRING) {
                ...
                Set_Animation(!Get_Definition().Fire1Anim.Is_Empty() ? Get_Definition().Fire1Anim
                                                                     : Get_Definition().Fire0Anim);
            }
            else {
...
{
    if ((Peek_Vehicle_Phys() == nullptr) || (Peek_Vehicle_Phys()->Get_VehiclePhysDef()->Is_Fake())) {
        return;
    }
                ...
                if (wheel->Get_Flag(SuspensionElementClass::FAKE) == false) {
                    ...
                    if (wheel->Get_Flag(SuspensionElementClass::INCONTACT)) {
                        if (wheel->Get_Slip_Factor() > EMITTER_SLIDE_THRESHOLD) {
                            ...
                            // when attached to a cinematic, we deduce whether the wheels are
                            // rolling by using the rotation delta feature of wheels
                            if (Is_Attached_To_An_Object()) {
                                float wheel_avel = WWMath::Fabs(wheel->Get_Rotation_Delta()
                    ...
                    SurfaceEffectsManager::Update_Persistant_Emitter(
                        WheelSurfaceEmitters[emitter_index], surface_type, hitter_type,
                        wheel->Get_Wheel_Transform());

                    emitter_index++;

                    // If this is the sound generating wheel, update the sound
                    // I'm just using the first engine wheel as the one that generates the tire
                    // sound effects.
                    if (!sound_done && wheel->Get_Flag(SuspensionElementClass::ENGINE)) {
                        SurfaceEffectsManager::Update_Persistant_Sound(
                            WheelSurfaceSound, surface_type, hitter_type, Get_Transform());
...
void VehicleGameObj::Update_Sound_Effects(void)
{
    ...
    case ENGINE_SOUND_STATE_STARTING: {
        // play our ENGINE_STARTING sound, if it is finished or not found, destroy and go to
        // ENGINE_RUNNING
        if ((CachedEngineSound == nullptr)
            || ((CachedEngineSound != nullptr)
                && (CachedEngineSound->Get_State() == AudibleSoundClass::STATE_STOPPED))) {
    ...
    case ENGINE_SOUND_STATE_STOPPING: {
        // play the ENGINE_STOPPING sound, if it is finished or not found, destroy and go to
        // ENGINE_OFF
        if ((CachedEngineSound == nullptr)
            || ((CachedEngineSound != nullptr)
                && (CachedEngineSound->Get_State() == AudibleSoundClass::STATE_STOPPED))) {
...
void VehicleGameObj::Change_Engine_Sound_State(int new_state)
{
    ...
    DefinitionClass* sound_def
        = DefinitionMgrClass::Find_Definition(Get_Definition().EngineSound[new_state]);
...
bool VehicleGameObj::Is_Entry_Permitted(SoldierGameObj* p_soldier)
{
    ...
    for (i = 0; i < TransitionInstances.Count(); i++) {
        //		if ( ( TransitionInstances[i]->Get_Type() ==
        //TransitionDataClass::LEGACY_VEHICLE_ENTER_0 ) || 			  ( TransitionInstances[i]->Get_Type() ==
        //TransitionDataClass::LEGACY_VEHICLE_ENTER_1 ) ) {
...
{
    cEncoderList::Set_Precision(BITPACK_VEHICLE_VELOCITY, -90.0f, 90.0f,
                                0.01f); // 200 mph = approx. 90 m/s
    cEncoderList::Set_Precision(BITPACK_VEHICLE_ANGULAR_VELOCITY, -20.0f, 20.0f,
                                0.01f); // to approx 6 PI
...
void VehicleGameObj::Apply_Damage(const OffenseObjectClass& damager, float scale,
                                  int alternate_skin)
{
                ...
                case VEHICLE_TYPE_TANK:
                {
                        ...
                        switch (CombatManager::Get_Client_Interpolation_Model()) {
                                ...
                                case CLIENT_INTERPOLATION_PATHFIND: {
                                        ...
                                        if (p_driver != nullptr && p_driver->Get_Control_Owner() ==
