        ...
        case CHUNKID_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, LEGACY_MICROCHUNKID_DEF_DEFAULT_PLAYER_TYPE,
                                     DefaultPlayerType);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_KILLED_EXPLOSION, KilledExplosion);
                    READ_MICRO_CHUNK(cload, LEGACY_MICROCHUNKID_DEF_TRANSLATED_NAME_ID,
                                     TranslatedNameID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_DEFAULT_HIBERNATION_ENABLE,
                                     DefaultHibernationEnable);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_ALLOW_INNATE_CONVERSATIONS,
                                     AllowInnateConversations);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_ORATOR_TYPE, OratorType);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_USE_CREATION_EFFECT,
                                     UseCreationEffect);
...
bool PhysicalGameObj::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_IS_INNATE_CONVERSATIONS_ENABLED,
                      IsInnateConversationsEnabled);
        ...
        case CHUNKID_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_POKABLE_INDICATOR,
                                     HUDPokableIndicatorEnabled);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_IS_INNATE_CONVERSATIONS_ENABLED,
                                     IsInnateConversationsEnabled);
...
void PhysicalGameObj::Apply_Damage(const OffenseObjectClass& damager, float scale,
                                   int alternate_skin)
{
    ...
    if (HostGameObj.Get_Ptr()) {
        ...
        if (model != NULL) {
            ...
            */
            ...
            /*
            ** If we are some kind of moveable object see if we can teleport to the desired
            *position.
            ...
            */
            if (Peek_Physical_Object()->As_MoveablePhysClass()) {
                ...
                */
                PhysClass* host_phys
                    = ((PhysicalGameObj*)HostGameObj.Get_Ptr())->Peek_Physical_Object();
                if (host_phys != NULL) {
...
void PhysicalGameObj::Post_Think(void)
{
    if (AnimControl != NULL) {
        if (AnimControl->Peek_Model() != Peek_Model()) {
            ...
            // For some reason???  some vehicles come in with an anim control, but no model in the
            // anim control.
    ...
    if (AnimControl != NULL) {
        ...
        if (!anim_complete && AnimControl->Is_Complete()) {
            // we just completed.  Return Animation_Complete IF this is not a smart obj with
            // animation action
            if (As_SmartGameObj() == NULL || !As_SmartGameObj()->Get_Action()->Is_Animating()) {
                ...
                for (int index = 0; index < observer_list.Count(); index++) {
                    observer_list[index]->Animation_Complete(this,
                                                             AnimControl->Get_Animation_Name());
                }
            }
        }
    }
...
void PhysicalGameObj::Reset_Hibernating(void)
{
    ...
    HibernationTimer
        = MIN(HIBERNATION_DELAY, HibernationTimer + TimeManager::Get_Frame_Seconds() * 2);
}
...
void PhysicalGameObj::Import_Frequent(BitStreamClass& packet)
{
    ...
    if ((on_host_bone) && (Peek_Physical_Object())
...
void PhysicalGameObj::Object_Shattered_Something(PhysClass* observed_obj, PhysClass* shattered_obj,
                                                 int surface_type)
{
    const Matrix3D& tm = observed_obj->Get_Transform();

    SurfaceEffectsManager::Apply_Effect(surface_type, SurfaceEffectsManager::HITTER_TYPE_BULLET, tm,
                                        NULL, NULL,
                                        false, // no decals
                                        false // no emitter
    );
