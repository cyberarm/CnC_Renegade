    ...
    : UserCount(0)
{
    ...
    WWASSERT(AmplitudeBuffer != nullptr);
    Reset();
}
...
void SoundEnvironmentClass::Update(PhysicsSceneClass* scene, CameraClass* camera)
{
    ...
    if (UserCount > 0) {
        ...
        // Expand the bounding box by a small amount so that we can distinguish between collisions
        // with geometry and collisions with the bounding box.
        scenemin.Z -= 1.0f;
        scenemax.Z += 1.0f;
        ...
        // Cast a ray from the camera position upwards.
        {
            ...
            if (raytest.Result->Fraction < 1.0f) {
                ...
                // Is the camera in an environment zone (in which case environmental sounds should
                // not be heard)?
