...
public:
    ...
    // Damage
    virtual void Apply_Damage(const OffenseObjectClass& damager, float scale = 1.0f,
    ...
    virtual void Start_Transition_Animation(const char* anim_name,
                                            TransitionCompletionDataStruct* completion_data);
    virtual void Set_Animation(const char* animation_name, bool looping = true,
                               float frame_offset = 0.0f);
    virtual void Set_Blended_Animation(const char* animation_name, bool looping = true,
                                       float frame_offset = 0.0f, bool play_backwards = false);
    ...
    void Interpret_Sc_State_Data(HumanStateClass::HumanStateType state, int sub_state,
    ...
    */
    ...
    //
    static void Enable_Ghost_Collision_Debug_Display(bool onoff)
    {
        DisplayDebugBoxForGhostCollision = onoff;
    }
    static bool Is_Ghost_Collision_Debug_Display_Enabled(void)
    {
        return DisplayDebugBoxForGhostCollision;
    }
...
protected:
    ...
    // int						CtfTeamFlag; // Capture The Flag - use
    // NO_FLAG for no flag
