...
DECLARE_SCRIPT(DAK_Electric_Death_DAK,
               "") { void Created(GameObject * obj) { int time = Commands->Get_Random(1, 3);
Commands->Send_Custom_Event(obj, obj, 0, 0, time);
}

void Damaged(GameObject* obj, GameObject* damager, float amount)
{
    Commands->Send_Custom_Event(obj, obj, 1, 0,
                                1); // wait a second before applying next ammount of damage.
}
