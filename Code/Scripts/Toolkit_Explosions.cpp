               ...
               "Explosion_Name:string, Origin:vector3, Debug_Mode=0:int")
{
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        SCRIPT_DEBUG_MESSAGE(
            ("M00_Explosion_Create_RMV received custom type %d, param %d.\n", type, param));
...
DECLARE_SCRIPT(M00_Explosion_Create_At_Bone_RMV,
               "Start_Now=0:int, Receive_Type:int, Receive_Param_On:int, Explosion_Name:string, "
               "Object_ID:int, Bone_Name:string, Debug_Mode=0:int")
{
    int target_id;
    int custom_type;
    int parameter;
    ...
    void Created(GameObject * obj)
    {
        ...
        if (Get_Int_Parameter("Start_Now")) {
            ...
            if (Commands->Find_Object(target_id) != nullptr) {
                Commands->Create_Explosion_At_Bone(Get_Parameter("Explosion_Name"),
    ...
    {
        SCRIPT_DEBUG_MESSAGE(
            ("M00_Explosion_Create_At_Bone_RMV received custom type %d, param %d.\n", type, param));
        if ((type == custom_type) && (param == parameter)) {
            SCRIPT_DEBUG_MESSAGE(("M00_Explosion_Create_At_Bone_RMV ACTIVATED.\n"));
            if (Commands->Find_Object(target_id) != nullptr) {
                Commands->Create_Explosion_At_Bone(Get_Parameter("Explosion_Name"),
...
DECLARE_SCRIPT(M00_Create_Random_Explosion_DLS,
               "Explosion_Name:string, Delay_Min=0.0f:float, Delay_Max=4.0f:float, Loc_ID0=0:int, "
               "Loc_ID1=0:int, Loc_ID2=0:int, Loc_ID3=0:int, Loc_ID4=0:int, Loc_ID5=0:int, "
               "Loc_ID6=0:int, Loc_ID7=0:int, Loc_ID8=0:int, Loc_ID9=0:int")
{
    bool active;
    int random;
    int last;
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if (type == M00_CREATE_RANDOM_EXPLOSION && param == 1) {
            active = true;
            float delay = Commands->Get_Random(Get_Float_Parameter("Delay_Min"),
                                               Get_Float_Parameter("Delay_Max"));
...
void Custom(GameObject* obj, int type, int param, GameObject* sender)
{
    if (type == 1) {
        ...
        if (NukeCloud) {
            ...
            Commands->Attach_Script(NukeCloud, "M00_PlayAnimation_DestroyObject_DAY",
                                    "Nuke_cloud.Nuke_cloud");
        }
        ...
        if (NukeCloudTwo) {
            ...
            Commands->Attach_Script(NukeCloudTwo, "M00_PlayAnimation_DestroyObject_DAY",
                                    "XG_Nukecloud_02");
        }
    }
}
