...
DECLARE_SCRIPT(MX0_MissionStart_DME, "") // 1200001
{
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        ...
        if (timer_id == CROUCH_WANDER && Commands->Find_Object(sniper_1)) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(engineer1), CROUCH_WANDER,
                                        sniper_1, 0.0f);
            Commands->Send_Custom_Event(obj, Commands->Find_Object(engineer2), CROUCH_WANDER,
                                        sniper_1, 0.0f);
        ...
        if (timer_id == SNIPER_EXCHANGE) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(MX0_A02_CONTROLLER_ID),
                                        MX0_A02_CUSTOM_TYPE_MAIN_STARTUP, 0, 0.0f);
        }
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        if (type == SNIPER1KILLED) {
            if (!SniperNotify) {
                ...
                Commands->Join_Conversation(Commands->Find_Object(engineer1), id, false, true,
                                            true);
                Commands->Join_Conversation(STAR, id, true, true, true);
        ...
        if (type == START_SNIPER) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(sniper_1), START_SNIPER, 0,
                                        0.0f);
            Commands->Send_Custom_Event(obj, Commands->Find_Object(sniper_2), START_SNIPER, 0,
                                        0.0f);
        }
        ...
        if (type == 99) {
            ...
            //			Vector3 drop_loc = Commands->Get_Position (
            //Commands->Find_Object(1200001)); 			float chin_face = Commands->Get_Facing (
            //Commands->Find_Object(1200001));

            //			GameObject * chinook = Commands->Create_Object ( "Invisible_Object",
            //drop_loc); 			Commands->Set_Facing(chinook, chin_face); 			Commands->Attach_Script(chinook,
            //"Test_Cinematic", "MX0_TroopDrop_2X.txt");

            GameObject* block1
                = Commands->Create_Object("Large_Blocker", Vector3(-148.071f, -31.267f, -0.306f));
            Commands->Set_Facing(block1, -30.000f);
            Commands->Set_Is_Rendered(block1, false);
            GameObject* block2
                = Commands->Create_Object("Large_Blocker", Vector3(-148.234f, -31.542f, 1.357f));
        ...
        if (type == ENGINEER_CROUCH) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(engineer1), ENGINEER_CROUCH, 0,
                                        0.0f);
        }
        ...
        if (type == RUNNING_CONV) {
            ...
            Commands->Join_Conversation_Facing (Commands->Find_Object (engineer2), id,
            Commands->Get_ID (STAR)); Commands->Join_Conversation (STAR, id, true, true, true );
        ...
        if (type == COUNTER) {
            count = param;
            Commands->Send_Custom_Event(obj, Commands->Find_Object(engineer1), COUNT_UP, count,
                                        0.0f);
            Commands->Send_Custom_Event(obj, Commands->Find_Object(engineer2), COUNT_UP, count,
                                        0.0f);
        }
        ...
        if (type == 100002) {
            ...
            Commands->Join_Conversation(Commands->Find_Object(engineer1), id, false, true, true);
            Commands->Join_Conversation_Facing(Commands->Find_Object(engineer1), id,
                                               Commands->Get_ID(STAR));
            Commands->Join_Conversation(Commands->Find_Object(engineer2), id, false, true, true);
            Commands->Join_Conversation_Facing(Commands->Find_Object(engineer2), id,
                                               Commands->Get_ID(STAR));
            Commands->Join_Conversation(STAR, id, true, true, true);
        ...
        if (type == 100006) {
            if (Commands->Find_Object(sniper_1)) {
                Commands->Send_Custom_Event(obj, Commands->Find_Object(engineer2), SNIPER_ANIM, 0,
                                            0.0f);

                Commands->Start_Timer(obj, this, 5.0f, SNIPER_REMINDER);

                int id = Commands->Create_Conversation("MX0CON006", 97, 2000, false);
                Commands->Join_Conversation(Commands->Find_Object(engineer2), id, false, true,
                                            true);
                Commands->Join_Conversation_Facing(Commands->Find_Object(engineer2), id,
                                                   Commands->Get_ID(STAR));
                Commands->Join_Conversation(STAR, id, true, true, true);
        ...
        if (type == 100007) {
            if (Commands->Find_Object(sniper_1)) {
                if (curr_conv7 == 0 || curr_conv7 == 2 || curr_conv7 == 4) {
                    ...
                    char* conv7[6] = {
                        "MX0CON012", "MX0CON013", "MX0CON014",
                        "MX0CON015", "MX0CON016", "MX0CON017",
                    };
...
DECLARE_SCRIPT(MX0_Engineer1, "Damage_multiplier:float")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        Commands->Send_Custom_Event(obj, Commands->Find_Object(1200001), SET_ENGINEER,
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        if (type == CROUCH_WANDER) {
            ...
            if (sniper1) {
                ...
                params.Set_Movement(Commands->Find_Object(crouch_goto[rand_goto]), WALK, 0.2f,
                                    true);
...
DECLARE_SCRIPT(MX0_Engineer2, "Damage_multiplier:float")
{
    ...
    void Action_Complete(GameObject * obj, int action_id, ActionCompleteReason reason)
    {
        ...
        if (action_id == ENGINEER_GOTO && count == 4 && reason == ACTION_COMPLETE_NORMAL) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(1200001), ENGINEER_CROUCH, 0,
                                        0.0f);
...
DECLARE_SCRIPT(MX0_Engineer_Return, "")
{
    ...
    void Entered(GameObject * obj, GameObject * enterer)
    {
        if (!already_entered) {
            ...
            Commands->Send_Custom_Event(obj, Commands->Find_Object(1200001), ENGINEER_RETURN, 1,
                                        0.0f);
        }
    }
};
...
DECLARE_SCRIPT(MX0_Engineer_Goto, "GotoDest1:int, GotoDest2:int, Count:int")
{
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        if (type == RETURN_ENGINEER1) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(param), LEAD,
                                        Get_Int_Parameter("GotoDest1"), 0.0f);
        }

        if (type == RETURN_ENGINEER2) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(param), LEAD,
                                        Get_Int_Parameter("GotoDest2"), 0.0f);
        }
    }
    ...
    void Entered(GameObject * obj, GameObject * enterer)
    {
        if (!already_entered) {
            ...
            Commands->Send_Custom_Event(obj, Commands->Find_Object(1200001), COUNTER,
                                        Get_Int_Parameter("Count"), 0.0f);

            Commands->Send_Custom_Event(obj, Commands->Find_Object(1200001), CHECK_ENGINEER, 1,
                                        0.0f);
            Commands->Send_Custom_Event(obj, Commands->Find_Object(1200001), CHECK_ENGINEER, 2,
                                        0.0f);
    ...
    void Entered(GameObject * obj, GameObject * enterer)
    {
        if (!already_entered) {
            if (Get_Int_Parameter("Count") != 5) {
                ...
                // Commands->Send_Custom_Event( obj, Commands->Find_Object (1200001), 100004, 1,
                // 0.0f );
            }
            ...
            if (Get_Int_Parameter("Count") == 5) {
                ...
                Commands->Send_Custom_Event(obj, Commands->Find_Object(1200001), CROUCH_WANDER, 0,
                                            0.0f);
    ...
    void Entered(GameObject * obj, GameObject * enterer)
    {
        if (!already_entered) {
            ...
            if (Commands->Find_Object(1200017)) {
                Commands->Send_Custom_Event(obj, Commands->Find_Object(1200001), KILL_SNIPER, 0,
                                            0.0f);
            }
        }
    }
...
DECLARE_SCRIPT(MX0_AmbientBattle, "")
{
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        if (timer_id == AMB_SOUND) {
            battle_sound
                = Commands->Create_Sound("MX0_AmbientBattle_DME", Commands->Get_Position(obj), obj);
...
DECLARE_SCRIPT(MX0_KillNotify, "")
{
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        ...
        if (timer_id == SNIPE_CONV_DONE) {
            if (hit) {
                ...
                int id
                    = Commands->Create_Conversation(snipe_conv[rand_conv_snipe], 95, 2000, false);
                Commands->Join_Conversation(obj, id, true, true, true);
                Commands->Start_Conversation(id, 100021);
                Commands->Monitor_Conversation(obj, id);
    ...
    {
        if (killer == STAR) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(1200001), SNIPER1KILLED, 0,
                                        0.0f);
        }
    }
};
...
DECLARE_SCRIPT(DAK_MX0_Sec_3_Humvee, "") {
    void Entered(GameObject * obj, GameObject* enterer) { ActionParamsStruct params;
...
DECLARE_SCRIPT(MX0_GDI_ORCA, "")
{
    ...
    void Action_Complete(GameObject * obj, int action_id, ActionCompleteReason reason)
    {
        if (reason == ACTION_COMPLETE_CONVERSATION_ENDED) {
            if (action_id == 1) {
                ...
                Commands->Send_Custom_Event(obj, harvester, MX0_A03_CUSTOM_NOD_HARVESTER_DMG_SELF,
                                            4, (1 + 198 / 30));
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if (type == 2) {
            ...
            // "Trooper1: Woah. "That Rocked!" Orca6: "This is Orca 6 -- bingo Fuel. Returning to
            // Base..."
            ...
            // send custom to trooper1 to fire off "Blast that harvester for us sir!" after 10
            // seconds.
            if (Trooper_One) {
...
DECLARE_SCRIPT(MX0_NOD_INFANTRY, "troop_num:int")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        if (troop_num == 1) {
            Commands->Attach_Script(obj, "M00_Send_Object_ID",
                                    "1400041,15,0"); // sending id to A03 Controller
        }
        else {
            Commands->Attach_Script(obj, "M00_Send_Object_ID",
                                    "1400041,17,0"); // sending id to A03 Controller
        }
    }
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* target)
    {
        ...
        if (type == 1) // you've landed! move to your position.
        {
            ...
            if (troop_num == 1) // move to left pos.
            {
                ...
                // Commands->Debug_Message( "***** DAK ***** Setting NOD Ledge Troop 1 to move to
                // left position.\n" );
                params.Set_Movement(Commands->Find_Object(MX0_A03_NOD_LEDGE_LEFT_LOC_ID), RUN,
                                    0.25f);
                Commands->Action_Goto(obj, params);
            }
            else // move to right pos.
            {
                ActionParamsStruct params;
                params.Set_Basic(this, INNATE_PRIORITY_ENEMY_SEEN, 0);
                // Commands->Debug_Message( "***** DAK ***** Setting NOD Ledge Troop 2 to move to
                // right position.\n" );
                params.Set_Movement(Commands->Find_Object(MX0_A03_NOD_LEDGE_RIGHT_LOC_ID), RUN,
                                    0.25f);
                Commands->Action_Goto(obj, params);
            }
        }
    ...
    void Killed(GameObject * obj, GameObject * killer)
    {
        GameObject* controller = Commands->Find_Object(MX0_A03_CONTROLLER_ID);
        if (controller) {
            Commands->Send_Custom_Event(obj, controller, MX0_A03_CUSTOM_NOD_LEDGE_KILLED, troop_num,
                                        2);
        }
    }
};
...
DECLARE_SCRIPT(MX0_A03_GDI_INFANTRY, "troop_num:int")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        // disable inate
        // Commands->Innate_Disable (obj);
        Commands->Attach_Script(obj, "M00_Damage_Modifier_DME",
                                "0.15,0,1,1,1"); // Setting Damage Modifier.
...
DECLARE_SCRIPT(MX0_A03_CONTROLLER_DAK, "")
{
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if (type == MX0_A03_CUSTOM_NOD_LEDGE_KILLED) {
                ...
                && !harvester_Dead) // all ledge troops have been killed. bring in some more
            {
                GameObject* minigunners = Commands->Create_Object(
                    "Invisible_Object",
                    Commands->Get_Position(Commands->Find_Object(MX0_A03_NOD_LEDGE_DROP_LOC_ID)));
        ...
        if (type == MX0_A03_CUSTOM_TYPE_START_ZONE) {
            ...
            if (humvee) {
                Commands->Debug_Message(
                    "***** sending custom, dropping humvee, attaching script\n");
            ...
            GameObject* minigunner_one
                = Commands->Find_Object(MX0_A03_NOD_PLACED_MINIGUNNER_ONE_ID);
            if (minigunner_one) {
                Commands->Send_Custom_Event(
                    obj, minigunner_one, MX0_A03_CUSTOM_NOD_PLACED_MINIGUNNER_INNATE_ENABLE, 0, 0);
            }

            GameObject* minigunner_two
                = Commands->Find_Object(MX0_A03_NOD_PLACED_MINIGUNNER_TWO_ID);
            if (minigunner_two) {
                Commands->Send_Custom_Event(
                    obj, minigunner_two, MX0_A03_CUSTOM_NOD_PLACED_MINIGUNNER_INNATE_ENABLE, 0, 0);
            }
        }
        if (type == MX0_A03_CUSTOM_BUGGIE_DEAD) {
            ...
            /*			if ( zone_not_finished )
                                    {
                                            GameObject *Trooper_One = Commands->Find_Object(
               Trooper_One_Id );
               ...
               "MX0_A03_06" ); Commands->Join_Conversation( Trooper_One, conv_id, false, false,
               true); Commands->Start_Conversation (conv_id, 0);
                                            }
                                            else
                                            {
                                                    // Trooper1: "What was he thinking? Standing in
               a Tiberium field!" int conv_id = Commands->Create_Conversation ( "MX0_A03_07" );
                                                    Commands->Join_Conversation( Trooper_One,
               conv_id, false, false, true); Commands->Start_Conversation (conv_id, 0);
                                            }
                                    }
            */
        ...
        if (type == M00_SEND_OBJECT_ID) {
            ...
            if (param == 4) {
                Commands->Send_Custom_Event(obj, sender, M00_SEND_OBJECT_ID, GDI_Drop_Troop_One_Id,
                                            0);
            }
            if (param == 5) {
            ...
            if (param == 6) {
                Commands->Send_Custom_Event(obj, sender, M00_SEND_OBJECT_ID, GDI_Drop_Troop_Two_Id,
                                            0);
            }
            if (param == 7) {
            ...
            if (param == 8) {
                Commands->Send_Custom_Event(obj, sender, M00_SEND_OBJECT_ID,
                                            GDI_Drop_Troop_Three_Id, 0);
            }
            if (param == 9) {
            ...
            if (param == 13) {
                ...
                Commands->Attach_Script(obj, "M00_Send_Object_ID",
                                        "1500017,10,1.0f"); // sending Havoc_Tank_Id to shuman.
            }
            if (param == 14) {
...
DECLARE_SCRIPT(MX0_A03_HUMVEE, "") // moves humvee
{
    ...
    void Created(GameObject * obj)
    {
        ...
        // send id to controller.
        Commands->Attach_Script(obj, "M00_Send_Object_ID",
                                "1400041,1,0"); // sending humvee id to A03 Controller.

        // send id to shuman fool.
        Commands->Attach_Script(obj, "M00_Send_Object_ID",
                                "1500017,1,1.0f"); // sending humvee id to shuman
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if (type == 3) {
            Commands->Debug_Message(
                "***** DAK ***** Humvee: Recieved custom to move to section 4.\n");
...
DECLARE_SCRIPT(MX0_A03_TANK, "") // moves tank
{
    ...
    void Created(GameObject * obj)
    {
        ...
        // send id to controller.
        Commands->Attach_Script(obj, "M00_Send_Object_ID",
                                "1400041,9,0"); // sending humvee id to A03 Controller.

        // send Id to shuman
        Commands->Attach_Script(obj, "M00_Send_Object_ID",
                                "1500017,3,1.0f"); // sending tank id to A03 Controller.
        Commands->Attach_Script(obj, "M00_Send_Object_ID",
                                "1500020,3,1.0f"); // sending tank id to A03 Controller.
...
DECLARE_SCRIPT(MX0_A03_NOD_BUGGIE,
               "") { void Created(GameObject * obj) {}
                     ...
                     void Custom(GameObject * obj, int type, int param,
                                 GameObject* sender) { if (type == 1) { // move buggie
                                                                        ActionParamsStruct params;
...
DECLARE_SCRIPT(MX0_A03_GDI_TROOPER_ONE, "")
{
    ...
    {
        Commands->Attach_Script(obj, "M00_Send_Object_ID",
                                "1500017,3,1.0f"); // sending Trooper_One_Id to shuman.
        Commands->Innate_Disable(obj);
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        if (type == CUSTOM_EVENT_VEHICLE_ENTERED) {
            if (first_time) // first time player enters tank. send trooper 1 ahead.
            {
                GameObject* controller = Commands->Find_Object(MX0_A03_CONTROLLER_ID);
                if (controller) {
                    // Commands->Debug_Message( "***** sending custom
                    // MX0_A03_CUSTOM_TYPE_START_ZONE\n" );
                    Commands->Send_Custom_Event(obj, controller, MX0_A03_CUSTOM_TYPE_START_ZONE, 0,
                                                0);
                }
                first_time = false;
            }
...
DECLARE_SCRIPT(MX0_A03_TROOPER_ONE_TEST, "") { void Created(GameObject * obj) {
    Commands->Attach_Script(obj, "M00_Send_Object_ID",
                            "1400041,11,0.0f"); // sending Trooper_One_Id to A03_Controller
}
}
;
