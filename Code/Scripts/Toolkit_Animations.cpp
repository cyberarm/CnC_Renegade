...
void M00_Controller_Animation_RMV(GameObject* obj, GameObjObserverClass* script,
                                  const char* animation_name, bool loop, int priority,
                                  int action_id, int drop_frame, const char* drop_object,
    ...
    if (drop_frame != 0) {
        if (drop_object != nullptr) {
            if (drop_bone != nullptr) {
                Commands->Send_Custom_Event(obj, obj, M00_CUSTOM_ANIMATION_DROP_OBJECT, 0,
                                            drop_frame / 30);
            }
        }
    }
    ...
    "Action_Priority:int, Action_ID:int, Animation:string, Loop=0:int, Debug_Mode=0:int")
{
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        SCRIPT_DEBUG_MESSAGE(
            ("M00_Animation_Play_RMV received custom type %d, param %d.\n", type, param));
...
DECLARE_SCRIPT(M00_Animation_Play_Drop_Object_RMV,
               "Start_Now=0:int, Receive_Type:int, Receive_Param_On=1:int, "
               "Receive_Param_Off=1:int, Action_Priority:int, Action_ID:int, Animation:string, "
               "Drop_Frame:int, Drop_Object:string, Drop_Bone:string, Debug_Mode=0:int")
{
    int priority, action_id;
    bool debug_mode;
    ...
    void Created(GameObject * obj)
    {
        ...
        if (Get_Int_Parameter("Start_Now")) {
            ...
            M00_Controller_Animation_RMV(obj, this, anim, 0, priority, action_id, frame, object,
                                         bone);
        }
    }
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        SCRIPT_DEBUG_MESSAGE(
            ("M00_Animation_Play_Drop_Object_RMV received custom type %d, param %d.\n", type,
             param));
        if (type == Get_Int_Parameter("Receive_Type")) {
            if (param == Get_Int_Parameter("Receive_Param_On")) {
                SCRIPT_DEBUG_MESSAGE(("M00_Animation_Play_Drop_Object_RMV ACTIVATED.\n"));
        ...
        if ((type == M00_CUSTOM_ANIMATION_DROP_OBJECT) && (param == 0)) {
            ...
            SCRIPT_DEBUG_MESSAGE(
                ("Object %s created by M00_Animation_Play_Drop_Object_RMV.\n", object));
        }
    }
};
    ...
    "Drop_Bone:string, Script_Name:string, Script_Params:string, Debug_Mode=0:int")
{
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if (type == Get_Int_Parameter("Receive_Type")) {
            if (param == Get_Int_Parameter("Receive_Param_On")) {
                SCRIPT_DEBUG_MESSAGE(
                    ("M00_Animation_Play_Drop_Object_Attach_Script_RMV ACTIVATED.\n"));
                M00_Controller_Animation_RMV(obj, this, anim, 0, priority, action_id, frame, object,
                                             bone);
            }
            if (param == Get_Int_Parameter("Receive_Param_Off")) {
                SCRIPT_DEBUG_MESSAGE(
                    ("M00_Animation_Play_Drop_Object_Attach_Script_RMV DEACTIVATED.\n"));
                Commands->Action_Reset(obj, (priority + 1));
            }
        }
        if ((type == M00_CUSTOM_ANIMATION_DROP_OBJECT) && (param == 0)) {
            ...
            SCRIPT_DEBUG_MESSAGE(
                ("Object %s created by M00_Animation_Play_Drop_Object_Attach_Script_RMV.\n",
                 object));
            ...
            SCRIPT_DEBUG_MESSAGE(("Script %s attached to new object by "
                                  "M00_Animation_Play_Drop_Object_Attach_Script_RMV.",
                                  script));
        }
    }
