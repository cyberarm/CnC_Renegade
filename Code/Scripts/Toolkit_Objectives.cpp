...
DECLARE_SCRIPT(M00_Objective_Controller_For_Objects_RMV,
               "Objective_ID:int, Objective_Type=1:int, Objective_Description_ID:int, "
               "Radar_Blip=1:int, Hidden=0:int, Object_ID:int, Custom_Type:int, "
               "Activate_Param=0:int, Unhide_Param=0:int, Success_Param:int, Failure_Param:int")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        if ((activate == 0) && (hidden == false)) {
            ...
            case 1:
                Commands->Add_Objective(objective_id, OBJECTIVE_TYPE_PRIMARY,
                                        OBJECTIVE_STATUS_PENDING, description_id);
                break;
            case 2:
                Commands->Add_Objective(objective_id, OBJECTIVE_TYPE_SECONDARY,
                                        OBJECTIVE_STATUS_PENDING, description_id);
                break;
            case 3:
                Commands->Add_Objective(objective_id, OBJECTIVE_TYPE_TERTIARY,
                                        OBJECTIVE_STATUS_PENDING, description_id);
                break;
            }
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        if ((type == custom_type) && (hidden == true)
            && (param == Get_Int_Parameter("Unhide_Param"))) {
...
DECLARE_SCRIPT(M00_Objective_Controller_For_Locations_RMV,
               "Objective_ID:int, Objective_Type=1:int, Objective_Description_ID:int, "
               "Radar_Blip=1:int, Hidden=0:int, Location:vector3, Custom_Type:int, "
               "Activate_Param=0:int, Unhide_Param=0:int, Success_Param:int, Failure_Param:int")
{
    int objective_id, objective_type, object_id, custom_type;
    int activate, success, failure;
...
DECLARE_SCRIPT(M00_Objective_Radar_Blip_On_Object_RMV,
    ...
    void Created(GameObject * obj) { bool activate = ((Get_Int_Parameter("Activate_Type") == 0)
                                                      && (Get_Int_Parameter("Activate_Param") == 0))
                                         ? true
                                         : false;
...
{
    int objective_id = Get_Int_Parameter("Objective_ID");
    if ((Get_Int_Parameter("Activate_Type") == type)
        && (Get_Int_Parameter("Activate_Param") == param)) {
        Commands->Set_Objective_Radar_Blip_Object(objective_id, obj);
    }
}
}
...
DECLARE_SCRIPT(M00_Objective_Controller_For_Objects_Multiple_Triggers_RMV,
               "Objective_ID:int, Objective_Type=1:int, Objective_Description_ID:int, "
               "Hidden=0:int, Custom_Type:int, Activate_Param=0:int, Number_Of_Triggers:int, "
               "Trigger_Param:int, Unhide_Param=0:int, Failure_Param:int")
{
