...
DECLARE_SCRIPT(M00_Object_Create_RMV,
               ...
               "Object_To_Create:string, Location:vector3, Facing=0.00:float, Debug_Mode=0:int")
{
    bool debug_mode;
    ...
    void Create_Object(void)
    {
        ...
        if (new_object) {
            ...
            SCRIPT_DEBUG_MESSAGE(
                ("ERROR - M00_Object_Create_RMV failed to create a new object!\n"));
        }
    }
};
...
DECLARE_SCRIPT(M00_Object_Create_Attach_Script_RMV,
               "Start_Now=0:int, Receive_Type:int, Receive_Param_On=1:int, "
               "Object_To_Create:string, Location:vector3, Facing=0.00:float, Debug_Mode=0:int, "
               "Script_To_Attach:string, Script_Params:string")
{
    bool debug_mode;
    ...
    void Create_Object(void)
    {
        ...
        if (new_object) {
            ...
            if (debug_mode) {
                Commands->Debug_Message(
                    "ERROR - M00_Object_Create_Attach_Script_RMV failed to create a new object!\n");
            }
        }
    }
...
DECLARE_SCRIPT(M00_Object_Destroy_Self_RMV,
               "Start_Now=1:int, Receive_Type=3:int, Receive_Param_On=1:int, "
               "Receive_Param_Off=0:int, Receive_Param_Activate:int, Debug_Mode=0:int")
{
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        if (debug_mode) {
            Commands->Debug_Message(
                "M00_Trigger_Destroy_Self_RMV received custom, type %d and param %d.\n", type,
                param);
        }
        if (type == receive_type) {
            if (param == receive_param_on) {
                ...
                if (debug_mode) {
                    Commands->Debug_Message(
                        "M00_Trigger_Destroy_Self_RMV on object %d made ACTIVE.\n",
                        Commands->Get_ID(obj));
                }
            }
            if (param == receive_param_off) {
                ...
                if (debug_mode) {
                    Commands->Debug_Message(
                        "M00_Trigger_Destroy_Self_RMV on object %d made INACTIVE.\n",
                        Commands->Get_ID(obj));
                }
            }
            if ((param == receive_param_activate) && (active)) {
                if (debug_mode) {
                    Commands->Debug_Message(
                        "M00_Trigger_Destroy_Self_RMV activated on object %d, destroying.\n",
                        Commands->Get_ID(obj));
                }
...
DECLARE_SCRIPT(M00_PowerUp_Create_When_Killed_JDG,
               "Preset_Name:string,Drop_Percentage=100:float,Create_At_Death_Pos=1:int,Position:"
               "vector3,Z_Offset=0.75:float,Spawn_Effect=0:int") {
 ...
 *		and weapon FX. (1 = flailing and smoke. 2 = Elec Twiching and smoke? 3 = choking and
 *green smoke. )
 *
 ***********************************************************************************************************/
...
DECLARE_SCRIPT(M00_Vehicle_Regen_DAK, "") {
    void Created(GameObject * obj) { Commands->Send_Custom_Event(obj, obj, 0, 0, 0);
}
...
DECLARE_SCRIPT(M00_Send_Object_ID, "Receiver_ID:int, Param=0:int, Delay=1.0f:int")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        if (receiver) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(receiver_id), M00_SEND_OBJECT_ID,
                                        parameter, delay);
        }
        else {
            Commands->Debug_Message(
                "M00_Send_Object_ID on object_id %d Warning! Receiver_ID %d does not exist! /n",
