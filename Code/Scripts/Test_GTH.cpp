...
** Drop_Height - float meters to add to the Z coord of the original object when creating the drop
*obj
** Probability - int between 1 and 100, chance that the object will be created
*/
DECLARE_SCRIPT(GTH_Drop_Object_On_Death,
               "Drop_Object=:string,Drop_Height=0.25:float,Probability=100:int") {
    void Killed(GameObject * obj,
                GameObject* killer) { const char* obj_name = Get_Parameter("Drop_Object");
...
**
** Params:
** Custom_Message - message id that turns this script on or off, use message ID's greater than
*10000!
** Drop_Object - name of the preset to create an instance of
** Drop_Height - float meters to add to the Z coord of the original object when creating the drop
*obj
...
DECLARE_SCRIPT(
    ...
    void Entered(GameObject * obj,
                 GameObject* enterer) { int message = Get_Int_Parameter("Enter_Message");
...
** Probability - integer between 1 and 100, chance on any given "Enter" that the object will be
*created
** Player_Type - type of player that can trigger integer, 0 = Nod, 1 = GDI, 2 = any
*/
DECLARE_SCRIPT(GTH_Create_Object_On_Enter,
               "Create_Object=:string,Position:vector3,Min_Delay=10:int,Max_Creations=1:int,"
               "Probability=100:int,Player_Type=2:int")
{
    bool script_enabled;
    int trigger_count;
...
protected:
    ...
    void Entered(GameObject * obj, GameObject * enterer)
    {
        ...
        // Inc our trigger count, if we're allowed to fire again, set a timer to re-enable the
        // script
        trigger_count++;
...
DECLARE_SCRIPT(GTH_On_Killed_Mission_Complete, "Success=1:int, Player_Type=2:int") {
    void Killed(GameObject * obj,
                GameObject* killer) { GTH_DEBUG_INT(0, "GTH_On_Killed_Mission_Complete::Killed\n");
...
**
*/
DECLARE_SCRIPT(GTH_Create_Objective,
               "Creation_Type=0:int,Objective_ID=0:int,Objective_Type=0:int,Short_Desc_ID=0:int,"
...
*/
DECLARE_SCRIPT(GTH_Objective_Complete_Enter_Kill_Poke,
...
** NOTE: this won't work on buildings, only things like turrets, characters, or vehicles so make
*your
...
DECLARE_SCRIPT(GTH_Enable_Spawner_On_Enter, "SpawnerID=0:int,Player_Type=2:int,Enable=1:int") {
    void Entered(GameObject * obj,
                 GameObject* enterer) { // Check player type, if it doesn't match, just return
                                        int our_player_type = Get_Int_Parameter("Player_Type");
if (our_player_type != 2) {
    int enter_player_type = Commands->Get_Player_Type(enterer);
    if (enter_player_type != our_player_type) {
...
** of your "flag", give it projectile collision and make a preset for it similar to the
*"Marker_Flag"
...
** - friend poking the flag sends it back to its initial position (only if its not carried by an
*enemy)
...
** Flag_Lost_Event_ID - custom event id to send when enemy team gets flag back to
*Enemy_Home_Position :-(
...
** Captures_Exceeded_Event_ID - custom event id to send when flag has been captured
*"Captures_Needed_To_Win" times.
    ...
    "Win_Object_To_Kill3=0:int,Win_Object_To_Kill4=0:int")
{
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        if (timer_id == CTF_UPDATE_TIMER) {
            ...
            // BMH 12/7/02 - Get_A_Star only returns the closest to the position.  So if someone
            // just sat on top of the flag that's the only star we'd ever check.  So check from a
            // random position within the pickup radius
            Vector3 my_pos = Commands->Get_Position(obj);
            ...
            if (captured_by_id == 0 && ctimer > 0) {
                ...
                if (dist > Get_Float_Parameter("Pickup_Radius")) {
                    ...
                    if (((((float)Commands->Get_Sync_Time()) / 1000.0) - capture_timer) > ctimer) {
                        ...
                        Commands->Set_Position(obj, home_position);
                        GTH_DEBUG_INT(0,
                                      "Flag timeout has been hit, respawning flag back at home.\n");
