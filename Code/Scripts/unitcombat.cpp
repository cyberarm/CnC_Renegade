               ...
               "Scoreboard_ID=0:int,Controller_ID=0:int,Script_Override=0:int,Soldier_Type=0:int")
{
    ...
    ************************************************************************************************/
    ...
    void Enemy_Seen(GameObject * obj, GameObject * enemy)
    {
        if (script_override > 0) {
            GameObject* controller = Commands->Find_Object(controller_id);
            if (controller) {
                Commands->Send_Custom_Event(obj, Commands->Find_Object(controller_id), OV_ENEMYSEEN,
                                            0);
            }
            return;
        }
    ...
    void Killed(GameObject * obj, GameObject * killer)
    {
        ...
        //		Commands->Create_Instant_Logical_Sound(
        //(CombatSoundType)(SOUND_DEATH), 30.0, obj, Commands->Get_Position( obj ) );
    ...
    void Sound_Heard(GameObject * obj, const CombatSound& sound)
    {
        if (script_override > 0) {
            ...
            //Commands->Find_Object(controller_id), OV_SOUNDHEARD, 0);
            return;
        }
    ...
    */
    ...
    void Function_Circle_Attack(GameObject * obj, GameObject * Enemy, float Accuracy, float Range,
                                float Radius)
    {
        ...
        //		Commands->Enable_Sound_Heard( Me, false);
        //		Commands->Create_Instant_Logical_Sound(
        //(CombatSoundType)(SOUND_ENEMY_SEEN), 20.0, Me, Commands->Get_Position( Me ) );
    ...
    void Function_Crouch_Attack(GameObject * obj, GameObject * Enemy, float Accuracy, float Range,
                                float Duration)
    {
        state = STATE_OFFENSIVE;
    ...
    /*
            void Score_Hit_Tally( GameObject * obj )
            {
                    ...
                    if ( scoreboard )
                    {
                            if ( direction == false )
                            {
                                    Commands->Send_Custom_Event ( obj, scoreboard, HITDIRECTION,
       HD_FRONTHIT );
                            }
                            else if ( direction == true )
                            {
       ...
       HD_BACKHIT );
                            ...
                            if (!strcmp(damaged_bone,"CHEADD"))
                            {
                                    Commands->Send_Custom_Event ( obj, scoreboard, BODYLOCATION,
       BL_HEAD );
                            }
