...
DECLARE_SCRIPT(PDS_Get_In_Vehicle_Do_Waypath,
               "VehicleID=:int,WaypathID=:int,V3Test=1.3 44.543 0:vector3")
{
    ...
    void Action_Complete(GameObject * game_obj, int action_id, ActionCompleteReason complete_reason)
    {
        ...
        if (m_StartedWaypath == false) {
            //			Commands->Action_Movement_Follow_Waypath (game_obj,
            //Get_Int_Parameter(1), 0, 0, true);
            ActionParamsStruct params;
            params.Set_Basic(this, 50, 0);
            params.Set_Movement(Vector3(0, 0, 0), 1, 1);
...
DECLARE_SCRIPT(PDS_Test_Follow_Waypath, "WaypathID=:int,WaypointStartID=:int,WaypointEndID=:int") {
    void Created(GameObject * game_obj) { Commands->Grant_Key(game_obj, 0);
...
void Timer_Expired(GameObject* game_obj, int timer_id)
{
    //		Commands->Action_Movement_Follow_Waypath (game_obj, Get_Int_Parameter(0),
    //Get_Int_Parameter(1), Get_Int_Parameter(2), true);
    ...
    void Drive_To_Obj(int obj_id, bool force_backup)
    {
        ...
        if (dest_obj != NULL) {
            ...
            if (force_backup == false) {
                //				Commands->Action_Movement_Goto_Location (m_GameObj,
                //pos, 1.0F);
                ActionParamsStruct params;
                params.Set_Basic(this, 50, 0);
                params.Set_Movement(pos, 1, 1);
                ...
                //				Commands->Action_Movement_Backup_Goto_Location
                //(m_GameObj, pos, 1.0F);
                ActionParamsStruct params;
                params.Set_Basic(this, 50, 0);
                params.Set_Movement(pos, 1, 1);
...
DECLARE_SCRIPT(PDS_Test_Gunboat, "")
{
    ...
    void Start_Attack(void)
    {
        if (State != STATE_IN_TRANSIT) {
            // Commands->Start_Timer (GameObj, this, Commands->Get_Random (10.0F, 12.0F),
            // TIMER_ATTACK);
        }
