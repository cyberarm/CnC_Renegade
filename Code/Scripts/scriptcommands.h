...
typedef struct
{
    ...
    // Weapons
    void (*Trigger_Weapon)(GameObject* obj, bool trigger, const Vector3& target,
                           bool primary = true);
    void (*Select_Weapon)(GameObject* obj, const char* weapon_name);

    // Custom Script
    void (*Send_Custom_Event)(GameObject* from, GameObject* to, int type = 0, int param = 0,
                              float delay = 0);
    ...
    void (*Set_Animation)(GameObject* obj, const char* anim_name, bool looping,
                          const char* sub_obj_name = nullptr, float start_frame = 0.0F,
                          float end_frame = -1.0F, bool is_blended = false);
    ...
    int (*Create_Sound)(const char* sound_preset_name, const Vector3& position,
                        GameObject* creator);
    int (*Create_2D_Sound)(const char* sound_preset_name);
    int (*Create_2D_WAV_Sound)(const char* wav_filename);
    int (*Create_3D_WAV_Sound_At_Bone)(const char* wav_filename, GameObject* obj,
                                       const char* bone_name);
    int (*Create_3D_Sound_At_Bone)(const char* sound_preset_name, GameObject* obj,
                                   const char* bone_name);
    int (*Create_Logical_Sound)(GameObject* creator, int type, const Vector3& position,
                                float radius);
    ...
    // Display Text
    void (*Set_Display_Color)(unsigned char red = 255, unsigned char green = 255,
    ...
    // Explosions
    void (*Create_Explosion)(const char* explosion_def_name, const Vector3& pos,
                             GameObject* creator = nullptr);
    void (*Create_Explosion_At_Bone)(const char* explosion_def_name, GameObject* object,
                                     const char* bone_name, GameObject* creator = nullptr);
    ...
    void (*Set_Innate_Soldier_Home_Location)(GameObject* obj, const Vector3& home_pos,
    ...
    void (*Add_Objective)(int id, int type, int status, int short_description_id,
                          char* description_sound_filename = nullptr, int long_description_id = 0);
    ...
    void (*Set_Objective_HUD_Info)(int id, float priority, const char* texture_name,
                                   int message_id);
    void (*Set_Objective_HUD_Info_Position)(int id, float priority, const char* texture_name,
                                            int message_id, const Vector3& position);

    // Camaera Shakes
    void (*Shake_Camera)(const Vector3& pos, float radius = 25, float intensity = 0.25f,
                         float duration = 1.5f);
    ...
    void (*Attach_To_Object_Bone)(GameObject* object, GameObject* host_object,
                                  const char* bone_name);

    // Conversation
    int (*Create_Conversation)(const char* conversation_name, int priority = 0, float max_dist = 0,
                               bool is_interruptable = true);
    void (*Join_Conversation)(GameObject* object, int active_conversation_id,
                              bool allow_move = true, bool allow_head_turn = true,
                              bool allow_face = true);
    void (*Join_Conversation_Facing)(GameObject* object, int active_conversation_id,
                                     int obj_id_to_face);
