    ...
    void Killed(GameObject * obj, GameObject * killer)
    {
        ...
        case 0: {
            ...
            if (spawn_objectarmor) {
                Commands->Attach_Script(spawn_objectarmor, "M00_Create_Anim_Effect_DAY",
                                        "C4_EX1A.C4_EX1A");
            }
...
DECLARE_SCRIPT(DME_Test_Ejected_Soldier, "") { void Killed(GameObject * obj, GameObject* killer) {
...
DECLARE_SCRIPT(DME_Test_Worker_Wander, "Work_Area=3:int")
{
    ...
    void Custom(GameObject * obj, int type, int param,
                GameObject* sender) // recieves custom from dave's arrow as to if the location is
                                    // occupied or not.
    { // 100 not occupied, 200 is occupied.
        if (type == DME_OCCUPIED && param == 100) {
            Commands->Send_Custom_Event(
                obj, sender, 70, 70); // if not occupied, sends custom to set status to occupied.
        }
    ...
    void Worker_Loop(GameObject
                     * obj) // this is the main function that checks if the location is occupied
    ...
    void Action_Complete(GameObject * obj, int action_id,
                         ActionCompleteReason complete_reason) // this is called when the servant
                                                               // reaches the destination location.
    ...
    void Created(
        GameObject
        * obj) // this is the created function for the servant, it sets up most of the variables.
    {
    ...
    void Timer_Expired(GameObject * obj, int timer_id)
    {
        if (timer_id == 500) // this timer resets the servant from his animation loop and calls
                             // Worker_Loop to send him to a new location.
        {
            anim_playing = false;
        ...
        else if (timer_id == 510) // this timer resets the servant after seeing the star.  He
                                  // completes his animation and returns to work.
        {
            star_seen = false;
    ...
    void Enemy_Seen(GameObject * obj,
                    GameObject * enemy) // this function sets the servant in a "don't hurt me!"
                                        // stance (based on a timer)
    {
        if (star_seen == false) {
    ...
    void Poked(GameObject * obj,
               GameObject * poker) // function runs from a pre-defined random poke_choice.
    {
        if (poked == !true) {
            ...
            case 1: // call for guards
            {
                ...
                Commands->Join_Conversation(obj, conv_id, false, false);
                Commands->Start_Conversation(conv_id, 1);
                Commands->Create_Logical_Sound(obj, SOUND_TYPE_GUNSHOT, Commands->Get_Position(obj),
                                               75.0f);
            ...
            case 3: // give assistance
            {
                if (reward_given == false) {
                    ...
                    Vector3 pos = Commands->Get_Position(
                        obj); // specifies drop location and plays droping anim.
                    float facing = Commands->Get_Facing(obj);
...
DECLARE_SCRIPT(test_Ssm_Trigger,
    ...
    if (damager == STAR) { Commands->Send_Custom_Event(obj, obj, M00_LAUNCH_SSM, 0);
}
}
}
;
