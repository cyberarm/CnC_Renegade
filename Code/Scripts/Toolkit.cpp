...
/* GENERAL TOOLKIT INFORMATION
  ...
  Start_Now			Set this to 1 to activate upon creation.	Triggers and
  Controllers Receive_Type		Custom expected for activation/deactivation	Triggers and
  Controllers Receive_Param_On	Custom Parameter to activate this script	Triggers and
  Controllers Receive_Param_Off	Custom Parameter to deactivate this script	Triggers and
  Controllers
  ...
  Action_ID			The ID of the action applied.				Action
  Controllers Only

  (varies)			Various required parameters for the script	Triggers and
  Controllers
...
DECLARE_SCRIPT(M00_Debug_Text_File_RMV, "Description=Object:string, Filename=DebugLog.txt:string")
{
    ...
    {
        char* reasons[] = { "ACTION_COMPLETE_NORMAL",
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        fprintf(file,
                ...
                " %3.1f sec.\n",
                desc, Commands->Get_ID(obj), type, param, Commands->Get_ID(sender),
    ...
    void Sound_Heard(GameObject * obj, const CombatSound& sound)
    {
        ...
        //Commands->Get_ID(obj), current_time);
    }
...
DECLARE_SCRIPT(M00_Enable_Physical_Collision_JDG,
               "") { void Created(GameObject * obj) { Commands->Enable_Collisions(obj);
}
}
;
...
/***********************************************************************************************************
The two following scripts are a controller and a script for a gerneric barrel explosion chain
rection. You only need to place the controller in your level and enter the Vector3's and barrel
types as editor parameters See Joe G for further details
****************************************************************************************************************/

DECLARE_SCRIPT(M00_5MetalBarrels_ChainRxn_Controller_JDG,
               ...
               "Barrel05_Location:vector3, Barrel05_Type (1-8):int, Barrel05_Facing = 0:float")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        char* simple_barrels[8] = {
            "Simple_MetalDrum_01", "Simple_MetalDrum_02", "Simple_MetalDrum_03",
            "Simple_MetalDrum_04", "Simple_MetalDrum_05", "Simple_MetalDrum_06",
            "Simple_MetalDrum_07", "Simple_MetalDrum_08",
        };

        char* barrels_animations[8] = {
            "DSP_METDRUM01.DSP_METDRUM01", "DSP_METDRUM02.DSP_METDRUM02",
            "DSP_METDRUM03.DSP_METDRUM03", "DSP_METDRUM04.DSP_METDRUM04",
            "DSP_METDRUM05.DSP_METDRUM05", "DSP_METDRUM06.DSP_METDRUM06",
        ...
        GameObject* barrel_01
            = Commands->Create_Object(simple_barrels[barrel_01_type], barrel_01_location);
        ...
        GameObject* barrel_02
            = Commands->Create_Object(simple_barrels[barrel_02_type], barrel_02_location);
        ...
        GameObject* barrel_03
            = Commands->Create_Object(simple_barrels[barrel_03_type], barrel_03_location);
        ...
        GameObject* barrel_04
            = Commands->Create_Object(simple_barrels[barrel_04_type], barrel_04_location);
        ...
        GameObject* barrel_05
            = Commands->Create_Object(simple_barrels[barrel_05_type], barrel_05_location);
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        if (type == 0) {
            if (param == M01_IVE_BEEN_KILLED_JDG) {
                ...
                else if (sender == barrel_02) // barrel 02 has been destroyed--tell #1 and #3 to
                                              // self destruct
                {
                    Commands->Send_Custom_Event(obj, barrel_02, barrel_02_type,
                                                M01_MODIFY_YOUR_ACTION_JDG, 0);
                ...
                else if (sender == barrel_03) // barrel 03 has been destroyed--tell #2 and #4 to
                                              // self destruct
                {
                    Commands->Send_Custom_Event(obj, barrel_03, barrel_03_type,
                                                M01_MODIFY_YOUR_ACTION_JDG, 0);
                ...
                else if (sender == barrel_04) // barrel 04 has been destroyed--tell #3 and #5 to
                                              // self destruct
                {
                    Commands->Send_Custom_Event(obj, barrel_04, barrel_04_type,
                                                M01_MODIFY_YOUR_ACTION_JDG, 0);
                    ...
                    if (barrel_05 != nullptr) {
                        ...
                        Commands->Send_Custom_Event(obj, barrel_05, barrel_05_type,
                         ...
                         == barrel_05) // barrel 05 has been destroyed--tell #4 to self destruct
                {
                    Commands->Send_Custom_Event(obj, barrel_05, barrel_05_type,
                                                M01_MODIFY_YOUR_ACTION_JDG, 0);
...
DECLARE_SCRIPT(M00_ChainRxn_Barrel_JDG, "Controller_ID :int")
{
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        if (param == M01_MODIFY_YOUR_ACTION_JDG
            && deadYet == false) // you've been ordered to blow up--DO IT!
        {
            deadYet = true;
            int number = type;
...
DECLARE_SCRIPT(M00_Advanced_Guard_Tower_Gun, "")
{
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        else if (type == 3) // AGT has been killed...reset your action and prevent further firing --
                            // 02/12/2002 JDG
        {
            agt_is_dead = true;
            Commands->Action_Reset(obj, 100.0f);
    ...
    void Created(GameObject * obj)
    {
        ...
        if (obelisk) {
            ...
            Commands->Send_Custom_Event(obj,
                                        Commands->Find_Object(Get_Int_Parameter("Controller_ID")),
                                        M00_OBELISK_WEAPON_ID, Commands->Get_ID(obelisk), 0);
        }
    }
...
DECLARE_SCRIPT(M00_Obelisk_Weapon_CNC, "")
{
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        ...
        else if (type == 3) // obelisk has been killed, reset action and prevent further firing --
                            // 02/12/2002 JDG
        {
    ...
    void Created(GameObject * obj)
    {
        ...
        if (obelisk) {
            ...
            Commands->Send_Custom_Event(obj, Commands->Find_Object(controller_id),
                                        M00_OBELISK_WEAPON_ID, Commands->Get_ID(obelisk), 4.0f);
        }
    }
...
DECLARE_SCRIPT(M00_Change_L3Mutant_RadarMarker_JDG, "") { void Created(GameObject * obj) {
...
DECLARE_SCRIPT(M00_SSM_DLS, "")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        GameObject* ssm_missile
            = Commands->Create_Object_At_Bone(obj, "Saveable_Invisible_Object", "V_LAUNCHER");
...
DECLARE_SCRIPT(M00_Generic_Conv_DME, "ConvName:string") { void Created(GameObject * obj) {
...
DECLARE_SCRIPT(M00_Damage_Modifier_DME,
               "Damage_multiplier:float, Star_Modifier=1:int, NotStar_Modifier=1:int, "
...
DECLARE_SCRIPT(M00_Base_Defense,
