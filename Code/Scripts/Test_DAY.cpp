...
DECLARE_SCRIPT(DAY_TestScriptOne, "")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        params.Set_Movement(Commands->Get_Position(Commands->Find_Object(100077)), 0.6f, 1.0f,
                            false);
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        if (action == 1) {
            ...
            params.Set_Movement(Commands->Get_Position(Commands->Find_Object(100078)), 0.6f, 1.0f,
                                false);
            params.Set_Attack(Commands->Find_Object(100079), 40.0f, 0.0f, true);
            params.AttackCrouched = true;
            params.MoveCrouched = true;
        ...
        if (type == 1) {
            if (param == 1) {
                ...
                if (target) {
                    ActionParamsStruct params;
                    params.Set_Basic(this, 99, 1);
                    params.Set_Movement(Get_Circle_Position(obj, target, 5.0f, 0.1f), 1.0f, 1.0f,
                                        false);
...
DECLARE_SCRIPT(M00_DestroyedStateObject_DAY,
...
DECLARE_SCRIPT(M00_Play_Sound_Object_Bone_DAY,
               "Sound_Preset:string, Frequency_Min=-1.0:float, Frequency_Max:float") {
    void Created(GameObject
                 * obj) { if (Get_Int_Parameter("Frequency_Min") == -1) { Timer_Expired(obj, 0);
}
else
{
