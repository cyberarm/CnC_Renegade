...
DECLARE_SCRIPT(M00_Sound_Play_2D_RAD,
               "Start_Now=0:int, Receive_Type:int, Receive_Param_On=1:int, "
               "Receive_Param_Off=0:int, Sound_Effect:string, Play_Count=1:int, "
               "Sound_Delay_Min=0.1:float, Sound_Delay_Max=0.1:float, Debug_Mode=0:int")
{
    int play_count;
    int current_count;
    ...
    void Custom(GameObject * obj, int type, int param, GameObject* sender)
    {
        SCRIPT_DEBUG_MESSAGE(
            ("M00_Sound_Play_2D_RAD received custom type %d, param %d.\n", type, param));
...
DECLARE_SCRIPT(M00_Sound_Play_3D_On_Object_RAD,
               "Start_Now=0:int, Receive_Type:int, Receive_Param_On=1:int, "
               "Receive_Param_Off=0:int, Object_ID:int, Sound_Effect:string, Play_Count=1:int, "
               "Sound_Delay_Min=0.1:float, Sound_Delay_Max=0.1:float, Debug_Mode=0:int")
{
    int play_count;
    int current_count;
    ...
    {
        SCRIPT_DEBUG_MESSAGE(
            ("M00_Sound_Play_3D_On_Object_RAD received custom type %d, param %d.\n", type, param));
        if (type == Get_Int_Parameter("Receive_Type")) {
            if (param == Get_Int_Parameter("Receive_Param_On")) {
                SCRIPT_DEBUG_MESSAGE(("M00_Sound_Play_3D_On_Object_RAD ACTIVATED.\n"));
                if ((play_count > 1) || (!play_count)) {
                    delay_time = 0.1f;
                    Commands->Start_Timer(obj, this, delay_time,
                                          M00_TIMER_SOUND_PLAY_3D_ON_OBJECT_RAD);
                }
                else {
    ...
    "Origin:vector3, Debug_Mode=0:int")
{
    ...
    {
        SCRIPT_DEBUG_MESSAGE((
            "M00_Sound_Play_3D_At_Location_RMV received custom type %d, param %d.\n", type, param));
        if (type == Get_Int_Parameter("Receive_Type")) {
            if (param == Get_Int_Parameter("Receive_Param_On")) {
                SCRIPT_DEBUG_MESSAGE(("M00_Sound_Play_3D_At_Location_RMV ACTIVATED.\n"));
                if (play_count > 1) {
...
DECLARE_SCRIPT(
    ...
    "Start_Now=0:int, Receive_Type:int, Receive_Param_On=1:int, Receive_Param_Off=0:int, "
    "Object_ID:int, Bone_Name:string, Sound_Effect:string, Play_Count=1:int, "
    "Sound_Delay_Min=0.1:float, Sound_Delay_Max=0.1:float, Debug_Mode=0:int")
{
    int play_count;
    int current_count;
    ...
    {
        SCRIPT_DEBUG_MESSAGE(
            ("M00_Sound_Play_3D_At_Bone_RMV received custom type %d, param %d.\n", type, param));
        if (type == Get_Int_Parameter("Receive_Type")) {
            if (param == Get_Int_Parameter("Receive_Param_On")) {
                SCRIPT_DEBUG_MESSAGE(("M00_Sound_Play_3D_At_Bone_RMV ACTIVATED.\n"));
                if (play_count > 1) {
                    ...
                    if (target_object) {
                        const char* sound_effect;
                        sound_effect = Get_Parameter("Sound_Effect");
                        my_position = Commands->Get_Bone_Position(target_object,
                                                                  Get_Parameter("Bone_Name"));
                        M00_Controller_Sound_RAD(target_object, my_position, sound_effect, 0);
                    }
                }
            }
        ...
        if (timer_id == M00_TIMER_SOUND_PLAY_3D_ON_OBJECT_RAD) {
            ...
            if (current_count <= play_count) {
                ...
                if (target_object) {
                    ...
                    sound_effect = Get_Parameter("Sound_Effect");
                    my_position
                        = Commands->Get_Bone_Position(target_object, Get_Parameter("Bone_Name"));
                    M00_Controller_Sound_RAD(target_object, my_position, sound_effect, 0);
                }
...
DECLARE_SCRIPT(M00_Sound_Building_Ambient_RAD,
               "Building_ID=0:int, Building_Type=0:int, Building Facing=0:int") {

};

DECLARE_SCRIPT(RMV_Audio_Timer_Delay,
               "Target_ID:int, Custom_Type=0:int, Custom_Param=0:int, Delay_Min=0.0:float, "
...
DECLARE_SCRIPT(RMV_Audio_Sound_Player_WAV,
...
DECLARE_SCRIPT(RMV_Sound_Play_Near_Player,
               "Receive_Type:int, Receive_Param:int, Max_Offset:vector3, Sound_Preset:string, "
               "Frequency_Min:float, Frequency_Max=0.0:float")
{
    ...
    {
        if ((type == Get_Int_Parameter("Receive_Type"))
            && (param == Get_Int_Parameter("Receive_Param"))) {
            enabled = !enabled;
        }
...
DECLARE_SCRIPT(
    ...
    "Sound_Normal:string,Sound_Destroyed:string,BuildingController_ID:int,Is_3D=1:int,Offset:"
    "vector3,Offset_Randomness:vector3,Frequency_Min=-1:float,Frequency_Max:float,Is_3D_Destroyed="
    "1:int,Offset_Destroyed:vector3,Offset_Randomness_Destroyed:vector3,Frequency_Min_Destroyed=-1:"
    "float,Frequency_Max_Destroyed:float, Explosion_Name:string")
{
    ...
    void Created(GameObject * obj)
    {
        ...
        building_explode
            = true; // set play building explosions to true for when building is destroyed.
        ...
        if (Get_Int_Parameter("Frequency_Min") == -1) {
            ...
            float time = Commands->Get_Random(Get_Float_Parameter("Frequency_Min"),
        ...
        if (type == CUSTOM_EVENT_SOUND_ENDED) {
            if (building_destroyed == false) {
                ...
                if (Get_Int_Parameter("Frequency_Min_Destroyed") != -1) {
                    float time
                        = Commands->Get_Random(Get_Float_Parameter("Frequency_Min_Destroyed"),
...
DECLARE_SCRIPT(M00_BuildingStateSoundController, "BuildingSpeaker_ID:int")
{
    ...
    void Killed(GameObject * obj, GameObject * killer)
    {
        ...
        if (target) {
            Commands->Send_Custom_Event(obj, target, M00_CUSTOM_SOUND_BUILDING, 1,
                                        0.0f); // Play Destroyed Sounds
            Commands->Send_Custom_Event(obj, target, M00_CUSTOM_BUILDING_EXPLODE, 1,
                                        0.0f); // Play Explosion Sounds
        }
    }
