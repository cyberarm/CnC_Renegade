...
//
//////////////////////////////////////////////////////////////////////////
static inline bool Is_Numerical(char ch)
{
...
void VisPointGeneratorClass::Submit_Mesh(MeshClass& mesh)
{
    ...
    Vector3 offset(-(box.Center.X - box.Extent.X), -(box.Center.Y - box.Extent.Y),
    ...
    for (int cell = 0; cell < cell_count; cell++) {
        ...
        if (vis_point != nullptr) {
            ...
            //
            Matrix3D world_to_cam_tm(Vector3(0, -1, 0), Vector3(0, 0, 1), Vector3(-1, 0, 0),
                                     Vector3(0, 0, 0));
...
//
/////////////////////////////////////////////////////////////////////////
void VisPointGeneratorClass::Subdivide_And_Submit_Centers(const Vector3& point1,
...
//
/////////////////////////////////////////////////////////////////////////
bool VisPointGeneratorClass::Find_Valid_Points(const Vector3& start, const Vector3& end,
                                               const Matrix3& orientation, bool do_camera_ring)
{
    bool retval = false;
    ...
    for (int attempt = 0; (attempt < max_attempts) && !retval; attempt++) {
        ...
        while (!done) {
            ...
            PhysRayCollisionTestClass raytest(ray, &res, GAME_COLLISION_GROUP,
                                              COLLISION_TYPE_PHYSICAL | COLLISION_TYPE_VIS);
            ...
            if (res.Fraction == 1.0f) {
                ...
                bool is_vis_sector = (physobj->As_StaticPhysClass()
                                      && physobj->As_StaticPhysClass()->Is_Vis_Sector());

                if ((is_vis_sector) || (res.Normal.Z < 0.0f)) {

                    //
                    // If we hit a roof or another vis sector, snap the end point down and signal
                    // that we're done
                    //
                    end_point.Z = point.Z;
                    ok_to_sample = true;
...
bool VisPointGeneratorClass::Is_Grid_Cell_Empty(const Vector3& position)
{
    ...
    return (m_Grid.Get_At(grid_pos.X, grid_pos.Y, grid_pos.Z) == nullptr);
}
...
bool VisPointGeneratorClass::Submit_Point(const Matrix3D& vis_transform, bool do_camera_ring)
{
    ...
    if ((cell_contents == nullptr)
        || ((cell_contents->m_Transform.Get_Translation() - cell_center).Length()
            > new_point_dist)) {
...
bool VisPointGeneratorClass::Do_View_Planes_Pass(const Matrix3D& vis_transform)
{
    ...
    // result.Reset ();
    CastResultStruct result;
    PhysAABoxCollisionTestClass collision_test(box, Vector3(0, 0, 0), &result, GAME_COLLISION_GROUP,
                                               COLLISION_TYPE_ALL);
...
//
/////////////////////////////////////////////////////////////////////////
bool VisPointGeneratorClass::Test_Camera_Sim_Point(const Vector3& start_point,
                                                   ...
                                                   NodeClass** node_result)
{
    ...
    //
    CastResultStruct result;
    PhysAABoxCollisionTestClass collision_test(box, sweep_vector, &result, GAME_COLLISION_GROUP,
                                               COLLISION_TYPE_ALL);
    SceneEditorClass* scene = ::Get_Scene_Editor();
    scene->Cast_AABox(collision_test);
...
NodeClass* VisPointGeneratorClass::Find_Floor_Node(const Vector3& start_point)
{
    ...
    if (res.Fraction < 1.0F) {
        ...
        if (physobj != nullptr) {
            ...
            //
            HITTESTINFO* hittest = (HITTESTINFO*)physobj->Peek_Model()->Get_User_Data();
            if ((hittest != nullptr) && (hittest->Type == HITTESTINFO::Node)) {
                //
...
bool VisPointGeneratorClass::Is_Object_Invalid_Roof(RenderObjClass* render_obj)
{
    ...
    if (render_obj->Class_ID() == RenderObjClass::CLASSID_MESH) {
        ...
        if (model != nullptr) {
            ...
            // retval &= ((render_obj->Get_Collision_Type () & COLLISION_TYPE_VIS) ==
            // COLLISION_TYPE_VIS);
            retval &= ((render_obj->Get_Collision_Type() & COLLISION_TYPE_PHYSICAL)
...
void VisPointGeneratorClass::Generate_Camera_Locations(const Matrix3D& real_vis_point)
{
    ...
    for (int index = 0; index < m_CameraSimPointCount; index++) {
        ...
        //
        Test_Camera_Sim_Point(position, end_point, &sim_points[index].vis_point,
...
//
/////////////////////////////////////////////////////////////////////////
void VisPointGeneratorClass::Post_Process_Nodes(void)
{
    return;
}
...
void VisPointGeneratorClass::Add_Manual_Nodes(void)
{
    ...
    for (	vis_point = (VisPointNodeClass *)NodeMgrClass::Get_First (NODE_TYPE_VIS_POINT);
                    vis_point != nullptr;
                    vis_point = (VisPointNodeClass *)NodeMgrClass::Get_Next (vis_point,
    NODE_TYPE_VIS_POINT))
    {
            //
    ...
    Vector3 (0, 0, 0)); Matrix3D new_transform = transform * cam_transform;
