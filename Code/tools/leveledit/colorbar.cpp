...
// Create
//
BOOL ColorBarClass::Create(LPCTSTR /*lpszClassName*/, LPCTSTR lpszWindowName, DWORD dwStyle,
                           const RECT& rect, CWnd* pparent_wnd, UINT nID,
...
)
{
    ...
    HWND hwnd = ::CreateWindow("WWCOLORBAR", lpszWindowName, dwStyle, rect.left, rect.top,
                               rect.right - rect.left, rect.bottom - rect.top, hparent_wnd,
                               (HMENU)nID, ::AfxGetInstanceHandle(), this);
...
void ColorBarClass::Create_Bitmap(void)
{
    ...
    m_hBitmap = ::CreateDIBSection(hscreen_dc, (const BITMAPINFO*)&bitmap_info, DIB_RGB_COLORS,
                                   (void**)&m_pBits, nullptr, 0L);
                ...
                for (int row = y_pos; row < (y_pos + height); row++) {
                    ...
                    if (percent < height_percent) {
                        ...
                        if (prev_was == false) {
                            ...
                            pbits[bitmap_index + bitmap_offset + 1]
                                = 128 + UCHAR(((int)((1 - height_percent) * 128)) & 0xFF);
                            pbits[bitmap_index + bitmap_offset + 2]
                                = UCHAR(((int)((1 - height_percent) * 255)) & 0xFF);
                        }
                        prev_was = true;
                    }
                    else {
...
void ColorBarClass::Paint_DIB(void)
{
    ...
    //
    if (style & CBRS_HORZ) {
        Paint_Bar_Horz(m_ColorArea.left, m_ColorArea.top, m_ColorArea.Width(), m_ColorArea.Height(),
                       m_pBits);
    }
    else {
        Paint_Bar_Vert(m_ColorArea.left, m_ColorArea.top, m_ColorArea.Width(), m_ColorArea.Height(),
                       m_pBits);
    }
...
bool ColorBarClass::Insert_Point(CPoint point, DWORD flags)
{
    ...
    for (int index = 0; (index < m_iColorPoints) && !found; index++) {
        if ((position > m_ColorPoints[index].StartPos)
            ...
            float graph_delta = (m_ColorPoints[index].EndPos - m_ColorPoints[index].StartPos)
                * m_ColorPoints[index].GraphPercentInc;
...
// Insert_Point
//
bool ColorBarClass::Insert_Point(int index, float position, float red, float green, float blue,
                                 DWORD flags)
{
    // Assume failure
    bool retval = false;
    ...
    if (((m_iColorPoints + 1) < MAX_COLOR_POINTS) && (index >= 0) && (index <= m_iColorPoints)) {
        ...
        ::memmove(&m_ColorPoints[index + 1], &m_ColorPoints[index],
...
bool ColorBarClass::Delete_Point(int index)
{
    ...
    if ((index > 0) && (index < m_iColorPoints)) {
        ...
        // Bump all the points up one position in the array
        ::memmove(&m_ColorPoints[index], &m_ColorPoints[index + 1],
...
// Modify_Point
//
bool ColorBarClass::Modify_Point(int index, float position, float red, float green, float blue,
                                 DWORD flags)
{
    // Assume failure
    bool retval = false;
...
void ColorBarClass::Update_Point_Info(void)
{
    ...
    if (style & CBRS_HORZ) {
        ...
        for (int index = 0; index < m_iColorPoints; index++) {
            ...
            m_ColorPoints[index].StartPos
                = m_ColorArea.left + int(m_ColorPoints[index].PosPercent * width);
            ...
            if (index > 0) {
                ...
                m_ColorPoints[index - 1].RedInc
                    = (m_ColorPoints[index].StartRed - m_ColorPoints[index - 1].StartRed)
                    / ((float)width);
                m_ColorPoints[index - 1].GreenInc
                    = (m_ColorPoints[index].StartGreen - m_ColorPoints[index - 1].StartGreen)
                    / ((float)width);
                m_ColorPoints[index - 1].BlueInc
                    = (m_ColorPoints[index].StartBlue - m_ColorPoints[index - 1].StartBlue)
                    / ((float)width);

                // Assign the graph increment to the previous point
                m_ColorPoints[index - 1].GraphPercentInc
                    = (m_ColorPoints[index].StartGraphPercent
                       - m_ColorPoints[index - 1].StartGraphPercent)
                    / ((float)width);
            }
        }
        ...
        for (int index = 0; index < m_iColorPoints; index++) {
            ...
            // Update the absolute starting position for this point
            m_ColorPoints[index].StartPos
                = m_ColorArea.top + int(m_ColorPoints[index].PosPercent * height);
...
void ColorBarClass::Load_Key_Frame_BMP(void)
{
    ...
    m_KeyFrameDIB = ::CreateDIBSection(hscreen_dc, (const BITMAPINFO*)&bitmap_info, DIB_RGB_COLORS,
                                       (void**)&m_pKeyFrameBits, nullptr, 0L);
