...
//
/////////////////////////////////////////////////////////////////////////////
static void Trackball_Camera(HWND hwnd, CameraClass& camera, const Vector3& center, POINT point,
                             POINT last_point);
...
BOOL TransitionEditDialogClass::OnInitDialog(void)
{
    ...
    //	Setup the toolbar
    //
    m_Toolbar.CreateEx(this, TBSTYLE_FLAT, WS_BORDER | WS_CHILD | WS_VISIBLE | CBRS_ALIGN_TOP,
                       CRect(0, 0, 10, 10), 101);
    ...
    ScreenToClient(&rect);
    m_Toolbar.SetWindowPos(nullptr, rect.left + (rect.Width() >> 1) - (size.cx >> 1),
                           rect.top + (rect.Height() >> 1) - (size.cy >> 1), rect.Width(),
...
void TransitionEditDialogClass::Render_View(void)
{
    ...
    if (!render_lock) {
        ...
        //
        m_SwapChain->Present(nullptr, nullptr, nullptr, nullptr);

        //
        //	Restore the render target
        //
        DX8Wrapper::Set_Render_Target((LPDIRECT3DSURFACE8)nullptr);
...
//
/////////////////////////////////////////////////////////////////////////////
void CALLBACK TransitionEditDialogClass::fnUpdateTimer(UINT uID, UINT uMsg, DWORD user_data,
                                                       DWORD dw1, DWORD dw2)
{
...
TransitionEditDialogClass::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    if (message == (WM_USER + 101)) {
        ...
        //
        if (wParam == VK_NUMPAD8 || wParam == VK_NUMPAD2 || wParam == VK_NUMPAD4
            || wParam == VK_NUMPAD6 || wParam == VK_NUMPAD7 || wParam == VK_NUMPAD1) {
            return 0;
        }
    }
...
void TransitionEditDialogClass::Handle_Keypress(void)
{
    ...
    if ((::GetAsyncKeyState(VK_UP) < 0) || (::GetAsyncKeyState(VK_DOWN) < 0)
        ...
        //
        if (m_IsRotatingChar
            && ((::GetAsyncKeyState(VK_RIGHT) < 0) || (::GetAsyncKeyState(VK_LEFT) < 0))) {
            Matrix3D tm = m_CharacterObj->Get_Transform();
...
void TransitionEditDialogClass::Update_Status(void)
{
    ...
    CString status;
    status.Format("Zone Pos (%.2f, %.2f, %.2f)\nZone Size (%.2f, %.2f, %.2f)", zone_pos.X,
                  zone_pos.Y, zone_pos.Z, zone_size.X, zone_size.Y, zone_size.Z);
    ...
    status.Format("Char Pos (%.2f, %.2f, %.2f)\nChar Rot (%.2f)", char_pos.X, char_pos.Y,
                  char_pos.Z, RAD_TO_DEG(char_rot));
...
void TransitionEditDialogClass::Load_Character(void)
{
    if (m_CharacterObj == nullptr) {
        ...
        //
        SoldierGameObjDef* definition
            = (SoldierGameObjDef*)DefinitionMgrClass::Find_Typed_Definition(
...
//
/////////////////////////////////////////////////////////////////////////////
LRESULT CALLBACK TransitionEditDialogClass::fn3DWindow(HWND hwnd, UINT message, WPARAM wparam,
                                                       LPARAM lparam)
{
...
void TransitionEditDialogClass::Handle_MOUSEMOVE(WPARAM wparam, LPARAM lparam)
{
    ...
    if ((wparam & MK_LBUTTON) && (wparam & MK_RBUTTON)) {
        ...
        if (::GetCapture() == ::GetDlgItem(m_hWnd, IDC_3D_WINDOW)) {
            ::Trackball_Camera(::GetDlgItem(m_hWnd, IDC_3D_WINDOW), *m_Camera, Vector3(0, 0, 0),
                               point, m_LastPoint);
        }
    }
...
//
/////////////////////////////////////////////////////////////////////////////
void Trackball_Camera(HWND hwnd, CameraClass& camera, const Vector3& center, POINT point,
                      POINT last_point)
{
    RECT rect;
...
BOOL TransitionEditDialogClass::PreTranslateMessage(MSG* pMsg)
{
    if (pMsg->message == WM_KEYDOWN || pMsg->message == WM_KEYUP) {
        ...
        if (pMsg->wParam == VK_NUMPAD8 || pMsg->wParam == VK_NUMPAD2 || pMsg->wParam == VK_NUMPAD4
            || pMsg->wParam == VK_NUMPAD6 || pMsg->wParam == VK_NUMPAD7
            || pMsg->wParam == VK_NUMPAD1 || pMsg->wParam == VK_UP || pMsg->wParam == VK_DOWN
