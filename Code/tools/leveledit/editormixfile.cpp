...
//
/////////////////////////////////////////////////////////////////////////
void EditorMixFileCreator::Substitute_File(const char* fullpath, const char* name,
                                           char* substitutefullpath, char* substitutename)
{
    bool substitutefile;
    ...
    if (TexturesCompressed) {
        ...
        if (_stricmp(extension, ".tga") == 0) {
            ...
            // NOTE 0: bump_*.* files are bump maps and cannot be compressed because the pixel data
            // is interpreted as bump data. NOTE 1: font*.* files are font files that are not
            // processed by the run-time compressed texture loading system and, therefore, must be
            // excluded.
            if ((_strnicmp(filename, exclusionstring0, strlen(exclusionstring0)) != 0)
                ...
                // OPTIMIZATION: Has the .DDS file already been created and stored in the texture
                // cache?

                // NOTE:	If the name contains a subdirectory mangle it with the filename.
                // This will ensure that the filename is unique.
                _splitpath(name, nullptr, mangleddirectory, mangledname, nullptr);

                // NOTE: The cached name must be the same as the original name but with .DDS
                // extension.
                ...
                tgafile = CreateFile(fullpath, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                                     0L, nullptr);
                ddsfile = CreateFile(cachedfullpath, GENERIC_READ, FILE_SHARE_READ, nullptr,
                ...
                if (tgafile != INVALID_HANDLE_VALUE) {
                    ...
                    if (ddsfile != INVALID_HANDLE_VALUE) {
                        ...
                        if ((tgawritetime.dwLowDateTime == ddswritetime.dwLowDateTime)
                            ...
                            // Files have a different time stamp (and therefore it is assumed that
                            // the	.TGA file has been modified since the .DDS file was
                            // created). Create a new .DDS file from the .TGA file and give it the
                        ...
                        // Create a new .DDS file from the .TGA file and give it the same time stamp
                        // as the .TGA file.
                        substitutefile = Convert_File(fullpath, cachedfullpath, &tgawritetime);
                    }
                }
...
//
/////////////////////////////////////////////////////////////////////////
bool EditorMixFileCreator::Convert_File(const char* fullpath, const char* cachedfullpath,
                                        FILETIME* tgawritetimeptr)
{
