    ...
    if (m_Definition != nullptr && m_Definition->Is_Valid_Config(error_message)) {
        ...
        case CLASSID_GAME_OBJECTS:
            ...
            if (class_id == CLASSID_SPAWNER_DEF) {
                ...
                ::AfxGetMainWnd()->MessageBox("You cannot place C4 objects in the editor.",
                                              "Non-Placeable Object", MB_ICONEXCLAMATION | MB_OK);
            }
            else {
...
bool PresetClass::Is_Valid_Sound_Preset(void)
{
    ...
    if (m_Definition != nullptr && m_Definition->Get_Class_ID() == CLASSID_SOUND) {
        ...
        if (filename.GetLength() > 0
            && (file_attrs == 0xFFFFFFFF || file_attrs != FILE_ATTRIBUTE_DIRECTORY)) {
            retval = true;
        }
    }
