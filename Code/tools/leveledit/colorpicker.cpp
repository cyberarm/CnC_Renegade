...
//
/////////////////////////////////////////////////////////////////////////////
BOOL ColorPickerClass::Create(LPCTSTR /*lpszClassName*/, LPCTSTR lpszWindowName, DWORD dwStyle,
...
)
{
    ...
    HWND hwnd = ::CreateWindow("WWCOLORPICKER", lpszWindowName, dwStyle, rect.left, rect.top,
...
void ColorPickerClass::Paint_DIB(int width, int height, UCHAR* pbits)
{
    ...
    for (int icol = rect.left; icol < rect.right; icol++) {
        ...
        if ((icol - rect.left) == channel_switch_cols[curr_channel]) {
            ...
            curr_channel++;
            curr_inc = (255.0F
                        / ((float)(channel_switch_cols[curr_channel]
                                   - channel_switch_cols[curr_channel - 1])));
...
void ColorPickerClass::Erase_Marker(void)
{
    ...
    if (m_hMemDC != nullptr) {
        ...
        CRect rect;
        GetClientRect(&rect);
        ::BitBlt(hdc, m_CurrentPoint.x - 5, m_CurrentPoint.y - 5, 11, 11, m_hMemDC,
                 m_CurrentPoint.x - 5, m_CurrentPoint.y - 5, SRCCOPY);
...
void ColorPickerClass::Paint_Marker(void)
{
    ...
    if (m_hMemDC != nullptr) {
        ...
        CRect rect;
        GetClientRect(&rect);
        ::BitBlt(hdc, m_CurrentPoint.x - 5, m_CurrentPoint.y - 5, 11, 11, m_hMemDC, 0, 0,
                 SRCINVERT);
