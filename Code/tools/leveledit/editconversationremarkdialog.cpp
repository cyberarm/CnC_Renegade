...
const ORATOR_BUTTON_INFO ORATOR_BUTTONS[ORATOR_MAX]
    = { { IDC_ORATOR1_RADIO, IDI_ORATOR1 }, { IDC_ORATOR2_RADIO, IDI_ORATOR2 },
        { IDC_ORATOR3_RADIO, IDI_ORATOR3 }, { IDC_ORATOR4_RADIO, IDI_ORATOR4 },
        { IDC_ORATOR5_RADIO, IDI_ORATOR5 }, { IDC_ORATOR6_RADIO, IDI_ORATOR6 },
...
BOOL EditConversationRemarkDialogClass::OnInitDialog(void)
{
    ...
    for (int index = 0; index < ORATOR_MAX; index++) {
        HICON icon
            = ::LoadIcon(::AfxGetResourceHandle(), MAKEINTRESOURCE(ORATOR_BUTTONS[index].icon_id));
