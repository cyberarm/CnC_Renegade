...
//
///////////////////////////////////////////////////////////////////////////////
bool TGAToDXTClass::Convert(const char* inputpathname, const char* outputpathname,
                            FILETIME* writetimeptr, bool& redundantalpha)
{
    bool success;
    Targa targa;
    long error;
    ...
    if (error == 0) {
        ...
        validaspect = ((float)MAX(targa.Header.Width, targa.Header.Height))
                / ((float)MIN(targa.Header.Width, targa.Header.Height))
            <= 8.0f;
        if (validbitdepth && validsize && validaspect) {
            ...
            if (targa.Header.PixelDepth == 32) {
                ...
                // Analyse the alpha channel and ignore it if it contains redundant data (ie. is
                ...
                if ((*(byte + 3) == 0x00) || (*(byte + 3) == 0xff)) {
                    ...
                    redundantalpha = true;
                    for (unsigned p = 0;
                         p < ((unsigned)targa.Header.Width) * ((unsigned)targa.Header.Height);
                         p++) {
                ...
                if (!redundantalpha) {
                    ...
                    errorcode
                        = ::nvDXTcompress((unsigned char*)targa.GetImage(), targa.Header.Width,
                                          targa.Header.Height, TF_DXT5, true, false, 4);
                }
                else {
                    ...
                    nonalphaimage = new unsigned char[3 * ((unsigned)targa.Header.Width)
                                                      * ((unsigned)targa.Header.Height)];
                    ...
                    errorcode = ::nvDXTcompress(nonalphaimage, targa.Header.Width,
                                                targa.Header.Height, TF_DXT1, true, false, 3);
                    delete[] nonalphaimage;
                }
            }
            else {

                errorcode = ::nvDXTcompress((unsigned char*)targa.GetImage(), targa.Header.Width,
                                            targa.Header.Height, TF_DXT1, true, false, 3);
            }
...
void TGAToDXTClass::Write(const char* outputpathname)
{
    ...
    hfile = ::CreateFile(outputpathname, GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS, 0L,
                         nullptr);
