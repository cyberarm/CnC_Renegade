...
void OverlapPageClass::Detect_Overlaps(HTREEITEM root_item, NODE_TYPE node_type)
{
    ...
    for (node1 = NodeMgrClass::Get_First(node_type); node1 != nullptr;
        ...
        for (node2 = NodeMgrClass::Get_First(node_type); node2 != nullptr;
...
OverlapPageClass::Insert_Node(HTREEITEM parent_item, NodeClass* node)
{
    ...
    HTREEITEM tree_item
        = m_TreeCtrl.InsertItem(node->Get_Name(), icon_index, icon_index, parent_item);
