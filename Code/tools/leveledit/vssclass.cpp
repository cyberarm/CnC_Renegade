...
Sys_String_To_ANSI(BSTR bstr)
{
    ...
    ::WideCharToMultiByte(CP_ACP, 0, bstr, len, ansi_string, len + 1, nullptr, nullptr);
...
bool IDispatch_Get_Property(LPDISPATCH pdispatch, DISPID dispid, VARIANT* presult)
{
    ...
    if (pdispatch != nullptr) {
        ...
        retval = SUCCEEDED(pdispatch->Invoke(dispid, IID_nullptr, nullptr, DISPATCH_PROPERTYGET, &no_args,
...
bool IDispatch_Get_Property(LPDISPATCH pdispatch, DISPID dispid, VARIANT& var_arg, VARIANT* presult)
{
    ...
    if (pdispatch != nullptr) {
        ...
        retval = SUCCEEDED(pdispatch->Invoke(dispid, IID_nullptr, nullptr, DISPATCH_PROPERTYGET, &args,
                                             presult, &excep_info, nullptr));
    }
...
void VSSClass::Get_VSS_Interface(void)
{
    if (m_pIVSSDatabase == nullptr) {
        ...
        HRESULT hresult = ::CoCreateInstance(CLSID_VSSDatabase, nullptr, CLSCTX_INPROC_SERVER,
                                             IID_IVSSDatabase, (LPVOID*)&punknown);
    ...
    if (m_pIVSSDatabase != nullptr) {
        ...
        if (retval) {
            ...
            if (root_item != nullptr) {
                    ...
                    HRESULT result			= root_item->put_LocalSpec
            (bstr_local_path); ASSERT (SUCCEEDED (result));
...
IVSSItem* VSSClass::Get_VSS_Item_From_Local_Path(LPCTSTR local_filename)
{
    ...
    // State OK?
    if ((m_pIVSSDatabase != nullptr) && (local_filename != nullptr)) {
...
IVSSItem* VSSClass::Get_VSS_Item(LPCTSTR vss_path)
{
    ...
    // State OK?
    if ((m_pIVSSDatabase != nullptr) && (vss_path != nullptr)) {
    ...
    if (pitem != nullptr) {
        ...
        if (Get_File_Status(local_filename, nullptr, 0, pitem) != VSSFILE_CHECKEDOUT_ME) {
            ...
            if (!SUCCEEDED(hresult) || (is_different != 0)) {
                retval
                    = SUCCEEDED(pitem->Get(&bstr_filename, VSSFLAG_TIMEMOD | VSSFLAG_REPREPLACE));
            }
        ...
        if (!SUCCEEDED (hresult) || (is_different != 0)) {
                ...
                if (Get_File_Date (pitem, vss_file_time)) {
                        ...
                        if (vss_time > local_time) {
                                retval = SUCCEEDED (pitem->Get (&bstr_filename, VSSFLAG_TIMEMOD |
        VSSFLAG_REPREPLACE));
                        }
                }
        }*/
...
bool VSSClass::Get_Subproject(LPCTSTR local_filename)
{
    ...
    if (pitem != nullptr) {
        ...
        // Convert the ascii string to a COM-safe BSTR
        // BSTR bstr_filename = ::Alloc_Sys_String (::Get_File_Mgr ()->Make_Full_Path
        // (local_filename));
...
bool VSSClass::Check_Out(LPCTSTR local_filename, bool get_locally)
{
    ...
    if (pitem != nullptr) {
        ...
        retval = SUCCEEDED(pitem->Checkout(nullptr, bstr_filename,
                                           (get_locally ? VSSFLAG_TIMEMOD : VSSFLAG_GETNO)
...
bool VSSClass::Check_In(LPCTSTR local_filename, LPCTSTR comment)
{
    ...
    if (pitem != nullptr) {
        ...
        retval = SUCCEEDED(pitem->Checkin(
            bstr_comment, bstr_filename, VSSFLAG_CMPCHKSUM | VSSFLAG_RECURSNO | VSSFLAG_UPDUPDATE));
...
//	Get_File_Status
//
VSSFileStatus VSSClass::Get_File_Status(LPCTSTR local_filename, LPTSTR checked_out_username,
                                        DWORD buffer_size, IVSSItem* item_to_use)
{
    ...
    if (pitem != nullptr) {
        ...
        if ((status != VSSFILE_NOTCHECKEDOUT) && (checked_out_username != nullptr)) {
...
VSSClass::Get_Error_Description(HRESULT hresult)
{
    ...
    for (int error = 0; (error < error_count) && (description == nullptr); error++) {
...
//
///////////////////////////////////////////////////////////////////
AssetDatabaseClass::FILE_STATUS VSSClass::Get_File_Status(LPCTSTR local_filename,
