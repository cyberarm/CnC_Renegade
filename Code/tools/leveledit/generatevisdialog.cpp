...
void GenerateVisDialogClass::OnOK(void)
{
    ...
    if (SendDlgItemMessage(IDC_USE_EDGE_SAMPLING, BM_GETCHECK) != 0) {
        ...
        scene->Generate_Uniform_Sampled_Vis(granularity, sample_height, (ignore_bias == TRUE),
                                            (selection_only == TRUE));
    }
    ...
    CString message;
    message.Format("Total Elapsed Time: %d hours, %d minutes, %d seconds.", hours, minutes,
                   seconds);
