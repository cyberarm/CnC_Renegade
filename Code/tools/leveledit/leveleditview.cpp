...
bool CLevelEditView::Initialize_Render_Device(void)
{
    ...
    m_bRenderDeviceInitialized
        = (WW3D::Set_Render_Device(m_iDeviceIndex, cx, cy, bpp, m_iWindowed) == WW3D_ERROR_OK);
...
int CLevelEditView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    m_hMouseHook = ::SetWindowsHookEx(WH_MOUSE, fnMouseHookProc, ::AfxGetInstanceHandle(),
                                      ::GetCurrentThreadId());
    return 0;
}
