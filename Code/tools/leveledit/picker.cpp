      ...
      CStatic()
{
    ...
    //
    m_Icon = (HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_FOLDER_TINY),
...
bool PickerClass::Create_Picker(DWORD style, const RECT& rect, CWnd* parent, UINT id)
{
    ...
    //
    BOOL success
        = CreateEx(WS_EX_CLIENTEDGE, "STATIC", "",
                   (style | SS_WHITERECT | WS_CLIPCHILDREN) & (~WS_BORDER), rect, parent, id);
    if (success) {
        Initialize_Control();
    }
...
PickerClass::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if ((message == WM_GETTEXT) || (message == WM_GETTEXTLENGTH) || (message == WM_SETTEXT)) {
        ...
        return ::SendMessage(::GetParent(m_hWnd), message, MAKEWPARAM(id & 0xFFFF, HIWORD(wParam)),
                             (LPARAM)m_hWnd);
    }
...
void PickerClass::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT pDrawItemStruct)
{
    ...
    //
    UINT type = (pDrawItemStruct->itemState & ODS_SELECTED) ? (DFCS_SCROLLDOWN | DFCS_PUSHED)
                                                            : DFCS_SCROLLDOWN;
