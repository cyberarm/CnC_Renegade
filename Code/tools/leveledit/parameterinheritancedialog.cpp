...
BOOL ParameterInheritanceDialogClass::OnInitDialog(void)
{
    ...
    HICON icon1 = (HICON)::LoadImage(::AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_PARAM),
                                     IMAGE_ICON, 16, 16, LR_SHARED | LR_DEFAULTCOLOR);
    HICON icon2 = (HICON)::LoadImage(::AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_PHYS),
                                     IMAGE_ICON, 16, 16, LR_SHARED | LR_DEFAULTCOLOR);
    imagelist->Add(icon1);
    imagelist->Add(icon2);
...
void ParameterInheritanceDialogClass::Update_Tree_Entry_Check(HTREEITEM parent_item, bool checked)
{
    ...
    //
    for (HTREEITEM item = m_TreeCtrl.GetChildItem(parent_item); item != nullptr;
    ...
    for (int index = 0; index < preset_list.Count(); index++) {
        ...
        if (base_def != nullptr) {
            ...
            if (propogate_dialogue_settings
                ...
                SoldierGameObjDef* derived_soldier
                    = reinterpret_cast<SoldierGameObjDef*>(derived_def);
...
//
/////////////////////////////////////////////////////////////////////////////
bool Find_Parameter_In_List(DynamicVectorClass<DefinitionParameterClass*>& parameter_list,
                            ParameterClass* parameter)
{
    bool retval = false;
    ...
    for (int param_index = 0; is_valid && param_index < param_count; param_index++) {
        ...
        if (parameter->Get_Type() == curr_parameter->Get_Type()
            ...
            if (parameter->Get_Type() == ParameterClass::TYPE_MODELDEFINITIONID) {
                ...
                DefinitionClass* base_model_def
                    = DefinitionMgrClass::Find_Definition(base_id, false);
...
//
/////////////////////////////////////////////////////////////////////////////
void ParameterInheritanceDialogClass::Build_Preset_List(
    HTREEITEM parent_item, DynamicVectorClass<PresetClass*>& preset_list)
{
    //
