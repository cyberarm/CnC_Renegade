...
{
public:
    ChunkType(const char* name, void (*callback)(ChunkItem* item, CListCtrl* List),
              bool wrapper = false)
    {
        Name = name;
        Callback = callback;
...
public:
    ...
    void NewType(int ID, const char* name, void (*callback)(ChunkItem* item, CListCtrl* list) = 0,
                 bool wrapper = false);

protected:
    CMapPtrToPtr Types;

    static void AddItem(CListCtrl* List, int& Counter, const char* Name, const char* Value,
    ...
    static void AddItem(CListCtrl* List, int& Counter, const char* Name,
...
protected:
    void Add_Chunk(ChunkLoadClass& cload,
                   ChunkItem* Parent
                   = 0); // add a chunk to the list of chunks maintained by ChunkData
};
