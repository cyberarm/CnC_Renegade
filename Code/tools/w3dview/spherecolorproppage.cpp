...
void SphereColorPropPageClass::Initialize(void)
{
    ...
    if (m_RenderObj != nullptr) {
        ...
        m_EnableOpactiyVector
            = (m_RenderObj->Get_Flags() & SphereRenderObjClass::USE_ALPHA_VECTOR) != 0;
...
BOOL SphereColorPropPageClass::OnInitDialog(void)
{
    ...
    for (int index = 0; index < m_OrigColorChannel.Get_Key_Count(); index++) {
        m_ColorBar->Modify_Point(index, m_OrigColorChannel[index].Get_Time(),
    ...
    for (index = 0; index < m_OrigAlphaChannel.Get_Key_Count(); index++) {
        m_OpacityBar->Modify_Point(index, m_OrigAlphaChannel[index].Get_Time(),
    ...
    //
    CheckDlgButton(IDC_OPACITY_VECTOR_CHECK,
                   (m_RenderObj->Get_Flags() & SphereRenderObjClass::USE_ALPHA_VECTOR) != 0);
    CheckDlgButton(IDC_INVERT_VECTOR_CHECK,
                   (m_RenderObj->Get_Flags() & SphereRenderObjClass::USE_INVERSE_ALPHA) != 0);
    ...
    case IDC_OPACITY_BAR: {
        if (color_bar_hdr->hdr.code == CBRN_DBLCLK_POINT) {
            ...
            if (dialog.DoModal() == IDOK) {
                m_OpacityBar->Modify_Point(color_bar_hdr->key_index, color_bar_hdr->position,
    ...
    case IDC_COLOR_BAR: {
        if (color_bar_hdr->hdr.code == CBRN_DBLCLK_POINT) {
            ...
            if (Show_Color_Picker(&red, &green, &blue)) {
                m_ColorBar->Modify_Point(color_bar_hdr->key_index, color_bar_hdr->position, red,
                                         green, blue);
    ...
    case IDC_VECTOR_BAR: {
        ...
        if (color_bar_hdr->hdr.code == CBRN_DBLCLK_POINT) {
            ...
            if (next_data == nullptr) {
                ...
                new_data->intensity = prev_data->intensity
                    + ((next_data->intensity - prev_data->intensity) * percent);
