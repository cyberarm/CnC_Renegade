...
//	Local prototypes
/////////////////////////////////////////////////////////////
static bool Is_LERP(float last_value, float last_time, float curr_value, float curr_time,
                    float next_value, float next_time);
...
BOOL SphereSizePropPageClass::OnInitDialog(void)
{
    ...
    for (int index = 1; index < m_OrigScaleChannel.Get_Key_Count(); index++) {
        const LERPAnimationChannelClass<Vector3>::KeyClass& prev_value
            = m_OrigScaleChannel.Get_Key(index - 1);
        const LERPAnimationChannelClass<Vector3>::KeyClass& curr_value
            = m_OrigScaleChannel.Get_Key(index);
        ...
        if (index == (m_OrigScaleChannel.Get_Key_Count() - 1)) {
            ...
            const LERPAnimationChannelClass<Vector3>::KeyClass& next_value
                = m_OrigScaleChannel[index + 1];

            //
            //	Check to ensure the X-value isn't just a LERP of the 2 adjacent keys.
            //
            unique_x = (::Is_LERP(prev_value.Get_Value().X, prev_value.Get_Time(),
                                  curr_value.Get_Value().X, curr_value.Get_Time(),
                                  next_value.Get_Value().X, next_value.Get_Time())
                        == false);

            //
            //	Check to ensure the Y-value isn't just a LERP of the 2 adjacent keys.
            //
            unique_y = (::Is_LERP(prev_value.Get_Value().Y, prev_value.Get_Time(),
                                  curr_value.Get_Value().Y, curr_value.Get_Time(),
                                  next_value.Get_Value().Y, next_value.Get_Time())
                        == false);

            //
            //	Check to ensure the Z-value isn't just a LERP of the 2 adjacent keys.
            //
            unique_z = (::Is_LERP(prev_value.Get_Value().Z, prev_value.Get_Time(),
                                  curr_value.Get_Value().Z, curr_value.Get_Time(),
                                  next_value.Get_Value().Z, next_value.Get_Time())
                        == false);
        }
...
//
/////////////////////////////////////////////////////////////
bool Is_LERP(float last_value, float last_time, float curr_value, float curr_time, float next_value,
             float next_time)
{
