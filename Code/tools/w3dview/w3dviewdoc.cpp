...
void CW3DViewDoc::LoadAssetsFromFile(LPCTSTR lpszPathName)
{
    ...
    if (::lstrcmpi(extension, ".tga") == 0 || ::lstrcmpi(extension, ".dds") == 0) {
        ...
        TextureClass* ptexture
            = WW3DAssetManager::Get_Instance()->Get_Texture(::Get_Filename_From_Path(lpszPathName));
        if (ptexture != nullptr) {
...
//
///////////////////////////////////////////////////////////////
void CW3DViewDoc::Display_Emitter(ParticleEmitterClass* pemitter, bool use_global_reset_flag,
                                  bool allow_reset)
{
    ASSERT(m_pCScene);
        ...
        if (pemitter != nullptr) {
            ...
            if (pCGraphicView) {
                ...
                if ((use_global_reset_flag && m_bAutoCameraReset)
                    || ((use_global_reset_flag == false) && allow_reset) || m_bOneTimeReset) {
...
//
///////////////////////////////////////////////////////////////
void CW3DViewDoc::DisplayObject(RenderObjClass* pCModel, bool use_global_reset_flag,
                                bool allow_reset, bool add_ghost)
{
    ASSERT(m_pCScene);
...
void CW3DViewDoc::StepAnimation(int iFrameInc)
{
    if (m_pCRenderObj && m_pCAnimation) {
        ...
        float anim_speed = ::Get_Graphic_View()->GetAnimationSpeed();
        ((CMainFrame*)::AfxGetMainWnd())
            ->UpdateFrameCount(m_CurrentFrame, iTotalFrames - 1, frame_rate * anim_speed);
...
//
///////////////////////////////////////////////////////////////
void CW3DViewDoc::PlayAnimation(RenderObjClass* pCModel, LPCTSTR pszAnimationName,
                                bool use_global_reset_flag, bool allow_reset)
{
    ASSERT(m_pCScene);
    ASSERT(pCModel);
...
//
///////////////////////////////////////////////////////////////
void CW3DViewDoc::PlayAnimation(RenderObjClass* pCModel, HAnimComboClass* pCAnimCombo,
                                bool use_global_reset_flag, bool allow_reset)
{
    ASSERT(m_pCScene);
    ASSERT(pCModel);
...
void CW3DViewDoc::Update_Camera(void)
{
    ...
    if (m_bAnimateCamera && m_pCRenderObj != nullptr) {
        ...
        if (Get_Camera_Transform(m_pCRenderObj, transform)) {
            ...
            Matrix3D cam_transform(Vector3(0, -1, 0), Vector3(0, 0, 1), Vector3(-1, 0, 0),
                                   Vector3(0, 0, 0));
...
void CW3DViewDoc::UpdateFrame(float relativeTimeSlice)
{
    if (m_pCRenderObj && m_pCAnimation) {
        ...
        float anim_speed = ::Get_Graphic_View()->GetAnimationSpeed();
        ((CMainFrame*)::AfxGetMainWnd())
            ->UpdateFrameCount(m_CurrentFrame, total_frames - 1, frame_rate * anim_speed);
    ...
    if (pObjEnum) {
            ...
            if (WW3DAssetManager::Get_Instance()->Render_Obj_Exists(pszItemName)
                ...
                if (Is_Model_Part_of_LOD(pszItemName, pszLODBaseName, type)) {
                    ...
                    if (type == TYPE_COMMANDO) {
                        iStartingIndex
                            = min(iStartingIndex, ::atoi(&pszItemName[::lstrlen(pszItemName) - 1]));
                    }
                    else {
                        starting_char
                            = min(starting_char,
                                  (char)::toupper(pszItemName[::lstrlen(pszItemName) - 1]));
                    }
                }
            }
        }
        ...
        if (plod_array != nullptr) {
            ...
            for (int lod_index = 0; lod_index < lod_count; lod_index++) {
                ...
                plod_array[lod_count - (lod_index + 1)]
                    = WW3DAssetManager::Get_Instance()->Create_Render_Obj(lod_name);
            }
...
BOOL CW3DViewDoc::SaveSettings(LPCTSTR pszFilename, DWORD dwSettingsMask)
{
    ...
    // Is the filename OK?
    HANDLE hFile = ::CreateFile(pszFilename, 0, 0, nullptr, OPEN_ALWAYS, 0L, nullptr);

    ASSERT(hFile != nullptr);
    if (hFile == nullptr) {
        // Invalid file, let the user know
        ::AfxGetMainWnd()->MessageBox(
            "Unable to open file for writing.  Please select another filename.", "File Error",
            MB_ICONERROR | MB_OK);
    }
    else if (pszFilename && (dwSettingsMask != 0L) && (m_pCScene != nullptr)) {
        ...
        if (dwSettingsMask & SAVE_SETTINGS_LIGHT) {
            ...
            // Write the 'Red' component out to the file
            stringValue.Format("%f", colorSettings.X);
            ::WritePrivateProfileString("Settings", "SceneLightR", stringValue,
                                        (LPCTSTR)stringCompleteFilename);

            // Write the 'Green' component out to the file
            stringValue.Format("%f", colorSettings.Y);
            ::WritePrivateProfileString("Settings", "SceneLightG", stringValue,
                                        (LPCTSTR)stringCompleteFilename);

            // Write the 'Blue' component out to the file
            stringValue.Format("%f", colorSettings.Z);
            ::WritePrivateProfileString("Settings", "SceneLightB", stringValue,
                                        (LPCTSTR)stringCompleteFilename);
            ...
            ::WritePrivateProfileString("Settings", "SceneLightX", stringValue,
                                        (LPCTSTR)stringCompleteFilename);

            // Write the y-position out to the file
            stringValue.Format("%f", orientation.Y);
            ::WritePrivateProfileString("Settings", "SceneLightY", stringValue,
                                        (LPCTSTR)stringCompleteFilename);

            // Write the z-position out to the file
            stringValue.Format("%f", orientation.Z);
            ::WritePrivateProfileString("Settings", "SceneLightZ", stringValue,
                                        (LPCTSTR)stringCompleteFilename);

            // Write the w-position out to the file
            stringValue.Format("%f", orientation.W);
            ::WritePrivateProfileString("Settings", "SceneLightW", stringValue,
                                        (LPCTSTR)stringCompleteFilename);
            ...
            ::WritePrivateProfileString("Settings", "SceneLightDistance", stringValue,
                                        (LPCTSTR)stringCompleteFilename);

            // Write the intensity out to the file
            float intensity = m_pCSceneLight->Get_Intensity();
            stringValue.Format("%f", intensity);
            ::WritePrivateProfileString("Settings", "SceneLightIntensity", stringValue,
                                        (LPCTSTR)stringCompleteFilename);
            ...
            ::WritePrivateProfileString("Settings", "SceneLightAttenStart", stringValue,
                                        (LPCTSTR)stringCompleteFilename);

            // Write the end attenuation out to the file
            stringValue.Format("%f", end);
            ::WritePrivateProfileString("Settings", "SceneLightAttenEnd", stringValue,
                                        (LPCTSTR)stringCompleteFilename);

            // Write the end attenuation out to the file
            BOOL atten_on = m_pCSceneLight->Get_Flag(LightClass::FAR_ATTENUATION);
            stringValue.Format("%d", atten_on);
            ::WritePrivateProfileString("Settings", "SceneLightAttenOn", stringValue,
                                        (LPCTSTR)stringCompleteFilename);
        }
        ...
        if (dwSettingsMask & SAVE_SETTINGS_BACK) {
            ...
            ::WritePrivateProfileString("Settings", "BackgroundR", stringValue,
                                        (LPCTSTR)stringCompleteFilename);

            // Write the 'Green' component out to the file
            stringValue.Format("%f", m_backgroundColor.Y);
            ::WritePrivateProfileString("Settings", "BackgroundG", stringValue,
                                        (LPCTSTR)stringCompleteFilename);

            // Write the 'Blue' component out to the file
            stringValue.Format("%f", m_backgroundColor.Z);
            ::WritePrivateProfileString("Settings", "BackgroundB", stringValue,
                                        (LPCTSTR)stringCompleteFilename);
...
bool CW3DViewDoc::Save_Selected_LOD(void)
{
    ...
    // Is this an emitter?
    if ((m_pCRenderObj != nullptr) && m_pCRenderObj->Class_ID() == RenderObjClass::CLASSID_HLOD) {
...
bool CW3DViewDoc::Save_Current_LOD(const CString& filename)
{
    ...
    proto = (HLodPrototypeClass*)WW3DAssetManager::Get_Instance()->Find_Prototype(
        m_pCRenderObj->Get_Name());
...
void CW3DViewDoc::SetBackgroundObject(LPCTSTR pszBackgroundObjectName)
{
    ...
    if (m_pCBackObjectScene && m_pCBackObjectCamera) {
        ...
        if (pszBackgroundObjectName) {
            ...
            m_pCBackgroundObject
                = WW3DAssetManager::Get_Instance()->Create_Render_Obj(pszBackgroundObjectName);
            ...
            if (m_pCBackgroundObject) {
                ...
                if (pCGraphicView) {
                    ...
                    m_pCBackObjectCamera->Set_Transform(
                        pCGraphicView->GetCamera()->Get_Transform());
                }
...
bool CW3DViewDoc::Save_Current_Sphere(const CString& filename)
{
    ...
    proto = (SpherePrototypeClass*)WW3DAssetManager::Get_Instance()->Find_Prototype(
        m_pCRenderObj->Get_Name());
    ASSERT(proto != nullptr);
    if (proto != nullptr) {
...
bool CW3DViewDoc::Save_Current_Ring(const CString& filename)
{
    ...
    proto = (RingPrototypeClass*)WW3DAssetManager::Get_Instance()->Find_Prototype(
        m_pCRenderObj->Get_Name());
    ASSERT(proto != nullptr);
    if (proto != nullptr) {
...
bool CW3DViewDoc::Save_Selected_Sound_Object(void)
{
    ...
    //
    if ((m_pCRenderObj != nullptr) && m_pCRenderObj->Class_ID() == RenderObjClass::CLASSID_SOUND) {
        //
        // Build the default filename from the name of the emitter
        //
        CString default_filename = GetDataTreeView()->GetCurrentSelectionName();
...
bool CW3DViewDoc::Save_Current_Sound_Object(const CString& filename)
{
    ...
    proto = (SoundRenderObjPrototypeClass*)WW3DAssetManager::Get_Instance()->Find_Prototype(
        m_pCRenderObj->Get_Name());
        ...
        for (int index = 0; index < bone_count; index++) {
            ...
            if (WW3DAssetManager::Get_Instance()->Render_Obj_Exists(pbone_name)) {
                ...
                RenderObjClass* prender_obj
                    = WW3DAssetManager::Get_Instance()->Create_Render_Obj(pbone_name);
...
bool CW3DViewDoc::Save_Current_Aggregate(const CString& filename)
{
    ...
    proto = (AggregatePrototypeClass*)WW3DAssetManager::Get_Instance()->Find_Prototype(
        m_pCRenderObj->Get_Name());
    ASSERT(proto != nullptr);
    if (proto != nullptr) {
                                     ...
                                     RenderObjClass* render_obj)
{
    ...
    //
    if ((render_obj != nullptr) && (render_obj->Class_ID() == RenderObjClass::CLASSID_PARTICLEEMITTER)
...
//
///////////////////////////////////////////////////////////////
bool CW3DViewDoc::Is_Cursor_Shown(void) const
{
...
void CW3DViewDoc::Toggle_Alternate_Materials(RenderObjClass* render_obj)
{
    ...
    if (render_obj != nullptr) {
        ...
        if (render_obj->Class_ID() == RenderObjClass::CLASSID_MESH) {
            ...
            mdl->Enable_Alternate_Material_Description(
                !mdl->Is_Alternate_Material_Description_Enabled());
        }
...
void CW3DViewDoc::Copy_Assets_To_Dir(LPCTSTR directory)
{
    ...
    if (Lookup_Path(asset_name, filename)) {
        ...
        ::MessageBox(::AfxGetMainWnd()->m_hWnd, message, "File Not Found",
