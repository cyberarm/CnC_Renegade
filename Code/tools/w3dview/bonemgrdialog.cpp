...
BOOL BoneMgrDialogClass::OnInitDialog(void)
{
    ...
    pimagelist->Add((HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_FOLDER),
                                       IMAGE_ICON, 16, 16, LR_SHARED));

    pimagelist->Add((HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_OBJECT),
                                       IMAGE_ICON, 16, 16, LR_SHARED));
...
void BoneMgrDialogClass::Fill_Bone_Item(HTREEITEM hbone_item, int bone_index)
{
    ...
    RenderObjClass* porig_model
        = WW3DAssetManager::Get_Instance()->Create_Render_Obj(orig_model_name);
    ...
    for (index = 0; index < m_pBaseModel->Get_Num_Sub_Objects_On_Bone(bone_index); index++) {
...
//	Is_Object_In_List
//
bool BoneMgrDialogClass::Is_Object_In_List(const char* passet_name,
                                           DynamicVectorClass<RenderObjClass*>& node_list)
{
    // Assume failure
    bool retval = false;
...
void BoneMgrDialogClass::OnAttachButton(void)
{
    ...
    if (m_bAttach) {
        ...
        for (int index = 0; (index < count) && !found; index++) {
            ...
            if ((psub_obj != nullptr) && (::lstrcmpi(psub_obj->Get_Name(), name) == 0)) {
...
{
    // Loop through all the children of this bone
    for (HTREEITEM hchild_item = m_BoneTree.GetChildItem(bone_item); (hchild_item != nullptr);
         hchild_item = m_BoneTree.GetNextSiblingItem(hchild_item)) {
