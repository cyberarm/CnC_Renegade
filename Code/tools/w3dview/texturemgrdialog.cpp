...
BOOL TextureMgrDialogClass::OnInitDialog(void)
{
    ...
    m_pTextureImageListSmall->Create(TEXTURE_THUMBSMALL_X, TEXTURE_THUMBSMALL_Y, ILC_COLOR24, 10,
                                     20);

    // Load this icon and add it to our imagelist
    m_pImageList->Add((HICON)::LoadImage(::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_MESH_FOLDER),
                                         IMAGE_ICON, 32, 32, LR_SHARED));

    // Load this icon and add it to our imagelist
    m_pImageListSmall->Add((HICON)::LoadImage(
        ::AfxGetResourceHandle(), MAKEINTRESOURCE(IDI_MESH_FOLDER), IMAGE_ICON, 16, 16, LR_SHARED));
        ...
        for (int index = 0; index < pmat_info->Texture_Count(); index++) {
            ...
            if (ptexture != nullptr) {
                ...
                TextureListNodeClass* pnode
                    = new TextureListNodeClass(ptexture, ::Get_Texture_Name(*ptexture));
        ...
        if (pnode != nullptr) {
            ...
            if (pnode->Get_Type() == TextureListNodeClass::TYPE_MESH) {
                ...
                if (ptexture->getClassID() != ID_INDIRECT_TEXTURE_CLASS) {
                    MessageBox(
                        "This texture can not be edited.  Only indirect textures are modifiable",
                        "Invalid Type", MB_OK | MB_ICONEXCLAMATION);
                }
                else {
...
void TextureMgrDialogClass::Insert_Texture_Details(TextureListNodeClass* pnode, int index)
{
    ...
    if ((index != -1) && (ptexture != nullptr)) {
        ...
        m_ListCtrl.SetItem(index, COL_NAME, LVIF_IMAGE | LVIF_TEXT, texture_name,
                           pnode->Get_Icon_Index(), 0, 0, 0);
...
void TextureMgrDialogClass::OnPropagate(void)
{
    ...
    if (MessageBox(
            "Warning, this operation will copy the texture settings from the currently selected "
            "mesh into all meshes with the same texture count.  Do you wish to continue?",
            "Texture Propagate", MB_ICONQUESTION | MB_YESNO)
        == IDNO) {
        return;
    }
                ...
                if (curr_texture_node != nullptr && src_texture_node != nullptr) {
                    ...
                    if (curr_texture != nullptr && src_texture != nullptr
                        ...
                        TextureClass* real_texture
                            = ((IndirectTextureClass*)src_texture)->Peek_Texture();
