...
bit8 Wtime::ParseDate(char* in)
{
    ...
    struct tm t;
    char* ptr = in;
    while ((!isgraph(*ptr)) && (*ptr != 0)) {
        ptr++; // skip to start of string
    }
    if (*ptr == 0) {
        return (FALSE);
    }
    ...
    for (i = 0; i < 12; i++) { // match month
        if (strncmp(ptr, MONTHS[i], strlen(MONTHS[i])) == 0) {
            t.tm_mon = i;
        }
    }
    if (t.tm_mon == -1) {
...
bit8 Wtime::FormatTime(char* out, char* format)
{
    ...
    if (*ptr == '"') {
        ptr++; // skip past open quote if exists
    }
    ...
    while (*ptr != 0) {
        ...
        if (isspace(*ptr)) {
            ...
            sprintf(out + strlen(out), "%ld/%ld/%02ld %ld:%02ld:%02ld", GetMonth(), GetMDay(),
                    GetYear() % 100, GetHour(), GetMinute(), GetSecond());
            ptr += 1;
        }
        else if (strncmp(ptr, "dddddd", 6) == 0) {
            sprintf(out + strlen(out), "%s %02ld, %ld", FULLMONTHS[GetMonth() - 1], GetMDay(),
                    GetYear());
            ptr += 6;
        }
        else if (strncmp(ptr, "ddddd", 5) == 0) {
            sprintf(out + strlen(out), "%ld/%ld/%02ld", GetMonth(), GetMDay(), GetYear() % 100);
            ptr += 5;
        }
            ...
            sprintf(out + strlen(out), "%ld:%02ld:%02ld", GetHour(), GetMinute(), GetSecond());
            ptr += 5;
        }
