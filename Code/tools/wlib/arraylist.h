...
// copy constructor
template <class T> ArrayList<T>::ArrayList(ArrayList<T>& other)
{
    Entries_ = 0;
    Slots_ = 0;
    Vector_ = nullptr;
...
// assignment operator
template <class T> ArrayList<T>& ArrayList<T>::operator=(IN ArrayList<T>& other)
{
    T node;
    clear();
...
//
// **************************************************************************
template <class T> bit8 ArrayList<T>::setSize(sint32 newsize, IN T& filler)
{
...
template <class T> bit8 ArrayList<T>::add(IN T& node, sint32 pos)
{
    ...
    if (pos < Entries_) { // If there are elements after the add point
        memmove(Vector_ + pos + 1, Vector_ + pos,
                sizeof(T) * (Entries_ - pos)); // move them forward
    }
...
template <class T> bit8 ArrayList<T>::addHead(IN T& node)
{
    return (add(node, 0));
}

// Append to the end of the list
template <class T> bit8 ArrayList<T>::addTail(IN T& node)
{
...
// using addSorted.
template <class T> bit8 ArrayList<T>::addSortedAsc(IN T& node)
{
...
// addSortedX only works (properly) if evrerything else in the list is added
// using addSorted.
template <class T> bit8 ArrayList<T>::addSortedDes(IN T& node)
{
...
template <class T> sint32 ArrayList<T>::_sortedLookup(IN T& target, int ascending)
{
    int low, mid, high;
    T* lowtarget;
    T* hightarget;
...
//
template <class T> bit8 ArrayList<T>::replace(IN T& node, sint32 pos)
{
    if (Entries_ == 0) {
        return (FALSE);
    }
...
template <class T> bit8 ArrayList<T>::remove(sint32 pos)
{
    if (Entries_ == 0) {
        return (FALSE);
    }
...
// Remove at the zero based index specified by 'pos'.  When removing from
// a slot, all others get shifted up by one.
template <class T> bit8 ArrayList<T>::remove(OUT T& node, sint32 pos)
{
    bit8 retval;
    retval = get(node, pos);
    if (retval == FALSE) {
...
template <class T> bit8 ArrayList<T>::removeHead(OUT T& node)
{
    return (remove(node, 0));
}

// Remove the last node of the list
template <class T> bit8 ArrayList<T>::removeTail(OUT T& node)
{
...
template <class T> bit8 ArrayList<T>::getPointer(OUT T** node, sint32 pos) RO
{
...
// get a copy of an item
template <class T> bit8 ArrayList<T>::get(OUT T& node, sint32 pos) RO
{
    if ((pos < 0) || (pos >= Entries_)) {
        return (FALSE);
    }
...
template <class T> bit8 ArrayList<T>::getHead(OUT T& node) RO
{
    return (get(node, 0));
}

// get a copy of the last node
template <class T> bit8 ArrayList<T>::getTail(OUT T& node) RO
{
    return (get(node, Entries_ - 1));
}

// just for debugging
template <class T> void ArrayList<T>::print(FILE* out)
{
    fprintf(out, "--------------------\n");
    // for (int i=0; i<Entries_; i++)
    //   Vector_[i].print();
    fprintf(out, "Entries: %d  Slots:  %d  sizeof(T): %d\n", Entries_, Slots_, sizeof(T));
    fprintf(out, "--------------------\n");
}

// Return the current length of the list
template <class T> sint32 ArrayList<T>::length(void) RO
{
    return (Entries_);
}

// Grow the vector by a factor of 2X
template <class T> bit8 ArrayList<T>::growVector(void)
{
...
template <class T> bit8 ArrayList<T>::shrinkVector(void)
{
...
template <class T> sint32 ArrayList<T>::removeMany(OUT T* outarray, sint32 pos, sint32 howmany)
{
    if (Entries_ == 0) {
        return (FALSE);
    }
...
// artifact DLL in the archive and similar functions in this header file.
// Compiles, but not tested. LFeenanEA - January 27th 2025
template <class T> bit8 ArrayList<T>::addMany(IN T* inarray, sint32 pos, sint32 howmany)
{
    if (pos > Entries_) { // You can only access one of the end of the vector
        pos = Entries_;
    }
    ...
    //  slide the old objects forward.
    if (pos < Entries_) { // If there are elements after the add point
        memmove(Vector_ + pos + howmany, Vector_ + pos,
                sizeof(T) * (Entries_ - pos)); // move them forward
    }
