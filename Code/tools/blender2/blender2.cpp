...
static BOOL CALLBACK BlenderDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg) {
    case WM_INITDIALOG:
...
void Blender_Class::BeginEditParams(Interface* ip, IUtil* iu)
{
    this->iu = iu;
    this->ip = ip;
    hPanel = ip->AddRollupPage(hInstance, MAKEINTRESOURCE(IDD_ASCIIOUT_PANEL), BlenderDlgProc,
                               _T("Key Blender"), 0);
}
...
void Blender_Class::Init(HWND hWnd)
{
    ...
    first_frame_spin
        = SetupIntSpinner(hWnd, IDC_FIRST_SPIN, IDC_FIRST_EDIT, -65536, 65535, first_frame);

    first_frame_spin->SetResetValue(first_frame);

    first_match_spin = SetupIntSpinner(hWnd, IDC_FIRST_MATCH_SPIN, IDC_FIRST_MATCH_EDIT, -65536,
                                       65535, first_match);

    first_match_spin->SetResetValue(first_match);

    last_frame_spin
        = SetupIntSpinner(hWnd, IDC_LAST_SPIN, IDC_LAST_EDIT, -65536, 65535, last_frame);

    last_frame_spin->SetResetValue(last_frame);

    last_match_spin = SetupIntSpinner(hWnd, IDC_LAST_MATCH_SPIN, IDC_LAST_MATCH_EDIT, -65536, 65535,
                                      last_match);
...
void Blender_Class::Blend_Keys()
{
    ...
    for (int i = 0; i < number_of_nodes; ++i) {
        ...
        if (c != nullptr) {
            if (c->ClassID() != Class_ID(TCBINTERP_ROTATION_CLASS_ID, 0)) {
                ...
                data.heading_delta
                    = Heading_Delta_From_Quat(actual_end_orientation / actual_start_orientation);
                ...
                for (int j = 0; j < number_of_keys; ++j) {
                    ...
                    if (key_time >= start_time && key_time <= end_time) {
                        ...
                        Quat delta
                            = Slerp(rot_start_delta, zero, t) * Slerp(zero, rot_end_delta, t);
                ...
                for (int j = 0; j < number_of_keys; ++j) {
                    ...
                    if (key_time >= start_time && key_time <= end_time) {
                        ...
                        Point3 actual_position
                            = actual_start_position * (1.0f - t) + actual_end_position * t;
                        ...
                        Point3 desired_position
                            = desired_start_position * (1.0f - t) + desired_end_position * t;
    ...
    if (bad_controller_found) {
        MessageBox(GetActiveWindow(),
                   ...
                   "because they were not TCB controllers.",
                   "Warning", MB_OK);
    }
}
...
void Blender_Class::Set_Data_Chunk(INode* node, const Blender_Data_Chunk& new_data)
{
    AppDataChunk* chunk = node->GetAppDataChunk(Blender_Class_ID, UTILITY_CLASS_ID, 1);
    ...
    if (chunk != nullptr) {
        ...
        node->AddAppDataChunk(Blender_Class_ID, UTILITY_CLASS_ID, 1, sizeof(Blender_Data_Chunk),
                              data);
    }
...
{
    node->RemoveAppDataChunk(Blender_Class_ID, UTILITY_CLASS_ID, 1);
}
