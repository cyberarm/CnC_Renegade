...
const APP_INFO APPLICATIONS[APP_MAX] =
    // --- Title ---						---Source --- --Default Dir---
    // ---RegKey---             ---Ctrl_ID---      ---Clean CtrlID--     --Dir CtrlID--- --Clean
    // default- copy to appdir1, copy to appdir2
    { { "W3DViewer", "\\\\cabal\\mis\\r&d\\w3d\\w3dview", "c:\\w3d\\w3dview", "W3DUpdate\\W3DView",
        IDC_VIEWER_CHECK, IDC_VIEWER_CLEAN_CHECK, IDC_VIEWER_DIR_BUTTON, true, -1, -1 },
      { "Max3 Plugin", "\\\\cabal\\mis\\r&d\\w3d\\maxplugins\\max3",
        "c:\\3dsmax3\\plugins\\westwood", "W3DUpdate\\MaxPlugin30", IDC_PLUGIN_CHECK,
        IDC_PLUGIN_CLEAN_CHECK, IDC_PLUGIN_DIR_BUTTON, false, -1, -1 },
      { "WDump", "\\\\cabal\\mis\\r&d\\w3d\\wdump", "c:\\w3d\\wdump", "W3DUpdate\\WDump",
        IDC_WDUMP_CHECK, IDC_WDUMP_CLEAN_CHECK, IDC_WDUMP_DIR_BUTTON, true, -1, -1 },
      { "Max4 Plugin", "\\\\cabal\\mis\\r&d\\w3d\\maxplugins\\max4",
        "c:\\3dsmax4\\plugins\\westwood", "W3DUpdate\\MaxPlugin40", IDC_MAX4_PLUGIN_CHECK,
        IDC_MAX4_PLUGIN_CLEAN_CHECK, IDC_MAX4_PLUGIN_DIR_BUTTON, false, -1, -1 },
      { "Renegade Dazzles",
        "\\\\mobius\\project7\\projects\\renegade\\asset management\\assets\\dazzle", "",
        "W3DUpdate\\Reneg Dazzle", IDC_RENEGDAZZLE_CHECK, -1, -1, false, APP_VIEWER, APP_PLUGIN },
      { "W3D Shell Extension", "\\\\cabal\\mis\\r&d\\w3d\\W3DShellExt", "",
        "W3DUpdate\\W3DShellExt", IDC_W3DSHELLEXT, -1, -1, false, -1, -1 },
      { "Gmax Settings", "\\\\cabal\\mis\\r&d\\w3d\\GmaxRenX", "", "W3DUpdate\\GmaxRenX",
        IDC_GMAX_CHECK, IDC_GMAX_CLEAN_CHECK2, -1, false, -1, -1 } };
    ...
    for (int index = 0; (index < APP_MAX) && success; index++) {
        ...
        if (SendDlgItemMessage(app_info.ctrl_id, BM_GETCHECK) == 1) {
            ...
            if (app_info.copy_to_app_dir1 != -1) {
                ...
                success
                    = Install_App(app_info.title, app_info.src, local_path, reg_key_name, false);
                ...
                if (success) {
                    if (app_info.copy_to_app_dir2 != -1) {
                        ...
                        success = Install_App(app_info.title, app_info.src, local_path,
                                              reg_key_name, false);
                    }
                }
            }
                ...
                //
                success
                    = Install_App(app_info.title, app_info.src, local_path, reg_key_name, clean);
            }
...
//
//////////////////////////////////////////////////////////////////////////////////
void CW3DUpdateDlg::Get_Destination_Path(int app_id, CString& dest_path)
{
    //
...
//
//////////////////////////////////////////////////////////////////////////////////
bool CW3DUpdateDlg::Install_App(const CString& title, const CString& src_path,
                                const CString& dest_path, const CString& reg_key_name, bool clean)
{
...
//
//////////////////////////////////////////////////////////////////////////////////
BOOL CW3DUpdateDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
    for (int index = 0; index < APP_MAX; index++) {
        const APP_INFO& app_info = APPLICATIONS[index];
...
//
//////////////////////////////////////////////////////////////////////////////////
void CW3DUpdateDlg::OnDefaults(void)
{
    for (int index = 0; index < APP_MAX; index++) {
        const APP_INFO& app_info = APPLICATIONS[index];
...
void CW3DUpdateDlg::RegisterShellExt()
{
    ...
    if (ERROR_SUCCESS == ret_val) {
        ...
        ret_val = ::RegSetValueEx(reg_key, value_name, 0L, REG_SZ, (BYTE*)(LPCTSTR)reg_value,
                                  reg_value.GetLength() + 1);
    }
    ::RegCloseKey(reg_key);
