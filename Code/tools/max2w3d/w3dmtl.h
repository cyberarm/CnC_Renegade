...
private:
    int Add_Vertex_Material(W3dVertexMaterialStruct* vmat, const char* mapper_args0,
                            const char* mapper_args1, int pass, const char* name);
    int Add_Shader(const W3dShaderStruct& shader, int pass);
    int Add_Texture(W3dMapClass* map, int pass, int stage);
    unsigned long Compute_Crc(const W3dVertexMaterialStruct& vmat, const char* mapper_args0,
    ...
    */
    ...
    public:
        ...
        char* MapperArgs[W3dMaterialClass::MAX_STAGES]; // note: these strings are new'ed, not
                                                        // malloc'ed (unlike Name)
