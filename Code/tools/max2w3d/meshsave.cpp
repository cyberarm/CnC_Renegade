 ...
 *=============================================================================================*/
MeshSaveClass::MeshSaveClass(char* mesh_name, char* container_name, INode* inode,
                             const Mesh* input_mesh, Matrix3& exportspace,
                             W3DAppData2Struct& exportoptions, HierarchySaveClass* htree,
        ...
        for (vert_counter = 0; vert_counter < 3; vert_counter++) {
            ...
            */
            ...
            if (is_skin) {
                ...
                if ((skin_bone_index != -1) && (skin_bone_index < skinobj->Num_Bones())
 ...
 * HISTORY:                                                                                    *
 *=============================================================================================*/
void MeshSaveClass::get_skin_modifier_objects(SkinDataClass** skin_data_ptr,
                                              SkinWSMObjectClass** skin_obj_ptr)
{
        ...
        if (refTarg != nullptr && refTarg->ClassID() == Class_ID(WSM_DERIVOB_CLASS_ID, 0)) {
            ...
            for (int j = 0; j < wsm_der_obj->NumModifiers(); j++) {
                ...
                if (mod->ClassID() == SKIN_MOD_CLASS_ID) {
                    ...
                    *skin_obj_ptr
                        = (SkinWSMObjectClass*)skinmod->GetReference(SkinModifierClass::OBJ_REF);
                }
            }
        }
    }
...
void MeshSaveClass::inv_deform_mesh()
{
    ...
    for (int vert_index = 0; vert_index < Builder.Get_Vertex_Count(); vert_index++) {
        ...
        if (vert.BoneIndex == 0) {
            ...
            // transformed relative to the user or scene origin and do not need to be further
            // modified.
...
{
    if (((Header.Attributes & W3D_MESH_FLAG_GEOMETRY_TYPE_MASK) != W3D_MESH_FLAG_GEOMETRY_TYPE_SKIN)
        || !(Header.VertexChannels & W3D_VERTEX_CHANNEL_BONEID) || (VertInfluences == nullptr)) {
        return 0;
    }
...
{
    if (MaterialDesc.Vertex_Material_Count() <= 0) {
        return 0;
    }
...
{
    if (MaterialDesc.Texture_Count() <= 0) {
        return 0;
    }
    ...
    for (int i = 0; i < MaterialDesc.Texture_Count(); i++) {
        ...
        if (csave.Write(map->Filename, strlen(map->Filename) + 1) != strlen(map->Filename) + 1) {
            return 1;
        }
        csave.End_Chunk();
...
int MeshSaveClass::write_vertex_material_ids(ChunkSaveClass& csave, int pass)
{
    ...
    if (stats.HasPerVertexMaterial[pass]) {
        for (int i = 0; i < Builder.Get_Vertex_Count(); i++) {
            ...
            if (csave.Write(&matid, sizeof(uint32)) != sizeof(uint32)) {
                return 1;
            }
        }
    }
...
int MeshSaveClass::write_shader_ids(ChunkSaveClass& csave, int pass)
{
    ...
    if (stats.HasPerPolyShader[pass]) {
        for (int i = 0; i < Builder.Get_Face_Count(); i++) {
            ...
            if (csave.Write(&shaderid, sizeof(uint32)) != sizeof(uint32)) {
                return 1;
            }
        }
    }
...
{
    const MeshBuilderClass::MeshStatsStruct& stats = Builder.Get_Mesh_Stats();
    if (!stats.HasTexture[pass][stage]) {
        return 0;
    }
...
int MeshSaveClass::write_texture_ids(ChunkSaveClass& csave, int pass, int stage)
{
    ...
    if (stats.HasPerPolyTexture[pass][stage]) {
        for (int i = 0; i < Builder.Get_Face_Count(); i++) {
            ...
            if (csave.Write(&texid, sizeof(uint32)) != sizeof(uint32)) {
                return 1;
            }
        }
    }
...
{
    const MeshBuilderClass::MeshStatsStruct& stats = Builder.Get_Mesh_Stats();
    if (stats.HasTexCoords[pass][stage] == false) {
        return 0;
    }
...
void MeshSaveClass::create_materials(Mtl* nodemtl, DWORD wirecolor)
{
    ...
    // - The material is nullptr: use wire color
            ...
            if (MaterialRemapTable[mi] != -1) {
                ...
                if (err == W3dMaterialDescClass::INCONSISTENT_PASSES) {
                    sprintf(_string1,
                            ...
                            "other materials have %d passes.\nAll Materials must have the same "
                            "number of passes.\n",
                ...
                if (err == W3dMaterialDescClass::INCONSISTENT_SORT_LEVEL) {
                    sprintf(_string1,
                            "Exporting Materials for Mesh: %s\nMaterial %s does not have the same "
                            "Static Sort Level as other materials used on the mesh.\nAll materials "
                            "for a mesh must use the same Static Sort Level value.\n",
                            Header.MeshName, nodemtl->GetSubMtl(mi)->GetName());
                    ExportLog::printf(_string1);
                    throw ErrorClass(_string1);
                }
 ...
 *   08/01/1997 GH  : Created.                                                                 *
 *=============================================================================================*/
void MeshSaveClass::compute_physical_constants(INode* inode, Progress_Meter_Class& meter,
                                               bool voxelize)
{
