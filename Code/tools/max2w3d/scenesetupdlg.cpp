...
int SceneSetupDlg::DoModal(void)
{
    ...
    BOOL result
        = DialogBoxParam(AppInstance, MAKEINTRESOURCE(IDD_SCENE_SETUP),
                         m_MaxInterface->GetMAXHWnd(), (DLGPROC)_thunk_dialog_proc, (LPARAM)this);
...
BOOL SceneSetupDlg::OnOK()
{
    if (!ValidateEditFloat(IDC_LOD_OFFSET)) {
        MessageBox(m_hWnd, "You must enter a valid number for the LOD Offset.", "Not a Number",
                   MB_OK);
        SetFocus(GetDlgItem(m_hWnd, IDC_LOD_OFFSET));
        return FALSE;
    }
    if (!ValidateEditFloat(IDC_DAMAGE_OFFSET)) {
        MessageBox(m_hWnd, "You must enter a valid number for the Damage Offset.", "Not a Number",
                   MB_OK);
