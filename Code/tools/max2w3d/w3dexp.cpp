 ...
 *=============================================================================================*/
int W3dExportClass::DoExport(const TCHAR* filename, ExpInterface* export, Interface* max,
        ...
        */
        ...
        for (i = 0; i < damage_list->Num_Nodes(); i++) {
            if (!Export_Damage_Animations(rootname, csave, damagemeter, (*damage_list)[i])) {
                MessageBox(nullptr, "Damage Animation Export Failure!", "Error",
...
bool W3dExportClass::Export_Animation(char* name, ChunkSaveClass& csave,
                                      Progress_Meter_Class& meter, INode* root)
{
    if (!ExportOptions.ExportAnimation) {
        return true;
    }
    HierarchySaveClass* htree = get_hierarchy_tree();
    ...
    try {
        motion = new MotionClass(ExportInterface->theScene, root, htree, ExportOptions, FrameRate,
                                 &meter, MaxInterface->GetMAXHWnd(), name);
    } catch (ErrorClass err) {
        MessageBox(nullptr, err.error_message, "Error!", MB_OK | MB_SETFOREGROUND);
        return false;
    }
...
bool W3dExportClass::Export_Damage_Animations(char* name, ChunkSaveClass& csave,
                                              Progress_Meter_Class& meter, INode* damage_root)
{
    if (!ExportOptions.ExportAnimation) {
        return true;
    }
    ...
    */
    ...
    for (current_region = 0; current_region < MAX_DAMAGE_REGIONS; current_region++) {
        ...
        MotionClass* motion = nullptr;
        try {
            motion = new MotionClass(ExportInterface->theScene, &bone_list, htree, ExportOptions,
                                     FrameRate, &meter, MaxInterface->GetMAXHWnd(), anim_name,
                                     originoffset);
        } catch (ErrorClass err) {
            MessageBox(nullptr, err.error_message, "Error!", MB_OK | MB_SETFOREGROUND);
            return false;
                                     ...
                                     INode* root, MeshConnectionsClass** out_connection)
{
    ...
    */
    GeometryExportContextClass context(name, csave, world_info, ExportOptions, htree, root,
                                       get_origin_list(), CurTime);
    /*
                                 ...
                                 int lod_count)
{
    ...
    HLodSaveClass hlod_save(connections, lod_count, CurTime, name, htree_name, meter,
                            get_origin_list());
    ...
    */
    if (!ExportOptions.ExportHierarchy) {
        ...
        if (HierarchyTree) {
            ...
            char buf[256];
            sprintf(buf,
                    "Unable to load hierarchy file: %s\nIf this Max file has been moved, please "
                    "re-select the hierarchy file.",
                    HierarchyFilename);
...
bool W3dExportClass::get_export_options(BOOL suppress_prompts)
{
    ...
    if (appdata) {
        ...
        MaxInterface->GetScenePointer()->AddAppDataChunk(
            W3D_EXPORTER_CLASS_ID, SCENE_EXPORT_CLASS_ID, 0, sizeof(ExportInfoAppDataChunkStruct),
            appdata_struct);
    }
...
{
    MaxInterface->ProgressStart("Processing Triangle Mesh", TRUE, progress_callback, nullptr);
}
