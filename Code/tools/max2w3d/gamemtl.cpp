...
public:
    ...
    {
        TheMtl = new GameMtl(loading); // Do not call createNew GameMtl() since the system should
                                       // take care of the additionla stuff in that function
...
enum
{
    ...
    PB_STAGE1_MAPPING_TYPE
    = 53, // (gth) this can't be 55, it is used as an array index	// = 55,	// yes, 55
...
static ParamVersionDesc CurrentPassParameterBlockVersion(PassParameterBlockDescVer5,
                                                         sizeof(PassParameterBlockDescVer5)
                                                             / sizeof(ParamBlockDescID),
                                                         CURRENT_VERSION);
 ...
 *   11/23/98   GTH : Created.                                                                 *
 *=============================================================================================*/
RefResult GameMtl::NotifyRefChanged(Interval changeInt, RefTargetHandle hTarget, PartID& partID,
                                    RefMessage message)
{
    switch (message) {
    case REFMSG_CHANGE: {
...
void GameMtl::Reset()
{
    ...
    for (int pass = 0; pass < W3dMaterialClass::MAX_PASSES; pass++) {
        ...
        IParamBlock* pblock = CreateParameterBlock(
            ...
            sizeof(PassParameterBlockDescVer5) / sizeof(ParamBlockDescID), CURRENT_VERSION);
    ...
    */
    ...
    for (int pass = 0; pass < Get_Pass_Count(); pass++) {
        ...
        */
        ...
        if (pri_gradient.r > 1.0f) {
            pri_gradient.r = 1.0f;
        }
        if (pri_gradient.g > 1.0f) {
            pri_gradient.g = 1.0f;
        }
        if (pri_gradient.b > 1.0f) {
            pri_gradient.b = 1.0f;
        }
...
int GameMtl::Compute_PC_Shader_From_PS2_Shader(int pass)
{
    ...
    static const int src_blend[4] = { ST_ZERO, ST_ONE, ST_SRC_ALPHA, 1 - ST_SRC_ALPHA };
    static const int dest_blend[6]
        = { ST_ZERO, ST_ONE, ST_SRC, 1 - ST_SRC, ST_SRC_ALPHA, 1 - ST_SRC_ALPHA };
...
{
    GetCOREInterface()->GetActionManager()->ActivateActionTable(Game_Mtl_ActionCB, kGameMtlActions);
}
//============================================================================================
...
ActionTable* GameMaterialClassDesc::GetActionTable(int action_index)
{
    ...
    ActionTable* pTab;
    pTab = new ActionTable(kGameMtlActions, kGameMtlContext, name, hAccel, numOps, spActions,
                           AppInstance);
    if (!GetCOREInterface()->GetActionManager()->RegisterActionContext(kGameMtlContext,
                                                                       name.data())) {
