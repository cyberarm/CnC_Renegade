...
void W3dMapClass::Set_Filename(const char* fullpath)
{
    ...
    if (fullpath) {
        ...
        // strupr(fname);						(gth) need to preserve case
        // since unix/PS2 is case sensitive...
...
void W3dMaterialClass::Init(GameMtl* gamemtl)
{
    ...
    for (int pass = 0; pass < gamemtl->Get_Pass_Count(); pass++) {
        ...
        */
        ...
        shader.PostDetailColorFunc
            = gamemtl->Get_Detail_Color_Func(pass); // (gth) set up the post-detail options
        ...
        */
        ...
        for (int stage = 0; stage < MAX_STAGES; stage++) {
            ...
            if (gamemtl->Get_Texture_Enable(pass, stage) && gamemtl->Get_Texture(pass, stage)) {
                ...
                if ((stage == 0)
                    && (gamemtl->Get_Pri_Gradient(pass) == W3DSHADER_PRIGRADIENT_BUMPENVMAP)) {
...
bool W3dMaterialDescClass::MaterialRemapClass::operator==(const MaterialRemapClass& that)
{
    for (int pass = 0; pass < W3dMaterialClass::MAX_PASSES; pass++) {
        ...
        if (VertexMaterialIdx[pass] != that.VertexMaterialIdx[pass]) {
            return false;
        }
        if (ShaderIdx[pass] != that.ShaderIdx[pass]) {
            return false;
        }

        for (int stage = 0; stage < W3dMaterialClass::MAX_STAGES; stage++) {

            if (TextureIdx[pass][stage] != that.TextureIdx[pass][stage]) {
                return false;
            }
        }
    }
                                                                   ...
                                                                   const char* name)
{
    ...
    */
    ...
    if (mat.Is_Multi_Pass_Transparent()) {
        ...
        if (IDYES
            != MessageBox(window, msg, "Confirmation",
    ...
    */
    ...
    for (int pass = 0; pass < PassCount; pass++) {
        ...
        remap.VertexMaterialIdx[pass]
            = Add_Vertex_Material(mat.Get_Vertex_Material(pass), mat.Get_Mapper_Args(pass, 0),
...
bool W3dMaterialDescClass::Pass_Uses_Alpha(int pass)
{
    for (int mi = 0; mi < MaterialRemaps.Count(); mi++) {
        ...
        if ((dst_blend == W3DSHADER_DESTBLENDFUNC_SRC_ALPHA)
            ...
            || (src_blend == W3DSHADER_SRCBLENDFUNC_ONE_MINUS_SRC_ALPHA) || (alpha_test != 0)) {
            return true;
        }
    }
    return false;
...
int W3dMaterialDescClass::Add_Vertex_Material(W3dVertexMaterialStruct* vmat,
                                              const char* mapper_args0, const char* mapper_args1,
...
unsigned long W3dMaterialDescClass::Add_String_To_Crc(const char* str, unsigned long in_crc)
{
    ...
    if (str) {
        ...
        *p_out = 0;
        ...
        if (count) {
            // p_out now points to the ending nullptr - make it point to the
