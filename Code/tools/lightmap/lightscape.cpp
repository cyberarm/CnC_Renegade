 ...
 *   7/27/99    IML : Created.                                                                 *
 *=============================================================================================*/
LightscapeSolve::LightscapeSolve(const char* solvedirectoryname, const char* solvefilenamelist,
                                 CStatusBar* statusptr, const char* statusbarmessage,
                                 bool blendnoise)
    : LightmapPacker()
{
    const char* notsolutiontext = "One or more solve files are not solution (.ls) files.";
    ...
    // Initialize.
    SmoothingAngle
        = DEG_TO_RAD(options.Get_Smoothing_Angle()); //	Convert from degrees to radians.
    SpatialTolerance = options.Get_Spatial_Tolerance(); // No mapping required.
    FilterSharpness = pow(options.Get_Filter_Sharpness() / 50.0l,
                          5.0l); // Map from linear to exponential scale.
    ...
    try {
        ...
        // Parse the files, check that they are solve files and sum the patch cluster count for the
        // progress bar.
        ...
        while (strlen(solvefilename) > 0) {
            ...
            // NOTE: Material processing must be done first because mesh processing is dependant on
            // it.
            //			In order to guarantee this, the solution file is parsed twice, so
            ...
            // Sort the material-texture names by a key defined by the comparison function
            // Compare_Material_Names(). NOTE: qsort MUST (and does) allow for duplicate keys.
            if (MaterialTextureNames.Count() > 0) {
                qsort(&MaterialTextureNames[0], MaterialTextureNames.Count(), sizeof(char*),
                      Compare_Material_Texture_Names);
            }
        ...
        if (Vertices.Count() > 0) {
            ...
            // Sort the vertex pointers by a key defined by the comparison function
            // Compare_Vertices(). NOTE: qsort MUST (and does) allow for duplicate keys.
 ...
 *   7/27/99    IML : Created.                                                                 *
 *=============================================================================================*/
void LightscapeSolve::Add_Material_Texture_Name(const char* materialname,
...
void LightscapeSolve::Add_Texture_Name(unsigned patchindex, const char* materialname)
{
    ...
    if (MaterialTextureNames.Count() > 0) {
        ...
        materialnameptr
            = (char**)bsearch(keyptr, &MaterialTextureNames[0], MaterialTextureNames.Count(),
 ...
 *   1/25/00    IML : Created.                                                                 *
 *=============================================================================================*/
bool LightscapeSolve::Add_Vertex(unsigned vertexindex, const Vector3& p, const Vector3& n,
                                 unsigned patchindex, const Vector2& t)
{
    bool success;
    ...
    if (((int)vertexindex) < Vertices.Count()) {
        ...
        success = (Vertices[vertexindex].Point == p) && (Vertices[vertexindex].FaceNormal == n)
 ...
 *=============================================================================================*/
bool LightscapeSolve::Add_Vertex(unsigned vertexindex, const Vector3& p, const Vector3& n,
                                 unsigned patchindex, const ColorVector& c)
{
    bool success;
 ...
 *   9/16/99    IML : Created.                                                                 *
 *=============================================================================================*/
bool LightscapeSolve::Add_Patch_Face(unsigned patchfaceindex, unsigned patchindex,
                                     unsigned vertexindexcount, unsigned* vertexindices)
{
    bool success;
 ...
 *   1/07/00    IML : Created.                                                                 *
 *=============================================================================================*/
void LightscapeSolve::Find_Vertex(const Vector3& point, const Vector3* smoothingnormalptr,
        ...
        if (smoothingnormalptr != nullptr) {
            ...
            for (int v = 0; v < vertexset.Count(); v++) {
                ...
                if (vertexset[v]->ValidColor) {
                    ...
                    angle = acosf(
                        Vector3::Dot_Product(*smoothingnormalptr, vertexset[v]->FaceNormal));
 ...
 *=============================================================================================*/
void LightscapeSolve::Find_Triangle(const Vector3* points, const Vector3& normal,
                                    PackingTriangle& triangle, SolveStatistics& solvestatistics)
{
    const unsigned notextureid
        = 0xffffffff; // Arbitrary ID used to indicate that there is no associated texture.
 ...
 *  6/05/00    IML : Created.                                                                  *
 *=============================================================================================*/
void LightscapeSolve::Find_Adjacent_Triangles(const PackingTriangle& principaltriangle,
                                              DynamicVectorClass<Triangle>& adjtriangles)
{
    ...
    for (unsigned v = 0; v < Triangle::VERTICES_COUNT; v++) {
        ...
        for (int s = 0; s < spatialvertexset.Count(); s++) {
            ...
            // Is the face that is associated with this spatial vertex within the smoothing angle of
            // the principal triangle's normal?
            ...
            if (angle <= SmoothingAngle) {
                ...
                if (texturenamenodeptr != nullptr) {
                    ...
                    // Find a patch face in the database with patch index that matches the patch
                    // index of the spatial vertex.
                    ...
                    if (patchfaceptr != nullptr) {
                        ...
                        // that matches	when there are duplicate keys.
                        while (patchfaceptr > &PatchFaces[0]) {
                            if (Compare_Patch_Faces(&p, patchfaceptr - 1) != 0) {
                                break;
                            }
                            patchfaceptr--;
                        }
                        ...
                        while ((patchfaceptr < &PatchFaces[0] + PatchFaces.Count())
                               && (Compare_Patch_Faces(&p, patchfaceptr) == 0)) {
                            ...
                            trianglecount
                                = (patchfaceptr
                                       ->VertexIndices[PatchFaceStruct::VERTICES_PER_PATCH_FACE - 1]
                                   == PatchFaceStruct::VERTEX_INDEX_INVALID)
                                ? 1
                                : 2;
                            ...
                            for (unsigned t = 0; t < trianglecount; t++) {
                                ...
                                for (v = 0; v < Triangle::VERTICES_COUNT; v++) {
                                    validuvs
                                        &= Vertices[patchfaceptr->VertexIndices[_vertexindex[t][v]]]
                                               .ValidUV;
                                }
                                ...
                                if (validuvs) {
                                    ...
                                    for (v = 0; v < Triangle::VERTICES_COUNT; v++) {
                                        triangle.Vertices[v].Point
                                            = Vertices[patchfaceptr
                                                           ->VertexIndices[_vertexindex[t][v]]]
                                                  .Point;
                                        triangle.Vertices[v].UV
                                            = Vertices[patchfaceptr
                                                           ->VertexIndices[_vertexindex[t][v]]]
                                                  .UV;
                                    }

                                    // Linearly search the existing adjacent triangles to see if
                                    // this triangle already exists.
                                    hasequivalent = false;
 ...
 *   7/27/99    IML : Created.                                                                 *
 *=============================================================================================*/
void LightscapeSolve::Find_Triangle(const Vector3* points, const Vector3& facenormal,
                                    VerticesStruct* vertices, FaceResultEnum& faceresult)
{
 ...
 *   7/27/99    IML : Created.                                                                 *
 *=============================================================================================*/
void LightscapeSolve::Find_Triangle(const Vector3* points, float tolerance, bool centroidtest,
                                    ...
                                    FaceResultEnum& faceresult)
{
    ...
    for (n = 1; n < Triangle::VERTICES_COUNT; n++) {
        ...
        // Delete all indexes in the candidate index set that have materials that DO NOT occur in
        ...
        if (PatchFaces.Count() > 0) {
            ...
            while (c < candidatevertexset->Count()) {
                ...
                if (patchfaceptr != nullptr) {
                    ...
                    // matches	when there are duplicate keys.
                    while (patchfaceptr > &PatchFaces[0]) {
                        if (Compare_Patch_Faces(&p, patchfaceptr - 1) != 0) {
                            break;
                        }
                    ...
                    // Find all vertices whose points and normals approximately match the requested
                    // normal and add their patch indices to the patch array.
                    while ((patchfaceptr < &PatchFaces[0] + PatchFaces.Count())
                           && (Compare_Patch_Faces(&p, patchfaceptr) == 0)) {
                        ...
                        // NOTE: Define the first triangle to be the points 0-1-2. If the patch face
                        // is a quadrilateral,
                        //			then define the second triangle to be the points
                        //0-2-3.
                        trianglecount = (patchfaceptr->VertexIndices[3]
                                         == PatchFaceStruct::VERTEX_INDEX_INVALID)
                            ? 1
                            : 2;
                        for (unsigned t = 0; t < trianglecount; t++) {

                            // Create a triangle from the material face and test it against the
                            // centroid for containment.
                            TriClass triangle;
                            Vector3 normal;
    ...
    if ((candidatevertexset->Count() > 1) && (facenormal != nullptr)) {
        ...
        // Find the smallest angle (largest dot product) between the vertex face normal and the
        // target face normal.
 ...
 *   7/27/99    IML : Created.                                                                 *
 *=============================================================================================*/
void LightscapeSolve::Find_Vertices(const Vector3& point, float tolerance,
                                    DynamicVectorClass<VerticesStruct*>& vertices)
{
    if (VertexIndices.Count() > 0) {
        ...
        if (vertexindexptr != nullptr) {
            ...
            // Add all vertices that are valid and whose points match the requested point within
            // tolerance.
            while (vertexindexptr < &VertexIndices[0] + VertexIndices.Count()) {
                ...
                if (point.X - vertexptr->Point.X < -tolerance) {
                    break;
                }
...
LightscapeSolve::FaceSizeEnum LightscapeSolve::Face_Size(const Vector3* points, unsigned count)
{
    ...
    if (area < undersizedarea) {
        return (FACE_UNDERSIZED);
    }
 ...
 *   7/27/99    IML : Created.                                                                 *
 *=============================================================================================*/
int LightscapeSolve::Compare_Material_Texture_Names(const void* materialname0,
                                                    const void* materialname1)
{
...
{
    const char* unknownlighttext = "There is a light in the solve with unknown type.";
    ...
    if ((GetType() == LT_LAMP_DAYLIGHT) && (strcmp(GetName(), "Skylight") == 0)) {
        return (TRUE);
    }
    ...
    if ((GetFlags() & LT_LAMP_CAST_NO_DIRECT) && !(GetFlags() & LT_LAMP_RAY_TRACE_DIRECT)) {
        return (TRUE);
    }
    ...
    if (GetIntensity() > 0.0f) {
        ...
        GetDirection(dir);
        Matrix3 direction(dir[0][0], dir[0][1], dir[0][2], dir[1][0], dir[1][1], dir[1][2],
        ...
        case LT_LAMP_DIFFUSE: // Hemispherical distribution with intensity proportional to the
                              // cosine to the direction of light.
        ...
        case LT_LAMP_SPOTLIGHT: // Spotlight distribution. Intensity at beam angle is one-half
        ...
        case LT_LAMP_GENERAL: // General distribution is determined from table if intensities in
                              // given directions.
        ...
        RadianceMap radiancemap(LightscapeSolve::Importer()->Get_Brightness(),
                                LightscapeSolve::Importer()->Get_Contrast(),
                                LightscapeSolve::Importer()->Is_Daylight(),
                                LightscapeSolve::Importer()->Is_Exterior());
        float i = GetIntensity();
        LtTRGBColor irradiance(i * GetFilterColor().GetR(), i * GetFilterColor().GetG(),
                               i * GetFilterColor().GetB());
        ...
        if (GetType() != LT_LAMP_DAYLIGHT) {
            ...
            const float oocutoff
                = 1.0f / 0.025f; // Reciprocal of light intensity	at far attenuation.
            ...
            // ratio (cut-off), assuming an attenuation of c/d^2, where c = monochrome intensity of
            // light source.
...
LtTBool LsMeshBuilder::Finish()
{
 ...
 n patches must match, where n is the smallest patch count of the two files.";
    ...
    if (GetProperties() & LT_MESH_RAWTEXTURE) {
        ...
        for (vertexindex = 0; vertexindex < GetVertexCount(); vertexindex++) {
            ...
            if (usedvertices.Is_Used(vertexindex)) {
                ...
                // UV's must be normalized (in the range 0..1) in order to reference the lightmap
                // properly.
                normalized = ((t.U >= 0.0f) && (t.U <= 1.0f));
                normalized &= ((t.V >= 0.0f) && (t.V <= 1.0f));
                if (!normalized) {
                    throw(notnormalizedtext);
                }

                // Assign an index for this vertex which will uniquely identify it in the vertex
                // database.
                usedvertices.Set_Index(vertexindex, FactoryPtr->Vertex_Count());

                // Add the vertex to the database.
                success = LightscapeSolve::Importer()->Add_Vertex(FactoryPtr->Vertex_Count(), p, n,
                                                                  FactoryPtr->Patch_Count(), t);
                if (!success) {
        ...
        // NOTE: This mesh is not an M2T patch so it must instead contain valid vertex color
        // information. Build an array of face vertices initialized to their respective vertex
        // colors.
        for (vertexindex = 0; vertexindex < GetVertexCount(); vertexindex++) {
            if (usedvertices.Is_Used(vertexindex)) {
        ...
        for (vertexindex = 0; vertexindex < GetVertexCount(); vertexindex++) {
            if (!usedvertices.Is_Used(vertexindex)) {
                ...
                for (facevertexindex = 1; facevertexindex < facevertices.Count();
                     facevertexindex++) {
                    distanceratios[facevertexindex]
                        = d / (point - facevertices[facevertexindex].Point).Length();

                    // Raise the distance ratio to the user specified filter sharpness.
                    distanceratios[facevertexindex]
                        = pow(distanceratios[facevertexindex], filtersharpness);

                    s += distanceratios[facevertexindex];
                }

                for (facevertexindex = 0; facevertexindex < facevertices.Count();
                     facevertexindex++) {
        ...
        for (vertexindex = 0; vertexindex < GetVertexCount(); vertexindex++) {
            if (usedvertices.Is_Used(vertexindex)) {
                ...
                // Add the vertex to the database.
                success = LightscapeSolve::Importer()->Add_Vertex(
                    FactoryPtr->Vertex_Count(), facevertices[facevertexindex].Point, n,
                    FactoryPtr->Patch_Count(), c);
                if (!success) {
    ...
    for (int faceindex = 0; faceindex < GetFaceCount(); faceindex++) {
        ...
        success = LightscapeSolve::Importer()->Add_Patch_Face(FactoryPtr->Patch_Face_Count(),
                                                              FactoryPtr->Patch_Count(),
                                                              facevertexcount, facevertexindices);
        if (!success) {
            throw(unmatchedmodeltext);
        }
 ...
 *   7/30/99    IML : Created.                                                                 *
 *=============================================================================================*/
LightscapeMeshSolve::LightscapeMeshSolve(LightscapeSolve& solve, ChunkClass& trianglechunk,
                                         ChunkClass& vertexchunk)
    : Statistics(vertexchunk.Get_Size() / sizeof(W3dVectorStruct),
                 trianglechunk.Get_Size() / sizeof(W3dTriStruct))
{
        ...
        for (f = 0; f < FaceCount; f++) {
            for (v = 0; v < verticesperface; v++) {
                if (w3dfaces[f].Vindex[v] == vertexindex) {
                    ...
                    facenormal.Set(w3dfaces[f].Normal.X, w3dfaces[f].Normal.Y,
                                   w3dfaces[f].Normal.Z);
                    if (facenormal != zero) {
                        smoothingnormalptr = &facenormal;
                    }
                    break;
                }
            }
    ...
    // For each face in the w3d triangle chunk, match it up with a face in the Lightscape solve
    // database.
    for (f = 0; f < FaceCount; f++) {
