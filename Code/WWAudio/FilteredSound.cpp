...
void FilteredSoundClass::Initialize_Miles_Handle(void)
{
    ...
    if ((m_SoundHandle != nullptr) && (m_hFilter != INVALID_MILES_HANDLE)) {
        ...
        ::AIL_set_filter_sample_preference(m_SoundHandle->Get_HSAMPLE(), "Reverb level",
                                           &reverb_level);

        ::AIL_set_filter_sample_preference(m_SoundHandle->Get_HSAMPLE(), "Reverb reflect time",
                                           &reverb_reflect);

        ::AIL_set_filter_sample_preference(m_SoundHandle->Get_HSAMPLE(), "Reverb decay time",
                                           &reverb_decay);
    }
