      ...
      SubTitleManager(nullptr)
{
    ...
    for (y = 0; y < Bink->Height; y += max_height
             - 2) { // Two pixels are lost due to duplicated edges to prevent bilinear artifacts
    ...
    for (y = 0; y < Bink->Height; y += max_height - 1) {
        for (x = 0; x < Bink->Width; x += max_width - 1) {
            TextureInfos[cnt].Texture
                = new TextureClass(max_width, max_height, D3DFormat_To_WW3DFormat(D3DFMT_R5G6B5),
            ...
            if ((TextureInfos[cnt].TextureWidth + x) > Bink->Width) {
                ...
                TextureInfos[cnt].UV.Right
                    = float(TextureInfos[cnt].TextureWidth - 1) / float(max_width);
            }
            ...
            if ((TextureInfos[cnt].TextureHeight + y) > Bink->Height) {
                ...
                TextureInfos[cnt].UV.Bottom
                    = float(TextureInfos[cnt].TextureHeight + 1) / float(max_height);
            }
            ...
            TextureInfos[cnt].Rect.Right
                = float(TextureInfos[cnt].TextureLocX + TextureInfos[cnt].TextureWidth)
                / float(Bink->Width);
            TextureInfos[cnt].Rect.Bottom
                = float(TextureInfos[cnt].TextureLocY + TextureInfos[cnt].TextureHeight)
                / float(Bink->Height);
...
void BINKMovieClass::Render()
{
    ...
    if (FrameChanged) {
        ...
        BinkCopyToBuffer(Bink, TempBuffer, Bink->Width * 2, Bink->Height, 0, 0,
        ...
        for (unsigned t = 0; t < TextureCount; ++t) {
            ...
            if (d3d_texture) {
                unsigned char* cur_tex_ptr
                    = Get_Tex_Address(TempBuffer, TextureInfos[t].TextureLocX,
    ...
    for (unsigned t = 0; t < TextureCount; ++t) {
        ...
        Renderer.Set_Coordinate_Range(
            RectClass(0.0f, 0.0f, 1.0f, 1.0f)); // Bink->Width,Bink->Height));

        RectClass rect(TextureInfos[t].TextureLocX, TextureInfos[t].TextureLocY,
                       TextureInfos[t].TextureWidth, TextureInfos[t].TextureHeight);
