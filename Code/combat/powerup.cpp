...
SimplePersistFactoryClass<PowerUpGameObjDef, CHUNKID_GAME_OBJECT_DEF_POWERUP>
    _PowerUpGameObjDefPersistFactory;

DECLARE_DEFINITION_FACTORY(PowerUpGameObjDef, CLASSID_GAME_OBJECT_DEF_POWERUP, "PowerUp")
_PowerUpGameObjDefDefFactory;
      ...
      AlwaysAllowGrant(false)
{
    ...
    for (int param_counter = 0; param_counter < ArmorWarheadManager::Get_Num_Armor_Types();
         param_counter++) {
...
bool PowerUpGameObjDef::Save(ChunkSaveClass& csave)
{
    ...
    // WRITE_MICRO_CHUNK( csave, XXXMICROCHUNKID_DEF_IS_CAPTURE_THE_FLAG,
    // IsCaptureTheFlag );
        ...
        case CHUNKID_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_GRANT_SHIELD_STRENGTH,
                                     GrantShieldStrength);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_GRANT_SHIELD_STRENGTH_MAX,
                                     GrantShieldStrengthMax);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_GRANT_WEAPON_ROUNDS,
                                     GrantWeaponRounds);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_PERSISTENT, Persistent);
                    // READ_MICRO_CHUNK( cload, XXXMICROCHUNKID_DEF_IS_CAPTURE_THE_FLAG,
                    // IsCaptureTheFlag );
                    ...
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_GRANT_ANIMATION_NAME,
                                              GrantAnimationName);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_IDLE_ANIMATION_NAME,
                                              IdleAnimationName);
...
bool PowerUpGameObjDef::Grant(SmartGameObj* obj, PowerUpGameObj* p_powerup, bool hud_display) const
{
    ...
    if (GrantShieldStrengthMax != 0) {
        float add = GrantShieldStrengthMax
            * (float)obj->Get_Definition().Get_DefenseObjectDef().ShieldStrengthMax;
    ...
    if (GrantShieldStrength != 0) {
        if ((defense->Get_Shield_Strength() < defense->Get_Shield_Strength_Max())) {
            ...
            if (obj == COMBAT_STAR) {
                ...
                DIAG_LOG(("AMPU", "%1.2f; %1.2f; %1.2f; %1.2f; %1.2f", pos.X, pos.Y, pos.Z,
                          defense->Get_Shield_Strength(), defense->Get_Health()));
            }
        }
        else {
            no_grant_message = IDS_M00DSGN_DSGN1015I1DSGN_TXT; //"You are already at full shield."
    ...
    if (GrantHealth != 0) {
        if (defense->Get_Health() < defense->Get_Health_Max()) {
            ...
            if (obj == COMBAT_STAR) {
                Vector3 pos;
                obj->Get_Position(&pos);
                DIAG_LOG(("HEPU", "%1.2f; %1.2f; %1.2f; %1.2f; %1.2f", pos.X, pos.Y, pos.Z,
                          defense->Get_Shield_Strength(), defense->Get_Health()));
            }
        }
        else {
        ...
        if ((GrantWeapon && !obj->Get_Weapon_Bag()->Is_Weapon_Owned(GrantWeaponID))
            ...
            if (obj == COMBAT_STAR) {
                ...
                const WeaponDefinitionClass* def
                    = WeaponManager::Find_Weapon_Definition(GrantWeaponID);
                if (def) {
                ...
                DIAG_LOG(("WEPU", "%1.2f; %1.2f; %1.2f; %s; %d; %s; %d", pos.X, pos.Y, pos.Z,
                          grant_name, GrantWeaponRounds, weapon_name, ammo));
            }
        }
        else {
...
** PowerUpGameObj
*/
SimplePersistFactoryClass<PowerUpGameObj, CHUNKID_GAME_OBJECT_POWERUP>
    _PowerUpGameObjPersistFactory;
    ...
    if (state != State) {
        ...
        if (State == STATE_GRANTING) {
            ...
            if (Get_Definition().GrantSoundID != 0) {
                WWAudioClass::Get_Instance()->Create_Instant_Sound(Get_Definition().GrantSoundID,
                                                                   Get_Transform());
            }
            ...
            if (Get_Definition().GrantAnimationName.Get_Length() > 0) {
                ...
                HAnimClass* animation = WW3DAssetManager::Get_Instance()->Get_HAnim(
                    Get_Anim_Control()->Get_Animation_Name());
...
void PowerUpGameObj::Update_State(void)
{
    ...
    case STATE_BECOMING_IDLE:
        ...
        if (Get_Definition().IdleSoundID != 0) {
            if (IdleSoundObj == nullptr) {
                IdleSoundObj = WWAudioClass::Get_Instance()->Create_Continuous_Sound(
                    Get_Definition().IdleSoundID);
            }
            if (IdleSoundObj != nullptr) {
    ...
    if (CombatManager::I_Am_Server() && State != STATE_GRANTING) {
        ...
        for (smart_objnode = GameObjManager::Get_Smart_Game_Obj_List()->Head(); smart_objnode;
            ...
            if (soldier && soldier->Wants_Powerups()) {
                ...
                PhysAABoxIntersectionTestClass test(box, DEFAULT_COLLISION_GROUP,
