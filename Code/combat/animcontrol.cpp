...
void AnimChannelClass::Set_Animation(const char* name)
{
    ...
    if ((Animation != nullptr) && (name != nullptr)) {
        if (strcasecmp(Animation->Get_Name(), name) == 0) {
            return;
        }
    }
...
void BlendableAnimChannelClass::Set_Animation(const char* name, float blendtime, float start_frame)
{
    ...
    if ((NewChannel.Peek_Animation() != nullptr) && (name != nullptr)) {
        if (strcasecmp(NewChannel.Peek_Animation()->Get_Name(), name) == 0) {
            return;
        }
    }
...
void BlendableAnimChannelClass::Set_Animation(const HAnimClass* anim, float blendtime,
                                              float start_frame)
{
    // if setting to our current anim, bail
...
void BlendableAnimChannelClass::Update_Model(RenderObjClass* anim_model)
{
    ...
    if (OldChannel.Peek_Animation()) {
        anim_model->Set_Animation(OldChannel.Peek_Animation(), OldChannel.Get_Frame(),
                                  NewChannel.Peek_Animation(), NewChannel.Get_Frame(), blend_ratio);
    }
...
void SimpleAnimControlClass::Set_Animation(const HAnimClass* anim, float blendtime,
                                           float start_frame)
{
...
void HumanAnimControlClass::Build_Skeleton_Anim_Name(StringClass& new_name, const char* name)
{
    ...
    if (new_name.Get_Length() > 14 && Skeleton != 'A'
        && strncasecmp(new_name, "S_A_HUMAN.H_A_", 14) == 0) {
...
void HumanAnimControlClass::Set_Animation(const HAnimClass* anim, float blendtime,
                                          float start_frame)
{
    if (anim != nullptr) {
...
void HumanAnimControlClass::Set_Animation(const char* name1, const char* name2, float ratio,
                                          float blendtime)
{
...
void HumanAnimControlClass::Update(float dtime)
{
    ...
    if (Model != nullptr) {
        ...
        if (total_animations == 0) {
            ...
            Model->Set_Animation(DataList[0].Animation, DataList[0].Frame, DataList[1].Animation,
                                 DataList[1].Frame, percent);
        }
        else {
        ...
        if (_Monitor == this) {
            for (int i = 0; i < total_animations; i++) {
                ...
                a.Format("%s %1.0f%% %1.0f\n", DataList[i].Animation->Get_Name() + 10,
                         DataList[i].Weight * 100, DataList[i].Frame);
