...
{
public:
    WindClass(float heading, float speed, float variability,
...
public:
    ...
    enum RenderModeEnum
    {
        RENDER_MODE_AXIS_ALIGNED, // Render particle oriented about the particle's velocity vector
                                  // (suitable for rain, for example).
        RENDER_MODE_CAMERA_ALIGNED, // Render particle aligned with the camera direction (suitable
                                    // for snow, for example).
        RENDER_MODE_SURFACE_ALIGNED // Render particle such that it looks attached to the surface of
                                    // an object.
    };

    WeatherSystemClass(PhysicsSceneClass* scene, float emittersize, float emitterheight,
...
protected:
    ...
    public:
        ...
        bool RayCast; // Does this ray need to be relocated in the emitter (and therefore needs to
                      // be raycast).
    ...
    float MinRayEndZ; // Current lowest Z-value of end of any ray (used to determine a bounding box
                      // around this render object).
...
public:
    ...
    static bool Set_Precipitation(PrecipitationEnum precipitation, float density,
                                  float ramptime = 0.0f);
    static bool Override_Precipitation(PrecipitationEnum precipitation, float density,
                                       float ramptime = 0.0f);
...
#undef VARID_PARAMETER
    ...
    static bool Set_Wind(float heading, float speed, float variability, float ramptime,
                         bool override);
    static bool Set_Precipitation(PrecipitationEnum precipitation, float density, float ramptime,
                                  bool override);
