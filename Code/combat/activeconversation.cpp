...
void ActiveConversationClass::Start_Conversation(void)
{
    ...
    for (int orator_index = 0; main_speaker == nullptr && orator_index < OratorList.Count();
         orator_index++) {
    ...
    if (orator_info->Get_Flag(OratorClass::FLAG_DONT_TURN_HEAD) == false) {
        ...
        if (orator != nullptr) {
            ...
            if (is_something_to_look_at
                ...
                Matrix3D::Inverse_Transform_Vector(soldier->Get_Transform(), look_pos,
                                                   &relative_look_pos);
        ...
        if (translate_obj != nullptr) {
            ...
            if (sound_def_id != 0) {
                ...
                AudibleSoundClass* speech
                    = WWAudioClass::Get_Instance()->Create_Sound(sound_def_id);
                if (speech != nullptr) {
