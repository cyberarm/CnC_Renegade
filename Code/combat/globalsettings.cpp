...
SimplePersistFactoryClass<GlobalSettingsDef, CHUNKID_GLOBAL_SETTINGS_DEF_GENERAL>
    _GlobalSettingsDefPersistFactory;
...
bool GlobalSettingsDef::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_DEF_FALLING_DAMAGE_MIN_DISTANCE,
                      FallingDamageMinDistance);
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_DEF_FALLING_DAMAGE_MAX_DISTANCE,
    ...
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_GDI_CHARS_TEXTURE,
                               PurchaseGDICharactersTexture);
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_GDI_VEHICLES_TEXTURE,
                               PurchaseGDIVehiclesTexture);
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_GDI_EQUIP_TEXTURE,
                               PurchaseGDIEquipmentTexture);

    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_NOD_CHARS_TEXTURE,
                               PurchaseNODCharactersTexture);
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_NOD_VEHICLES_TEXTURE,
                               PurchaseNODVehiclesTexture);
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_NOD_EQUIP_TEXTURE,
                               PurchaseNODEquipmentTexture);

    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_GDI_MUT_CHARS_TEXTURE,
                               PurchaseGDIMUTCharactersTexture);
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_GDI_MUT_VEHICLES_TEXTURE,
                               PurchaseGDIMUTVehiclesTexture);
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_GDI_MUT_EQUIP_TEXTURE,
                               PurchaseGDIMUTEquipmentTexture);

    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_NOD_MUT_CHARS_TEXTURE,
                               PurchaseNODMUTCharactersTexture);
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_NOD_MUT_VEHICLES_TEXTURE,
                               PurchaseNODMUTVehiclesTexture);
    WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_DEF_PURCHASE_NOD_MUT_EQUIP_TEXTURE,
                               PurchaseNODMUTEquipmentTexture);
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_DEF_MP_STEALTH_DISTANCE_VEHICLE,
                      MPStealthDistanceVehicle);
    csave.End_Chunk();
        ...
        case CHUNKID_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_MAX_COMBAT_CONV_DIST,
                                     MaxCombatConversationDist);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_SOLDIER_WALK_SPEED, SoldierWalkSpeed);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_SOLDIER_CROUCH_SPEED,
                                     SoldierCrouchSpeed);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_FALLING_DAMAGE_MIN_DISTANCE,
                                     FallingDamageMinDistance);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_FALLING_DAMAGE_MAX_DISTANCE,
                                     FallingDamageMaxDistance);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_FALLING_DAMAGE_WARHEAD,
                                     FallingDamageWarhead);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_ENCY_EVENT_STRING_ID,
                                     EncyclopediaEventStringID);

                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_GDI_CHARS_TEXTURE,
                                              PurchaseGDICharactersTexture);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_GDI_VEHICLES_TEXTURE,
                                              PurchaseGDIVehiclesTexture);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_GDI_EQUIP_TEXTURE,
                                              PurchaseGDIEquipmentTexture);

                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_NOD_CHARS_TEXTURE,
                                              PurchaseNODCharactersTexture);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_NOD_VEHICLES_TEXTURE,
                                              PurchaseNODVehiclesTexture);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_DEF_PURCHASE_NOD_EQUIP_TEXTURE,
                                              PurchaseNODEquipmentTexture);

                    READ_MICRO_CHUNK_WWSTRING(cload,
                                              MICROCHUNKID_DEF_PURCHASE_GDI_MUT_CHARS_TEXTURE,
                                              PurchaseGDIMUTCharactersTexture);
                    READ_MICRO_CHUNK_WWSTRING(cload,
                                              MICROCHUNKID_DEF_PURCHASE_GDI_MUT_VEHICLES_TEXTURE,
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_STEALTH_DISTANCE_HUMAN,
                                     StealthDistanceHuman);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_STEALTH_DISTANCE_VEHICLE,
                                     StealthDistanceVehicle);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_MP_STEALTH_DISTANCE_HUMAN,
                                     MPStealthDistanceHuman);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_DEF_MP_STEALTH_DISTANCE_VEHICLE,
                                     MPStealthDistanceVehicle);
...
DECLARE_DEFINITION_FACTORY(HumanLoiterGlobalSettingsDef, CLASSID_GLOBAL_SETTINGS_DEF_HUMAN_LOITER,
                           "HumanLoiter")
...
bool HumanLoiterGlobalSettingsDef::Save(ChunkSaveClass& csave)
{
    ...
    for (int i = 0; i < LoiterAnimList.Count(); i++) {
        WRITE_MICRO_CHUNK_WWSTRING(csave, MICROCHUNKID_HL_DEF_LOITER_ANIM_LIST_ENTRY,
                                   LoiterAnimList[i]);
    }
    csave.End_Chunk();
...
HumanLoiterGlobalSettingsDef* HumanLoiterGlobalSettingsDef::Get_Weaponless_Loiters(void)
{
    if (WeaponlessLoiters == nullptr) {
        WeaponlessLoiters
            = (HumanLoiterGlobalSettingsDef*)DefinitionMgrClass::Find_Typed_Definition(
...
SimplePersistFactoryClass<HUDGlobalSettingsDef, CHUNKID_GLOBAL_SETTINGS_DEF_HUD>
    _HUDGlobalSettingsDefPersistFactory;
...
bool HUDGlobalSettingsDef::Save(ChunkSaveClass& csave)
{
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_HUD_DEF_SECONDARY_OBJECTIVE_COLOR,
                      SecondaryObjectiveColor);
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_HUD_DEF_STAR_BRACKET_TOP_ARMED_SIZE,
                      StarBracketTopArmedSize);
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_HUD_DEF_STAR_BRACKET_TOP_ARMED_OFFSET,
                      StarBracketTopArmedOffset);
    ...
    WRITE_MICRO_CHUNK(csave, MICROCHUNKID_HUD_DEF_SNIPER_DISTANCE_GRAPH_MAX,
        ...
        case CHUNKID_HL_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_PRIMARY_OBJECTIVE_COLOR,
                                     PrimaryObjectiveColor);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SECONDARY_OBJECTIVE_COLOR,
                                     SecondaryObjectiveColor);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TERTIARY_OBJECTIVE_COLOR,
                                     TertiaryObjectiveColor);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_HEALTH_HIGH_COLOR,
                                     HealthHighColor);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_NO_RELATION_COLOR,
                                     NoRelationColor);

                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_INFO_TEXTURE_SIZE,
                                     InfoTextureSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_BRACKET_SIZE,
                                     StarBracketSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_BRACKET_OFFSET,
                                     StarBracketOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_BRACKET_UV, StarBracketUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_BRACKET_TOP_SIZE,
                                     StarBracketTopSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_BRACKET_TOP_OFFSET,
                                     StarBracketTopOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_BRACKET_TOP_UV,
                                     StarBracketTopUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_BRACKET_TOP_ARMED_SIZE,
                                     StarBracketTopArmedSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_BRACKET_TOP_ARMED_OFFSET,
                                     StarBracketTopArmedOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_BRACKET_TOP_ARMED_UV,
                                     StarBracketTopArmedUV);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_BAR_END_OFFSET,
                                     StarBarEndOffset);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_HEALTH_OFFSET,
                                     StarHealthOffset);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_SHIELD_OFFSET,
                                     StarShieldOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_SHIELD_UV, StarShieldUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_WEAPON_ICON_SIZE,
                                     StarWeaponIconSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_STAR_WEAPON_ICON_OFFSET,
                                     StarWeaponIconOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_BRACKET_SIZE,
                                     TargetBracketSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_BRACKET_OFFSET,
                                     TargetBracketOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_BRACKET_UV,
                                     TargetBracketUV);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_ICON_OFFSET,
                                     TargetIconOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_NAME_BAR_SIZE,
                                     TargetNameBarSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_NAME_BAR_OFFSET,
                                     TargetNameBarOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_NAME_BAR_UV,
                                     TargetNameBarUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_NAME_OFFSET,
                                     TargetNameOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_BAR_SIZE, TargetBarSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_BAR_OFFSET,
                                     TargetBarOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_BAR_UV, TargetBarUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_BAR_END_SIZE,
                                     TargetBarEndSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_BAR_END_OFFSET,
                                     TargetBarEndOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_BAR_END_UV, TargetBarEndUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_HEALTH_SIZE,
                                     TargetHealthSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_HEALTH_OFFSET,
                                     TargetHealthOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_HEALTH_UV, TargetHealthUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_SHIELD_SIZE,
                                     TargetShieldSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_SHIELD_OFFSET,
                                     TargetShieldOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_TARGET_SHIELD_UV, TargetShieldUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_RADAR_TEXTURE_SIZE,
                                     RadarTextureSize);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_RADAR_COMPASS_OFFSET,
                                     RadarCompassOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_RADAR_COMPASS_SIZE,
                                     RadarCompassSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_RADAR_COMPASS_BASE_UV,
                                     RadarCompassBaseUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_RADAR_COMPASS_UV_OFFSET,
                                     RadarCompassUVOffset);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_RADAR_HUMAN_BLIP_UV,
                                     RadarHumanBlipUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_RADAR_VEHICLE_BLIP_UV,
                                     RadarVehicleBlipUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_RADAR_STATIONARY_BLIP_UV,
                                     RadarStationaryBlipUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_RADAR_OBJECTIVE_BLIP_UV,
                                     RadarObjectiveBlipUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_RADAR_BLIP_BRACKET_UV,
                                     RadarBlipBracketUV);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_RADAR_OFF_SOUND_ID,
                                     RadarOffSoundID);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_TEXTURE_SIZE,
                                     SniperTextureSize);
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_SCAN_LINE_UV,
                                     SniperScanLineUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_BLACK_COVER_UV,
                                     SniperBlackCoverUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_TILT_BAR, SniperTiltBar);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_TILT_BAR_RATE,
                                     SniperTiltBarRate);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_TILT_BAR_UV,
                                     SniperTiltBarUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_TURN_BAR, SniperTurnBar);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_TURN_BAR_RATE,
                                     SniperTurnBarRate);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_TURN_BAR_UV,
                                     SniperTurnBarUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_DISTANCE_GRAPH,
                                     SniperDistanceGraph);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_DISTANCE_GRAPH_UV,
                                     SniperDistanceGraphUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_DISTANCE_GRAPH_MAX,
                                     SniperDistanceGraphMax);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_ZOOM_GRAPH,
                                     SniperZoomGraph);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_SNIPER_ZOOM_GRAPH_UV,
                                     SniperZoomGraphUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_DAMAGE_INDICATOR_UV,
                                     DamageIndicatorUV);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_HUD_DEF_DAMAGE_DIAG_INDICATOR_UV,
...
bool HumanAnimOverrideDef::Load(ChunkLoadClass& cload)
{
    while (cload.Open_Chunk()) {
        ...
        case CHUNKID_HAO_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_HAO_DEF_RUN_EMPTY_HANDS,
                                              RunEmptyHands);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_HAO_DEF_WALK_EMPTY_HANDS,
                                              WalkEmptyHands);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_HAO_DEF_RUN_AT_CHEST, RunAtChest);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_HAO_DEF_WALK_AT_CHEST,
                                              WalkAtChest);
