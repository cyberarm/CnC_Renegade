...
static void Powerup_Add(const WCHAR* name, int number, const char* texture_name,
                        const RectClass& uv, const Vector2& offset, bool right_list = true)
{
...
#define ANIMATE_TIME 1.0f
    ...
    Vector2 start
        = Render2DClass::Get_Screen_Resolution().Lower_Right() - Vector2(POWERUP_BOX_BASE);
    ...
    for (i = 0; i < MAX_ICONS && i < LeftPowerupIconList.Count(); i++) {
      ...
      // The box is drawn additive, so his color is green alpha
        ...
        PowerupTextRenderer->Set_Location(
            Vector2(draw_box.Left + 1, draw_box.Top + POWERUP_BOX_HEIGHT - 15));
    ...
    for (i = 0; i < MAX_ICONS && i < RightPowerupIconList.Count(); i++) {
      ...
      // The box is drawn additive, so his color is green alpha
        ...
        PowerupTextRenderer->Set_Location(
            Vector2(left_edge, draw_box.Top + POWERUP_BOX_HEIGHT - 15));
        PowerupTextRenderer->Draw_Sentence(white);
...
static void Weapon_Init(void)
{
    ...
    draw_box += Render2DClass::Get_Screen_Resolution().Lower_Right() - Vector2(WEAPON_OFFSET)
        - draw_box.Upper_Left();
...
static void Weapon_Update(void)
{
    ...
    if (weapon != nullptr) {
        ...
        if (weapon->Get_Total_Rounds() == -1) {
            ...
            //			text.Format( "%03d", weapon->Get_Total_Rounds() -
            //weapon->Get_Clip_Rounds() );
            Generate_WChar_Text_From_Number(tmp_text, 3, 3,
                                            weapon->Get_Total_Rounds() - weapon->Get_Clip_Rounds());
        }
    ...
    if (COMBAT_STAR->Get_Vehicle()) {
        ...
        if (_LastVehicleSeat != seat) {
            ...
            WeaponNameRenderer->Set_Location(Render2DClass::Get_Screen_Resolution().Lower_Right()
                                             - text_size);
        ...
        if (weapon != nullptr) {
            ...
            //			SurfaceClass::SurfaceDescription surface_desc;
            //			WeaponImageRenderer->Peek_Texture()->Get_Level_Description(
            //surface_desc ); 			float size = surface_desc.Width;		// Assume square
...
static void Build_Weapon_Chart_Icons(void)
{
    ...
    for (int column = 1; column <= 10; column++) {
        ...
        // Add column header
        int key
            = Input::Get_Primary_Key_For_Function(INPUT_FUNCTION_SELECT_WEAPON_0 + (column % 10));
        ...
        Vector2 text_offset
            = pos - text_size / 2 - Vector2(0, WeaponChartSpacing.Y * screen_scale * 0.75f);
        ...
        for (int i = 0; i < weapon_bag->Get_Count(); i++) {
            ...
            if (weapon != nullptr) {
                ...
                if (renderer->Peek_Texture() != nullptr) {
                    //					SurfaceClass::SurfaceDescription
                    //surface_desc; 					renderer->Peek_Texture()->Get_Level_Description( surface_desc );
                    //					float size = surface_desc.Width;
                    //// Assume square
...
{
    if (COMBAT_STAR == nullptr || COMBAT_STAR->Get_Weapon_Bag() == nullptr
        || COMBAT_STAR->Get_Vehicle() != nullptr) {
        Clear_Weapon_Chart_Icons();
        return;
    }
    ...
    // subtract 1 because the first is always nullptr
    ...
    for (int column = 1; column <= 10; column++) {
        ...
        for (i = 0; i < weapon_bag->Get_Count(); i++) {
            ...
            if (weapon != nullptr) {
                ...
                //weapon->Is_Switching() || weapon->Get_Total_Rounds() == 0 ) { 				if (
                //weapon->Is_Switching() || weapon->Get_Total_Rounds() == 0 ) {
...
static void Damage_Update(void)
{
    ...
    Damage_Add_Indicator(2, OFFSET_X - VERT_WIDTH / 2, -VERT_HEIGHT / 2, OFFSET_X + VERT_WIDTH / 2,
                         VERT_HEIGHT / 2);
    Damage_Add_Indicator(4, -HORIZ_WIDTH / 2, OFFSET_Y - HORIZ_HEIGHT / 2, HORIZ_WIDTH / 2,
                         OFFSET_Y + HORIZ_HEIGHT / 2);
    Damage_Add_Indicator(6, -OFFSET_X - VERT_WIDTH / 2, -VERT_HEIGHT / 2,
                         -OFFSET_X + VERT_WIDTH / 2, VERT_HEIGHT / 2);
...
WideStringClass TargetNameString; // Cache the current target name string so that we know when the
                                  // string changes
...
static void Target_Update(void)
{
    ...
    if (d_obj != nullptr) {
        ...
        if (obj && COMBAT_STAR) {
            ...
            int color
                = HUDGlobalSettingsDef::Get_Instance()->Get_No_Relation_Color().Convert_To_ARGB();
            if (p_obj != nullptr) {
                if (COMBAT_STAR->Is_Teammate(p_obj)) {
                    color = HUDGlobalSettingsDef::Get_Instance()
                                ->Get_Friendly_Color()
                                .Convert_To_ARGB();
                }
                else if (COMBAT_STAR->Is_Enemy(p_obj)) {
                    color
                        = HUDGlobalSettingsDef::Get_Instance()->Get_Enemy_Color().Convert_To_ARGB();
                }
            }
            ...
            if (draw_health) {
                ...
                if (def) {
                    ...
                    //				health_percent = WWMath::Clamp( def->Get_Health() /
                    //def->Get_Health_Max(), 0, 1 );
            ...
            if (obj->Get_Translated_Name_ID() != 0) {
                TDBObjClass* translate_obj
                    = TranslateDBClass::Find_Object(obj->Get_Translated_Name_ID());
                if (translate_obj) {
            ...
            if (draw_chevrons) {
                ...
                enterable_box
                    += Vector2(0, TARGET_ENTERABLE_BOUNCE * (WWMath::Sin(enterable_bounce) - 1));
                ...
                TargetRenderer->Add_Tri(enterable_box.Upper_Right(), enterable_box.Upper_Left(),
                                        enterable_box.Center(), uv.Upper_Right(), uv.Upper_Left(),
                                        uv.Center(), color);
                enterable_box += Vector2(0, enterable_box.Height() * 0.6f);
                TargetRenderer->Add_Tri(enterable_box.Upper_Right(), enterable_box.Upper_Left(),
                                        enterable_box.Center(), uv.Upper_Right(), uv.Upper_Left(),
...
static RectClass Get_Target_Box(PhysicalGameObj* obj)
{
    ...
    if (po != nullptr) {
        ...
        boxview_tm.Look_At(COMBAT_CAMERA->Get_Transform().Get_Translation(),
                           obj_tm * obj_box.Center, 0.0f);
        boxview_tm.Get_Orthogonal_Inverse(boxview_inv_tm);
        Matrix3D obj_to_boxview_tm = boxview_inv_tm * obj_tm;

        AABoxClass boxview_box;
        obj_to_boxview_tm.Transform_Center_Extent_AABox(obj_box.Center, obj_box.Extent,
                                                        &boxview_box.Center, &boxview_box.Extent);
        ...
        boxview_to_camera_tm.Transform_Center_Extent_AABox(boxview_box.Center, boxview_box.Extent,
                                                           &camera_box.Center, &camera_box.Extent);
...
{
        Font3DInstanceClass * font = WW3DAssetManager::Get_Instance()->Get_Font3DInstance(
LARGE_FONT ); SET_REF_OWNER( font ); ScoreRenderer = new Render2DTextClass( font );
...
#define POG_SPACING Vector2(10, 0)
    ...
    if (rebuild || ObjectiveManager::Are_HUD_Objectives_Changed()) {
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if (renderer) {
                ...
                if (age < POG_FLY_TIME) {
                    Vector2 offset = Render2DClass::Get_Screen_Resolution().Center()
                        - Render2DClass::Get_Screen_Resolution().Upper_Right();
                    offset.X *= 0.75f;
    ...
    if (count > 0) {
        ...
        Vector3 objective_pos
            = ObjectiveManager::Get_HUD_Objectives_Location(CurrentObjectiveIndex);

        if (COMBAT_STAR) {
            Vector3 rel_pos;
            Matrix3D::Inverse_Transform_Vector(COMBAT_STAR->Get_Transform(), objective_pos,
                                               &rel_pos);
    ...
    if (CachedObjectiveIndex != CurrentObjectiveIndex || irange != CachedRange) {
        ...
        if (objective_count > 0) {
            ...
            WideStringClass str(ObjectiveManager::Get_HUD_Objectives_Message(CurrentObjectiveIndex),
                                true);
...
static void Info_Editor_Update(void)
{
    ...
    float move = Input::Get_Amount(INPUT_FUNCTION_MOVE_LEFT)
    ...
    float _left = 0;
    float left = Input::Get_Amount(INPUT_FUNCTION_TURN_LEFT)
    ...
    if (changed) {
        Debug_Say(("Vector2 %s( %d, %d );\n", InfoEditorFieldList[index].Name,
                   (int)InfoEditorFieldList[index].Value->U,
...
static void Info_Editor_Shutdown(void)
{
    while (InfoEditorFieldList.Count() > 0) {
        Debug_Say(("Vector2 %s( %d, %d );\n", InfoEditorFieldList[0].Name,
                   (int)InfoEditorFieldList[0].Value->U, (int)InfoEditorFieldList[0].Value->V));
...
static void Info_Update_Health_Shield(void)
{
    ...
    if (COMBAT_STAR) {
        ...
        if (def && def->Get_Shield_Strength_Max()) {
            ...
            shield_percent
                = WWMath::Clamp(def->Get_Shield_Strength() / def->Get_Shield_Strength_Max(), 0, 1);
        }
    }
...
void HUDClass::Init(bool render_available)
{
    ...
    RenderImages[ACTION_STATUSBAR_RENDERER]->Set_Coordinate_Range(
    ...
    RenderImages[DEMO_HUD_IMAGE]->Add_Quad(
        RectClass(float(8), float(-8), float(256 + 8), float(128 - 8)),
    ...
    RenderImages[DEMO_HUD_IMAGE2]->Add_Quad(
        RectClass(rect.Right + float(-256 - 8), rect.Bottom + float(-128 + 8),
...
void HUDClass::Think()
{
    ...
    if (HUDInfo::Display_Action_Status_Bar()) {
        ...
        RenderImages[ACTION_STATUSBAR_RENDERER]->Add_Line(
            status_bar_rect.Upper_Left(), status_bar_rect.Upper_Right(), 1, 0xFFFFFFFF);
        RenderImages[ACTION_STATUSBAR_RENDERER]->Add_Line(
            status_bar_rect.Upper_Right(), status_bar_rect.Lower_Right(), 1, 0xFFFFFFFF);
        RenderImages[ACTION_STATUSBAR_RENDERER]->Add_Line(
            status_bar_rect.Lower_Right(), status_bar_rect.Lower_Left(), 1, 0xFFFFFFFF);
        RenderImages[ACTION_STATUSBAR_RENDERER]->Add_Line(
            status_bar_rect.Lower_Left(), status_bar_rect.Upper_Left(), 1, 0xFFFFFFFF);
    ...
    unsigned long reticle_color
        = HUDGlobalSettingsDef::Get_Instance()->Get_No_Relation_Color().Convert_To_ARGB();

    if (HUDInfo::Get_Weapon_Target_Object() != nullptr) {
        reticle_color
            = HUDGlobalSettingsDef::Get_Instance()->Get_Friendly_Color().Convert_To_ARGB();
    ...
    RenderImages[RETICLE]->Add_Quad(
        RectClass(reticle_offset.X - RETICLE_WIDTH / 2, reticle_offset.Y - RETICLE_HEIGHT / 2,
                  reticle_offset.X + RETICLE_WIDTH / 2, reticle_offset.Y + RETICLE_HEIGHT / 2),
        reticle_color);
    ...
    // TSS092401 if ( weapon ) {
    if (CombatManager::Is_Gameplay_Permitted() && (weapon != nullptr)
        && CombatManager::Is_Hit_Reticle_Enabled()) {
        WWPROFILE("Reticle");
        ...
        RenderImages[RETICLE_HIT]->Add_Quad(RectClass(reticle_hit_offset.X - RETICLE_WIDTH / 2,
                                                      reticle_hit_offset.Y - RETICLE_HEIGHT / 2,
                                                      reticle_hit_offset.X + RETICLE_WIDTH / 2,
                                                      reticle_hit_offset.Y + RETICLE_HEIGHT / 2),
                                            reticle_color);
...
void HUDClass::Add_Powerup_Weapon(int id, int rounds)
{
    ...
    if (def) {
        if (!def->IconTextureName.Is_Empty()) {
            Powerup_Add(TranslateDBClass::Get_String(def->IconNameID), rounds, def->IconTextureName,
                        def->IconTextureUV, def->IconOffset);
        }
    }
}
...
void HUDClass::Add_Shield_Grant(float strength)
{
    ...
    Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_Armor_00), (int)strength, texture_name,
                RectClass(0, 0, 64, 64), Vector2(10, 40));
}
...
void HUDClass::Add_Health_Grant(float amount)
{
    ...
    Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_Health_00), (int)amount, texture_name,
                RectClass(0, 0, 64, 64), Vector2(10, 40));
}

void HUDClass::Add_Shield_Upgrade_Grant(float strength)
{
    Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_Armor_Upgrade), (int)strength,
                "hud_armedal.tga", RectClass(0, 0, 64, 64), Vector2(10, 40), false);
}

void HUDClass::Add_Health_Upgrade_Grant(float amount)
{
    Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_Health_Upgrade), (int)amount,
                "hud_hemedal.tga", RectClass(0, 0, 64, 64), Vector2(10, 40), false);
}
...
void HUDClass::Add_Key_Grant(int key)
{
    ...
    Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_SecurityCard), 0, texture_name,
...
void HUDClass::Add_Objective(int type)
{
    if (type == ObjectiveManager::TYPE_PRIMARY) {
        Powerup_Add(TranslateDBClass::Get_String(IDS_Enc_Obj_Priority_0_Primary), 0, "p_eva1.tga",
                    RectClass(0, 0, 64, 64), Vector2(10, 40), false);
    }
...
void HUDClass::Add_Data_Link(void)
{
    ...
    last = cur;
    Powerup_Add(TranslateDBClass::Get_String(IDS_Power_up_DataDisc_01), 0, "hud_cd_rom.tga",
                RectClass(0, 0, 64, 64), Vector2(10, 40), false);
}
...
void HUDClass::Add_Map_Reveal(void)
{
    Add_Data_Link();
    //	Powerup_Add( TranslateDBClass::Get_String(IDS_Power_up_DataDisc_01),  0, "hud_cd_rom.tga",
    //RectClass( 0,0,64,64 ),	Vector2( 10,40 ), false ); 	Powerup_Add(
    //TranslateDBClass::Get_String(IDS_Enc_Pow_Bonus_Map_Name),  0, "hud_cd_rom.tga", RectClass(
    //0,0,64,64 ),	Vector2( 10,40 ), false );
}
