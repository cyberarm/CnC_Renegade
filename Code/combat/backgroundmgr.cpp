...
void StarfieldClass::Configure(const Vector3& orientation, float length, float radius,
                               const Vector3& color0, const Vector3& color1, float alpha)
{
...
void StarfieldClass::Configure()
{
    ...
    for (t = 0; t < TriangleCount; t++) {
        ...
        // NOTE: Generate all random numbers regardless of orientation to ensure a consistent
        // starfield.
        ...
        // Optimization: If the star is located an arbitrary angle above the horizon deem it
        // visible.
        if (d.Z >= theta) {
...
void StarfieldClass::Render()
{
    if (Is_Visible()) {
        ...
        DynamicVBAccessClass dynamicvb(BUFFER_TYPE_DYNAMIC_DX8, dynamic_fvf_type,
                                       ActiveVertexCount);
        {
            const float texcoordarray[VERTICES_PER_TRIANGLE][2]
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
CloudLayerClass::CloudLayerClass(float maxdistance, const char* texturename,
                                 const Vector2& velocity, float tilefactor, bool rotate)
    : RowCount(4)
{
    const Vector3 white(1.0f, 1.0f, 1.0f);
    ...
    for (segment = 0; segment < segmentcount; segment++) {
        latitude = segment * twopioosegmentcount;
        for (row = 0; row < RowCount + 1; row++) {
            longitude = (sinf(row * oorowcount * maxlongitude) * oosinmaxlongitude) * row
                * oorowcount * maxlongitude;
    ...
    Texture = WW3DAssetManager::Get_Instance()->Get_Texture(
        texturename, TextureClass::MIP_LEVELS_ALL, WW3D_FORMAT_UNKNOWN);
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
void SkyGlowClass::Configure(const Vector2& hotdirection, const Vector3& horizoncolor,
                             float coldintensity)
{
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
LightningBoltClass::LightningBoltClass(int branchcount, Matrix3D& m, float length,
                                       float childlength, float width, float amplitude)
    : Branches(nullptr)
{
    ...
    texture
        = WW3DAssetManager::Get_Instance()->Get_Texture(texturename, TextureClass::MIP_LEVELS_3);
    Set_Texture(texture);
    REF_PTR_RELEASE(texture);
    ...
    if (BranchCount > 0) {
        ...
        for (int b = 0; b < BranchCount; b++) {
            ...
            angle = WWMath::Lerp(minbranchangle, maxbranchangle,
            ...
            v = MIN(((int)vertexcount) - 1,
                    b * oobranchcount * (((int)vertexcount) - 1)
                        + _RandomNumber(0, +branchrandomness));
            ...
            if (m0.Get_Translation().Z > 0.0f) {
                Branches[b].LightningBolt
                    = NEW_REF(LightningBoltClass, (branchcount, m0, l, l, w, a));
            }
            else {
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
LightningClass::LightningClass(float extent, float startdistance, float enddistance, float heading,
                               float distribution)
    : Time(0),
      PlayedThunder(false)
{
    ...
    static const char* _thundersamplename[majorsamplecount][minorsamplecount]
        = { { "Thunder01", "Thunder02", "Thunder03" }, { "Thunder04", "Thunder05", "Thunder06" } };
    ...
    Distance
        = WWMath::Lerp(startdistance, enddistance, _RandomNumber(0, randomness) * oorandomness);
    ThunderDelayTime
        = WWMath::Lerp((float)minthunderdelaytime, (float)maxthunderdelaytime, Distance);
    ...
    latitude = heading + (0.5f * WWMATH_PI)
        + (_RandomNumber(-((int)randomness), ((int)randomness)) * oorandomness * WWMATH_PI
           * distribution);
    x = cosf(latitude);
    y = sinf(latitude);
    Direction.Set(x, y);
    ...
    LightningBolt
        = NEW_REF(LightningBoltClass, (branchcount, m, length, childlength, width, amplitude));
    ...
    lightningsourceheight
        = WWMath::Lerp(maxlightningsourceheight, minlightningsourceheight, Distance);
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
bool LightningClass::Update(Matrix3D& t, Vector3& additivecolor,
                            SoundEnvironmentClass* soundenvironment)
{
    ...
    if (Time < lightningtime) {
        ...
        const static float _intensities[phasecount]
            = { 1.00f, 0.75f, 0.50f, 0.50f, 1.00f, 0.50f, 0.10f };
        const static bool _renderbranches[phasecount]
        ...
        if (Time < lightningtime + ThunderDelayTime) {
            ...
            if (!PlayedThunder) {
                ...
                sound = WWAudioClass::Get_Instance()->Create_Sound(ThunderSampleName, nullptr, 0,
                                                                   CLASSID_PSEUDO3D);
                if (sound != nullptr) {
 ...
 *=============================================================================================*/
WarBlitzClass::WarBlitzClass(float extent, float startdistance, float enddistance, float heading,
                             float distribution)
    : Time(0),
      PlayedSample(false)
{
...
bool WarBlitzClass::Update(Matrix3D& t, Vector3& additivecolor)
{
    ...
    if (Time < warblitztime) {
        ...
        const unsigned phasecount = 7;
        const static float _intensities[phasecount]
            = { 0.50f, 1.00f, 0.80f, 0.60f, 0.40f, 0.20f, 0.10f };
        ...
        if (Time >= warblitztime + SampleDelayTime) {
            if (!PlayedSample) {
                ...
                sound = WWAudioClass::Get_Instance()->Create_Sound(samplename, nullptr, 0,
                                                                   CLASSID_PSEUDO3D);
                if (sound != nullptr) {
      ...
      WarBlitzDistribution(0.5f)
{
    ...
    CloudLayer0
        = new CloudLayerClass(Extent, "CloudLayer.tga", Vector2(0.0030f, 0.0006f), 1.2f, false);
    WWASSERT(CloudLayer0 != nullptr);

    CloudLayer1
        = new CloudLayerClass(Extent, "CloudLayer.tga", Vector2(0.0050f, 0.0010f), 0.8f, true);
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
Vector3 SkyClass::Interpolate_Color(const unsigned char colortable[][3], unsigned colorcount,
                                    float interpolant)
{
    ...
    lowercolor
        = Vector3(colortable[lowerindex][0] * ooucharmax, colortable[lowerindex][1] * ooucharmax,
                  colortable[lowerindex][2] * ooucharmax);
    uppercolor
        = Vector3(colortable[upperindex][0] * ooucharmax, colortable[upperindex][1] * ooucharmax,
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
float SkyClass::Interpolate_Scalar(const unsigned char scalartable[], unsigned scalarcount,
                                   float interpolant)
{
    const float ooucharmax = 1.0f / UCHAR_MAX;
        ...
        if (LightningCountdown[l] <= 0) {
            ...
            if (Lightning[l] == nullptr) {
                ...
                if (LightningIntensity > 0.0f) {
                    Lightning[l]
                        = new LightningClass(Extent, LightningStartDistance, LightningEndDistance,
    ...
    if (WarBlitzCountdown <= 0) {
        ...
        if (WarBlitz == nullptr) {
            ...
            if (WarBlitzIntensity > 0.0f) {
                WarBlitz = new WarBlitzClass(Extent, WarBlitzStartDistance, WarBlitzEndDistance,
                                             WarBlitzHeading, WarBlitzDistribution);
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
void BackgroundMgrClass::Init(SimpleSceneClass* renderscene,
                              SoundEnvironmentClass* soundenvironment, bool render_available)
{
...
bool BackgroundMgrClass::Set_Clouds(float cloudcover, float cloudgloominess, float ramptime,
                                    bool override)
{
    if ((cloudcover >= 0.0f) && (cloudcover <= 1.0f) && (cloudgloominess >= 0.0f)
        && (cloudgloominess <= 1.0f) && (ramptime >= 0.0f)) {
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
bool BackgroundMgrClass::Set_Lightning(float intensity, float startdistance, float enddistance,
                                       float heading, float distribution, float ramptime)
{
    if (CombatManager::I_Am_Server()) {
        if (Set_Lightning(intensity, startdistance, enddistance, heading, distribution, ramptime,
                          false)) {
            _TheBackgroundMgr.Set_Object_Dirty_Bit(NetworkObjectClass::BIT_RARE, true);
            return (true);
        }
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
bool BackgroundMgrClass::Override_Lightning(float intensity, float startdistance, float enddistance,
                                            float heading, float distribution, float ramptime)
{
    if (CombatManager::I_Am_Server()) {
...
bool BackgroundMgrClass::Set_Lightning(float intensity, float startdistance, float enddistance,
                                       ...
                                       bool override)
{
    if ((intensity >= 0.0f) && (intensity <= 1.0f) && (startdistance >= 0.0f)
        && (startdistance <= 1.0f) && (enddistance >= 0.0f) && (enddistance <= 1.0f)
        && (heading >= 0.0f) && (heading <= 360.0f) && (distribution >= 0.0f)
        && (distribution <= 1.0f) && (ramptime >= 0.0f)) {
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
void BackgroundMgrClass::Get_Lightning(float& intensity, float& startdistance, float& enddistance,
                                       float& heading, float& distribution)
{
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
bool BackgroundMgrClass::Set_War_Blitz(float intensity, float startdistance, float enddistance,
                                       float heading, float distribution, float ramptime,
                                       bool override)
{
    if ((intensity >= 0.0f) && (intensity <= 1.0f) && (startdistance >= 0.0f)
 ...
 *   09/15/00    IML : Created.                                                                *
 *=============================================================================================*/
void BackgroundMgrClass::Get_War_Blitz(float& intensity, float& startdistance, float& enddistance,
                                       float& heading, float& distribution)
{
...
void BackgroundMgrClass::Update(PhysicsSceneClass* mainscene, CameraClass* camera)
{
    ...
    if (Is_Dirty() || cloudsmodified) {
        _Sky->Set_Clouds(_Parameters[PARAMETER_CLOUD_COVER].Value(),
                         _Parameters[PARAMETER_CLOUD_GLOOMINESS].Value());
    }

    skytintmodified
        = _Parameters[PARAMETER_SKY_TINT_FACTOR].Update(time, _SkyTintOverrideCount > 0);
    if (Is_Dirty() || skytintmodified) {
        _Sky->Set_Tint_Factor(_Parameters[PARAMETER_SKY_TINT_FACTOR].Value());
    }

    lightningmodified
        = _Parameters[PARAMETER_LIGHTNING_INTENSITY].Update(time, _LightningOverrideCount > 0);
    lightningmodified |= _Parameters[PARAMETER_LIGHTNING_START_DISTANCE].Update(
        time, _LightningOverrideCount > 0);
    lightningmodified
        |= _Parameters[PARAMETER_LIGHTNING_END_DISTANCE].Update(time, _LightningOverrideCount > 0);
    lightningmodified
        |= _Parameters[PARAMETER_LIGHTNING_HEADING].Update(time, _LightningOverrideCount > 0);
    lightningmodified
        |= _Parameters[PARAMETER_LIGHTNING_DISTRIBUTION].Update(time, _LightningOverrideCount > 0);
    if (Is_Dirty() || lightningmodified) {
        _Sky->Set_Lightning(_Parameters[PARAMETER_LIGHTNING_INTENSITY].Value(),
                            _Parameters[PARAMETER_LIGHTNING_START_DISTANCE].Value(),
                            _Parameters[PARAMETER_LIGHTNING_END_DISTANCE].Value(),
                            _Parameters[PARAMETER_LIGHTNING_HEADING].Value(),
    ...
    if (Is_Dirty() || warblitzmodified) {
        _Sky->Set_War_Blitz(_Parameters[PARAMETER_WAR_BLITZ_INTENSITY].Value(),
                            _Parameters[PARAMETER_WAR_BLITZ_START_DISTANCE].Value(),
                            _Parameters[PARAMETER_WAR_BLITZ_END_DISTANCE].Value(),
                            _Parameters[PARAMETER_WAR_BLITZ_HEADING].Value(),
