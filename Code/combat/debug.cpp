...
bool wwdebug_trigger_handler(int trigger_num)
{
    ...
    case WWPHYS_TRIGGER_COLLISION_DEBUGGING:
        return DebugManager::Option_Is_Enabled(
            DebugManager::DEBUG_COLLISION_MESSAGES); // Input::Get_State(
                                                     // INPUT_FUNCTION_DEBUG_COLLISION_MESSAGES );
    case WWPHYS_TRIGGER_COLLISION_DISPLAY:
        return DebugManager::Option_Is_Enabled(
            DebugManager::DEBUG_COLLISION_DISPLAY); // Input::Get_State(
                                                    // INPUT_FUNCTION_DEBUG_SHOW_COLLISIONS );
    ...
    case WW3D_TRIGGER_PROCESS_STATS:
        return DebugManager::Option_Is_Enabled(
            DebugManager::DEBUG_PROCESS_STATS); // Input::Get_State(
                                                // INPUT_FUNCTION_DEBUG_RENDER_STATS );
    ...
    case WW3D_TRIGGER_SURFACE_CACHE_STATS:
        return DebugManager::Option_Is_Enabled(
            DebugManager::DEBUG_SURFACE_CACHE); // Input::Get_State(
                                                // INPUT_FUNCTION_DEBUG_SURFACE_CACHE );
    default:
...
** AND _CRTDBG_MAP_ALLOC is not defined (this causes link errors).  If you are using
*_CRTDBG_MAP_ALLOC,
...
/*
** List of addresses to 'watch'. You can stuff them in here and compile or manually poke them in
*with the debugger at run time.
*/
...
void* __cdecl operator new(unsigned int s)
{
    ...
    */
    ...
    /*
    ** Extract the return address. This is the default worst case amount of info we can get about
    *the
    ** stack.
    */
    __asm
    ...
    */
    ...
    for (_i = 0; _i < min(_address_strings, ARRAY_SIZE(NewAddressList)); _i++) {
        if (memcmp(NewAddressList[_i].Addresses, ReturnAddresses,
                   WALK_FRAMES * sizeof(ReturnAddresses[0]))
            == 0) {
    ...
    ** entry.
    */
    memcpy(NewAddressList[_index].Addresses, ReturnAddresses,
           WALK_FRAMES * sizeof(ReturnAddresses[0]));
    if (num_frames == 0) {
        ...
        */
        ...
        for (_i = 0; _i < WALK_FRAMES; _i++) {
            ...
            if (_i == 0) {
                sprintf(ptr, "\nAddr: %08X (%s + %d)\n", NewAddressList[_index].Addresses[_i],
                        _temp_str, _displacement);
            }
            else {
                ...
                if (strlen(ptr) + strlen(_compose_str)
                    >= sizeof(NewAddressList[_index].AddressName)) {
                    break;
                }
    ...
    */
    void* newmem = ::operator new(s, 1, NewAddressList[_index].AddressName,
                                  NewAddressList[_index].AddressLine);
    if (NewMutex) {
        ReleaseMutex(NewMutex);
    }
    return (newmem);
...
static char const* ImagehelpFunctionNames[]
    = { "SymCleanup",    "SymGetSymFromAddr",      "SymInitialize",
        "SymLoadModule", "SymSetOptions",          "SymUnloadModule",
        "StackWalk",     "SymFunctionTableAccess", "SymGetModuleBaseType",
        nullptr };
