...
*/
SimplePersistFactoryClass<ExplosionDefinitionClass, CHUNKID_EXPLOSION_DEF>
    _ExplosionDefPersistFactory;

DECLARE_DEFINITION_FACTORY(ExplosionDefinitionClass, CLASSID_DEF_EXPLOSION, "Explosion")
_ExplosionDefDefFactory;

uint32 ExplosionDefinitionClass::Get_Class_ID(void) const
{
    return CLASSID_DEF_EXPLOSION;
}
const PersistFactoryClass& ExplosionDefinitionClass::Get_Factory(void) const
{
    return _ExplosionDefPersistFactory;
}
        ...
        case CHUNKID_EXPLOSION_DEF_VARIABLES:
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_EXPLOSION_DEF_DAMAGE_STRENGTH,
                                     DamageStrength);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_EXPLOSION_DEF_DAMAGE_WARHEAD,
                                     DamageWarhead);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_EXPLOSION_DEF_DAMAGE_IS_SCALED,
                                     DamageIsScaled);
                    READ_MICRO_CHUNK_WWSTRING(cload, MICROCHUNKID_EXPLOSION_DEF_DECAL_FILENAME,
                                              DecalFilename);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_EXPLOSION_DEF_DECAL_SIZE, DecalSize);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_EXPLOSION_DEF_ANIMATED_EXPLOLSION,
                                     AnimatedExplosion);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_EXPLOSION_DEF_CAMERASHAKEINTENSITY,
                                     CameraShakeIntensity);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_EXPLOSION_DEF_CAMERASHAKERADIUS,
                                     CameraShakeRadius);
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_EXPLOSION_DEF_CAMERASHAKEDURATION,
                                     CameraShakeDuration);
                default:
...
**
*/
void ExplosionManager::Create_Explosion_At(int explosion_def_id, const Vector3& pos,
                                           ArmedGameObj* damager, const Vector3& blast_direction,
...
void ExplosionManager::Create_Explosion_At(int explosion_def_id, const Matrix3D& up_tm,
                                           ArmedGameObj* damager, const Vector3& blast_direction,
                                           DamageableGameObj* force_victim)
{
    ...
    if (blast_direction.Length2() < WWMATH_EPSILON) {
        WWDEBUG_SAY(
            ("ExplosionManager::Create_Explosion_At - Invalid Blast Direction! %f %f %f\r\n",
    ...
    if (explosion_def->PhysDefID != 0) {
        ...
        PhysDefClass* phys_def
            = (PhysDefClass*)DefinitionMgrClass::Find_Definition(explosion_def->PhysDefID);
        if (phys_def != nullptr) {
            WWASSERT(phys_def);
            ...
            if (explosion) {
                ...
                if (model != nullptr) {
                    ...
                    if (model->Get_HTree() != nullptr && explosion_def->AnimatedExplosion) {
                        ...
                        exp_anim_name.Format("%s.%s", model->Get_HTree()->Get_Name(),
                                             model->Get_HTree()->Get_Name());
                        WWASSERT(WW3DAssetManager::Get_Instance() != nullptr);
                        HAnimClass* anim
                            = WW3DAssetManager::Get_Instance()->Get_HAnim(exp_anim_name);
                        if (anim != nullptr) {
    ...
    if (!explosion_def->DecalFilename.Is_Empty()) {
        ...
        PhysicsSceneClass::Get_Instance()->Create_Decal(blast_tm, new_name,
                                                        explosion_def->DecalSize, false, nullptr);
    }
    ...
    //	if ( CombatManager::I_Am_Server() )		 Clients can create damage now...
    {
        ...
        if (radius > 0.0f) {
            ...
            OffenseObjectClass offense(explosion_def->DamageStrength, explosion_def->DamageWarhead,
                                       damager);
            ...
            for (objnode = GameObjManager::Get_Game_Obj_List()->Head(); objnode;
                ...
                if (objnode->Data() == force_victim) {
                    ...
                    if (obj && obj->Peek_Physical_Object()) { // zones have no phy obj  CHANGE THIS
                        ...
                        if (!obj->Takes_Explosion_Damage()) { // Cinematics shouldn't take explosion
                                                              // damage
                        ...
                        if (dist <= radius) {
                            ...
                            PhysRayCollisionTestClass raytest(
                                ray, &res, obj->Peek_Physical_Object()->Get_Collision_Group(),
    ...
    if (explosion_def->SoundDefID != 0) {
        ...
        WWAudioClass::Get_Instance()->Create_Instant_Sound(explosion_def->SoundDefID, up_tm,
                                                           owner_ref);
    ...
    if (explosion_def->CameraShakeIntensity > 0.0f) {
        COMBAT_SCENE->Add_Camera_Shake(pos, explosion_def->CameraShakeRadius,
                                       explosion_def->CameraShakeDuration,
                                       explosion_def->CameraShakeIntensity);
    }
}

void ExplosionManager::Explosion_Damage_Building(int explosion_def_id, BuildingGameObj* building,
                                                 bool mct_damage, ArmedGameObj* damager)
{
...
void ExplosionManager::Explode(int explosion_def_id, const Vector3& pos, int owner_id,
                               int victim_id)
{
