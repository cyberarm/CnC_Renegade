...
SimplePersistFactoryClass<SakuraBossGameObj, CHUNKID_GAME_OBJECT_SAKURA_BOSS>
    _SakuraBossGameObjPersistFactory;
      ...
      RocketDoorOpenSoundID(0)
{
    ...
    EDITABLE_PARAM(SakuraBossGameObjDef, ParameterClass::TYPE_WEAPONOBJDEFINITIONID,
                   RocketLauncherDefID);
    EDITABLE_PARAM(SakuraBossGameObjDef, ParameterClass::TYPE_WEAPONOBJDEFINITIONID,
                   GattlingGunDefID);
    EDITABLE_PARAM(SakuraBossGameObjDef, ParameterClass::TYPE_SOUNDDEFINITIONID,
                   GattlingGunRevSoundDefID);
    EDITABLE_PARAM(SakuraBossGameObjDef, ParameterClass::TYPE_SOUNDDEFINITIONID,
                   RocketDoorOpenSoundID);
    EDITABLE_PARAM(SakuraBossGameObjDef, ParameterClass::TYPE_EXPLOSIONDEFINITIONID,
                   RocketDestroyedExplosionID);
    PARAM_SEPARATOR(SakuraBossGameObjDef, "");
    return;
}
...
//
//////////////////////////////////////////////////////////////////////////
uint32 SakuraBossGameObjDef::Get_Class_ID(void) const
{
...
void SakuraBossGameObjDef::Load_Variables(ChunkLoadClass& cload)
{
    while (cload.Open_Micro_Chunk()) {
        switch (cload.Cur_Micro_Chunk_ID()) {
            ...
            READ_MICRO_CHUNK(cload, VARID_DEF_ROCKET_DESTROYED_EXPLOSION_ID,
                             RocketDestroyedExplosionID);
...
//
///////////////////////////////////////////////////////////////////////////
const PersistFactoryClass& SakuraBossGameObjDef::Get_Factory(void) const
{
...
//
///////////////////////////////////////////////////////////////////////////
const PersistFactoryClass& SakuraBossGameObj::Get_Factory(void) const
{
...
void SakuraBossGameObj::Init(const SakuraBossGameObjDef& definition)
{
    ...
    const WeaponDefinitionClass* gatling_gun_def
        = WeaponManager::Find_Weapon_Definition(definition.GattlingGunDefID);
...
void SakuraBossGameObj::Load_Variables(ChunkLoadClass& cload)
{
    while (cload.Open_Micro_Chunk()) {
        switch (cload.Cur_Micro_Chunk_ID()) {
            ...
            READ_MICRO_CHUNK(cload, VARID_ROCKET_LAUNCHER_STATE_TIME_LEFT,
                             RocketLauncherStateTimeLeft);
...
void SakuraBossGameObj::Decide_New_Overall_State(void)
{
    ...
    if (Should_Change_Overall_State()) {
        ...
        if (CurrentHealth < 0.45F && FreeRandom.Get_Int(4) == 1) {
            ...
            else if (OverallState != STATE_CIRCLE_REFINERY && TargetPos.X < -125.0F
                     && TargetPos.Y > -19.0F) {

                Request_Overall_State(STATE_CIRCLE_REFINERY);
            }
            else if (OverallState != STATE_CIRCLE_POWER_PLANT && TargetPos.X > -84.0F
                     && TargetPos.Z > 11.0F) {
...
void SakuraBossGameObj::Update_Vehicle_State(void)
{
    ...
    case VEHICLE_STATE_MOVING: {
        ...
        Matrix3D::Rotate_Vector(Get_Transform(), Vector3(bounding_box.Extent.X / 2.0F, 0, 0),
                                &center_to_tip_vector);
...
void SakuraBossGameObj::Set_Sakura_State(int new_state)
{
    if (SakuraState != new_state) {
        ...
        case SAKURA_STATE_TAUNTING: {
            ...
            if (conv != nullptr) {
                ActiveConversationClass* taunt_conversation
                    = ConversationMgrClass::Create_New_Conversation(conv);
    ...
    if (GattlingGunState != new_state) {
        ...
        case GATLING_STATE_REVVING_UP: {
            AudibleSoundClass* sound = WWAudioClass::Get_Instance()->Create_Sound(
                Get_Definition().GattlingGunRevSoundDefID);
            if (sound != nullptr) {
    ...
    if (RocketLauncherState != new_state) {
        ...
        case ROCKET_STATE_OPENING: {
            ...
            if (AvailableWeapons & WEAPON_ROCKETS_LEFT) {
                ...
                WWAudioClass::Get_Instance()->Create_Instant_Sound(
                    Get_Definition().RocketDoorOpenSoundID, left_tm);
            }
            ...
            if (AvailableWeapons & WEAPON_ROCKETS_RIGHT) {
                ...
                WWAudioClass::Get_Instance()->Create_Instant_Sound(
                    Get_Definition().RocketDoorOpenSoundID, right_tm);
            }
...
//
///////////////////////////////////////////////////////////////////////////
void SakuraBossGameObj::Apply_Damage_Extended(const OffenseObjectClass& damager, float scale,
...
SoldierGameObj* SakuraBossGameObj::Find_Closest_Human_Player(void)
{
    ...
    for (it.First(); !it.Is_Done(); it.Next()) {
        ...
        if (it.Peek_Obj()->Get_Observer() != nullptr) {
            gameobj
                = ((CombatPhysObserverClass*)it.Peek_Obj()->Get_Observer())->As_PhysicalGameObj();
        }
