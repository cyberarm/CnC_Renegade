...
** This is a static object that simply manages the active beam effect.  Its job is to cache un-used
*beams
** so that we aren't allocating them at run time and to add new beam effects to the scene when
*needed.
*/
...
public:
    ...
    */
    void Create_Beam_Effect(const AmmoDefinitionClass* ammo_def, const Vector3& p0,
                            const Vector3& p1);
...
private:
    void Internal_Get_New_Beam(SegmentedLineClass** beam_model,
                               TimedDecorationPhysClass** beam_wrapper);
...
void BeamEffectManagerClass::Create_Beam_Effect(const AmmoDefinitionClass* ammo_def,
                                                const Vector3& p0, const Vector3& p1)
{
    ...
    */
    ...
    if (ammo_def->BeamSubdivisionEnabled) {
        beam_model->Set_Noise_Amplitude(
            (BASE_AMPLITUDE + AMPLITUDE_PER_METER * (p1 - p0).Quick_Length())
...
CollisionReactionType BulletDataClass::Bullet_Collision_Occurred(const CollisionEventClass& event)
{
    ...
    // If the mesh is shatterable it is shattered,  Apply a surface effect depending on the surface
    // type that was collided with.  In the case that the mesh was shattered, we disable surface
    // effect decals.
    //
    bool shattered = false;
    if ((event.CollidedRenderObj != nullptr)
        && (event.CollidedRenderObj->Class_ID() == RenderObjClass::CLASSID_MESH)) {
        ...
        if ((mesh->Get_W3D_Flags() & W3D_MESH_FLAG_SHATTERABLE) && (mesh->Is_Not_Hidden_At_All())) {
            ...
            PhysicsSceneClass::Get_Instance()->Shatter_Mesh(mesh, collision_point, collision_normal,
                                                            Velocity);
            ...
            // windows that have different materials on the two sides.  We don't want bullets to
            // ever shatter only one "side" of a window!
    ...
    //
    // Apply Damage
    // There are several types of objects that can be damaged: normal dynamic game objects,
    // damageable static physics objects, and buildings.  A building is damaged whenever any of its
    // meshes or aggregates are hit by a bullet. Also, we only apply damage if the mesh wasn't
    // shattered.  Bullets pretend like nothing happened if they shatter something.
    //
    if (!shattered) {
        ...
        if (other) { // if hitting a game object,
            ...
            if (other->As_C4GameObj() != nullptr) {
                ...
                if ((host != nullptr) && (host->As_PhysicalGameObj() != nullptr)) {
                    host->As_PhysicalGameObj()->Apply_Damage_Extended(offense, 1.0f, Velocity,
                                                                      nullptr);
                }
            }
            //			}
            ...
            if ((!other->Is_Soft()) || (++SoftPierceCount >= AmmoDefinition->SoftPierceLimit)) {
                ...
                if (AmmoDefinition->ExplosionDefID) {
                    ExplosionManager::Create_Explosion_At(AmmoDefinition->ExplosionDefID,
                                                          collision_point, Get_Owner(),
            ...
            if (event.OtherObj->Get_Factory().Chunk_ID() == PHYSICS_CHUNKID_DAMAGEABLESTATICPHYS
                ...
                OffenseObjectClass offense(AmmoDefinition->Damage, (int)AmmoDefinition->Warhead,
                                           Get_Owner());
                offense.EnableClientDamage = true; // Only bullets apply to client damage;
            ...
            if (!SurfaceEffectsManager::Does_Surface_Stop_Bullets(
                    event.CollisionResult->SurfaceType)) {
                return_value = COLLISION_REACTION_NO_BOUNCE;
            }
            else {

                // If the ammo explodes when it hits terrain, then create an explosion and kill the
                // bullet
    ...
    if (!DidExplode) {
        Matrix3D surface_tm;
        surface_tm.Look_At(collision_point, collision_point - collision_normal,
                           FreeRandom.Get_Float(0, DEG_TO_RADF(360)));
        int my_type = AmmoDefinition->HitterType;
        bool allow_decals = (shattered == false);
        if ((float)AmmoDefinition->Damage < 0.0f) {
            allow_decals = false;
        }
        SurfaceEffectsManager::Apply_Effect(event.CollisionResult->SurfaceType, my_type, surface_tm,
                                            event.OtherObj, Get_Owner(), allow_decals);
    }
...
ExpirationReactionType BulletDataClass::Bullet_Expired(void)
{
    ...
    if (AmmoDefinition->TimeActivated && AmmoDefinition->ExplosionDefID && !DidExplode) {
        ExplosionManager::Create_Explosion_At(AmmoDefinition->ExplosionDefID, Position,
                                              Get_Owner());
    }
...
*/
class BulletClass : public CombatPhysObserverClass,
                       ...
                       DamageableGameObj* target_object)
{
    ...
    if (Projectile->Peek_Model() != nullptr && Projectile->Peek_Model()->Get_Name() != nullptr
        && BulletData.AmmoDefinition->ModelName.Compare_No_Case(
               Projectile->Peek_Model()->Get_Name())
            == 0) {
        // We don't need to do anything
    }
    else {
        RenderObjClass* model = nullptr;
        model = WW3DAssetManager::Get_Instance()->Create_Render_Obj(
    ...
    float duration
        = (float)BulletData.AmmoDefinition->Range / (float)BulletData.AmmoDefinition->Velocity;
    if (duration <= 0.0) {
        ...
        case CHUNKID_VARIABLES: {
            ...
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_SOFT_PIERCE_COUNT,
                                     BulletData.SoftPierceCount);
...
void BulletClass::Think(void)
{
    ...
    if (BulletData.AmmoDefinition->IsTracking) {
        ...
        if (TrackingErrorTimer < 0) {
            ...
            TrackingError = RANDOM_VECTOR(BulletData.AmmoDefinition->RandomTrackingScale
                                          * target_distance / 2);
        }
        ...
        if (cross.Length() > WWMATH_EPSILON) {
            ...
            if (WWMath::Fabs(dot) >= 1.0f) {
                ...
                float allowed_turn
                    = BulletData.AmmoDefinition->TurnRate * TimeManager::Get_Frame_Seconds();
