        ...
        case CHUNKID_VARIABLES:
            ...
            while (cload.Open_Micro_Chunk()) {
                switch (cload.Cur_Micro_Chunk_ID()) {
                    ...
                    READ_MICRO_CHUNK(cload, MICROCHUNKID_STEALTH_POWERUP_TIMER,
                                     StealthPowerupTimer);
...
bool SmartGameObj::Is_Controlled_By_Me(void)
{
    ...
    return game_obj->Is_Human_Controlled()
        && (game_obj->ControlOwner == CombatManager::Get_My_Id());
}
...
void SmartGameObj::Apply_Control(void)
{
    ...
    for (int weapon_key_num = -1; weapon_key_num <= 9; weapon_key_num++) {
        if (Control.Get_Boolean((ControlClass::BooleanControl)(
                (int)ControlClass::BOOLEAN_SELECT_WEAPON_0 + weapon_key_num))) {
    ...
    if (this == COMBAT_STAR && change_type != nullptr) {
        ...
        DIAG_LOG(("WCHN", "%1.2f; %1.2f; %1.2f; %s; %d; %s", pos.X, pos.Y, pos.Z, weapon_name, ammo,
                  change_type));
    }
    ...
    if (ControlEnabled) {
        if ((Get_Weapon() != nullptr) && (!Is_Delete_Pending())) {
            ...
            Get_Weapon()->Set_Secondary_Triggered(
...
{
    {
        ...
        if (MovingSoundTimer < 0) {
            WWPROFILE("See");
            //			MovingSoundTimer += FreeRandom.Get_Float( 1, 2 );	// sound
            //every 1-2 seconds
            ...
            if (Is_Enemy_Seen_Enabled()) {
                ...
                for (objnode = GameObjManager::Get_Game_Obj_List()->Head(); objnode;
                    ...
                    if (obj) {
                        ...
                        if (obj != COMBAT_STAR && obj->Peek_Model()
                            && obj->Peek_Model()->Is_Hidden()) {
                            continue;
                        }
