...
void SoundSceneClass::Collect_Logical_Sounds(int listener_count)
{
    ...
    for (int total = 0; total < count && priority_queue.Process_Head(&listener); total++) {
        ...
        for (cull_obj = m_LogicalCullingSystem.Get_First_Collected_Object(); cull_obj != nullptr;
            ...
            if ((listener_pos - sound_pos).Length2() <= test_radius2) {
                ...
                if (sound_obj->Allow_Notify(timestamp)) {
                    listener->On_Event(AudioCallbackClass::EVENT_LOGICAL_HEARD, (uint32)listener,
                                       (uint32)sound_obj);
                }
            }
        }
...
//
////////////////////////////////////////////////////////////////////////////////////////////////
void SoundSceneClass::Remove_Logical_Sound(LogicalSoundClass* sound_obj, bool single_shot,
...
void SoundSceneClass::Flush_Scene(void)
{
    ...
    for (temp_single_iterator.First(); !temp_single_iterator.Is_Done();
         temp_single_iterator.Next()) {
    ...
    for (temp_single_logical_it.First(); !temp_single_logical_it.Is_Done();
         temp_single_logical_it.Next()) {
