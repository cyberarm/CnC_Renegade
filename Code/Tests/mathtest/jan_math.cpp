    ...
    for (int i = 0; i < SINTABLESIZE + 1; i++) {
        ...
        for (double dy = 1.000178; dy < 1.000537; dy += 0.0000001) {
            ...
            for (double dz = 1.000534; dz < 1.000715; dz += 0.0000001) {
                ...
                double by = x * 8.0 / 27.0 + y * dy * 12.0 / 27.0 + z * dz * 6.0 / 27.0 + w / 27.0;

                double bz = x / 27.0 + y * dy * 6.0 / 27.0 + z * dz * 12.0 / 27.0 + w * 8.0 / 27.0;
        ...
        cout << setw(16) << x << "f, " << setw(16) << y * my << "f, " << setw(16) << z * mz << "f,"
             << endl;
    }
}
...
float acosTable[ACOSTABLESIZE + 1][3] = { 0,
                                          0,
                                          0,
                                          0,
                                          0,
...
{
    float table[] = { 1.101460576f,    +.5764093225f,   -.09735417608f, +.003083774398f,
                      -.000844493744f, +.005722363752f, -.005962135984f };
...
int intFloor(const float& f)
{
    ...
    int r = (unsigned int(mantissa | (1 << 23)) << 8)
        >> (31 - exponent); // ((1<<exponent)*(mantissa|hidden bit))>>24 -- (we know that mantissa >
                            // (1<<24))
...
int jan_main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        //		cout << "x: " << setw(8) <<  foo << " sin(x): " << setw(8) << r0 << "
        //bez_sin(x): " << setw(8) << r1 << " ratio: " << setw(8) << r0-r1 << endl; 		cout << "x: " <<
        //setw(8) <<  foo << " acos(x): " << setw(8) << r0 << " bez_acos(x): " << setw(8) << r1 << "
        //ratio: " << setw(8) << r0/r1 << endl;
        cout << "acos clocks: " << acos_sum << " bez clocks: " << bez_sum
             << " table clocks: " << table_sum << endl;
    }
