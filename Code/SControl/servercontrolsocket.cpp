...
bool ServerControlSocketClass::Open(int port, bool loopback, unsigned long ip)
{
    ...
    */
    ...
    if (bind(Socket, (LPSOCKADDR)&addr, sizeof(addr)) == SOCKET_ERROR) {
        DebugString(
            ("ServerControlSocketClass - bind failed with error code %d\n", WSAGetLastError()));
        Close();
        return (false);
    }
    ...
    */
    ...
    if (err == INVALID_SOCKET) {
        DebugString(
            ("ServerControlSocketClass - Failed to set socket option SO_RCVBUF - error code %d.\n",
             LAST_ERROR));
    ...
    */
    ...
    if (err == INVALID_SOCKET) {
        DebugString(
            ("ServerControlSocketClass - Failed to set socket option SO_SNDBUF - error code %d.\n",
             LAST_ERROR));
        fw_assert(err != INVALID_SOCKET);
    }
    else {
    ...
    */
    ...
    if (err) {
        DebugString(
            ("ServerControlSocketClass - Failed to set socket to non-blocking - error code %d.\n",
             LAST_ERROR));
    }
...
void ServerControlSocketClass::Close(void)
{
    if (Socket != INVALID_SOCKET) {
        DebugString(
            ("ServerControlSocketClass - Closing socket %d bound to port %d\n", Socket, Port));
        if (closesocket(Socket) != 0) {
            DebugString(
                ("ServerControlSocketClass - closesocket failed with error code %d\n", LAST_ERROR));
        }
        Socket = INVALID_SOCKET;
    }
}
 ...
 *    3/1/00 11:51AM ST : Created                                                              *
 *=============================================================================================*/
void ServerControlSocketClass::Write(void* buffer, int buffer_len, void* address,
                                     unsigned short port)
{
    /*
 ...
 *=============================================================================================*/
int ServerControlSocketClass::Read(void* buffer, int buffer_len, void* address,
                                   unsigned short* port, int packetnum)
{
    /*
    ...
    */
    ...
    DebugString(("ServerControlSocketClass - ServerControlSocketClass::Read - Returning %d bytes. "
                 "%d InBuffers left\n",
 ...
 *    3/8/00 1:12PM ST : Created                                                               *
 *=============================================================================================*/
int ServerControlSocketClass::Peek(void* buffer, int buffer_len, void* address,
                                   unsigned short* port, int packetnum)
{
    /*
    ** Call the Service function in case there are any outstanding unqueued packets.
    ...
    */
    ...
    DebugString(("ServerControlSocketClass - ServerControlSocketClass::Peek - Returning packet %d "
                 "of %d bytes. %d InBuffers left\n",
...
void ServerControlSocketClass::Service(void)
{
    ...
    for (;;) {
        ...
        */
        if (result != 0) {
            ...
            /*
            ** If there is outstanding data, 'bytes' will contain the size of the next queued
            *datagram.
            */
            if (bytes == 0) {
                break;
                ...
                */
                ...
                result = recvfrom(Socket, (char*)ReceiveBuffer, sizeof(ReceiveBuffer), 0,
                ...
                if (result == SOCKET_ERROR) {
                    ...
                    */
                    if (!Passes_CRC_Check(packet)) {
                        ...
                        */
                        DebugString(
                            ("ServerControlSocketClass - Throwing away malformed packet\n"));
                        if (packet->IsAllocated) {
                            delete packet;
                        }
                        else {
                        ...
                        */
                        ...
                        // IPAddressClass(packet->Address).As_String(), (unsigned int)((unsigned
                        // short)ntohs(addr.sin_port))));
    ...
    */
    while (OutBuffers.Count()) {
        ...
        */
        result
            = sendto(Socket, ((char const*)packet->Buffer) - sizeof(packet->CRC),
                     packet->BufferLen + sizeof(packet->CRC), 0, (LPSOCKADDR)&addr, sizeof(addr));

        if (result == SOCKET_ERROR) {
            if (LAST_ERROR != WSAEWOULDBLOCK) {
                DebugString(
                    ("ServerControlSocketClass - sendto returned error code %d\n", LAST_ERROR));
                Clear_Socket_Error();
            }
            else {
