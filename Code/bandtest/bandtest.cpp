...
static bool Send_Ping(char* payload, int payload_size, SOCKET socket, struct sockaddr* address,
                      int sequence_id);
static bool Get_Ping_Response(SOCKET socket, int& seq_id, struct sockaddr* address,
                              unsigned long validate_addr, unsigned long& my_address);
...
static unsigned long Upstream_Detect(unsigned long server_ip, unsigned long my_ip,
                                     int& failure_code, unsigned long& downstream,
                                     BandtestSettingsStruct* settings);
static int Ping_Host(unsigned long host_ip, unsigned long my_ip, int times, int payload_size,
                     unsigned long* ping_times, unsigned long timeout);
                              ...
                              unsigned long& downstream, BandtestSettingsStruct* settings)
{
    ...
    */
    if (my_ip == 0) {
        ...
        */
        ...
        seq_id = -1;
        while (seq_id == -1) {
            Get_Ping_Response(ICMPRawSocket, seq_id, (struct sockaddr*)&address,
                              ntohl(host_address.sin_addr.s_addr), ping_dest_address);
    ...
    */
    ...
    for (i = first_router; i < hops_to_server; i++) {
        if ((path_to_server[i] & 0xffff0000) != (my_ip & 0xffff0000)) {
            ...
            */
            ...
            /*
            ** If it responded, this is our man, ping it some more to establish a reference ping
            *average.
            */
            if (pings) {
                performance_timer = timeGetTime();
    ...
    /*
    ** If the ping time is low, it's *probably* a high bandwidth connection so we can get a more
    *accurate test by sending more
    ...
    */
    ...
    for (i = 0; i < num_udp_packets; i++) {
        ...
        */
        ...
        /*
        ** We have to fill the packet with different garbage every time we send it otherwise a modem
        *will be able to
    ...
    /*
    ** Set the TTL to max on the ICMP socket. This shouldn't be needed but I'm doing it just in case
    *there are any bugs in
    ...
    */
    ...
    /*
    ** Now see what the ping time to the router is. Since there are n UDP packets ahead of this ping
    *before it goes out, the
    ...
    */
    Send_Ping((char*)temp_buffer, 0, ICMPRawSocket, (struct sockaddr*)&router_addr,
              packet_sequencer);
    Send_Ping((char*)temp_buffer, 0, ICMPRawSocket, (struct sockaddr*)&router_addr,
              packet_sequencer + 1);
    ...
    /*
    ** If the bandwidth in the registry is close to what we just calculated then use the old
    *downstream calculation from the
    ** registry.
    */
    ...
    */
    ...
    /*
    **
    **  Well, I suppose, since we are here, we might as well have a stab at downstream bandwidth
    *too.
    **
    **
    **  Try sending max size pings to our friendly router and subtracting how long we think the
    *upstream should have taken
    ...
    if (calc_down && upstream_bandwidth < 576 * 1000 && upstream_bandwidth > 8) {
        ...
        if (method_one) {
            ...
            */
            ...
            /*
            ** Send a shitload of UDP packets to the next router down the list after the one we just
            *pinged.
            ...
            */
            ...
            for (i = 0; i < num_udp_packets; i++) {
                ...
                /*
                ** We have to fill the packet with different garbage every time we send it otherwise
                *a modem will be able to
                ** do delta compression on the packets.
                */
                for (int b = 0; b < sizeof(temp_buffer); b++) {
                    temp_buffer[b] = (unsigned char)(rand() & 0xff);
                ...
                start_time = timeGetTime();
                Send_Raw_UDP(temp_buffer, 540, RawSocket, (struct sockaddr*)&host_address, 1234,
                             4321);
        ...
        */
        int new_ttl = 255;
        int result
            = setsockopt(ICMPRawSocket, IPPROTO_IP, IP_TTL, (char*)&new_ttl, sizeof(new_ttl));
        if (result == SOCKET_ERROR) {
            DebugString("setsockopt failed to set IP_TTL = %d - error code %d\n", new_ttl,
                        WSAGetLastError());
            failure_code = BANDTEST_NO_TTL_SET;
        ...
        */
        int pings = Ping_Host(ntohl(router_addr.sin_addr.s_addr), my_ip, 1, 540, ping_times,
                              new_ping_timeout);
        if (pings == 0) {
        ...
        if (pings) {
            ...
            */
            ...
            pings = Ping_Host(ntohl(router_addr.sin_addr.s_addr), my_ip, num_pings, 540, ping_times,
                              timeout);
    ...
    if (method_one) {
        ...
        if (time_down > 0) {
            ...
            if (bps_down > 100000) {
                ...
                DebugString("Downstream bandwidth from external router by alt method is %.1f "
                            "kilobits per second\n",
                            floater);
            }
            else {
                DebugString("Downstream bandwidth from external router by alt method is %f bits "
                            "per second\n",
                            bps_down);
            }
        }
...
void Ping_Profile(SOCKADDR_IN* router_addr, unsigned long my_ip)
{
    ...
    */
    ...
    for (int packet_size = 0; packet_size < 541; packet_size += 10) {
        ...
        ** Ping the router once to get a ballpark trip time.
        */
        int pings = Ping_Host(ntohl(router_addr->sin_addr.s_addr), my_ip, 1, packet_size,
                              ping_times, 1000);
        if (pings == 0) {
            pings = Ping_Host(ntohl(router_addr->sin_addr.s_addr), my_ip, 1, packet_size,
                              ping_times, 2 * TIMER_SECOND);
            if (pings == 0) {
                pings = Ping_Host(ntohl(router_addr->sin_addr.s_addr), my_ip, 1, packet_size,
                                  ping_times, 2 * TIMER_SECOND);
            }
        ...
        if (pings) {
            ...
            */
            ...
            pings = Ping_Host(ntohl(router_addr->sin_addr.s_addr), my_ip, num_pings, packet_size,
                              ping_times, timeout);
...
int Get_Path_To_Server(unsigned long* path, unsigned long my_ip, unsigned long server_ip)
{
    ...
    */
    ...
    /*
    ** Try to find a router with a different address class than ours. Start at TTL = 1 and increase
    *until we find what we
    ** are looking for.
    */
    int ttl = 1;
    int hops_to_server = 0;
 ...
 *=============================================================================================*/
int Ping_Host(unsigned long host_ip, unsigned long my_ip, int times, int payload_size,
              unsigned long* ping_times, unsigned long timeout)
{
    ...
    */
    for (int i = 0; i < times; i++) {
        ...
        ** Send the ping.
        */
        Send_Ping((char*)temp_buffer, payload_size, ICMPRawSocket, (struct sockaddr*)&host_addr,
                  _packet_sequencer);
        PingsSent++;
        ...
        */
        while (seq_id != _packet_sequencer) {
            Get_Ping_Response(ICMPRawSocket, seq_id, (struct sockaddr*)&address,
                              ntohl(host_addr.sin_addr.s_addr), ping_dest_address);
        ...
        */
        if (seq_id == _packet_sequencer) {
            ...
            DebugString("Ping time %d to external router %s is %d ms\n", num_pings,
                        Addr_As_String2(&host_addr), router_ping_time);
...
float Average_Ping(int num_pings, unsigned long* ping_times, bool ignore_low_high)
{
    ...
    /*
    ** If we have lots of pings, see if we can improve the accurracy by filtering out very small or
    *very big pings.
    */
    if (num_pings > 5) {
 ...
 *=============================================================================================*/
bool Send_Raw_UDP(char* payload, int payload_size, SOCKET socket, struct sockaddr* address,
                  unsigned short source_port, unsigned short dest_port)
{
    /*
    ** Asserts.
 ...
 *=============================================================================================*/
bool Send_Ping(char* payload, int payload_size, SOCKET socket, struct sockaddr* address,
               int sequence_id)
{
    // DebugString("Send ping. payload=%08X, size=%d, socket=%d, address=%s, seq_id=%d\n", payload,
    // payload_size, socket, Addr_As_String2((struct sockaddr_in *)address), sequence_id);
    /*
    ** Asserts.
    */
    assert(payload_size < 550);
 ...
 *=============================================================================================*/
bool Get_Ping_Response(SOCKET socket, int& seq_id, struct sockaddr* address,
                       unsigned long validate_address, unsigned long& my_address)
{
        ...
        */
        if (bytes == 0) {
            ...
            */
            ...
            result = recvfrom(socket, recv_buffer, sizeof(recv_buffer), 0, (LPSOCKADDR)&addr,
                              &addr_len);
            ...
            */
            ...
            /*
            ** Get the header length. Length specified in the header is in longs so multiply it by 4
            *to get bytes.
            */
            ...
            if (ip_header->Protocol == PROTOCOL_ICMP) {
                ...
                */
                case ICMP_TIME_EXCEEDED:
                    ...
                    {
                        /*
                        ** Find the packets original IP header. ICMP_TIME_EXCEEDED is 8 bytes
                        *followed by a copy of the original IP
                        ** header followed by the original ICMP header followed by 64 bits of the
                        *original payload. Phew. (the original
                        ** payload seems to be frequently lost).
                        */
                        IPHeaderType* original_ip_header
                            = (IPHeaderType*)(((char*)icmp_header) + 8);
...
bool Open_Raw_Sockets(int& failure_code)
{
    ...
    */
    ...
    if (use_group) {
        if (getsockopt(RawSocket, SOL_SOCKET, SO_GROUP_ID, (char*)&group, &length)
            == SOCKET_ERROR) {
    ...
    */
    ...
    int result = setsockopt(RawSocket, SOL_SOCKET, SO_GROUP_PRIORITY, (char*)&new_priority,
                            sizeof(new_priority));
    if (result != 0) {
        DebugString("Unable to set priority on UDP socket - error code %d\n", WSAGetLastError());
    }
    new_priority = 1;
    result = setsockopt(ICMPRawSocket, SOL_SOCKET, SO_GROUP_PRIORITY, (char*)&new_priority,
                        sizeof(new_priority));
    if (result != 0) {
...
{
    int result
        = RegSetValueEx(RegistryKey, name, 0, REG_DWORD, (unsigned char*)&value, sizeof(value));
...
char* Addr_As_String2(struct sockaddr_in* addr)
{
    ...
    sprintf(_string, "%d.%d.%d.%d ; %d", (int)(addr->sin_addr.S_un.S_un_b.s_b1),
            ...
            (int)(addr->sin_addr.S_un.S_un_b.s_b4), (int)(addr->sin_port));
    return (_string);
}
    ...
    */
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (seq_id == packet_sequencer) {
            unsigned long router_ping_time = timeGetTime() - start_time;
            DebugString("Ping time %d to external router %s is %d ms\n", num_pings,
                        Addr_As_String2(&router_addr), router_ping_time);
            ping_times[num_pings++] = router_ping_time;
        }
        packet_sequencer++;
    }
