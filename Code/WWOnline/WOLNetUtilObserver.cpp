...
namespace WWOnline
{
    ...
    STDMETHODIMP NetUtilObserver::OnLadderList(HRESULT result, WOL::Ladder* list, int rungCount,
    ...
    void NetUtilObserver::ProcessLadderListResults(WOL::Ladder* list, long timeStamp)
    {
        if (mOuter->mLadderPending) {
            ...
            while (wolLadder) {
                ...
                mbstowcs(ladderName, (const char*)wolLadder->login_name,
                         sizeof(wolLadder->login_name));

                WWDEBUG_SAY(("WOL: LadderInfo [%08lX] Requested '%S', Received '%S'\n", type,
                             requestName, ladderName));

                // If the ladder name matches the requested name then there is ladder info
                // available.
            ...
            // If there are no pending ladders then remove the satisfied requests
            if ((mOuter->mLadderPending & LADDERTYPE_MASK) == 0) {
                WWASSERT(ladderCount == mOuter->mLadderPending
                         && "LadderInfo received doesn't match number requested");
    ...
    void NetUtilObserver::NotifyClanLadderUpdate(const UserList& users,
                                                 const RefPtr<SquadData>& squad)
    {
    ...
    STDMETHODIMP NetUtilObserver::OnNewNick(HRESULT result, LPCSTR message, LPCSTR nickname,
                                            LPCSTR password)
    {
        if (mOuter == NULL) {
            WWDEBUG_SAY(("WOLERROR: Session not initialized\n"));
    ...
    STDMETHODIMP NetUtilObserver::OnHighscore(HRESULT result, WOL::Highscore* list, int count,
                                              long time, int keyRung)
    {
