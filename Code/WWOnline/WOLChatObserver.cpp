...
namespace WWOnline
{
        ...
        if (result == CHAT_S_PINGLIST) {
            ...
            while (wolChannel) {
                const RefPtr<ChannelData>& channel
                    = mOuter->FindChannel((const char*)wolChannel->name);
                                             ...
                                             WOL::User* inUser)
    {
        ...
        WWDEBUG_SAY(("WOL: User '%s' @ %s joining channel '%s'\n", (char*)inUser->name,
                     inet_ntoa(addr), (char*)inChannel->name));
        ...
        // Get the time they joined the channel. They have 2 mins to get into the game or they are
        // kicked.
                                          ...
                                          WOL::User* inUsers)
    {
        ...
        while (curUser) {
            ...
            if (user.IsValid()) {
                ...
                if (mOuter->mCurrentChannel.IsValid()
                    && (mOuter->mCurrentChannel->GetType() != 0)) {
                    mOuter->AutoRequestUserDetails(user);
                }
            }
    ...
    */
    ...
    bool
    PrivateGameOptionsTrackingClass::operator==(PrivateGameOptionsTrackingClass const& whatever)
    {
                                           ...
                                           WOL::User* inUsers, int gameID)
    {
        ...
        WWDEBUG_SAY(
            ("WOL: OnGameStart GameID %ld, Channel '%s'\n", gameID, (char*)inChannel->name));
                                          ...
                                          WOL::User* inUser, WOL::User* kicker)
    {
        ...
        WWDEBUG_SAY(("WOL: '%s' has been kicked from channel '%s' by '%s'\n", (char*)inUser->name,
                     (char*)inChannel->name, (char*)kicker->name));
                                           ...
                                           WOL::Squad* inSquad)
    {
        ...
        while (wolSquad) {
            ...
            //inSquad->name, inSquad->abbreviation));
    ...
    void ChatObserver::ProcessSquadRequest(const RefPtr<SquadData>& squad)
    {
        ...
        if (!mOuter->mSquadPending.empty()) {
            // Get the first pending squad request from the queue and see if it's a request by ID or
            // Name.
            const WideStringClass& pending = mOuter->mSquadPending[0];

            if (squad.IsValid()) {
                WWDEBUG_SAY(
                    ("WOL: Squad %s found for %S\n", squad->GetAbbr(), (const WCHAR*)pending));
    ...
    STDMETHODIMP ChatObserver::OnUserLocale(HRESULT result, WOL::User* inUsers)
    {
        ...
        while (wolUser) {
            ...
            WWASSERT(locale >= WOL::LOC_UNKNOWN && locale <= WOL::LOC_TURKEY
                     && "OnUserLocale Locale out of range!");
