...
namespace WWOnline
{
    ...
    bool Download::Start(void)
    {
        ...
        if (!dirCreated && (ERROR_ALREADY_EXISTS != GetLastError())) {
            ...
            SetError(DOWNLOADEVENT_LOCALFILEOPENFAILED,
                     GetOnErrorText(DOWNLOADEVENT_LOCALFILEOPENFAILED));
            return false;
        }
        ...
        WWDEBUG_SAY(
            ("WOL: Downloading '%s' to '%s'\n", (const char*)downloadFile, (const char*)localFile));
        HRESULT hr = mDownloadObject->DownloadFile(server, login, password, downloadFile, localFile,
                                                   regPath);
    ...
    void Download::GetProgress(int& bytesRead, int& totalSize, int& timeElapsed,
    ...
    STDMETHODIMP Download::OnError(int error)
    {
        ...
        static const char* _errors[]
            = { "DOWNLOADEVENT_NOSUCHSERVER",        "DOWNLOADEVENT_COULDNOTCONNECT",
    ...
    STDMETHODIMP Download::OnProgressUpdate(int bytesRead, int totalSize, int timeElapsed,
                                            int timeRemaining)
    {
