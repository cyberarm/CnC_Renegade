...
namespace WWOnline
{
    ...
    {
        if (mEndResult == Waiting) {
            if ((event.GetEvent() == ChannelListEvent::NewList)
                && (event.GetChannelType() == mType)) {
    ...
    JoinChannelWait::JoinChannelWait(const RefPtr<Session>& session, const wchar_t* channelName,
                                     const wchar_t* password, int type)
        : SingleWait(WOLSTRING("WOL_CHANNELJOIN")),
          mSession(session),
    ...
    void CreateChannelWait::WaitBeginning(void)
    {
        ...
        if (FAILED(hr)) {
            WWDEBUG_SAY(
                ("WOLERROR: CreateChanneWait request failed '%s'\n", GetChatErrorString(hr)));
