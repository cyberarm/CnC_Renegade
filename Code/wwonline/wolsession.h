...
#define REQUEST_NONE 0
    ...
    public:
        ...
        RefPtr<WaitCondition> CreateChannel(const wchar_t* channelName, const wchar_t* password,
                                            int type);
        RefPtr<WaitCondition> CreateChannel(const RefPtr<ChannelData>&, const wchar_t* password);

        // Join a channel
        RefPtr<WaitCondition> JoinChannel(const RefPtr<ChannelData>& channel,
    ...
    {
    public:
        static RefPtr<ChatAdvisement> Create(const CComPtr<WOL::IChat>&,
                                             const CComPtr<WOL::IChatEvent>&);
