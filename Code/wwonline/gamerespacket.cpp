...
namespace WWOnline
{
     ...
     *   04/22/1996 PWG : Created.                                            *
     *========================================================================*/
    unsigned char* GameResPacket::Create_Comms_Packet(unsigned long& size, char* sig_name,
                                                      unsigned long& sig_offset)
    {
        GameResField* current;
        sig_offset = 0;

        // Size starts at 8 because that is the size of the packet header. (size[4] + id[2] +
        // reserved[2])
        ...
        *(unsigned short*)curbuf = htons(mReserved);
        ...
        for (current = mHead; current; current = current->mNext) {
            ...
            if (sig_name && strncmp(sig_name, current->mID, 4) == 0) {
                // If sig_offset isn't zero then that would indicate that there were multiple sig
