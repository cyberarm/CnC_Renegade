...
namespace WWOnline
{
    ...
    bool Session::FinalizeCreate(void)
    {
        ...
        if (FAILED(hr)) {
            WWDEBUG_SAY(
                ("WOLERROR: SetAttributeValue(RegPath) HRESULT = %s\n", GetChatErrorString(hr)));
            return false;
        }
        ...
        if (FAILED(hr)) {
            WWDEBUG_SAY(
                ("WOLERROR: SetAttributeValue(AutoTopic) HRESULT = %s\n", GetChatErrorString(hr)));
            return false;
        }
                                               ...
                                               const RefPtr<LoginInfo>& login)
    {
        ...
        if (mCurrentServer == server) {
            if (server.IsValid()) {
                ...
                if (ConnectionConnected == mCurrentConnectionStatus) {
                    ...
                    return EventValueWait<ConnectionStatus>::CreateAndObserve(
                        *this, ConnectionConnected, WOLSTRING("WOL_CONNECTING"));
                }
            }
            else {
        ...
        if (ConnectionConnected == mCurrentConnectionStatus) {
            ...
            mGameChannels.clear();
            ChannelListEvent gameListEvent(ChannelListEvent::NewList, mGameChannels,
                                           product->GetGameCode());
        ...
        if (ConnectionDisconnecting == mCurrentConnectionStatus) {
            ...
            RefPtr<EventValueWait<ConnectionStatus>> finish
                = EventValueWait<ConnectionStatus>::CreateAndObserve(
                    *this, ConnectionDisconnected, WOLSTRING("WOL_DISCONNECTING"));
            ...
            RefPtr<EventValueWait<ConnectionStatus>> finish
                = EventValueWait<ConnectionStatus>::CreateAndObserve(
                    *this, ConnectionConnected, WOLSTRING("WOL_DISCONNECTING"));
            serverWait->Add(finish);
        }
        ...
        if (mCurrentServer.IsValid() || (mPendingServer.IsValid() && (mPendingServer != server))) {
            ...
            if ((ConnectionConnected == mCurrentConnectionStatus)
                || (ConnectionConnecting == mCurrentConnectionStatus)) {
    ...
    RefPtr<WaitCondition> Session::JoinChannel(const wchar_t* chanName, const wchar_t* password,
                                               int type)
    {
        // If we are not connected then we cannot join a channel.
        if (mCurrentConnectionStatus != ConnectionConnected) {
        ...
        if (wait.IsValid()) {
            ...
            if (chanName) {
                // Join new channel
                RefPtr<WaitCondition> joinWait
                    = JoinChannelWait::Create(this, chanName, password, type);
                wait->Add(joinWait);
            }
        }
    ...
    bool Session::RequestChannelJoin(const RefPtr<ChannelData>& channel, const wchar_t* password)
    {
        if (channel.IsValid() && password) {
            ...
            if (FAILED(hr)) {
                WWDEBUG_SAY(
                    ("WOLERROR: RequestChannelJoin() HRESULT = %s\n", GetChatErrorString(hr)));
            }
    ...
    RefPtr<WaitCondition> Session::CreateChannel(const wchar_t* name, const wchar_t* password,
                                                 int type)
    {
    ...
    bool Session::SendChannelTopic(void)
    {
        ...
        if (channel.IsValid()) {
            ...
            if (topic) {
                ...
                WWDEBUG_SAY(
                    ("WOLERROR: SendChannelTopic() HRESULT = %s\n", GetChatErrorString(hr)));
            }
        }
    ...
    bool Session::SendChannelExtraInfo(void)
    {
        ...
        if (channel.IsValid()) {
            ...
            if (exInfo) {
                ...
                WWDEBUG_SAY(
                    ("WOLERROR: ChangeChannelExtraInfo() HRESULT = %s\n", GetChatErrorString(hr)));
            }
        }
    ...
    bool Session::PageUser(const wchar_t* username, const wchar_t* message)
    {
        if ((username && wcslen(username)) && (message && wcslen(message))) {
            ...
            if (IsAnsiText(message)) {
                ...
                if (FAILED(hr)) {
                    WWDEBUG_SAY(
                        ("WOLERROR: RequestUnicodePage() HRESULT = %s\n", GetChatErrorString(hr)));
                }
        ...
        if (!mLocaleRequests.empty()) {
            ...
            if (users) {
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    WWDEBUG_SAY(
                        ("WOLERROR: RequestUserLocale() HRESULT = %s\n", GetChatErrorString(hr)));
                }
    ...
    void Session::MakeSquadRequests(void)
    {
        if (!mSquadRequests.empty() && mSquadPending.empty()) {
            ...
            for (unsigned int index = 0; index < count; ++index) {
                ...
                if (FAILED(hr)) {
                    WWDEBUG_SAY(
                        ("WOLERROR: RequestSquadInfo() HRESULT = %s\n", GetChatErrorString(hr)));
                    break;
                }
        ...
        if (!mLadderRequests.empty() && mLadderPending == 0) {
            ...
            if (mLadderServer.IsValid() && product.IsValid()) {
                ...
                if (firstRequest[1] == L'T') {
                    HRESULT hr = mNetUtil->RequestLadderList(hostAddr, port, keys,
                                                             (sku | LadderType_Team), -1, 0, 0);
    ...
    bool Session::RequestBuddyList(void)
    {
        ...
        if (FAILED(hr)) {
            WWDEBUG_SAY(
                ("WOLERROR: RequestBuddyList() failed HRESULT = %s\n", GetChatErrorString(hr)));
        }
    ...
    bool Session::AddBuddy(const wchar_t* buddyName)
    {
        ...
        if (FAILED(hr)) {
            WWDEBUG_SAY(
                ("WOLERROR: RequestBuddyAdd() failed HRESULT = %s\n", GetChatErrorString(hr)));
        }
    ...
    bool Session::RemoveBuddy(const wchar_t* buddyName)
    {
        ...
        if (FAILED(hr)) {
            WWDEBUG_SAY(
                ("WOLERROR: RequestBuddyDelete() failed HRESULT = %s\n", GetChatErrorString(hr)));
        }
    ...
    bool Session::SetBadLanguageFilter(bool enabled)
    {
        ...
        if (FAILED(hr)) {
            WWDEBUG_SAY(
                ("WOLERROR: SetLangFilter() failed HRESULT = %s\n", GetChatErrorString(hr)));
        }
        ...
        if (address) {
            ...
            while (iter != mPingRequests.end()) {
                if (stricmp(iter->GetHostAddress(), address) == 0) {
                    //				WWDEBUG_SAY(("WOLWARNING: Ping request already
                    //pending %s\n", address));
                    return;
                }
        ...
        if (mPingEnable && (mPingRequests.size() > mPingsPending)) {
            for (unsigned int index = 0; index < mPingRequests.size(); index++) {
                ...
                if (ping->GetHandle() == -1) {
                    ...
                    HRESULT hr
                        = mNetUtil->RequestPing(ping->GetHostAddress(), ping->GetTime(), &handle);
    ...
    ChatAdvisement::ChatAdvisement(const CComPtr<WOL::IChat>& chat,
                                   const CComPtr<WOL::IChatEvent>& sink)
        : mChat(chat),
          mChatCookie(0)
    {
