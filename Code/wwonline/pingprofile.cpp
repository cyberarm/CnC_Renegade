...
int EncodePingProfile(const PingProfile& pings, char* buffer)
{
    ...
    char temp[18];
    int count = sprintf(temp, "%02X%02X%02X%02X%02X%02X%02X%02X", pings.Pings[0], pings.Pings[1],
                        pings.Pings[2], pings.Pings[3], pings.Pings[4], pings.Pings[5],
                        pings.Pings[6], pings.Pings[7]);
...
void DecodePingProfile(const char* buffer, PingProfile& pings)
{
    if (buffer) {
        ...
        sscanf(buffer, "%02X%02X%02X%02X%02X%02X%02X%02X", &ping[0], &ping[1], &ping[2], &ping[3],
               &ping[4], &ping[5], &ping[6], &ping[7]);
